...
HI_S32 Disp_ISR_CheckDelayTimeMS(HI_DRV_DISPLAY_E enDisp, HI_U32 u32ISRDelayTimeMS)
{
    ...
    {
        DISP_ERROR("No support enDisp = %d > disp1 %d", enDisp);
        return HI_FAILURE;
    }
    ...
    switch(eType)
    {
        ...
        case HI_DRV_DISP_C_DHD0_WBC:
         ...
         case HI_DRV_DISP_C_SMMU:
            ...
            break;
         case HI_DRV_DISP_C_RETURN_FRAME_ISR:
             {
                 pfOpt->PF_SetIntEnable(DISP_INTERRUPT_RETURN_FRAME,  bEnable);
             }
             break;
         case HI_DRV_DISP_C_FRAME_FINISH:
              {
                  pfOpt->PF_SetVPSSIntEnable(DISP_INTERRUPT_VPSS_FRMAE_FINISH,  bEnable);
              }
            break;
        case HI_DRV_DISP_C_FRAME_TIMEOUT0:
              {
                  pfOpt->PF_SetVPSSIntEnable(DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT0,  bEnable);
              }
            break;
        case HI_DRV_DISP_C_FRAME_TIMEOUT1:
              {
                  pfOpt->PF_SetVPSSIntEnable(DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT1,  bEnable);
              }
            break;
        case HI_DRV_DISP_C_FRAME_RXERR:
              {
                  pfOpt->PF_SetVPSSIntEnable(DISP_INTERRUPT_VPSS_FRMAE_RXERR,  bEnable);
              }
            break;
        default :
            break;
...
static HI_U32 s_DispIntTable[HI_DRV_DISPLAY_BUTT][HI_DRV_DISP_C_TYPE_BUTT] =
{
    ...
    {
        0, DISP_INTERRUPT_D0_0_PERCENT, DISP_INTERRUPT_D0_90_PERCENT, DISP_INTERRUPT_D0_100_PERCENT, 0,
        0, 0, 0, 0, DISP_INTERRUPT_RETURN_FRAME,
        DISP_INTERRUPT_VPSS_FRMAE_FINISH, DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT0, DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT1, DISP_INTERRUPT_VPSS_FRMAE_RXERR
    },
    {
        0, DISP_INTERRUPT_D1_0_PERCENT, DISP_INTERRUPT_D1_90_PERCENT, DISP_INTERRUPT_D1_100_PERCENT, DISP_INTERRUPT_WBCDHD_PARTFNI,
        DISP_INTERRUPT_WBC_VP, 0, 0, 0, DISP_INTERRUPT_RETURN_FRAME,
        DISP_INTERRUPT_VPSS_FRMAE_FINISH, DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT0, DISP_INTERRUPT_VPSS_FRMAE_TIMEOUT1, DISP_INTERRUPT_VPSS_FRMAE_RXERR
    },
    {0, 0, 0, 0, 0, 0, 0},
};
...
#if 0
    if (eIntType == HI_DRV_DISP_C_FRAME_FINISH)
    {
        DISP_ERROR(" int bitmap %#llu\n", u64IsrBitMap);
    }
#endif
#if 1
...
HI_S32 DISP_ISR_Main(HI_S32 irq, HI_VOID *dev_id)
{
    ...
    DispCheckNullPointer( pfOpt->PF_GetMaskedIntState1);
    DispCheckNullPointer( pfOpt->PF_CleanIntState1);
    DispCheckNullPointer( pfOpt->PF_GetMaskedVPSSIntState);
    DispCheckNullPointer( pfOpt->PF_CleanVPSSIntState);
    ...
    pfOpt->PF_GetMaskedVPSSIntState(&u32IntStateVPSS);

    // s2 clear interrupt state
    pfOpt->PF_CleanIntState(u32IntState);
    pfOpt->PF_CleanIntState1(u32IntState1);
    if (0x0 != u32IntStateVPSS)
    {
        //DISP_ERROR("VPSS int %#x\n", u32IntStateVPSS);
        pfOpt->PF_CleanVPSSIntState(u32IntStateVPSS);
    }
    ...
    for (i = (HI_S32) HI_DRV_DISPLAY_1; i >= 0; i--)
    {
        ...
        for (eIntType = HI_DRV_DISP_C_INTPOS_0_PERCENT; eIntType < HI_DRV_DISP_C_FRAME_FINISH; eIntType++)
        {
            HI_BOOL bBtm;
            HI_U32 vcnt;

            if (0 == (s_DispIntTable[enDisp][eIntType] & u32IntState))
            {
                continue;
            }
    ...
    pstDisp = &s_DispISRMngr.stDispChn[HI_DRV_DISPLAY_1];
    for (eIntType = HI_DRV_DISP_C_FRAME_FINISH; eIntType < HI_DRV_DISP_C_TYPE_BUTT; eIntType++)
    {
        if (0 == (s_DispIntTable[HI_DRV_DISPLAY_1][eIntType] & u32IntStateVPSS))
        {
            continue;
        }

        DISP_ISR_Execute_ByPriorty(pstDisp, eIntType);
        u32IntStateVPSS = u32IntStateVPSS & (~(HI_U32)s_DispIntTable[HI_DRV_DISPLAY_1][eIntType]);
    }

    if (u32IntState || u32IntStateVPSS)
    {
        DISP_FATAL("Unespexted interrup 0x%x 0x%x happened!\n", u32IntState, u32IntStateVPSS);
    }
