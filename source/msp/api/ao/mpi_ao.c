...
HI_S32 HI_MPI_AO_Track_Create(HI_UNF_SND_E enSound, const HI_UNF_AUDIOTRACK_ATTR_S* pstAttr, HI_HANDLE* phTrack)
{
    ...
    HI_BOOL bSupport = HI_FALSE;
#endif
    HI_HANDLE hTrack = HI_NULL;
    AO_Track_Create_Param_S stTrackParam = {0};
...
HI_S32   HI_MPI_AO_Track_SetMute(HI_HANDLE hTrack, HI_BOOL bMute)
{
    ...
    if (HI_TRUE == RENDER_CheckTrack(hTrack))
    {
        ...
        {
            return RENDER_SetTrackMute(hTrack, bMute);
        }
    }
#endif

    stMute.hTrack = hSrcTrack;
    stMute.bMute  = bMute;
...
HI_S32   HI_MPI_AO_Track_GetMute(HI_HANDLE hTrack, HI_BOOL* pbMute)
{
    ...
    if (HI_TRUE == RENDER_CheckTrack(hTrack))
    {
        ...
        if (HI_TRUE != bAttached)
        {
            return RENDER_GetTrackMute(hTrack, pbMute);
        }
    }
#endif
...
HI_S32   HI_MPI_AO_Track_SetChannelMode(HI_HANDLE hTrack, HI_UNF_TRACK_MODE_E enMode)
{
    ...
    if (HI_TRUE == RENDER_CheckTrack(hTrack))
    {
        return RENDER_SetTrackChannelMode(hTrack, enMode);
    }
#endif
...
HI_S32   HI_MPI_AO_Track_GetChannelMode(HI_HANDLE hTrack, HI_UNF_TRACK_MODE_E* penMode)
{
    ...
    if (HI_TRUE == RENDER_CheckTrack(hTrack))
    {
        return RENDER_GetTrackChannelMode(hTrack, penMode);
    }
#endif
...
HI_S32 HI_MPI_AO_Track_SetAdBalance(const HI_HANDLE hTrack, HI_S16 s16AdBalance)
{
    ...
    if (HI_NULL == g_AD_SlaveTrack)
    {
        ...
        if (s32Ret != HI_SUCCESS)
        {
            HI_ERR_AO("Set AD Enable failed\n");
            return s32Ret;
        }
    }
...
HI_S32 HI_MPI_AO_SND_SetAtmosLockEnable(HI_UNF_SND_E enSound, HI_BOOL bAtmosLockEnable)
{
#ifdef HI_SND_MS12_SUPPORT
    HI_S32 s32Ret;
    HI_BOOL bSupport;
    ...
    if (HI_FALSE == bSupport)
    {
        HI_WARN_AO("Don't support AtmosLock function\n", s32Ret);
        return HI_SUCCESS;
    }

    return RENDER_SetAtmosLockEnable(bAtmosLockEnable);
#else
    return HI_ERR_AO_NOTSUPPORT;
#endif
}

HI_S32 HI_MPI_AO_SND_GetAtmosLockEnable(HI_UNF_SND_E enSound, HI_BOOL* pbAtmosLockEnable)
{
#ifdef HI_SND_MS12_SUPPORT
    HI_S32 s32Ret;
    HI_BOOL bSupport;

    CHECK_AO_NULL_PTR(pbAtmosLockEnable);
    ...
    if (HI_FALSE == bSupport)
    {
        *pbAtmosLockEnable = HI_FALSE;
        HI_WARN_AO("Don't support AtmosLock function\n", s32Ret);
        return HI_SUCCESS;
    }

    return RENDER_GetAtmosLockEnable(pbAtmosLockEnable);
#else
    return HI_ERR_AO_NOTSUPPORT;
#endif
}
