...
{
    HI_S32 s32Ret = HI_SUCCESS;
    GFX2D_WAITALLDONE_CMD_S stCmd = {HI_GFX2D_DEV_ID_0, 0};
                            ...
                            const HI_U32 u32Timeout)
{
    ...
    {
        pstDst->ExtInfo.AlphaSum[Index] = stCmd.stDst.ExtInfo.AlphaSum[Index];
    }
                             ...
                             HI_GFX2D_SURFACE_S *pstDst)
{
    ...
    {/** 本身意图超过7层叠加也支持, 内部拆分, 但是fence不支持超过7层的 **/
        return HI_ERR_GFX2D_TIMEOUT;
    }

    if (pstDst->s32ReleaseFenceFd < 0)
    {
        return HI_SUCCESS;
    }

    for (Index = 0; Index < pstComposeList->u32ComposeCnt; Index++)
    {
        pstComposeList->pstCompose[Index].stSurface.s32ReleaseFenceFd = dup(pstDst->s32ReleaseFenceFd);
        if (pstComposeList->pstCompose[Index].stSurface.s32ReleaseFenceFd < 0)
        {
            return HI_ERR_GFX2D_SYS;
        }
    }
