...
HI_VOID HAL_HDMI_HardwareStatusGet(struct hiHDMI_HAL_S *pstHdmiHal, HDMI_HARDWARE_STATUS_S* pstHwStatus)
{
    ...
    HDCP14_STAUTS_S      *pstHdcp14Stauts   = HI_NULL;
    HDCP14_RUN_STAT_S    stHdcp14RunStat    = {0};
    HDCP22_RUN_STATUS_S  stHdcp22RunStat    = {0};
    HDCP22_AUTH_STATUS_S *pstHdcp22AuthStat = HI_NULL;

    INTF_NULL_CHK_NO_RET(pstHdmiHal);
    INTF_NULL_CHK_NO_RET(pstHwStatus);

    pstHdcp14Stauts = HDMI_VMALLOC(sizeof(HDCP14_STAUTS_S));
    INTF_NULL_CHK_NO_RET(pstHdcp14Stauts);

    pstHdcp22AuthStat = HDMI_VMALLOC(sizeof(HDCP22_AUTH_STATUS_S));
    if(HI_NULL == pstHdcp22AuthStat)
    {
        if (pstHdcp14Stauts)
        {
            HDMI_VFREE(pstHdcp14Stauts);
        }
        HDMI_ERR("null pointer!.\n");
        return ;
    }

    HDMI_MEMSET(pstHdcp14Stauts, 0, sizeof(HDCP14_STAUTS_S));
    HDMI_MEMSET(&stHdcp14RunStat, 0, sizeof(HDCP14_RUN_STAT_S));
    HDMI_MEMSET(&stHdcp22RunStat, 0, sizeof(HDCP22_RUN_STATUS_S));
    HDMI_MEMSET(pstHdcp22AuthStat, 0, sizeof(HDCP22_AUTH_STATUS_S));

    pstHdcp22AuthStat->u8RecvId[0] = 0;
    pstHdcp22AuthStat->u8RecvId[1] = 0;
    pstHdcp22AuthStat->u8RecvId[2] = 0;
    pstHdcp22AuthStat->u8RecvId[3] = 0;
    pstHdcp22AuthStat->u8RecvId[4] = 0;
    ...
    HAL_HDMI_Hdcp14StatusGet(pstHdmiHal->stHalCtx.u32HdmiID,pstHdcp14Stauts);
    HAL_HDMI_Hdcp22RunStatusGet(pstHdmiHal->stHalCtx.u32HdmiID, &stHdcp22RunStat);
    HAL_HDMI_Hdcp22AuthStatusGet(pstHdmiHal->stHalCtx.u32HdmiID, pstHdcp22AuthStat);
    ...
    if(stHdcp14RunStat.bEnable)
    {
        ...
        pstHwStatus->stHdcpStatus.u8BKSVs[0] = pstHdcp14Stauts->stPartI.au8Bksv[0];
        pstHwStatus->stHdcpStatus.u8BKSVs[1] = pstHdcp14Stauts->stPartI.au8Bksv[1];
        pstHwStatus->stHdcpStatus.u8BKSVs[2] = pstHdcp14Stauts->stPartI.au8Bksv[2];
        pstHwStatus->stHdcpStatus.u8BKSVs[3] = pstHdcp14Stauts->stPartI.au8Bksv[3];
        pstHwStatus->stHdcpStatus.u8BKSVs[4] = pstHdcp14Stauts->stPartI.au8Bksv[4];

        pstHwStatus->stHdcpStatus.u8AKSVs[0] = pstHdcp14Stauts->stPartI.au8Aksv[0];
        pstHwStatus->stHdcpStatus.u8AKSVs[1] = pstHdcp14Stauts->stPartI.au8Aksv[1];
        pstHwStatus->stHdcpStatus.u8AKSVs[2] = pstHdcp14Stauts->stPartI.au8Aksv[2];
        pstHwStatus->stHdcpStatus.u8AKSVs[3] = pstHdcp14Stauts->stPartI.au8Aksv[3];
        pstHwStatus->stHdcpStatus.u8AKSVs[4] = pstHdcp14Stauts->stPartI.au8Aksv[4];

        pstHwStatus->stHdcpStatus.bRepeaterOn = pstHdcp14Stauts->stPartI.unBCaps.u8.bzRepeater;

    }

    if(stHdcp22RunStat.bEnable)
    {
        pstHwStatus->stHdcpStatus.u8BKSVs[0] = pstHdcp22AuthStat->u8RecvId[0];
        pstHwStatus->stHdcpStatus.u8BKSVs[1] = pstHdcp22AuthStat->u8RecvId[1];
        pstHwStatus->stHdcpStatus.u8BKSVs[2] = pstHdcp22AuthStat->u8RecvId[2];
        pstHwStatus->stHdcpStatus.u8BKSVs[3] = pstHdcp22AuthStat->u8RecvId[3];
        pstHwStatus->stHdcpStatus.u8BKSVs[4] = pstHdcp22AuthStat->u8RecvId[4];
        ...
        pstHwStatus->stHdcpStatus.bRepeaterOn = pstHdcp22AuthStat->stRxCaps.bRepeater;
    }
    ...
    if (pstHdcp22AuthStat)
    {
        HDMI_VFREE(pstHdcp22AuthStat);
    }
...
HI_VOID HAL_HDMI_CecMsgClear(struct hiHDMI_HAL_S *pstHdmiHal)
{
    ...
    HAL_HDMI_CecClearPool(pstHdmiHal->stHalCtx.u32HdmiID);
...
#endif
    HAL_HDMI_PhyDeInit();
    DRV_HDMI_ProdCrgAllResetSet(HI_FALSE);
