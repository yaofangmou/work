...
#define DRV_MAILBOX_WAIT(timeout, func) \
    do { \
        ...
        if ((timeout) == 0) return HI_ERR_MAILBOX_TIMEOUT; \
    } while(0)


#define MAILBOX_LOCK() \
            do { \
                if (mutex_lock_interruptible(&s_mailbox_lock)) \
                { \
...
#define MAILBOX_UNLOCK() \
            do { \
                mutex_unlock(&s_mailbox_lock); \
            } while (0)


static DEFINE_MUTEX(s_mailbox_lock);
static DRV_MAILBOX_SESSION_S s_mailbox_session;
static DRV_MAILBOX_RECEIVE_BUFFER_S *pReceiveBuffer = HI_NULL;

static HI_S32 send_msg(HI_U32 receiver, HI_U8 *message, HI_U32 message_len, HI_U64 *timeout, DRV_MAILBOX_SEND_S *func)
{
    HI_S32 i;
    HI_U32 block_size     = 0;
...
static HI_S32 send_msg_to_scpu(HI_U8 *message, HI_U32 message_len, HI_U64 *timeout)
{
...
static HI_S32 send_msg_to_smcu(HI_U32 receiver, HI_VOID *message, HI_U32 message_len, HI_U64 *timeout)
{
    HI_S32 ret = HI_FAILURE;
    DRV_MAILBOX_SEND_S param;
...
static HI_S32 send_msg_to_receiver(HI_U32 receiver, HI_U8 *message, HI_U32 message_len, HI_U64 *timeout)
{
    HI_S32 ret = HI_FAILURE;
    ...
    if (MID_SCPU_COMMON == receiver)
    {
        ...
        if (HI_SUCCESS != ret)
        {
          ...
          || (MID_SMCU_RUNTIME_CHECK_FS == receiver) || (MID_SMCU_DOLBY_CAPABLE == receiver))
    {
...
static HI_S32 receive_msg_by_poll(HI_U32 sender, HI_VOID *output, HI_U32 *output_len, HI_U64 *timeout, DRV_MAILBOX_RECEIVE_S *func)
{
...
static HI_S32 receive_msg_by_interrupt(HI_U64 *timeout)
{
    HI_S64 rest = 0;
    MAILBOX_FUNC_ENTER();

    rest = wait_event_timeout(pReceiveBuffer->receive_queue, (pReceiveBuffer->status == MAILBOX_BUFFER_STATUS_FINISH), usecs_to_jiffies(*timeout));
    if (rest <= 0)
    {
        MAILBOX_PrintErrorCode(rest);
        return HI_ERR_MAILBOX_TIMEOUT;
    }
    else
    {
...
static HI_S32 recieve_msg_from_scpu(HI_U8 *output, HI_U32 *output_len, HI_U64 *timeout)
{
    HI_S32 ret = HI_FAILURE;
...
static HI_S32 receive_msg_from_smcu(HI_U32 receiver, HI_U8 *output, HI_U32 *output_len, HI_U64 *timeout)
{
    HI_S32 ret = HI_FAILURE;
...
static HI_S32 get_response_from_receiver(HI_U32 receiver, HI_VOID *output, HI_U32 *output_len, HI_U64 *timeout)
{
    HI_S32 ret = HI_FAILURE;
    ...
    if (MID_SCPU_COMMON == receiver)
    {
        ret = recieve_msg_from_scpu(output, output_len, timeout);
        if (HI_SUCCESS != ret)
        {
            MAILBOX_PrintErrorFunc(recieve_msg_from_scpu, ret);
            return ret;
        }
    }
    else
...
)
{
    ...
    block_time = (flag == DRV_MAILBOX_FLAG_BLOCK) ? DRV_MAILBOX_INFINITE_TIME : ((HI_U64)param * 1000);
...
irqreturn_t mailbox_spcu_irq(int irq,void *dev_id)
{
    ...
    if (p == HI_NULL)
    {
        MAILBOX_PrintErrorCode(MAILBOX_BUFFER_STATUS_INVALID_PARAM);
        goto RET;
    }
...
HI_S32 DRV_MAILBOX_Init(HI_VOID)
{
    ...
    pReceiveBuffer = (DRV_MAILBOX_RECEIVE_BUFFER_S *) HI_KMALLOC (HI_ID_MAILBOX, sizeof (DRV_MAILBOX_RECEIVE_BUFFER_S), GFP_KERNEL);
    ...
    {
        HAL_MAILBOX_DeInit();
        HI_KFREE (HI_ID_MAILBOX, pReceiveBuffer);
