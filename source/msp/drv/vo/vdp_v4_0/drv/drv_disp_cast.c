                       ...
                       DISP_CAST_S** cast_ptr)
{
    ...
    // get hal operation
    nRet = DISP_HAL_GetOperation(&pstFunction);
    if (nRet)
    {
        DISP_ERROR("Cast get hal operation failed!\n");
        goto _ERR_MALLOC;
    }
    memcpy(&pstCast->stIntfOpt, pstFunction, sizeof(DISP_INTF_OPERATION_S));
...
static HI_DRV_VIDEO_FRAME_S g_stCastFrame;
...
static HI_S32 Cast_PushFrameToBackMod(DISP_CAST_S *pstCast)
{
    ...
    if (pstCast->attach_pairs[0].pfnQueueFrm)
    {
        while(1)
        {
            ...
            Cast_ERROR_DEAL(BP_GetFullBuf(&pstCast->stBP, &u32BufId), 0);
            Cast_ERROR_DEAL(BP_DelFullBuf(&pstCast->stBP, u32BufId), 1);
            Cast_ERROR_DEAL(BP_GetFrame(&pstCast->stBP, u32BufId, &g_stCastFrame), 1);

            venc_queue_pfn = pstCast->attach_pairs[0].pfnQueueFrm;

            nRet = venc_queue_pfn(pstCast->attach_pairs[0].hSink, &g_stCastFrame);
            if (nRet == HI_SUCCESS)
            {
                Cast_SendTimeStamps(pstCast, &g_stCastFrame, EVENT_CAST_FRM_OUT);
                pstCast->u32CastAcquireOkCnt ++;
            }
            else
            {
                DISP_CastReleaseFrame(pstCast, &g_stCastFrame);
                break;
            }
        }
    }
...
HI_DRV_VIDEO_FRAME_S g_stCurFrame;
...
HI_VOID DISP_CastCB_GenarateFrame(HI_VOID *cast_ptr, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    if (pstInfo->eEventType == HI_DRV_DISP_C_VT_INT)
    {
        ...
        nRet = Cast_GetFrame(pstCast, &u32BufId, 0, &g_stCurFrame);
        if (nRet)
        {
        ...
        while(Cast_CheckOutputSizeChange(pstCast, &g_stCurFrame))
        {
        ...
        (HI_VOID)DispCastSetFrameInfo(pstCast, &g_stCurFrame);
        ...
        if ((!nRet) && (!BP_GetFrame(&pstCast->stBP, u32BufId, &g_stCurFrame)))
        {
                Cast_SendTimeStamps(pstCast, &g_stCurFrame, EVENT_CAST_FRM_BEGIN);
        }
...
HI_S32 DISP_Acquire_Snapshot(HI_DRV_DISPLAY_E enDisp, HI_VOID **snapshotHandle, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    DISP_INTF_OPERATION_S *pstFunc = HI_NULL;
    DISP_WBC_E eWBC;
    ...
    *snapshotHandle  = 0;
    ...
    ret = DISP_HAL_GetOperation(&pstFunc);
    if (ret)
    {
        goto __ERR_EXIT__;
    }

    ret = pstFunc->PF_AcquireWbcByChn(enDisp, &eWBC);
    if (ret) {
    ...
    // config pixformat
    pstFunc->PF_SetWbcPixFmt(eWBC, HI_DRV_PIX_FMT_NV21);

    /*FIXME:  ourrect  why be stInfo.stFmtResolution? error?*/

    (HI_VOID)pstFunc->PF_GetPixelRepeatTimesAccordingFmt(HI_DRV_DISPLAY_1,
                                              stInfo.eFmt,
                                              &u32PixelRepeatTimes);
    ...
    pstFunc->PF_SetWbcIORect(eWBC, &stInfo, &stInfo.stPixelFmtResolution, &stOutputSize);

    pstFunc->PF_SetWbc3DInfo(eWBC, &stInfo, &stInfo.stPixelFmtResolution, &stOutputSize);

    // config csc
    pstFunc->PF_SetWbcColorSpace(eWBC, stInfo.eColorSpace, HI_DRV_CS_BT709_YUV_LIMITED);

    // config addr
    pstFunc->PF_SetWbcAddr(eWBC, &(pstVideoFrame->stBufAddr[0]));

#ifdef  CFG_VDP_MMU_SUPPORT
    pstFunc->PF_SetWbcSmmuBypass(eWBC, HI_FALSE);
#else
    pstFunc->PF_SetWbcSmmuBypass(eWBC, HI_TRUE);
#endif

    // set enable FIXME!!!
    pstFunc->PF_SetWbcEnable(eWBC, HI_TRUE);
    pstFunc->PF_UpdateWbc(eWBC);
