...
extern HI_S32 g_CfgErrRatio;
extern VDEC_KPI_STATINFO_S g_KpiInfo;
...
static HI_VOID ReadKpiInfo(struct seq_file *p)
{
    PROC_PRINT(p, "--------------------------KPI Information------------------------\n"
               "Creat                              : %d ms\n"
               "Start                              : %d ms\n"
               "ResetVdec                          : %d ms\n"
               "Stop                               : %d ms\n"
               ...
               g_KpiInfo.CreateDecoder,
               g_KpiInfo.StartDecoder,

               g_KpiInfo.CreatVpss,
               g_KpiInfo.StartVpss,
               g_KpiInfo.ResetVpss,
               g_KpiInfo.StopVpss,
               g_KpiInfo.DestroyVpss,

               g_KpiInfo.ReceiveFirstFrameTime - g_KpiInfo.ChanStartTime);
    return;
}
...
static HI_S32 VDEC_DRV_CtrlReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "echo  errratio     value           -- set default error ratio\n");
    PROC_PRINT(p, "===================================================================\n");
...
static HI_VOID SetErrRatio(HI_CHAR *pdat2)
{
    HI_S32 ErrRatio = -1;

    if (pdat2[0] != HI_NULL)
    {
        str2val(pdat2, &ErrRatio);

        if (ErrRatio < 0 || ErrRatio > 100)
        {
            HI_ERR_VDEC("[%s]ERROR: ErrRatio %d out of range [0, 100].\n", __func__, ErrRatio);
            return;
        }

        g_CfgErrRatio = ErrRatio;
    }
                                     ...
                                     const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (0 == strncmp(dat1, "help", 5))
    {
        ...
        HI_DRV_PROC_EchoHelper("echo  errratio       value             -- set default error ratio\n");
    ...
    {
        strncpy(TmpString, "maskdfs", 8);
        SetMaskBit(dat2, TmpString, 0);
    }
    else if (strncmp(dat1, "errratio", 7) == 0)
    {
        strncpy(TmpString, "errratio", 8);
        SetErrRatio(dat2);
    }
    else if (0 == strncmp(dat1, "bypass", 7))
    {
        strncpy(TmpString, "bypass", 7);
        SetMaskBit(dat2, TmpString, 1);
    }
                                ...
                                VDEC_CHANNEL_S *pstChan)
{
    ...
    PROC_PRINT(p,
               "First Frame INFO\n"
               "Total                               : %d ms\n"
               "Put Stream    - Chan Start          : %d ms\n"
               "Get Stream    - Put Stream          : %d ms\n"
               "Decoder Start - Get Stream          : %d ms\n"
               "Decoder Over  - Decoder Start       : %d ms\n"
               "Read Image    - Decoder Over        : %d ms\n"
               "Vpsss Reort   - Read Image          : %d ms\n"
               "Receive Frame - Vpsss Reort         : %d ms\n",
               pstStatInfo->u32ReceiveFrameTime - pstStatInfo->u32ChanStartTime,
               pstStatInfo->u32PutStreamTime - pstStatInfo->u32ChanStartTime,
               pstStatInfo->u32DecoderReadStreamTime - pstStatInfo->u32PutStreamTime,
               pstStatInfo->u32DecoderStartVDMTime - pstStatInfo->u32DecoderReadStreamTime,
               pstStatInfo->u32DecoderReportTime - pstStatInfo->u32DecoderStartVDMTime,
               pstStatInfo->u32VpssReadImageTime - pstStatInfo->u32DecoderReportTime,
               pstStatInfo->u32VpssReportTime - pstStatInfo->u32VpssReadImageTime,
               pstStatInfo->u32ReceiveFrameTime - pstStatInfo->u32VpssReportTime);
    return;
}
...
static HI_S32 VDEC_DRV_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (pstChan)
    {
        ...
        PROC_PRINT(p,   "--------------------------EOS Information-----------------------------\n"
                        "Set EOS Flag                        : %d\n"
                        "Dec Report Status                   : %d\n"
                        "Report EOS                          : %d\n",

                        pstChan->bReceiveEosFlag,
                        pstChan->eEofReportStatus,
                        pstChan->bReportEos
                        );
