...
extern HI_VOID JPEG_DEC_SaveBmp(JPEG_HDEC_HANDLE_S_PTR pJpegHandle);
/***************************** API realization ****************************************************/
...
//   HI_U32 Row = 0;
    ...
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("SrcSurface.u32YWidth      = %d\n",SrcSurface.u32YWidth);
        HI_GFX_Print("SrcSurface.u32YHeight     = %d\n",SrcSurface.u32YHeight);
        HI_GFX_Print("SrcSurface.u32YStride     = %d\n",SrcSurface.u32YStride);
        HI_GFX_Print("SrcSurface.u32CbCrStride  = %d\n",SrcSurface.u32CbCrStride);
        HI_GFX_Print("============================================================================\n");
    }
    ...
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("DstSurface.u32Width  = %d\n",DstSurface.u32Width);
        HI_GFX_Print("DstSurface.u32Height = %d\n",DstSurface.u32Height);
        HI_GFX_Print("DstSurface.u32Stride = %d\n",DstSurface.u32Stride);
        HI_GFX_Print("SrcRect.s32Xpos      = %d\n",SrcRect.s32Xpos);
        HI_GFX_Print("SrcRect.s32Ypos      = %d\n",SrcRect.s32Ypos);
        HI_GFX_Print("SrcRect.u32Width     = %d\n",SrcRect.u32Width);
        HI_GFX_Print("SrcRect.u32Height    = %d\n",SrcRect.u32Height);
        HI_GFX_Print("DstRect.s32Xpos      = %d\n",DstRect.s32Xpos);
        HI_GFX_Print("DstRect.s32Ypos      = %d\n",DstRect.s32Ypos);
        HI_GFX_Print("DstRect.u32Width     = %d\n",DstRect.u32Width);
        HI_GFX_Print("DstRect.u32Height    = %d\n",DstRect.u32Height);
        HI_GFX_Print("============================================================================\n");
    }
    ...
    while (Row < pJpegHandle->stOutSurface.stCropRect.h)
    {
        ...
        if ((s32Handle = JPEG_HDEC_CSC_BeginJob(pJpegHandle->CscDev)) != HI_ERR_TDE_INVALID_HANDLE)
        {
          ...
          if (HI_SUCCESS != Ret)
          {
              HI_GFX_Print("==== JPEG_HDEC_CSC_MbBlit Failure,s32Ret = 0x%x!\n",Ret);
              return HI_FAILURE;
          }
          ...
          if (HI_SUCCESS != Ret)
          {
              HI_GFX_Print("==== JPEG_HDEC_CSC_EndJob Failure,s32Ret = 0x%x!\n",Ret);
              return HI_FAILURE;
          }
        }
#if 0
    ...
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("tde mb blit finish\n");
        HI_GFX_Print("============================================================================\n");
    }
...
static inline HI_VOID JPEG_HDEC_NeedCpyDataToUserBuffer(HI_ULONG HdecHandle, HI_U32 MaxLines, HI_U32 *pCurScanlines)
{
    HI_U32 Cnt = 0;
    JPEG_HDEC_HANDLE_S_PTR pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)HdecHandle;
    /** inline function,should not check null pointer **/
...
static HI_U32 JPEG_HDEC_OutUserBuf(HI_ULONG HdecHandle, HI_CHAR* ScanlineBuf, HI_U32 MaxLines, HI_U32 *pCurScanlines)
{
    ...
    JPEG_HDEC_NeedCpyDataToUserBuffer(HdecHandle,MaxLines,pCurScanlines);
