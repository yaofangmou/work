    ...
    for (i = (HI_S32) HI_DRV_DISPLAY_1; i >= 0; i--)
    {
        ...
        for (eIntType = HI_DRV_DISP_C_INTPOS_0_PERCENT; eIntType < HI_DRV_DISP_C_TYPE_BUTT; eIntType++)
        {
            ...
            if ( s_DispIntTable[enDisp][HI_DRV_DISP_C_INTPOS_0_PERCENT] & u32IntState )
            {
                ...
                if ((HI_DRV_DISPLAY_1 == enDisp) && (pstDisp->stISRDebug.u32ISRDelayTimeMS > 0 ))
                {
                    if ((u32PeriodTime > (u32CircleTime + pstDisp->stISRDebug.u32ISRDelayTimeMS))
                        || ((u32FinishSysTime - u32StartSysTime) > pstDisp->stISRDebug.u32ISRDelayTimeMS))

                    {
                        DISP_WARN("set-%d-isr,delay:%dms, left -%dms,use-%dms,line-(L-%d,C-%d)\n", pstDisp->stISRDebug.u32ISRDelayTimeMS, (u32PeriodTime - u32CircleTime), u32LeftTime,(u32FinishSysTime - u32StartSysTime), pstDisp->stCBInfo.stDispInfo.u32LastVline, pstDisp->stCBInfo.stDispInfo.u32Vline);
                    }
                }
