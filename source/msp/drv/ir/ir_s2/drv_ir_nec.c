...
enum IR_MATCH_RESULT nec_Check_Frame(struct ir_buffer *buf, struct ir_protocol *ip,struct key_attr *key, int idx)
{
    ...
    if (key_match_phase(key, &ip->attr.header))
    {
        ...
        else
        {
            ...
            hiir_debug("(%d)[0x%.08llx,0x%.08llx](%s) exists\n", idx,nec_last_key[idx].lower,
                nec_last_key[idx].upper, nec_last_key[idx].key_stat == KEY_STAT_DOWN?"DWON":
                    nec_last_key[idx].key_stat == KEY_STAT_UP?"UP":"HOLD");
            return IR_MATCH_NOT_MATCH;
        }
    }
    else
    ...
    do
    {
        ...
        else
        {
            hiir_debug("repeat parsed ,last_key[l,u,s] [0x%x,0x%x,%d],"
                       ...
                       idx);
            redundant_frame_flag++;
            if (key_stat_time(fail, last_key, ir->key_repeat_event, idx) && redundant_frame_flag > 1)
            {
