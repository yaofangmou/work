...
HI_VOID DmxHalDvbPortSetClkInPol(HI_U32 RawPortId, HI_BOOL Pol)
{
    ...
    || defined(CHIP_TYPE_hi3716mv420)
        ...
        default:
         ...
         case 1 :
            /*pvr_tsi2_pctrl acturally points to pin_tsi0,the name tsi2 because logic code use this name,
            if PeriCtrl.bits.pvr_tsi1_sel is 1, pvr_tsi1_pctrl points pin_tsi3, else pvr_tsi1_pctrl points internal qam */
            PeriCrg63.bits.pvr_tsi2_pctrl = Pol;
            break;
         ...
         case 4 :
         {
            U_PERI_CTRL PeriCtrl;
            PeriCtrl.u32 = g_pstRegPeri->PERI_CTRL.u32;
            if (0x1 == PeriCtrl.bits.pvr_tsi1_sel)
            {
                PeriCrg63.bits.pvr_tsi1_pctrl = Pol;
            }
            break;
...
#endif  /* end of (ifdef HI_DMX_REC_INDEX_SUPPORT) */
...
* Others:
***********************************************************************************/
HI_VOID DmxHalSetIPBackPressure(Dmx_Set_S *DmxSet, HI_U32 DmxId, HI_BOOL bPress)
{
    ...
    if (HI_TRUE == bPress)
    {
        switch_block.bits.dav_buff_cfg |= (1U << DmxId);
    }
    else
    {
        switch_block.bits.dav_buff_cfg &= ~(1U << DmxId);
...
HI_VOID DmxHalCFGD2BBP(HI_VOID)
{
    ...
    unCfgD2B_BP.bits.fp2dmx_aful_cfg = 2;  /* play channel */
    unCfgD2B_BP.bits.rp2dmx_aful_cfg = 2;  /* record channel */
...
/* TSO support or not */
   ...
   || defined(CHIP_TYPE_hi3716mv430)
    ...
    /* init the lowpower configure for 98mv310 & 16mv430 */
    DmxSet = GetDefDmxSet();
    if (unlikely(!DmxSet))
    {
...
#endif /* end of HI_DMX_TSO_SUPPORT */
...
HI_VOID DmxHalLowPowerInit(Dmx_Set_S *DmxSet)
{
    ...
    PvrClkEn.bits.mdsc_core_clk_gt_en = 0x0; /* close the dynamic clock control, especially for 16mv430 nagra  */
#else
...
HI_VOID DmxHalLowPowerInit(Dmx_Set_S *DmxSet)
{
    ...
    PvrClkEn.bits.pvr_csa3_clken = 0x1;     /* open csa3 clock */
    PvrClkEn.bits.pvr_debug_clken = 0x1;    /* open debug clock */
    PvrClkEn.bits.pvr_davcnt_clken = 0x1;   /* open davcnt clock */
    PvrClkEn.bits.pvr_ip0_6_clken = 0x1;    /* open the ramport clock */
    PvrClkEn.bits.pvr_inf_clken = 0x1;      /* open the dvb port clock */
    PvrClkEn.bits.pvr_scd_clken = 0x1;      /* open the scd clock */
#ifdef HI_ADVCA_TYPE_NAGRA
    PvrClkEn.bits.mdsc_core_clk_gt_en = 0x0; /* close the dynamic clock control, especially for 16mv430 nagra  */
#else
    PvrClkEn.bits.mdsc_core_clk_gt_en = 0x1; /* open the dynamic clock control  */
...
HI_VOID DmxHalEnableGblMmu(Dmx_Mmu_S *DmxMmu)
{
    ...
    /*disable the mmu interrupt for 98cv200 keyladder */
