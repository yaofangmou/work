...
#define PROC_CMD_SET_Thread_TimeOut (0x906)
#define PROC_CMD_SET_VDH_DIST_Timeout (0x907)
...
IMAGE_LIST_S            *g_pNoOutList = NULL;
#if (1 == DEBUG_SAVE_SUPPORT)
...
static HI_U32            g_ThreadTimeOut = 5;
...
extern UINT32 g_VdhDistOuttime;
...
VDEC_CID_SECURE_E TVP_VDEC_ConvertCmdToSecure(VDEC_CID_E eCmdID)
{
    ...
    switch (eCmdID)
    {
        ...
        case VDEC_CID_RESET_CHAN_WITH_OPTION:
            eResultCmd = VDEC_CID_SEC_RESET_CHAN_WITH_OPTION;
            break;
...
VOID TVP_VDEC_ConvertOptionToSecure(SINT32 ChanID, VDEC_CHAN_OPTION_S *pChanNsOption, VDEC_SECURE_CHAN_OPTION_S *pChanSecOption)
{
    ...
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail.ChanMemVdh), &(pChanNsOption->MemDetail.ChanMemVdh), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail.ChanMemScd), &(pChanNsOption->MemDetail.ChanMemScd), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail.ChanMemCtx), &(pChanNsOption->MemDetail.ChanMemCtx), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail.ChanMemRpu), &(pChanNsOption->MemDetail.ChanMemRpu), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanSecOption->MemDetail.ChanMemEs), &(pChanNsOption->MemDetail.ChanMemEs), sizeof(MEM_DESC_S));
...
VOID TVP_VDEC_ConvertOptionToNS(SINT32 ChanID, VDEC_SECURE_CHAN_OPTION_S *pChanSecOption, VDEC_CHAN_OPTION_S *pChanNsOption)
{
    ...
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail.ChanMemVdh), &(pChanSecOption->MemDetail.ChanMemVdh), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail.ChanMemScd), &(pChanSecOption->MemDetail.ChanMemScd), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail.ChanMemCtx), &(pChanSecOption->MemDetail.ChanMemCtx), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail.ChanMemRpu), &(pChanSecOption->MemDetail.ChanMemRpu), sizeof(MEM_DESC_S));
    OSAL_FP_memcpy(&(pChanNsOption->MemDetail.ChanMemEs), &(pChanSecOption->MemDetail.ChanMemEs), sizeof(MEM_DESC_S));
...
SINT32 TVP_VDEC_FindOccupiedFrame(SINT32 ChanID, VDEC_SPECIAL_FRM_S *pSpecialFrmInfo)
{
    ...
    IMAGE_LIST_S *pStOutputList;
    IMAGE_LIST_S *pStNoOutput;
    SINT32 number = 0;
    SINT32 index = 0;
    ...
    if (NULL != g_pImageQue)
    {
        ...
        // decoded frame not output yet
        if (pSpecialFrmInfo->needRecNoOutput == 1)
        {
            pStOutputList = &(g_pImageQue[ChanID].StOutputImg);
            pStNoOutput = (IMAGE_LIST_S *)(g_pNoOutList + ChanID);
            if (pStOutputList->Head != pStOutputList->Tail)
            {
                number = (pStOutputList->Tail > pStOutputList->Head) ? (pStOutputList->Tail - pStOutputList->Head) : (pStOutputList->Tail + MAX_QUEUE_IMAGE_NUM - pStOutputList->Head);
                for (i = 0; i < number; i++)
                {
                    index = (pStOutputList->Head + i) & (MAX_QUEUE_IMAGE_NUM - 1);
                    if (pStOutputList->StImage[index].top_luma_phy_addr != 0)
                    {
                        pSpecialFrmInfo->specialFrmRec[j++].PhyAddr = pStOutputList->StImage[index].top_luma_phy_addr;

                        OSAL_FP_memcpy(&(pStNoOutput->StImage[pStNoOutput->Tail]), &(pStOutputList->StImage[index]), sizeof(IMAGE));
                        pStNoOutput->Tail = (pStNoOutput->Tail + 1) & (MAX_QUEUE_IMAGE_NUM - 1);
                    }
                    else
                    {
                        dprint(PRN_FATAL, "%d, %d\n", pStOutputList->Tail, pStOutputList->Head);
                    }
                }
            }
        }
...
SINT32 TVP_VDEC_GetImage(SINT32 ChanID, IMAGE *pImage)
{
    ...
    if (1 == g_ChanContext[ChanID].OutputImgEn)
    {
        if ((NULL != g_pImageQue) && (ChanID >= TRUSTED_CHAN_BEGIN) && (ChanID < MAX_CHAN_NUM) && (NULL != pImage))
        {
            ...
            if (pStNoOutput != NULL)
            {
                if (pStNoOutput->Head != pStNoOutput->Tail)
                {
                    pOutFrame = pImage->pOutFrame;
                #ifdef CHIP_98MV200_ULTRAVMX
                    TVP_VDEC_ConvertImageToNs(ChanID, &(pStNoOutput->StImage[pStNoOutput->Head]), pImage);
                #else
                    OSAL_FP_memcpy(pImage, &(pStNoOutput->StImage[pStNoOutput->Head]), sizeof(IMAGE));
                #endif
                    pImage->pOutFrame = pOutFrame;
                    pStNoOutput->Head = (pStNoOutput->Head + 1) % MAX_QUEUE_IMAGE_NUM;
...
SINT32 TVP_VDEC_MiddleWare(VOID *pArgs)
{
    ...
    sleep:
        ...
        OSAL_WaitEvent_s(&g_SecThreadEvent, g_ThreadTimeOut);
    }
...
SINT32 TVP_VDEC_ConfigParam(UINT32 Option, UINT32 Value)
{
    switch (Option)
    {
        case PROC_CMD_SET_PRINT:
            g_SecPrintEnable = Value;
            SecPrint(PRN_ALWS, "Set SecPrintEnable = 0x%x\n", g_SecPrintEnable);
            break;
        case PROC_CMD_SELECT:
            g_SecProcSelect = Value;
            SecPrint(PRN_ALWS, "Set g_SecProcSelect = 0x%x\n", g_SecProcSelect);
            break;
        case PROC_CMD_SET_SEG_BYPASS:
            g_SecSegBypass = (Value == 0) ? 0 : 1;
            SecPrint(PRN_ALWS, "Set SecSegBypass = %d\n", g_SecSegBypass);
            break;
        case PROC_CMD_SET_Thread_TimeOut:
            g_ThreadTimeOut = (Value > 10) ? 10 : Value;
            SecPrint(PRN_ALWS, "Thread time out = %d\n", g_ThreadTimeOut);
            break;
        case PROC_CMD_SET_VDH_DIST_Timeout:
            g_VdhDistOuttime = (Value > 10) ? 10 : Value;
            SecPrint(PRN_ALWS, "VDH Distributor time out = %d\n", g_VdhDistOuttime);
            break;
    ...
    return (HI_TRUE == g_bSecEnvSetUp) ? HI_SUCCESS : HI_FAILURE;
}
...
SINT32 TVP_VDEC_ParamProccess(const SINT8 __user *buffer, size_t count, UINT32 *p_option, UINT32 *p_value)
{
    ...
    else
    {
        ...
        {
            ret = TVP_VDEC_ConfigParam(*p_option, *p_value);
        }
    }
...
static SINT32 TVP_VDEC_ReadProc(struct seq_file *p, VOID *v)
{
    ...
    PROC_PRINT(p, "ThreadTimeOut    :%-7d  | DistributorTimeout  :%d\n",     g_ThreadTimeOut,  g_VdhDistOuttime);
    PROC_PRINT(p, "\n");
    ...
    {
        PROC_PRINT(p, " No secure channel exist.\n");
    }
...
SINT32 TVP_VDEC_TrustDecoderInit(VDEC_OPERATION_S *pArgs)
{
    ...
    g_pNoOutList = (IMAGE_LIST_S *)OSAL_AllocVirMem(sizeof(IMAGE_LIST_S) * MAX_CHAN_NUM);
    if (g_pNoOutList == NULL)
    {
        SecPrint(PRN_FATAL, "Alloc vir Mem for g_pNoOutList failed!\n");
        goto InitWithfree_2;
    }
    OSAL_FP_memset(g_pNoOutList, 0, sizeof(IMAGE_LIST_S) * MAX_CHAN_NUM);
