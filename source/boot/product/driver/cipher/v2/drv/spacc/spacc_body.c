               ...
               unsigned int entry_phy_addr, void *entry_via_addr)
{
...
#ifndef REE_NONSECURE_ENABLE
    {
        ...
        used.u32 = SPACC_READ(NON_SEC_CHN_WHO_USED);
        sec_chn_cfg.u32 = SPACC_READ(SEC_CHN_CFG);
        even_key.u32    = SPACC_READ(SPACC_EVEN_KEY_TABLE_ADDR_SEC_CFG);
        odd_key.u32     = SPACC_READ(SPACC_ODD_KEY_TABLE_ADDR_SEC_CFG);
        ...
        even_key.bits.spacc_even_key_table_addr_sec |= SPACC_CHN_MASK;
        odd_key.bits.spacc_odd_key_table_addr_sec   |= SPACC_CHN_MASK;

        SPACC_WRITE(NON_SEC_CHN_WHO_USED, used.u32);
        SPACC_WRITE(SEC_CHN_CFG, sec_chn_cfg.u32);
        SPACC_WRITE(SPACC_EVEN_KEY_TABLE_ADDR_SEC_CFG, even_key.u32);
        SPACC_WRITE(SPACC_ODD_KEY_TABLE_ADDR_SEC_CFG, odd_key.u32);

        SPACC_DBG("SEC_CHN_CFG[%p]: 0x%x\n", SEC_CHN_CFG, sec_chn_cfg.u32);
        SPACC_DBG("SPACC_EVEN_KEY_TABLE_ADDR_SEC_CFG[0x%x]: 0x%x\n", SPACC_EVEN_KEY_TABLE_ADDR_SEC_CFG, even_key.u32);
        SPACC_DBG("SPACC_ODD_KEY_TABLE_ADDR_SEC_CFG[0x%x]: 0x%x\n", SPACC_ODD_KEY_TABLE_ADDR_SEC_CFG, odd_key.u32);
    }
#endif
    ...
    for(i=CIPHER_PKGxN_CHAN_MIN; i<=CIPHER_PKGxN_CHAN_MAX; i++)
    {
        ...
        SPACC_WRITE(CHANN_SYM_CALC_KEY_FLEX_ADDR_REG(i), i);
    }
...
int spacc_deinit(void)
{
    ...
    for(i=CIPHER_PKGxN_CHAN_MIN; i<=CIPHER_PKGxN_CHAN_MAX; i++)
    {
        CHN_WHO_USED_CLR(used.bits.non_sec_chnn_who_used, i);
    }
...
HI_S32  spacc_symc_set_key_16(u32 chn_num, u32 *key, u32 odd)
{
    HI_S32 ret = HI_FAILURE;
    EXT_CLEAR_CW_KLAD_PARA_S stCfg = {0};
    HI_BOOL bSecure = 0;

    SPACC_DBG("KLAD Load CPU Key, chn_num %d, odd %d\n", chn_num, odd);
    ...
    stCfg.bOdd     = (HI_BOOL)odd;
    stCfg.enSecEn  = (HI_TRUE == bSecure ? KLAD_SEC_ENABLE : KLAD_SEC_DISABLE);
    memcpy(stCfg.u8Key, key, AES_BLOCK_SIZE);

    ret = DRV_KLAD_SetClearKey(&stCfg);
    if (HI_SUCCESS != ret)
    {
        SPACC_DBG("DRV_KLAD_SetClearKey failed, ret = 0x%x\n", ret);
        return ret;
    }
...
int spacc_symc_setkey(unsigned int chn_num, unsigned int *even_key, unsigned int *odd_key, unsigned int klen)
{
    ...
    ret  = spacc_symc_set_key_16(chn_num, &even_key[4], 1);
    ret |= spacc_symc_set_key_16(chn_num, &even_key[0], 0);
