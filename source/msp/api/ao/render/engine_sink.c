...
HI_S32 Sink_GetSyncDelayMs(SOURCE_S* pstSource, HI_U32* pu32Pts, HI_U32* pu32OrgPtsMs)
{
    ...
    if (g_pstSinkManager)
    {
        ...
        *pu32OrgPtsMs = pstSource->u32SinkPts;
        ...
        if (HI_INVALID_PTS == pstSource->u32SinkPts)
        {
            *pu32Pts = pstSource->u32SinkPts;
        }
        else
        {
...
HI_S32 Sink_SetTrackChannelMode(HI_UNF_TRACK_MODE_E enMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (g_pstSinkManager)
    {
        s32Ret = HI_MPI_AO_Track_SetChannelMode(g_pstSinkManager->hSinkTrack, enMode);
        if (HI_SUCCESS != s32Ret)
        {
            HI_LOG_PrintFuncErr(HI_MPI_AO_Track_SetChannelMode, s32Ret);
        }

        HI_WARN_AO("drv Track_SetChannelMode(%d)\n", enMode);
    }

    return s32Ret;
}

HI_S32 Sink_GetTrackChannelMode(HI_UNF_TRACK_MODE_E* penMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (g_pstSinkManager)
    {
        s32Ret = HI_MPI_AO_Track_GetChannelMode(g_pstSinkManager->hSinkTrack, penMode);
        if (HI_SUCCESS != s32Ret)
        {
            HI_LOG_PrintFuncErr(HI_MPI_AO_Track_GetChannelMode, s32Ret);
        }

        HI_WARN_AO("drv Track_GetChannelMode(%d)\n", *penMode);
    }
