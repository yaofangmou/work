...
static HI_U32 resetTime = 3;//LSADC KEY press time for reset
#endif

#if defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3716mv450)
static HI_U32 g_u32GpioPortVal = 0x2a;
#elif  defined(CHIP_TYPE_hi3798mv310) || defined(CHIP_TYPE_hi3798mv200)
static HI_U32 g_u32GpioPortVal = 0x2b;//GPIO5_3
...
#if defined(CHIP_TYPE_hi3716mv430)
    HI_REG_WRITE32((dwPtr + 26), resetTime);
#endif
...
void boot_suspend(void)
{
    ...
    if (KERNEL_RUNNING != data)
    {
        if (MCU_LOAD_CODE != data)
        {
...
#if defined(HI_PM_SUSPEND_FROM_MEMORY) || defined(HI_PM_SUSPEND_WHEN_POWERUP)
#if defined(CHIP_TYPE_hi3798cv200)
            ...
            if (0x80 == data)
#elif  defined(CHIP_TYPE_hi3796mv200)|| defined(CHIP_TYPE_hi3798mv310)|| defined(CHIP_TYPE_hi3716mv430)
            /*no check*/
#else
...
{
#ifdef HI_PM_SUSPEND_FROM_MEMORY

    HI_PDM_PMOCPARAM_S stPmocParam;
    HI_U32 regValue = 0;
    HI_S32 Ret = HI_SUCCESS;

    memset(&stPmocParam, 0x00, sizeof(stPmocParam));
    Ret = HI_DRV_PDM_GetPmocParam(&stPmocParam);
    if (HI_SUCCESS != Ret)
    {
        HI_PRINT("call HI_DRV_PDM_GetPmocParam error Ret = : %x\n", Ret);
        return;
    }

    if (1 == stPmocParam.u32Flag)
    {
        HI_REG_READ32(MCU_STATUS_ADDR, regValue);

        if ((KERNEL_RUNNING != regValue) && (MCU_LOAD_CODE != regValue))
        {
            HI_PRINT("Enter memory standby...\n");
            boot_suspend();
        }
        else
        {
            ...
            stPmocParam.u32Flag = 0;

            Ret = HI_DRV_PDM_UpdatePmocParam(&stPmocParam);
            if (HI_SUCCESS != Ret)
            {
                HI_PRINT("call HI_DRV_PDM_UpdatePmocParam error Ret = : %x\n", Ret);
                return;
            }
        }
    }

    HI_REG_WRITE32(MCU_STATUS_ADDR, KERNEL_RUNNING);/*KERNEL_RUNNING means not power off. other value means power off*/
    HI_REG_WRITE32(MCU_BOOT_SUSPEND_ADDR, 0);

#ifdef HI_LPMCU_LOAD_IN_BOOT
        /* set flag of boot suspend to mcu */
        HI_REG_WRITE32(MCU_BOOT_SUSPEND_ADDR, MCU_BOOT_SUSPEND_FLAG);
