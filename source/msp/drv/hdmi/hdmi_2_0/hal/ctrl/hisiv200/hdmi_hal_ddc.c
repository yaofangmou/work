...
{
    HI_S32                  s32Ret = HI_SUCCESS;
    U_PWD_SLAVE_CFG         unRegPWD_SLAVE_CFG;

    DDC_NULL_CHK(pstCfg);

    unRegPWD_SLAVE_CFG.u32                   = 0;
    unRegPWD_SLAVE_CFG.bits.pwd_slave_seg    = pstCfg->u8Segment;
    unRegPWD_SLAVE_CFG.bits.pwd_slave_offset = pstCfg->u8Offset;

    switch(pstCfg->enFuncType)
    {
        case DDC_FUNC_TYPE_EDID:
            unRegPWD_SLAVE_CFG.bits.pwd_slave_addr = DDC_EDID_SALVE_ADDR;
            break;

        case DDC_FUNC_TYPE_HDCP:
            unRegPWD_SLAVE_CFG.bits.pwd_slave_addr = DDC_HDCP_SALVE_ADDR;
            break;

        case DDC_FUNC_TYPE_SCDC:
            unRegPWD_SLAVE_CFG.bits.pwd_slave_addr = DDC_SCDC_SALVE_ADDR;
            break;
    ...
    if (pstCfg->enMasterMode == DDC_MASTER_MODE_PWD)
    {
        ...
        HDMI_REG_CTRL_PWD_SLAVE_CFG_Set((HI_U32)unRegPWD_SLAVE_CFG.u32);
        HDMI_PWD_DATA_CNT_pwd_data_out_cntSet(pstCfg->s32DataSize);
        msleep(1);
