...
extern ISOGBNY_CFG_INFOR_S stIsogenyInfo;
...
extern HI_VOID ISOGENY_QueueFrameProcess(HI_VOID);
                                       ...
                                       HI_DRV_DISP_PROCESS_MODE_E  *penDispProcessMode)
{
    ...
    *penDispProcessMode = ((HI_DRV_DISP_TYPE_HDR10 == pstDisp->stSetting.stDispHDRAttr.enDispType)
    ...
    ISOGENY_QueueFrameProcess();
    pstWinShareInfo->stWBCCfg = stIsogenyInfo.stWBC0Cfg;
...
HI_VOID DISP_XDR_IsrProcess(HI_VOID *hHandle)
{
...
extern DISP_S *Disp_GetDiscriptPtr(HI_DRV_DISPLAY_E eDisp);
HI_VOID DISP_XDR_IsrProcess_ext(HI_DRV_DISPLAY_E eDisp)
{
    DISP_S *pstDisp = HI_NULL;

    pstDisp = Disp_GetDiscriptPtr(eDisp);
    if (HI_NULL != pstDisp)
    {
        DISP_XDR_IsrProcess(pstDisp);
    }
