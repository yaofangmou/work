...
static HI_S32 IsFullQueue(MSG_QUEUE_S* pstMsgQueue)
{
    ...
    if (pstMsgQueue->pstMsgRear >= pstMsgQueue->pstMsgFront)
    {
        u32RemainSpace = ((HI_U32)pstMsgQueue->pu8BaseAddr + pstMsgQueue->u32MaxLen) - (HI_U32)pstMsgQueue->pstMsgRear;

        if ((HI_U32)pstMsgQueue->pstMsgFront > (HI_U32)pstMsgQueue->pu8BaseAddr)
        {
            u32RemainSpace += ((HI_U32)pstMsgQueue->pstMsgFront - MIN_GAP - (HI_U32)pstMsgQueue->pu8BaseAddr);
        }
        else
        {
    ...
    {
        u32RemainSpace = (HI_U32)pstMsgQueue->pstMsgFront - MIN_GAP - (HI_U32)pstMsgQueue->pstMsgRear;
    }
...
static HI_S32  MsgQueueEn(MESSAGE_S* pstMsg)
{
    ...
    if (((HI_U32)s_stMsgQueue.pstMsgRear + u32ItemSize) > ((HI_U32)s_stMsgQueue.pu8BaseAddr + s_stMsgQueue.u32MaxLen))
    {
        s_stMsgQueue.pstMsgRear->pu8MsgData = (HI_U8*)s_stMsgQueue.pu8BaseAddr + u32DataTypeSize;
    }
    else
    {
        s_stMsgQueue.pstMsgRear->pu8MsgData = (HI_U8*)((HI_U32)s_stMsgQueue.pstMsgRear + u32DataTypeSize);
    }

    s_stMsgQueue.pstMsgRear->u16MsgLength = pstMsg->u16MsgLength;
    memcpy(s_stMsgQueue.pstMsgRear->pu8MsgData, pstMsg->pu8MsgData, pstMsg->u16MsgLength);

    pstNextMsgAddr = (MESSAGE_S*)((HI_U32)s_stMsgQueue.pstMsgRear + u32ItemSize);

    if (((HI_U32)pstNextMsgAddr + u32DataTypeSize) > ((HI_U32)s_stMsgQueue.pu8BaseAddr + s_stMsgQueue.u32MaxLen))
    {
