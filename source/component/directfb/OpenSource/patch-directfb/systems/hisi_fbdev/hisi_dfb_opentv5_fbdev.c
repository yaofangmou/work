...
static HI_S32 O5SDK_Disp_Init(HI_UNF_ENC_FMT_E enFormat)
{
    ...
    HI_U32 virtual_width  = 1280;
    HI_U32 virtual_height =  720;
    ...
    if ( dfb_config != NULL )
    {
        ...
        if ( conf != NULL )
        {
            if ( ( conf->config.flags & DLCONF_WIDTH ) &&
                 ( conf->config.flags & DLCONF_HEIGHT ) )
            {
                virtual_width  = conf->config.width;
                virtual_height = conf->config.height;
            }
        }
    }

    Ret = HI_UNF_DISP_SetVirtualScreen(HI_UNF_DISPLAY1, virtual_width, virtual_height);
