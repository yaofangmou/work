...
#ifdef HI_DMX_DESCRAMBLER_SUPPORT
...
#define DMX_FQ_VID_BLOCK_SIZE               (2 * 1024)   /* the min size of video block size */
...
#define DMX_MAX_INDEX_EMPTY_NUM             (100)      /* the max count of index empyt after starting record */
...
#define DMX_REC_AUD_TS_PACKETS_PER_BLOCK    (64 * DMX_REC_TS_PACKETS_UNIT)
...
#define DMX_REC_AUD_TS_WITH_TIMESTAMP_BLOCK_SIZE    (64 * DMX_REC_TS_WITH_TIMESTAMP_UNIT)
...
static HI_S32 DmxEsOQRelease(Dmx_Set_S *DmxSet, HI_U32 FqId, HI_U32 OqId)
{
    ...
    HI_DBG_DEMUX("Skip the block, i[0x%x], OqRead[0x%x]!\n", i, OqRead);
                                 ...
                                 DMX_UserMsg_S *MsgList /* [in] */)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("TEEC_CMD_FIXUP_SECTION_SEC_BUF failed(0x%x).\n", ret);
        goto out1;
    }
...
/* for 98cv200 keylad test */
#if !(defined(HI_ADVCA_TYPE_VERIMATRIX_ULTRA) && defined(CHIP_TYPE_hi3798cv200))
...
static HI_S32 DMXOsiIsr(int irq, void* priv)
{
    ...
    if (DmxSet == HI_NULL)
    {
        HI_ERR_DEMUX("Priv invalid: HI_NULL\n");
        return IRQ_HANDLED;
    }
...
#ifndef DMX_REDUNDANCE_CODE_DISABLE
...
#ifdef HI_DMX_REC_INDEX_SUPPORT
...
#endif
#endif /* HI_DMX_REC_INDEX_SUPPORT */
...
{
    /* bus internal buffer overflow intterrupt status of play channel*/
    if (unlikely(DmxHalGetD2BFOvflRawIntStatus(DmxSet)))
    {
        DmxHalClsD2BFOvflRawIntStatus(DmxSet);
        HI_PRINT("demux internal buffer overflow in play channel!\n");
    }
    ...
    if (unlikely(DmxHalGetD2BROvflRawIntStatus(DmxSet)))
    {
        ...
        HI_PRINT("demux internal buffer overflow in record channel!\n");
    }
}
...
static HI_S32 DMX_OsiInitDmxSet(Dmx_Set_S * DmxSet)
{
    ...
    {
        HI_UNF_DMX_TSO_PORT_ATTR_S  TSOPortAttr = {0};
        /* initialise TSO port */
        for ( i = 0 ; i < DmxSet->TSOPortCnt; i++ )
        {
...
#define ADD_DMX_SET(ID, DmxSetInst) ({ \
    ...
    DmxSetInst->DmxSet.TunerClkEn   = HI_FALSE;\
...
HI_S32 DMX_OsiDeInit(HI_VOID)
{
    ...
    {
        HI_ERR_DEMUX("Demux not initliased.\n");
        goto out;
    }
...
HI_S32 DMX_OsiAttachPort(const HI_U32 DmxId, const DMX_PORT_MODE_E PortMode, const HI_U32 PortId)
{
    ...
    switch(PortMode)
    {
...
#ifdef DMX_LOWPOWER_SUPPORT
            if (HI_TRUE != DmxSet->TunerClkEn)
            {
                /* enable the dvb clock */
                DmxHalDvbClkSet(DmxSet, HI_TRUE);
                /* enable the ts and ts error count */
                DmxHalDvbPortSetTsCountCtrl(DmxSet, PortId, TS_COUNT_CRTL_START);
                DmxHalDvbPortSetErrTsCountCtrl(DmxSet, PortId, TS_COUNT_CRTL_START);
                DmxSet->TunerClkEn = HI_TRUE;
            }
#endif
            break;
        }
        ...
        case DMX_PORT_MODE_RAM: /* each dmx set has private ram port, which can't mix bind. */
        {
            ...
            /* set ipport backpress switch */
            DmxHalSetIPBackPressure(DmxSet, DmxId, HI_TRUE);
...
HI_S32 DMX_OsiDetachPort(const HI_U32 DmxId)
{
    ...
    if (DMX_PORT_MODE_RAM == DmxInfo->PortMode)
    {
        /* clear the ipport back press switch */
        DmxHalSetIPBackPressure(DmxSet, DmxId, HI_FALSE);
#ifndef DMX_TEE_CMD_DISABLE
...
HI_S32 DMX_OsiGetPortId(const HI_U32 DmxId, DMX_PORT_MODE_E *PortMode, HI_U32 *PortId)
{
    ...
    {
        HI_WARN_DEMUX("demux not attach with any port, DmxId=%d.\n", DmxId);
    }
    mutex_unlock(&DmxInfo->LockDmx);
...
#ifdef DMX_RAM_PORT_LESS_DESC_DEPTH
...
static HI_S32 DmxTrimTsBufAttrs(const HI_UNF_DMX_TSBUF_ATTR_S *InAttr, HI_UNF_DMX_TSBUF_ATTR_S *OutAttr)
{
    ...
    {
        HI_ERR_DEMUX("TsBuffer size 0x%x outof range[0x%x, 0x%x]\n",
...
static HI_S32 DmxCreateTsBuffer(Dmx_Set_S *DmxSet, DMX_RamPort_Info_S *PortInfo, HI_U32 PortId, const HI_UNF_DMX_TSBUF_ATTR_S *pstTsBufAttr, DMX_MMZ_BUF_S *TsBuf)
{
    ...
    {
        HI_ERR_DEMUX("Create tsbuf failed.\n");
        ret = HI_ERR_DMX_ALLOC_MEM_FAILED;
        goto out0;
    }
...
HI_S32 DMX_OsiTsBufferGetStatus(const HI_U32 PortId, HI_UNF_DMX_TSBUF_STATUS_S *Status)
{
    ...
    for (index = 0; index < PortInfo->TotalBufBlkNR; index ++)
    {
        if (FREE_BLK_FLAG != PortInfo->pstBufBlkMap[index].BlkFlag)
        {
            if (PortInfo->pstBufBlkMap[index].BlkFlag != ALLOC_BLK_FLAG)
            {
                UsedBlocksByHardware++;
            }
    ...
    Status->u32HardwareUsedSize = UsedBlocksByHardware * DMX_MAX_BLOCK_SIZE;
...
static HI_S32 DmxFltAcquire(HI_U32 DmxId, HI_U32 *FltId)
{
    ...
    *FltId = find_first_zero_bit(DmxSet->FilterBitmap, DmxSet->DmxFilterCnt);
    ...
    {
        HI_ERR_DEMUX("No free filter now!\n");
...
static HI_S32 DmxTeecRegisterVidSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("TEEC_CMD_REGISTER_VID_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecUnregisterVidSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("UNREGISTER_VID_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecRegisterAudSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf, MMZ_BUFFER_S *MmzBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("REGISTER_AUD_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecUnregisterAudSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("UNREGISTER_AUD_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecRegisterSectionSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf, MMZ_BUFFER_S *MmzBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("REGISTER_SECTION_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecUnregisterSectionSecBuf(DMX_ChanInfo_S *ChanInfo, SMMU_BUFFER_S *MmuBuf)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("UNREGISTER_SECTION_SECBUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecFixupAudSecBuf(DMX_ChanInfo_S *ChanInfo, HI_U32 ParserAddr, HI_U32 ParserLen)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("FIXUP_AUD_SEC_BUF failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxAllocSecureAVBuf(Dmx_Set_S *DmxSet, DMX_ChanInfo_S *ChanInfo)
{
    ...
    */
    ...
    DmxHalSetOQWORDx(DmxSet, OqInfo->u32OQId, DMX_OQ_CFG_OFFSET, DMX_OQ_OUTINT_CNT << 26 |
                     OqInfo->OQDepth << 16 | (DMX_OQ_ALOVF_CNT & 0xff) << 8 | OqInfo->u32FQId);
    DmxHalSetOQWORDx(DmxSet, OqInfo->u32OQId, DMX_OQ_SZUS_OFFSET, (BlockSize << 16));
...
static HI_S32 DmxDeConfigureRecChannel(Dmx_Set_S *DmxSet, HI_U32 ChanId)
{
    ...
    {
        HI_ERR_DEMUX("channel(%d) is not an rec channel.\n", ChanId);
        ret = HI_ERR_DMX_INVALID_PARA;
        goto out;
    }
...
/* declare the DmxDelChannelFunc defined as blow */
...
#endif /* end of HI_DMX_REC_SUPPORT */
...
#ifdef DMX_PES_EXT_DATA_SUPPORT
...
static HI_S32 DmxOpenPlyChannel(Dmx_Set_S *DmxSet, HI_U32 ChanId)
{
    ...
    {
        HI_ERR_DEMUX("channel(%d) is not an play channel.\n", ChanId);
        return HI_ERR_DMX_INVALID_PARA;
    }
...
static HI_VOID DMX_ReportEsRelease(HI_HANDLE handle)
{
    ...
    if ((HI_SUCCESS == ret) && (HI_NULL != pVdecFunc->pfnVDEC_ReportEsRels))
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_WARN_DEMUX("VDEC_INFORM_DEMUX_RELEASE Failed!\n");
        }
    }
    else
    {
...
HI_S32 DMX_OsiSetChannelAttr(HI_U32 DmxId, HI_U32 ChanId, HI_UNF_DMX_CHAN_ATTR_S *ChanAttr)
{
    ...
    switch (ChanAttr->enChannelType)
    {
        ...
        case HI_UNF_DMX_CHAN_TYPE_POST :
            ...
            {
                HI_ERR_DEMUX("PES/VID/AUD/POST channot support change crc mode: %u\n", ChanAttr->enCRCMode);
                ret = HI_ERR_DMX_NOT_SUPPORT;
            }
            break;
    ...
    else //normal pes data for pes channel according to 13818-1
    {
        if (ParserLen >= 6)
        {
            ...
            *PesPktLen = ((ParserAddr[4] << 8) | ParserAddr[5]);
            HI_WARN_DEMUX("streamID:[0x%x] only for pes channel!\n", StreamId);
            ret = HI_SUCCESS;
        }
        else
        {
...
static HI_S32 DmxTeecParsePesDispCtrlInfo(DMX_ChanInfo_S *ChanInfo, HI_U32 ParserAddr, HI_U32 ParserLen, HI_U32  *PesHeaderLen, Disp_Control_t *pDispController)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("GET_DISP_CONTROL failed(0x%x).\n", ret);
        ret = HI_FAILURE;
    }
    else
    {
    ...
    )
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("PARSER_PES_HEADER failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
static HI_S32 DmxTeecGetPesHeaderLen(DMX_ChanInfo_S *ChanInfo, HI_U32 ParserAddr, HI_U32 ParserLen, HI_U32 *PesHeaderLen)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("GET_PES_HEADER_LEN failed(0x%x).\n", ret);
        ret = HI_FAILURE;
        goto out;
    }
...
errdata_loop:
    ...
    if (PvrCtrl & DMX_MASK_BIT_2)
    {
        ...
        if (HI_SUCCESS == ret)
        {
            ...
            {
                HI_ERR_DEMUX("Invalid buflen[%#x] or pesheadlen[%#x]\n", DataLen, PesHeadLen);
            }
        }
        else if (-2 == ret)/*this is for PVR smooth ctrl*/
        {
            ...
            {
                HI_ERR_DEMUX("Unexpect buflen=%#x, peshead=%#x\n", DataLen, PesHeadLen);
            }
        }
        else
        {
    ...
    EsData->u64BufPhyAddr    = (HI_U64)DataPhyAddr;
    EsData->u64BufKerVirAddr = (HI_U64)((HI_ULONG)DataKerAddr);
...
HI_S32 DMX_OsiReleaseReadEs(HI_U32 DmxId, HI_U32 ChanId, DMX_Stream_S *EsData)
{
    ...
    {
        HI_ERR_DEMUX("Release es data range[0x%x, 0x%x] is out of buf range[0x%x, 0x%x].\n", EsData->u32BufPhyAddr, EsData->u32BufPhyAddr + EsData->u32BufLen,
...
HI_S32 DmxGetPcrInstance(Dmx_Set_S *DmxSet, HI_U32 PcrId)
{
    ...
    if (unlikely(!(*p & mask)))
    {
        HI_ERR_DEMUX("Invalid Pcr(%d) instance.\n", PcrId);
        ret = HI_ERR_DMX_INVALID_PARA;
        goto out1;
    }
...
static HI_S32 DmxAllocSecureRecBuf(Dmx_Set_S *DmxSet, DMX_RecInfo_S  *RecInfo)
{
    ...
    DmxHalSetOQWORDx(DmxSet, RecInfo->RecOqId, DMX_OQ_SZUS_OFFSET, (RecFqBlockSize << 16));
...
static HI_S32 DmxAllocScdBuf(Dmx_Set_S *DmxSet, DMX_RecInfo_S  *RecInfo)
{
    ...
    switch(RecInfo->IndexType)
    {
        ...
        case HI_UNF_DMX_REC_INDEX_TYPE_AUDIO :
            ...
            break;
        case HI_UNF_DMX_REC_INDEX_TYPE_NONE :
            ret = HI_ERR_DMX_INVALID_REC_CHAN;
            goto out0;
        default:
            BUG();
    }
...
#endif  /* end of HI_DMX_REC_INDEX_SUPPORT */
#else
#ifndef DMX_TEE_CMD_DISABLE
static HI_S32 DmxAllocSecureRecBuf(Dmx_Set_S *DmxSet, DMX_RecInfo_S  *RecInfo)
{
...
#endif  /* end of HI_DMX_REC_INDEX_SUPPORT */
#endif  /* end of DMX_MMU_SUPPORT */

/* declare the function define blow */
...
#endif  /* end of ifndef DMX_SCD_VERSION_2 */
...
#ifdef HI_DMX_REC_INDEX_SUPPORT
#ifdef HI_DMX_REC_INDEX_HEVC_SUPPORT
...
#endif /* HI_DMX_REC_INDEX_HEVC_SUPPORT */
#endif /* HI_DMX_REC_INDEX_SUPPORT */

#ifdef DMX_REC_ALL_DATA_SUPPORT
...
out1:
        ...
        (HI_VOID)wait_event_interruptible_timeout(ScdOqInfo->OqWaitQueue, ScdOqInfo->OqWaitIndex, msecs_to_jiffies(10));
...
static HI_S32 DMXAcquireRecDataIndexHevc(Dmx_Set_S *DmxSet, HI_U32 RecId, HI_MPI_DMX_REC_DATA_INDEX_S *pstRecDataIdx)
{
    ...
    while(RemRecData->u32Len < FqInfo->BlockSize)
    {
        /* almost overflow maybe clear and disabled in ISR, so we always enable it here. */
        DmxFqCheckOverflowInt(DmxSet, RecInfo->RecFqId);
        if (unlikely(FqInfo->FqOverflowCount && RecInfo->FirstIndexEmptyCnt++ < DMX_MAX_INDEX_EMPTY_NUM))
        {
            HI_WARN_DEMUX("Rec Buf(%d) Data overflowed at beginning.\n", RecInfo->RecFqId);
            /* reset the record channel */
            /* stop */
            {
                DmxStopSCD(DmxSet, RecId);
                DmxFqStop(DmxSet, RecInfo->RecFqId);
        ...
        if (!list_empty(&RecInfo->head))
        {
            ...
            /* set the empty cnt */
            RecInfo->FirstIndexEmptyCnt = DMX_MAX_INDEX_EMPTY_NUM;
            /* new */
            if (0 == RecInfo->RemIdxCnt)
            {
                if (HI_FALSE == Entry->Rewind)
