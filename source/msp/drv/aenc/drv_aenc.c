...
static HI_S32 AENC_DRV_WriteProc(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    ...
    {
        HI_CHAR filePath[256] = {0};
        if (HI_SUCCESS != HI_DRV_FILE_GetStorePath(filePath, AENC_PATH_NAME_MAXLEN))
        {
        ...
        if (enSaveCmd == AENC_CMD_PROC_SAVE_PCM)
        {
            ...
            {
                snprintf(pstAencProc->filePath, sizeof(pstAencProc->filePath), "%s/aenc%02d_%d.pcm", filePath, u32ChNum, pstAencProc->u32SavePcmCnt);
        ...
        else if (enSaveCmd == AENC_CMD_PROC_SAVE_ES)
        {
            ...
            {
                snprintf(pstAencProc->filePath, sizeof(pstAencProc->filePath), "%s/aenc%02d_%d.es", filePath, u32ChNum, pstAencProc->u32SaveEsCnt);
