...
typedef struct tagUMP_PARAM_S
{
    ...
    struct semaphore stSemDirEntry; /* Semaphore for Dir Entry */
...
static int hi_usrmodeproc_open(struct inode *inode, struct file *file)
{
    ...
    if (atomic_inc_return(&g_stUProcParam.atmOpenCnt) == 1)
    {
        ...
        HI_INIT_MUTEX(&g_stUProcParam.stSemDirEntry);
...
UMP_DIR_S * PROC_AddDir(const HI_CHAR* pszName, HI_MPI_UPROC_PARENT_DIR pszParent, struct file *pstFile)
{
    UMP_DIR_S *pstDir;
    ...
    if (HI_MPI_UPROC_PARENT_DEFAULT == pszParent)
    {
        parent = g_pHisi_proc;
    }
    else
    ...
    pstDir->parent = parent;
    pstDir->pstFile = pstFile;

    UPROC_K_UNLOCK(g_stUProcParam.stSem);
    /* Make proc directory */
    pstDir->entry = proc_mkdir(pstDir->dir_name, pstDir->parent);
...
out2:
    remove_proc_entry(pstDir->dir_name, pstDir->parent);
...
HI_S32 PROC_RemoveDirByName(const HI_CHAR* pszName, HI_MPI_UPROC_PARENT_DIR pszParent)
{
    ...
    if (HI_NULL == pstDir || parent != pstDir->parent)
    {
                ...
                unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        case UMPIOC_ADD_ENTRY:
        {
            ...
            UPROC_K_LOCK(g_stUProcParam.stSemDirEntry);
            pstEntry = PROC_AddEntry(&ump_entry, HI_TRUE);
            UPROC_K_UNLOCK(g_stUProcParam.stSemDirEntry);
            if (HI_NULL == pstEntry)
            {
                ret = HI_FAILURE;
                break;
        ...
        case UMPIOC_ADD_DIR:
        {
            HI_PROC_DIR_NAME_S *ptrDirName = &stName;
            ...
            stName.stDirName[sizeof(stName.stDirName)-1] = 0;

            UPROC_K_LOCK(g_stUProcParam.stSemDirEntry);
            pstDir = PROC_AddDir(stName.stDirName, stName.stParent, file);
            UPROC_K_UNLOCK(g_stUProcParam.stSemDirEntry);
            if (HI_NULL == pstDir)
            {
                ret = HI_FAILURE;
        ...
        case UMPIOC_REMOVE_DIR:
        {
            ...
            UPROC_K_LOCK(g_stUProcParam.stSemDirEntry);
            ret = PROC_RemoveDirByName(stName.stDirName, stName.stParent);
            UPROC_K_UNLOCK(g_stUProcParam.stSemDirEntry);
            break;
        }
        case UMPIOC_GETCMD:
        ...
        {
            HI_PROC_SHOW_BUFFER_S ShowBuf = {0};
            ...
            {
                if (ShowBuf.u32Size <= HI_PROC_BUFFER_SIZE && HI_NULL != ShowBuf.pu8Buf)
                {
                    UPROC_K_LOCK(g_stUProcParam.stSemReadWrite);
