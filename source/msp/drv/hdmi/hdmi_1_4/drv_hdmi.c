...
HI_U32 DRV_HDMI_SetFormat(HI_UNF_HDMI_ID_E enHdmi, HI_DRV_DISP_FMT_E enFmt, HI_DRV_DISP_STEREO_E enStereo)
{
   ...
   if ((DRV_Get_IsLCDFmt(enEncodingFormat))
    ...
    DRV_HDMI_SetAVMute(HI_UNF_HDMI_ID_0, HI_FALSE);
    SI_SetHdmiAudioMute(HI_FALSE);//add for cts switch
#if defined (DEBUG_TIMER)
    end = SI_timer_count();
    SI_timer_stop();
...
#endif
    SI_SetHdmiAudioMute(HI_TRUE);// add for cts switch
...
HI_U32 DRV_HDMI_LoadKey(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_LOAD_KEY_S *pstLoadKey)
{
    HI_U32 u32Ret = HI_SUCCESS;
#if defined (HDCP_SUPPORT)
    HDMI_HDCPKEY_ATTR_S * pstFlashEncrytedHdcpKey  = DRV_Get_HdcpKeyInfo(enHdmi);
    ...
    {
        /*load hdcp key */
        if (copy_from_user(pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey.u8Key, (void __user *)pstLoadKey->pu8InputEncryptedKey, pstLoadKey->u32KeyLength))
        {
            HDCP_ERR("Copy hdcp key from user failed!\n");
            return HI_FAILURE;
        }
        pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey.u32KeyLen = pstLoadKey->u32KeyLength;
        pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey.enHDCPKeyType = HI_DRV_HDCPKEY_TX0;
        pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey.enHDCPVersion = HI_DRV_HDCP_VERIOSN_1x;
        u32Ret = (g_stCIPHERExportFunctionLists->pfnCipherLoadHdcpKey)(&pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey);
        ...
        {
            pstFlashEncrytedHdcpKey->KeyLoadStat = HI_TRUE;
...
HI_S32 DRV_HDMI_SetAPPAttr(HI_UNF_HDMI_ID_E enHdmi,HDMI_APP_ATTR_S *pstHDMIAppAttr,HI_BOOL UpdateFlag)
{
    ...
    || defined(CHIP_TYPE_hi3716mv420) \
    || defined(CHIP_TYPE_hi3716mv410)
HI_VOID DRV_HDMI_SetSWAudioCTS(HDMI_AUDIOINTERFACE_E  enSoundIntf,\
                                                HI_DRV_DISP_FMT_E eDispFmt,\
                                                HI_UNF_HDMI_DEEP_COLOR_E eDeepColor,\
                                                HI_U32 audioSampleRate)
{
    HI_U32 u32Freq = 0;
        HI_U32              u32TmpCts = 0xffffffff;
    ...
    u32TmpCts = SI_HDMI_CtsValueGet(audioSampleRate,u32Freq/1000);
    COM_INFO(" func:%s ,line:%d,audioSampleRate:%d,u32Freq:%d,u32TmpCts:%d  !\n",__func__,__LINE__,audioSampleRate,u32Freq,u32TmpCts );
    WriteByteHDMITXP1(0x08,((u32TmpCts>>16)&0xff));  // 0x7A:0x0b
    WriteByteHDMITXP1(0x07,((u32TmpCts>>8)&0xff));  // 0x7A:0x0a
    WriteByteHDMITXP1(0x06,(u32TmpCts&0xff));  // 0x7A:0x09
}
#endif
#if    defined(CHIP_TYPE_hi3798mv100) \
    || defined(CHIP_TYPE_hi3796mv100) \
...
HI_S32 DRV_HDMI_SetAOAttr(HI_UNF_HDMI_ID_E enHdmi,HDMI_AUDIO_ATTR_S *pstHDMIAOAttr,HI_BOOL UpdateFlag)
{
    ...
    || defined(CHIP_TYPE_hi3716mv410)
        ...
        DRV_HDMI_SetSWAudioCTS(pstAudAttr->enSoundIntf,
                                    pstHDMIVOAttr->enVideoFmt,
                                    pstAppAttr->enDeepColorMode,
                                    audioSampleRate);
#endif
