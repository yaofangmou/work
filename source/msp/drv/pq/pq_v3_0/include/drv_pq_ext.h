...
typedef struct hi_PQ_IFMD_CALC_S
{
    ...
    HI_VOID *pstIfmdHardOutReg; /* 读取ifmd的状态寄存器，内部转换为S_VPSS_STT_REGS_TYPE类型处理*/
...
typedef struct hi_PQ_PFMD_CALC_S
{
    ...
    HI_S32  s32FieldOrder; /* 顶底场序 顶场优先底场优先 */
    HI_S32  s32FieldMode;  /* 顶底场标志 */
    //S_VPSS_STT_REGS_TYPE* pstpfmdHardOutReg; /* 读取pfmd的状态寄存器 */
...
typedef struct hiPQ_DB_CALC_INFO_S
{
    ...
    HI_U32 u32Width;
    HI_U32 u32Height;
    HI_VOID *pstDbCalcWbcReg;/*内部转换为S_VPSS_STT_REGS_TYPE类型处理*/
...
\param[in] *pstWbcReg:VPSS 回写寄存器地址，内部转换为S_VPSS_STT_REGS_TYPE类型处理。
\param[in] *pstVPSSReg:VPSS 虚拟寄存器地址，内部转换为S_VPSS_REGS_TYPE类型处理；如果为NULL，则表示销毁该路地址,
...
extern HI_S32 DRV_PQ_UpdateVpssPQ(HI_U32 u32HandleNo, HI_VPSS_PQ_INFO_S *pstTimingInfo, HI_VOID *pstVPSSReg, HI_VOID *pstWbcReg, HI_PQ_VPSS_MODULE_S *pstPQModule);
...
extern HI_S32 DRV_PQ_UpdateVdpPQ(HI_PQ_DISPLAY_E enDisplayId, HI_VDP_PQ_INFO_S *pstTimingInfo, HI_VOID *pstVDPReg);
...
无
 ...
 \param[in] u32LayerId:
 \param[in] *pstReg：VPSS 虚拟寄存器地址，内部转换为S_VPSS_REGS_TYPE类型处理
 \param[in] pstZmeDrvPara:
 \param[in] bFirEnable
 ...
 */
extern HI_S32 DRV_PQ_SetVpssZme(HI_PQ_VPSS_ZME_LAYER_E enLayerId, HI_VOID *pstReg, HI_PQ_ZME_PARA_IN_S *pstZmeDrvPara, HI_BOOL bFirEnable);
