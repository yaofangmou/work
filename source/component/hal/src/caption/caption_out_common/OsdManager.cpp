...
#if (PLATFORM_SDK_VERSION >= 26)
    #define USE_TDE_INTERFACE  //only for androidO and so on?
#endif
...
namespace android
{
    ...
    const HI_U32 CAPTION_FONTSIZE_SMALL = 15;
    const HI_U32 CAPTION_FONTSIZE_STANDARD = 27;
    const HI_U32 CAPTION_FONTSIZE_LARGE = 31;
    ...
    static int Set_SkBitmap(SkBitmap *pBitmap, HI_U32 u32Width, HI_U32 u32Height, void **ppu8Buffer, SkColorType ColorType, SkAlphaType AlphaType)
    {
#if (PLATFORM_SDK_VERSION > 19)
        if (NULL == *ppu8Buffer)
        {
            *ppu8Buffer = malloc(u32Width * u32Height * 4);
            if (HI_NULL == *ppu8Buffer)
            {
        ...
        pBitmap->installPixels(info, const_cast<void*>(*ppu8Buffer), u32Width * 4);
#else
        pBitmap->setConfig (ColorType, u32Width, u32Height);
        if (NULL! = *ppu8Buffer)
        {
            pBitmap->setPixels(*ppu8Buffer);
        }
        else
        {
    ...
    /* use setsurface with priority */
    HI_S32 OsdManager::SetCommonSurface(HI_VOID* pvCommonSurface)
    {
        bUsedSetSurface = HI_TRUE;
        Surface* tmpsurface = (Surface*)pvCommonSurface;
        m_DispSurface[0].surface = tmpsurface;
        m_DispSurface[0].hSurface = SURFACE_HANDLE_INDEX;

        return HI_SUCCESS;
    }

    /* if SetCommonSurface is called , CreateSurface do not create surface */
    HI_S32 OsdManager::CreateSurface(CAPTION_MGR_SURFACE_ATTR_S* pstAttr, HI_HANDLE* phSurface)
    {
        ...
        if (bUsedSetSurface)
        {
            memcpy(&(m_DispSurface[0].stAttr), pstAttr, sizeof(CAPTION_MGR_SURFACE_ATTR_S));
            *phSurface = m_DispSurface[0].hSurface;
        ...
        {
            ALOGE("CreateSurface failed, there is no free display surface!\n");
            return HI_FAILURE;
        }
        ...
        //bitmap for save the surface
        if (HI_NULL == m_DispSurface[i].pBitmapAddr)
        {
            m_DispSurface[i].pBitmapAddr = malloc(pstAttr->u32Width * pstAttr->u32Height * 4);
            if (HI_NULL == m_DispSurface[i].pBitmapAddr)
            {
                ALOGE("pBitmapAddr map error\n");
            }
    ...
    HI_S32 OsdManager::DestroySurface(HI_HANDLE hSurface)
    {
        ...
        if (HI_NULL != pSurface->pBitmapAddr)
        {
            free(pSurface->pBitmapAddr);
            pSurface->pBitmapAddr = HI_NULL;
        }
    ...
    HI_S32 OsdManager::ClearSurface(HI_HANDLE hSurface)
    {
        ...
        HI_S32 s32Ret;
        //ALOGE("[%s] %#x", __FUNCTION__, hSurface);
    ...
    HI_S32 OsdManager::FillRect(HI_HANDLE hSurface, CAPTION_MGR_SURFACE_ATTR_S* pstSurfaceAttr, HI_RECT_S* pstRect, HI_U32 u32Color)
    {
        CAPTION_MGR_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        SkBitmap   tempBitmap;
        ...
        if ((HI_NULL == pPixels ) || (captionSurfaceW != pstSurfaceAttr->u32Width ) || (captionSurfaceH != pstSurfaceAttr->u32Height))
        {
            ...
            if (0 == captionSurfaceW * captionSurfaceH * CAPTION_PIXBYTE)
            {
                ALOGE("pPixels malloc size is 0!\n");
                return HI_FAILURE;
            }
    ...
    /* Just fill to surface, not to display. */
    HI_S32 OsdManager::FillRect_Ex(HI_HANDLE hSurface, CAPTION_MGR_SURFACE_ATTR_S* pstSurfaceAttr, HI_RECT_S* pstRect, HI_U32 u32Color)
    {
        CAPTION_MGR_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        pSurface = GetSurfaceInst(hSurface);

        /*ALOGE("[%s] %#x [%d,%d,%d,%d] color = %#x", __FUNCTION__, hSurface,
            pstRect->s32X, pstRect->s32Y, pstRect->s32Width, pstRect->s32Height, u32Color);*/
        ...
        const SkImageInfo bitMapInfo = SkImageInfo::Make(pSurface->stAttr.u32Width, pSurface->stAttr.u32Height,
                                       kRGBA_8888_SkColorType, kPremul_SkAlphaType);

        bitmap.installPixels(bitMapInfo, const_cast<void*>((void*)pSurface->pBitmapAddr), pSurface->stAttr.u32Width * CAPTION_PIXBYTE);
#endif
        SkCanvas canvas(bitmap);
        SkPaint paint;
        SkColor color;
    ...
    HI_S32 OsdManager::Refresh(HI_HANDLE hSurface)
    {
        ...
        tempBitmap.installPixels(tmpBitMapInfo, const_cast<void*>(pSurface->pBitmapAddr), pSurface->stAttr.u32Width * CAPTION_PIXBYTE);
#else
        ...
        void    *pPixels1 = tempBitmap.getPixels();
        char * p1 = (char *)pPixels1;
    ...
    HI_S32 OsdManager::Blit(HI_HANDLE hSrcSurface, HI_RECT_S* pstSrcRect, HI_HANDLE hDstSurface, HI_RECT_S* pstDstRect)
    {
...
#else //AndroidO... use TDE

        HI_S32              Ret;
        CAPTION_MGR_DISP_SURFACE_S*  pSrcSurface, *pDstSurface;
        ALOGE("[%s] %#x -> %#x [%d,%d,%d,%d] -> [%d,%d,%d,%d]", __FUNCTION__, hSrcSurface, hDstSurface,
            pstSrcRect->s32X, pstSrcRect->s32Y, pstSrcRect->s32Width, pstSrcRect->s32Height,
            pstDstRect->s32X, pstDstRect->s32Y, pstDstRect->s32Width, pstDstRect->s32Height);
...
#if (PLATFORM_SDK_VERSION >= 22)
        const SkImageInfo bitMapInfo_src = SkImageInfo::Make(pSrcSurface->stAttr.u32Width, pSrcSurface->stAttr.u32Height,
                                       kRGBA_8888_SkColorType, kPremul_SkAlphaType);

        bitmap_src.installPixels(bitMapInfo_src, const_cast<void*>((void*)pSrcSurface->pBitmapAddr), pSrcSurface->stAttr.u32Width * CAPTION_PIXBYTE);

        const SkImageInfo bitMapInfo_dst = SkImageInfo::Make(pDstSurface->stAttr.u32Width, pDstSurface->stAttr.u32Height,
                                          kRGBA_8888_SkColorType, kPremul_SkAlphaType);

        bitmap_dst.installPixels(bitMapInfo_dst, const_cast<void*>(pDstSurface->pBitmapAddr), pDstSurface->stAttr.u32Width * CAPTION_PIXBYTE);
#endif

        SkCanvas canvas_src(bitmap_src);
        SkCanvas canvas_dst(bitmap_dst);
        SkPaint paint;
        SkRect src;
        SkRect dst;

        if ((pstSrcRect->s32Width == 0) && (pstSrcRect->s32Height == 0))
        {
            src.set(pstSrcRect->s32X, pstSrcRect->s32Y, pstSrcRect->s32X + pSrcSurface->stAttr.u32Width, pstSrcRect->s32Y + pSrcSurface->stAttr.u32Height);
        }
        else
        {
        ...
        canvas_dst.drawBitmapRect(bitmap_src, src, dst, &paint);
#else
        canvas_dst.drawBitmapRectToRect(bitmap_src, &src, dst, &paint);
#endif
        canvas_dst.restore();
    ...
    HI_S32 OsdManager::DrawCCTextToSurface(HI_HANDLE hSurface, CAPTION_MGR_SURFACE_ATTR_S* pstSurfaceAttr, HI_RECT_S* pstRect, HI_CHAR* pTest, CAPTION_MGR_CCTEXT_ATTR_S* pstAttr)
    {
        CAPTION_MGR_DISP_SURFACE_S*  pSurface = HI_NULL;
        SkBitmap   bitmap;
        //ALOGE("[%s] %#x [%c]", __FUNCTION__, hSurface, pTest[0]);
        ...
        {
            ts = SkIntToScalar(CAPTION_FONTSIZE_SMALL);
        }
        else if (3 == pstAttr->u8fontSize) //HI_UNF_CC_FONTSIZE_LARGE
        {
            ts = SkIntToScalar(CAPTION_FONTSIZE_LARGE);
        }
        else//HI_UNF_CC_FONTSIZE_STANDARD
        {
            ts = SkIntToScalar(CAPTION_FONTSIZE_STANDARD);
        }
        ts = SkIntToScalar(CAPTION_FONTSIZE_STANDARD);
    ...
    HI_S32 OsdManager::GetCCTextSize(HI_CHAR* pTest, HI_S32* pWidth, HI_S32* pHeigh )
    {
        ...
        SkPaint paint;
        SkScalar ts = SkIntToScalar(CAPTION_FONTSIZE_STANDARD); //normal
        paint.setTextSize(ts);
#if (PLATFORM_SDK_VERSION >= 26)
        paint.setTypeface(SkTypeface::MakeFromName(CC_FONT_FILE_MONOSPACE, SkFontStyle::FromOldStyle(SkTypeface::kNormal)));
#else
        ...
        *pHeigh = CAPTION_FONTSIZE_STANDARD;
    ...
    HI_S32 OsdManager::TeletextDrawBitmap(HI_HANDLE hSurface, HI_U8* bitmapBuffer, HI_RECT_S* pstSrcRect, HI_RECT_S* pstDstRect)
    {
        ...
        tempBitmap.installPixels(bitMapInfo, const_cast<void*>((void*)bitmapBuffer), pstSrcRect->s32Width * CAPTION_PIXBYTE);

         //set pBitmapAddr to bitmap
         if (0 == pSurface->stAttr.u32Width * pSurface->stAttr.u32Height * CAPTION_PIXBYTE)
         {
             ALOGE("pBitmapAddr map size is 0\n");
             return HI_FAILURE;
         }
