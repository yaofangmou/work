                           ...
                           HI_WLAN_AP_CONFIG_S *pstApCfg)
{
    ...
    if ((strlen(ifname)>0) && (strlen(ifname)<sizeof(wrq.ifr_name)-1))
...
int ensure_entropy_file_exists(const HI_CHAR *entropy_file)
{
    ...
    dir = opendir(file);
    if (!dir) {
        DBGPRINT(("WiFi: entropy file '%s' already exists\n", file));
        return 0;
    }
    closedir(dir);
...
HI_S32 HI_WLAN_AP_Start(HI_CHAR *ifname, HI_WLAN_AP_CONFIG_S *pstApCfg)
{
    ...
    if (pstApCfg->security >= HI_WLAN_SECURITY_BUTT)
        return HI_WLAN_INVALID_PARAMETER;
    ...
    if (device_id == WIFI_RALINK_RT3070 || device_id == WIFI_RALINK_RT5370
      ...
      || device_id == WIFI_RALINK_MT7601U) {
        ...
        if ((strlen(ifname)>0) && (strlen(ifname)<sizeof(ifr.ifr_name)-1))
...
HI_S32 HI_WLAN_AP_GetMacAddress(HI_CHAR *ifname, HI_CHAR *pstMac)
{
    ...
    if (ioctl(s, SIOCGIFHWADDR, &ifr) >= 0) {
        ...
        snprintf(mac, sizeof(mac), MACSTR, MAC2STR(ifr.ifr_hwaddr.sa_data));
        strncpy(pstMac, mac, sizeof(ifr.ifr_hwaddr.sa_data));
