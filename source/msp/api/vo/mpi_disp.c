...
HI_S32 HI_MPI_DISP_SetTechnicolorBrightness(HI_DRV_DISPLAY_E enDisp, HI_S32 s32Brightness)
{
    HI_S32 Ret = HI_FAILURE;
    DISP_TECHNI_BRIGHT_S stDispTechniBright;
    ...
    stDispTechniBright.enDisp = enDisp;
    stDispTechniBright.s32TechniDispBright = s32Brightness;

    Ret = ioctl(g_DispDevFd, CMD_DISP_SET_TECHNI_BRIGHT, &stDispTechniBright);

    return Ret;
}

HI_S32 HI_MPI_DISP_GetTechnicolorBrightness(HI_DRV_DISPLAY_E enDisp, HI_S32 *ps32Brightness)
{
    HI_S32 Ret = HI_FAILURE;
    DISP_TECHNI_BRIGHT_S stDispTechniBright;
    ...
    Ret = ioctl(g_DispDevFd, CMD_DISP_GET_TECHNI_BRIGHT, &stDispTechniBright);
    if(HI_SUCCESS != Ret)
    {
       return Ret;
    }

    *ps32Brightness = stDispTechniBright.s32TechniDispBright;
