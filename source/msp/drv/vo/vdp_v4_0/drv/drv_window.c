...
static const HI_DRV_AFD stInput4x3AFDConfiguration[] =
{
    {
        HI_DRV_AFD_TYPE_SAME_AS_SRC,
        .stSrcCropInfo = {0, 0, 0, 0,},
        .stOutInfo4_3 =
            {// Out4x3
                0, 0, 0, 0,
                0, 0, 0, 0,
...
static const HI_DRV_AFD stInput16x9AFDConfiguration[] =
{
    {
        ...
        WIN_INFO_S     *pstWinInfo)
{
    ...
    pstWinInfo->bMask         = pstWinPara->bMask;
...
HI_S32 WIN_Init(HI_VOID)
{
    ...
    HI_S32 s32Ret;
    DISP_INTF_OPERATION_S *pstFunction = HI_NULL;
    ...
    (HI_VOID)DISP_HAL_GetOperation(&pstFunction);
    if (HI_NULL == pstFunction->PF_GetIsogenyMode)
    {
        WIN_ERROR("PF_GetIsogenyMode is NULL!\n");
        return HI_ERR_VO_NULL_PTR;
    }
    (HI_VOID)pstFunction->PF_GetIsogenyMode(&stDispWindow.eIsogeny_mode);
...
HI_S32 WIN_Sync_QueueFrame(HI_HANDLE hWin, WIN_XDR_FRAME_S *pstWinXdrFrm, HI_U32 *pu32FenceFd)
{
#ifdef HI_VO_WIN_SYNC_SUPPORT
    WINDOW_S *pstWin = HI_NULL;
    HI_S32 s32Ret = HI_SUCCESS;
        ...
        || (pstWin->enType == HI_DRV_WIN_ACTIVE_SINGLE))
    {
        ...
        if (*pu32FenceFd != 0x0)
        {
            *pu32FenceFd = DRV_SYNC_CreateFence(&pstWin->stSyncInfo,
                                                pstWinXdrFrm->stBaseFrm.u32FrameIndex,
                                                pstWinXdrFrm->stBaseFrm.stBufAddr[0].u32PhyAddr_Y);
        }
        else
        {
    ...
    if (!bVirtual)
    {
        ...
        if (pstWin->bEnable == HI_TRUE)
        {
            ...
            DRV_WIN_GetFramePlayCounts(pstWin->bQuickMode,
                                       pstWin->hFrc,
                                       pFrameInfo->u32FrameRate,
                                       pstWin->stDispInfo.u32RefreshRate * 10,
                                       &u32PlayCnt,
...
HI_S32 WIN_QueueSyncFrame(HI_HANDLE hWin, WIN_XDR_FRAME_S *pstWinXdrFrm, HI_U32 *pu32FenceFd)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_DRV_VIDEO_PRIVATE_S *pstPriv = HI_NULL;
    ...
    pstWinXdrFrm->stBaseFrm.u32StillFrame =  HI_DRV_FRAME_NORMAL;
    pstWinXdrFrm->stBaseFrm.enFieldMode = HI_DRV_FIELD_ALL;

    pstPriv = (HI_DRV_VIDEO_PRIVATE_S *)pstWinXdrFrm->stBaseFrm.u32Priv;
    pstPriv->eOriginField = HI_DRV_FIELD_ALL;
    pstPriv->bForFenceUse = HI_TRUE;

    if (HI_DRV_SYS_GetTimeStampMs(&(pstWinXdrFrm->stBaseFrm.stLowdelayStat.u32WinGetFrameTime)))
    {
        DISP_ERROR("get sys time failed, not init.\n");
        return HI_FAILURE;
    }
...
#ifdef HI_VO_WIN_SYNC_SUPPORT
    s32Ret = WIN_Sync_QueueFrame(hWin, pstWinXdrFrm, pu32FenceFd);
#endif
...
HI_DRV_VIDEO_FRAME_S g_stRlsFrm = {0};
...
HI_VOID ISR_WinReleaseUSLFrame(WINDOW_S *pstWin)
{
    ...
    memset(&g_stRlsFrm, 0, sizeof(g_stRlsFrm));
    /* release useless frame */
    nRet = WinBufferGetULSFrame(&pstWin->stBuffer, &g_stRlsFrm);
    while (HI_SUCCESS == nRet)
    {
        if (HI_DRV_FRAME_VDP_ALLOCATE_STILL == g_stRlsFrm.u32StillFrame)
        {
            (HI_VOID)WIN_DestroyStillFrame(&g_stRlsFrm);
        }
        else if (pstSource->pfRlsFrame)
        {
            if ((pstSource->hSecondSrc != HI_INVALID_HANDLE)
                && (HI_DRV_VIDEO_FRAME_TYPE_DOLBY_EL == g_stRlsFrm.enSrcFrameType))
            {
                pstSource->pfRlsFrame(pstSource->hSecondSrc, &g_stRlsFrm);
            }
            else if (pstSource->hSrc != HI_INVALID_HANDLE)
            {
                pstSource->pfRlsFrame(pstSource->hSrc, &g_stRlsFrm);
            }
            else
            {
                WIN_ERROR("Cannot Release USL frame,invalid hSrc or FrameType! FrameType %d hSrc %#x\n",
                          g_stRlsFrm.enSrcFrameType,
                          pstSource->hSrc);
            }
        }
...
HI_VOID Win_UpdateHdFramInfo(WIN_HAL_PARA_S *pstLayerPara, WIN_CONFIG_S *pstCfg, HI_BOOL bSecure)
{
    ...
    if ( VDP_LAYER_VID0 == pstLayerPara->u32LayerNO )
    {
        ...
        if (pstLayerPara->pstFrame)
        {
            stHDWinInfo.enV0WinSrcPixelFmt  = pstLayerPara->pstFrame->ePixFormat;
        }
        else
        {
            stHDWinInfo.enV0WinSrcPixelFmt = HI_DRV_PIX_FMT_NV21;
        }
    }
...
HI_VOID ISR_WinConfigFrameMute(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    Win_Policy_GetPQDegradingConfigFromTEE(HI_FALSE,
                                           ...
                                           &stLayerPara.bMask,
                                           &pstWin->blastNoFrameMonitorMute,
                                           &pstWin->u32StandMuteCount,
                      ...
                      HI_BOOL bHdrMode)
{
    ...
    HI_DRV_WIN_ATTR_S *pstAttr = &pstWin->stUsingAttr;
    VIDEO_LAYER_CAPABILITY_S  stVideoLayerCap;
    DISP_INTF_OPERATION_S *pstFunction = HI_NULL;//{0};//
    ...
    if (HI_NULL != pstFunction->PF_GetPixelRepeatTimesAccordingFmt)
    {
        pstFunction->PF_GetPixelRepeatTimesAccordingFmt(pstWin->enDisp,
                pstDispInfo->eFmt,
    ...
    DRV_PQ_UpdateVdpPQ((HI_PQ_DISPLAY_E)pstWin->enDisp, &stTimingInfo, (HI_VOID *)g_pstVdpBaseAddr);
...
static HI_BOOL Win_AFDCheck(WINDOW_S *pstWin,
                                const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo,
                                HI_DRV_VIDEO_FRAME_S *pstFrame,
                                ...
                                VDP_ALG_RATIO_OUT_PARA_S* pstOutPara)
{
    if (((pstWin->stCfg.stAttrBuf.stOutRect.s32Width != 0) && (pstWin->stCfg.stAttrBuf.stOutRect.s32Height != 0))
        &&((pstWin->stCfg.stAttrBuf.stOutRect.s32Width != pstInfo->stDispInfo.stVirtaulScreen.s32Width)
        || (pstWin->stCfg.stAttrBuf.stOutRect.s32Height != pstInfo->stDispInfo.stVirtaulScreen.s32Height)))
    {
        return HI_FALSE;
    }

    // Debug Mode
    if (HI_TRUE == pstWin->stWinDebugInfo.stDebugAFDInfo.bEnable)
    {
        pstFrame->active_format = pstWin->stWinDebugInfo.stDebugAFDInfo.eActiveFormat;
    }

    if (((pstFrame->active_format < HI_DRV_AFD_TYPE_SAME_AS_SRC) || (pstFrame->active_format >= HI_DRV_AFD_TYPE_BUTT)
        || (HI_DRV_ASP_RAT_MODE_LETTERBOX != pstDrvPara->eAspMode)))
    {
        return HI_FALSE;
    }

    if (((pstDrvPara->AspectWidth != 4) || (pstDrvPara->AspectHeight != 3))
        && ((pstDrvPara->AspectWidth != 16) || (pstDrvPara->AspectHeight != 9)))
    {
        return HI_FALSE;
    }

    if (((pstDrvPara->DeviceWidth != 4) || (pstDrvPara->DeviceHeight != 3))
        && ((pstDrvPara->DeviceWidth != 16) || (pstDrvPara->DeviceHeight != 9)))
    {
        return HI_FALSE;
    }
...
static void Win_AFDProcess(HI_U32 u32AfdVal,  VDP_ALG_RATIO_DRV_PARA_S* pstDrvPara, VDP_ALG_RATIO_OUT_PARA_S* pstOutPara)
{
    const HI_DRV_AFD *pstAFDConfiguration = HI_NULL;
    const HI_DRV_AFD_OUT *stOutInfo = {0};
    HI_U32 u32Top = 0;
    HI_U32 u32Bottom = 0;
    HI_U32 u32Left = 0;
    HI_U32 u32Right = 0;

    u32AfdVal &= 0x7;// Low 3-bit value is valid

    if ((pstDrvPara->AspectWidth == 4) && (pstDrvPara->AspectHeight == 3))
    {
        pstAFDConfiguration = &stInput4x3AFDConfiguration[u32AfdVal];
    }
    else //if ((pstDrvPara->AspectWidth == 16) && (pstDrvPara->AspectHeight == 9))
    {
        pstAFDConfiguration = &stInput16x9AFDConfiguration[u32AfdVal];
    }

    if ((pstDrvPara->DeviceWidth == 4) && (pstDrvPara->DeviceHeight == 3)) // Out 4:3
    {
        stOutInfo = &pstAFDConfiguration->stOutInfo4_3;
    }
    else //if (pstDrvPara->DeviceWidth == 16 && pstDrvPara->DeviceHeight == 9) // Out 16:9
    {
        stOutInfo = &pstAFDConfiguration->stOutInfo16_9;
    }

    // Step 1: Source Image Crop
    u32Left = pstDrvPara->stInWnd.s32Width * pstAFDConfiguration->stSrcCropInfo.u32CropLeft / 10000;
    u32Right = pstDrvPara->stInWnd.s32Width * pstAFDConfiguration->stSrcCropInfo.u32CropRight / 10000;
    u32Top = pstDrvPara->stInWnd.s32Height * pstAFDConfiguration->stSrcCropInfo.u32CropTop / 10000;
    u32Bottom = pstDrvPara->stInWnd.s32Height * pstAFDConfiguration->stSrcCropInfo.u32CropBottom / 10000;

    pstDrvPara->stInWnd.s32X = u32Left;
    pstDrvPara->stInWnd.s32Y = u32Top;
    pstDrvPara->stInWnd.s32Width = pstDrvPara->stInWnd.s32Width - (u32Left + u32Right);
    pstDrvPara->stInWnd.s32Height = pstDrvPara->stInWnd.s32Height - (u32Top + u32Bottom);

    // Step 2: Shoot & Protect
    if ((stOutInfo->u32ShootProtectLeft != 0) || (stOutInfo->u32ShootProtectRight != 0))
    {
        u32Left = pstDrvPara->stInWnd.s32Width * stOutInfo->u32ShootProtectLeft / 10000;
        u32Right = pstDrvPara->stInWnd.s32Width * stOutInfo->u32ShootProtectRight / 10000;

        pstDrvPara->stInWnd.s32X = u32Left;
        pstDrvPara->stInWnd.s32Width = (0 == u32Left) ? pstDrvPara->stInWnd.s32Width
            : (pstDrvPara->stInWnd.s32Width - (u32Left + u32Right));
    }

    if ((stOutInfo->u32ShootProtectTop != 0) || (stOutInfo->u32ShootProtectBottom != 0))
    {
        u32Top = pstDrvPara->stInWnd.s32Height * stOutInfo->u32ShootProtectTop / 10000;
        u32Bottom = pstDrvPara->stInWnd.s32Height * stOutInfo->u32ShootProtectBottom / 10000;

        pstDrvPara->stInWnd.s32Y = u32Top;
        pstDrvPara->stInWnd.s32Height = (u32Top == 0) ? pstDrvPara->stInWnd.s32Height
            : (pstDrvPara->stInWnd.s32Height - (u32Top + u32Bottom));
    }

    // step 3: add LetterBox Out
    u32Left = pstDrvPara->stOutWnd.s32Width * stOutInfo->u32AddLetterBoxLeft / 10000;
    u32Right = pstDrvPara->stOutWnd.s32Width * stOutInfo->u32AddLetterBoxRight / 10000;
    u32Top = pstDrvPara->stOutWnd.s32Height * stOutInfo->u32AddLetterBoxTop / 10000;
    u32Bottom = pstDrvPara->stOutWnd.s32Height * stOutInfo->u32AddLetterBoxBottom / 10000;

    pstDrvPara->stOutWnd.s32X = u32Left;
    pstDrvPara->stOutWnd.s32Y = u32Top;
    pstDrvPara->stOutWnd.s32Width = pstDrvPara->stOutWnd.s32Width - (u32Left + u32Right);
    pstDrvPara->stOutWnd.s32Height = pstDrvPara->stOutWnd.s32Height - (u32Top + u32Bottom);
                               ...
                               WIN_HAL_PARA_S *pstLayerPara)
{
    ...
    HI_RECT_S stFrameIntermediaRect = {0}; /* 16 Byte */
    DISP_INTF_OPERATION_S *pstFunction = HI_NULL;//{0};// /* 344 Byte */
    ...
    if ((!pstPriv->bAspectProcessed) && ( stVideoLayerCap.bZme))
    {
        ...
        {
            //AFD
            if (HI_TRUE == Win_AFDCheck(pstWin, pstInfo, pstFrame, &stDrvPara, &stOutPara))
            {
                Win_AFDProcess(pstFrame->active_format, &stDrvPara, &stOutPara);
            }
            else
            {
            ...
            // proc
            pstWin->stWinDebugInfo.stDrvPara = stDrvPara;
            pstWin->stWinDebugInfo.stOutPara = stOutPara;
        }
    ...
    (HI_VOID)DISP_HAL_GetOperation(&pstFunction);
    if (HI_NULL == pstFunction->FP_GetChnBottomFlag)
    {
        WIN_INFO("stFunction->FP_GetChnBottomFlag is NULL!\n");
        return ;
    }
    pstFunction->FP_GetChnBottomFlag(pstWin->enDisp, &bBtm, &u32Vcnt);
...
WIN_HAL_PARA_S stLayerPara = {0}; /* 164 Byte */
WIN_INFO_S stWinInfo = {0}; /* 172 Byte */

HI_S32 ISR_WinConfigFrameNormal(WINDOW_S *pstWin, HI_DRV_VIDEO_FRAME_S *pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    HI_DRV_DISP_COLOR_SETTING_S stColor; /* 52 */
    HI_U32 u32Fidelity = 0;
    DISP_INTF_OPERATION_S *pstDispOpt = HI_NULL;
    HI_RECT_S stCanvas; /* 16 */
    HIDRV_DISP_NODE_DELAYTIME   *pstDispNodedelayTime = HI_NULL;
    ...
    if (HI_TRUE == stLayerPara.bRegionMute)
    {
        return HI_FAILURE;
    }
                           ...
                           const HI_DRV_DISP_CALLBACK_INFO_S *pstInfo)
{
    ...
    else
    {
        ...
        {
            if (HI_SUCCESS != ISR_WinConfigFrameNormal(pstWin, pstFrame, pstInfo))
            {
                ISR_WinConfigFrameMute(pstWin, pstFrame, pstInfo);
            }
        }
...
HI_S32 WinGetProcInfo(HI_HANDLE hWin, WIN_PROC_INFO_S *pstInfo)
{
    ...
    VdpWinDebugLogicInfo(pstInfo->u32LayerId, pstInfo->u32LayerRegionNo, (HI_VOID *)&(pstInfo->stWinDebugInfo.stWinLogicInfo));
...
HI_S32 WinProcessFrameToSP420(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U32 u32DstSmmuAddr, WIN_EXTERN_S *pstExtern)
{
    ...
    WIN_EXTERN_VPSS_Stop(pstExtern->hVpss);//After capture a frame ,stop the extern vpss
...
HI_S32 WIN_GetTechniOutputBright(HI_U32 *ps32TechniDispBright)
{
    HI_S32  s32Ret = HI_FAILURE;

    WinCheckNullPointer(ps32TechniDispBright);

    s32Ret = DISP_GetTechniDispBright(HI_DRV_DISPLAY_1, ps32TechniDispBright);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }
