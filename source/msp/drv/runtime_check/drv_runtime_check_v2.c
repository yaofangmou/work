...
extern HI_BOOL get_smcu_status(HI_VOID);
#endif
...
{
    *pu32StartAddr = virt_to_phys(_stext);
    *pu32EndAddr = virt_to_phys(_etext);
...
static int send_check_vector(void)
{
    ...
    //HI_FATAL_RTC("u32KernelStartAddr: 0x%x u32KernelEndAddr: 0x%x\n", u32KernelStartAddr, u32KernelEndAddr);
    //HI_FATAL_RTC("g_u32ModulePhyAddr: 0x%x g_u32ModulePhyAddr + g_u32ModuleUsedSize: 0x%x\n", g_u32ModulePhyAddr, g_u32ModulePhyAddr + g_u32ModuleUsedSize);
