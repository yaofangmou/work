...
#define YXSB_WATERMARK_CODE                        0x1B1  /* 等同于user data */
...
#define MPEG2DEC_ASSERT_RET(cond, else_print)                            \
do {                                                                \
    if (!(cond))                                                    \
    {                                                               \
        dprint(PRN_FATAL,"mpeg2.c,L%d: %s\n", __LINE__, else_print ); \
        return MPEG2_ERROR;                                             \
    }                                                               \
}while(0)
...
typedef struct
{
    ...
    UINT32        FakeFrmWidthInMb;    //add for fake frame
