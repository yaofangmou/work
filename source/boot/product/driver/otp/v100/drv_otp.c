...
HI_S32 DRV_OTP_SetTrustZoneEnable(HI_VOID)
{
    HI_S32 ret  = HI_FAILURE;
    HI_U32 id_word = 0;

    ret = HAL_OTP_Read(OTP_ADVCA_ID_WORD_ADDR, &id_word);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_OTP("failed to get id word\n");
        return HI_FAILURE;
    ...
    if (ADVCA_ID_WORD != id_word)
    {
        HI_ERR_OTP("It's not secure chipset, can not enable tee!\n");
        return HI_FAILURE;
    }

    ret = HAL_OTP_WriteByte(OTP_INTERNAL_PV_1, 0x02);
    if (HI_SUCCESS != ret)
    {
    ...
    //lock fuse.
    ret = HAL_OTP_WriteByte(OTP_INTERNAL_PVLOCK_1, 0x02);
...
HI_S32 DRV_OTP_GetTrustZoneStat(HI_BOOL *pbEnable)
{
    ...
    ret = HAL_OTP_Read(OTP_INTERNAL_PV_1, &value);
    if (HI_SUCCESS != ret)
    {
    ...
    *pbEnable = ((value & 0x02) == 0x02) ? HI_TRUE : HI_FALSE;
