...
static HI_U32 u32LPF_KHz[TUNER_NUM] = {0};
...
HI_S32 av2018_init_tuner(HI_U32 u32TunerPort)
{
    ...
    reg[10]=0x88;
    reg[11]=0xb4;
    reg[12]=0xd6; //[bit6]RFLP=ON at Power on initial
    reg[13]=0x40;
    reg[14]=0x94;
    reg[15]=0x4a;
    //#else
    ...
    reg[37]=0x02; //Enable FT-function at Power on initial
    reg[38]=0xab;
    reg[39]=0x97;
    reg[40]=0xc5;
    reg[41]=0xa8;
...
HI_S32 av2018_set_tuner(HI_U32 u32TunerPort, HI_U8 enI2cChannel, HI_U32 u32PuRF/*kHz*/)  //freq :unit MHZ, symbol:unit  Khz
{
    ...
    //HI_U8  auto_scan=0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 /*u32RFfreq_MHz = 0,*/ tuner_crystal_kHz, round_div;
    ...
    if((u32PuRF % tuner_crystal_kHz) > (tuner_crystal_kHz-501))
    {
		  u32PuRF /= 1000;
		  u32PuRF *= 1000;
		  u32PuRF += 1000;
    }
	else if((u32PuRF % tuner_crystal_kHz) < 500)
    {
		  u32PuRF /= 1000;
		  u32PuRF *= 1000;
	}
    ...
    fracN = ((u32PuRF / 100) << 17) / (tuner_crystal_kHz / 100);
    fracN = fracN & 0x1ffff;
    reg[1] = (fracN >> 9) & 0xff;
    reg[2] = (fracN >> 1) & 0xff;
    // reg[3]_D7 is frac<0>, D6~D0 is 0x50

    reg[3] = (((fracN << 7) & 0x80) | 0x50 | 0x4);	// default is 0x50

    {
        ...
        reg_start = 2;
        av2018_write(u32TunerPort, enI2cChannel, tuner_address, reg_start, reg + 2, 1);
        reg_start = 1;
        av2018_write(u32TunerPort, enI2cChannel, tuner_address, reg_start, reg + 1, 1);
        reg_start = 0;
        av2018_write(u32TunerPort, enI2cChannel, tuner_address, reg_start, reg, 1);
        tuner_mdelay(100);
        reg_start = 3;
        av2018_write(u32TunerPort, enI2cChannel, tuner_address, reg_start, reg + 3, 1);
        reg_start = 2;
        ...
        if (0 == (u32PuRF % tuner_crystal_kHz))
        {
            reg[30] = 0xe5;
        }
        else
        ...
        reg_start = 30;
        av2018_write(u32TunerPort, enI2cChannel, tuner_address, reg_start, reg + 30, 1);

        tuner_mdelay(4);

        av2018_get_lpf(u32TunerPort, &BW);
        BW_CLK_KHz = (tuner_crystal_kHz / 2 + 64 / 2) / 64;
        ...
        BF = (BW * 127 + 100 * BW_CLK_KHz / 2) / (100 * BW_CLK_KHz); // 	BW * 1.27 / 211KHz
        if (BF > 0xff)
            BF = 0xff;
...
HI_VOID av2018_set_lpf(HI_U32 u32TunerPort, HI_U32 u32LPFKHz)
{
    u32LPF_KHz[u32TunerPort] = u32LPFKHz;
    return;
}

HI_S32  av2018_get_lpf(HI_U32 u32TunerPort, HI_U32 *pu32LPFKHz)
{
    HI_TUNER_CHECKPOINTER(pu32LPFKHz);

    *pu32LPFKHz = u32LPF_KHz[u32TunerPort];
    return HI_SUCCESS;
}
