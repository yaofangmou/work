...
static int _HI_Flash_Init(void)
{
	...
	chars_len = strlen(bootargs) + 1;
	if (chars_len < strlen(bootargs)) {
		ERR_FALSH_TRACE("Integer overflow\n");
		goto fail;
	}

	flash_part = (char *)malloc(chars_len);
	if (!flash_part) {
		ERR_FALSH_TRACE("Fail to malloc for bootargs!\n");
		goto fail;
	}
	strncpy(flash_part, bootargs, chars_len);

	bootargs_tmp = (char *)malloc(chars_len);
	if (!bootargs_tmp) {
		ERR_FALSH_TRACE("Fail to malloc for bootargs!\n");
		goto fail;
	}
	strncpy(bootargs_tmp, bootargs, chars_len);
		...
		HI_S32 s32Withoob )
{
	...
	if (s32Withoob) {
		...
		if (u32PageOOBOffset) {
			if ((u32PageSize + nand->oobused) <= 0) {
				ERR_FALSH_TRACE("Invalid parameter.");
				return HI_FAILURE;
			}
