...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	|| defined(CONFIG_ARCH_HI3796MV2X) || defined (CONFIG_ARCH_HI3798MV310)  || defined (CONFIG_ARCH_HI3716MV430)
...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_NONE:
		if (load == blob_start || load == image_start) {
			...
			dcache_enable(0);
#endif
			printf ("   Loading %s from 0x%lx to 0x%lx ... ", type_name, image_start, load);
...
#if defined(CFG_DTBO_ADDR)
	snprintf(cmdstr, sizeof(cmdstr), "bootm 0x%X - 0x%X", addr, CONFIG_DTB_LOAD_ADDR + 0x40);
#else
