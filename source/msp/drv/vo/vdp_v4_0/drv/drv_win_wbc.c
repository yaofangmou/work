...
HI_VOID ISOGENY_UpDateHDWinFramInfo(WBC_HD_WIN_INFOR_S* pstHDWinInfo, HI_U32 u32VideoLayer)
{
    if ( 0 == u32VideoLayer )
    {
        ...
        stWnHDFrameInfor.enV0WinSrcPixelFmt = pstHDWinInfo->enV0WinSrcPixelFmt;
...
{
    HI_VDP_PQ_INFO_S stTimingInfo = {0};
    ...
    DRV_PQ_UpdateVdpPQ((HI_PQ_DISPLAY_E)HI_DRV_DISPLAY_0, &stTimingInfo, (HI_VOID *)g_pstVdpBaseAddr);
                                                      ...
                                                      WIN_INFO_S     *pstWinInfo)
{
    ...
    if (HI_NULL != pstWinPara->pstFrame)
    {
        pstWinInfo->bSecure   = pstWinPara->pstFrame->bSecure;
    }
    else
    {
...
HI_VOID ISR_SlaveWinConfigFrame( HI_DRV_VIDEO_FRAME_S* pstFrame, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo)
{
    ...
    if (HI_NULL != pstFrame)
    {
        ...
        stLayerPara.pstFrame->bSecure = pstFrame->bSecure;
...
HI_DRV_FIELD_MODE_E  ISOGBNY_GetWbcOutputFieldMode(HI_VOID)
{
    ...
    {
        pstFunction->FP_GetChnBottomFlag(HI_DRV_DISPLAY_0, &bBtm, &u32Vcnt);
    }
    else
    {
...
HI_VOID ISOGBNY_ConfigWbcConvert(WIN_WBC_CONFIG_S* pWBCConfig, ISOGBNY_WBC_CFG_S* pstWBC0Cfg)
{
    ...
    pWBCConfig->eSrcPixelFmt = pstWBC0Cfg->eSrcPixelFmt;
...
HI_S32 ISOGBNY_ConfigWbc0(ISOGBNY_WBC_CFG_S* pstWBC0Cfg, HI_DRV_VIDEO_FRAME_S* pstCfgFrame)
{
    ...
    pstWBC0Cfg->eSrcPixelFmt = stHDWinInfo.enV0WinSrcPixelFmt;
