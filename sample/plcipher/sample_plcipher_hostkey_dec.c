...
{
    printf(" usage: sample_plcipher_Dec configfile file_to_decrypt file_decrypted pid1 pid2 pid3 pid4 pid5 \n");
    printf(" examples: \n");
    printf(" sample_plcipher_Dec cw_key_config.ini encrypted.ts plain.ts 0x20 0x21\n");
}
...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    while((ReadLen=fread(EncrytedBuf1.user_viraddr, 1, MAXBB, pEncInTsFile)) != 0)
    {
        ...
        Ret = HI_UNF_PLCIPHER_Decrypt(hPLCipher, (HI_VOID *)((HI_ULONG)(EncrytedBuf1.phyaddr)), (HI_VOID *)((HI_ULONG)PlainBuf1.phyaddr), ReadLen);
        if (HI_SUCCESS != Ret)
        {
