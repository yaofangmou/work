...
#define LOG_D_MODULE_ID            HI_ID_AVPLAY
...
#define  AVPLAY_PVR_TPLAYTIME_REPEAT_COUNT  1
#define  AVPLAY_STEP_PLAY_COUNT             1

#define  AVPLAY_MAX_AUD_TPLAY_SPEED      2000
#define  AVPLAY_MIN_AUD_TPLAY_SPEED      200
...
#define  HI_AVPLAY_DEBUG_FUNC_ENTER()       HI_FuncEnter()
#define  HI_AVPLAY_DEBUG_FUNC_EXIT()        HI_FuncExit()

#define  HI_AVPLAY_CHECK_PARA(value)\
    do \
    { \
        if (value)\
        {\
            LOG_PrintError("ERR: %s\n",#value);\
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_INVALID_PARA);\
            return HI_ERR_AVPLAY_INVALID_PARA;\
        }\
    }while (0)

#define  HI_AVPLAY_CHECK_NULL_PTR(ptr)\
    do \
    { \
        if (HI_NULL == ptr)\
        {\
            LOG_PrintError("ERR: ptr is null\n");\
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_NULL_PTR);\
            return HI_ERR_AVPLAY_NULL_PTR;\
        }\
    }while (0)

#define  HI_AVPLAY_CHECK_INVALID_HANDLE(handle)\
    do \
    { \
        if (HI_INVALID_HANDLE == handle)\
        {\
            LOG_PrintError("ERR: handle is invalid\n");\
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_INVALID_PARA);\
            return HI_ERR_AVPLAY_INVALID_PARA;\
        }\
    }while (0)
...
#define AVPLAY_BASIC_SUPPORT
#endif

#ifndef CHIP_TYPE_hi3716mv430
#define AVPLAY_MS12_SUPPORT
#endif
...
typedef struct
{
    ...
    HI_DRV_VDEC_FRAME_S             IFrame;
#else
    HI_DRV_VIDEO_FRAME_S            CurFrame;
    HI_U32                          CurFrameSendCount;
    HI_DRV_VIDEO_FRAME_S            LastFrame;
    HI_DRV_VIDEO_FRAME_S            IFrame;
#endif
...
static HI_S32 AVPLAY_ConvertFrameAndSave(HI_DRV_VIDEO_FRAME_S* pFrame);
#endif
...
HI_VOID AVPLAY_Usleep(HI_U32 TimeUs)
{
    HI_S32 Ret      = HI_FAILURE;
    struct timespec SleepTime;

    SleepTime.tv_sec = 0;
    SleepTime.tv_nsec = TimeUs * 1000;
...
HI_S32 AVPLAY_GetStatusInfo(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_STATUS_INFO_S *pstStatusInfo)
{
    ...
    if (   (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        && ((HI_INVALID_HANDLE != pAvplay->hAdec) && pAvplay->AudEnable))
    {
...
#ifdef AVPLAY_MS12_SUPPORT
    else if ((AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType) && pAvplay->AudEnable)
    {
        ...
        && (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType) )
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_AO_Track_GetDelayMs, Ret);
            return Ret;
        }
    ...
    if ((HI_INVALID_HANDLE != pAvplay->hVdec) && pAvplay->VidEnable)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_VDEC_GetChanStatusInfo, Ret);
            return Ret;
        }
        ...
        if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_PrintFuncResult(HI_MPI_DMX_GetPESBufferStatus, Ret);
                return Ret;
            }
    ...
    if (HI_INVALID_HANDLE != pAvplay->hSync)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_SYNC_GetStatus, Ret);
            return Ret;
        }
    }
    else
...
{
    HI_S32  Ret;
    VDEC_BUFFER_ATTR_S  VdecBufAttr;

    memset(&VdecBufAttr, 0, sizeof(VdecBufAttr));
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_AVPLAY("HI_MPI_VDEC_SetTVP failed 0x%x\n", Ret);
        return Ret;
    }

    VdecBufAttr.u32BufSize = (HI_UNF_AVPLAY_STREAM_TYPE_ES == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        ? pAvplay->AvplayAttr.stStreamAttr.u32VidBufSize : 0;
    VdecBufAttr.bTvp = pAvplay->TVPAttr.bEnable;
#endif

    if (   (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        || (0 != pAvplay->AvplayAttr.stStreamAttr.u32VidBufSize) )
    {
        HI_U32 BufSize = (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
            ? 0 : pAvplay->AvplayAttr.stStreamAttr.u32VidBufSize;

        HI_U32 hDmxVid = (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
            ? pAvplay->hDmxVid : HI_INVALID_HANDLE;

        Ret = HI_MPI_VDEC_ChanBufferInit(hVdec, BufSize, hDmxVid, &VdecBufAttr);
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_VDEC_ChanBufferInit, Ret);
            return Ret;
        }
    }
...
HI_S32 AVPLAY_FreeVidBuffer(AVPLAY_S *pAvplay, HI_HANDLE hVdec)
{
    ...
    if (0 != pAvplay->AvplayAttr.stStreamAttr.u32VidBufSize)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_VDEC_ChanBufferDeInit, Ret);
            return Ret;
        }
    }
...
HI_VOID AVPLAY_Notify(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_EVENT_E EvtMsg, HI_VOID *pEvtPara)
{
    ...
    if (pAvplay->EvtCbFunc64Flag)
    {
        if (pAvplay->EvtCbFunc64[EvtMsg])
        {
            ...
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(pAvplay->EvtCbFunc64, Ret);
            }
        }
    }
#ifndef __aarch64__
...
HI_UNF_AVPLAY_BUF_STATE_E AVPLAY_CaclBufState(const AVPLAY_S *pAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn, HI_U32 UsedBufPercent)
{
    ...
    if (HI_UNF_AVPLAY_MEDIA_CHAN_AUD == enChn)
    {
        ...
        else if (UsedBufPercent < AVPLAY_ES_AUD_LOW_PERCENT)
        {
            ...
            FrameTime = AdecStatusinfo.u32UsedBufNum * pAvplay->AudInfo.FrameTime;

            if (UsedBufPercent < AVPLAY_ES_AUD_EMPTY_PERCENT)
            {
                if (AoBufTime + FrameTime <= 150)
                {
            ...
            else
            {
                ...
                else if (AoBufTime + FrameTime <= 240)
                {
                    CurBufState = HI_UNF_AVPLAY_BUF_STATE_LOW;
                }
            }
    ...
    else
    {
        ...
        else if (UsedBufPercent < AVPLAY_ES_VID_LOW_PERCENT)
        {
            ...
            {
                StreamTime = (VdecFrmBuf.u32StrmSize * 1000)/(VdecFrmBuf.u32StrmInBps);
            }
...
HI_VOID AVPLAY_ProcRenderTrack(AVPLAY_S *pAvplay)
{
    ...
    {
        Ret = HI_MPI_AO_Track_GetInfo(pAvplay->hRenderTrack, TRACK_INFO_EOSSTATEFLAG, &IsAudEos);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("call HI_MPI_AO_Track_GetInfo fail:%d\n", Ret);
            return;
        }

        if (IsAudEos)
        {
            HI_INFO_AVPLAY("Aud report eos\n");
            return;
        }
    }
...
HI_VOID AVPLAY_ProcVidFrameRate(AVPLAY_S *pAvplay, HI_DRV_VIDEO_FRAME_S *pVidFrameInfo)
{
   /*
    * Private video playback:
    * 1.Set the input frame rate is equal to the output frame rate;
    ...
    */
    ...
    pVidFrameInfo->u32FrameRate = WinInfo.u32DispRate * 10;
#endif
...
HI_VOID AVPLAY_ProcVidFrc(AVPLAY_S *pAvplay)
{
    ...
    pVideoPriv = (HI_DRV_VIDEO_PRIVATE_S*)(pAvplay->CurFrame.u32Priv);
#endif
    ...
    {
#ifndef  AVPLAY_BASIC_SUPPORT
        if ((HI_DRV_FIELD_TOP == pVideoPriv->eOriginField) || (HI_DRV_FIELD_BOTTOM == pVideoPriv->eOriginField))
        {
    ...
    {
#ifdef  AVPLAY_BASIC_SUPPORT
        pAvplay->FrcParamCfg.u32InRate = pAvplay->CurFrame.u32FrameRate / 10;
#else
...
HI_VOID AVPLAY_ProcVidSync(AVPLAY_S *pAvplay)
{
...
#else
    hWindow = pAvplay->MasterFrmChn.hWindow;
    pFrmPriv = (HI_DRV_VIDEO_PRIVATE_S *)(pAvplay->CurFrame.u32Priv);

    pAvplay->VidInfo.SrcPts = pAvplay->CurFrame.u32SrcPts;
    pAvplay->VidInfo.Pts = pAvplay->CurFrame.u32Pts;

    if (0 != pAvplay->CurFrame.u32OriFrameRate)
    {
        pAvplay->VidInfo.FrameTime = 1000000 / pAvplay->CurFrame.u32OriFrameRate;
    }
    else
    {
        pAvplay->VidInfo.FrameTime = 40;
...
#ifdef AVPLAY_BASIC_SUPPORT
        if (   (HI_UNF_SYNC_PROC_REPEAT == stVidPlaytime.enSyncProc)
            && (SYNC_PROC_REPEAT != pAvplay->VidOpt.SyncProc))
        {
            pAvplay->VidOpt.Repeat = AVPLAY_PVR_TPLAYTIME_REPEAT_COUNT;
        }
#endif
    ...
    pAvplay->CurFrame.enTBAdjust = pAvplay->VidOpt.enTBAdjust;

    if (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
    {
        pAvplay->CurFrame.enTBAdjust = HI_DRV_VIDEO_TB_PLAY;
    }
#else
    for (i = 0; i < pAvplay->CurFrmPack.u32FrmNum; i++)
    {
    ...
    if (pAvplay->bDualChnPlay)
    {
        ...
        if (HI_TRUE == pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO])
        {
            if (HI_FALSE == pAvplay->bSendedFrmToMasterWin)
            {
                ...
                if ( (HI_INVALID_HANDLE != pAvplay->hElVdec) && (-1 != ElPortId)  && (HI_TRUE != pAvplay->bMixed) )
                {
        ...
        else
        {
            ...
            if ( (HI_TRUE == pAvplay->bElFrmProcFlag) && (-1 != ElPortId)  && (HI_TRUE != pAvplay->bMixed) )
            {
                Ret = HI_MPI_VDEC_ReleaseFrame(pAvplay->hElPort, &pAvplay->CurElFrmPack.stFrame[ElPortId].stFrameVideo);
                if (HI_SUCCESS != Ret)
                {
...
HI_VOID AVPLAY_ProcHDRMixPlay(AVPLAY_S *pAvplay)
{
    HI_S32             Ret;
    HI_U32             BlPortId;
    Ret = AVPLAY_GetBlFramePort(pAvplay, pAvplay->hBlPort, &BlPortId);
    ...
    else if (HI_ERR_VO_BUFQUE_FULL != Ret)
    {
        ...
        HI_INFO_AVPLAY("HI_ERR_VO_BUFQUE ERR :%d\n",Ret);
        pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO] = HI_FALSE;
...
HI_VOID AVPLAY_ProcHDRPlay(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        ...
        if (pAvplay->bMixed)
        {
            (HI_VOID)AVPLAY_ProcHDRMixPlay(pAvplay);
            return;
        }
...
HI_VOID AVPLAY_ProcHDRMixQuickOutput(AVPLAY_S *pAvplay)
{
    HI_S32             Ret;
    HI_U32             BlPortId;
    Ret = AVPLAY_GetBlFramePort(pAvplay, pAvplay->hBlPort, &BlPortId);
...
HI_VOID AVPLAY_ProcHDRQuickOutput(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        if (pAvplay->bMixed)
        {
            (HI_VOID)AVPLAY_ProcHDRMixQuickOutput(pAvplay);
            return;
        }
...
HI_VOID AVPLAY_ProcHDRMixRepeat(AVPLAY_S *pAvplay)
{
    HI_S32             Ret;
    HI_U32             BlPortId = 0;
    Ret = AVPLAY_GetBlFramePort(pAvplay, pAvplay->hBlPort, &BlPortId);
    ...
    if (0 == pAvplay->LstBlFrmPack.u32FrmNum)
    {
        return;
    }
...
HI_VOID AVPLAY_ProcHDRRepeat(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        ...
        if (pAvplay->bMixed)
        {
            (HI_VOID)AVPLAY_ProcHDRMixRepeat(pAvplay);
            return;
        }
...
HI_VOID AVPLAY_ProcHDRMixDiscard(AVPLAY_S *pAvplay)
{
    HI_S32             Ret;
    HI_U32             BlPortId;
    Ret = AVPLAY_GetBlFramePort(pAvplay, pAvplay->hBlPort, &BlPortId);
...
HI_VOID AVPLAY_ProcHDRDiscard(AVPLAY_S *pAvplay)
{
    ...
    if (HI_TRUE == pAvplay->bDualChnPlay)
    {
        if (pAvplay->bMixed)
        {
            (HI_VOID)AVPLAY_ProcHDRMixDiscard(pAvplay);
            return;
        }
...
HI_VOID AVPLAY_ProcBlFrame(AVPLAY_S *pAvplay)
{
    ...
    if(pAvplay->AvplayHDRAttr.enHDRStreamType == HI_UNF_AVPLAY_HDR_STREAM_TYPE_DL_SINGLE_VES)
    {
        if ((pAvplay->CurBlFrmPack.stFrame[0].stFrameVideo.enSrcFrameType != HI_DRV_VIDEO_FRAME_TYPE_DOLBY_BL)
        || (!pAvplay->CurBlFrmPack.stFrame[0].stFrameVideo.DVDualLayer))
        {
            pAvplay->bMixed = HI_TRUE;
            pAvplay->BlSyncProc = AVPLAY_HDR_SYNC_PROC_PLAY;
            return;
        }
    }

    pAvplay->bMixed = HI_FALSE;
    ...
    if (pAvplay->bMixed)
    {
        if (pAvplay->bElFrmProcFlag)
        {
            ...
            if (HI_SUCCESS == Ret)
            {
                ...
                memset(&pAvplay->CurElFrmPack.stFrame[PortId].stFrameVideo, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));
            }
...
HI_S32 AVPLAY_ReceiveVidFrame(AVPLAY_S *pAvplay, HI_U32 TimeoutMs)
{
    ...
    Ret = HI_MPI_VDEC_ReceiveFrame(pAvplay->hVdec, &pAvplay->CurFrame, TimeoutMs);
    if (HI_SUCCESS != Ret)
    {
    ...
    if (pAvplay->IsCaptureYUVFrame == HI_TRUE)
    {
        pAvplay->IsCaptureYUVFrame = HI_FALSE;
        Ret = AVPLAY_ConvertFrameAndSave(&pAvplay->CurFrame);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("Call AVPLAY_ConvertFrameAndSave Failed!\n", Ret);
            return HI_FAILURE;
        }
    }
    ...
    HI_INFO_AVPLAY("=====Receive a new frame, Sys=%u, Id=%u, Pts=%u, FrameType=%d =====\n",
        AVPLAY_GetSysTime(), pAvplay->CurFrame.u32FrameIndex,
        pAvplay->CurFrame.u32Pts,
        pAvplay->CurFrame.enSrcFrameType);
...
HI_VOID AVPLAY_ProcVidPlay(AVPLAY_S *pAvplay)
{
    ...
    {
        HI_ERR_AVPLAY("master win is invalide\n");
        return;
    }

    pAvplay->DebugInfo.MasterVidStat.SendNum++;
    pAvplay->bSendedFrmToMasterWin = HI_TRUE;
    pAvplay->CurFrameSendCount++;

    AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->CurFrame);

    Ret = HI_MPI_WIN_QueueFrame(hWindow, &pAvplay->CurFrame);
    if (HI_SUCCESS == Ret)
    {
        /* record low delay event */
        LdEvent.evt_id = EVENT_AVPLAY_FRM_OUT;
        LdEvent.frame = pAvplay->CurFrame.u32FrameIndex;
        LdEvent.handle = pAvplay->CurFrame.hTunnelSrc;
        (HI_VOID)HI_SYS_GetTimeStampMs(&(LdEvent.time));

        (HI_VOID)HI_MPI_STAT_NotifyLowDelayEvent(&LdEvent);

        /* record program switch event */
        if (pAvplay->CurFrame.bIsFirstIFrame)
        {
            HI_MPI_STAT_Event(STAT_EVENT_VOGETFRM, 0);
        }
    ...
    else if (HI_ERR_VO_BUFQUE_FULL != Ret)
    {
        ...
        Ret = HI_MPI_VDEC_ReleaseFrame(pAvplay->hVdec,
            &pAvplay->CurFrame);
        if (HI_SUCCESS != Ret)
        {
...
{
    HI_FRAMEPLAYINFOR_S *pstPlayInfor = &pAvplay->CurFrame.stPlayInfor;

    switch (pAvplay->VidOpt.SyncProc)
    {
        ...
        {
            pstPlayInfor->u32PlayCnt = pAvplay->FrcNeedPlayCnt;
            break;
        }

        case SYNC_PROC_REPEAT:
        {
            pstPlayInfor->u32PlayCnt = pAvplay->FrcNeedPlayCnt + pAvplay->VidOpt.Repeat;
            break;
        }

        default:
            HI_ERR_AVPLAY("SYNC_PROC is Invalide:0x%x\n",pAvplay->VidOpt.SyncProc);
            pstPlayInfor->u32PlayCnt = pAvplay->FrcNeedPlayCnt;
            break;
    }

    HI_INFO_AVPLAY("===QueueVidFrame:PlayCnt:%d, FrcNeedPlayCnt:%d, Repeatcnt%d\n",
        pstPlayInfor->u32PlayCnt, pAvplay->FrcNeedPlayCnt, pAvplay->VidOpt.Repeat);

    (HI_VOID)AVPLAY_ProcVidPlay(pAvplay);
...
HI_S32 AVPLAY_ReceiveVidFrame(AVPLAY_S *pAvplay, HI_U32 TimeoutMs)
{
...
#endif
    {
        ...
        {
            pAvplay->IsCaptureYUVFrame = HI_FALSE;
            Ret = AVPLAY_ConvertFrameAndSave(&pAvplay->CurFrmPack.stFrame[0].stFrameVideo);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("Call AVPLAY_ConvertFrameAndSave Failed!\n", Ret);
                return HI_FAILURE;
...
HI_VOID AVPLAY_CorrectFrameTime(AVPLAY_S *pAvplay)
{
    ...
    if ((HI_DRV_FIELD_TOP == pstFrmPriv->eOriginField) || (HI_DRV_FIELD_BOTTOM == pstFrmPriv->eOriginField))
    {
...
{
    HI_DRV_VIDEO_FRAME_S          *pCurVdecFrame = HI_NULL;
#ifndef AVPLAY_BASIC_SUPPORT
    HI_DRV_VIDEO_FRAME_S          *pLstVdecFrame = HI_NULL;
...
#ifdef AVPLAY_BASIC_SUPPORT
    pCurVdecFrame = &pAvplay->CurFrame;
#else
    pCurVdecFrame = &pAvplay->CurFrmPack.stFrame[0].stFrameVideo;
    pLstVdecFrame = &pAvplay->LstFrmPack.stFrame[0].stFrameVideo;
#endif
    AVPLAY_DRV2UNF_VidFrm(pCurVdecFrame, &VidFrm, &VidFrmStatusInfo);

    pstFrmPriv = (HI_DRV_VIDEO_PRIVATE_S *)(pCurVdecFrame->u32Priv);
    if (   (HI_DRV_LAST_FRAME_FLAG == pstFrmPriv->u32LastFlag)
        || (HI_DRV_LAST_ERROR_FLAG == pstFrmPriv->u32LastFlag)
        )
    {
        FlsFlag = HI_TRUE;
...
#ifndef AVPLAY_BASIC_SUPPORT
    if (   (pCurVdecFrame->bIsFirstIFrame)
        || (pCurVdecFrame->DispCtrlWidth  != pLstVdecFrame->DispCtrlWidth)
        || (pCurVdecFrame->DispCtrlHeight != pLstVdecFrame->DispCtrlHeight) )
    {
        if (   (AVPLAY_HDCP_RESOLUTION_UNRESTRICTED == pCurVdecFrame->DispCtrlWidth)
            && (AVPLAY_HDCP_RESOLUTION_UNRESTRICTED == pCurVdecFrame->DispCtrlHeight))
        {
        ...
        else if (   (AVPLAY_HDCP_RESOLUTION_MUTE == pCurVdecFrame->DispCtrlHeight)
                 && (AVPLAY_HDCP_RESOLUTION_MUTE == pCurVdecFrame->DispCtrlWidth))
        {
        ...
        else
        {
            ...
            HdcpPara.u32ImageWidth  = pCurVdecFrame->DispCtrlWidth;
            HdcpPara.u32ImageHeight = pCurVdecFrame->DispCtrlHeight;
        }
    ...
    if(!((pAvplay->VidInfo.bProgressive == HI_FALSE)&&
      (pCurVdecFrame->bProgressive== HI_TRUE)&&
      (pAvplay->LstFrmPack.u32FrmNum != 0)&&
      (pAvplay->LstFrmPack.stFrame[0].stFrameVideo.u32FrameIndex == pCurVdecFrame->u32FrameIndex)))
    {
        VidFrmStatusInfo.u32DecoderBuffer = AVPLAY_DECODER_BUFFER_NORMAL;

        if (pCurVdecFrame->bDiscard)
        {
    ...
    LdEvent.frame  = pCurVdecFrame->u32FrameIndex;
    LdEvent.handle = pCurVdecFrame->hTunnelSrc;
    (HI_VOID)HI_SYS_GetTimeStampMs(&(LdEvent.time));
...
HI_VOID AVPLAY_ProcVdecToVo(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->bStepMode)
    {
        ...
        {
#ifdef AVPLAY_BASIC_SUPPORT
            pAvplay->CurFrame.stPlayInfor.u32PlayCnt = AVPLAY_STEP_PLAY_COUNT;
#endif
            AVPLAY_ProcVidPlay(pAvplay);
...
#ifdef AVPLAY_BASIC_SUPPORT
    if (0 == pAvplay->CurFrameSendCount)
#else
    if (0 == pAvplay->FrcCurPlayCnt)
#endif
    {
...
#else
    HI_INFO_AVPLAY("sys:%u, frm:%d, need:%u, cur:%u, sync:%u, delay:%u\n",
        AVPLAY_GetSysTime(),pAvplay->CurFrame.u32FrameIndex, pAvplay->FrcNeedPlayCnt,
        pAvplay->FrcCurPlayCnt, pAvplay->VidOpt.SyncProc, pAvplay->VidInfo.DelayTime);
#endif
...
HI_VOID AVPLAY_GetAudBufState(AVPLAY_S *pAvplay, HI_U32 *pAudBufPercent, HI_UNF_AVPLAY_BUF_STATE_E *pCurAudBufState)
{
    ...
    HI_U32                             AudBufPercent = 0;
    HI_U32                             AdecDelay = 0;
    HI_U32                             DmxAudBufPercent = 0;
    HI_U32                             AdecBufPercent = 0;
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
         if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
         {
             ...
             if (HI_SUCCESS != Ret)
             {
                 HI_ERR_AVPLAY("call HI_MPI_DMX_GetPESBufferStatus fail:%d\n", Ret);
                 return;
             }
             ...
             if (HI_SUCCESS != Ret)
             {
                 HI_ERR_AVPLAY("call HI_MPI_ADEC_GetDelayMs fail:%d", Ret);
                 return;
             }

             if (AudChnBuf.u32BufSize > 0)
             {
                 DmxAudBufPercent = AudChnBuf.u32UsedSize * 100 / AudChnBuf.u32BufSize;
             }
             else
             {
                 DmxAudBufPercent = 0;
             ...
             if (AdecDelay > AVPLAY_ADEC_MAX_DELAY)
             {
                 AdecBufPercent = 100;
             }
             else
             {
                 AdecBufPercent = AdecDelay * 100 / AVPLAY_ADEC_MAX_DELAY;
             }

             /* In ts Mode adec use 2/3 audio buffer, dmx use 1/3 buffer */
             AudBufPercent = (AdecBufPercent * 2 + DmxAudBufPercent) / 3;
         }
         else
         {
             Ret = HI_MPI_ADEC_GetInfo(pAvplay->hAdec, HI_MPI_ADEC_BUFFERSTATUS, &AdecBuf);
             if (HI_SUCCESS != Ret)
             {
                HI_ERR_AVPLAY("call HI_MPI_ADEC_GetInfo fail:%d", Ret);
                return;
             }
    ...
    {
        HI_U32 RenderUsedBuff = 0;

        AVPLAY_GetRenderBufPercent(pAvplay, &AudBufPercent, &RenderUsedBuff);
        CurAudBufState = AVPLAY_CaclRenderBufState(pAvplay, AudBufPercent, RenderUsedBuff);
    }
#endif
...
#else
        AVPLAY_NotifyVidFrmDisplayInfo(pAvplay, pAvplay->MasterFrmChn.hWindow, pAvplay->CurFrame.u32Pts);
#endif
        pAvplay->bQueuedVideoFrame = HI_FALSE;
    }
...
HI_VOID AVPLAY_ResetProcFlag(AVPLAY_S *pAvplay)
{
    ...
    pAvplay->IFrame.hport = HI_INVALID_HANDLE;
    memset(&pAvplay->IFrame.stFrameVideo, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));

    pAvplay->bJumpPlayFinish = HI_FALSE;
#else
    memset(&pAvplay->LastFrame, 0, sizeof(HI_DRV_VIDEO_FRAME_S));
    memset(&pAvplay->IFrame, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));
#endif
...
HI_S32 AVPLAY_PcrEnable(AVPLAY_S *pAvplay, HI_U32 PcrPid)
{
    HI_S32  Ret;
    ...
    if ((HI_INVALID_HANDLE == pAvplay->hDmxPcr) && (DEMUX_INVALID_PID != PcrPid))
    {
        ...
        Ret = HI_MPI_DMX_PcrPidSet(pAvplay->hDmxPcr, PcrPid);
        if (Ret != HI_SUCCESS)
        {
...
HI_S32 AVPLAY_SetPid(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, const HI_U32 Pid)
{
    ...
    {
        if ((HI_UNF_AVPLAY_STATUS_PLAY == pAvplay->CurStatus) || (HI_UNF_AVPLAY_STATUS_PREPLAY == pAvplay->CurStatus))
        {
            if (pAvplay->hDmxPcr != HI_INVALID_HANDLE)
            {
            ...
            {
                Ret = AVPLAY_PcrEnable(pAvplay, Pid);
                if (Ret != HI_SUCCESS)
                {
                    HI_ERR_AVPLAY("call AVPLAY_PcrEnable failed 0x%x\n", Ret);
                    return Ret;
                }
            }
        ...
        else if (HI_UNF_AVPLAY_STATUS_STOP == pAvplay->CurStatus)
        {
            pAvplay->DmxPcrPid = Pid;
        }
        else
        {
            HI_ERR_AVPLAY("avplay state isinvalid, can not set pcr pid.\n");
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }
...
HI_S32 AVPLAY_SetLowDelay(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_LOW_DELAY_ATTR_S *pstAttr)
{
    ...
    HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E FroceFrameProg = WIN_FRAME_FORCE_CLOSE;
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(HI_MPI_WIN_SetQuickOutput, Ret);
        return Ret;
    }
    ...
    if (pstAttr->bEnable)
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_SYNC_GetAttr, Ret);
            return Ret;
        }
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_SYNC_SetAttr, Ret);
            return Ret;
        }

        FroceFrameProg = WIN_FRAME_FORCE_PROG;
        Ret = HI_MPI_WIN_SetFrameProgInterleaveMode(pAvplay->MasterFrmChn.hWindow, FroceFrameProg);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_WIN_SetFrameProgInterleaveMode, Ret);
            return Ret;
        }
    ...
    else
    {
        ...
        FroceFrameProg = WIN_FRAME_FORCE_CLOSE;
        Ret = HI_MPI_WIN_SetFrameProgInterleaveMode(pAvplay->MasterFrmChn.hWindow, FroceFrameProg);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_WIN_SetFrameProgInterleaveMode, Ret);
...
HI_S32 AVPLAY_SetAdAttr(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_AD_ATTR_S *pstAttr)
{
    ...
    /* AdBalance range can dynamic adjust */
    if (((pstAttr->bAdEnble != pAvplay->AdAttr.bAdEnble)
        || (pstAttr->u32AdPid != pAvplay->AdAttr.u32AdPid)
        || (pstAttr->bAdToSomePort != pAvplay->AdAttr.bAdToSomePort))
        && pAvplay->AudEnable)
    {
...
HI_S32 AVPLAY_RelAllChnFrame(AVPLAY_S *pAvplay)
{
    ...
    if (HI_INVALID_HANDLE != pAvplay->MasterFrmChn.hWindow)
    {
        ...
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_VDEC_ReleaseFrame, Ret);
        }
    }
...
/*
 * Description  : Config window pause/resume, note vir window not support pause/resume opertation
 * Input        : pAvplay : avplay struct pointer
                  IsSetWinPause : HI_TRUE - pause window, HI_FALSE - resume window
 * Output       : NA
 * Return       : HI_SUCCESS - config success, others - config failure
 */
HI_S32 AVPLAY_SetWindowPause(const AVPLAY_S *pAvplay, HI_BOOL IsSetWinPause)
{
    HI_S32 Ret = HI_SUCCESS;
#ifndef AVPLAY_BASIC_SUPPORT
    HI_U32 i = 0;
    ...
    if (HI_INVALID_HANDLE != pAvplay->MasterFrmChn.hWindow)
    {
        Ret = HI_MPI_WIN_Pause(pAvplay->MasterFrmChn.hWindow, IsSetWinPause);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(HI_MPI_WIN_Pause, Ret);
            return Ret;
        }
    }
    ...
    for (i = 0; i < AVPLAY_MAX_SLAVE_FRMCHAN; i++)
    {
        if (HI_INVALID_HANDLE != pAvplay->SlaveFrmChn[i].hWindow)
        {
            Ret = HI_MPI_WIN_Pause(pAvplay->SlaveFrmChn[i].hWindow, IsSetWinPause);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(HI_MPI_WIN_Pause, Ret);
                return Ret;
...
static HI_S32 AVPLAY_StartVidChn(AVPLAY_S *pAvplay)
{
    ...
    Ret = AVPLAY_SetWindowPause(pAvplay, HI_FALSE);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(AVPLAY_SetWindowPause, Ret);
        return Ret;
    }
...
HI_S32  AVPLAY_AudResume(AVPLAY_S* pAvplay)
{
    ...
    {
        TplayParam.bTplayEnable = HI_FALSE;
        TplayParam.u32SpeedDeci = 0;
        TplayParam.u32SpeedInt  = 1;

        Ret = HI_MPI_ADEC_SetTplay(pAvplay->hAdec, &TplayParam);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("set Adec tplay failed 0x%x\n", Ret);
            return HI_FAILURE;
        }
...
HI_S32 AVPLAY_StopNormalAudio(AVPLAY_S *pAvplay)
{
    ...
    if (pAvplay->AudEnable)
    {
        ...
        if (HI_UNF_AVPLAY_STREAM_TYPE_TS == pAvplay->AvplayAttr.stStreamAttr.enStreamType)
        {
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_AVPLAY("Avplay free dmx Aud chn failed 0x%x\n", Ret);
                return Ret;
            }
        }
    #endif

        Ret = AVPLAY_AudResume(pAvplay);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_AVPLAY("call AVPLAY_AudResume failed 0x%x\n", Ret);
            return Ret;
        }
...
HI_S32 AVPLAY_AttachWindow(AVPLAY_S *pAvplay, HI_HANDLE hWin)
{
    ...
    WinSrcInfo.enSrcMode  = HI_ID_VDEC;
    WinSrcInfo.hSrc       = pAvplay->hVdec;
    WinSrcInfo.hSecondSrc = HI_INVALID_HANDLE;
    Ret = HI_MPI_WIN_SetSource(hWin, &WinSrcInfo);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(HI_MPI_WIN_SetSource, Ret);
        return HI_ERR_AVPLAY_INVALID_OPT;
    }
...
static HI_S32 AVPLAY_ConvertFrameAndSave(HI_DRV_VIDEO_FRAME_S* pFrame)
{
    ...
    HI_U32                     j = 0;
    FILE                      *fp = HI_NULL;
    HI_U8                     *pYVirutalAddr = NULL;
    HI_U8                     *pYdata = HI_NULL;
    HI_U8                     *pUdata = HI_NULL;
    HI_U8                     *pVdata = HI_NULL;
    HI_U32                     YSize = 0;
    HI_U32                     USize = 0;
    HI_U32                     VSize = 0;
    HI_CHAR                    FileName[128] = {0};

    struct tm                 *pTimeinfo = HI_NULL;
    time_t                     nowtime = 0;

    nowtime = time(HI_NULL);

    pTimeinfo = localtime(&nowtime);
    if (HI_NULL == pTimeinfo)
    {
        HI_ERR_AVPLAY("call localtime() error.\n");
        goto ERR_RETURN;
    }

    snprintf(FileName, sizeof(FileName), "%s/avplay_YUV_%d_%d_%d_%d_%d_%d.yuv",
             "/mnt",
    ...
    if (HI_NULL == fp)
    {
        HI_ERR_AVPLAY("fopen yuv error \n");
        goto ERR_RETURN;
    }

    YSize = pFrame->u32Width * pFrame->u32Height + (4 - (pFrame->u32Width * pFrame->u32Height) % 4);
    USize = pFrame->u32Width * pFrame->u32Height / 4 + (4 - (pFrame->u32Width * pFrame->u32Height / 4) % 4);
    VSize = pFrame->u32Width * pFrame->u32Height / 4 + (4 - (pFrame->u32Width * pFrame->u32Height / 4) % 4);

    pYdata = (HI_U8 *)HI_MALLOC(HI_ID_AVPLAY, sizeof(HI_U8) * (YSize + USize + VSize));
    if (HI_NULL == pYdata)
    {
        HI_ERR_AVPLAY("HI_MALLOC (size=%d) fail.\n", YSize + USize + VSize);
        goto ERR_CLOSE_FILE;
    }
    ...
    pYVirutalAddr = (HI_U8 *)HI_MMZ_Map(pFrame->stBufAddr[0].u32PhyAddr_Y, 0);
    if (HI_NULL == pYVirutalAddr)
    {
        HI_ERR_AVPLAY("call HI_MMZ_Map(u32PhyAddr_Y:%#x) error.\n", pFrame->stBufAddr[0].u32PhyAddr_Y);
        goto ERR_FREE;
    }

    memcpy(pYdata, pYVirutalAddr, sizeof(HI_U8) * (pFrame->u32Width * pFrame->u32Height));

    /* Write Y*/
    fwrite(pYdata, 1, pFrame->u32Width * pFrame->u32Height, fp);

    pYVirutalAddr += YSize;

    /* Write U,V*/
    for (i = 0; i < pFrame->u32Height / 2; i++)
    {
        for (j = 0; j < pFrame->u32Width / 2; j++)
        {
            if (HI_DRV_PIX_FMT_NV21 == pFrame->ePixFormat)
            {
                pUdata[i * pFrame->u32Width / 2 + j] = pYVirutalAddr[2 * j + 1];
                pVdata[i * pFrame->u32Width / 2 + j] = pYVirutalAddr[2 * j];
            }
            else
            {
                pUdata[i * pFrame->u32Width / 2 + j] = pYVirutalAddr[2 * j];
                pVdata[i * pFrame->u32Width / 2 + j] = pYVirutalAddr[2 * j + 1];
            }
        }
        pYVirutalAddr += pFrame->u32Width;
    }

    fwrite(pUdata, 1, pFrame->u32Width * pFrame->u32Height / 4, fp);

    fwrite(pVdata, 1, pFrame->u32Width * pFrame->u32Height / 4, fp);

    Ret = HI_MMZ_Unmap(pFrame->stBufAddr[0].u32PhyAddr_Y);
    if(Ret != HI_SUCCESS)
    {
        HI_ERR_AVPLAY("call HI_MMZ_Unmap error.\n");
    }

ERR_FREE:
    HI_FREE(HI_ID_AVPLAY, pYdata);
...
static HI_VOID AVPLAY_VidProcInfo(AVPLAY_S *pAvplay, HI_PROC_SHOW_BUFFER_S *ProcBuf)
{
    ...
    {
#ifdef AVPLAY_BASIC_SUPPORT
        HI_PROC_Printf(ProcBuf, "FrameChanID           :hVdec%04x->win%04x(master)\n",
                        pAvplay->hVdec & 0xffff,
...
static HI_S32 AVPLAY_ProcWrite(HI_PROC_SHOW_BUFFER_S *ProcBuf, HI_U32 Argc, HI_U8 *Argv[], HI_VOID *Data)
{
    ...
    if ( (Argc < 1) || (0 == strncmp(String, "help", sizeof("help"))) )
    {
        ...
        HI_PROC_Printf(ProcBuf, "echo CaptureYUV > /proc/msp/avplayxx, Capture current frame and save \n");
        return HI_FAILURE;
    }
    ...
    else if (0 == strncmp(String, "CaptureYUV", strlen("CaptureYUV")))
    {
        AVPLAY_INST_LOCK(Id);
        ...
        pAvplay->IsCaptureYUVFrame = HI_TRUE;
        AVPLAY_INST_UNLOCK(Id);
    }
                        ...
                        HI_UNF_AVPLAY_PUTBUFEX_OPT_S *pstExOpt)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID == enBufId)
    {
        ...
        pAvplay->AvplayVidEsBuf.bDualLayer      = HI_FALSE;
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID_HDR_BL == enBufId)
    {
        ...
        pAvplay->AvplayBlEsBuf.bDualLayer     = HI_FALSE;
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID_HDR_DL == enBufId)
    {
        ...
        pAvplay->AvplayBlEsBuf.bEndOfFrame    = pstExOpt->bEndOfFrm;
        pAvplay->AvplayBlEsBuf.bDiscontinuous = pstExOpt->bContinue ? HI_FALSE : HI_TRUE;
        pAvplay->AvplayBlEsBuf.bDualLayer     = HI_TRUE;

        pAvplay->AvplayElEsBuf.u32BufSize     = u32ValidDataLen;
        pAvplay->AvplayElEsBuf.u64Pts         = u32Pts;
        pAvplay->AvplayElEsBuf.bEndOfFrame    = pstExOpt->bEndOfFrm;
        pAvplay->AvplayElEsBuf.bDiscontinuous = pstExOpt->bContinue ? HI_FALSE : HI_TRUE;
        pAvplay->AvplayElEsBuf.bDualLayer     = HI_TRUE;
                        ...
                        HI_U32                        u32Pts)
{
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_AUD_AD == enBufId)
    {
        ...
        else if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
        {
            ...
            pAvplay->AvplayAdAudEsBuf.u32Size = u32ValidDataLen;
            Ret = HI_MPI_AO_Track_PutBuffer(pAvplay->hRenderTrack, HI_MPI_AO_BUF_ID_ES_ASSOC, &pAvplay->AvplayAdAudEsBuf, u32Pts);
            if (Ret != HI_SUCCESS)
            {
...
{
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstAvAttr);
...
HI_S32 HI_MPI_AVPLAY_Create(const HI_UNF_AVPLAY_ATTR_S *pstAvAttr, HI_HANDLE *phAvplay)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstAvAttr);
    HI_AVPLAY_CHECK_NULL_PTR(phAvplay);
...
HI_S32 AVPLAY_SetFrmPackingType(AVPLAY_S *pAvplay, HI_UNF_VIDEO_FRAME_PACKING_TYPE_E *pFrmPackingType)
{
    ...
    return HI_SUCCESS;
#else
    /* currently 3716mv430 chip is not support */
    return HI_ERR_AVPLAY_INVALID_OPT;
#endif
}
...
HI_S32 HI_MPI_AVPLAY_SetAttr(HI_HANDLE hAvplay, HI_UNF_AVPLAY_ATTR_ID_E enAttrID, HI_VOID *pPara)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pPara);
                                              ...
                                              HI_UNF_VIDEO_FRAME_INFO_S *pstCapPicture)
{
...
#ifdef AVPLAY_BASIC_SUPPORT
    HI_DRV_VIDEO_FRAME_S            Frame = {0};
#else
    HI_DRV_VPSS_PORT_CFG_S          VpssPortOldCfg = {0};
    HI_DRV_VPSS_PORT_CFG_S          VpssPortNewCfg = {0};
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstIframe);
...
#ifdef AVPLAY_BASIC_SUPPORT
    if (pAvplay->IFrame.stBufAddr[0].u32PhyAddr_Y)
#else
    if (pAvplay->IFrame.hport != HI_INVALID_HANDLE)
#endif
    {
        AVPLAY_INST_UNLOCK(Id);
...
#else
    memset(&Frame, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));

    Ret = HI_MPI_VDEC_ChanIFrameDecode(pAvplay->hVdec, (HI_UNF_AVPLAY_I_FRAME_S *)pstIframe, &Frame, bCapture);
    if (Ret != HI_SUCCESS)
    {
        AVPLAY_INST_UNLOCK(Id);
...
#ifdef AVPLAY_BASIC_SUPPORT
        hWindow = pAvplay->MasterFrmChn.hWindow;
        Frame.stPlayInfor.u32PlayCnt = 1;
        Ret = HI_MPI_WIN_QueueFrame(hWindow, &Frame);
#else
    ...
    {
#ifdef AVPLAY_BASIC_SUPPORT
        memcpy(&pAvplay->IFrame, &Frame, sizeof(HI_DRV_VIDEO_FRAME_S));
        AVPLAY_DRV2UNF_VidFrm(&pAvplay->IFrame, pstCapPicture, HI_NULL);
#else
        /*use frame of port0, release others*/
        memcpy(&pAvplay->IFrame, &FrmPack.stFrame[0], sizeof(HI_DRV_VDEC_FRAME_S));
        ...
        AVPLAY_DRV2UNF_VidFrm(&(pAvplay->IFrame.stFrameVideo), pstCapPicture, HI_NULL);
#endif
    }
...
HI_S32 HI_MPI_AVPLAY_ReleaseIFrame(HI_HANDLE hAvplay, HI_UNF_VIDEO_FRAME_INFO_S *pstCapPicture)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstCapPicture);
...
#ifdef AVPLAY_BASIC_SUPPORT
    Ret = HI_MPI_VDEC_ReleaseFrame(pAvplay->hVdec, &pAvplay->IFrame);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(HI_MPI_VDEC_ReleaseFrame, Ret);
    }
    ...
    else
    {
        if (pAvplay->IFrame.hport != HI_INVALID_HANDLE)
        {
            Ret = HI_MPI_VDEC_ReleaseFrame(pAvplay->IFrame.hport, &pAvplay->IFrame.stFrameVideo);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_AVPLAY("avplay release frame failed 0x%x\n", Ret);
            }
...
HI_S32 HI_MPI_AVPLAY_SetDecodeMode(HI_HANDLE hAvplay, HI_UNF_VCODEC_MODE_E enDecodeMode)
{
    ...
    HI_U32                  Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S               *pAvplay = HI_NULL;
    HI_UNF_VCODEC_ATTR_S    VdecAttr = {0};
    ...
    )
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pfnEventCB);
...
HI_S32 HI_MPI_AVPLAY_RegisterAcodecLib(const HI_CHAR *pFileName)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pFileName);
...
HI_S32 HI_MPI_AVPLAY_FoundSupportDeoder(const HA_FORMAT_E enFormat,HI_U32 * penDstCodecID)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(penDstCodecID);
...
HI_S32 HI_MPI_AVPLAY_PreStart(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn)
{
    ...
    Ret = AVPLAY_PcrEnable(pAvplay, pAvplay->DmxPcrPid);
    if (HI_SUCCESS != Ret)
    {
        AVPLAY_INST_UNLOCK(Id);
        return Ret;
...
HI_S32 HI_MPI_AVPLAY_Stop(HI_HANDLE hAvplay, HI_UNF_AVPLAY_MEDIA_CHAN_E enChn, const HI_UNF_AVPLAY_STOP_OPT_S *pStop)
{
    ...
    HI_U32                      Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S                   *pAvplay = HI_NULL;
    HI_UNF_AVPLAY_STOP_OPT_S    StopOpt = {0};
...
HI_S32 HI_MPI_AVPLAY_Pause(HI_HANDLE hAvplay)
{
    ...
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_PrintFuncResult(HI_MPI_SYNC_Pause, Ret);
        goto ERR;
    }
    ...
    if (pAvplay->VidEnable)
    {
        Ret = AVPLAY_SetWindowPause(pAvplay, HI_TRUE);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_PrintFuncResult(AVPLAY_SetWindowPause, Ret);
            goto ERR;
    ...
    if (pAvplay->AudEnable)
    {
        if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
        {
            ...
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_PrintFuncResult(HI_MPI_AO_Track_Pause, Ret);
                goto ERR;
            }
        }
#ifdef AVPLAY_MS12_SUPPORT
...
HI_S32  AVPLAY_SetAudTPlay(AVPLAY_S* pAvplay, const HI_UNF_AVPLAY_TPLAY_OPT_S* pstTplayOpt)
{
    ...
    /* aud only support 0.2~2 tplay */
    if ((u32SpeedInteger <= 2) && (u32SpeedDecimal <= 999))
    {
        u32TplayRate = u32SpeedInteger * 1000 + u32SpeedDecimal;
    }

    if ((u32TplayRate >= AVPLAY_MIN_AUD_TPLAY_SPEED) && (u32TplayRate <= AVPLAY_MAX_AUD_TPLAY_SPEED))
    {
        if (!pAvplay->bAudTplay)
        {
            if (   (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus)
...
HI_S32 HI_MPI_AVPLAY_Tplay(HI_HANDLE hAvplay, const HI_UNF_AVPLAY_TPLAY_OPT_S *pstTplayOpt)
{
    ...
    AVPLAY_S   *pAvplay = HI_NULL;
    HI_U32      i = 0;
    HI_U32      AvplayRatio = 0;
    HI_BOOL     bSetEosFlag = HI_FALSE;
...
HI_S32 HI_MPI_AVPLAY_Resume(HI_HANDLE hAvplay)
{
        ...
        || (HI_UNF_AVPLAY_STATUS_TPLAY == pAvplay->CurStatus))
    {
        if (pAvplay->bTplayResetMode)
        {
            ...
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_Reset, Ret);
                goto ERR;
            }
        }
    ...
    if (HI_UNF_AVPLAY_STATUS_PAUSE == pAvplay->CurStatus)
    {
        ...
        if (pAvplay->AudEnable)
        {
            if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
            {
                ...
                if (Ret != HI_SUCCESS)
                {
                    HI_ERR_PrintFuncResult(HI_MPI_AO_Track_Resume, Ret);
                    goto ERR;
                }
            }
#ifdef AVPLAY_MS12_SUPPORT
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_SYNC_Resume, Ret);
            goto ERR;
        }
    }
                            ...
                            HI_U32                u32TimeOutMs)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstData);
                            ...
                            HI_UNF_AVPLAY_PUTBUFEX_OPT_S *pstExOpt)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstExOpt);
...
HI_S32 HI_MPI_AVPLAY_GetSyncVdecHandle(HI_HANDLE hAvplay, HI_HANDLE *phVdec, HI_HANDLE *phSync)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phVdec);
    HI_AVPLAY_CHECK_NULL_PTR(phSync);
...
HI_S32 HI_MPI_AVPLAY_GetSndHandle(HI_HANDLE hAvplay, HI_HANDLE *phTrack)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phTrack);
...
HI_S32 HI_MPI_AVPLAY_GetWindowHandle(HI_HANDLE hAvplay, HI_HANDLE *phWindow)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phWindow);
...
HI_S32 HI_MPI_AVPLAY_AttachWindow(HI_HANDLE hAvplay, HI_HANDLE hWindow)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_INVALID_HANDLE(hWindow);
...
HI_S32 HI_MPI_AVPLAY_SetWindowRepeat(HI_HANDLE hAvplay, HI_U32 u32Repeat)
{
    ...
    AvplayRatio = AVPLAY_ALG_FRC_BASE_PLAY_RATIO / u32Repeat;
...
HI_S32 HI_MPI_AVPLAY_AttachRender(HI_HANDLE hAvplay, HI_HANDLE hRender)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_INVALID_HANDLE(hRender);
...
HI_S32 HI_MPI_AVPLAY_AttachSnd(HI_HANDLE hAvplay, HI_HANDLE hTrack)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_INVALID_HANDLE(hTrack);
...
HI_S32 HI_MPI_AVPLAY_GetDmxAudChnHandle(HI_HANDLE hAvplay, HI_HANDLE *phDmxAudChn)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phDmxAudChn);
...
HI_S32 HI_MPI_AVPLAY_GetDmxAdAudChnHandle(HI_HANDLE hAvplay, HI_HANDLE *phDmxAdAudChn)
{
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
    AVPLAY_S   *pAvplay = HI_NULL;

    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phDmxAdAudChn);
    ...
    if (!pAvplay->AdAttr.bAdEnble)
    {
        HI_ERR_AVPLAY("adaud chn is not enable\n");
        AVPLAY_INST_UNLOCK(Id);
         return HI_ERR_AVPLAY_INVALID_OPT;
    }

    *phDmxAdAudChn = pAvplay->hDmxAdAud;
...
HI_S32 HI_MPI_AVPLAY_GetDmxVidChnHandle(HI_HANDLE hAvplay, HI_HANDLE *phDmxVidChn)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(phDmxVidChn);
...
HI_S32 HI_MPI_AVPLAY_GetStatusInfo(HI_HANDLE hAvplay, HI_UNF_AVPLAY_STATUS_INFO_S *pstStatusInfo)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstStatusInfo);
...
HI_S32 HI_MPI_AVPLAY_GetStreamInfo(HI_HANDLE hAvplay, HI_UNF_AVPLAY_STREAM_INFO_S *pstStreamInfo)
{
    ...
    HI_AVPLAY_DEBUG_FUNC_ENTER();
    HI_AVPLAY_CHECK_NULL_PTR(pstStreamInfo);
    ...
    if (pAvplay->hAdec != HI_INVALID_HANDLE)
    {
        ...
        else
        {
            ...
            pstStreamInfo->stAudStreamInfo.enAudioFormat = AdecStreaminfo.enAudioFormat;
            pstStreamInfo->stAudStreamInfo.enChannelMode = AdecStreaminfo.enChannelMode;
            pstStreamInfo->stAudStreamInfo.bLfePresent   = AdecStreaminfo.bLfePresent;
        }
    }
...
HI_S32 HI_MPI_AVPLAY_GetAudioSpectrum(HI_HANDLE hAvplay, HI_U16 *pSpectrum, HI_U32 u32BandNum)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pSpectrum);
...
HI_S32 HI_MPI_AVPLAY_IsBuffEmpty(HI_HANDLE hAvplay, HI_BOOL *pbIsEmpty)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pbIsEmpty);
...
HI_S32 HI_MPI_AVPLAY_SwitchDmxAudChn(HI_HANDLE hAvplay, HI_HANDLE hNewDmxAud, HI_HANDLE *phOldDmxAud)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(phOldDmxAud);
    HI_AVPLAY_CHECK_INVALID_HANDLE(hNewDmxAud);
...
HI_S32 HI_MPI_AVPLAY_PutAudPts(HI_HANDLE hAvplay, HI_U32 AudPts)
{
    HI_U32      Id = GET_AVPLAY_ID(hAvplay);
    HI_S32      Ret = HI_SUCCESS;
    AVPLAY_S   *pAvplay = HI_NULL;
    ...
    pAvplay->AudInfo.SrcPts = AudPts;
    pAvplay->AudInfo.Pts = AudPts;
...
HI_S32 AVPLAY_Invoke_ProcVidCmd(AVPLAY_S *pAvplay, HI_CODEC_VIDEO_CMD_S *pVidCmd)
{
    HI_S32 Ret = HI_SUCCESS;
    ...
    switch (pVidCmd->u32CmdID)
    {
        case HI_UNF_AVPLAY_SET_SCENE_MODE_CMD:
        {
        ...
        case HI_UNF_AVPLAY_GET_VDEC_HANDLE_CMD:
        {
            *(HI_U32 *)(pVidCmd->pPara) = pAvplay->hVdec;
            return HI_SUCCESS;
        }

        case HI_UNF_AVPLAY_SET_VDEC_LSTFRAME_CMD:
        {
            if (HI_TRUE == *(HI_BOOL *)(pVidCmd->pPara))
            {
                Ret = HI_MPI_VDEC_SetEosFlag(pAvplay->hVdec);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_PrintFuncResult(HI_MPI_VDEC_SetEosFlag, Ret);
                    return Ret;
                }
            }
        ...
        case HI_UNF_AVPLAY_SET_PROGRESSIVE_CMD:
        {
        #ifdef AVPLAY_BASIC_SUPPORT
            HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E FroceFrameProg = WIN_FRAME_FORCE_CLOSE;

            if (*(HI_U32 *)(pVidCmd->pPara))
            {
                FroceFrameProg = WIN_FRAME_FORCE_PROG;
            }
            ...
            pAvplay->bSetProgressive = *(HI_U32 *)(pVidCmd->pPara);
            return HI_SUCCESS;
        #endif
        ...
        case HI_UNF_AVPLAY_KEEP_SPS_PPS_INFO_CMD:
        {
            pAvplay->bKeepSPSPPSFlag = *(HI_BOOL*)(pVidCmd->pPara);
            pVidCmd->u32CmdID = AVPLAY_VID_KEEP_SPS_PPS_INFO_CMD;
            break;
        }

        case HI_UNF_AVPLAY_GET_VDEC_INFO_CMD:
        case HI_UNF_AVPLAY_SET_TPLAY_PARA_CMD:
        case HI_UNF_AVPLAY_SET_CTRL_INFO_CMD:
        case HI_UNF_AVPLAY_SET_COLORSPACE_CMD:
        case HI_UNF_AVPLAY_SET_DPBFULL_CTRL_CMD:
        {
            break;
        }
        ...
        default:
        {
            HI_ERR_AVPLAY("InvokeType is unsupport :0x%x\n", pVidCmd->u32CmdID);
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_INVALID_OPT);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }

    Ret = HI_MPI_VDEC_Invoke(pAvplay->hVdec, pVidCmd);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_PrintFuncResult(HI_MPI_VDEC_Invoke, Ret);
        return Ret;
    }
...
HI_S32 AVPLAY_Invoke_ProcAudCmd(AVPLAY_S *pAvplay, HI_VOID* pPara)
{
    HI_S32 Ret = HI_SUCCESS;
    ...
    if (AVPLAY_AUDIO_TYPE_NORMAL == pAvplay->AvplayAudioType)
    {
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_ADEC_SetCodecCmd, Ret);
            return Ret;
        }
    }
#ifdef AVPLAY_MS12_SUPPORT
    else if (AVPLAY_AUDIO_TYPE_RENDER == pAvplay->AvplayAudioType)
    {
        if (HI_INVALID_HANDLE == pAvplay->hRenderTrack)
        {
            HI_ERR_AVPLAY("aud chn is close, can not set renser track cmd.\n");
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_INVALID_OPT);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
        ...
        if (Ret != HI_SUCCESS)
        {
            HI_ERR_PrintFuncResult(HI_MPI_AO_Track_SetRenderCmd, Ret);
            return Ret;
        }
    }
#endif
...
HI_S32 AVPLAY_Invoke_ProcAvplayCmd(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_INVOKE_E InvokeType, HI_VOID *pPara)
{
    HI_S32 Ret = HI_SUCCESS;

    switch (InvokeType)
    {
        case HI_UNF_AVPLAY_INVOKE_SET_SYNC_MODE:
        {
            if (*(HI_U32 *)pPara == 1)
            {
                SYNC_STOP_REGION_S  SyncStopRegion = {0};

                SyncStopRegion.bUseStopRegion = HI_FALSE;
                Ret = HI_MPI_SYNC_SetExtInfo(pAvplay->hSync, SYNC_EXT_INFO_STOP_REGION, (HI_VOID *)&SyncStopRegion);
                if (HI_SUCCESS != Ret)
                {
                    HI_ERR_PrintFuncResult(HI_MPI_SYNC_SetExtInfo, Ret);
                    return Ret;
                }
            }
        ...
        case HI_UNF_AVPLAY_INVOKE_SET_TPLAY_AUDIO_ENABLE:
        {
            Ret = AVPLAY_SetAudTPlayEnable(pAvplay, *(HI_U32 *)pPara);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_SetAudTPlayEnable, Ret);
                return Ret;
            }
        ...
        case HI_UNF_AVPLAY_INVOKE_SET_TPLAY_RESET_ENABLE:
        {
            pAvplay->bTplayResetMode = (*(HI_U32 *)pPara == 1) ? HI_TRUE : HI_FALSE;
            break;
        }

        default:
        {
            HI_ERR_AVPLAY("InvokeType is invalid :0x%x\n", InvokeType);
            HI_ERR_PrintErrCode(HI_ERR_AVPLAY_INVALID_OPT);
            return HI_ERR_AVPLAY_INVALID_OPT;
        }
    }
...
HI_S32 AVPLAY_Invoke_ProcDispCmd(AVPLAY_S *pAvplay, HI_UNF_AVPLAY_INVOKE_E InvokeType, HI_VOID *pPara)
{
    HI_S32 Ret = HI_SUCCESS;

    switch (InvokeType)
    {
        case HI_UNF_AVPLAY_INVOKE_GET_PRIV_PLAYINFO:
        {
            HI_DRV_VIDEO_FRAME_S                 VidFrame;
            HI_UNF_AVPLAY_PRIVATE_STATUS_INFO_S  PlayInfo;
            HI_DRV_VIDEO_PRIVATE_S               VidPrivate;
            ...
            memset(&VidFrame, 0x0, sizeof(HI_DRV_VIDEO_FRAME_S));
            memset(&PlayInfo, 0x0, sizeof(HI_UNF_AVPLAY_PRIVATE_STATUS_INFO_S));
            memset(&VidPrivate, 0x0, sizeof(HI_DRV_VIDEO_PRIVATE_S));

            Ret = HI_MPI_WIN_GetLatestFrameInfo(pAvplay->MasterFrmChn.hWindow, &VidFrame);
            if (Ret != HI_SUCCESS)
            {
                HI_ERR_PrintFuncResult(HI_MPI_WIN_GetLatestFrameInfo, Ret);
                return Ret;
            }

            memcpy(&VidPrivate, (HI_DRV_VIDEO_PRIVATE_S *)(VidFrame.u32Priv), sizeof(HI_DRV_VIDEO_PRIVATE_S));

            PlayInfo.u32LastPts = VidFrame.u32Pts;
            PlayInfo.u32LastPlayTime = VidPrivate.u32PrivDispTime;
            PlayInfo.u32DispOptimizeFlag = pAvplay->u32DispOptimizeFlag;

            memcpy((HI_UNF_AVPLAY_PRIVATE_STATUS_INFO_S *)pPara, &PlayInfo, sizeof(HI_UNF_AVPLAY_PRIVATE_STATUS_INFO_S));
            break;
        }
        case HI_UNF_AVPLAY_INVOKE_SET_DISP_OPTIMIZE_FLAG:
        {
...
HI_S32 HI_MPI_AVPLAY_Invoke(HI_HANDLE hAvplay, HI_UNF_AVPLAY_INVOKE_E enInvokeType, HI_VOID *pPara)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pPara);
    HI_AVPLAY_CHECK_PARA(HI_UNF_AVPLAY_INVOKE_BUTT <= enInvokeType);

    AVPLAY_INST_LOCK_CHECK(hAvplay, Id);

    switch (enInvokeType)
    {
        case HI_UNF_AVPLAY_INVOKE_VCODEC:
        {
            Ret = AVPLAY_Invoke_ProcVidCmd(pAvplay, (HI_CODEC_VIDEO_CMD_S*)pPara);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_Invoke_ProcVidCmd, Ret);
                AVPLAY_INST_UNLOCK(Id);
                return Ret;
            }
        ...
        case HI_UNF_AVPLAY_INVOKE_ACODEC:
        {
            Ret = AVPLAY_Invoke_ProcAudCmd(pAvplay, pPara);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_Invoke_ProcAudCmd, Ret);
                AVPLAY_INST_UNLOCK(Id);
                return Ret;
            }
            break;
        ...
        case HI_UNF_AVPLAY_INVOKE_SET_TPLAY_RESET_ENABLE:
        {
            Ret = AVPLAY_Invoke_ProcAvplayCmd(pAvplay, enInvokeType, pPara);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_Invoke_ProcAvplayCmd, Ret);
                AVPLAY_INST_UNLOCK(Id);
                return Ret;
            }
            break;
        ...
        case HI_UNF_AVPLAY_INVOKE_SET_DISP_OPTIMIZE_FLAG:
        {
            Ret = AVPLAY_Invoke_ProcDispCmd(pAvplay, enInvokeType, pPara);
            if (HI_SUCCESS != Ret)
            {
                HI_ERR_PrintFuncResult(AVPLAY_Invoke_ProcDispCmd, Ret);
                AVPLAY_INST_UNLOCK(Id);
                return Ret;
            }
            break;
...
HI_S32 HI_MPI_AVPLAY_AcqUserData(HI_HANDLE hAvplay, HI_UNF_VIDEO_USERDATA_S *pstUserData, HI_UNF_VIDEO_USERDATA_TYPE_E *penType)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pstUserData);
    HI_AVPLAY_CHECK_NULL_PTR(penType);
...
HI_S32 HI_MPI_AVPLAY_GetVidChnOpenParam(HI_HANDLE hAvplay, HI_UNF_AVPLAY_OPEN_OPT_S *pstOpenPara)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pstOpenPara);
...
HI_S32 HI_MPI_AVPLAY_UseExternalBuffer(HI_HANDLE hAvplay, HI_MPI_EXT_BUFFER_S* pstExtBuf)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pstExtBuf);
...
HI_S32 HI_MPI_AVPLAY_GetAdecOutFrameInfo(HI_HANDLE hAvplay, HI_U32 *u32OutFrames, HI_U32 *u32SampleRate)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(u32OutFrames);
    HI_AVPLAY_CHECK_NULL_PTR(u32SampleRate);
        ...
        HI_U32 u32RefreshRate, HI_S32* ps32RepeatCnt)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pstFrame);
    HI_AVPLAY_CHECK_NULL_PTR(ps32RepeatCnt);
...
HI_S32 HI_MPI_AVPLAY_GetLeftStreamFrm(HI_HANDLE hAvplay, HI_U32 *pLeftStreamFrm)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pLeftStreamFrm);
...
HI_S32 HI_MPI_AVPLAY_SetHDRInfo(HI_HANDLE hAvplay, HI_UNF_VIDEO_HDR_INFO_S* pstHdrInfo, HI_BOOL bPaddingHdrInfo, HI_U32 u32PtsMs)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pstHdrInfo);
...
HI_S32 HI_MPI_AVPLAY_CheckIfSupportRandomStream(HI_HANDLE hAvplay, HI_BOOL *pbSupport)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pbSupport);
...
HI_S32 HI_MPI_AVPLAY_GetHandle(HI_HANDLE hAvplay, HI_MOD_ID_E enModID, HI_HANDLE *pHandle, HI_U32 *pu32Reserve)
{
    ...
    HI_AVPLAY_CHECK_NULL_PTR(pHandle);
    HI_AVPLAY_CHECK_NULL_PTR(pu32Reserve);
...
HI_S32 HI_MPI_AVPLAY_STCCalibration(HI_HANDLE hAvplay, HI_S32 StcPPM)
{
    ...
    {
        AVPLAY_INST_UNLOCK(Id);
        HI_ERR_PrintFuncResult(HI_MPI_SYNC_STCCalibration, Ret);
        return Ret;
    }
