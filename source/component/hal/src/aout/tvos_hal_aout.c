...
static S32 build_sound_attr(HI_UNF_SND_ATTR_S* pstAttr, AOUT_SETTINGS_S stSettings)
{
    ...
    /* speaker and arc is not supported in STB*/
    if ((AOUT_DEVICE_SPEAKER == stSettings.enOutputDevice) || (AOUT_DEVICE_ARC == stSettings.enOutputDevice)
        || ((AOUT_DEVICE_SPEAKER | AOUT_DEVICE_ARC) == stSettings.enOutputDevice))
    {
...
S32 aout_invoke(struct _AOUT_DEVICE_S* pstDev, const HANDLE hInvokeHandle, AOUT_INVOKE_E eCmd, VOID* pArg)
{
    trace();
    Check_Inited();
    Check_AoutHandle(hInvokeHandle);
...
#ifdef HAL_HISI_EXTEND_H
    stAdev->aout_set_config = aout_set_config;
    stAdev->aout_get_config = aout_get_config;
    stAdev->aout_invoke = aout_invoke;
#endif
...
#ifdef HAL_HISI_EXTEND_H
    .aout_set_config = aout_set_config,
    .aout_get_config = aout_get_config,
    .aout_invoke = aout_invoke,
#endif
