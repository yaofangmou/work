...
static VOID *List_AllocVirMem(VMM_ALLOC_MODE_E AllocMode, UINT32 Size)
{
    ...
    switch (AllocMode)
    {
        ...
        default:
            PRINT(VMM_ERROR, "%s,%d, not support this alloc mode(%d),alloc size = %d\n", __func__, __LINE__, AllocMode, Size);
            break;
    }
...
static VOID List_FreeVirMem(VMM_ALLOC_MODE_E AllocMode, VOID *pVirMem)
{
    switch (AllocMode)
    {
        ...
        default:
            PRINT(VMM_ERROR, "%s,%d, not support this free mode(%d),free VirMem = %p\n", __func__, __LINE__, AllocMode, pVirMem);
            break;
    }
...
SINT32 List_ExpandLen(VMM_List_S *pList)
{
    ...
    {
        PRINT(VMM_ERROR, "%s,%d,Check_ListCfg failed!\n", __func__, __LINE__);
    ...
    {
        PRINT(VMM_ERROR, "List(No.%d) CurExpansionFactor(%d) is >= MaxExpansionFactor(%d). Can not be expanded!\n",
              pListCfg->MarkNo, pList->CurExpansionFactor, pListCfg->MaxExpansionFactor);

        return ERR_LIST_FULL;
    }

    PRINT(VMM_DBG, "%s,%d,===> List(No.%d) CurExpansionFactor(%d), MaxExpansionFactor(%d), CurNum = %d\n",
...
SINT32  List_IsEmpty(VMM_List_S *pList, VMM_LIST_TYPE_E eType)
{
    ...
    switch (eType)
    {
        ...
        default:
            PRINT(VMM_ERROR, "eType '%d' is not support.\n", eType);
            break;
    }
...
VMM_List_S *List_Init(const VMM_List_Config_S *pListCfg)
{
    ...
    if (!pList)
    {
        PRINT(VMM_ERROR, "%s,%d,alloc vir mem (size = %d)failed!\n", __func__, __LINE__, sizeof(VMM_List_S));
