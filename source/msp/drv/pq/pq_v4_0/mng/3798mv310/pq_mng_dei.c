...
HI_S32 PQ_MNG_ALG_GetMcDeiParam(MOTION_INPUT_S *pstMotionInput)
{
    ...
    if (pstMotionInput->u32Width <= 1920 && pstMotionInput->u32Height <= 1080 && \
        (g_pstDeiSwAlgPara.s32GlbFrmIndex[0] == 0))
    {
        sg_stPQDEIInfo.s32FRCountMC = 0;/*if seek or start, make cnt = 0*/
    }

    if (sg_stPQDEIInfo.s32FRCountMC <= 10)
    {
        sg_pstDeiVar->stMDINFO.rec_mode_output_mode = 1;
    }
    else
    {
        sg_pstDeiVar->stMDINFO.rec_mode_output_mode = 0;
    }
    ...
    if (pstMotionInput->u32Height == 576 && sg_stPQDEIInfo.u32StreamId == 0x68)
    {
        u32R0_MC   = 234;
    }
    else
    ...
    if ((pstMotionInput->u32Height == 1080) && (HI_MNG_PQ_MCDI_SPLIT_MODE_5 == pstMotionInput->enSplitMode))
    {
        u32R0_MC   = 230;
    }
    else
