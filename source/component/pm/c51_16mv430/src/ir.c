...
code IRREGSETTING irRegSetting[IR_STD_NUM] = { {0x00001f17, 0x033c03cc, 0x019e01e6, 0x00b4010e, 0x00270054, 0x008700cb}, //nec simple
...
HI_U8 regValLow = 0;
HI_U8 regValHigh = 0;
HI_U16 u16regValLow = 0;
HI_U16 u16regValHigh = 0;
...
code XMPIRFMT irfmtxmp[IR_XMP_NUM] = { {36, 8, 100, 70, 36, 8, 32} };
...
{
    HI_U8 i;
    HI_U8 addr, subaddr, subaddr2, toggle, oem, obc1, obc2, sum1, sum2;

    if(start > 16 || divider < 50)
    {
        return -1;
    }
    ...
    for (i = start; i < start + 16; ++i)
    {
        ...
        if(((xmp_code[i] - XMP_NIBBLE_PREFIX)%divider) > (divider * 78 /100))
        {
            xmp_code[i] = (xmp_code[i] - XMP_NIBBLE_PREFIX) / divider + 1;
        }
        else
        {
            xmp_code[i] = (xmp_code[i] - XMP_NIBBLE_PREFIX) / divider;
        }
    }

    sum1 = (15 + xmp_code[start] + xmp_code[start+1] + xmp_code[start+2] + xmp_code[start+3] +
        xmp_code[start+4] + xmp_code[start+5] + xmp_code[start+6] + xmp_code[start+7]) % 16;
    sum2 = (15 + xmp_code[start+8] + xmp_code[start+9] + xmp_code[start+10] + xmp_code[start+11] +
        xmp_code[start+12] + xmp_code[start+13] + xmp_code[start+14] + xmp_code[start+15]) % 16;
    ...
    subaddr  = xmp_code[start] << 4 | xmp_code[start+2];
    subaddr2 = xmp_code[start+8] << 4 | xmp_code[start+11];
    oem      = xmp_code[start+4] << 4 | xmp_code[start+5];
    addr     = xmp_code[start+6] << 4 | xmp_code[start+7];
    toggle   = xmp_code[start+10];
    obc1 = xmp_code[start+12] << 4 | xmp_code[start+13];
    obc2 = xmp_code[start+14] << 4 | xmp_code[start+15];
    if (subaddr != subaddr2)
    {
        ir_xmp_reset();
        return -1;
    ...
    irRawHKey.val32 = 0;
    irRawLKey.val8[0] = addr;
    irRawLKey.val8[1] = subaddr;
    irRawLKey.val8[2] = obc1;
    irRawLKey.val8[3] = obc2;

    return 0;
}

void  ir_xmp_parsekey(void)
{
    if (ir_count < irfmtxmp[0].bits_num && u16regValHigh < 0x464)
    {
        xmp_code[ir_count++] = (u16regValLow + u16regValHigh - 21) * 10;

        if (ir_count < irfmtxmp[0].bits_num)
        {
            return;
        }
    }
    ...
    if (ir_count == irfmtxmp[0].bits_num)
    {
        ...
        divider = (xmp_code[3] - XMP_NIBBLE_PREFIX) / 15 - 2;
        if (divider < 50)
        {
again:
            divider = (xmp_code[19] - XMP_NIBBLE_PREFIX) /15 - 2;
            (void)ir_xmp_analyse(16, divider);
            return;
        }
        else
        {
    ...
    else if (ir_count > irfmtxmp[0].bits_num)
    {
        ir_xmp_reset();
    }
...
void ir_raw_getfmtval(void)
{
    ...
    {
        if (key_match(irfmtxmp[i].lead_s_up, irfmtxmp[i].lead_s_low, u16regValLow)
            && key_match(irfmtxmp[i].lead_e_up, irfmtxmp[i].lead_e_low, u16regValHigh))
        {
            irfmt = IR_TYPE_XMP;
            xmp_code[ir_count++] = (u16regValLow + u16regValHigh - 21) * 10;
            return;
...
void ir_raw_isr()
{
    ...
    else //symbol or timeout interrupt
    {
        ...
        while (cnt-- > 0)
        {
            ...
            u16regValLow = regData.val16[1];
            u16regValHigh = regData.val16[0];
