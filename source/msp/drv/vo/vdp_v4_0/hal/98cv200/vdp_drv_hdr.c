...
HI_S32 VDP_DRV_SetHdrMode(VDP_HDR_CFG_S *stHdrCfg, VDP_FIXPT_MAIN_CFG_S *pstCompCfg, DmKsFxp_t *pstDmKs)
{
    ...
    VDP_VID_SetUvorder                 (VDP_LAYER_VID0, stHdrCfg->bUvOrder);
    ...
    if (((VDP_VID_IFMT_SP_TILE == stHdrCfg->enVdpBlVidIFmt) || (VDP_VID_IFMT_SP_TILE_64 == stHdrCfg->enVdpBlVidIFmt))
        && ((2 * stOutReso.u32Hgt) < stHdrCfg->stVdpDispInfo.stFmtResolution.u32Hgt))
    {
        VDP_VID_SetDataWidth               (VDP_LAYER_VID0, VDP_DATA_WTH_8);
    }
    else
    {
    ...
    if ((stHdrCfg->stVidReso.u32Wth != stOutReso.u32Wth) || (stHdrCfg->stVidReso.u32Hgt != stOutReso.u32Hgt))
    {
        ...
        VDP_VID_SetZmeVerRatio         (VDP_LAYER_VID0, ((stHdrCfg->stVidReso.u32Hgt / (1<<stZmeInfo.enDrawMode))* ZME_VPREC) / stOutReso.u32Hgt);
