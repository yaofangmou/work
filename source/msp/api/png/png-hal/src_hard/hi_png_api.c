...
HI_S32 HI_PNG_Open(HI_VOID)
{
    ...
    {
        g_u32PngRef++;
        (HI_VOID)pthread_mutex_unlock(&g_PngMutex);
        return HI_SUCCESS;
    }
...
HI_VOID PNG_UnMap_DecoderBuf(HI_PNG_HANDLE s32Handle)
{
    ...
    (HI_VOID)pthread_mutex_unlock(&gs_PngApiInstance[s32Handle - 1].stLock);
...
HI_S32 HI_PNG_SetStreamLen(HI_PNG_HANDLE s32Handle, HI_U32 u32Phyaddr, HI_U32 u32Len)
{
	...
	{
		(HI_VOID)(HI_VOID)(HI_VOID)pthread_mutex_unlock(&gs_PngApiInstance[s32Handle - 1].stLock);
		return HI_ERR_PNG_INVALID_PARAM;
	}
	else
	{
...
HI_S32 HI_PNG_Decode(HI_PNG_HANDLE s32Handle,  const HI_PNG_DECINFO_S *pstDecInfo)
{
    ...
    {
#ifdef CONFIG_GFX_MEM_MMZ
        HI_GFX_Flush(pTemp->stBuf.u32PhyAddr, HI_TRUE);
#else
        HI_GFX_Flush(g_s32MemFd, pTemp->stBuf.u32PhyAddr, pTemp->stBuf.pMemHandle);
#endif
        pTemp = pTemp->pNext;
    }
