...
#define SHARED_BUFFER_SIZE (0x500000)
...
CryptoClient::CryptoClient(const HI_U8 uuid[HI_DRM_UUID_LEN], const HI_VOID* initData, HI_S32 initSize)
{
    ...
    mDealer = new MemoryDealer(SHARED_BUFFER_SIZE, "DrmServiceInBuffer");
    mSharedMem = mDealer->allocate(SHARED_BUFFER_SIZE);
}
    ...
    AString* errorDetailMsg)
{
    ...
    if (mSharedMem == NULL)
    {
        HI_LOGE("%s: IMemory mSharedMem is NULL", __FUNCTION__);
        return HI_FAILURE;
    }
    memcpy(mSharedMem->pointer(), sharedBuffer, totalSize);
