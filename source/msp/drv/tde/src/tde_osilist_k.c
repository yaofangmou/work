...
STATIC INLINE HI_VOID TdeOsiListSafeDestroyJob(TDE_SWJOB_S *pstJob)
{
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_INT, pstJob->s32Handle);
...
HI_S32 TdeOsiListInit(HI_VOID)
{
    ...
    if (!initial_handle())
    {
        HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, initial_handle, FAILURE_TAG);
        return HI_FAILURE;
    }
...
HI_VOID TdeOsiListTerm(HI_VOID)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(s_pstTDEOsiJobList);
...
static HI_VOID TdeOsiListFlushJob(TDE_SWJOB_S *pstJob)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstJob);
    pstFstCmd  = pstJob->pstFirstCmd;
    pstLastCmd =  pstJob->pstTailNode;
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstFstCmd);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstLastCmd);
...
HI_S32 TdeOsiListBeginJob(TDE_HANDLE *pHandle,HI_VOID *private_data)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pHandle, HI_FAILURE);
    ...
    {
        TDE_FREE(pHandleMgr);
        HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, get_handle, FAILURE_TAG);
        return HI_ERR_TDE_NO_MEM;
    }
...
STATIC HI_VOID TdeOsiListDestroyJob(TDE_SWJOB_S *pstJob)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstJob->s32Handle);
        HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, query_handle, FAILURE_TAG);
        return;
    }
                           ...
                           TDE_NOTIFY_MODE_E enNotiType)
{
    ...
    if (TDE_JOB_WAKE_NOTIFY == enNotiType)
    {
        ...
        else
        {
            ...
            {
                HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstJob->s32Handle);
                HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, wait_event_interruptible_timeout, FAILURE_TAG);
                HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "handle interrupt");
            ...
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstJob->s32Handle);
            HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, wait_event_interruptible_timeout, FAILURE_TAG);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "handle timeout");
...
HI_VOID TdeOsiListComp()
{
    ...
    if (NULL == s_pstTDEOsiJobList)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, s_pstTDEOsiJobList);
        return;
    }
...
HI_VOID TdeOsiListNodeComp()
{
    ...
    else
    {
        ...
        {
            TDE_UNLOCK(&s_pstTDEOsiJobList->lock, lockflags);
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_INT, s32FinishedHandle);
            HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "No pre Job left");
            HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_EXIT_FUNC);
            return;
        }
    ...
    if (!query_handle(s32FinishedHandle, &pHandleMgr))
    {
        TDE_UNLOCK(&s_pstTDEOsiJobList->lock, lockflags);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_INT, s32FinishedHandle);
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "handle already delete");
        HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_EXIT_FUNC);
        return;
    }
    ...
    while (!list_empty(&s_pstTDEOsiJobList->stList))
    {
        ...
        if (TDE_JOB_WAKE_NOTIFY == pstDelJob->enNotiType)
        {
            ...
            {
                HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_INT, pstDelJob->s32Handle);
                HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "handle has callback func");
                TDE_UNLOCK(&s_pstTDEOsiJobList->lock, lockflags);
            ...
            {
                HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_INT, pstDelJob->s32Handle);
                HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "query handle complete");
        ...
        {
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Error Status!!");
        }
...
HI_S32 TdeOsiListLockWorkingFlag(HI_SIZE_T *lock)
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
#ifdef TDE_HWC_COOPERATE
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(lock, HI_FAILURE);
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(bFlag, HI_FAILURE);
...
HI_VOID TdeFreePendingJob(HI_VOID *private_data)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(s_pstTdeHandleList);
    ...
    if (NULL == pstHandle)
    {
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "No pending job!");
        TDE_UNLOCK(&s_pstTDEOsiJobList->lock, lockflags);
        return;
    }
    ...
    while (pstHandle != s_pstTdeHandleList)
    {
        ...
        if (NULL == pstJob)
        {
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "pstJob Null Pointer!");
            TDE_UNLOCK(&s_pstTDEOsiJobList->lock, lockflags);
            return;
        }
...
HI_VOID TdeOsiListProc(struct seq_file *p)
{
    if ((NULL != p) && (NULL != s_pstTDEOsiJobList))
    {
        PROC_PRINT(p, "finish handle\t: %u\n",  s_pstTDEOsiJobList->s32HandleFinished);
    }
    return;
}
#endif
