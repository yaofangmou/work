...
4. This tool is used as follows:
  ...
  If the ROOT_RSA_Private_Key is not set while the External_public_key_file and External_private_key_file are set, the image and signature of other areas and the image of the key area are generated.
  If ROOT_RSA_Private_Key£¬ External_private_key_file and External_public_key_file are not set, the image of other area area are genrated, the external public key in key area will be set all 0, each image will not be signed, the signature in Finalboot will be set all 0 or FF.
  If the ROOT_RSA_Private_Key and External_private_key_file are not set while the External_pubblic_key_file is set, the image of other area are genrated, each image will not be signed, the signature in Finalboot will be set all 0 or FF.
  
  Sign boot area except key area:
  If the ROOT_RSA_Private_Key and External_public_key_file are not set, and External_pivate_key_file is filled, the image and signature of other areas are generated.
  ...
  8>./CASignTool 10 -K keyFilePath -R srcFilePath -O destFilePath -sw SoftwareVersion(if in hex format, please start with 0x or 0X) [-P pDataPath] [-c ChipType] [-crypto 0/1]
  First step of signing the non-boot image in M mode
  The -K, -R, and -O options are available for specifying the paths of the key file, source file, and target file, respectively. Use the -sw option to specify the software ID, the -c option to specify the chip ID, the -P to specify the private data path and -crypto to specify the  

  9>./CASignTool 11 -K keyFilePath -R srcFilePath -O destFilePath [-SWPK SWPKFile] [-c ChipType] [-crypto 0/1]
  Second step of signing the non-boot image in M mode
  The -K, -R, -O and -SWPK options are available for specifying the paths of the key file, source file, target file and swpkFile respectively. Use the -swpk option to specify the  encryption key file (within 32-bit hexadecimal character string), and the -c option to specify the chip ID. 
   
  10>./CASignTool 14 cfgFilePath -PRIV privateKeyFilePath -O destFilePath [-R srcFilePath] [-PK protectionKeyFilePath] [-PUB publicKeyFilePath] [-EncPK EncryptedKeyPath]  
  ...
  11> ./CASignTool 15 -R pemFilePath -ot b/t -O destFilePath
  ...
  12> ./CASignTool 16 mergeImage.cfg -r srcDirPath -O destFilePath
  Merge image.
