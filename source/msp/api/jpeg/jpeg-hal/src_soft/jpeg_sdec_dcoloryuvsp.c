...
static inline HI_VOID JPEG_SDEC_OutY (j_decompress_ptr cinfo, JSAMPROW InputYBuf)
{
    ...
    JPEG_DEC_GetOutUsrBuf((HI_ULONG)cinfo->client_data, &OutYBuf, &YStride, &YSize, NULL, NULL, NULL);
    if (NULL == OutYBuf)
    {
       return;
    }
...
static inline HI_VOID JPEG_SDEC_OutWholeCbCr (j_decompress_ptr cinfo, JSAMPROW InputCbBuf, JSAMPROW InputCrBuf)
{
    ...
    JPEG_DEC_GetOutUsrBuf((HI_ULONG)cinfo->client_data, NULL, NULL, NULL, &OutCbCrBuf, &UVStride, &UVSize);
    if (NULL == OutCbCrBuf)
    {
       return;
    }
...
static inline HI_S32 JPEG_SDEC_InitOutToYuv420Sp (j_decompress_ptr cinfo)
{
    ...
    switch (InputFmt)
    {
      case HI_JPEG_FMT_YUV444:
           ...
           JPEG_DEC_SaveCallFunc((unsigned long)cinfo->client_data, __FUNCTION__, __LINE__);
           break;
...
static inline HI_VOID JPEG_SDEC_CheckYUVSPBuf(j_decompress_ptr cinfo)
{
   ...
   JPEG_DEC_GetOutUsrBuf((HI_ULONG)cinfo->client_data, &OutYBuf, NULL, NULL, &OutUVBuf, NULL, NULL);
   if (NULL == OutYBuf)
   {
      ERREXIT(cinfo, JERR_CONVERSION_NOTIMPL);
   }
