               ...
               CoreSurfacePoolDescription *ret_desc )
{
     ...
     if (CONFIG_HISI_MEMPOOL_SIZE)
     {
        if(nLayerNum == 0)
        {
             ...
             ret = dfb_surfacemanager_create( core, CONFIG_HISI_MEMPOOL_SIZE, &data->manager );
             if (ret)
             {
                  hisi_mem_free_pools();
                  return ret;
...
#if defined(CONFIG_HISI_MEM_SUPPORT) && (CONFIG_HISI_MEMPOOL_SIZE)
     if(nLayerNum == 0)
#endif
     {
...
//tag, support fence sync
#ifdef CONFIG_DFB_SUPPORT_FENCE
          if (allocation->resource_id == DLID_PRIMARY)
          {
