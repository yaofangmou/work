...
HI_S32 PDM_GetDispParam(HI_U8 *pBuf, HI_UNF_DISP_E enDisp, HI_UNF_PDM_DISP_PARAM_S *pstDispParam)
{
    HI_S32 Ret;
    HI_DB_S BaseDB = {0};
    HI_DB_TABLE_S stTable = {0};
    HI_DB_KEY_S stKey = {{0}};
    HI_UNF_PDM_DISP_PARAM_S stDefDispParam;
    HI_CHAR *pTableName = HI_NULL;
    ...
    pTableName = (HI_UNF_DISPLAY0 == enDisp) ? MCE_BASE_TABLENAME_DISP0 : MCE_BASE_TABLENAME_DISP1;

    Ret = HI_DB_GetTableByName(&BaseDB, pTableName, &stTable);
    if (HI_SUCCESS != Ret)
    {
        return HI_FAILURE;
    }
...
HI_S32 PDM_GetSoundParam(HI_U8 *pBuf, HI_UNF_PDM_BASEPARAM_TYPE_E enSound, HI_UNF_PDM_SOUND_PARAM_S *pstSoundParam)
{
    ...
    {
        pTableName = MCE_BASE_TABLENAME_SOUND0;
    }
    else if (HI_UNF_PDM_BASEPARAM_SOUND1 == enSound)
    {
        pTableName = MCE_BASE_TABLENAME_SOUND1;
    }
    else
    {
        pTableName = MCE_BASE_TABLENAME_SOUND2;
    }

    Ret = HI_DB_GetTableByName(&stBaseDB, pTableName, &stTable);
    if (HI_SUCCESS != Ret)
    {
...
HI_S32 PDM_UpdateDispParam(HI_UNF_DISP_E enDisp, HI_UNF_PDM_DISP_PARAM_S *pstDispParam, HI_U8 *pBuf)
{
    ...
    pTalbeName = (HI_UNF_DISPLAY0 == enDisp) ? MCE_BASE_TABLENAME_DISP0 : MCE_BASE_TABLENAME_DISP1;

    Ret = HI_DB_GetTableByName(&stBaseDB, pTalbeName, &stTable);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PDM("ERR: HI_DB_GetTableByName!");
        return Ret;
