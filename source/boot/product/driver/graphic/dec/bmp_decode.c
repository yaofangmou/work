...
static HI_S32 BMP_Read_DecData(BMP_DECODER_S *pBmpDec,HI_HANDLE hSurface)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pBmpDec, HI_FAILURE);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Width[0]);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MINWIDTH);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MAXWIDTH);
        return HI_FAILURE;
    }

    if ((pstSurface->u32Height[0] < CONFIG_GFX_MINHEIGHT) || (pstSurface->u32Height[0] > CONFIG_GFX_MAXHEIGHT))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Height[0]);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MINHEIGHT);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MAXHEIGHT);
        return HI_FAILURE;
    }
    ...
    if ((pstSurface->u32Stride[0] < CONFIG_GFX_MINSTRIDE) || (pstSurface->u32Stride[0] > CONFIG_GFX_MAXSTRIDE))
    {
        ...
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MINSTRIDE);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_MAXSTRIDE);
        return HI_FAILURE;
    }

    if ((0 == pBmpDec->InfoHeader.biBitCount) || (pBmpDec->InfoHeader.biBitCount > CONFIG_GFX_BITSPERPIX))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pBmpDec->InfoHeader.biBitCount);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, CONFIG_GFX_BITSPERPIX);
        return HI_FAILURE;
    }

    if (pstStream->u32Length < pstStream->u32Position)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstStream->u32Length);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstStream->u32Position);
        return HI_FAILURE;
    }
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, SrcBufSize);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstStream->u32Length);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstStream->u32Position);
        return HI_FAILURE;
    }
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pDstData, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pSrcData, HI_FAILURE);
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->u32Width[0]);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pBmpDec->InfoHeader.biBitCount);
...
static HI_S32 BMP_ParseFileHeader(HIGFX_IO_INSTANCE_S* const pstStream, HIGFX_BITMAPFILEHEADER *pFileHeader)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstStream, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pFileHeader, HI_FAILURE);

    /** FileType */
    Ret = HI_GFX_IORead(pstStream, Bytes, 2, &CopyLen, &EndFlag);
    if ((HI_SUCCESS != Ret) || (CopyLen < 2))
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, CopyLen);
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_GFX_IORead, Ret);
        return HI_FAILURE;
    }
    ...
    {
       HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, CopyLen);
       HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, BMP_DWORD_LEN);
       HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_GFX_IORead, Ret);
       return HI_FAILURE;
    }
    ...
    if (0 != Reserved)
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, Reserved);
        return HI_FAILURE;
    }
...
static HI_S32 BMP_ParseBitMapInfoHeader(HIGFX_IO_INSTANCE_S* const pstStream, HIGFX_BITMAPINFOHEADER *pInfoHeader)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstStream, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pInfoHeader, HI_FAILURE);
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pInfoHeader->biSize);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, BMP_INFOHEADER_LEN);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, BMP_INFOV4HEADER_LEN);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, BMP_INFOV5HEADER_LEN);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseBitMapV3InfoHeader, Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseBitMapV4InfoHeader, Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseBitMapV5InfoHeader, Ret);
        return HI_FAILURE;
    }
...
static HI_S32 BMP_ParseFile(BMP_DECODER_S *pDecoder)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pDecoder, HI_FAILURE);

    Ret = BMP_ParseFileHeader((HIGFX_IO_INSTANCE_S* const)(&(pDecoder->stStream)), &(pDecoder->FileHeader));
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseFileHeader, Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseBitMapInfoHeader, Ret);
        return HI_FAILURE;
    }
...
static HI_VOID BMP_GetPixelFmt(const BMP_DECODER_S *pBmpDec, HI_GFX_PF_E *enPixelFormat)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pBmpDec);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, enPixelFormat);
        return;
    }
...
HI_S32 BMP_ADP_CreateDecode(HI_U32 *pDecHandle,HI_CHAR* pSrcBuf,HI_U32 u32SrcLen)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_ParseFile, Ret);
        goto ERR2;
    }

    if (0 != pBmpDecoder->InfoHeader.biCompression)
    {/** 只支持非压缩的的bmp文件 **/
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "only support uncompress bmp file");
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pBmpDecoder->InfoHeader.biCompression);
        goto ERR2;
    }
...
HI_S32 BMP_ADP_StartDecode(HI_U32 DecHandle, HI_HANDLE hSurface)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_GFX_IOSeek, Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, BMP_Read_DecData, Ret);
        return HI_FAILURE;
    }
...
HI_S32 BMP_ADP_GetImgInfo(HI_U32 DecHandle, HI_HANDLE hSurface)
{
    ...
    if (HI_GFX_PF_BUTT == pstSurface->enPixelFormat)
    {
        ...
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "HI_GFX_PF_BUTT = 17");
        return HI_FAILURE;
    }
    ...
    if ((pstSurface->u32Width[0] < CONFIG_GFX_MINWIDTH) || (pstSurface->u32Width[0] > CONFIG_GFX_MAXWIDTH))
    {
        ...
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "CONFIG_GFX_MAXWIDTH = 8192");
        return HI_FAILURE;
    }

    if ((pstSurface->u32Height[0] < CONFIG_GFX_MINHEIGHT) || (pstSurface->u32Height[0] > CONFIG_GFX_MAXHEIGHT))
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->u32Height[0]);
