...
HI_VOID DmxHalInitSpeCWOrder(Dmx_Set_S * DmxSet)
{
    ...
    Value |= 0x30033300;   //for aes novel, spe type:byte and word change
    DMX_WRITE_REG(DmxSet->IoBase, DMX_CW_CTRL0, Value);
}

/***********************************************************************************
* Function      : DmxHalInitSms4CWOrder
* Description   : set ca type sms4 cw order as byte and word change
* Input         :
* Output        :
* Return        :
* Others        :
***********************************************************************************/
HI_VOID DmxHalInitSms4CWOrder(Dmx_Set_S * DmxSet)
{
    HI_U32 Value;

    Value = DMX_READ_REG(DmxSet->IoBase, DMX_CW_CTRL1);

    Value |= 0x33303333;   //for aes ctr ,ase cbc_l, aes ecb_l, sms4:byte and word change
    DMX_WRITE_REG(DmxSet->IoBase, DMX_CW_CTRL1, Value);
}
...
HI_VOID DmxHalSetSecureKey(Dmx_Set_S *DmxSet, HI_U32 KeyId, HI_BOOL Secure)
{
    HI_U32 value;
