...
static void hash_set_entry(u32 chn, compat_addr dma_addr, void *cpu_addr)
{
    ...
    HI_LOG_DEBUG("CHANn_HASH_IN_NODE_CFG[0x%x]: \t0x%x, PHY: 0x%x, VIA %p\n",
...
static void hash_smmu_bypass(void)
{
    ...
    HI_LOG_DEBUG("HASH_IN_SMMU_EN[0x%x]  : 0x%x\n", HASH_IN_SMMU_EN, hash_in_smmu_en.u32);
#endif
}
...
void hash_enrty_init(crypto_mem mem)
{
    ...
    U_HASH_NON_SEC_CHN_WHO_USED used;
    u32 my_mask = HASH_CHN_WHO_USED_NON_SEC;

    if (HI_TRUE == crypto_is_sec_cpu())
    {
        my_mask = HASH_CHN_WHO_USED_SEC;
    }

    HI_LOG_DEBUG("symc entry list configure\n");
    ADDR_U64(mmz_addr) = ADDR_U64(mem.mmz_addr);
    cpu_addr = mem.dma_virt;
    used.u32 = HASH_READ(HASH_NON_SEC_CHN_WHO_USED);

    for (i=0; i<CRYPTO_HARD_CHANNEL_MAX; i++)
    {
        if ((HASH_HARD_CHANNEL_MASK >> i) & 0x01) /*valid channel*/
        {
            HASH_CHN_WHO_USED_SET(used.bits.non_sec_chnn_who_used, i, my_mask);
            HI_LOG_DEBUG("HASH_NON_SEC_CHN_WHO_USED 0x%x, ", used.u32);
    ...
    HASH_WRITE(HASH_NON_SEC_CHN_WHO_USED, used.u32);
...
void hash_enrty_deinit(void)
{
    ...
    for (i=0; i<CRYPTO_HARD_CHANNEL_MAX; i++)
    {
        if ((HASH_HARD_CHANNEL_MASK >> i) & 0x01) /*valid channel*/
        {
            HASH_CHN_WHO_USED_CLR(used.bits.non_sec_chnn_who_used, i);
        }
    }
    HASH_WRITE(HASH_NON_SEC_CHN_WHO_USED, used.u32);
...
s32 drv_hash_init(void)
{
    ...
    ret = crypto_mem_create(&hash_dma, SEC_MMZ, "CIPHER_SHA_NODE", HASH_NODE_LIST_SIZE);
    if (HI_SUCCESS != ret)
    {
    ...
    HI_LOG_DEBUG("HASH DMA buffer, MMU 0x%x, MMZ 0x%x, VIA %p, size 0x%x\n",
...
//  flush_cache(); /* may reduce the performance */
    ...
    HI_LOG_DEBUG("CHANn_HASH_IN_NODE_CFG: 0x%x\n", in_node_cfg.u32);
