...
HI_U8 *g_pWinProgInterleave[WIN_FRAME_FORCE_BUTT] =
{
    "PROG_REVISE_CLOSE",
    "PROG_REVISE_PROGRESSIVE",
    "PROG_REVISE_iNTERLEAVE",
};
...
static HI_S32 WINProcessCmdSetWindowProgInterleave( HI_VOID *arg, WIN_STATE_S *pstWinState);
static HI_S32 WINProcessCmdResetFirstFrameStatus( HI_VOID *arg, WIN_STATE_S *pstWinState);
static HI_S32 WINProcessCmdSetWindowProgInterleave( HI_VOID *arg, WIN_STATE_S *pstWinState);
static HI_S32 WINProcessCmdGetWindowProgInterleave( HI_VOID *arg, WIN_STATE_S *pstWinState);
static HI_S32 WINProcessCmdNoSupprot(HI_VOID *arg, WIN_STATE_S *pstWinState);
...
static const stWINProcessCmdFunc s_aWINCmdFuncTab[] =
{
    ...
    {CMD_WIN_QUDOLBY_FRAME, WINProcessCmdNoSupprot},
    {CMD_WIN_QUSYNC_FRAME, WINProcessCmdNoSupprot},
    ...
    {CMD_WIN_SET_ROTATION, WINProcessCmdNoSupprot},
    {CMD_WIN_GET_ROTATION, WINProcessCmdNoSupprot},
    {CMD_WIN_SET_FLIP, WINProcessCmdNoSupprot},
    {CMD_WIN_GET_FLIP, WINProcessCmdNoSupprot},
    {CMD_WIN_SET_FROST_MODE, WINProcessCmdNoSupprot},
    {CMD_WIN_GET_FROST_MODE, WINProcessCmdNoSupprot},
    {CMD_WIN_SET_MODE, WINProcessCmdNoSupprot},
    {CMD_WIN_GET_OUT_TYPE_AND_ENGINE, WINProcessCmdGetOutTypeAndEngine},
    {CMD_WIN_GET_DISP_OUT_TYPE_AND_GFX_STATE, WINProcessCmdNoSupprot},
    {CMD_WIN_GET_UNLOAD, WINProcessCmdGetUnload},
    {CMD_WIN_CLOSE_HDR_PATH, WINProcessCmdCloseHDRPath},
    {CMD_WIN_SET_DOLBY_LIB_STATUS, WINProcessCmdNoSupprot},
    {CMD_WIN_DEBUG, WINProcessCmdDebug},
    {CMD_WIN_SET_ALPHA, WINProcessCmdSetAlpha},
    {CMD_WIN_GET_ALPHA, WINProcessCmdGetAlpha},
    {CMD_WIN_SET_FRAME_PROGINTER, WINProcessCmdSetWindowProgInterleave},
    {CMD_WIN_GET_FRAME_PROGINTER, WINProcessCmdGetWindowProgInterleave},
    {CMD_WIN_RESET_FIRST_FRAME_STATUS, WINProcessCmdResetFirstFrameStatus},
};

static HI_S32 WINProcessCmdNoSupprot(HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    HI_ERR_WIN("no supprot this cmd.\n");
...
HI_VOID Win_GetProcBufStatus(struct seq_file *p, WIN_PROC_INFO_S *pstProcInfo)
{
    HI_U32 i = 0;
    ...
    if (g_pWinProcLevel >= WIN_PROC_EXTREN_WIN_POSI)
    {
        PROC_PRINT(p,
                   "Remain playcnts            :%d\n"
                   "Remain FrameCnts           :%d\n"
                   "Remain DequeedFrameCnts    :%d\n",
                   pstProcInfo->u32BuffedPlayCnts,
                   pstProcInfo->u32BuffedQueueNodeCnts,
                   pstProcInfo->u32BuffedDequeueNodeCnts);
    }
    ...
    PROC_PRINT(p,
               ...
               "During Time          :%d\n"
               "Total queue frame    :%d\n"
               "Total queue playcnts :%d\n",
               pstProcInfo->stWinDebugInfo.stWinPlayStatus.u32CurrentTime - pstProcInfo->stWinDebugInfo.stWinPlayStatus.u32FirstQueueTime,
               pstProcInfo->stWinDebugInfo.stWinPlayStatus.u32TotalFrame,
               pstProcInfo->stWinDebugInfo.stWinPlayStatus.u32TotalPlayCnt);
...
extern hiXDP_PQ_LIST_S g_stPQListBuf;
extern hiXDP_PQ_LIST_S g_stPQListPara;
...
HI_S32 DRV_WIN_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    if ( (pstProcInfo->enType != HI_DRV_WIN_ACTIVE_SLAVE))
    {
        ...
        PROC_PRINT(p, "%-19s:%-20s|\n", "ProgInter Flag", g_pWinProgInterleave[pstProcInfo->stWinDebugInfo.stWinPlayCtrl.eWinFrameForceFlag]);
    ...
    if (g_pWinProcLevel >= WIN_PROC_EXTREN_WIN_POSI)
    {
        PROC_PRINT(p, "-----------------Buf List(node cnt %d)-----------------\n",
            g_stPQListBuf.u32ListNodeCnt);
        Win_PQAlg_ShowListState(p, &g_stPQListBuf);
        PROC_PRINT(p, "-----------------Para List(node cnt %d)-----------------\n",
            g_stPQListBuf.u32ListNodeCnt);

#ifdef XDP_SUPPORT_ALG_STAT
        Win_PQAlg_ShowListState(p, &g_stPQListPara);
        Win_PQAlg_ShowTimeStat(p, &g_stPQListBuf);
#endif
        Win_PQAlg_ShowTaskState(p);
    }
...
HI_VOID WIN_ProcWriteHelp(HI_VOID)
{
    ...
    HI_DRV_PROC_EchoHelper("echo run    0,1,2,3...       > /proc/msp/winXXXX  \n");
#ifdef XDP_SUPPORT_MEM_MAGIC
    HI_DRV_PROC_EchoHelper("echo magic   on/off          > /proc/msp/winXXXX  \n");
#endif
}

#ifdef HI_PROC_SUPPORT
extern HI_S32 g_s32TestQueueCnt;
HI_S32 WinProcSetQueueCnt(HI_HANDLE hWin, HI_CHAR *pArg2)
{
    g_s32TestQueueCnt = (HI_S32)simple_strtol(pArg2, NULL, 10);
    HI_DRV_PROC_EchoHelper("set queue count %d\n", g_s32TestQueueCnt);
    return HI_SUCCESS;
}
...
HI_S32 WinProcSetMagicEnable(HI_HANDLE hWin, HI_CHAR *pArg2)
{
    HI_S32  s32Ret = HI_SUCCESS;
#ifdef XDP_SUPPORT_MEM_MAGIC
    if (0 == HI_OSAL_Strncmp(pArg2, "on", strlen("on")))
    {
        s32Ret = WIN_SetMagicEnable(hWin, HI_TRUE);
    }
    else if (0 == HI_OSAL_Strncmp(pArg2, "off", strlen("off")))
    {
        s32Ret = WIN_SetMagicEnable(hWin, HI_FALSE);
    }
    else
    {
        s32Ret = HI_FAILURE;
...
#else
    s32Ret = HI_FAILURE;
    HI_ERR_WIN("Magic is not supported!\n");
#endif
    return s32Ret;
}
static const WIN_PROC_COMMAND WinProcCmds[] =
{
    ...
    {"asp",  WinProcSetASP},
    {"frc",  WinProcSetFRC},
    {"run", WinProcSetQueueCnt},
    {"magic", WinProcSetMagicEnable},
...
static HI_S32 WINProcessCmdReset( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_INFO("win reset win0x%x, %s,use-%dms,ret=%d\n", pVoWinReset->hWindow,
...
static HI_S32 WINProcessCmdGetLatestFrameInfo( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    Ret = WinGetLatestFrameInfor(frame_info->hWindow, &frame_info->stFrame);
...
static HI_S32 WINProcessCmdSetWindowProgInterleave( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    WIN_FORCEFRAME_FLAG_S *pstWinFrameForceFlag = HI_NULL ;

    pstWinFrameForceFlag  = (WIN_FORCEFRAME_FLAG_S *)arg;
    Ret = WIN_CheckHanlde(pstWinFrameForceFlag->hWindow, pstWinState);
    if (HI_SUCCESS == Ret)
    {
        Ret = WIN_SetWindowProgInterleave(pstWinFrameForceFlag->hWindow,pstWinFrameForceFlag->eForceFrameProgFlag);
    }
...
static HI_S32 WINProcessCmdGetWindowProgInterleave( HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    if (HI_SUCCESS == Ret)
    {
        Ret = WIN_GetWindowProgInterleave(pstWinFrameForceFlag->hWindow,&pstWinFrameForceFlag->eForceFrameProgFlag);
    }
...
HI_S32 HI_DRV_WIN_SetWinPlayCtrl(HI_DRV_WIN_PLAYCTRL_OPERATION_INFOR_S *pstWinCtrlInfor)
{
    HI_S32 Ret = HI_SUCCESS;
    Ret = WIN_SetWindowTotalPlayCtrl(pstWinCtrlInfor);
    return Ret;
}
...
HI_S32 HI_DRV_WIN_GetDeiMemsize(HI_HANDLE hVdec, HI_DRV_DEI_MEM_INFO_S *pDeiMemInfo)
{
    HI_U32 u32BufSize = 0;
    HI_S32 ret = HI_FAILURE;

    ret = Win_GetAlgMemsize(hVdec, pDeiMemInfo->u32BitDepth, &u32BufSize, HI_NULL);
    if (HI_SUCCESS == ret)
    {
        pDeiMemInfo->u32DeiMemSize = u32BufSize;
    }
...
HI_S32 HI_DRV_WIN_SetWindowProgInterleave(HI_HANDLE hWin, HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E eForceFrameProgFlag)
{
    HI_S32     Ret = 0;
    WIN_FORCEFRAME_FLAG_S stWinProgInterleave = {0};

    stWinProgInterleave.hWindow             = hWin;
    stWinProgInterleave.eForceFrameProgFlag = eForceFrameProgFlag;
    Ret = DRV_WIN_Process(CMD_WIN_SET_FRAME_PROGINTER, &stWinProgInterleave);

    return Ret;
}


HI_S32 HI_DRV_WIN_GetWindowProgInterleave(HI_HANDLE hWin,
                                                          HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E *peForceFrameProgFlag)
{
    ...
    stWinProgInterleave.hWindow = hWin;
    Ret = DRV_WIN_Process(CMD_WIN_GET_FRAME_PROGINTER, &stWinProgInterleave);
    if (HI_SUCCESS == Ret)
    {
        *peForceFrameProgFlag = stWinProgInterleave.eForceFrameProgFlag;
    }
...
static WIN_EXPORT_FUNC_S s_stWinExportFuncs =
{
    ...
    .pfnWinGetAlgMemSize = HI_DRV_WIN_GetDeiMemsize,
};

/**for vir window cut code**/
