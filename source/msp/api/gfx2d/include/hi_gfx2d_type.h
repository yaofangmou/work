...
typedef enum hiGFX2D_SURFACE_TYPE_E
{
    ...
    HI_GFX2D_SURFACE_TYPE_COLOR,       /**<CNcomment:纯色类型surface，不需要等fence之类，无内存操作 */
...
typedef enum hiGFX2D_FILTER_MODE_E
{
    ...
    HI_GFX2D_FILTER_NONE,      /**<CNcomment:不进行滤波,使用复制模式, 可能有锯齿, 对哪个做滤波看效果调试 */
...
typedef struct hiGFX2D_SURFACE_S
{
    ...
    /**<CNcomment:内存地址信息,只对mem类型的surface有效
                  ...
                  planar格式:         u32Phyaddr[0]表示Y分量地址,  u32Phyaddr[1]表示Cb分量地址, u32Phyaddr[2]表示Cr分量地址
      **/
    ...
    /**<CNcomment:行跨度,只对mem类型的surface有效
                  ...
                  planar格式:      u32Stride[0]表示Y分量行跨度,   u32Stride[1]表示Cb分量行跨度, u32Stride[2]表示Cr分量行跨度
     **/
    HI_U32 u32Stride[HI_GFX_SURFACE_STRIDE_CNT];
    HI_BOOL bPremulti;                   /**<CNcomment:给ARGB乘于一个alpha(像素alpha值), 是否为预乘数据, 非预乘数据做预乘, 预乘数据不做预乘, 和GPU保持一致 */
    HI_GFX2D_COMPRESS_S stCompressInfo;  /**<CNcomment:压缩信息，只对MEM类型surface有效*/
    HI_U32  u32HistogramPhyaddr;         /**<CNcomment:直方图物理地址，只对MEM类型surface有效，暂时没有使用 */
    HI_GFX2D_ALPHAEXT_S stAlphaExt;      /**<CNcomment:alpha扩展信息,只对MEM类型surface的ARGB1555格式有效*/
    HI_U32  u32PalettePhyaddr;           /**<CNcomment:调色板地址,只对MEM类型surface的调色板格式有效，HWC逻辑不支持 */
...
{
    HI_BOOL bCovBlend;          /** 当前层是否和下一层做叠加, TRUE: 不做叠加, 当前层直接覆盖下一层, 6是最上层 **/
    HI_BOOL bPixelAlphaEnable;  /** alpha参与像素点的分量运算     */
    HI_BOOL bGlobalAlphaEnable; /** 全局alpha参与像素点的分量运算 */
...
typedef struct hiGFX2D_COMPOSE_OPT_S
{
    ...
    HI_GFX2D_CLIP_S          stClip;     /**<CNcomment:剪切域信息. HWC逻辑现在不支持 */
    HI_GFX2D_ROP_S           stRop;      /**<CNcomment:ROP信息. HWC逻辑现在不支持    */
    HI_GFX2D_COLORKEY_S      stKey;      /**<CNcomment:key信息. HWC逻辑现在不支持    */
...
{
    HI_GFX2D_COMPOSE_S *pstCompose;     /**<CNcomment: 叠加层链表。按照由下到上的顺序排列, 0在最下面 */
    HI_U32             u32ComposeCnt;   /**<CNcomment: 叠加层数量。*/
    HI_U32             u32BgColor;      /**<CNcomment: 叠加背景色。软件只需配置一个背景色，虚拟层，在最下层 */
