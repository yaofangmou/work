...
*                           ALL RIGHTS RESERVED
* FileName: hi_unf_ci.c
* Description:
*
* History:
* Version   Date                Author     DefectNum    Description
* main\1    2011-08-02         sdk   NULL         Create this file.
***********************************************************************************/
#include <stdio.h>
#include <sys/types.h>
#include <sys/stat.h>
...
HI_S32 HI_MPI_CI_PCCD_GetCIS(HI_UNF_CI_PORT_E enCIPort, HI_UNF_CI_PCCD_E enCardId, HI_U8 *pu8CisBuffer, HI_U32 *pu32CisLen)
{
    HI_S32 s32Ret;
    CI_PCCD_CIS_S stCIS;
    if (enCIPort >= HI_UNF_CI_PORT_BUTT)
    ...
    if (HI_NULL == pu8CisBuffer)
    {
        HI_ERR_CI("Invalid pu8CisBuffer.\n");
        return HI_ERR_CI_INVALID_PARA;
    }
    
    if (HI_NULL == pu32CisLen)
    {
        HI_ERR_CI("Invalid pu32CisLen.\n");
        return HI_ERR_CI_INVALID_PARA;
    }
    
    CHECK_CI_OPEN();

    stCIS.enCIPort = enCIPort;
    stCIS.enCardId = enCardId;
    stCIS.pu8Buffer = pu8CisBuffer;
    stCIS.pu32CisLen = pu32CisLen;
    s32Ret = ioctl(s_CIDevFd, CMD_CI_PCCD_GETCIS, &stCIS);

    return s32Ret;
}


HI_S32 HI_MPI_CI_PCCD_WriteCOREx(HI_UNF_CI_PORT_E enCIPort, HI_UNF_CI_PCCD_E enCardId, HI_U16 u16Addr, HI_U8 u8Data)
{
    HI_S32 s32Ret;
    CI_PCCD_COR_S stCOR;
    if (enCIPort >= HI_UNF_CI_PORT_BUTT)
    ...
    stCOR.enCIPort = enCIPort;
    stCOR.enCardId = enCardId;
    stCOR.u16Addr = u16Addr;
    stCOR.u8Data = u8Data;
    s32Ret = ioctl(s_CIDevFd, CMD_CI_PCCD_WRITECOREX, &stCOR);
