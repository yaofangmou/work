...
static SINT32 SOS_show_main_interface_time(SINT32 ChanId, SINT8 *Page, SINT32 PageSize)
{
    SINT32 UsedSize = 0;
    VFMW_CHAN_STAT_S *pChanStat = NULL;
    ...
    STAT_DUMP(Page, "---------------------- Main Interface Time INFO -----------------------\n");
    STAT_DUMP(Page, "%-40s :%d ms\n",    "Chan Creat   use", pChanStat->u32ChanCreatTime);
...
SINT32 SOS_PROC_ReadChan(SINT8 *Page, SINT32 PageSize)
{
    ...
    UsedSize += SOS_show_main_interface_time(ChanId, Page, PageSize);
