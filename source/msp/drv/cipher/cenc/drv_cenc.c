...
s32 cenc_ddr_alloc(u32 chn_num, u32 length)
{
    ...
    if (cenc_info.local.u32Size < length)
    {
        ...
        HI_LOG_DEBUG("cenc alloc ddr, phy 0x%x, via %p, size 0x%x\n",
...
void cenc_ddr_release(void)
{
    MMZ_BUFFER_S stMBuf = {0};

    HI_LOG_FuncEnter();

    if (0x00 != cenc_info.local.u32Size)
    {
        stMBuf.u32StartPhyAddr = cenc_info.local.u32StartSmmuAddr;
        stMBuf.pu8StartVirAddr = cenc_info.local.pu8StartVirAddr;
        stMBuf.u32Size         = cenc_info.local.u32Size;
...
s32 cenc_ddr_alloc(u32 chn_num, u32 length)
{
    ...
    if (cenc_info.local.u32Size < length)
    {
        ...
        cenc_ddr_release();
         ...
         /* allocate a large mmz, only support 32-bit ddr */
        ret = HI_DRV_MMZ_AllocAndMap(CENC_MMU_NAME, HI_NULL, ALIGN4k(length), 0, &stMBuf);
        if (HI_SUCCESS != ret)
        {
            HI_LOG_PrintFuncErr(HI_DRV_MMZ_AllocAndMap, ret);
            return ret;
        }
        cenc_info.local.u32StartSmmuAddr = stMBuf.u32StartPhyAddr;
        cenc_info.local.pu8StartVirAddr  = stMBuf.pu8StartVirAddr;
        cenc_info.local.u32Size          = ALIGN4k(length);
        HI_LOG_DEBUG("cenc alloc ddr, phy 0x%x, via %p, size 0x%x\n",
            cenc_info.local.u32StartSmmuAddr, cenc_info.local.pu8StartVirAddr, cenc_info.local.u32Size);
...
static s32 cenc_ddr_map(SMMU_BUFFER_S *input, SMMU_BUFFER_S *output)
{
    ...
    stMBuf.u32StartPhyAddr = input->u32StartSmmuAddr;
    stMBuf.u32Size         = input->u32Size;
    ...
    input->pu8StartVirAddr = stMBuf.pu8StartVirAddr;

    stMBuf.u32StartPhyAddr = output->u32StartSmmuAddr;
    stMBuf.u32Size         = output->u32Size;
    ret = HI_DRV_MMZ_Map(&stMBuf);
    if (HI_SUCCESS != ret)
    {
        stMBuf.u32StartPhyAddr = input->u32StartSmmuAddr;
        stMBuf.pu8StartVirAddr = input->pu8StartVirAddr;
        stMBuf.u32Size         = input->u32Size;
        HI_DRV_MMZ_Unmap(&stMBuf);
        HI_LOG_PrintFuncErr(HI_DRV_MMZ_Map, ret);
        return ret;
    }
    output->pu8StartVirAddr = stMBuf.pu8StartVirAddr;
...
static s32 cenc_ddr_unmap(SMMU_BUFFER_S *input, SMMU_BUFFER_S *output)
{
    ...
    {
        stMBuf.u32StartPhyAddr = output->u32StartSmmuAddr;
        stMBuf.pu8StartVirAddr = output->pu8StartVirAddr;
        stMBuf.u32Size         = output->u32Size;
        HI_DRV_MMZ_Unmap(&stMBuf);
    }
...
static HI_VOID cenc_config(u32 chn_num, CRYP_CIPHER_CTRL_S *ctrl)
{
    ...
    ret = drv_symc_config(chn_num, SYMC_ALG_AES, mode, SYMC_DAT_WIDTH_128,
                                     ...
                                     u32 *penclen, u32 *pu32DecLen)
{
    ...
    *pu32DecLen = 0x00;
    ...
    HI_LOG_DEBUG("First Pattern, enc_len 0x%x, dec len 0x%x\n", *penclen, *pu32DecLen);
}
...
static u32 cenc_ctr128_iv_blocks(HI_U8* iv)
{
    ...
    HI_LOG_DEBUG("high iv_number 0x%x\n", iv_number);
    ...
    iv_number |= ((HI_U32)iv[id++]) << 8;
    iv_number |= ((HI_U32)iv[id++]);
    HI_LOG_DEBUG("low iv_number 0x%x\n", iv_number);
...
static u32 cenc_node_get_counter_length(u32 length)
{
    ...
    HI_LOG_DEBUG("CTR IV Counter 0x%x, total len 0x%x, cur len 0x%x, cur counter 0x%x, first len 0x%x\n",
                                        ...
                                        CRYP_CIPHER_SUBSAMPLE_EX_S *pstSubsample)
{
    ...
    while (index < subsample_len)
    {
        skip = cenc_get_clear_len(enWorkMode, pstSubsample, firstoffset,
                                       index, &offset, &length);
        HI_LOG_DEBUG("CBC Pattern clear data, index 0x%x, offset 0x%x, len 0x%x, skip 0x%x\n",
                                ...
                                CRYP_CIPHER_SUBSAMPLE_EX_S *pstSubsample)
{
    ...
    while (index < subsample_len)
    {
        ...
        HI_LOG_DEBUG("Pattern encrypt data, index 0x%x, enc offset 0x%x, len 0x%x, skip 0x%x, usage 0x%x\n",
                            ...
                            u32 number)
{
    ...
    for (i=0; i<number; i++)
    {
        ...
        HI_LOG_DEBUG("subsample %d, input 0x%x, output 0x%x, iv-change %d\n", i,
                                ...
                                CRYP_CIPHER_SUBSAMPLE_EX_S *pstSubsample)
{
    ...
    while (index < subsample_len)
    {
        /* get a pattern's encrypt data from subsample */
        skip = cenc_get_encrypt_len(enWorkMode, pstSubsample, first_offset, index, &offset, &length);
        HI_LOG_DEBUG("Pattern encrypt data, index 0x%x, enc offset 0x%x, len 0x%x, skip 0x%x\n",
            index, offset, length, skip);
                            ...
                            u32 number)
{
    ...
    for (i=0; i<number; i++)
    {
        HI_LOG_DEBUG("subsample %d, input 0x%x, offset 0x%x, output 0x%x, iv-change %d\n", i,
                    ...
                    u32 number)
{
    s32 ret = HI_FAILURE;
    u32 chn_num = 0;
    u32 fast_ctr = HI_FALSE;
    CRYP_CIPHER_CTRL_S ctrl;
    SMMU_BUFFER_S inputBuffer = {0};
    SMMU_BUFFER_S outputBuffer = {0};
    ...
    if (HI_TRUE == fast_ctr)
    {
        /* fast CTR may be write the overflow ddr which less than 16 */
        inputBuffer.u32Size  += AES_BLOCK_SIZE;
        outputBuffer.u32Size += AES_BLOCK_SIZE;
    }
                                  ...
                                  CRYP_CIPHER_SUBSAMPLE_EX_S *subsample_ex)
{
    ...
    if (HI_FALSE == oddkey_support)
    {
        ...
        HI_LOG_DEBUG("*** copy SubSample to SubSampleEx ***\n");
    }
    else
    {
