...
HI_VOID processor_save_src_frame(OMXVDEC_CHAN_CTX *pchan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (OMX_H263 == pchan->protocol || OMX_SORENSON == pchan->protocol)
    {
...
static HI_U32 processor_get_stride(HI_U32 protocol, HI_U32 width, HI_BOOL is_native_output)
{
    ...
    if (is_native_output)
    {
        if ((protocol == OMX_HEVC) || (protocol == OMX_VP9) || (protocol == OMX_AVS2))
        {
...
static HI_S32 processor_get_frame_buffer(OMXVDEC_CHAN_CTX *pchan, HI_VOID *pstArgs)
{
    ...
    if (pVpssFrm->u32FrmW != pchan->out_width || pVpssFrm->u32FrmH != pchan->out_height)
    {
        ...
        if (pchan->is_adaptive == HI_FALSE
            || (pVpssFrm->u32FrmW > pchan->max_width || pVpssFrm->u32FrmH > pchan->max_height))
        {
            if (pVpssFrm->u32FrmW > pchan->max_width)
            {
                pchan->max_width = pVpssFrm->u32FrmW;
            }

            if (pVpssFrm->u32FrmH > pchan->max_height)
            {
                pchan->max_height = pVpssFrm->u32FrmH;
            }
        ...
        {
            OmxPrint(OMX_INFO, "only report size change\n");
            ImgSize.frame_stride = processor_get_stride(pchan->protocol, pchan->max_width, pchan->m_use_native_buf);
            pchan->out_stride = ImgSize.frame_stride;
...
static HI_VOID processor_convert_desc_info_part1(OMXVDEC_CHAN_CTX *pchan, IMAGE *pstImage, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (pchan->protocol == OMX_VP6 || pchan->protocol == OMX_VP6F || pchan->protocol == OMX_VP6A)
    {
        pstFrame->u32Circumrotate = (OMX_VP6A == pchan->protocol) ? pchan->bReversed & 0x1 : !(pchan->bReversed & 0x1);
    }
    else
    {
    ...
    if ((pstImage->disp_height <= 288 && pchan->protocol != OMX_HEVC && pchan->protocol != OMX_H264 && pchan->protocol != OMX_AVS2)
...
static HI_VOID processor_convert_desc_info_part2(OMXVDEC_CHAN_CTX *pchan, IMAGE *pstImage, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    if (pchan->protocol == OMX_HEVC || pchan->protocol == OMX_AVS2)
    {
        pstFrame->enBufValidMode = HI_DRV_FIELD_ALL;
    }
    else if (pchan->protocol == OMX_H264)
    {
        pstFrame->enBufValidMode = pstFrame->enFieldMode;
    }

    if ((pchan->protocol == OMX_HEVC || pchan->protocol == OMX_VP9 || pchan->protocol == OMX_AVS2)
...
HI_S32 processor_create_inst(OMXVDEC_CHAN_CTX *pchan, OMX_PIX_FORMAT_E color_format)
{
...
#endif
    if (pchan->bProDetectOff == HI_TRUE)
    {
        stVpssCfg.enProgInfo = HI_DRV_VPSS_PRODETECT_OFF;
    }
    ...
    if (pchan->android_test == HI_TRUE)
    {
        stVpssPortCfg.bPassThrough = VPSS_FORCE_NOT_BYPASS;
    }
    else
    {
        stVpssPortCfg.bPassThrough = (pchan->force_bypass == HI_TRUE) ? VPSS_FORCE_BYPASS : VPSS_BYPASS_AUTOBYVPSS;
    }

    if (pchan->is_adaptive == HI_TRUE)
    {
        stVpssPortCfg.bFixdOmxBuf = HI_TRUE;
        stVpssPortCfg.u32OmxBufWidth = pchan->max_width;
        stVpssPortCfg.u32OmxBufHeight = pchan->max_height;

        OmxPrint(OMX_INFO, "%s w:%d h:%d\n", __func__, pchan->max_width, pchan->max_height);
    }
...
HI_S32 processor_set_bypass(OMXVDEC_CHAN_CTX *pchan)
{
    ...
    OmxPrint(OMX_INFO, "%s force_bypass:%d output_view:%d\n", __func__, pchan->force_bypass, pchan->output_view);

    ret = processor_set_port(pchan);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_FATAL, "%s processor_set_port failed\n", __func__);
        return HI_FAILURE;
    }
...
HI_S32 processor_set_port(OMXVDEC_CHAN_CTX *pchan)
{
    HI_S32 ret;
    HI_DRV_VPSS_PORT_CFG_S stVpssPortCfg;
    ...
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_FATAL, "%s call VpssGetPortCfg failed\n", __func__);
        return HI_FAILURE;
    }

    if ((pchan->output_view == 0) || (pchan->android_test)) //1: view  0: texture
    {
        stVpssPortCfg.bPassThrough = VPSS_FORCE_NOT_BYPASS;
    }
    else
    {
        if (pchan->force_bypass)
        {
            stVpssPortCfg.bPassThrough = VPSS_FORCE_BYPASS;
        }
        else
        {
            stVpssPortCfg.bPassThrough = VPSS_BYPASS_AUTOBYVPSS;
        }
    }
    ...
    if (pchan->is_adaptive == HI_TRUE)
    {
        ...
        OmxPrint(OMX_FATAL, "%s w:%d h:%d\n", __func__, pchan->max_width, pchan->max_height);
    }

    ret = (pVpssFunc->pfnVpssSetPortCfg)(pchan->port_id, &stVpssPortCfg);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_FATAL, "%s call VpssSetPortCfg failed\n", __func__);
        return HI_FAILURE;
    }
