...
#define A_HC_TUNE                       0x9F8400
#define A_HC_REPLACE                    0x9F8401
#define A_HC_CLEAR_REPLACE              0x9F8402
#define A_HC_ASK_RELEASE                0x9F8403

#define WAIT_TIME (1000)
#define HOST_BUFFERSIZE (1024)
#define EVENT_PARAM_BUF_SIZE  4096

#define MAX_TPDU_BUF_SIZE   (EVENT_PARAM_BUF_SIZE - 64)
#define LPDU_HEAD_SIZE      2
#define MAX_TPDU_HEAD_SIZE  10
#define MAX_TPDU_SIZE       (MAX_TPDU_BUF_SIZE - LPDU_HEAD_SIZE)
...
static HI_VOID QueueReset(HI_UNF_CI_PCCD_E enCardId)
{
    ...
    while(stQueue != HI_NULL)
    {
        ...
        {
            stQueue->stEvent.enCardId = HI_UNF_CI_PCCD_BUTT;
        }
...
static HI_VOID QueueDestory(HI_VOID)
{
    CI_EVENT_QUEUE_S *stQueueTmp;

    pthread_mutex_lock(&g_CIMutex);
    while(s_stQueueHead != HI_NULL)
    {
        stQueueTmp = s_stQueueHead;
...
static HI_S32 QueuePutEvent( HI_UNF_CI_PCCD_E enCardId, CI_EVENT_E enEvent, TPDU_S *pstTpdu)
{
    CI_EVENT_QUEUE_S *stQueue;
    ...
    while(stQueue != HI_NULL)
    {
        if (stQueue->stEvent.enCardId == HI_UNF_CI_PCCD_BUTT)
        {
            break;
        }
        stQueue = stQueue->stNext;
    ...
    stQueue->stEvent.enEvent = enEvent;
    stQueue->stEvent.enCardId = enCardId;
    if(pstTpdu != HI_NULL)
    {
        stQueue->stEvent.stTpdu = *pstTpdu;
    }
    sem_post(&s_CISem);
...
static HI_S32 QueuePutTPDU(HI_UNF_CI_PCCD_E enCardId, TPDU_S *pstTPDU)
{
    ...
    return QueuePutEvent(enCardId, CI_EVENT_DATA_SEND, pstTPDU);
}
...
static HI_S32 QueueGetEvent(CI_EVENT_S *pstEvent, HI_U32 u2TimeOutMs)
{
    ...
    stQueue = s_stQueueHead;
    while((stQueue != HI_NULL) && (stQueue->stEvent.enCardId == HI_UNF_CI_PCCD_BUTT))
    {
        stQueue = stQueue->stNext;
    }
    if (stQueue == HI_NULL)
    ...
    memcpy((HI_VOID*)pstEvent, (HI_VOID*)&stQueue->stEvent, sizeof(CI_EVENT_S));
    stQueue->stEvent.enCardId = HI_UNF_CI_PCCD_BUTT;
             ...
             HI_U16 service_id)
{
    ...
    HI_PRINT("network_id          0x%04X\n", network_id);
    HI_PRINT("original_network_id 0x%04X\n", original_network_id);
    HI_PRINT("transport_stream_id 0x%04X\n", transport_stream_id);
    HI_PRINT("service_id          0x%04X\n", service_id);
...
HI_VOID GetDateTime(HI_U8* mjd)
{
    ...
    mjd[0] = (HI_U8)((days >> 8) & 0xff);         /* msb of date   */
    mjd[1] = (HI_U8)(days & 0xff);                /* lsb of date   */
    mjd[2] = (HI_U8)(((p->tm_hour/ 10U) << 4) | (p->tm_hour % 10)); /* hour          */
    mjd[3] = (HI_U8)(((p->tm_min / 10U) << 4) | (p->tm_min % 10));   /* min           */
    mjd[4] = (HI_U8)(((p->tm_sec / 10U) << 4) | (p->tm_sec % 10));   /* sec           */
    mjd[5] = 0;
    mjd[6] = 0;

    HI_PRINT("current date: %d.%d.%d %d:%d:%d\n", p->tm_year + 1900, p->tm_mon+1,
...
static HI_S32 ApplicationLayer(HI_UNF_CI_PCCD_E enCardId, HI_U16 u16SessionNum, HI_U8 *pu8APDU, HI_U32 u32CmdLen)
{
   ...
   case A_HC_TUNE:
        ...
        break;
    case A_HC_REPLACE:
        HI_PRINT("Tune Replace Info:\n");

        HI_PRINT("replacement_ref     0x%02X\n", pu8APDU[u32Index++]);

        u32Nb  = (HI_U32)(pu8APDU[u32Index++] & 0x1F) * 256;
        u32Nb += (HI_U32)pu8APDU[u32Index++];
        HI_PRINT("replaced_PID        0x%04X\n", u32Nb);

        u32Nb  = (HI_U32)(pu8APDU[u32Index++] & 0x1F) * 256;
        u32Nb += (HI_U32)pu8APDU[u32Index++];
        HI_PRINT("replacement_PID     0x%04X\n", u32Nb);

        // TODO: here replace to a specially service

        break;
    case A_HC_CLEAR_REPLACE:
        HI_PRINT("Tune Clear Replace Info:\n");

        HI_PRINT("replacement_ref     0x%02X\n", pu8APDU[u32Index++]);

        // TODO: here clear a specially service
...
static HI_VOID CITthreadMonitor(HI_VOID *args)
{
            ...
            else
            {
                ...
                if (enStatus != aenCardStatus[enCardId])
                {
                    ...
                    if (HI_UNF_CI_PCCD_STATUS_ABSENT == enStatus)
                    {
                        ...
                        enEvent = CI_EVENT_CAM_REMOVE;
                    }
                    /* Insert */
                    else //HI_UNF_CI_PCCD_STATUS_PRESENT
                    {
                        HI_PRINT("Card %c insert.\n", enCardId == HI_UNF_CI_PCCD_A ? 'A' : 'B');
                        enEvent = CI_EVENT_CAM_INSERT;
                    }
                    aenCardStatus[enCardId] = enStatus;
                    s32Ret = QueuePutEvent(enCardId, enEvent, HI_NULL);
                    if (HI_SUCCESS != s32Ret)
                    {
                ...
                {
                    s32Ret = QueuePutEvent(enCardId, CI_EVENT_POLLING, HI_NULL);
                }
            }
        }
        usleep(100000);    /* 100ms */
    ...
    s32Ret = QueuePutEvent(HI_UNF_CI_PCCD_A, CI_EVENT_DESTORY, HI_NULL);
...
HI_U32 HI_CI_UpdatePMT(HI_UNF_CI_PCCD_E enCardId, HI_U8  *pPmtData, CA_PMT_LIST_MANAGEMENT_E enListManagement)
{
    APDU_S stAPDU;
    ...
    stAPDU.u8Data[stAPDU.u32Size++] = enListManagement; //ca_pmt_list_management
