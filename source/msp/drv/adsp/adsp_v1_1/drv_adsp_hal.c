...
static HI_S32 DSPMapShareSRam(HI_VOID)
{
    ...
    s32Ret = HI_DRV_MMZ_Alloc("AOE_SHARED_RAM",
                              MMZ_OTHERS,
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADSP("HI_DRV_MMZ_Alloc AOE_SHARED_RAM failed\n");
        return s32Ret;
    }

    s32Ret = HI_DRV_MMZ_MapCache(&g_stDspRegMmz);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADSP("HI_DRV_MMZ_MapCache AOE_SHARED_RAM failed\n");
        HI_DRV_MMZ_Release(&g_stDspRegMmz);
        return s32Ret;
    }
#else
...
static HI_VOID DSPUnMapShareSRam(HI_VOID)
{
    ...
    {
#if defined(CHIP_TYPE_hi3716mv430)
        HI_DRV_MMZ_Unmap(&g_stDspRegMmz);
        HI_DRV_MMZ_Release(&g_stDspRegMmz);
#else
