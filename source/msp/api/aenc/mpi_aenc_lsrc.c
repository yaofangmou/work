        ...
        if (insamps > 0)
        {
            while (pInst->Remainder < pInst->OutRate)
            {
                for (ch = 0; ch < Channels; ch++)
                {
                    ...
                    sum = CLIPTOSHORT(((HI_U32)sum) >> 14); /* IvtSFFactor is 2.30 format */
                    pOutpcmBuf[Channels * out + ch] = ((HI_U32)sum) << 16;
#else
        ...
        if (insamps > 0)
        {
            while (pInst->Remainder < pInst->OutRate)
            {
                for (ch = 0; ch < Channels; ch++)
                {
                    ...
                    sum  = MULSHIFT32(pcmbuf[Channels * (in + 0) + ch], pInst->IvtSFFactor * pInst->Remainder);
                    sum += MULSHIFT32(pInst->PreSample[ch], pInst->IvtSFFactor * (pInst->OutRate - pInst->Remainder));
                    pOutpcmBuf[Channels * out + ch] = CLIPTOSHORT(((HI_U32)sum) << 2);/* IvtSFFactor is 2.30 format */
#else
