...
HI_S32 DRV_CIPHER_CalcRsa_SW(CIPHER_RSA_DATA_S *pCipherRsaData)
{
    ...
    ret = mbedtls_mpi_read_binary( &rsa.N, pCipherRsaData->pu8N, pCipherRsaData->u16NLen);
    if( 0 != ret )
    {
        return HI_FAILURE;
    }
    ...
    {
        ret = mbedtls_mpi_read_binary( &rsa.E, pCipherRsaData->pu8K, pCipherRsaData->u16KLen);
        if( 0 != ret )
        {
            ret = HI_FAILURE;
            goto cleanup;
        ...
        ret = mbedtls_rsa_public(&rsa, pCipherRsaData->pu8Input, pCipherRsaData->pu8Output);
        if( 0 != ret )
        {
            ret = HI_FAILURE;
            goto cleanup;
