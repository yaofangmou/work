...
HI_VOID DRV_PNG_OutputMessage(const HI_CHAR* pFunc, const HI_U32 Line, const HI_GFX_MSG_TYPE_E eMsgType, const HI_U32 Format, ...)
{
    va_list args;
    HI_U32 MsgLen = 0;
    ...
    if (NULL == pFunc)
    {
        HI_GFX_Print("[module-gfx-png][err] : input func pointer %p\n",pFunc);
        return;
    }

    if ((HI_GFX_MSG_BEG == eMsgType) || (HI_GFX_MSG_END == eMsgType))
    {
        FormatMessage("png", pFunc, Line, eMsgType, TmpFormat, &LogErr, &LogFunc, &LogValue);
        return;
    }
...
HI_VOID DRV_PNG_SetLogFunc(HI_CHAR* pFunc)
{
    DRV_GFX_SetLogFunc("png", pFunc);
    return;
}

HI_VOID DRV_PNG_SetLogLevel(HI_U32 LogLevel)
{
    DRV_GFX_SetLogLevel(LogLevel);
    return;
}

HI_VOID DRV_PNG_SetLogSave(HI_U32 LogSave)
{
...
#ifdef CONFIG_GFX_PROC_SUPPORT
HI_VOID DRV_PNG_ProcMsg(struct seq_file *p)
{
    DRV_GFX_ProcMsg(p);
}
#endif
