...
#define FASTBOOT_DTB_NAME "dtbo_a"
#define FASTBOOT_RECOVERY_OFFSET 0
#define FASTBOOT_DTB_OFFSET 0
...
int load_dtb_image(void)
{
	int ret;
	HI_HANDLE flashhandle;
	HI_Flash_InterInfo_S pFlashInfo;

	flashhandle = HI_Flash_OpenByName(FASTBOOT_DTB_NAME);
	if ((0 == flashhandle) || (HI_INVALID_HANDLE == flashhandle)) {
		printf("HI_Flash_Open partion %s error\n",
		       FASTBOOT_DTB_NAME);
		return HI_FAILURE;
	...
	ret =
	    HI_Flash_Read(flashhandle, FASTBOOT_DTB_OFFSET, (HI_U8*)CONFIG_DTB_LOAD_ADDR,
	...
	printf("Automatic bootm dtb at addr 0x%x ...\n", CONFIG_DTB_LOAD_ADDR);
	return 0;
}
