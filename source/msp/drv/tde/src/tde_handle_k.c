...
HI_BOOL initial_handle(HI_VOID)
{
    ...
    if (NULL == s_pstTdeHandleList)
    {
        ...
        if (NULL == s_pstTdeHandleList)
        {
            HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_GFX_Malloc, FAILURE_TAG);
            return HI_FALSE;
        }
...
#endif
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(res);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(handle);
...
HI_BOOL query_handle(HI_S32 handle, HI_HANDLE_MGR **res)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(res, HI_FALSE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(s_pstTdeHandleList, HI_FALSE);
    ...
    {
        TDE_UNLOCK(&s_pstTdeHandleList->lock, lockflags);
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, list_entry, FAILURE_TAG);
        return HI_FALSE;
    }
...
HI_BOOL release_handle(HI_S32 handle)
{
    ...
    {
        TDE_UNLOCK(&s_pstTdeHandleList->lock, lockflags);
        HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, list_entry, FAILURE_TAG);
        return HI_FALSE;
    }
    ...
    HI_GFX_OutMiniErrFuncMsg(HI_GFX_MSG_ERR_FUNC, FAILURE_TAG, FAILURE_TAG);
    return HI_FALSE;
}
