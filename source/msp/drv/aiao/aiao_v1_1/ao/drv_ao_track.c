...
static HI_VOID HDMISetAudioMute(SND_CARD_STATE_S* pCard)
{
    ...
    if (HI_TRUE == pCard->bHdmiMute)
    {
        return;
    }

    s32Ret = SND_SetOpMute(pCard, HI_UNF_SND_OUTPUTPORT_HDMI0, HI_TRUE);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("call SND_SetOpMute HI_TRUE failed(0x%x)\n", s32Ret);
        return;
    }
...
static HI_VOID HDMISetAudioUnMute(SND_CARD_STATE_S* pCard)
{
    ...
    if (HI_TRUE == pCard->bUserSetHdmiMute)
    {
        return;
    }
    ...
    s32Ret = SND_SetOpMute(pCard, HI_UNF_SND_OUTPUTPORT_HDMI0, HI_FALSE);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_AO("call SND_SetOpMute HI_FALSE failed(0x%x)\n", s32Ret);
        return;
    }
...
static HI_S32 TRACKPauseAip(SND_CARD_STATE_S* pCard, SND_TRACK_STATE_S* pTrack)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (HI_UNF_SND_TRACK_TYPE_MASTER == pTrack->stUserTrackAttr.enTrackType)
    {
        HI_U32 u32AIPPauseMask = 0;
        AOE_AIP_STATUS_E enStatus;
        HAL_AOE_AIP_GetStatus(pTrack->enAIP[SND_AIP_TYPE_PCM], &enStatus);

        if (enStatus == AOE_AIP_STATUS_START)
        {
            u32AIPPauseMask |= (1 << (HI_U32)pTrack->enAIP[SND_AIP_TYPE_PCM]);
        }
        ...
        if (SND_MODE_NONE != pCard->enSpdifPassthrough)
        {
            ...
            if (enStatus == AOE_AIP_STATUS_START)
            {
                u32AIPPauseMask |= (1 << (HI_U32)pTrack->enAIP[SND_AIP_TYPE_SPDIF_RAW]);
            }
        }
        ...
        if (SND_MODE_NONE != pCard->enHdmiPassthrough)
        {
            ...
            if (enStatus == AOE_AIP_STATUS_START)
            {
                u32AIPPauseMask |= (1 << (HI_U32)pTrack->enAIP[SND_AIP_TYPE_HDMI_RAW]);
            }
        }

        //the interface is only used for master track to pause
        if (u32AIPPauseMask != 0)
        {
            s32Ret = HAL_AOE_AIP_Group_Pause(u32AIPPauseMask);
            if (s32Ret != HI_SUCCESS)
            {
                HI_WARN_AO("Master Track AIP Pause Group Error\n");
            }
        }
    }
    else
    {
        s32Ret = HAL_AOE_AIP_Pause(pTrack->enAIP[SND_AIP_TYPE_PCM]);
        if (s32Ret != HI_SUCCESS)
        {
            HI_WARN_AO("Slave track AIP Pause PCM Error\n");
        }
    }
...
static HI_VOID TRACKSavePcmData(SND_TRACK_STATE_S* pTrack, SND_TRACK_STREAM_ATTR_S* pstStreamAttr)
{
    ...
    if (HI_FALSE == pTrack->bAlsaTrack && HI_NULL == pTrack->pTmpBuf)
    {
    ...
    {
        /* save alsa pcm data */
        u32WriteSize = CIRC_BUF_Write(&pTrack->stSaveAlsa.stCB, pstStreamAttr->pPcmDataBuf, u32DataSize, HI_TRUE);
        if (u32WriteSize != u32DataSize)
        {
            HI_ERR_AO("CIRC_BUF_Write Alsa track error, u32WriteSize(0x%x) != u32DataSize(0x%x)", u32WriteSize, u32DataSize);
        }
...
HI_S32 TRACK_Pause(SND_CARD_STATE_S* pCard, HI_U32 u32TrackID)
{
    ...
    Ret = TRACKPauseAip(pCard, pTrack);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_AO("TRACKPauseAip failed!\n");
        return Ret;
    }
