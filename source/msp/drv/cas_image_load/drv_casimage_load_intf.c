...
static long CASIMAGE_Ioctl(struct file* ffile, unsigned int cmd, unsigned long arg)
{
    return (long)HI_DRV_UserCopy(ffile->f_path.dentry->d_inode, ffile, cmd, arg, DRV_CASIMAGE_Ioctl);
}
...
static struct file_operations casiamge_load_fops =
{
    .owner          = THIS_MODULE,
    .open           = HI_NULL,
    .unlocked_ioctl = CASIMAGE_Ioctl,
#ifdef CONFIG_COMPAT
    .compat_ioctl   = CASIMAGE_Ioctl,
#endif
    .release        = HI_NULL,
};

HI_S32 CASIMAGE_LOAD_PM_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    HI_S32 ret = HI_FAILURE;

    ret = DRV_CASIMAGE_Suspend();
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PRINT(HI_ID_CASIMAGE, "DRV_CASIMAGE_Suspend failed, ret:%x\n", ret);
    }

    HI_PRINT("casimage suspend OK\n");
    return HI_SUCCESS;
}

HI_S32 CASIMAGE_LOAD_PM_Resume(PM_BASEDEV_S *pdev)
{
    HI_S32 ret = HI_FAILURE;
