...
HI_S32 PWM_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    memset(&stAttrCmd, 0, sizeof(PWM_ATTR_CMD_PARA_S));
    memset(&stSigCmd, 0, sizeof(PWM_SIGNAL_CMD_PARA_S));
    ...
    switch (cmd)
    {
        ...
        {
            if(copy_from_user(&stSigCmd, (PWM_SIGNAL_CMD_PARA_S __user *)arg, sizeof(PWM_SIGNAL_CMD_PARA_S)))
            {
                HI_ERR_PWM("copy data from user fail\n");
                return HI_FAILURE;
            }
