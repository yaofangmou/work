...
static HI_VOID HalHdmiAudioNCTSValueGet(HDMI_AUDIO_CONFIG_S* pstAudioCfg, HI_U32 *pNValue, HI_U32 *pCTSValue)
{
    ...
    *pCTSValue = (0 != u23CTSValue)? u23CTSValue: u23Temp;
    ...
    HDMI_INFO("u32TmdsClk: %d, u32PixelRepeat:%d\n", pstAudioCfg->u32TmdsClk, pstAudioCfg->u32PixelRepeat);
    HDMI_INFO("Result, N: %d, CTS:%d\n", *pNValue, *pCTSValue);

    return;
}

HI_VOID Hal_Hdmi_AudioCfg(HI_U32 u32TmdsClk, HI_U32 u32PixelRepeat)
{
    HI_U32              u32RefN = 0;
    HI_U32              u32RefCts = 0;
    HI_U32              u32Tmp = 0;
    HDMI_AUDIO_CONFIG_S stAudioCfg = {0};

    stAudioCfg.u32TmdsClk     = u32TmdsClk;
    stAudioCfg.u32PixelRepeat = u32PixelRepeat;
    stAudioCfg.enSampleFs     = HDMI_SAMPLE_RATE_48K;
    HalHdmiAudioNCTSValueGet(&stAudioCfg,&u32RefN,&u32RefCts);

    //sel I2S
    HDMI_RegWriteMap(0xf8a208b0, 0x00050002);

    //rst   0x3 -> 0x0
    HDMI_RegWriteMap( 0xf8ce28b0 , 0x3);

    //bit[3:4]   0x2-48k
    HDMI_RegWriteMap(  0xf8ce298c , 0x00000082);

    //LAYOUT bit[0] 0b0:2ch 0b1:8ch   mute: bit[7] 0b0  0x80
    HDMI_RegWriteMap(  0xf8ce28bc , 0x00000000);

    //AUD_MODE reg_i2s_en SD0 en
    HDMI_RegWriteMap(  0xf8ce2850 , 0x00000010);

    // reg_aud_in_en bit[0] en-0b1
    HDMI_RegWriteMap( 0xf8ce284c , 0x1);

    //ACR enalbe bit[1]
    HDMI_RegWriteMap( 0xf8ce2804 , 0xe);

    //ch status 48k 16bit 2ch
    HDMI_RegWriteMap( 0xf8ce2878 , 0x00000000 );
    HDMI_RegWriteMap( 0xf8ce287c , 0x00000000 );
    HDMI_RegWriteMap( 0xf8ce2880 , 0x00000000 );
    HDMI_RegWriteMap( 0xf8ce2884 , 0x00000002);
    HDMI_RegWriteMap( 0xf8ce2888 , 0x00000002 );
    HDMI_RegWriteMap( 0xf8ce288c , 0x00000010 );
    HDMI_RegWriteMap( 0xf8ce2890 , 0x0000000b );
    HDMI_RegWriteMap( 0xf8ce2894 , 0x00000000);

    //N bit[7:0]  //N bit[15:8]  //N bit[19:16] 6144 (0x1800)
    u32Tmp = (u32RefN >> 0) & 0xff;
    HDMI_RegWriteMap(  0xf8ce280c , u32Tmp);
    u32Tmp = (u32RefN >> 8) & 0xff;
    HDMI_RegWriteMap(  0xf8ce2810 , u32Tmp);
    u32Tmp = (u32RefN >> 16) & 0xff;
    HDMI_RegWriteMap(  0xf8ce2814 , u32Tmp);

    //SW CTS sel bit[0]
    HDMI_RegWriteMap( 0xf8ce2804 , 0xf);

    //SW CTS 74250(0x01220a)
    u32Tmp = (u32RefCts>> 0) & 0xff;
    HDMI_RegWriteMap( 0xf8ce2818 , u32Tmp);
    u32Tmp = (u32RefCts>> 8) & 0xff;
    HDMI_RegWriteMap( 0xf8ce281c , u32Tmp);
    u32Tmp = (u32RefCts>> 16) & 0xff;
    HDMI_RegWriteMap( 0xf8ce2820 , u32Tmp);

    //de-rst
    HDMI_RegWriteMap( 0xf8ce28b0 , 0x0);
...
HI_VOID HAL_HDMI_HardwareStatusGet(HDMI_HAL_S  *pstHdmiHal, HDMI_HARDWARE_STATUS_S* pstHwStatus)
{
    ...
    pstAudioStatus->u32RegCtsvalue = phdmiStatus->audioFmt.hw_cts;
    pstAudioStatus->u32RegCtsSw  = phdmiStatus->audioFmt.sw_cts;
    pstAudioStatus->bSwCts       = phdmiStatus->audioFmt.bswcts;
...
HI_VOID HAL_HDMI_Debug(HDMI_HAL_S *pstHdmiHal, HDMI_HAL_DEBUG_CMD_E enDebugCmd,HI_VOID *pData)
{
    ...
    switch(enDebugCmd)
    {
        ...
        case HDMI_DEBUG_CMD_SW_MODE:
        {
            ...
            u32RegVal = HDMI_RegReadMap(0xf8ce2804);
            if ( *(HI_BOOL *)pData )
            {
                u32RegVal |= BIT_MSK__ACR_CTRL__REG_CTS_HW_SW_SEL;
            }
            else
            {
                u32RegVal &= ~BIT_MSK__ACR_CTRL__REG_CTS_HW_SW_SEL;
            }
            HDMI_RegWriteMap(0xf8ce2804, u32RegVal);
            break;
        }
        case HDMI_DEBUG_CMD_SWCTS:
        {
            ...
            // Set CTS value
            u8CtsValue = *(HI_U32 *)pData & 0xff;
            HDMI_RegWriteMap(0xf8ce2818, u8CtsValue);

            u8CtsValue = (*(HI_U32 *)pData >> 8) & 0xff;
            HDMI_RegWriteMap(0xf8ce281c, u8CtsValue);

            u8CtsValue = (*(HI_U32 *)pData >> 16) & 0x0f;
            HDMI_RegWriteMap(0xf8ce2820, u8CtsValue);
            break;
        }
        case HDMI_DEBUG_CMD_N:
        {
            ...
            // Set N value
            u8NValue = *(HI_U32 *)pData & 0xff;
            HDMI_RegWriteMap(0xf8ce280c, u8NValue);

            u8NValue = (*(HI_U32 *)pData >> 8) & 0xff;
            HDMI_RegWriteMap(0xf8ce2810, u8NValue);

            u8NValue = (*(HI_U32 *)pData >> 16) & 0x0f;
            HDMI_RegWriteMap(0xf8ce2814, u8NValue);
...
HI_S32 HAL_HDMI_AudioPathSet(HDMI_HAL_S *pstHdmiHal, HDMI_AUDIO_CONFIG_S* pstAudioCfg)
{
    ...
    HalHdmiAudioNCTSValueGet(pstAudioCfg, &(AudioFormat.n), &(AudioFormat.sw_cts));
    DRV_HDMI_CompatAudNCtsGet(pstHdmiHal->stHalCtx.u32HdmiID, &(AudioFormat.n), &(AudioFormat.sw_cts));
...
#ifdef HDMI_BUILD_IN_BOOT
    {
        HI_U32 u32PixelRepeat = 1;

        if (  (pstVideoCfg->enTimming == HDMI_VIDEO_TIMING_1440X480I_60000)
            ||(pstVideoCfg->enTimming == HDMI_VIDEO_TIMING_1440X576I_50000))
        {
            u32PixelRepeat = 2;
        }
        Hal_Hdmi_AudioCfg(pstVideoCfg->u32TmdsClk, u32PixelRepeat);
        HAL_HDMI_InfoframeEnableSet(pstHdmiHal, HDMI_INFOFRAME_TYPE_AUDIO, HI_TRUE);
    }
#endif
...
HI_VOID HAL_HDMI_CecMsgClear(struct hiHDMI_HAL_S *pstHdmiHal)
{
    return ;
}
...
HI_S32 HAL_HDMI_Open(HDMI_HAL_INIT_S *pstHalInit, HDMI_HAL_S** pstHalHandle)
{
    ...
    pstHdmiHal->HAL_HDMI_CecMsgClear             = HAL_HDMI_CecMsgClear;
#endif
#ifdef HDMI_SCDC_SUPPORT
    pstHdmiHal->HAL_HDMI_ScdcStatusSet           = HAL_HDMI_ScdcStatusSet;
