...
#define TECHNICOLOR_BRIGNTNESS_MAX  5000
...
{
    DISP_HAL_ENCFMT_PARAM_S stFmt = {0};
    ...
    if (eFmt < HI_DRV_DISP_FMT_CUSTOM)
    {
        ...
        if (pstDisp->enDisp == HI_DRV_DISPLAY_0)
        {
            ...
            DISP_S *pstSDDisp = HI_NULL;
            DispGetPointerByID(HI_DRV_DISPLAY_1, pstHDDisp);
            DispGetPointerByID(HI_DRV_DISPLAY_0, pstSDDisp);

            if ((0 != pstSDDisp->stDispInfo.stVirtaulScreen.s32Width)
                && (0 != pstSDDisp->stDispInfo.stVirtaulScreen.s32Height))
            {
                pstInfo->stOffsetInfo.u32Left = (pstHDDisp->stSetting.stOffsetInfo.u32Left + pstSDDisp->stSetting.stOffsetInfo.u32Left)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Width) / (pstSDDisp->stDispInfo.stVirtaulScreen.s32Width);
                pstInfo->stOffsetInfo.u32Right = (pstHDDisp->stSetting.stOffsetInfo.u32Right + pstSDDisp->stSetting.stOffsetInfo.u32Right)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Width) / (pstSDDisp->stDispInfo.stVirtaulScreen.s32Width);
                pstInfo->stOffsetInfo.u32Top = (pstHDDisp->stSetting.stOffsetInfo.u32Top + pstSDDisp->stSetting.stOffsetInfo.u32Top)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Height) / (pstSDDisp->stDispInfo.stVirtaulScreen.s32Height);
                pstInfo->stOffsetInfo.u32Bottom = (pstHDDisp->stSetting.stOffsetInfo.u32Bottom + pstSDDisp->stSetting.stOffsetInfo.u32Bottom)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Height) / (pstSDDisp->stDispInfo.stVirtaulScreen.s32Height);
            }
            else
            {
                DISP_WARN("pstSDDisp->stDispInfo.stVirtaulScreen.s32Width is %d, pstSDDisp->stDispInfo.stVirtaulScreen.s32Height is %d!\n",
                    pstSDDisp->stDispInfo.stVirtaulScreen.s32Width,
                    pstSDDisp->stDispInfo.stVirtaulScreen.s32Height);
                pstInfo->stOffsetInfo.u32Left = (pstHDDisp->stSetting.stOffsetInfo.u32Left + pstSDDisp->stSetting.stOffsetInfo.u32Left)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Width) / (DISPLAY_DEFAULT_VIRT_SCREEN_WIDTH);
                pstInfo->stOffsetInfo.u32Right = (pstHDDisp->stSetting.stOffsetInfo.u32Right + pstSDDisp->stSetting.stOffsetInfo.u32Right)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Width) / (DISPLAY_DEFAULT_VIRT_SCREEN_WIDTH);
                pstInfo->stOffsetInfo.u32Top = (pstHDDisp->stSetting.stOffsetInfo.u32Top + pstSDDisp->stSetting.stOffsetInfo.u32Top)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Height) / (DISPLAY_DEFAULT_VIRT_SCREEN_HEIGHT);
                pstInfo->stOffsetInfo.u32Bottom = (pstHDDisp->stSetting.stOffsetInfo.u32Bottom + pstSDDisp->stSetting.stOffsetInfo.u32Bottom)
                                                 * (pstSDDisp->stDispInfo.stFmtResolution.s32Height) / (DISPLAY_DEFAULT_VIRT_SCREEN_HEIGHT);
            }
        }
...
HI_S32 DispSetEnable(DISP_S* pstDisp, HI_BOOL bEnable)
{
    ...
    nRet = DISP_GetFormat(HI_DRV_DISPLAY_1, &enStereo1, &enEncFmt1);
    if (nRet != HI_SUCCESS)
    {
        return nRet;
    }
...
HI_S32 DISP_UpdatePqInfor(HI_DRV_DISPLAY_E enDisp)
{
    ...
    DRV_PQ_UpdateVdpPQ((HI_PQ_DISPLAY_E)enDisp, &stTimingInfo, (HI_VOID *)g_pstVdpBaseAddr);
#endif
    return HI_SUCCESS;
...
HI_DRV_PDM_DISP_PARAM_S g_stPdmDispParam;
...
HI_S32 DISP_Init(HI_VOID)
{
    ...
    DISP_MEMSET(&g_stPdmDispParam, 0x0, sizeof(g_stPdmDispParam));
    ...
    pVdpMonitorReg = ioremap_nocache(VDP_MONITOR_CTRL_ADDR, sizeof(S_VDP_MONITOR_REGS_TYPE));
#else
    g_pu32DispUboot2KernelFlag = (volatile HI_U32 *)DISP_USED_COMMON_REGISTER;
    pVdpComReg = (S_VDP_COM_REGS_TYPE *)VDP_ANALOG_CTRL_ADDR;
    pVdpMonitorReg = (S_VDP_MONITOR_REGS_TYPE *)VDP_MONITOR_CTRL_ADDR;
    DRV_InitComReg();
#endif
    ...
    if (HI_SUCCESS == DISP_HAL_GetOperation(&pstFunction))
    {
        memcpy(&s_stDisplayDevice.stIntfOpt, pstFunction, sizeof(DISP_INTF_OPERATION_S));
    }
    ...
    GetPdmDispParam(&g_stPdmDispParam);

    // s2 inited display
    for (eDisp = HI_DRV_DISPLAY_0; eDisp < HI_DRV_DISPLAY_2; eDisp++)
    {
        DispInitDisplay(eDisp, &g_stPdmDispParam);
    }

    DispGetPointerByIDNoReturn(HI_DRV_DISPLAY_1, pstDisp);
#ifdef HI_DISP_BUILD_FULL
    nRet = DISP_ISR_Init();
#endif
        ...
        if (Disp_GetFastbootupFlag() ==  DISP_FASTBOOTUP_FLAG)
        {
            ...
            for (enD = HI_DRV_DISPLAY_0; enD < HI_DRV_DISPLAY_2; enD++)
            {
                ...
                *pstDisp_bak = *pstDisp;
                ...
                GetPdmDispParam(&g_stPdmDispParam);
                DispInitDisplay(enD, &g_stPdmDispParam);
...
HI_S32 DISP_SetScreenOffset(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_OFFSET_S *pstScreenOffset)
{
    ...
    if (HI_DRV_DISPLAY_0 == pstDisp->enDisp)
    {
        HI_DRV_DISP_OFFSET_S stHDScreenOffset = {0};

        nRet = DISP_GetScreenOffset(HI_DRV_DISPLAY_1, &stHDScreenOffset);
        if (nRet != HI_SUCCESS)
        {
            return nRet;
        }

        if (  (pstScreenOffset->u32Bottom +  stHDScreenOffset.u32Bottom > HI_DRV_DISP_OFFSET_MAX)
            ||(pstScreenOffset->u32Left   +  stHDScreenOffset.u32Left   > HI_DRV_DISP_OFFSET_MAX)
            ||(pstScreenOffset->u32Right  +  stHDScreenOffset.u32Right  > HI_DRV_DISP_OFFSET_MAX)
            ||(pstScreenOffset->u32Top    +  stHDScreenOffset.u32Top    > HI_DRV_DISP_OFFSET_MAX))
        {
            DISP_ERROR("screen offset must less then %d! SD: L:%d, R:%d, T:%d, B:%d; HD: L:%d, R:%d, T:%d, B:%d.\n",
                    HI_DRV_DISP_OFFSET_MAX, pstScreenOffset->u32Left, pstScreenOffset->u32Right,
                    pstScreenOffset->u32Top, pstScreenOffset->u32Bottom, stHDScreenOffset.u32Left,
                    stHDScreenOffset.u32Right, stHDScreenOffset.u32Top, stHDScreenOffset.u32Bottom);
            return HI_ERR_DISP_INVALID_PARA;
        }
    }
    ...
    {
        DISP_ERROR("screen offset must less then %d! L:%d, R:%d, T:%d, B:%d.\n", HI_DRV_DISP_OFFSET_MAX,
    ...
    {
        DISP_S* pstDisp0 = NULL;
        DispGetPointerByID(HI_DRV_DISPLAY_0, pstDisp0);
        pstDisp0->bDispSettingChange = HI_TRUE;
    }
#if 0
     if (pstDisp->bIsMaster)
    {
...
HI_S32 DISP_GetProcInto(HI_DRV_DISPLAY_E enDisp, DISP_PROC_INFO_S* pstInfo)
{
    ...
    pstInfo->stVirtaulScreen = pstDisp->stSetting.stVirtaulScreen;
    pstInfo->stOffsetInfo    = pstDisp->stSetting.stOffsetInfo;
    pstInfo->stActualOffsetInfo = pstDisp->stDispInfo.stOffsetInfo;
...
HI_S32 DISP_CheckRegConfig(HI_CHAR *pcheckItem)
{
    ...
    if (HI_SUCCESS == DISP_HAL_GetOperation(&pstFunction))
    {
        pstFunction->PF_CheckRegConfig(pcheckItem);
    }
...
HI_S32 DISP_GetTechniDispBright(HI_DRV_DISPLAY_E enDisp, HI_S32 *ps32TechniDispBright)
{
    DISP_S* pstDisp;

    DispCheckDeviceState();
    DispCheckID(enDisp);
    DispCheckNullPointer(ps32TechniDispBright);
    DispGetPointerByID(enDisp, pstDisp);

    *ps32TechniDispBright = pstDisp->stSetting.s32TechniDispBright;

    return HI_SUCCESS;

}

HI_S32 DISP_SetTechniDispBright(HI_DRV_DISPLAY_E enDisp, HI_S32 s32TechniDispBright)
{
    DISP_S* pstDisp;
    ...
    if (s32TechniDispBright < 0 || s32TechniDispBright > TECHNICOLOR_BRIGNTNESS_MAX)
    {
        DISP_ERROR("Invalid para!s32TechniDispBright->[%d]\n",s32TechniDispBright);
        return HI_ERR_DISP_INVALID_PARA;
    }

    pstDisp->stSetting.s32TechniDispBright = s32TechniDispBright;
...
static HI_DRV_COLOR_SPACE_E genAutoOutColorSpaceStrategy[HI_DRV_VIDEO_FRAME_TYPE_BUTT][HI_DRV_DISP_COLOR_SPACE_BUTT][2][HI_DRV_DISP_TYPE_BUTT] =
{
    ...
    /* HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR-----Not Support Currently!! */
    {
        /* HI_DRV_DISP_COLOR_SPACE_AUTO */
        {
            /* sink not support bt2020 */
...
HI_S32 DISP_SetOutputColorSpace(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_COLOR_SPACE_E enColorSpace)
{
        ...
        && (HI_DRV_DISP_FMT_1440x480i_60 >= pstDisp->stDispInfo.eFmt))
    {
        ...
        {
            DISP_ERROR("Unsupport to set output color space in SD format.[%d][%d]\n",enColorSpace,pstDisp->stDispInfo.eFmt);
            return HI_ERR_DISP_NOT_SUPPORT;
        }
    }
