...
static HI_S32 HAL_RSA_Occupy(void)
{
    HI_U32 val = 0;
    void * reg = HI_NULL;
    HI_U32 i;

    reg = cipher_ioremap_nocache(SMCU_CRYPTO_ACCESS_CTRL, 0x100);
    if (HI_NULL == reg)
    {
        HI_ERR_CIPHER("occupy rsa ioremap failed\n");
    ...
    {
        HI_ERR_CIPHER("occupy rsa failed\n");
        return HI_ERR_CIPHER_BUSY;
    }
...
static void  HAL_RSA_Revert(void)
{
    HI_U32 val = 0;
    void * reg = HI_NULL;
...
//    HI_PRINT_HEX("K", pCipherRsaData->pu8K, pCipherRsaData->u16NLen);
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_CIPHER("HAL_RSA_Occupy failed\n");
        return ret;
    }
