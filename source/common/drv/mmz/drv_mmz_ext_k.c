...
HI_S32 HI_DRV_SMMU_GetSysPoolSize(unsigned long *total_size, unsigned long *pool_limit)
{

	if (!total_size || !pool_limit)
		return HI_FAILURE;
	*total_size = get_smmu_pool_size(pool_limit);
...
EXPORT_SYMBOL(HI_DRV_SMMU_GetSysPoolSize);
