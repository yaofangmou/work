...
#define HEVC_MAX_TEMPLAYER                   (7)             //max number of temporal layer
...
#ifdef MV_HEVC_SUPPORT
    UINT32 layer_set_layer_id_list[HEVC_MAX_VPS_OUTPUTLAYER_SETS][HEVC_MAX_NUM_LAYERS];
    UINT32 num_layers_in_id_list[HEVC_MAX_VPS_OUTPUTLAYER_SETS];
...
typedef struct
{
    ...
    UINT8  tone_mapping_output_fine_tuning_x[10];
    UINT8  tone_mapping_output_fine_tuning_y[10];
    UINT8  saturation_gain_num_val;                 // shall be in the range of 0 to 6
    UINT8  saturation_gain_x[6];
    UINT8  saturation_gain_y[6];
...
typedef struct
{
    ...
    /*distribution            -> dist     */
    /**************************************/
    UINT8   u8ItuCountryCode;
    UINT16  u16ItuTerminalProviderCode;
    UINT16  u16ItuTerminalProviderOrientedCode;
    UINT8   u8ApplicationIdentifier;
    UINT8   u8ApplicationVersion;
    UINT8   u8NumWindows;

    UINT16  u16WindowUpperLeftCornerX[MAX_NUM_WINDOWS];
    UINT16  u16WindowUpperLeftCornerY[MAX_NUM_WINDOWS];
    UINT16  u16WindowLowerRightCornerX[MAX_NUM_WINDOWS];
    UINT16  u16WindowLowerRightCornerY[MAX_NUM_WINDOWS];
    UINT16  u16CenterOfEllipseX[MAX_NUM_WINDOWS];
    UINT16  u16CenterOfEllipseY[MAX_NUM_WINDOWS];
    UINT8   u8RotationAngle[MAX_NUM_WINDOWS];
    UINT16  u16SemimajorAxisInternalEllipse[MAX_NUM_WINDOWS];
    UINT16  u16SemimajorAxisExternalEllipse[MAX_NUM_WINDOWS];
    UINT16  u16SemiminorAxisExternalEllipse[MAX_NUM_WINDOWS];
    UINT8   u8OverlapProcessOption[MAX_NUM_WINDOWS];

    UINT32  u32_T_DisplayMaximumLuminance;

    VBOOL b_T_Display_APL_Flag;
    UINT8   u8NumRows_T_Display_APL;
    UINT8   u8NumCols_T_Display_APL;
    UINT8   u8_T_display_APL[MAX_NUM_ROWS_T_DISPLAY_APL][MAX_NUM_COLS_T_DISPLAY_APL];

    UINT32  u32MaxScl[MAX_NUM_WINDOWS][3];
    UINT32  u32AverageMaxrgb[MAX_NUM_WINDOWS];
    UINT8   u8NumDistMaxrgbPercentiles[MAX_NUM_WINDOWS];
    UINT8   u8DistMaxrgbPercentages[MAX_NUM_WINDOWS][MAX_NUM_DIST_MAXRGB_PERCENTILES];
    UINT32  u32DistMaxrgbPercentiles[MAX_NUM_WINDOWS][MAX_NUM_DIST_MAXRGB_PERCENTILES];
    UINT16  u16FractionBrightPixels[MAX_NUM_WINDOWS];

    VBOOL b_M_Display_APL_Flag;
    UINT8   u8NumRows_M_Display_APL;
    UINT8   u8NumCols_M_Display_APL;
    UINT8   u8_M_Display_APL[MAX_NUM_ROWS_M_DISPLAY_APL][MAX_NUM_COLS_M_DISPLAY_APL];

    VBOOL bToneMappingFlag[MAX_NUM_WINDOWS];
    UINT16  u16KneePointX[MAX_NUM_WINDOWS];
    UINT16  u16KneePointY[MAX_NUM_WINDOWS];
    UINT8   u8NumBezierCurveAnchors[MAX_NUM_WINDOWS];
    UINT16  u16bezierCurveAnchors[MAX_NUM_WINDOWS][MAX_NUM_BEZIER_CURVE_ANCHORS];

    VBOOL bColorSaturationMappingFlag[MAX_NUM_WINDOWS];
    UINT8   u8ColorSaturationWeight[MAX_NUM_WINDOWS];
    VBOOL   bAvalible;
} HEVC_SEI_HDR10PLUS_S;
...
typedef struct
{
    ...
    HEVC_SEI_HDR10PLUS_S hdr10plus;
...
typedef struct
{
    ...
    UADDR  bytespos;           // stream VDH will used start bytes ph addr, 4words/16bytes addr aligned     unit: byte
...
VOID   HEVCDEC_FlushRemainImg(HEVC_CTX_S *pHevcCtx);
