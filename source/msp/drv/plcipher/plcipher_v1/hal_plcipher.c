...
/*
read the register before write to avoid the plcipher logic bug that regiter
write error probabily
*/
HI_VOID HAL_PLCIPHER_WriteReg(HI_U32* addr, HI_U32 u32Val)
{
    HI_SIZE_T LockFlag;
    HI_U32 u32TmpVal;
    spin_lock_irqsave(&g_LockReg, LockFlag);
    HI_REG_READ32(addr, u32TmpVal);
    udelay(1);
    HI_REG_WRITE32(addr, u32Val);
    spin_unlock_irqrestore(&g_LockReg, LockFlag);
    return;
}
