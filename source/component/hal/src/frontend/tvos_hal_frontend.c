...
{
    S32 s32Ret = SUCCESS;
    FRONTEND_TSOUT_PLP_PARAM_S* pstPlpParam = NULL;
    FRONTEND_TSOUT_PLP_INFO_S* pstPlpInfo   = NULL;

    HI_UNF_TUNER_TER_PLP_PARA_S stUnfPlpParams;
    HI_UNF_TUNER_TER_PLP_INFO_S stUnfPlpInfo;

    memset(&stUnfPlpParams, 0, sizeof(stUnfPlpParams));
    memset(&stUnfPlpInfo, 0, sizeof(stUnfPlpInfo));
    ...
    switch (enTsoutCmd)
    {
        ...
        case FRONTEND_TSOUT_TER2_GET_PLPINFO:
            pstPlpInfo              = (FRONTEND_TSOUT_PLP_INFO_S*)pParam;
            s32Ret = HI_UNF_TUNER_GetPLPInfo(pstDevInfo->u32TunerId, pstPlpInfo->u8PlpIndex, &stUnfPlpInfo);

            if (SUCCESS == s32Ret)
            {
                pstPlpInfo->u8PlpId    = stUnfPlpInfo.u8PlpId;
                pstPlpInfo->u8PlpGrpId = stUnfPlpInfo.u8PlpGrpId;
                pstPlpInfo->enPlpType  = stUnfPlpInfo.enPlpType;
            }

            break;

        case FRONTEND_TSOUT_TER2_SET_PLPPARAM:
            pstPlpParam                  = (FRONTEND_TSOUT_PLP_PARAM_S*)pParam;
            stUnfPlpParams.u8PlpId       = pstPlpParam->u8PlpId;
            stUnfPlpParams.u8CommPlpId   = pstPlpParam->u8CommPlpId;
            stUnfPlpParams.u8Combination = pstPlpParam->u8Combination;

            s32Ret = HI_UNF_TUNER_SetPLPPara(pstDevInfo->u32TunerId, &stUnfPlpParams);

            pstDevInfo->enOldStatus = FRONTEND_STATUS_SCANNING;

            if (SUCCESS == s32Ret)
            {
                pstDevInfo->stT2PlpParam.u8T2PlpID                = pstPlpParam->u8PlpId;
                pstDevInfo->stT2PlpParam.u8T2CommonPlpID          = pstPlpParam->u8CommPlpId;
                pstDevInfo->stT2PlpParam.u8T2CommonPlpCombination = pstPlpParam->u8Combination;
            }
...
static S32 frontend_config_tsout(struct _FRONTEND_DEVICE_S* pstDev, const HANDLE hFrontend, const FRONTEND_TSOUT_CMD_TYPE_E enTsoutCmd, void* pParam)
{
    ...
    if (enTsoutCmd <= FRONTEND_TSOUT_TER2_SET_PLPPARAM)
    {
