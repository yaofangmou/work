...
static void CC708DataThread(void *arg)
{
    ...
    memset(&cc_element, 0, sizeof(DTVCC_Element_S));
...
static void CC608Thread(void *arg)
{
    ...
    memset(&cc608_element, 0, sizeof(CC608_ELEMENT_S));
...
{
    CC_LOCAL_PARAM_S *pstParam = (CC_LOCAL_PARAM_S *)arg;
    while(s_bCCEnableFlag)
    {
        if(s_bCCStartFlag)
        {
            if (pstParam->enCCType == HI_UNF_CC_DATA_TYPE_ARIB)
            {
                (void)AribCC_CharFlash();
            }
            else
            {
                CC708_CharFlash();
...
static void StartCCDataThread(CC_LOCAL_PARAM_S *pstParam)
{
    ...
    {
        if(pthread_create(&u32CharFlashThread, HI_NULL, (void* (*)(void *))CharFlashThread, (void*)pstParam)!=0)
        {
