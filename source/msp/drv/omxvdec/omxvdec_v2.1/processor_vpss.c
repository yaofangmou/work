...
HI_S32 processor_convert_frame_to_image(HI_DRV_VIDEO_FRAME_S *pFrame, IMAGE *pImage)
{
    ...
    OMXVDEC_ASSERT_RETURN(pVideoPriv != HI_NULL, "pVideoPriv = null");

    pFramePriv = (HI_VDEC_PRIV_FRAMEINFO_S *)(pVideoPriv->u32Reserve);
    OMXVDEC_ASSERT_RETURN(pFramePriv != HI_NULL, "pFramePriv = null");
...
HI_S32 processor_release_image(OMXVDEC_CHAN_CTX *pchan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    ret = processor_convert_frame_to_image(pstFrame, &stImage);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_FATAL, "%s pstFrame = NULL!\n", __func__);
        return HI_FAILURE;
