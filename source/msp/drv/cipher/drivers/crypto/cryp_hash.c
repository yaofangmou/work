...
static s32 cryp_hash_create_mem(void)
{
    ...
    {
        ret = crypto_mem_create(&hash_mem, SEC_MMZ, "CIPHER_SHA_MSG", length);
        if(HI_SUCCESS == ret)
        {
            hash_mem_ref = 0x01;
            HI_LOG_INFO("create ddr for hash msg, addr 0x%x, size 0x%x\n", ADDR_L32(hash_mem.dma_addr), hash_mem.dma_size);
            return HI_SUCCESS;
        }
        else
        {
...
static void cryp_register_hash_default(u32 capacity, hash_mode mode, u32 blocksize, u32 hashlen)
{
    ...
    crypto_memset(&func, sizeof(hash_func), 0 ,sizeof(hash_func));
...
s32 cryp_hash_release(void)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_LOG_PrintFuncErr(cryp_hash_destory_mem, ret);
        return ret;
    }
...
void cryp_get_hash_mode(u8 *mode, u32 *count)
{
    u32 i = 0;
    u32 id = 0;
    ...
    for(i=0; i<HASH_FUNC_TAB_SIZE; i++)
    {
        if (hash_descriptor[i].valid)
        {
            mode[id++] = (u8)hash_descriptor[i].mode;
        }
    }
