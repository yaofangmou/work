...
HI_S32 Sample_Osd_ShowElem(SAMPLE_OSD_ELEMENT_INFO_S *pstElm, HI_BOOL bHighLight)
{
    ...
    if (pstElm->enType == SAMPLE_OSD_ELEMENT_BOTH)
    {
        if ((pstElm->u32StrNum == 2) && (pstElm->u32ValNum == 2))
            snprintf(szContent, sizeof(szContent), "%s%d%s%d", pstElm->szContent0, pstElm->u32Value[0],  pstElm->szContent1, pstElm->u32Value[1]);
        else if ((pstElm->u32StrNum == 2) && (pstElm->u32ValNum == 1))
            snprintf(szContent, sizeof(szContent), "%s%d%s", pstElm->szContent0, pstElm->u32Value[0],  pstElm->szContent1);
        else if ((pstElm->u32StrNum == 1) && (pstElm->u32ValNum == 1))
            snprintf(szContent, sizeof(szContent), "%s%d", pstElm->szContent0, pstElm->u32Value[0]);
        else
    ...
    else
    {
        if (pstElm->u32StrNum == 2)
            snprintf(szContent, sizeof(szContent), "%s%s", pstElm->szContent0, pstElm->szContent1);
        else
            snprintf(szContent, sizeof(szContent), "%s", pstElm->szContent0);
    }
