...
HI_S32 HI_DRV_HDMI_Suspend(PM_BASEDEV_S *pdev, pm_message_t state)
{
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        if(pstHdmiDev->stCecInfo.bCecEnable )
        {
            ...
            stDrvCecCmd.CECCmd.u8Opcode = CEC_OPCODE_INACTIVE_SOURCE;
            stDrvCecCmd.CECCmd.stOperand.stRawData.u8Length  = 0x2;
            stDrvCecCmd.CECCmd.stOperand.stRawData.u8Data[0] = ((pstHdmiDev->stCecInfo.au8PhysicalAddr[0] << 4) & 0xf0) | (pstHdmiDev->stCecInfo.au8PhysicalAddr[1] & 0x0f);
            stDrvCecCmd.CECCmd.stOperand.stRawData.u8Data[1] = ((pstHdmiDev->stCecInfo.au8PhysicalAddr[2] << 4) & 0xf0) | (pstHdmiDev->stCecInfo.au8PhysicalAddr[3] & 0x0f);
            ret = HdmiExtIoctl(CMD_HDMI_SET_CEC, &stDrvCecCmd);
            if (ret != HI_SUCCESS)
            {
                HDMI_ERR("Set CEC err!:0x%x\n",ret);
            }
            HDMI_INFO("INACTIVE_SOURCE cec!\n");
        }
#endif
        ...
        {
            HDMI_HAL_CALL(HAL_HDMI_CecMsgClear, pstHdmiDev->pstHdmiHal);
        }
#endif
...
HI_S32 HI_DRV_HDMI_Resume(PM_BASEDEV_S *pdev)
{
    ...
    DRV_HDMI_APP_ATTR_S        stHDMIAttr  = {0};
    ...
    while (enHdmi < HDMI_DEVICE_ID_BUTT)
    {
        ...
        if(pstHdmiDev->stCecInfo.bCecEnable )
        {
            ...
            ret = HdmiExtIoctl(CMD_HDMI_CEC_ENABLE, &enHdmi);
            if (ret != HI_SUCCESS)
            {
                HDMI_ERR("CEC enable  err!:0x%x\n",ret);
            }
            ...
            ret = HdmiExtIoctl(CMD_HDMI_GET_CECSTATUS, &stDrvCecInfo);
            if (ret != HI_SUCCESS)
            {
                HDMI_ERR("Get CEC status err!:0x%x\n",ret);
            }
            stDrvCecCmd.CECCmd.u8Opcode = CEC_OPCODE_ACTIVE_SOURCE;
            stDrvCecCmd.CECCmd.stOperand.stRawData.u8Length  = 0x2;
        ...
        HDMI_INFO("HdmiExtIoctl force CMD_HDMI_START ok! \n");
#endif
        {
            HI_U32 i = 0;
            ...
            for ( i=0 ; i < 110; i++)
            {
                ret = HdmiExtIoctl(CMD_HDMI_GET_STATUS, &stDrvStatus);
                if (ret != HI_SUCCESS)
                {
                    HDMI_ERR("Get status  err!:0x%x\n",ret);
                }
            ...
            HDMI_INFO("wait HPD %d ms,HPD=%d\n",i,stDrvStatus.stHdmiStatus.bConnected);
            if (!stDrvStatus.stHdmiStatus.bConnected) // report HOTUNPLUG event when wakeup no hotplug,because hardware  init no UnHPD intterupt in this case.
            {
                HDMI_INFO("HPD=%d!report un-plug event!\n",stDrvStatus.stHdmiStatus.bConnected);
                DRV_HDMI_EventPoolWrite(pstHdmiDev->u32HdmiDevId, HDMI_EVENT_HOTUNPLUG, HI_FALSE);
            }

        }
#else   //HDMI_RESUME_FAST_KPI
        stDrvStatus.enHdmiID = enHdmi;
        ret =  HdmiExtIoctl(CMD_HDMI_GET_STATUS, &stDrvStatus);
        if (ret != HI_SUCCESS)
        {
...
#ifndef HDMI_MASK_START
        /* we need to output signal as soon as possible in resume case. */
        if (stDrvStatus.stHdmiStatus.bConnected)
        {
        ...
        {
            HDMI_INFO("HdmiExtIoctl CMD_HDMI_START fail becasuse Hpd=0 \n");
        }
#endif
#endif  //HDMI_RESUME_FAST_KPI
        enHdmi++;
    }
