...
HI_S32 WIN_ProcessCmd(unsigned int cmd, HI_VOID *arg, WIN_STATE_S *pstWinState)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_WIN_QUSYNC_FRAME:
        {
            ...
            Ret = WIN_CheckHanlde(pVoWinFrame->stXdrFrame.hWindow, pstWinState);
            if (HI_SUCCESS == Ret)
            {
                Ret = WIN_QueueSyncFrame(pVoWinFrame->stXdrFrame.hWindow, &pVoWinFrame->stXdrFrame.stBaseFrm,&pVoWinFrame->u32FenceFd);
            }
            break;
        }
