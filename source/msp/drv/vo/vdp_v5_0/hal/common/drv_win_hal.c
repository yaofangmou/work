...
HI_VOID GetSpecialLayerPriorty(HI_U32 u32layer_id, HI_U32 *pu32prio)
{
    ...
    else if (u32layer_id == VDP_LAYER_SD0)
    {
                         ...
                         CBB_CFG_EXT_S *pVDPCfgExtPara)
{
    ...
    else
    {
        ...
        pCbbLayerBasePara->stSdVideoRect      = pstPara->stSdVideoRect;
        pCbbLayerBasePara->stSdOutRect        = pstPara->stSdOutRect;
        ...
        TransvertPixelFmt2CBB(&pstPara->eInPutPixelFmt, &pCbbLayerBasePara->eInPutPixelFmt);
...
#endif
        pCbbLayerBasePara->bFieldMatch   = pstPara->bFieldMatch;
    }
...
{
    HI_BOOL bVdmArray[WINDOW_MAX_NUMBER] = {0};

    WinCheckNullPointer(pstWinInfo);

    ADAPTER_SetLayerCfg(u32Layer, (WIN_INFO_S *)pstWinInfo, bVdmArray, HI_TRUE);
    return HI_SUCCESS;
}
...
HI_S32 HAL_SetEnable(HI_U32 eLayer, HI_BOOL bEnable)
{
    LAYERPROCESS_SetLayerEnable(eLayer, bEnable);
    return HI_SUCCESS;
}

HI_S32 HAL_GetEnable(HI_U32 eLayer, HI_BOOL *pbEnable)
{
    LAYERPROCESS_GetLayerEnable(eLayer, pbEnable);
    return HI_SUCCESS;
}
...
HI_S32 VideoLayer_Init(HI_VOID)
{
    ...
    s_stVieoLayerFunc.PF_GetEnable      = HAL_GetEnable;
    s_stVieoLayerFunc.PF_SetLayerMute    = HW_VID_SetLayerMute;

    s_stVieoLayerFunc.PF_InitialLayer     = VDP_DRV_InitLayer;

    s_stVieoLayerFunc.PF_DeInitialLayer     = VDP_DRV_DeInitLayer;
    ...
    s_stVieoLayerFunc.PF_DeuqueBufNode          = HW_DRV_DeQueueFrame;//HAL_GetTotalBufStatus;
    s_stVieoLayerFunc.PF_ResetHwBuf             = HW_DRV_SetLayerResetMode;//HAL_ResetBufList;

    s_stVieoLayerFunc.PF_GetTotalBufStatus      = HW_BUF_GetTotalBufStatus;
    s_stVieoLayerFunc.PF_PauseHwBuf             = VDP_DRV_SetLayerPlayMode;
    s_stVieoLayerFunc.PF_SetHwBufQuickoutMode   = VDP_DRV_SetLayerPlayMode;
    s_stVieoLayerFunc.PF_CheckResetFinish       = HW_BUF_CheckResetFinish;
    s_stVieoLayerFunc.PF_HW_ChangeApbConfig     = HW_ChangeApbConfig;
    s_stVieoLayerFunc.PF_HW_BUF_SetStepMode     = HW_BUF_SetStepMode;

    s_stVieoLayerFunc.PF_HW_BUF_GetFramePrivateInfor   = HW_BUF_GetFramePrivateInfor;

    s_stVieoLayerFunc.PF_HW_BUF_GetCurrentPlayFrameInfor   = HW_BUF_GetCurrentPlayFrameInfor;

    s_stVieoLayerFunc.PF_HW_ALG_POLICY_GetVpssBufferSize   = HW_ALG_POLICY_GetVpssBufferSize;
    s_stVieoLayerFunc.PF_HW_BUF_GetLastHardwareConfig      = HW_BUF_GetLastHardwareConfig;

    /*just for alg get or set.*/
    s_stVieoLayerFunc.PF_Alg_GetMemcfg           = HI_NULL;//HAL_Alg_GetMemcfg;
    s_stVieoLayerFunc.PF_Alg_GetAlgEnablePolicy  = HI_NULL;//HAL_Alg_GetAlgEnablePolicy;
    s_stVieoLayerFunc.PF_Alg_GetWbcConfig = HW_ALG_GetWbcConfig;
    s_stVieoLayerFunc.PF_Alg_SetWbcConfig = HW_ALG_SetWbcConfigParam;
    s_stVieoLayerFunc.PF_OfflineReset = HW_OfflineReset;
