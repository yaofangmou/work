                                ...
                                DRV_ADVCA_CA_TARGET_E enCwTarget)
{
    ...
    {
        if (g_SPInfo.bKeyReset == HI_FALSE && enLevel < HI_UNF_ADVCA_KEYLADDER_LEV5)
        {
                            ...
                            DRV_ADVCA_CA_TARGET_E enCwTarget)
{
    ...
    if (CA_OTP_VENDOR_VERIMATRIX == enCaVendorType)
    {
        if (g_MiscInfo.bKeyReset == HI_FALSE && enLevel < HI_UNF_ADVCA_KEYLADDER_LEV5)
        {
...
HI_S32 HAL_ADVCA_V300_ResetOTP(HI_VOID)
{
    ...
    CA_readReg(pu8VirAddr, &OtpV200CrgCtrl.u32);
    OtpV200CrgCtrl.bits.otp_srst_req = 1;
    CA_writeReg(pu8VirAddr, OtpV200CrgCtrl.u32);
...
static HI_U8 *g_otpc_vir_addr = 0;
HI_S32 HAL_OTPC_RegisterMap(void)
{
    HI_VOID *ptr;
    ptr = ioremap_nocache(OTPC_REG_ADDR, 0x100);
    if (ptr == NULL)
    {
        HI_ERR_CA("ioremap_nocache err!\n");
        return HI_FAILURE;
    }
    g_otpc_vir_addr = ptr;
    return HI_SUCCESS;
...
HI_VOID HAL_OTPC_RegisterUnMap(void)
{
    if (g_otpc_vir_addr)
    {
        iounmap((HI_VOID *)g_otpc_vir_addr);
        g_otpc_vir_addr = 0;
    }
    return;
}

HI_VOID HAL_OTPC_WriteReg(HI_U32 addr, HI_U32 val)
{
    CA_writeReg((g_otpc_vir_addr + (addr - OTPC_REG_ADDR)), val);
    return;
}

HI_U32 HAL_OTPC_ReadReg(HI_U32 addr)
{
    HI_U32 val = 0;
    CA_readReg((g_otpc_vir_addr + (addr - OTPC_REG_ADDR)), &val);
    return val;
}

HI_S32 HAL_OTPC_CRCFinish(HI_BOOL is_sm2)
{
    HI_U32 cnt = 0;
    OTPC_CRC_CHECK_STATUS_U crc_check_sta;

    while (cnt < 5000)
    {
        crc_check_sta.u32 = 0;
        crc_check_sta.u32 = DRV_ADVCA_ReadReg(OTPC_CRC_CHECK_STATUS);
        if(is_sm2){
            if (crc_check_sta.bits.sm2_crc_finish == 1)
            {
                HI_INFO_CA("ca_decrypted finished, cnt:%d\n", cnt);
                break;
            }
        }else{
            if (crc_check_sta.bits.rsa_crc_finish == 1)
            {
                HI_INFO_CA("ca_decrypted finished, cnt:%d\n", cnt);
                break;
            }
...
HI_S32 HAL_ADVCA_ProcGetReginfo(HI_U32 au32RegInfo[4])
{
    HI_S32 ret = HI_SUCCESS;
    OTPC_CRC_CHECK_STATUS_U crc_check_sta;
    OTPC_CRC_CHECK_CTRL_U crc_check_ctrl;

    if(HI_SUCCESS != HAL_OTPC_RegisterMap())
    {
        ret = HI_FAILURE;
        goto RET;
    }
    crc_check_sta.u32 = HAL_OTPC_ReadReg(OTPC_CRC_CHECK_STATUS);
    if(crc_check_sta.bits.rsa_crc_finish != 1)
    {
        crc_check_ctrl.u32 = 0;
        crc_check_ctrl.bits.crc_check_sel = 0;
        crc_check_ctrl.bits.rd_crc_start = 1;
        HAL_OTPC_WriteReg(OTPC_CRC_CHECK_CTRL, crc_check_ctrl.u32);
        if(HI_SUCCESS != HAL_OTPC_CRCFinish(HI_FALSE)){
            ret = HI_FAILURE;
        }
    }
    if(crc_check_sta.bits.sm2_crc_finish != 1)
    {
        crc_check_ctrl.u32 = 0;
        crc_check_ctrl.bits.crc_check_sel = 1;
        crc_check_ctrl.bits.rd_crc_start = 1;
        HAL_OTPC_WriteReg(OTPC_CRC_CHECK_CTRL, crc_check_ctrl.u32);
        if(HI_SUCCESS != HAL_OTPC_CRCFinish(HI_TRUE)){
            ret = HI_FAILURE;
        }
    }
    au32RegInfo[0] = OTPC_CRC_CHECK_STATUS;
    au32RegInfo[1] = HAL_OTPC_ReadReg(OTPC_CRC_CHECK_STATUS);
RET:
    HAL_OTPC_RegisterUnMap();
    return ret;
}
#else
...
HI_S32 HAL_ADVCA_V300_CryptbyStbRootkey(HI_U32 AddrID)
{
    ...
    CA_V300_CONFIG_STATE_U unConfigStatus;
#if defined(CHIP_TYPE_hi3798mv310)
    CA_V300_STB_RK2_CTRL_U unStbRk2Ctrl;
#else
...
#if defined(CHIP_TYPE_hi3798mv310)
    unStbRk2Ctrl.u32 = 0;
    unStbRk2Ctrl.bits.mc_if_rdy = 1;
    unStbRk2Ctrl.bits.key_addr = (AddrID << 1) & 0xFF;
    DRV_ADVCA_ReadReg(CA_V300_STB_RK2_CTRL);
    DRV_ADVCA_WriteReg(CA_V300_STB_RK2_CTRL, unStbRk2Ctrl.u32);
#else
...
HI_VOID HAL_ADVCA_V300_Resume(HI_VOID)
{
    g_SPInfo.bKeyReset = HI_TRUE;
    g_MiscInfo.bKeyReset = HI_TRUE;
    return;
