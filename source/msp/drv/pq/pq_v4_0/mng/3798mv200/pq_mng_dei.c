...
static HI_BOOL sg_abStaticImage[VPSS_HANDLE_NUM][3] = {{0}};

#define STATIC_RATION_MAX 995
...
static HI_S32 PQ_MNG_API_GMD_MA(HI_U32 u32HandleNo, HI_U32 u32FrameIndex, const HI_U32 *pu32MotionData,
                                HI_U32 *pu32GlobalMotionCur, HI_BOOL *pbStaticImage)
{
    HI_U32 i;
    static HI_U32 global_motion_current = 255, global_motion_last = 255, u32GlobalStaticRatio = 255;
    HI_U32 motion_num[5]     = {0}, motion_sum[5]     = {0};
    HI_U64 global_motion_num = 0, global_motion_sum = 0;
    HI_U64 global_motion_result;
    HI_U32 u32GlobalMotionStaticSum = 0;
    ...
    *pu32GlobalMotionCur = global_motion_current;

    /* add last three field */
    u32GlobalMotionStaticSum = motion_num[0] + motion_num[1] + motion_num[2] + motion_num[3] + motion_num[4];
    if (u32GlobalMotionStaticSum == 0)
    {
        u32GlobalMotionStaticSum = 1;
    }
    u32GlobalStaticRatio = (motion_num[0] * 1000) / u32GlobalMotionStaticSum;

    if (u32FrameIndex % 3 == 0)
    {
        sg_abStaticImage[u32HandleNo][0] = u32GlobalStaticRatio;
    }
    else if (u32FrameIndex % 3 == 1)
    {
        sg_abStaticImage[u32HandleNo][1] = u32GlobalStaticRatio;
    }
    else
    {
        sg_abStaticImage[u32HandleNo][2] = u32GlobalStaticRatio;
    }

    if ((sg_abStaticImage[u32HandleNo][0] > STATIC_RATION_MAX) &&
        (sg_abStaticImage[u32HandleNo][1] > STATIC_RATION_MAX) &&
        (sg_abStaticImage[u32HandleNo][2] > STATIC_RATION_MAX))
    {
        *pbStaticImage = 1;
    }
    else
...
*****************************************************************************/
HI_S32 PQ_MNG_ALG_GetMcDeiParam(MOTION_INPUT_S *pstMotionInput, HI_PQ_WBC_INFO_S *pstVpssWbcInfo)
{
    S_VPSS_STT_REGS_TYPE *pstSTTReg = HI_NULL;
    HI_S32 nAdjK      = 0;
    HI_S32 nAdjK_STAT = 0;
    HI_S32 nVmvSum    = 0;
    HI_S32 nHmvSum    = 0;
    ...
    PQ_MNG_API_GMD_MA(pstMotionInput->u32HandleNo, pstMotionInput->u32FrameIndex, au32MotionData, &nGlobalMotion, &pstVpssWbcInfo->bStaticImage);
...
HI_S32 PQ_MNG_SetDeiParam(HI_PQ_WBC_INFO_S *pstVpssWbcInfo)
{
    ...
    stMotionInput.pstMotionReg = (S_VPSS_STT_REGS_TYPE *)(pstVpssWbcInfo->pstVPSSWbcReg);
    ...
    PQ_MNG_ALG_GetMcDeiParam(&stMotionInput, pstVpssWbcInfo);
