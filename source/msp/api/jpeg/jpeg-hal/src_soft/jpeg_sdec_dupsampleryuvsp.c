                                                         ...
                                                         JDIMENSION out_rows_avail)
{
    ...
    if (OutYBufOffset > (YSize - YStride))
    {
        return;
    }
                                                             ...
                                                             JDIMENSION out_rows_avail)
{
    ...
    /**<-- 输出Y  数据>**/
    OutYBufOffset = YStride * cinfo->output_scanline;
    if (OutYBufOffset > (YSize - 2 * YStride))
    {
        return;
    }
    OutYBuf = OutYBuf + OutYBufOffset;
                                             		    ...
                                             		    JDIMENSION out_rows_avail)
{
    ...
    JPEG_DEC_GetOutUsrBuf((HI_ULONG)cinfo->client_data, &OutYBuf, &YStride, &YSize, &OutCbCrBuf, &CbCrStride, &CbCrSize);
    ...
    if (OutCbCrBufOffset > (CbCrSize - CbCrStride))
    {
        return;
    }
