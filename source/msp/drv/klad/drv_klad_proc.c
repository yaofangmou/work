...
#define HI_RSA_CRC_CHECK_FLAG       (HI_KLAD_BASE_RKP + (0x4044))
...
typedef union
{
    struct
    {
        HI_U32 rd_rsa_crc_start         : 1; //[0]
        HI_U32 check_sum_rsa_area_flag  : 1; //[1]
        HI_U32 rd_rsa_crc_finish        : 1; //[2]
        HI_U32 reserved                 : 28;//[3~30]
        HI_U32 rd_rsa_crc_start_err     : 1; //[31]
    } bits;
    HI_U32 u32;
} KL_RSA_CRC_CHECK_FLAG_U;
...
static DRV_KLAD_ROOTKEY_CRC C3_rootkey_crc[] = {
    {0, "CM0_CSA2_RootKey",    0x5024, 8},
    {1, "CM0_CSA3_RootKey",    0x5024, 9},
    {2, "CM0_SP_RootKey",      0x5024, 10},
    {3, "CM0_R2R_Rootkey",     0x5024, 11},
    {4, "CM0_LP_Rootkey",      0x5024, 13},
};
#else
static DRV_KLAD_ROOTKEY_CRC C3_rootkey_crc[] = {
...
static DRV_KLAD_PROCINFO C6_klad_info[] = {
    {0, HI_UNF_KLAD_VERIMATRIX_TYPE_SP,   "SP  ", HAL_KLAD_VERIMATRIX_SPGetLevel  },
    {1, HI_UNF_KLAD_VERIMATRIX_TYPE_R2R,  "R2R ", HAL_KLAD_VERIMATRIX_R2RGetLevel },
    {2, HI_UNF_KLAD_VERIMATRIX_TYPE_MISC, "MISC", HAL_KLAD_VERIMATRIX_MISCGetLevel},
};

static DRV_KLAD_ROOTKEY_CRC C6_rootkey_crc[] = {
    {0, "CM1_SP_RootKey",     0x5028, 8},
    {1, "CM1_R2R_RootKey",    0x5028, 9},
    {2, "CM1_MISC_RootKey",   0x5028, 10},
};
#else
static DRV_KLAD_PROCINFO C6_klad_info[] = {
...
static DRV_KLAD_ROOTKEY_CRC Basic_rootkey_crc[] = {
    {0, "BOOT_Rootkey",     0x502c, 15},
    {1, "HDCP_Rootkey",     0x502c, 9},
    {2, "STBM_R2R_Rootkey", 0x502c, 8},
    {3, "SW_Rootkey",       0x502c, 11},
    {4, "JTAG_KEY",         0x502c, 16},
    {5, "dcas_rk_ext_key",  0x502c, 13},
    {6, "smk_ext_key",      0x502c, 14},
    {7, "Hisi_Rootkey",     0x502c, 17},
};
#else
static DRV_KLAD_ROOTKEY_CRC Basic_rootkey_crc[] = {
...
#if defined(CHIP_TYPE_hi3716mv430)
HI_S32 HAL_OTPC_CheckRsaCrcStat(HI_VOID)
{
    HI_U32 cnt = 0;
    KL_RSA_CRC_CHECK_FLAG_U state;

    while (1)
    {
        udelay(5);
        state.u32 = 0;
        if(HI_SUCCESS != HAL_KLAD_ProcReadOtpShadowReg(HI_RSA_CRC_CHECK_FLAG, &state.u32))
        {
            return HI_FAILURE;
        }
        if (state.bits.rd_rsa_crc_finish == 1)
        {
            break;
        }
...
static HI_S32 DRV_KLAD_GetRsaCrcVerifyStatus(HI_U8 *verify)
{
    KL_RSA_CRC_CHECK_FLAG_U state;
    ...
    {
        state.u32 = 0;
        state.bits.rd_rsa_crc_start = 1;
        if(HI_SUCCESS != HAL_KLAD_ProcWriteOtpShadowReg(HI_RSA_CRC_CHECK_FLAG, state.u32))
        {
            return HI_FAILURE;
        }
        if(HI_SUCCESS != HAL_OTPC_CheckRsaCrcStat())
        {
            return HI_FAILURE;
        }
    }
    ...
    *verify = state.bits.check_sum_rsa_area_flag & 0x1;
...
#if defined(CHIP_TYPE_hi3716mv430)
    if(HI_SUCCESS != DRV_KLAD_GetRsaCrcVerifyStatus(&rsa_verify))
    {
        PROC_PRINT(p, "RSA Root Public Key crc is not calculated.\n");
        return HI_FAILURE;
    }
#else
