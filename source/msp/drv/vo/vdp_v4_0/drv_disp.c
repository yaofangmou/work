...
static HI_S32 DISPProcessCmdSetTechniBright(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser);
static HI_S32 DISPProcessCmdGetTechniBright(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser);
...
static const stDISPProcessCmdFunc s_aDISPCmdFuncTab[] =
{
    ...
    {CMD_DISP_SET_TECHNI_BRIGHT, DISPProcessCmdSetTechniBright},
    {CMD_DISP_GET_TECHNI_BRIGHT, DISPProcessCmdGetTechniBright},
};
...
static HI_VOID DISP_DolbyHdrProcRead(struct seq_file *p,DISP_PROC_INFO_S *pstDispAttr)
{
    ...
    if (HI_NULL != pstFrm)
    {
        ...
        {
            CHECK_ARRAY_RANGE(pstFrm->unHDRInfo.stDolbyInfo.bCompatible, DISP_YN_STRING_BUTT);
            PROC_PRINT(p, "%-20s:%s\n", "bCompatible",
                       g_pDispYNString[pstFrm->unHDRInfo.stDolbyInfo.bCompatible]);
        }
        CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.bELVaild, DISP_YN_STRING_BUTT);
        PROC_PRINT(p, "%-20s:%s\n", "HdrCfg->ELValid",
    ...
    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmCfg.base.signal_chroma_format, DISP_HDR_CFG_CHRM_FORMAT_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmCfg->chrm",
      g_pDispHdrCfgChrmFormat[(HI_U32)pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmCfg.base.signal_chroma_format]);

    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmCfg.base.signal_color_space, DISP_HDR_CFG_CS_STRING_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmCfg->color space",
      g_pDispHdrCfgCsString[(HI_U32)pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmCfg.base.signal_color_space]);

    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksIMap.clr, DISP_HDR_CFG_CLR_STRING_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmKs->IMap.clr",
      g_pDispHdrCfgClrString[(HI_U32)pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksIMap.clr]);

    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksOMap.clr, DISP_HDR_CFG_CLR_STRING_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmKs->OMap.clr",
      g_pDispHdrCfgClrString[(HI_U32)pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksOMap.clr]);

    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksIMap.eotfParam.eotf, DISP_HDR_CFG_EOTF_TYPE_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Src.eotf",
      g_pDispHdrCfgEotfType[(HI_U32)pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksIMap.eotfParam.eotf]);

    CHECK_ARRAY_RANGE(pstDispAttr->stDolbyInfo.stDolbyCoeff.stDmKs.ksOMap.oetfParam.oetf, DISP_HDR_CFG_EOTF_TYPE_BUTT);
    PROC_PRINT(p, "%-20s:%s\n", "DmKs->Tag.eotf",
...
static HI_S32 DISP_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    if (g_pDispProcLevel > DISP_PROC_BASE)
    {
        PROC_PRINT(p, "%-20s:%d/%d/%d/%d\n", "ActualOffset(L/T/R/B)",s_DispAttr.stActualOffsetInfo.u32Left,
                                                      s_DispAttr.stActualOffsetInfo.u32Top,
                                                      s_DispAttr.stActualOffsetInfo.u32Right,
                                                      s_DispAttr.stActualOffsetInfo.u32Bottom);
    }
        ...
        if (g_pDispProcLevel >= DISP_PROC_EXTREN_XDR_INFO)
        {
            ...
            for (u32Index = 0; u32Index < WINDOW_MAX_NUMBER; u32Index++)
            {
                ...
                if (WIN_SHARE_HandleValid(pstWinInfo->hWinHandle))
                {
                    ...
                    CHECK_ARRAY_RANGE(pstWinInfo->bRegionMute, DISP_YN_STRING_BUTT);
...
HI_S32 DispProcSetFormat(HI_DRV_DISPLAY_E enDisp, HI_CHAR *pArg2)
{
    ...
    if (HI_DRV_DISP_FMT_BUTT == fmt)
    {
        ...
        for (idx=0; idx<(sizeof(s_aFmtStringMap)/sizeof(DRV_DISP_FMT_STRING_S)); idx++)
        {
            HI_DRV_PROC_EchoHelper("   %s\n", s_aFmtStringMap[idx].pfmtString);
        }
        HI_DRV_PROC_EchoHelper("\n");
...
static DISP_CAST_FRAME_S g_stCastFrame = {0};
...
HI_S32  HI_DRV_DISP_AcquireCastFrame(HI_HANDLE hCast, HI_DRV_VIDEO_FRAME_S *pstCastFrame)
{
    ...
    memset(&g_stCastFrame, 0, sizeof(DISP_CAST_FRAME_S));
    g_stCastFrame.hCast = hCast;
    Ret = HI_DRV_DISP_Process(CMD_DISP_ACQUIRE_CAST_FRAME, &g_stCastFrame);
    if(!Ret)
    {
        *pstCastFrame = g_stCastFrame.stFrame;
    }
    return Ret ;
}
...
HI_S32 HI_DRV_DISP_ReleaseCastFrame(HI_HANDLE hCast, HI_DRV_VIDEO_FRAME_S *pstCastFrame)
{
    ...
    memset(&g_stCastFrame, 0, sizeof(DISP_CAST_FRAME_S));
    g_stCastFrame.hCast = hCast;
    g_stCastFrame.stFrame = *pstCastFrame;

    Ret = HI_DRV_DISP_Process_Intr(CMD_DISP_RELEASE_CAST_FRAME, &g_stCastFrame);
...
static HI_S32 DISPProcessCmdAddIntf(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,add ID-%d,Y Pb Pr sync(%d,%d,%d,%d)-ret -0x%x\n",
        pDispIntf->enDisp,
        pDispIntf->stIntf.eID,
        pDispIntf->stIntf.u8VDAC_Y_G,
        pDispIntf->stIntf.u8VDAC_Pb_B,
        pDispIntf->stIntf.u8VDAC_Pr_R,
        pDispIntf->stIntf.bDacSync,
        Ret);
...
static HI_S32 DISPProcessCmdDelIntf(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,del ID-%d,Y Pb Pr sync(%d,%d,%d,%d)-ret -0x%x\n",
        pDispIntf->enDisp,
        pDispIntf->stIntf.eID,
        pDispIntf->stIntf.u8VDAC_Y_G,
        pDispIntf->stIntf.u8VDAC_Pb_B,
...
static HI_S32 DISPProcessCmdSetWSS(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d set WSS,enable-(%d),data-(%d),-ret -0x%x\n",
              pDispWss->enDisp,
              pDispWss->WssData.bEnable,
              pDispWss->WssData.u16Data,
              Ret);
...
static HI_S32 DISPProcessCmdSetMCRVSN(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set MCRVSN ,mode-(%d),-ret -0x%x\n",
                  pDispMcrvsn->enDisp,
                  pDispMcrvsn->eMcrvsn,
                  Ret);
...
static HI_S32 DISPProcessCmdSetCGMS(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set CGMS enable(%d) ,type-(%d),,mode-(%d),-ret -0x%x\n",
              pDispCgms->enDisp,
              pDispCgms->stCgmsCfg.bEnable,
              pDispCgms->stCgmsCfg.enType,
              pDispCgms->stCgmsCfg.enMode,
              Ret);
...
static HI_S32 DISPProcessCmdSetOutputType(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set Output Type(%d)-ret -0x%x\n",
              pstDispOutput->enDisp,
              pstDispOutput->enDispType,
              Ret);
    return Ret;
}
...
static HI_S32 DISPProcessCmdSetOutputColorSpace(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set Color Space (%d)-ret -0x%x\n",
                  pstDispColorSpace->enDisp,
                  pstDispColorSpace->enColorSpace,
                  Ret);
...
static HI_S32 DISPProcessCmdSetXdrEngineType(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set XdrEngineType (%d)-ret -0x%x\n",
              pstDispXdrEngine->enDisp,
              pstDispXdrEngine->enXdrEngine,
              Ret);
...
static HI_S32 DISPProcessCmdSetAlpha(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    DISP_WARN("disp%d ,set Alpha (%d)-ret -0x%x\n",
              pstDispAlpha->enDisp,
              pstDispAlpha->u32ALpha,
              Ret);
...
{
    HI_S32 Ret = HI_FAILURE;
    DISP_TECHNI_BRIGHT_S *pstDispTechniBright = HI_NULL;
    pstDispTechniBright = (DISP_TECHNI_BRIGHT_S*)arg;
    DISP_DEBUG_CALLER();

    Ret = DISP_SetTechniDispBright(pstDispTechniBright->enDisp, pstDispTechniBright->s32TechniDispBright);
    DISP_WARN("disp%d ,Techni Bright(%d)-ret -0x%x\n",
        pstDispTechniBright->enDisp,
        pstDispTechniBright->s32TechniDispBright,
        Ret);
    return Ret;
}
...
static HI_S32 DISPProcessCmdGetTechniBright(HI_VOID* arg, DRV_DISP_STATE_S* pDispState, HI_BOOL bUser)
{
    ...
    Ret = DISP_GetTechniDispBright(pstDispTechniBright->enDisp, &pstDispTechniBright->s32TechniDispBright);
    return Ret;
}
