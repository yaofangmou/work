...
HI_S32 JPEG_ADP_CreateDecode(HI_U32 *pDecHandle,HI_CHAR* pSrcBuf,HI_U32 u32SrcLen)
{
    ...
    HI_GFX_CHECK_EQUAL_ZERO_RETURN_VALUE(u32SrcLen, HI_FAILURE);

    DecHandle = HI_JPEG_CreateDecompress((HI_U32)pSrcBuf, u32SrcLen, pSrcBuf, u32SrcLen, NULL, 0);
    if (0 == DecHandle)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_JPEG_CreateDecompress, DecHandle);
        return HI_FAILURE;
    }

    if (DecHandle > UINT_MAX)
    {
       HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, DecHandle);
...
HI_S32 JPEG_ADP_GetImgInfo(HI_U32 DecHandle,HI_HANDLE hSurface)
{
    ...
    HI_GFX_CHECK_EQUAL_ZERO_RETURN_VALUE(DecHandle, HI_FAILURE);

    Ret = HI_JPEG_ParseHeader(DecHandle);
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_JPEG_ParseHeader, Ret);
        return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_JPEG_GetJpegInfo, Ret);
        return HI_FAILURE;
    }
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stJpegInfo.u32Width[0]);
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stJpegInfo.u32Height[0]);
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stJpegInfo.u32Width[1]);
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "CONFIG_GFX_MINWIDTH = 32");
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "CONFIG_GFX_MINWIDTH = 8192");
        return HI_FAILURE;
    }
    ...
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stJpegInfo.u32Height[1]);
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "CONFIG_GFX_MINHEIGHT = 32");
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "CONFIG_GFX_MAXHEIGHT = 8192");
        return HI_FAILURE;
    }
    ...
    switch (stJpegInfo.enFmt)
    {
        ...
        case HI_JPEG_FMT_YUV422_21:
            ...
            break;
        default:
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stJpegInfo.enFmt);
            return HI_FAILURE;
    }
...
HI_S32 JPEG_ADP_StartDecode(HI_U32 DecHandle, HI_HANDLE hSurface)
{
    ...
    stSurfaceDesc.u32OutSize[0]   = stJpegInfo.u32OutSize[0];
    stSurfaceDesc.u32OutSize[1]   = stJpegInfo.u32OutSize[1];

    Ret = HI_JPEG_SetOutDesc(DecHandle,&stSurfaceDesc);
    if (HI_SUCCESS != Ret)
    {
       HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_JPEG_SetOutDesc, Ret);
       return HI_FAILURE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_JPEG_StartDecompress, Ret);
        return HI_FAILURE;
    }
