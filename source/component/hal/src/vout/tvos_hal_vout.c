...
#ifdef TVOS_PLATFORM
    HI_UNF_ENC_FMT_E       enFmt = HI_UNF_ENC_FMT_BUTT;
#endif
...
S32 vout_channel_bind(struct _VOUT_DEVICE_S* pstVoutDev, VOUT_DISPLAY_CHANNEL_E enDstDisp, VOUT_DISPLAY_CHANNEL_E enSrcDisp)
{
    ...
    HAL_CHK_RETURN(((VOUT_DISPLAY_HD0 > enDstDisp) || (VOUT_DISPLAY_SD2 < enDstDisp)), ERROR_INVALID_PARAM, "input is invalid");
    HAL_CHK_RETURN(((VOUT_DISPLAY_HD0 > enSrcDisp) || (VOUT_DISPLAY_SD2 < enSrcDisp)), ERROR_INVALID_PARAM, "input is invalid");
    HAL_CHK_RETURN((enDstDisp <= enSrcDisp), ERROR_INVALID_PARAM, "dst channel should not be the same as the src channel and only support bind hd to sd");
...
S32 vout_autodetect3dformat(struct _VOUT_DEVICE_S* pstVoutDev, const HANDLE hVoutWindow, VOUT_3D_FORMAT_E* pen3dFormat)
{
    ...
    HAL_CHK_RETURN((NULL == pen3dFormat), ERROR_NULL_PTR, "input is null ptr");
                       ...
                       const VOUT_WINDOW_CREATE_PARAMS_S* const pstCreateParams)
{
    ...
    if (NULL == phVoutWindow)
    {
        HAL_ERROR("vout_window_create Invalid vout handle!!.");
        return ERROR_NULL_PTR;
    }
                                                       ...
                                                       BOOL* pbQuickOutputEnable)
{
    ...
    s32Ret = HI_UNF_VO_GetQuickOutputStatus(hVoutWindow, &bEnable);
    ...
    *pbQuickOutputEnable = unf_to_hal_for_bool(bEnable);
                            ...
                            struct hw_device_t** device)
{
    ...
    stVOUTdev->vout_channel_bind         = vout_channel_bind;
    stVOUTdev->vout_channel_unbind       = vout_channel_unbind;
    ...
    stVOUTdev->vout_set_3d_lr_switch     = vout_set_3d_lr_switch;
    stVOUTdev->vout_autodetect3dformat   = vout_autodetect3dformat;
    ...
    stVOUTdev->vout_window_enable_panorama      = vout_window_enable_panorama;
    ...
    stVOUTdev->vout_invoke                      = vout_invoke;
#endif
...
static  VOUT_DEVICE_S s_stVoutDev=
{
    ...
    .vout_init                 = vout_init,
    .vout_term                 = vout_term,
    .vout_channel_bind         = vout_channel_bind,
    .vout_channel_unbind       = vout_channel_unbind,
    ...
    .vout_set_3d_lr_switch     = vout_set_3d_lr_switch,
    .vout_autodetect3dformat   = vout_autodetect3dformat,
    ...
    .vout_window_enable_panorama      = vout_window_enable_panorama,
