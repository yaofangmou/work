...
#if defined(CHIP_TYPE_hi3798mv310)
    {1400000, HI_VDD_MPU_OPP6_HPM, HI_VDD_MPU_OPP6_VMIN, HI_VDD_MPU_OPP6_UV},
#endif
...
static unsigned int cpu_volt_otp_adjust(unsigned int freq)
{
    ...
    if (MAX_FREQ_NUM == i)
    {
        PM_ERR_PrintInfo("Not find this cpu frequency .\n");
        return 0;
    }
    ...
    if (0 == (regval & 0x40))
    {
        ...
        PM_INFO_PrintHex(otp_volt);
    }
    else
    {
        g_u8CpuOtpAddHPM = (regval & 0xf) * 5;
        PM_INFO_PrintHex(g_u8CpuOtpAddHPM);
        otp_volt = 0;
    }
...
unsigned int cpu_get_svb_volt(unsigned int freq)
{
    ...
    PM_ERR_PrintInfo(" can't find this freq in dms table:");
    PM_ERR_PrintS32(freq);
...
#if defined(CHIP_TYPE_hi3798mv310)
    PM_ERR_PrintInfo("Don't suppurt adjust core volt! \n");
    return 0;
#endif
...
static int _dvfs_scale(struct device *target_dev, struct hi_dvfs_info *tdvfs_info)
{
    ...
    if ((HI_CHIP_TYPE_HI3716D == g_enChipType) && (HI_CHIP_VERSION_V110 == g_enChipID))
    {
        if (tdvfs_info->new_freq > 800000)
        {
            PM_INFO_PrintInfo("This freq is not support:");
            PM_INFO_PrintS32(tdvfs_info->new_freq);
            return 0;
        }
    }
    ...
    {
        PM_ERR_PrintInfo("This freq is not support:");
...
unsigned int cpu_get_volt_from_chip(unsigned int freq, unsigned int dvfs_volt)
{
    ...
    {
        /* y= -2.59x+1830+40, max 1185mv*/
        volt  = 1870 - (2590 * g_corner_hpm) / 1000;
        if (volt >= 1185)
        {
            volt = 1185;
        }
