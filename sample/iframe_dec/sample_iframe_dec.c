...
#if defined(CHIP_TYPE_hi3716mv430) && (!defined(JPEG_SOFT_ENCODE_ENABLE))
HIGO_IMGTYPE_E g_ExpectType = HIGO_IMGTYPE_BMP;
HI_CHAR g_FileName[]="Iframe_picture.bmp";
#else
HIGO_IMGTYPE_E g_ExpectType = HIGO_IMGTYPE_JPEG;
HI_CHAR g_FileName[]="Iframe_picture.jpg";
#endif
#endif
...
HI_S32 save_picture(HI_HANDLE hAvplay, HI_UNF_AVPLAY_I_FRAME_S *pIframe, HI_CHAR* FileName)
{
    ...
    SAMPLE_VIDEO_FREMAE_INFO_S stVideoFrameInfo = {0};
#else
...
#ifdef USE_JPEG_SOFT_ENCODE
    stVideoFrameInfo.Height[0] = stCapPic.u32Height;
    stVideoFrameInfo.Width[0] = stCapPic.u32Width;

    stVideoFrameInfo.YuvFmtType = JPGE_INPUT_FMT_YUV420;
    stVideoFrameInfo.YuvSampleType = JPGE_INPUT_SAMPLER_SEMIPLANNAR;
    stVideoFrameInfo.Stride[0] = stCapPic.stVideoFrameAddr[0].u32YStride;
    stVideoFrameInfo.Stride[1] = stCapPic.stVideoFrameAddr[0].u32CStride;
    stVideoFrameInfo.PhyBuf[0] = stCapPic.stVideoFrameAddr[0].u32YAddr;
    stVideoFrameInfo.PhyBuf[1] = stCapPic.stVideoFrameAddr[0].u32CAddr;

    JPEG_Soft_EncToFile(FileName, &stVideoFrameInfo);
#else
    ...
    if (HIGO_PF_BUTT != MemSurfaceInfo.PixelFormat)
    {
        ...
        EncAttr.ExpectType   = g_ExpectType;
...
HI_S32 update_logo(HI_HANDLE hAvplay, HI_UNF_AVPLAY_I_FRAME_S *pIframe)
{
    ...
    Ret = JPEG_Soft_EncToMem(&stVideoFrameInfo, &pTmpBuf, &LogoSize);
    if (HI_SUCCESS != Ret)
    {
...
HI_S32 main(HI_S32 argc,HI_CHAR** argv)
{
    ...
    else
    {
        ...
        while (1)
        {
            ...
            if ('s' == InputCmd[0])
            {
                Ret = save_picture(hAvplay, &stIFrame, g_FileName);
                if (HI_SUCCESS != Ret)
                {
