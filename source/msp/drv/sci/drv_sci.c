...
#if (defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT))
extern volatile SCI_LDO_REG           *g_PSciLdoReg[HI_UNF_SCI_PORT_BUTT];
extern volatile SCI_LDO_IRQ_REG       *g_PSciLdoIrqReg[HI_UNF_SCI_PORT_BUTT];
#endif
...
#ifdef HI_TEE_SCI_SUPPORT
static HI_UNF_SCI_SECURE_MODE_E g_SecurityMod[HI_UNF_SCI_PORT_BUTT] = {0};
static HI_BOOL g_SciOpneFlag[HI_UNF_SCI_PORT_BUTT] = {0};
#endif
...
HI_VOID SCI_DefaultConfig(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    ***************************/
    ...
    SCI_HAL_EnableDelayTime(enSciPort);
    SCI_HAL_SetSciItime(enSciPort, SCI_DFT_I_TIME);
    SCI_HAL_SetSciCtime(enSciPort, SCI_DFT_C_TIME);
#endif
...
HI_VOID SCI_AtrParamSet(HI_UNF_SCI_PORT_E enSciPort, HI_U8 AtrData)
{
    ...
    case 0xA1:        /*character A1*/
    {
        ...
        if (g_SciPara[enSciPort].SciState.SciArtPara.enClkRate > 0)
        {
            HI_U32 Di;
            HI_U32 Fi;
...
HI_VOID SCI_Init(HI_VOID)
{
...
#if (defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT))
    SCI_LDO_REG        * sciLdoKerAdrr;
    SCI_LDO_IRQ_REG    * sciLdoIrqKerAdrr;
#endif
    ...
    for (i = HI_UNF_SCI_PORT0; i < HI_SCI_PORT_NUM; i++)
    {
        ...
        if (HI_UNF_SCI_PORT0 == i)
        {
#if (defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT))
            sciLdoKerAdrr = (SCI_LDO_REG *)ioremap_nocache(stPortAttr.u32RegBaseAddr + SCI_LDO_OFFSET, sizeof(SCI_LDO_REG));
            if (HI_NULL == sciLdoKerAdrr)
            {
                HI_ERR_SCI("sci Ldo ioremap_nocache err!\n");
                return ;
            }
            sciLdoIrqKerAdrr = (SCI_LDO_IRQ_REG *)ioremap_nocache(stPortAttr.u32RegBaseAddr + SCI_LDO_IRQ_OFFSET, sizeof(SCI_LDO_IRQ_REG));
            if (HI_NULL == sciLdoIrqKerAdrr)
            {
                HI_ERR_SCI("sci LDO Irq ioremap_nocache err!\n");
                return ;
            }
            SCI_HAL_LDO_Init(i, sciLdoKerAdrr);
            SCI_HAL_LDO_IRQ_Init(i, sciLdoIrqKerAdrr);
#endif
        }
    }
...
HI_VOID SCI_DeInit(HI_VOID)
{
    ...
    iounmap(g_PSciLdoReg[HI_UNF_SCI_PORT0]);
    iounmap(g_PSciLdoIrqReg[HI_UNF_SCI_PORT0]);
...
#ifdef HI_TEE_SCI_SUPPORT
HI_S32 SCI_SetSecurityMode(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_SECURE_MODE_E SecurityMode)
{
    HI_S32 Ret;

    if (enSciPort >= HI_UNF_SCI_PORT_BUTT || SecurityMode >= HI_UNF_SCI_SECURE_MODE_BUTT)
    {
        HI_ERR_SCI("enSciPort or SecurityMode is invalid! \n");
        return HI_ERR_SCI_INVALID_PARA;
    }

    Ret = down_interruptible(&g_SciPara[enSciPort].SciSem);
    if (g_SciOpneFlag[enSciPort] == HI_TRUE)
    {
        HI_ERR_SCI("Port %d is opend!\n", enSciPort);
        return HI_ERR_SCI_INVALID_OPT;
    }

    g_SecurityMod[enSciPort] = SecurityMode;
...
HI_S32 SCI_Open(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    ...
    if (HI_UNF_SCI_SECURE_MODE_TEE == g_SecurityMod[enSciPort])
    {
        Ret = SCI_Open_TEE(enSciPort, enSciProtocol, Frequency);
    }
    else
    {
        Ret = SCI_Open_REE(enSciPort, enSciProtocol, Frequency);
    }

    if (HI_SUCCESS == Ret)
    {
        g_SciOpneFlag[enSciPort] = HI_TRUE;
    }
...
#ifdef HI_TEE_SCI_SUPPORT
HI_S32 SCI_Open_TEE(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    ...
    Ret = hisi_secsci_client_open(enSciPort, enSciProtocol, Frequency);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_SCI("TEE_SCI_Open is failed! \n");
        return HI_FAILURE;
    }
    sci_agent_register();
...
HI_S32 SCI_Open_REE(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_PROTOCOL_E enSciProtocol, HI_U32 Frequency)
{
    HI_S32 Ret;
...
HI_S32 SCI_Close(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    if (g_SecurityMod[enSciPort] == HI_UNF_SCI_SECURE_MODE_TEE)
    {
        Ret = SCI_Close_TEE(enSciPort);
    }
    else
   {
        Ret = SCI_Close_REE(enSciPort);
    }
...
#ifdef HI_TEE_SCI_SUPPORT
HI_S32 SCI_Close_TEE(HI_UNF_SCI_PORT_E enSciPort)
{
    HI_S32 Ret;
    ...
    sci_agent_unregister();

    tee_sci_irq_deinit();

    Ret = hisi_secsci_client_close(enSciPort);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_SCI("TEE_SCI_Close is failed! \n");
        return HI_FAILURE;
    }
...
HI_S32 SCI_Close_REE(HI_UNF_SCI_PORT_E enSciPort)
{
    SCI_PORT_ATTR_S stPortAttr;
    static HI_CHAR s8Buff[HI_UNF_SCI_PORT_BUTT][16];
...
HI_VOID SCI_ResetAction(HI_UNF_SCI_PORT_E enSciPort, HI_BOOL bWarmReset)
{
    if (!bWarmReset)
    {
...
{
    HI_S32 Ret;
#if (defined(HI_SCI_VCC_SELECT_AUTO))
    HI_U32 times = 0;
#endif
    CHECK_PORTID();
    ...
    if (SCI_HAL_GetCardPresent(enSciPort))
    {
        ...
        {
#if defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT)
            SCI_HAL_SetLdoenhz(enSciPort, HI_TRUE);
            SCI_HAL_SetLdoVcc(enSciPort, HI_FALSE);
#endif
        }

#if (!defined(HI_SCI_VCC_SELECT_5V))
     SCI_ResetAction(enSciPort, bWarmReset);
#endif
        ...
        if (HI_UNF_SCI_PORT0 == enSciPort)
        {
            ...
            for(times = 0; times < 50; times++)
            {
                ...
                HI_INFO_SCI("SCI Use Vcc 3V reset count %d!\n", times);
            }
            if (g_SciPara[enSciPort].SciState.enSciCrtState >= HI_UNF_SCI_STATUS_READATR)
            {
                up(&g_SciPara[enSciPort].SciSem);
            ...
            SCI_HAL_SetLdoenhz(enSciPort, HI_FALSE);
            msleep(10);
            SCI_HAL_DisableAllInt(enSciPort);
            SCI_HAL_ClearAllInt(enSciPort);
...
#if (defined(HI_SCI_VCC_SELECT_5V) || defined(HI_SCI_VCC_SELECT_AUTO))
            SCI_HAL_SetLdoVcc(enSciPort, HI_TRUE);
            SCI_ResetAction(enSciPort, bWarmReset);
#endif
        }
    }
...
HI_S32 SCI_GetStatus(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_STATUS_E *penState)
{
    ...
    if (g_SecurityMod[enSciPort] == HI_UNF_SCI_SECURE_MODE_TEE)
    {
        Ret = hi_sci_tee_get_card_status_via_irq(penState);
    }
    else
    {
        if (!g_SciPara[enSciPort].bSciEnable)
...
#if (defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT))
    if (HI_UNF_SCI_PORT0 == ii)
    {
        LdoIntState = SCI_HAL_GetLdoIntState(enSciPort);
        while(LdoIntState)
        {
            if ((LdoIntState & SCI_LDO_INT_OCP) || (LdoIntState & SCI_LEO_INT_SCP))
            {
                times ++;
                HI_ERR_SCI("sci%d LDO OCP or SCP occured! \n");
                SCI_HAL_SetLdoenhz(enSciPort, HI_FALSE);
                SCI_HAL_ClearInt(enSciPort, SCI_LDO_INT_OCP);
                SCI_HAL_SetVcc(enSciPort, HI_FALSE);
                if (times < 3)
                {
                    SCI_HAL_SetLdoenhz(enSciPort, HI_TRUE);
                }
