...
HI_S32 HI_MPI_MAILBOX_BeginTransaction(HI_HANDLE handle)
{
    HI_S32 ret = HI_FAILURE;

    MAILBOX_CHECK_HANDLE(handle);

    ret = ioctl(s_mailbox_dev_fd, CMD_MAILBOX_BEGINTRANSACTION, HI_NULL);
    if (HI_SUCCESS != ret)
    {
        MAILBOX_PrintErrorFunc(ioctl, ret);
        return ret;
...
HI_S32 HI_MPI_MAILBOX_EndTransaction(HI_HANDLE handle)
{
    HI_S32 ret = HI_FAILURE;

    MAILBOX_CHECK_HANDLE(handle);

    ret = ioctl(s_mailbox_dev_fd, CMD_MAILBOX_ENDTRANSACTION, HI_NULL);
    if (HI_SUCCESS != ret)
    {
        MAILBOX_PrintErrorFunc(ioctl, ret);
        return ret;
