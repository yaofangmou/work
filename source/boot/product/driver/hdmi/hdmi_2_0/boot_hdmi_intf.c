...
static HI_S32 HdmiScrambleEnableSet(HDMI_DEVICE_S* pstHdmiDev, HI_BOOL bEnable)
{
    ...
    memset(&stCurrentStatus, 0, sizeof(HDMI_SCDC_STATUS_S));
...
HI_S32 HI_DRV_HDMI_Open(HI_UNF_HDMI_ID_E enHdmiID)
{
    ...
    static HI_BOOL s_bIsOpen = HI_FALSE;

    if (HI_FALSE == s_bIsOpen)
    {
        memset(&s_stHdmiDev[enHdmiID], 0, sizeof(HDMI_DEVICE_S));
        s_stHdmiDev[enHdmiID].u32HdmiDevId = enHdmiID;
        s32Ret = DRV_HDMI_Open(&s_stHdmiDev[enHdmiID]);
        if (HI_SUCCESS != s32Ret)
        {
            HDMI_ERR("DRV_HDMI_Open failed!\n");
            return HI_FAILURE;
        }
        s_stHdmiDev[enHdmiID].u64InitTime = get_ticks();
        s_bIsOpen = HI_TRUE;
        HDMI_INFO("Init time = %lluus \n", s_stHdmiDev[enHdmiID].u64InitTime);
    }
    else
    {
        HDMI_INFO("mutil-open.\n");
    }
    return s32Ret;
}
...
HI_S32 HI_DRV_HDMI_GetHotplug(HI_UNF_HDMI_ID_E enHdmiID, HI_BOOL *pbHotplug)
{
    HI_U64        u64CurTime = 0;
    HDMI_DEVICE_S *pstHdmiDev  = HI_NULL;

    CHECK_POINTER(pbHotplug);

    if (enHdmiID >= HI_UNF_HDMI_ID_BUTT)
    {
        HDMI_ERR("Invalid enHdmiID!\n");
        return HI_FAILURE;
    }

    *pbHotplug = HI_FALSE;
    pstHdmiDev = &s_stHdmiDev[enHdmiID];
    HDMI_INFO("Call in time=%lluus\n", get_ticks());

    do
    {
        u64CurTime = get_ticks();
        HDMI_HAL_CALL(HAL_HDMI_HotPlugStatusGet, pstHdmiDev->pstHdmiHal, pbHotplug);
        if (HI_TRUE == *pbHotplug)
        {
            break;
        }
    }while( (u64CurTime - pstHdmiDev->u64InitTime) < 100000 );//100*1000us

    HDMI_INFO("Call out time=%lluus\n", u64CurTime);
    HDMI_INFO("hotplug = %d \n", *pbHotplug);
