...
static HI_S32 HdmiModeStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    else
    {
        ...
        if (pstVoAttr->u32BaseTmdsClk >= HDMI_EDID_MAX_HDMI14_TMDS_RATE)
        {
            HDMI_INFO("BaseTmds=%u,force de-scdc!\n",pstVoAttr->u32BaseTmdsClk);
...
static HI_S32 HdcpStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    if (bTmp)
    {
        ...
        /* edid is invalid and sink is < 2013 donot hdcp22 authentication */
        if (HDMI_EDID_DATA_INVALID == DRV_HDMI_EdidCapabilityGet(&pstHdmiDev->stEdidInfo, &pstSinkCap))
        {
            s32Ret |= HI_FAILURE;
        }
        else if ((pstSinkCap->stMfrsInfo.u32Year < 2013) && (HI_FALSE == pstHdmiDev->stAttr.stAppAttr.bAuthMode))
        {
            s32Ret |= HI_FAILURE;
        }
    }
...
static HI_S32 HdcpStrategy(HDMI_DEVICE_S* pstHdmiDev)
{
    ...
    switch(pstAppAttr->enHDCPMode)
    {
        case HDMI_HDCP_MODE_AUTO:
            ...
            break;
        case HDMI_HDCP_MODE_2_2 :
            HDMI_WARN("Not support HDCP2.2,force to 1.4\n");
            enTmpHdcpMode = HDMI_HDCP_MODE_1_4;
            break;
        case HDMI_HDCP_MODE_1_4 :
            enTmpHdcpMode = HDMI_HDCP_MODE_1_4;
...
{
    HDMI_HARDWARE_STATUS_S   *pstHwStatus = HI_NULL;
    ...
    pstHwStatus = HDMI_VMALLOC(sizeof(HDMI_HARDWARE_STATUS_S));
    CHECK_POINTER(pstHwStatus);

    memset(pstHwStatus, 0, sizeof(HDMI_HARDWARE_STATUS_S));
    HDMI_HAL_CALL(HAL_HDMI_HardwareStatusGet, pstHdmiDev->pstHdmiHal, pstHwStatus);
    /* user attribute construct*/
    pstHdmiDev->enTmdsMode = pstHwStatus->stCommonStatus.enTmdsMode;
    ...
    pstAppAttr->bEnableAudio     = (pstHwStatus->stAudioStatus.bAudioEnable &&
                                   pstHwStatus->stInfoFrameStatus.bAUDIOEnable);
    ...
    bIsDolby = (   (HDMI_HDR_USERMODE_DOLBY == pstHdmiDev->stAttr.stHdrAttr.enUserHdrMode)
                && (   (!pstHwStatus->stVideoStatus.bRGB2YCbCr)
                    && (!pstHwStatus->stVideoStatus.bYCbCr2RGB)
                    && (!pstHwStatus->stVideoStatus.bYCbCr444_422)
                    && (!pstHwStatus->stVideoStatus.bYCbCr422_420)
                    && (!pstHwStatus->stVideoStatus.bYCbCr422_444)
                    && (!pstHwStatus->stVideoStatus.bYCbCr420_422)));

    pstAppAttr->enOutColorSpace = (HI_TRUE == bIsDolby) ? HDMI_COLORSPACE_YCbCr444 : pstHwStatus->stVideoStatus.enOutColorSpace;

    BITDEPTH_2_DEEPCOLOR(pstHwStatus->stVideoStatus.enOutBitDepth, pstAppAttr->enDeepColorMode);

    pstAppAttr->bEnableAviInfoFrame  = pstHwStatus->stInfoFrameStatus.bAVIEnable;
    pstAppAttr->bEnableAudInfoFrame  = pstHwStatus->stInfoFrameStatus.bAUDIOEnable;
    pstAppAttr->bxvYCCMode          = pstHwStatus->stInfoFrameStatus.bGBDEnable;
    /* video attribute construct*/
    pstVideoAttr->enInColorSpace = pstAppAttr->enOutColorSpace;
    pstVideoAttr->bVSyncPol      = pstHwStatus->stVideoStatus.bVSyncPol;
    pstVideoAttr->bHSyncPol      = pstHwStatus->stVideoStatus.bHSyncPol;
    pstVideoAttr->bDEPol         = pstHwStatus->stVideoStatus.bDEPol;
    pstVideoAttr->enInColorSpace = pstHwStatus->stVideoStatus.enInColorSpace;

    if (pstHwStatus->stInfoFrameStatus.bAVIEnable)
    {
        HDMI_VIDEO_CODE_E  enVideoCode  = pstHwStatus->stInfoFrameStatus.u8AVI[7];
        ...
        bAspectIs256 = (   ((enVideoCode == 0) && (pstHwStatus->stInfoFrameStatus.u8VSIF[8] == 4))
                        || ((enVideoCode >= HDMI_4096X2160P25_256_135) && (enVideoCode <= HDMI_4096X2160P60_256_135)));

        pstVideoAttr->enPictureAspect = (HI_TRUE == bAspectIs256)? HDMI_PICTURE_ASPECT_256_135 : ((pstHwStatus->stInfoFrameStatus.u8AVI[5] >> 4) & 0x3);

        pstVideoAttr->enActiveAspect    = pstHwStatus->stInfoFrameStatus.u8AVI[5] & 0xf;
        pstVideoAttr->enColorimetry     = (pstHwStatus->stInfoFrameStatus.u8AVI[5] >> 6) & 0x3;
        pstVideoAttr->enRGBQuantization = (pstHwStatus->stInfoFrameStatus.u8AVI[6] >> 2) & 0x3;
        pstVideoAttr->enYCCQuantization = (pstHwStatus->stInfoFrameStatus.u8AVI[8] >> 6) & 0x3;
        pstVideoAttr->u32PixelRepeat    = (pstHwStatus->stInfoFrameStatus.u8AVI[8] & 0xf) + 1;
        pstVideoAttr->enExtendedColorimetry = (pstHwStatus->stInfoFrameStatus.u8AVI[6] >> 4) & 0x07;
        ...
        if ((!pstHwStatus->stInfoFrameStatus.bVSIFEnable) && (0 == enVideoCode))
        {
            pstVideoAttr->enVideoTiming = HDMI_VIDEO_TIMING_UNKNOWN;
        }
    }
    pstVideoAttr->enStereoMode = HDMI_3D_BUTT;
    if (pstHwStatus->stInfoFrameStatus.bVSIFEnable)
    {
        enum hdmi_video_format format = (pstHwStatus->stInfoFrameStatus.u8VSIF[7] >> 5) & 0x3;
        if (format == HDMI_VIDEO_FORMAT_4K)
        {
            HDMI_VSIF_VIC_E enVideoCode = pstHwStatus->stInfoFrameStatus.u8VSIF[8];
        ...
        {
            pstVideoAttr->enStereoMode    = (pstHwStatus->stInfoFrameStatus.u8VSIF[8] >> 4) & 0xf;
        }
    }

    /* audio attrbute construct*/
    pstAudioAttr->bDownSample   = pstHwStatus->stAudioStatus.bDownSample;
    pstAudioAttr->enChanels     = pstHwStatus->stAudioStatus.enLayout;
    pstAudioAttr->enSampleDepth = pstHwStatus->stAudioStatus.enSampleDepth;
    pstAudioAttr->enSampleFs    = pstHwStatus->stAudioStatus.enSampleFs;
    pstAudioAttr->enSoundIntf   = pstHwStatus->stAudioStatus.enSoundIntf;
    if (pstHwStatus->stInfoFrameStatus.bAUDIOEnable)
    {
        pstAudioAttr->enAudioCode = (pstHwStatus->stInfoFrameStatus.u8AUDIO[4] >> 4) & 0xf;
    }

    if (pstHwStatus)
    {
        HDMI_VFREE(pstHwStatus);
    }
    return HI_SUCCESS;
}
...
#endif
#ifdef HDMI_HDR_SUPPORT
    DRV_HDMI_CompatHdrDebugModeGet(pstHdmiDev->u32HdmiDevId,&pstHdmiDev->stHdrInfo.enHdrDebugMode);
#endif
    HDMI_INFO("<<< out...\n");
...
{
    HDMI_VIDEO_TIMING_E enTiming = HDMI_VIDEO_TIMING_1280X720P_50000;

    if (pstHdmiDev && pstHdmiDev->pstHdmiHal && pstHdmiDev->pstHdmiHal->HAL_HDMI_DispFmt2HdmiTiming)
    {
        enTiming = (HDMI_VIDEO_TIMING_E)HDMI_HAL_CALL_RET(HAL_HDMI_DispFmt2HdmiTiming, pstHdmiDev->pstHdmiHal, u32DispFmt);
    }
...
static HI_S32 HdmiDeviceInit(HDMI_DEVICE_S * pstHdmiDev)
{
...
#endif
    /* make sure init bResumeEnable is true */
    pstHdmiDev->bResumeEnable       = HI_TRUE;
    /* connon para init */
    ...
    if (pstHdmiDev->pstHdmiHal && pstHdmiDev->pstHdmiHal->HAL_HDMI_DispFmtGet)
    {
        ...
        pstVideoAttr->u32BaseTmdsClk= stBaseParam.u32PixClk;

        if(HDMI_COLORSPACE_YCbCr420 == stBaseParam.enColorSpace)
        {
            HDMI_MULTIPLE_0P5(pstVideoAttr->u32BaseTmdsClk);
        }
        if(HDMI_DEEP_COLOR_30BIT == stBaseParam.enDeepColor)
        {
            HDMI_MULTIPLE_1P25(pstVideoAttr->u32BaseTmdsClk);
        }
        if(HDMI_DEEP_COLOR_36BIT == stBaseParam.enDeepColor)
        {
            HDMI_MULTIPLE_1P5(pstVideoAttr->u32BaseTmdsClk);
        }
    ...
    pstHdmiDev->stAudioEn.bAoAppEnable   = HI_TRUE;
    pstHdmiDev->stAudioEn.bAoIfAppEanble = HI_TRUE;
    pstHdmiDev->stAudioEn.bAoDrvEnable   = HI_TRUE;
    pstHdmiDev->stAudioEn.bAoSelEanble   = HI_TRUE;
...
#ifdef HDMI_RESUME_FAST_KPI
    HDMI_HAL_CALL(HAL_HDMI_HotPlugStatusGet, pstHdmiDev->pstHdmiHal, &bHotPLug);
    HDMI_INFO("no wait HPD when reset!bHotPLug=%d\n",bHotPLug);
#else
{
    HI_U32 i = 0;
    for (i = 0,bHotPLug = HI_FALSE; i < 110; i++)
    {
         HDMI_HAL_CALL(HAL_HDMI_HotPlugStatusGet, pstHdmiDev->pstHdmiHal, &bHotPLug);
         if(bHotPLug)
         {
    ...
    HDMI_INFO("HPD=%d,wait=%d ms!",bHotPLug,i);
}
#endif
    if (bHotPLug)
    {
...
#ifdef HDMI_HDR_SUPPORT
    /* if the current HDR mode is DolbyVision, not support deepcolor. */
                   ...
                   HDMI_EXTENDED_COLORIMETRY *penSdrExtColorimetryBack, HDMI_HDR_MODE_E enHdrModeBack)
{
    ...
    HDMI_APP_ATTR_S         *pstAppAttr      = HI_NULL;
    HDMI_HDR_INFO_S         *pstHdrInfo      = HI_NULL;
    HI_BOOL                  bHdrModeChange  = HI_TRUE;
    ...
    if (  (HDMI_HDR_MODE_CEA_861_3 == enHdrModeBack)
        &&(HDMI_HDR_MODE_CEA_861_3 == pstAppHdrAttr->stHdrAttr.enHdrMode))
    {
        HDMI_INFO("HDR mode not change, old(%d)->new(%d)\n", enHdrModeBack, pstAppHdrAttr->stHdrAttr.enHdrMode);
        bHdrModeChange = HI_FALSE;
    }
    if( (HDMI_HDR_DEBUG_MODE_NONE != pstHdrInfo->enHdrDebugMode) && (HI_TRUE == bHdrModeChange))
    {
...
#if defined(HDMI_PHY_DIFF_MODE) || defined(HDMI_SUPPORT_LOGIC_SILICON) || defined(CHIP_TYPE_hi3716mv430)
...
#if defined(CHIP_TYPE_hi3796mv200) || defined(CHIP_TYPE_hi3798mv200) || defined(CHIP_TYPE_hi3798mv300) || defined(CHIP_TYPE_hi3716mv450) || defined(CHIP_TYPE_hi3798mv310) || defined(CHIP_TYPE_hi3716mv430)
...
static HI_S32 HdmiFileOpen(struct inode *inode, struct file *filp)
{
    ...
    (HI_VOID)atomic_inc_return(&s_HdmiCount);
    ...
    switch (cmd)
    {
        ...
        case CMD_HDMI_UPDATE_EDID:
        {
            ...
            else
            {
                s32Ret = DRV_HDMI_EdidUpdate(&pstHdmiDev->stEdidInfo, HDMI_EDID_UPDATE_SINK);
#ifdef HDMI_HDCP_SUPPORT
                if ( (pstHdmiDev->stEdidInfo.stCapability.stMfrsInfo.u32Year >= 2013)
                    || (HI_TRUE == pstHdmiDev->stAttr.stAppAttr.bAuthMode) )
                {
                    HDMI_HAL_CALL(HAL_HDMI_HdcpSinkCapabilityGet, pstHdmiDev->pstHdmiHal, &pstHdmiDev->stHdcpInfo.stHdcpCap);
                }
