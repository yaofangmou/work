...
static HI_S32 channel_vdec_report_process(OMXVDEC_CHAN_CTX *pchan, HI_U32 EventID, HI_U32 *pstatus, HI_VOID *PrivData)
{
    ...
    switch (EventID)
    {
        ...
        case VDEC_EVT_REPORT_IMG_SIZE_CHG:
        {
            ...
            channel_report_message(pchan, VDEC_EVT_REPORT_IMG_SIZE_CHG, &ImgSize);
...
HI_S32 channel_global_release_frame(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    pstImage = kzalloc(sizeof(IMAGE), GFP_KERNEL);
    if (HI_NULL == pstImage)
    {
        OmxPrint(OMX_FATAL, "%s call kzalloc for pstImage failed.\n", __func__);
        return HI_FAILURE;
    }

    ret = processor_convert_frame_to_image(&stFrame, pstImage);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_ERR, "%s processor_convert_frame_to_image failed\n", __func__);
        goto err;
    }

    ret = pchan->image_ops.global_release_image(pstImage);
    if (ret != HI_SUCCESS)
    {
        OmxPrint(OMX_ERR, "%s global release frame(0x%x) failed!\n", __func__,stFrame.stBufAddr[0].u32PhyAddr_YHead);
        goto err;
    }
