...
HI_S32 HI_UNF_SCI_SetSecurityMode(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_SECURE_MODE_E SecurityMode)
{
    ...
    HI_S32 Ret;
    SCI_SECURITY_MODE_S SciSecurityMode;
    ...
    if (SecurityMode >= HI_UNF_SCI_SECURE_MODE_BUTT)
    {
        HI_ERR_SCI("para SecurityMode is invalid.\n");
        return HI_ERR_SCI_INVALID_PARA;
    }

    SciSecurityMode.enSciPort = enSciPort;
    SciSecurityMode.SecurityMode = SecurityMode;

    Ret = ioctl(g_SciDevFd, CMD_SCI_SET_SECURITY_MODE, &SciSecurityMode);

    return Ret;
#else

    HI_ERR_SCI("Current mode is not support TEE SCI!\n");
