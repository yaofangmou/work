...
echo -e "rootbox p1:$1 p2:$2  p3:$3 p4:$4  "
...
if [ $mode == "DEBUG" ]; then
    ...
    file_to_del+=" $rootbox_dir/usr/bin/lxc-freeze "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-unfreeze "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-autostart "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-copy "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-info "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-top "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-checkconfig "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-destroy "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-monitor "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-unshare "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-create "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-cgroup "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-checkpoint "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-device "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-snapshot "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-usernsexec "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-config "
    file_to_del+=" $rootbox_dir/usr/bin/lxc-wait "
...
if [ $strip_status == "STRIP_OPEN" ]; then
...
if [ -x $etc_dir/lxc_create_lxc2.sh ];then
   $etc_dir/lxc_create_lxc2.sh $mode $rootbox_dir lxc2 $toolchain_name $etc_dir
fi
if [ -x $etc_dir/lxc_create_lxc1.sh ];then
   $etc_dir/lxc_create_lxc1.sh $mode $rootbox_dir lxc1 $toolchain_name $etc_dir
fi
if [ -x $etc_dir/lxc_create_lxc0.sh -a "ss$lxc_enable_net" = "ssyes" ];then
   $etc_dir/lxc_create_lxc0.sh $mode $rootbox_dir lxc0 $toolchain_name $etc_dir
fi
