                    ...
                    HI_UNF_CIPHER_SUBSAMPLE_S *pstSubSample, HI_U32 u32SubsampleNum)
{
    ...
    printBuffer("CENC-ORI:", enc, u32TestDataLen);

    memset(pOutputAddrVir, 0x0, u32TestDataLen);
    HI_PRINT("u32TestDataLen 0x%x\n", u32TestDataLen);

    s32Ret = HI_UNF_CIPHER_CENCDecrypt(hTestchnid, key, iv, u32InputAddrPhy,
    ...
    printBuffer("CENC-DEC:", pOutputAddrVir, u32TestDataLen);
...
/* encrypt data using special chn*/
HI_S32 sample_cenc_ex(HI_U8 *key, HI_U8 *iv, HI_U8 *enc, HI_U8 *dec, HI_UNF_CIPHER_WORK_MODE_E mode,
                    HI_U32 u32TestDataLen, HI_U32 u32FirstEncryptOffset,
...
HI_S32 main(int argc, char* argv[])
{
    ...
    u32Conut = 0;
    for (id=0; id < sizeof(cenc_ex_sample_data)/sizeof(cenc_ex_sample_data[0]); id++)
    {
        s32Ret = sample_cenc_ex(cenc_ex_sample_data[id].key, cenc_ex_sample_data[id].iv,
            cenc_ex_sample_data[id].enc, cenc_ex_sample_data[id].dec,
            cenc_ex_sample_data[id].mode, cenc_ex_sample_data[id].length,
            cenc_ex_sample_data[id].u32FirstEncryptOffset,
            cenc_ex_sample_data[id].subsample, cenc_ex_sample_data[id].subsampleNum);
        HI_PRINT("*************** sample %d test end ****************\n", id);
    ...
    HI_PRINT("\n**********************************************************\n");
    HI_PRINT("             CENC_EX Test End, Success %d, Faile %d\n", u32Conut, id - u32Conut);
    HI_PRINT("\n**********************************************************\n");
