                                    ...
                                    HI_DRV_PIX_FORMAT_E ePixFormat)
{
    ...
    {
        printk("--format:%d, nv21:%d,12cmp:%d,21cmp:%d!\n", ePixFormat,
                                 HI_DRV_PIX_FMT_NV21,
                                 HI_DRV_PIX_FMT_NV12_TILE_CMP,
                                 HI_DRV_PIX_FMT_NV21_TILE_CMP);

        datalen = u32height * y_stride * 3 / 2;
    }
...
#define VDP_DEBUG_STR_MAX   256
static HI_CHAR u8VdpDebugStr[VDP_DEBUG_STR_MAX];
...
HI_S32 vdp_DebugSaveYUVImg(HI_DRV_VIDEO_FRAME_S *pstCurFrame, HI_CHAR *buffer, HI_U32 count)
{
    ...
    for(; (i < count) && (j < VDP_DEBUG_STR_MAX - 1); i++)
    {
        if ((0 == j) && (' ' == buffer[i]))
        {
            continue;
    ...
    // open file in kernel
    HI_OSAL_Snprintf(u8VdpDebugStr, VDP_DEBUG_STR_MAX, "%s/vdp_0x%x_%d_%d_%02u_%02u_%02u.yuv", s_VdpSavePath,
...
HI_U32 g_u32DfxFunctionComposer = 0;
struct file *g_pstFile = HI_NULL;

HI_VOID Disp_SetDebugFlag(HI_U32 u32mask,HI_BOOL bClear)
{
    if (HI_FALSE == bClear)
    {
        g_u32DfxFunctionComposer |= u32mask;
    }
    else
    {
        g_u32DfxFunctionComposer = 0;
    }
...
{
    g_pstFile = vdp_k_fopen(pDebugFileName, O_RDWR|O_CREAT, 0);

    if (HI_NULL == g_pstFile)
    {
        DISP_ERROR("---create debug name:%s!\n", pDebugFileName);
        return HI_FAILURE;
    }
...
#if 0
HI_VOID DISP_PRINT_FUNC(const HI_CHAR *pFunctionName, HI_U32 u32LineNumber, HI_CHAR *pFmt)
{
    HI_U8 u8printk[128] = {0};

    if (HI_NULL != g_pstFile)
    {
        snprintf(u8printk, sizeof(u8printk), "%s:%d, %s!\n", pFunctionName, u32LineNumber, pFmt);
        vdp_k_fwrite(u8printk, strlen(u8printk), g_pstFile);
    }
...
HI_VOID DISP_PRINT_FUNC(const HI_CHAR *pFunctionName, HI_U32 u32LineNumber, const HI_CHAR *format, ...)
{
    va_list args;
    HI_U32  MsgLen = 0;
    char    log_str[LOG_MIN]={'a'};
    HI_U8   u8printk[LOG_MAX] = {0};

    log_str[LOG_MIN-1] = 'b';
    log_str[LOG_MIN-2] = 'c';

    va_start(args, format);
    MsgLen = HI_OSAL_Vsnprintf(log_str, LOG_MIN, format, args);
    va_end(args);

    if (MsgLen >= LOG_MIN)
    {
        log_str[LOG_MIN-1] = '\0';  /* even the 'vsnprintf' commond will do it */
        log_str[LOG_MIN-2] = '\n';
        log_str[LOG_MIN-3] = '.';
        log_str[LOG_MIN-4] = '.';
        log_str[LOG_MIN-5] = '.';
    ...
    if (HI_NULL != g_pstFile)
    {
        snprintf(u8printk, sizeof(u8printk), "%s:%d,%s!\n", pFunctionName, u32LineNumber, log_str);
        vdp_k_fwrite(u8printk, strlen(u8printk), g_pstFile);
    }
