                              ...
                              HI_VOID *pUserData)
{
    ...
    HI_U32 u32Origin;
    static HI_U32 s_u32Count = 0;
    HI_U32 u32TotalWriteLen = 0, u32CurWriteLen = u32DataSize;
    ...
    do
    {
        if (u32CurWriteLen > 846*1024)
        {
            u32CurWriteLen = 846*1024;
        }
        memset(&stCopyOperation, 0x00, sizeof(stCopyOperation));
        stCopyOperation.started = 1;
        stCopyOperation.params[1].value.a = pstDstBuf->u32PhyAddr + u32TotalWriteLen;
        stCopyOperation.params[1].value.b = 0xA5A5A5A5;
        stCopyOperation.params[2].value.a = pstSrcBuf->u32PhyAddr + u32TotalWriteLen;
        stCopyOperation.params[2].value.b = u32CurWriteLen;
        stCopyOperation.paramTypes = TEEC_PARAM_TYPES(TEEC_VALUE_INOUT,
                                                TEEC_VALUE_INOUT,
                                                TEEC_VALUE_INOUT,
                                                TEEC_NONE);
        ...
        u32TotalWriteLen += u32CurWriteLen;
        u32CurWriteLen = u32DataSize - u32TotalWriteLen;
        if (u32TotalWriteLen >= u32DataSize)
            break;
    }while(1);
