...
static void drv_symc_enable_secure(void)
{
    ...
    HI_LOG_DEBUG("REG_SEC_CHN_CFG 0x%x\n", sec.u32);
...
HI_S32 drv_cipher_aes_test(HI_VOID)
{
    ...
    ret = crypto_mem_create(&mem, SEC_MMZ, "CIPHER_TEST", AES_BLOCK_SIZE);
    if (HI_SUCCESS != ret)
    {
...
s32 drv_symc_init(void)
{
    ...
    ret = crypto_mem_create(&symc_dma, SEC_MMZ, "CIPHER_NODE", SYMC_NODE_LIST_SIZE);
    if (HI_SUCCESS != ret)
    {
...
void drv_symc_proc_status(symc_chn_status *status, symc_module_status *module)
{
    ...
    U_INT_RAW        stIntRaw;
    U_INT_EN         stIntEn;
    U_INT_STATUS     stIntStatus;
    U_SEC_CHN_CFG    stSec;
    U_CHANn_RD_DAT_ADDR_SMMU_BYPASS stInSmmuEn;
    U_CHANn_WR_DAT_ADDR_SMMU_BYPASS stOutSmmuEn;
    ...
    sec_chn_cfg.u32 = SYMC_READ(REG_SEC_CHN_CFG);
    ...
    module->name        = name;
#ifdef CRYPTO_SMMU_SUPPORT
    module->smmu_base   = SMMU_READ(module->sec_cpu ? REG_MMU_SCB_TTBR : REG_MMU_CB_TTBR);
#endif
    module->err_code    = 0x00;
    module->int_en      = (module->sec_cpu ? stIntEn.bits.sec_int_en : stIntEn.bits.int_en);
    ...
    for (i = 1; i < CRYPTO_HARD_CHANNEL_MAX; i++)
    {
        ...
        status[i].open = symc_hard_channel[i].open;

        /* ctrl */
        ctrl.u32 = SYMC_READ(REG_CHANn_CIPHER_CTRL(i));
        status[i].decrypt = ctrl.bits.decrypt;
        status[i].alg  = ctrl.bits.alg_sel;
        status[i].mode = ctrl.bits.mode;
        status[i].klen = ctrl.bits.key_length;
        status[i].ksrc = ctrl.bits.key_sel;
        ...
        /* secure channel */
        stSec.u32 = SYMC_READ(REG_SEC_CHN_CFG);
        status[i].secure = (stSec.bits.sec_chn_cfg >> i) & 0x1;

        /* interrupt */
        stIntEn.u32     = SYMC_READ(REG_INT_EN);
        stIntRaw.u32    = SYMC_READ(REG_INT_RAW);
        stIntStatus.u32 = SYMC_READ(REG_INT_STATUS);
        status[i].int_en     = (stIntEn.bits.chn_obuf_en >> i) & 0x1;
        status[i].int_raw    = (stIntRaw.bits.chn_obuf_raw >> i) & 0x1;
        status[i].int_status = (stIntStatus.bits.chn_obuf_int >> i) & 0x1;

        /* in node */
        status[i].in_node_head  = SYMC_READ(REG_CHANn_SRC_LST_SADDR(i));
        status[i].in_node_depth = SYMC_READ(REG_CHANn_IBUF_NUM(i));
        status[i].in_node_rptr  = SYMC_READ(REG_CHANn_SRC_LST_RADDR(i));
        status[i].in_node_wptr  = SYMC_READ(REG_CHANn_SRC_LST_RADDR(i));
        status[i].inaddr        = SYMC_READ(CHANn_SRC_ADDR(i));

        /* out node */
        status[i].out_node_head  = SYMC_READ(REG_CHANn_DEST_LST_SADDR(i));
        status[i].out_node_depth = SYMC_READ(REG_CHANn_OBUF_NUM(i));
        status[i].out_node_rptr  = SYMC_READ(REG_CHANn_DEST_LST_RADDR(i));
        status[i].out_node_wptr  = SYMC_READ(REG_CHANn_DEST_LST_RADDR(i));
        status[i].outaddr        = SYMC_READ(REG_CHANn_DEST_ADDR(i));

        /* SMMU Enable */
        stInSmmuEn.u32  = SYMC_READ(CHANn_RD_DAT_ADDR_SMMU_BYPASS);
        stOutSmmuEn.u32 = SYMC_READ(CHANn_WR_DAT_ADDR_SMMU_BYPASS);
        status[i].smmu_enable  = !((stInSmmuEn.bits.chann_rd_dat_addr_smmu_bypass  >> (i - 1)) & 0x1);
        status[i].smmu_enable |= !((stOutSmmuEn.bits.chann_wr_dat_addr_smmu_bypass  >> (i - 1)) & 0x1);
    }
}
#endif
/******* proc function end ********/
