...
HI_S32 hdmi_Resume(PM_BASEDEV_S *pdev)
{
    ...
    HDMI_HDCPKEY_ATTR_S * pstFlashEncrytedHdcpKey  = DRV_Get_HdcpKeyInfo(HI_UNF_HDMI_ID_0);
#endif
    COM_INFO(">>>in...\n");
    ...
    suspend_flag = 0;
#if defined (HDCP_SUPPORT)
    if( HI_TRUE == pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].KeyLoadStat )
    {
        ret = HI_DRV_MODULE_GetFunction(HI_ID_CIPHER, (HI_VOID**)&g_stCIPHERExportFunctionLists);
        if((NULL == g_stCIPHERExportFunctionLists) || (ret != HI_SUCCESS))
        {
            COM_ERR("Get cipher functions failed!\n");
            return HI_FAILURE;
        }
        (g_stCIPHERExportFunctionLists->pfnCipherLoadHdcpKey)(&pstFlashEncrytedHdcpKey[HI_UNF_HDMI_ID_0].stFlashEncrytedHdcpKey);
    }
#endif
    COM_INFO("<<<out...\n");
                           ...
                           unsigned int cmd, HI_VOID *arg)
{
    ...
    if(0 != s32Ret)
    {
        COM_ERR(" acquire g_hdmiMutex fail ! \n ");
        return HI_FAILURE;
    }
...
{
    COM_INFO(" set audio mute  \n ");
    SI_SetHdmiAudioMute(HI_TRUE);
    return HI_SUCCESS;
}

HI_S32 HI_DRV_HDMI_SetAudioUnMute(HI_UNF_HDMI_ID_E enHdmi)
{
    COM_INFO(" set audio nmute  \n ");
    SI_SetHdmiAudioMute(HI_FALSE);
    return HI_SUCCESS;
}
