...
HI_S32 HI_DRV_VDEC_AllocAndMap(const char *bufname, char *zone_name, HI_U32 size, int align, VDEC_BUFFER_S *psMBuf)
{
    ...
    {
        HI_FATAL_BUFMNG("Alloc %s failed! size:%d\n", bufname, size);
    }
...
HI_VOID HI_DRV_VDEC_Release_TVP(VDEC_BUFFER_S *psMBuf)
{
    ...
    stMMZBuf.u32StartPhyAddr = psMBuf->u32StartPhyAddr;
    stMMZBuf.u32Size         = psMBuf->u32Size;
    s32Ret = HI_DRV_SECMMZ_Release(&stMMZBuf);
#else
    stSMMUBuf.pu8StartVirAddr  = psMBuf->pu8StartVirAddr;
    stSMMUBuf.u32StartSmmuAddr = psMBuf->u32StartPhyAddr;
    stSMMUBuf.u32Size          = psMBuf->u32Size;
    s32Ret = HI_DRV_SECSMMU_Release(&stSMMUBuf);
#endif

    if (s32Ret != HI_SUCCESS)
    {
        HI_FATAL_BUFMNG("Release Phy:0x%x failed! size:%d\n", psMBuf->u32StartPhyAddr, psMBuf->u32Size);
    }
}
#endif
...
HI_S32 HI_DRV_VDEC_Map(VDEC_BUFFER_S *psMBuf)
{
    ...
    BUFMNG_ASSERT_RETURN(psMBuf != HI_NULL, HI_FAILURE);
    BUFMNG_ASSERT_RETURN(psMBuf->u32StartPhyAddr != 0, HI_FAILURE);
    BUFMNG_ASSERT_RETURN(psMBuf->u32Size != 0, HI_FAILURE);
...
static HI_VOID ConvertYPIXForNV21TileCmpOff(HI_DRV_VIDEO_FRAME_S *pstFrame, IMAGE *pImage, HI_U8 *Y_Addr, VDEC_BUFFER_S *pstFrmBuf)
{
    HI_U32 i = 0, j = 0, k = 0;
    HI_U8 *dst, *src;
    HI_U32 Stride = 0;
    ...
    HI_U8 *Yaddress_2b;
    HI_U8 TileHeight_2b;
    HI_U32 DecBitDepth = pImage->stDecInfo.bit_depth_luma;

    Stride = pstFrame->stBufAddr[0].u32Stride_Y * 16;
    SlotWidth = (pstFrame->u32Width + (256 - 1)) & (~(256 - 1));
    TileHeight_2b = 32;
    Stride_2b = SlotWidth / 4 * TileHeight_2b;
    Yaddress_2b = pstFrmBuf->pu8StartVirAddr + (pImage->stDispInfo.luma_phy_addr_2bit - pImage->stDispInfo.luma_phy_addr);
        ...
        for (j = 0; j < pstFrame->u32Width; j += 64)
        {
            if (DecBitDepth == 10)
            {
                ...
                else
                {
                    for (k = 0; k < 64; k++)
                    {
                        u8b[k] = (HI_U8)Clip1((u10b[k] + (1 << 1)) >> 2);
                    }
                    src = (HI_U8 *)u8b;
...
static HI_VOID ConvertUVPIXForNV21TileCmpOff(HI_DRV_VIDEO_FRAME_S *pstFrame, IMAGE *pImage, HI_U8 *C_Addr, VDEC_BUFFER_S *pstFrmBuf)
{
    HI_U32 i, j, k;
    HI_U8 *dst, *src, *tmp;
    HI_U32 Stride = 0;
    ...
    Caddress_2b = pstFrmBuf->pu8StartVirAddr + (pImage->stDispInfo.chrom_phy_addr_2bit - pImage->stDispInfo.luma_phy_addr);
                                     ...
                                     mm_segment_t oldfs)
{
    ...
    ConvertYPIXForNV21TileCmpOff(pstFrame, pImage, Y_Addr, pstFrmBuf);
    ...
    ConvertUVPIXForNV21TileCmpOff(pstFrame, pImage, C_Addr, pstFrmBuf);
...
HI_S32 BUFMNG_SaveYuv(HI_S32 Handle, HI_UNF_VCODEC_TYPE_E enType, HI_VOID *pSrcFrame, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    HI_S32 len = 0;
    ...
    else
    {
        ...
        {
            SaveYUVNV21TileCmpOff(pstFrame, (IMAGE *)pSrcFrame, Y_Addr, C_Addr, &stFrmBuf, oldfs);
        }
