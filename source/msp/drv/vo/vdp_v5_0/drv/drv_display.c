...
extern HI_VOID DISP_XDR_IsrProcess(HI_VOID* hHandle);
...
{                                \
    ...
    {                            \
        DISP_ERROR("DISP not init sta:%d!\n",s_s32DisplayGlobalFlag); \
...
{                              \
    if(!DISP_IsOpened(id))     \
    {                          \
        DISP_ERROR("Disp not open!\n"); \
        return HI_ERR_DISP_NOT_OPEN;  \
    }                          \
}

DISP_S *Disp_GetDiscriptPtr(HI_DRV_DISPLAY_E eDisp)
{
    if (eDisp >= HI_DRV_DISPLAY_2)
    {
        DISP_ERROR("error dispid!\n");
        return HI_NULL;
    }
    return  &s_stDisplayDevice.stDisp[eDisp - HI_DRV_DISPLAY_0];
}
...
HI_VOID DispParserInitParam(DISP_S* pstDisp, HI_DRV_DISP_SETTING_S* pstSetting)
{
    ...
    pstS->stOffsetCfgInfo = pstSetting->stOffsetInfo;
...
#ifdef HI_DISP_BUILD_FULL
    if (osal_request_irq(112, (irq_handler_t)DISP_ISR_Main, IRQF_SHARED, "vdp", &g_DispIrqHandle) != 0)
    {
...
HI_VOID DISP_CB_PreProcess(HI_VOID* hHandle, const HI_DRV_DISP_CALLBACK_INFO_S* pstInfo)
{
    ...
    u32OffRdErr=(pVdpReg->OFFLINE_STATE0.bits.offline_err_state)&0x1;

    if(u32OffRdErr == 1)
    {
        pstDisp->u32VfRerrCnt++;
        pVdpReg->ON_OFFLINE_BUF_RESET.bits.state_clr  = 1;
    }
    ...
    switch (pstDisp->eState)
    {
        ...
        case DISP_PRIV_STATE_ENABLE:
            if (!pstDisp->bEnable)
            {
                VDP_DISP_GetDhdEven(0, &u32dhd_even);
                if (u32dhd_even != 0)
                {
                    return;
                }
...
HI_S32 DISP_SetVirtScreen(HI_DRV_DISPLAY_E enDisp, HI_RECT_S virtscreen)
{
    ...
    else if (HI_TRUE == pstDisp->bIsSlave)
    {
        ...
        if (pstDisp->enAttachedDisp >= HI_DRV_DISPLAY_2)
        {
            pstDisp->bDispSettingChange = HI_FALSE;
            pstDisp->stSetting.stVirtaulScreen.s32Width = virtscreen.s32Width & 0xFFFFFFFEul;
            pstDisp->stSetting.stVirtaulScreen.s32Height = virtscreen.s32Height & 0xFFFFFFFCul;
...
HI_S32 DISP_SetScreenOffset(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_OFFSET_S *pstScreenOffset)
{
    ...
    pstDisp->bDispSettingChange = HI_TRUE;
    pstDisp->stSetting.stOffsetCfgInfo = *pstScreenOffset;
     ...
     if (pstDisp->bIsMaster)
    {
        ...
        pSlivestDisp->stSetting.stOffsetCfgInfo = *pstScreenOffset;
...
HI_S32 DISP_GetScreenOffset(HI_DRV_DISPLAY_E enDisp, HI_DRV_DISP_OFFSET_S* pstScreenOffset)
{
    ...
    *pstScreenOffset = pstDisp->stSetting.stOffsetCfgInfo;
    return HI_SUCCESS;
}
...
HI_S32 DISP_GetProcInto(HI_DRV_DISPLAY_E enDisp, DISP_PROC_INFO_S* pstInfo)
{
    ...
    pstInfo->u32VfRerrCnt = pstDisp->u32VfRerrCnt;
