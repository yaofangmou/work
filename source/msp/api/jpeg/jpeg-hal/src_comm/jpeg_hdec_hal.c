...
HI_VOID* JPEG_DEC_CreateDecompress()
{
    ...
    if (NULL == pJpegHandle)
    {
        HI_GFX_Print("++++++++HI_JPEG_CreatHdecHandle failure\n");
        return NULL;
    }
    ...
    JPEG_HDEC_WhetherDebug(&pJpegHandle->LogLevel);
...
static inline HI_VOID JPEG_DEC_ReleaseRes(HI_ULONG HdecHandle)
{
    ...
    JPEG_DEC_SaveYuvSp(pJpegHandle);
...
* description  : 保存被调用的函数\n
* param[in]    : HdecHandle  CNcomment:    解码器句柄 CNend\n
* param[in]    : pFunc       CNcomment:    被调用的函数 CNend\n
* retval       : NA
* retval       : NA
* others:      : NA
*****************************************************************************/
HI_VOID JPEG_DEC_SaveCallFunc(HI_ULONG HdecHandle, const HI_CHAR* pFunc, HI_U32 Line)
{
    ...
    JPEG_HDEC_HANDLE_S_PTR pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)HdecHandle;
    if ((NULL == pJpegHandle) || (1 != pJpegHandle->LogLevel) || (pJpegHandle->CallFuncNum >= CONFIG_JPEG_CALL_FUNC_NUM))
    {
        return;
    }

    for (CallFuncIndex = 0; CallFuncIndex < pJpegHandle->CallFuncNum; CallFuncIndex++)
    {
        if (0 == strncmp(pJpegHandle->CallFunc[CallFuncIndex], pFunc, CONFIG_JPEG_CALL_FUNC_LEN))
        {
            return;
        }
    }

    HI_GFX_Snprintf(pJpegHandle->CallFunc[pJpegHandle->CallFuncNum], CONFIG_JPEG_CALL_FUNC_LEN, "%s", pFunc);
    pJpegHandle->CallFuncLine[pJpegHandle->CallFuncNum] = Line;
...
HI_VOID JPEG_DEC_SaveSoftDecTimes(HI_ULONG HdecHandle)
{
    ...
    if (NULL != pJpegHandle)
    {
        PreTime = pJpegHandle->stProcInfo.SoftDectTimes;
        JPEG_HDEC_GetTimeStamp(&pJpegHandle->stProcInfo.SoftDectTimes,NULL);
        pJpegHandle->stProcInfo.SoftDectTimes = pJpegHandle->stProcInfo.SoftDectTimes - PreTime;
    }
    return;
}
...
HI_VOID JPEG_DEC_GetOutUsrBuf(HI_ULONG HdecHandle, HI_CHAR** OutYBuf, HI_U32 *YStride, HI_U32 *YSize, HI_CHAR** OutUVBuf, HI_U32 *UVStride, HI_U32 *UVSize)
{
    ...
    if (NULL != YSize)
    {
        *YSize = pJpegHandle->stOutSurface.Size[0];
    }

    if (NULL != UVSize)
    {
        *UVSize = pJpegHandle->stOutSurface.Size[1];
    }
...
HI_VOID JPEG_DEC_SeekInputBuffer(HI_ULONG HdecHandle, HI_CHAR** pCurBuf, HI_U32 *pBufPos, HI_U32 *pSeekPos)
{
    ...
    stImgInfo.bytes_in_buffer = 0;
    stImgInfo.curpos_in_buffer = 0;
...
* description  :
                 CNcomment: 是否支持保存输入的图片数据 CNend\n
* param[in]    : pSupportSave  CNcomment:    是否支持保存数据的标记 CNend\n
* retval       : NA
* retval       : NA
* others:      : NA
*****************************************************************************/
HI_VOID JPEG_DEC_SupportSaveInputImg(HI_BOOL *pSupportSave)
{
    ...
    if (((2 == LogLevel) || (3 == LogLevel)) && (NULL != pSupportSave))
    {
        *pSupportSave = HI_TRUE;
    }
...
* description  : save input image
                 CNcomment:  保存输入的解码数据 CNend\n
* param[in]    : pBuf  CNcomment:    保存数据的buffer CNend\n
* param[in]    : Size  CNcomment:    保存数据的buffer大小 CNend\n
* retval       : NA
* retval       : NA
* others:      : NA
*****************************************************************************/
...
{
    JPEG_DEC_SaveInputFile(pBuf, Size, bFinishRead);
    JPEG_DEC_SaveInputMjpeg(pBuf, Size);
}

/*****************************************************************************
* func         : JPEG_DEC_PrintMsg
* description  : print information
                 CNcomment: 打印相关信息 CNend\n
* param[in]    : HdecHandle          CNcomment:解码器句柄       CNend\n
* param[in]    : pFunc               CNcomment:要打印的函数     CNend\n
* param[in]    : Line                CNcomment:要打印的函数行号 CNend\n
* param[in]    : bBegPrint           CNcomment:开始打印         CNend\n
* retval       : NA
* retval       : NA
* others:      : NA
*****************************************************************************/
...
{
    JPEG_HDEC_HANDLE_S_PTR pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)HdecHandle;
    if ((NULL == pJpegHandle) || (NULL == pFunc) || (8 != pJpegHandle->LogLevel))
    {
        return;
    }

    if (HI_TRUE == bBegPrint)
    {
        HI_GFX_Print("[module-gfx-jpeg][beg]:++++++++++%s(%d)\n",pFunc, Line);
    }
    else
    {
        HI_GFX_Print("[module-gfx-jpeg][end]:++++++++++%s(%d)\n",pFunc, Line);
    }
