...
#define android_printLog __android_log_print
#endif
...
HI_BOOL LOGLevelChk(HI_S32 s32Levle, HI_U32 u32ModId, HI_BOOL* pUsrModeIDFlag)
{
    if (u32ModId < LOG_CONFIG_BUF_SIZE/sizeof(LOG_CONFIG_INFO_S))
    {
        ...
        if (g_LogModInit && (s32Levle <= g_pLogConfigInfo[u32ModId].u8LogLevel) && (g_pLogConfigInfo[u32ModId].u8LogLevel != HI_LOG_LEVEL_TRACE))
        {
            return HI_TRUE;
        }
        if (g_LogModInit && (s32Levle == g_pLogConfigInfo[u32ModId].u8LogLevel) && (g_pLogConfigInfo[u32ModId].u8LogLevel == HI_LOG_LEVEL_TRACE))
