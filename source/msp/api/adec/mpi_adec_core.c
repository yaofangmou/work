                             ...
                             HI_U32 u32AdecChID)
{
    ...
    for (i = 0; i < pstOutBuf->u32OutBufNum; i++)
    {
        ...
        pstOutBuf->outBuf[i].u32BufStatus = ADEC_OUTPUT_FILL_THIS_BUF;
    }
                                       ...
                                       HI_HADECODE_OUTPUT_S* pstAout, HI_U32 u32StreamReadPos, HI_U32 u32ConsumeBytes)
{
    ...
    if (ADEC_INVALID_PTS == ptRetBuf->u32PTS)
    {
        HI_U32 u32EndPos = 0;
        ADEC_STREAM_BUFFER_S* pstInBuf = &(pstAdecChan->InStreamBuf);
    ...
    if ((ptRetBuf->u32PTS < pstAdecChan->u32OutputDelay) || (ADEC_INVALID_PTS == ptRetBuf->u32PTS))
    {
        ptRetBuf->u32PTS = ADEC_INVALID_PTS;
    }
    else
    {
...
static HI_VOID ADECGetCodecOutputDelay(ADEC_CHAN_S* pstAdecChan)
{
    ...
    switch(pstAdecChan->u32CodecID)
    {
        ...
        {
            HA_CODEC_OUTOUTDELAY_GET_PARAM_S stOutputDelay = {HA_CODEC_OUTPUTDELAY_GET_CMD, 0};
...
static HI_U32 ADECCalOutFrameMs(ADEC_OUTPUTBUF_S* pstOutBuf)
{
    if (0 == pstOutBuf->u32PcmOutSamplesPerFrame)
    {
        return 0;
    }

    return (HI_U32)(pstOutBuf->u32PcmOutSamplesPerFrame * 1000 / pstOutBuf->u32OutSampleRate);
}

/* only works for pcm data */
static HI_U32 ADECCalOutPcmDataSize(ADEC_OUTPUTBUF_S* pstOutBuf)
{
    if (0 == pstOutBuf->u32PcmOutSamplesPerFrame)
    {
        return 0;
    ...
    if (16 == pstOutBuf->u32BitPerSample)
    {
        return pstOutBuf->u32PcmOutSamplesPerFrame * pstOutBuf->u32OutChannels * 2;
    }
    else if (24 == pstOutBuf->u32BitPerSample)
    {
        return pstOutBuf->u32PcmOutSamplesPerFrame * pstOutBuf->u32OutChannels * 4;
    }
    else
    {
        return 0;
...
static inline HI_BOOL ADECCheckNeedOutputLongFrame(ADEC_CHAN_S* pstAdecChan)
{
    if (HI_FALSE == pstAdecChan->bLowPerformanceChip)
    {
        return HI_FALSE;
    }

    if (HA_AUDIO_ID_BLYRAYLPCM == pstAdecChan->pstAdecInfo->u32CodecID)
    {
        return HI_TRUE;
    }

    if (HA_AUDIO_ID_FFMPEG_DECODE == pstAdecChan->pstAdecInfo->u32CodecID)
    {
        return HI_TRUE;
    }
...
static HI_VOID ADECUpdateOutBuf(ADEC_CHAN_S* pstAdecChan)
{
    ADEC_STREAM_OUTBUF_S* pstOutBuf = &(pstAdecChan->outStreamBuf);
    ADEC_PROC_ITEM_S* pstAdecInfo   = pstAdecChan->pstAdecInfo;

    HI_U32 u32CurReadIdx  = pstOutBuf->u32BufReadIdx;
    HI_U32 u32CurWriteIdx = pstOutBuf->u32BufWriteIdx;
    ADEC_OUTPUTBUF_S* pstCurBuf = &pstOutBuf->outBuf[u32CurWriteIdx];
    ADEC_OUTPUTBUF_S* pstLastBuf = 0;
    HI_U32 u32LastWriteIdx = 0;

    HI_U32 u32CurFrameMs = 0;
    HI_U32 u32LastFrameMs = 0;

    if (HI_FALSE == ADECCheckNeedOutputLongFrame(pstAdecChan))
    {
        pstOutBuf->outBuf[pstOutBuf->u32BufWriteIdx].u32BufStatus = ADEC_OUTPUT_EMPTY_THIS_BUF;
    ...
    /* we can just dump 5.1 LPCM and 7.1 LPCM in low performance chip */
    if (pstCurBuf->u32OutChannels > 2)
    {
        pstCurBuf->u32OutChannels = 2;
        pstAdecInfo->u32OutChannels = 2;
    }

    pstCurBuf->u32PcmOutSamplesPerFrame = (pstCurBuf->u32PcmOutSamplesPerFrame & 0xffff);

    u32CurFrameMs = ADECCalOutFrameMs(pstCurBuf);

    /* adec out buffer has only one frame */
    if (u32CurReadIdx == u32CurWriteIdx)
    {
        if (u32CurFrameMs >= ADEC_DEFAULT_OUTPUT_FRAME_MS_LIMIT)
        {
            pstCurBuf->u32BufStatus = ADEC_OUTPUT_EMPTY_THIS_BUF;
        }

        /* move write index any way */
        pstOutBuf->u32BufWriteIdx  = (pstOutBuf->u32BufWriteIdx + 1) % (pstOutBuf->u32OutBufNum);
        pstAdecInfo->u32FrameWrite = pstOutBuf->u32BufWriteIdx;
        return;
    }
    ...
    if (0 == u32CurWriteIdx)
    {
        u32LastWriteIdx = pstOutBuf->u32OutBufNum - 1;
    }
    else
    {
        u32LastWriteIdx = u32CurWriteIdx - 1;
    }

    pstLastBuf = &pstOutBuf->outBuf[u32LastWriteIdx];

    /* last frame has enough data and ready for output, we can not change last frame any more */
    if (ADEC_OUTPUT_EMPTY_THIS_BUF == pstLastBuf->u32BufStatus)
    {
        if (u32CurFrameMs >= ADEC_DEFAULT_OUTPUT_FRAME_MS_LIMIT)
        {
            pstCurBuf->u32BufStatus = ADEC_OUTPUT_EMPTY_THIS_BUF;
        ...
        /* move write index because last frame is ready */
        pstOutBuf->u32BufWriteIdx  = (pstOutBuf->u32BufWriteIdx + 1) % (pstOutBuf->u32OutBufNum);
        pstAdecInfo->u32FrameWrite = pstOutBuf->u32BufWriteIdx;
        return;
    }
    ...
    {
        u32LastFrameMs = ADECCalOutFrameMs(pstLastBuf);

        /* we have too much data in current frame and last frame */
        if (u32LastFrameMs + u32CurFrameMs > ADEC_DEFAULT_OUTPUT_FRAME_MS_LIMIT)
        {
            pstLastBuf->u32BufStatus = ADEC_OUTPUT_EMPTY_THIS_BUF;
            ...
            /* move write index because last frame is set to ready */
            pstOutBuf->u32BufWriteIdx  = (pstOutBuf->u32BufWriteIdx + 1) % (pstOutBuf->u32OutBufNum);
            pstAdecInfo->u32FrameWrite = pstOutBuf->u32BufWriteIdx;
            return;
        }
        ...
        {
            /* we need to combine current frame and last frame */
            HI_VOID* pLastFrameWp = (HI_VOID*)pstLastBuf->ps32PcmOutBuf;  /* last frame write pos */
            HI_U32   u32CurFrameSize = ADECCalOutPcmDataSize(pstCurBuf);  /* current frame data size in bytes */

            /* copy current frame data into last frame */
            pLastFrameWp += ADECCalOutPcmDataSize(pstLastBuf);
            memcpy(pLastFrameWp, (HI_VOID*)pstCurBuf->ps32PcmOutBuf, u32CurFrameSize);

            pstLastBuf->u32PcmOutSamplesPerFrame += pstCurBuf->u32PcmOutSamplesPerFrame;
            ...
            pstAdecInfo->u32PcmSamplesPerFrame = (pstLastBuf->u32PcmOutSamplesPerFrame & 0xffff);
            /* hold current frame buffer write index, wait for more data */
        }
    }
}
...
static HI_VOID ADECCheckChipPerformance(ADEC_CHAN_S* pstAdecChan)
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_SYS_VERSION_S stVersion;

    memset(&stVersion, 0, sizeof(HI_SYS_VERSION_S));
    stVersion.enChipTypeSoft = HI_CHIP_TYPE_BUTT;
    stVersion.enChipTypeHardWare = HI_CHIP_TYPE_BUTT;
    stVersion.enChipVersion = HI_CHIP_VERSION_BUTT;

    s32Ret = HI_SYS_GetVersion(&stVersion);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_ADEC("call HI_SYS_GetVersion failed(0x%x)\n", s32Ret);
        pstAdecChan->bLowPerformanceChip = HI_FALSE;
        return;
    }

    pstAdecChan->bLowPerformanceChip =
        ((HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeSoft && HI_CHIP_VERSION_V410 == stVersion.enChipVersion) ||
         (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeSoft && HI_CHIP_VERSION_V420 == stVersion.enChipVersion) ||
         (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeSoft && HI_CHIP_VERSION_V430 == stVersion.enChipVersion));
}

HI_U32 ADEC_CountAlive(HI_VOID)
{
    HI_U32 i = 0, chan_nbr = 0;

    ADEC_LOCK(&g_AdecMutex);

    for ( i = 0; i < ADEC_INSTANCE_MAXNUM; i++ )
    {
        if ( NULL != g_pstAdecChan[i] )
        {
            chan_nbr++;
        }
    }
...
HI_S32 ADEC_Open(HI_HANDLE* phAdec)
{
    ...
    ADECCheckChipPerformance(pstAdecChan);
...
HI_S32 ADEC_ReceiveFrame(HI_HANDLE hAdec, HI_UNF_AO_FRAMEINFO_S* pstAOFrame, ADEC_EXTFRAMEINFO_S* pstExtInfo)
{
    ...
    if (ADEC_OUTPUT_EMPTY_THIS_BUF != ptOutElem->u32BufStatus && pstAdecChan->bAdecEosFlag == HI_FALSE)
    {
        ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
        ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
        ...
        || (ptOutElem->u32BitPerSample > 24) || (ptOutElem->u32BitPerSample < 8))
    {
        ...
        ptOutElem->u32BufStatus = ADEC_OUTPUT_FILL_THIS_BUF;
...
HI_S32 ADEC_ReleaseFrame(HI_HANDLE hAdec, const HI_UNF_AO_FRAMEINFO_S* pstAOFrame)
{
    ...
    (pstOutBuf->outBuf)[pstAOFrame->u32FrameIndex].u32BufStatus = ADEC_OUTPUT_FILL_THIS_BUF;
    ADEC_UNLOCK(&g_AdecIOMutex[hAdec]);
    ADEC_UNLOCK(&g_AdecApiMutex[hAdec]);
    return HI_SUCCESS;
}
