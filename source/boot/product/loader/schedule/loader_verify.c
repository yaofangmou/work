...
)
{
    ...
    {
        if (0 == LOADER_MemCmp(&pstTocEntry->stUuid, &stNullUUID, sizeof(FIP_UUID_S)))
        {
            LOADER_PrintErrCode(LOADER_ERR_ILLEGAL_IMAGE);
            return LOADER_ERR_ILLEGAL_IMAGE;
        }

        if (0 == LOADER_MemCmp(&pstTocEntry->stUuid, &stLoaderUUID, sizeof(FIP_UUID_S)))
        {
...
)
{
    ...
    if (0 >= s32ReadSize)
    {
        LOADER_PrintFuncErr(HI_Flash_Read, LOADER_ERR_FLASH);
        return LOADER_ERR_FLASH;
    }
...
)
{
    ...
    s32Ret = loaderPartitionRead(hFlashHandleSrc, pu8Buffer, u32BufferSize, &u32DataOffset, &u32DataSize);
    if (HI_SUCCESS != s32Ret)
    {
        LOADER_PrintFuncErr(loaderPartitionRead, s32Ret);
        loaderPartitionClose(hFlashHandleDst);
