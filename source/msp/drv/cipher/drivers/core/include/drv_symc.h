...
{
    SYMC_KEY_SRC_CPU = 0,
    SYMC_KEY_SRC_KLAD_HARD,
    SYMC_KEY_SRC_KLAD_SOFT,
    SYMC_KEY_SRC_COUNT,
}symc_key_source;
...
typedef struct
{
    u32 id;                 /* channel number */
    u8 open;             /* Open or closed */
    u8 alg;              /* algorithm */
    u8 mode;             /* work mode */
    u8 klen;               /* Length of key */
    u8 ksrc;             /* Hard or soft key */
    u8 decrypt;             /* Decrypt or Encrypt */
    u32 inaddr;             /* In DDR address */
    u32 outaddr;            /* Out DDR address */
    u8 int_status;          /* Status interrupt */
    u8 int_en;              /* Enable interrupt */
    u8 int_raw;             /* Raw interrupt */
    u8 secure;              /* Secure channel or not */
    u8  smmu_enable;        /* SMMU Enable */
    u32 in_node_head;       /* In node list head */
    u8  in_node_depth;      /* In node depth */
    u8  in_node_rptr;        /* In node list read index */
    u8  in_node_wptr;        /* In node list write index */
    u32 out_node_head;      /* Out node list head */
    u8  out_node_depth;     /* Out node depth */
    u8  out_node_rptr;       /* Out node list read index */
    u8  out_node_wptr;       /* Out node list write index */
    u8  in_last_block;       /* first word of last input block */
    crypto_owner owner;     /* Process PID of owner */
    u8  iv[AES_IV_SIZE *2 + 1];    /* Out IV */
}symc_chn_status;

typedef struct
{
   u8  sec_cpu;       /* secure CPU or not */
   u8  allow_reset;   /* allow reset CPU or not */
   u32 int_num;       /* interrupt number */
   const char *name;  /* interrupt name */
   u8 int_en;         /* interrupt enable */
   u32 smmu_base;     /* smmu base address */
