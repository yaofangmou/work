...
{
    HI_CODEC_SUPPORT_S *pCurrent = HI_NULL;
    HI_CODEC_SUPPORT_S *pNext = HI_NULL;
    ...
    if (1 == s_stVdecAdpParam.u32InitCount)
    {
        ...
        {
            pCurrent = s_stVdecAdpParam.stCap.pstSupport;

            while (pCurrent)
            {
                pNext = pCurrent->pstNext;
                HI_FREE_VDEC(pCurrent);
                pCurrent = pNext;
            }
...
static HI_S32 VFMW_GetCap(HI_CODEC_CAP_S *pstCodecCap)
{
    ...
    HI_U32 i;
    HI_U32 codec[32];
    VDEC_IOCTL_MSG vdec_ioctl_msg;
    HI_CODEC_SUPPORT_S *pCodecListHead = HI_NULL;
    HI_CODEC_SUPPORT_S *pNode = HI_NULL;
    HI_CODEC_SUPPORT_S *pCurrent = HI_NULL;
    HI_CODEC_SUPPORT_S *pNext = HI_NULL;
    ...
    memset(&vdec_ioctl_msg, 0, sizeof(VDEC_IOCTL_MSG));

    /* If get capability the first time */
    if (HI_NULL == s_stVdecAdpParam.stCap.pstSupport)
    {
        vdec_ioctl_msg.up_msg = (HI_SIZE_T)(codec);

        /* Get VFMW capability */
        s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_GETCODEC, &vdec_ioctl_msg);

        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_VDEC("ioctl UMAPC_VDEC_GETCODEC failed\n");
            VDEC_UNLOCK(s_stVdecAdpParam.stMutex);
            return HI_FAILURE;
        }
        ...
        for (i = 0; i < 32; i++)
        {
            if (codec[i] == HI_CODEC_ID_BUTT)
            {
                break;
            }

            pNode = (HI_CODEC_SUPPORT_S *)HI_MALLOC_VDEC(sizeof(HI_CODEC_SUPPORT_S));
            if (pNode == HI_NULL)
            {
                HI_ERR_VDEC("HI_VMALLOC failed\n");
                pCurrent = pCodecListHead;
            ...
            pNode->u32Type = HI_CODEC_TYPE_DEC;
            pNode->enID = codec[i];
            pNode->pstNext = HI_NULL;

            if (i == 0)
            {
                pCodecListHead = pNode;
                pCurrent = pCodecListHead;
            }
            else
            {
                pCurrent->pstNext = pNode;
                pCurrent = pNode;
            }
        }

        s_stVdecAdpParam.stCap.pstSupport = pCodecListHead;
    }
...
static HI_S32 VFMW_Create(HI_HANDLE *phInst, const HI_CODEC_OPENPARAM_S *pstParam)
{
    ...
    if (NULL != pstParam)
    {
        ...
        if ((HI_CODEC_ID_VIDEO_MJPEG != pstParam->enID)
    #ifndef CHIP_TYPE_hi3716mv430
            && (HI_CODEC_ID_VIDEO_H263 != pstParam->enID)
...
static HI_S32 VFMW_IFrameDecode(HI_HANDLE hInst, HI_VOID *pParam)
{
    ...
    if ((HI_NULL == pstParam)
        || (HI_NULL == pstParam->pstIFrameStream)
    #ifdef CHIP_TYPE_hi3716mv430
        || (HI_NULL == pstParam->pstFrame)
	#endif
	    )
    {
        HI_ERR_VDEC("Bad param.\n");
...
#ifdef CHIP_TYPE_hi3716mv430
    *(pstParam->pstFrame) = stParam.stFrame;
#endif
...
HI_S32 VFMW_ReceiveFrame(HI_HANDLE hInst, HI_VOID *pParam)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_INFO_VDEC("Chan %d RecvFrm err:%x!\n", stParam.hHandle, s32Ret);
        return HI_ERR_CODEC_OPERATEFAIL;
    }
...
HI_S32 VFMW_ReceiveFrameWithTimeOut(HI_HANDLE hInst, HI_VOID *pParam)
{
    HI_S32 s32Ret;
    VDEC_CMD_VO_FRAME_S stParam;
    VDEC_RECVFRM_WITH_TIMEOUT_S *pstFrameInfo = (VDEC_RECVFRM_WITH_TIMEOUT_S *)pParam;
    ...
    stParam.hHandle = VFMW_INST_HANDLE(hInst);
    stParam.TimeOut = pstFrameInfo->TimeOut;
    ...
    *(pstFrameInfo->pVidFrm) = stParam.stFrame;
    HI_INFO_VDEC("Chan %d RecvFrm OK\n", stParam.hHandle);
...
static const VFMW_CTRL_CMD_NODE g_VfmwCtrlTable[] =
{
    ...
    {VFMW_CMD_RECEIVEFRAME,             VFMW_ReceiveFrame},
    {VFMW_CMD_RELEASEFRAME,             VFMW_ReleaseFrame},
    ...
    {VFMW_CMD_RECEIVE_FRAME_WITH_TIMEOUT, VFMW_ReceiveFrameWithTimeOut},
...
#ifndef HI_SMMU_SUPPORT
    s32Ret = HI_MPI_MMZ_Unmap(pstBufInst->stUserBuffer.u32PhyAddr);
#else
    s32Ret = HI_MPI_SMMU_Unmap(pstBufInst->stUserBuffer.u32PhyAddr);
#endif

    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Unmap user buffer faield\n");
        return HI_FAILURE;
    }
...
HI_S32 VDEC_ResetFrameBuf(HI_HANDLE hBuf)
{
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("Unmap user buffer failed\n");
        return HI_FAILURE;
    }
...
HI_S32 VDEC_GlobalReleaseAllFrm(HI_DRV_VIDEO_FRAME_S *pVideoFrame)
{
    ...
    s32Ret = ioctl(s_stVdecAdpParam.s32DevFd, UMAPC_VDEC_CHAN_GLOBALRELALL, pVideoFrame);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("VDEC_GlobalReleaseAllFrm err: 0x%x\n", s32Ret);
    }
