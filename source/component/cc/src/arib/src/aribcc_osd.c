...
static HI_U16 _AribCC_MapUnicode(HI_U8 u8Text, HI_U8 u8CharSet,ARIBCC_PRTDIRECT_E stPrtDirection,HI_U8 *IsNonspace)
{
   ...
   {0x3000,0x3001,0x3002,0xFF0C,0xFF0E,0x30FB,0xFF1A,0xFF1B,0xFF1F,0xFF01,0x309B,0x309C,0x00B4,0xFF40,0x00A8,0xFF3E,
    ...
    {
        {0x0020, 0x266A, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020, 0x0020},
        {0x00A4, 0x00A6, 0x00A8, 0x00B4, 0x00B8, 0x00BC, 0x00BD, 0x00BE, 0x0020, 0x0020, 0x0020, 0x0020},
        {0x2026, /*0x2530*/0x2588, 0x2018, 0x2019, 0x201C, 0x201D, 0x2022, 0x2122, 0x215B, /*0x2153*/0x215C, 0x215D, 0x215E}
    };
...
static HI_S32 _AribCC_DrawCaptionToWindow(HI_S32 CCType,HI_U16 *pu16Buf, HI_U16 u16BufLen,HI_U8 u8HasNonspace)
{
    ...
    while( u16BufLen > 0 )
    {
        ...
        if ((u16BufLen >= empty_cols) && (pstText->stFormat.stPrtDirect != CCARIB_PRINT_VERTICAL))
        {
        ...
        if ((CCARIB_PRINT_VERTICAL == pstText->stFormat.stPrtDirect) && (draw_length > MOVE_Y_FOR_VERTICAL))
        {
            if (y > (MOVE_Y_FOR_VERTICAL * char_height_with_interval))
            {
                y -= (MOVE_Y_FOR_VERTICAL * char_height_with_interval);
            }
        }
        ...
        else
        {
            ...
            //clear screen when draw from the front line
            if (pstText->stCursor.u16Row < pstText->u16LastRowNum)
            {
                OSD_TRACE("FillWindow \n");
                ARIBCC_GRAPHIC_ClearScreen(CCType, HI_FALSE);
            }

            ARIBCC_GRAPHIC_TextOut(CCType, x, y, pu16Buf+counter, draw_length, pstText->u8CharInterval, pstText->u8LineInterval, bBlit);
...
static HI_S32 _AribCC_IsCCSDOverlap(void)
{
    ...
    HI_U32 horizontal_overlap = 0;
    HI_U32 vertical_overlap = 0;
...
void ARIBCC_OSD_ClearScreen(HI_S32 CCType, HI_BOOL bBlit)
{
    ...
    {
        HI_U32 u32HscrollTimeout = 100; //wait 3s
        gIsHscrollCancel = 1;
        while ((pAribText->u8IsHscrollFinish == 0) && (u32HscrollTimeout--))
        {
...
HI_S32 ARIBCC_OSD_CharFlash(HI_VOID)
{
    (void)ARIBCC_Text_CharFlash();
    return HI_SUCCESS;
}
