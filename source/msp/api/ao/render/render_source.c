...
HI_S32 SourceGetStreamInfo(SOURCE_ID_E enSourceID, ADEC_STREAMINFO_S* pstStreamInfo)
{
    ...
    pstStreamInfo->bLfePresent  = pstSource->stRenderEvent.stStreamInfo.bLfePresent;
    if (MS12_STREAM_DD == pstSource->stRenderEvent.stStreamInfo.enACodecType)
    {
        pstStreamInfo->enAudioFormat = FORMAT_AC3;
    }
    else if (MS12_STREAM_DDP == pstSource->stRenderEvent.stStreamInfo.enACodecType)
    {
        pstStreamInfo->enAudioFormat = FORMAT_EAC3;
    }
    else if (MS12_STREAM_ATMOS == pstSource->stRenderEvent.stStreamInfo.enACodecType)
    {
        pstStreamInfo->enAudioFormat = RENDER_FORMAT_ATMOS;
    }
    else if (MS12_STREAM_AAC == pstSource->stRenderEvent.stStreamInfo.enACodecType)
    {
        pstStreamInfo->enAudioFormat = FORMAT_AAC;
    }

    u32Acmod = pstSource->stRenderEvent.stStreamInfo.u32Channel;
    if (/*(HA_AUDIO_CH_MODE_DUAL_MONO <= u32Acmod) && */ (HA_AUDIO_CH_MODE_3_2_MODE >= u32Acmod))
    {
        pstStreamInfo->enChannelMode = u32Acmod;
    }
    else
    {
        pstStreamInfo->enChannelMode = HA_AUDIO_CH_MODE_BUTT;
    }
...
HI_S32 Source_Client_FlushTrack(HI_HANDLE hTrack)
{
    ...
    s32Ret = IPC_Client_FlushTrack(SrcTrack);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    if (HI_TRUE == CheckIsMediaTrack(hTrack))
    {
        s32Ret = ClientFindActivedSourceFromTrack(SrcTrack, 0, &pstSource);
        if (HI_SUCCESS != s32Ret)
        {
            HI_WARN_AO("Track(0x%x) not instantiated or actived\n", hTrack);
        }
    ...
    {
        SOURCE_ID_E enSourceID;
        CHECK_AND_GET_SOURCEIDFROMTRACKID(SrcTrack, enSourceID);
...
HI_S32 Source_Client_SetTrackChannelMode(HI_HANDLE hTrack, HI_UNF_TRACK_MODE_E enMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    HI_WARN_AO("Source SetTrackChannelMode 0x%x\n", hTrack);

    if (enMode >= HI_UNF_TRACK_MODE_BUTT /*|| enMode < HI_UNF_TRACK_MODE_STEREO */)
    {
        HI_ERR_AO("Invalid Track Channel Mode:%d!\n", enMode);
        return HI_ERR_AO_INVALID_PARA;
    }

    if (HI_TRUE != CheckIsMediaTrack(hTrack))
    {
        HI_WARN_AO("Unsupport not main track:0x%x!\n", hTrack);
        return HI_SUCCESS;
    }

    s32Ret = IPC_Client_SetTrackChannelMode(hTrack, enMode);
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_PrintFuncErr(IPC_Client_SetTrackChannelMode, s32Ret);
        return s32Ret;
    }
...
HI_S32 Source_Client_GetTrackChannelMode(HI_HANDLE hTrack, HI_UNF_TRACK_MODE_E* penMode)
{
    HI_S32 s32Ret = HI_SUCCESS;

    if (HI_TRUE != CheckIsMediaTrack(hTrack))
    {
        HI_ERR_AO("Unsupport not main track:0x%x!\n", hTrack);
        return HI_ERR_AO_NOTSUPPORT;
    }

    s32Ret = IPC_Client_GetTrackChannelMode(hTrack, penMode);
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_PrintFuncErr(IPC_Client_GetTrackChannelMode, s32Ret);
        return s32Ret;
    }
...
HI_S32 Source_Client_SetAtmosLockEnable(HI_BOOL bAtmosLockEnable)
{
    HI_S32 s32Ret = HI_SUCCESS;

    s32Ret = IPC_Client_SetAtmosLockEnable(bAtmosLockEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_PrintFuncErr(IPC_Client_SetAtmosLockEnable, s32Ret);
    }

    return s32Ret;
}

HI_S32 Source_Client_GetAtmosLockEnable(HI_BOOL* pbAtmosLockEnable)
{
    HI_S32 s32Ret = HI_SUCCESS;

    s32Ret = IPC_Client_GetAtmosLockEnable(pbAtmosLockEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_LOG_PrintFuncErr(IPC_Client_GetAtmosLockEnable, s32Ret);
    }
