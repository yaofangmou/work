...
static const HI_U8 des_key[24] = {0x01, 0x23, 0x45, 0x67, 0x89, 0xAB, 0xCD, 0xEF, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x11, 0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x22, 0x22};
...
static const HI_U8 des_ecb_enc_dst_buf[16] = {0xba, 0xbb, 0x69, 0x00, 0x12, 0xbe, 0xc8, 0x59};
static const HI_U8 des_ecb_dec_src_buf[16] = {0xba, 0xbb, 0x69, 0x00, 0x12, 0xbe, 0xc8, 0x59};
...
HI_S32 multiCipher_3DES_ECB_ENC()
{
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        {
            HI_ERR_CIPHER("MultiCipher 3DES ECB Encryption run failed!\n");
            printBuffer("3DES", pOutputAddrVir[i], u32TestDataLen);
            printBuffer("GOLD", des_ecb_enc_dst_buf, u32TestDataLen);
            s32Ret = HI_FAILURE;
            goto __CIPHER_EXIT__;
        }
    }

    HI_INFO_CIPHER("MultiCipher 3DES ECB Encryption run success!\n");
...
HI_S32 multiCipher_3DES_ECB_DEC()
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32TestDataLen = 8;
    HI_U32 u32Testcached = 0;
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        {
            HI_ERR_CIPHER("MultiCipher 3DES ECB Decryption run failed!\n");
            printBuffer("3DES", pOutputAddrVir[i], u32TestDataLen);
            printBuffer("GOLD", des_ecb_enc_dst_buf, u32TestDataLen);
            s32Ret = HI_FAILURE;
            goto __CIPHER_EXIT__;
    ...
    HI_INFO_CIPHER("MultiCipher 3DES ECB Decryption run success!\n");
...
int main(int argc, char* argv[])
{
    ...
    if ( argc != 2)
    {
        ...
        HI_INFO_CIPHER("#########cipher sample##########\n"
                        "[0] multiCipher_3DES_ECB_ENC\n"
                        "[1] multiCipher_3DES_ECB_DEC\n"
    ...
    if ( 0 == funcNumber )
    {
        s32Ret = multiCipher_3DES_ECB_ENC();
    }
    else if ( 1 == funcNumber )
    {
        s32Ret = multiCipher_3DES_ECB_DEC();
    }
