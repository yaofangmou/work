...
static HI_S32 gHue;
static HI_S32 gCscColotTemperation[3];
...
static void HelpMsg(void)
{
    ...
    Printf("   -cscct, --csc colortemperation       <0~100>       Change the csc temperation (Red,Green,Blue)\n");
...
static HI_S32  getCscColorTemperation(HI_UNF_DISP_E enChan, HI_S32 val[])
{
    HI_S32 s32Ret;
    if (enChan > HI_UNF_DISPLAY1)
    { return HI_FAILURE; }
    ...
    HI_UNF_PQ_COLORTEMPERATURE_S stColorTemperature = {50,50,50};

     s32Ret = HI_UNF_PQ_GetColorTemperature(enChan, &stColorTemperature);
    if (HI_FAILURE == s32Ret)
    {
        Printf("sample getCscColorTemperation fail!\n");
        return HI_FAILURE ;
    }

    val[0] = stColorTemperature.u32Red;
    val[1] = stColorTemperature.u32Green;
    val[2] = stColorTemperature.u32Blue;

    Printf("get getCscColorTemperation:\nRed:%d \nGreen:%d \nBlue:%d\n", \
           val[0], val[1], val[2]);

    return s32Ret ;
}

HI_S32 setCscColorTemperation(HI_UNF_DISP_E enChan,  HI_S32 val[])
{
    HI_S32 s32Ret;
    if (enChan > HI_UNF_DISPLAY1)
    { return HI_FAILURE; }
    ...
    HI_UNF_PQ_COLORTEMPERATURE_S stColorTemperature = {50,50,50};
    stColorTemperature.u32Red   = val[0];
    stColorTemperature.u32Green = val[1];
    stColorTemperature.u32Blue  = val[2];

    s32Ret = HI_UNF_PQ_SetColorTemperature(enChan,stColorTemperature);
    if (HI_FAILURE == s32Ret)
    {
        Printf("sample setCscColorTemperation fail!\n");
        return HI_FAILURE ;
    }

    Printf("sample setCscColorTemperation:\nRed:%d \nGreen:%d \nBlue:%d\n", \
           val[0], val[1], val[2]);
    return s32Ret ;
...
static HI_S32 ParseCommandLine( int argc, char* argv[] )
{
    ...
    for (n = 1; n < argc; n++)
    {
        ...
        if (strncmp (arg, "-cscct", strlen(arg)) == 0 || strncmp (arg, "--csccolortemperation", strlen(arg)) == 0)
        {
            HI_S32 i;
            ...
            for (i = 0; i < 3; i++)
            {
                ...
                gCscColotTemperation[i] = atoi(argv[n]);
                RANGECHECK(gCscColotTemperation[i], 0, 100);
            }
...
HI_S32 main(int argc, char* argv[])
{
    ...
    if (0 == gSetType)
    {
        ...
        else if (strncmp (gpGetOption, "-cscct", strlen(gpGetOption)) == 0 || strncmp (gpGetOption, "--csccolortemperation", strlen(gpGetOption)) == 0)
        {
            getCscColorTemperation((HI_UNF_DISP_E)gChan, gCscColotTemperation);
        }
    ...
    else if (1 == gSetType)
    {
        ...
        if (gCscColotTemperation[2] > -1)
        {
            setCscColorTemperation((HI_UNF_DISP_E)gChan, gCscColotTemperation);
        }
        if (g6BaseColor[5] > -1)
        {
