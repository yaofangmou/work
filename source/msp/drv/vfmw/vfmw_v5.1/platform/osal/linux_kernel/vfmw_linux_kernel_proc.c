...
static VOID show_stdard_information(struct seq_file *p, SINT32 ChanId)
{
    ...
    else if (pChan->eVidStd == VFMW_USER)
    {
        ...
        seq_printf(p, "Recent UsrdecFrame: yuvFormat = %d, w = %d, h = %d, Ystride = %d, Cstride = %d, PTS = %lld\n",
                   FrameDesc->yuvFormat, FrameDesc->s32YWidth, FrameDesc->s32YHeight, FrameDesc->s32LumaStride, FrameDesc->s32ChromStride, FrameDesc->Pts);
    }
#endif
...
static VOID show_main_interface_time(struct seq_file *p, SINT32 ChanId)
{
    VFMW_CHAN_STAT_S *pChanStat = NULL;

    pChanStat = &g_VfmwChanStat[ChanId];

    seq_printf(p, "---------------- Main Interface Time INFO -------------\n");
    seq_printf(p, "%-40s :%d ms\n",    "Chan Creat   use", pChanStat->u32ChanCreatTime);
    seq_printf(p, "%-40s :%d ms\n",    "Alloc Mem    use", pChanStat->u32ChanAllocMemTime);
    seq_printf(p, "%-40s :%d ms\n",    "Chan Start   use", pChanStat->u32ChanStartTime);
    seq_printf(p, "%-40s :%d ms\n",    "Chan Destroy use", pChanStat->u32ChanDestroyTime);
    seq_printf(p, "%-40s :%d ms\n",    "Free Men     use", pChanStat->u32ChanFreeMemTime);
    seq_printf(p, "%-40s :%d ms\n",    "Chan Reset   use", pChanStat->u32ChanRestTime);
    seq_printf(p, "\n");
}
...
#define PRINT_WORD(x)       seq_printf(p, "  %-15s  0x%x\n", #x, 1<<(x))
    ...
    seq_printf(p, "echo [arg1] [arg2] > /proc/msp/vfmw_info\n");
    seq_printf(p, "\n");

    seq_printf(p, "---------------------------------------------------------------\n");
    seq_printf(p, "'proc command list' cmd id & cmd desc\n");
    seq_printf(p, "---------------------------------------------------------------\n");
    seq_printf(p, "  arg1              arg2\n");
    ...
    seq_printf(p, "---------------------------------------------------------------\n");
    seq_printf(p, "print level definition:\n");
    seq_printf(p, "---------------------------------------------------------------\n");
...
SINT32 vfmw_read_proc_chn(struct seq_file *p, VOID *v)
{
    ...
    seq_printf(p, "%-40s :%d\n",          "CfgSedOnly",            pChan->stChanCfg.s32SedOnlyEnable);
    ...
    show_main_interface_time(p, ChanId);
