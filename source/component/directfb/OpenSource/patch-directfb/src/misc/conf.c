...
#endif
     if (strcmp (name, "layer-num" ) == 0) {
          if (value)
          {
               int layer_num;
	           int i;

               if (direct_sscanf( value, "%d", &layer_num ) < 1)
               {
                    D_ERROR("DirectFB/Config 'layer_num': Could not parse value!\n");
                    return DFB_INVARG;
               }

               dfb_config->layer_num = layer_num;

               if (1 == dfb_config->layer_num)
               {
                   dfb_config->layers[0].init = true;
                   dfb_config->layers[0].stacking = (1 << DWSC_UPPER)  | (1 << DWSC_MIDDLE) | (1 << DWSC_LOWER);
               }
               else
               {/** DWSC_LOWER get to 1 layer use **/
                   dfb_config->layers[0].stacking = (1 << DWSC_UPPER)  | (1 << DWSC_MIDDLE);
               }

    	       for (i = 1; i < layer_num; i++)
    	       {
                   dfb_config->layers[i].init       = true;
                   dfb_config->layers[i].stacking   = (1 << DWSC_LOWER);
               }
          }
          else
          {
               D_ERROR("DirectFB/Config 'layer_num': No value specified!\n");
               return DFB_INVARG;
          }
     } else
//HISILICON add end
