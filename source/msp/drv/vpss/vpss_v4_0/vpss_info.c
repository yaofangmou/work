...
static VPSS_IN_IMAGELIST_STATE_S g_stImgListState;
...
static HI_CHAR  g_chProcWriteCmd[DEF_FILE_NAMELENGTH];
static HI_CHAR  g_chProcWriteArg1[DEF_FILE_NAMELENGTH];
static HI_CHAR  g_chProcWriteArg2[DEF_FILE_NAMELENGTH];
static HI_CHAR  g_chProcWriteArg3[DEF_FILE_NAMELENGTH];
static HI_CHAR  g_chProcWriteArg4[DEF_FILE_NAMELENGTH];

static VPSS_PROC_REF_TABLE_S g_au32VPSSRotationRefTable [] =
{
...
static VPSS_PROC_REF_TABLE_S g_au32VPSSLBoxColorRefTable [] =
{
...
static VPSS_PROC_REF_TABLE_S g_au32VPSSOutBitWidthRefTbl [] =
{
...
#ifdef VPSS_SUPPORT_PROC_V2
static VPSS_PROC_REF_TABLE_S g_au32VPSSProcLevelRefTbl [] =
{
...
#endif
static VPSS_PROC_REF_TABLE_S g_au32VPSSProcMenuRefTbl [] =
{
...
static VPSS_PROC_REF_TABLE_S g_au32VPSSProcPortRefTbl [] =
{
...
HI_VOID VPSS_DBG_PrintFrameInfo(HI_DRV_VIDEO_FRAME_S *pstFrame, HI_BOOL bSrcFrame, HI_U32 u32StartIndex, HI_U32 u32EndIndex)
{
    ...
    if (HI_TRUE == bSrcFrame)
    {
        VPSS_PROC_MSG_PRINT("Src Frame : Index %d Type %d Format %d W %d H %d Prog %d topfirst %d FieldMode %d enBufValidMode %d\n"
                            ...
                            "            AspW %u AspH %u  DispCtrlW %u DispCtrlH %u DispRect xywh: %d %d %d %d\n"
    ...
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
    {
        ...
        if (pstPort->bEnble)
        {
            ...
            VPSS_PROC_INFO_PRINT(p,
                                 ...
                                 ,
                                 VPSS_PROC_PRINT_ENUM(pstUsrPortCfg->bPassThrough,
                                         VPSS_BYPASS_BUTT, g_pVpssBypassModeString),
    ...
    for (u32Count = 0; u32Count < VPSS_PROC_TIMESTAMP_MAXCNT; u32Count++)
    {
        ...
        if (HI_TRUE == bTunnelEnable)
        {
            VPSS_PROC_INFO_PRINT(p,
                                 ...
                                 VPSS_PROC_LEVEL4 "TrySrc               : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "GetSrc               : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "InBufDone            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "PQInitStart          : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "PQInitEnd            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "SrcReady             : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "OutBufReady          : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "InstReady            : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "LogicStart           : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "TunnelOutInt         : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "ThreadWakeupTunenl   : %lu(+%lums)"
                                 VPSS_PROC_LEVEL4 "SendOutFrame         : %lu(+%lums)"
                                 ...
                                 pstInstance->aulTrySrcTime[u32Count],
                                 pstInstance->aulTrySrcTime[u32Count] - pstInstance->aulBeginTime[u32Count],
                                 pstInstance->aulGetSrcTime[u32Count],
                                 pstInstance->aulGetSrcTime[u32Count] - pstInstance->aulTrySrcTime[u32Count],
                                 pstInstance->aulInBufDoneTime[u32Count],
                                 pstInstance->aulInBufDoneTime[u32Count] - pstInstance->aulGetSrcTime[u32Count],

                                 pstInstance->aulPQInitStartTime[u32Count],
                                 pstInstance->aulPQInitStartTime[u32Count] - pstInstance->aulInBufDoneTime[u32Count],
                                 pstInstance->aulPQInitEndTime[u32Count],
                                 pstInstance->aulPQInitEndTime[u32Count] - pstInstance->aulPQInitStartTime[u32Count],
                                 pstInstance->aulSrcReadyTime[u32Count],
                                 pstInstance->aulSrcReadyTime[u32Count] - pstInstance->aulPQInitEndTime[u32Count],
                                 pstInstance->aulOutBufReadyTime[u32Count],
                                 pstInstance->aulOutBufReadyTime[u32Count] - pstInstance->aulSrcReadyTime[u32Count],

                                 pstInstance->aulInstReadyTime[u32Count],
                                 pstInstance->aulInstReadyTime[u32Count] - pstInstance->aulOutBufReadyTime[u32Count],
                                 pstInstance->aulLogicStartTime[u32Count],
                                 pstInstance->aulLogicStartTime[u32Count] - pstInstance->aulInstReadyTime[u32Count],
                                 ...
                                 pstInstance->aulSendOutTime[u32Count],
                                 pstInstance->aulSendOutTime[u32Count] - pstInstance->aulThreadWakeUpTunnel[u32Count],
                                 pstInstance->aulLogicEndTime[u32Count],
                                 pstInstance->aulLogicEndTime[u32Count] - pstInstance->aulSendOutTime[u32Count],
        ...
        else
        {
            VPSS_PROC_INFO_PRINT(p,
                                 ...
                                 pstInstance->aulSendOutTime[u32Count],
                                 pstInstance->aulSendOutTime[u32Count] - pstInstance->aulLogicEndTime[u32Count],
                                 pstInstance->aulThreadWakeUpAll[u32Count],
                                 pstInstance->aulThreadWakeUpAll[u32Count] - pstInstance->aulSendOutTime[u32Count],
...
HI_S32 VPSS_CTRL_ProcPrint_V2(struct seq_file *p, HI_VOID *v)
{
    ...
    VPSS_PROC_INFO_PRINT(p,   VPSS_PROC_LEVEL1
                         ...
                         VPSS_PROC_PRINT_YN_COL(pstInstance->bAndroidCts)
                        );
...
HI_S32 VPSS_CTRL_ProcPrint_V1(struct seq_file *p, HI_VOID *v)
{
    ...
    VPSS_INST_GetSrcListState(pstInstance, &g_stImgListState);
    ...
    VPSS_PROC_INFO_PRINT(p,
                         ...
                         g_stImgListState.u32GetUsrTotal,      g_stImgListState.u32GetUsrSuccess,
                         ...
                         g_stImgListState.u32RelUsrTotal,      g_stImgListState.u32RelUsrSuccess,
                           ...
                           const char __user *buf, size_t count, loff_t *ppos)
{
    ...
    memset(g_chProcWriteCmd, 0, DEF_FILE_NAMELENGTH);
    memset(g_chProcWriteArg1, 0, DEF_FILE_NAMELENGTH);
    memset(g_chProcWriteArg2, 0, DEF_FILE_NAMELENGTH);
    memset(g_chProcWriteArg3, 0, DEF_FILE_NAMELENGTH);
    memset(g_chProcWriteArg4, 0, DEF_FILE_NAMELENGTH);
    ...
    if (copy_from_user(g_chProcWriteCmd, buf, count))
    {
    ...
    VPSS_OSAL_GetProcArg(g_chProcWriteCmd, g_chProcWriteArg1, 1);
    VPSS_OSAL_GetProcArg(g_chProcWriteCmd, g_chProcWriteArg2, 2);
    VPSS_OSAL_GetProcArg(g_chProcWriteCmd, g_chProcWriteArg3, 3);
    VPSS_OSAL_GetProcArg(g_chProcWriteCmd, g_chProcWriteArg4, 4);


    stDbgCmd.enDbgType = DBG_BUTT;
    for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSProcMenuRefTbl); s32Idx++)
    {
        if (0 == HI_OSAL_Strncmp(g_chProcWriteArg1, g_au32VPSSProcMenuRefTbl[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
        {
            stDbgCmd.enDbgType = g_au32VPSSProcMenuRefTbl[s32Idx].u32Context;
            break;
        }
    }
    ...
    {
        if (!HI_OSAL_Strncmp(g_chProcWriteArg1, DEF_DBG_CMD_SETBYPASS, DEF_FILE_NAMELENGTH))
        {
            pstInstDbg->unInfo.bits.vpssbypass = 0;
            if (!HI_OSAL_Strncmp(g_chProcWriteArg2, DEF_DBG_SET_ON, DEF_FILE_NAMELENGTH))
            {
        ...
        else if (!HI_OSAL_Strncmp(g_chProcWriteArg1, DEF_DBG_CMD_RESET, DEF_FILE_NAMELENGTH))
        {
        ...
        else if (!HI_OSAL_Strncmp(g_chProcWriteArg1, DEF_DBG_CMD_SETRATE, DEF_FILE_NAMELENGTH))
        {
            ...
            pstInstDbg->unInfo.bits.framerate = (HI_U32)(simple_strtol(g_chProcWriteArg3, NULL, 10));
            if (strlen(g_chProcWriteArg3) == 0)
            {
...
#ifdef VPSS_SUPPORT_PROC_V2
        else if (!HI_OSAL_Strncmp(g_chProcWriteArg1, DEF_DBG_CMD_SETPROCLEVEL, DEF_FILE_NAMELENGTH))
        {
            pstInstDbg->eProcPrintLevel = PROC_PRINT_LEVEL_ORI;
            for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSProcLevelRefTbl); s32Idx++)
            {
                if (0 == HI_OSAL_Strncmp(g_chProcWriteArg2, g_au32VPSSProcLevelRefTbl[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
                {
                    pstInstDbg->eProcPrintLevel = (VPSS_PROC_PRINT_LEVEL_E)g_au32VPSSProcLevelRefTbl[s32Idx].u32Context;
                    break;
                }
            }
            return count;
    ...
    for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSProcPortRefTbl); s32Idx++)
    {
        if (0 == HI_OSAL_Strncmp(g_chProcWriteArg2, g_au32VPSSProcPortRefTbl[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
        {
            stDbgCmd.hDbgPart = g_au32VPSSProcPortRefTbl[s32Idx].u32Context;
            break;
        }
    }
    ...
    if (stDbgCmd.enDbgType == DBG_SET_RATATION)
    {
        ...
        for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSRotationRefTable); s32Idx++)
        {
            if (0 == HI_OSAL_Strncmp(g_chProcWriteArg3, g_au32VPSSRotationRefTable[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
            {
                stDbgCmd.hDbgValue = (VPSS_HANDLE)g_au32VPSSRotationRefTable[s32Idx].u32Context;
                break;
            }
        }
    }
    else if (stDbgCmd.enDbgType == DBG_SET_LBX_COLOR)
    {
        ...
        for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSLBoxColorRefTable); s32Idx++)
        {
            if (0 == HI_OSAL_Strncmp(g_chProcWriteArg3, g_au32VPSSLBoxColorRefTable[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
            {
                stDbgCmd.hDbgValue = (VPSS_HANDLE)g_au32VPSSLBoxColorRefTable[s32Idx].u32Context;
                break;
            }
        }
    }
    else if (stDbgCmd.enDbgType == DBG_SET_OUTBITWIDTH)
    {
        ...
        for (s32Idx = 0; s32Idx < VPSS_GET_ARRAY_CNT(g_au32VPSSOutBitWidthRefTbl); s32Idx++)
        {
            if (0 == HI_OSAL_Strncmp(g_chProcWriteArg3, g_au32VPSSOutBitWidthRefTbl[s32Idx].pInputString, DEF_FILE_NAMELENGTH))
            {
                stDbgCmd.hDbgValue = (VPSS_HANDLE)g_au32VPSSOutBitWidthRefTbl[s32Idx].u32Context;
                break;
            }
        }
    }
    ...
    {
        stDbgCmd.hDbgValue = simple_strtoul(g_chProcWriteArg3, NULL, 10);
        stDbgCmd.hDbgValue = stDbgCmd.hDbgValue << 16;
        stDbgCmd.hDbgValue = stDbgCmd.hDbgValue + (simple_strtoul(g_chProcWriteArg4, NULL, 10) & 0xFFFF);
    }
    ...
    else
    {
        ...
        if (!HI_OSAL_Strncmp(g_chProcWriteArg3, DEF_DBG_SET_ON, DEF_FILE_NAMELENGTH))
        {
