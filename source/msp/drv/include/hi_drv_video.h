...
typedef struct sl_hdr_metadata_variables
{
	HI_S32 s32TmInputSignalBlackLevelOffset;
	HI_S32 s32TmInputSignalWhiteLevelOffset;
	HI_S32 s32ShadowGain;
	HI_S32 s32HighlightGain;
	HI_S32 s32MidToneWidthAdjFactor;
	HI_S32 s32TmOutputFineTuningNumVal;
	HI_S32 s32TmOutputFineTuningX[10];
	HI_S32 s32TmOutputFineTuningY[10];
	HI_S32 s32SaturationGainNumVal;
	HI_S32 s32SaturationGainX[6];
	HI_S32 s32SaturationGainY[6];
} HI_SL_HDR_Metadata_Var_S;

typedef struct sl_hdr_metadata_tables
{
	HI_S32 s32LuminanceMappingNumVal;
	HI_S32 s32LuminanceMappingX[65];
	HI_S32 s32LuminanceMappingY[65];
	HI_S32 s32ColourCorrectionNumVal;
	HI_S32 s32ColourCorrectionX[65];
	HI_S32 s32ColourCorrectionY[65];
} HI_SL_HDR_Metadata_Tab_S;

typedef struct sl_hdr_metadata
{
	HI_S32  s32PartID;
	HI_S32  s32MajorSpecVersionID;
	HI_S32  s32MinorSpecVersionID;
	HI_S32  s32PayloadMode;
	HI_S32  s32HdrPicColourSpace;
	HI_S32  s32HdrDisplayColourSpace;
	HI_S32  s32HdrDisplayMaxLuminance;
	HI_S32  s32HdrDisplayMinLuminance;
	HI_S32  s32SdrPicColourSpace;
	HI_S32  s32SdrDisplayColourSpace;
	HI_S32  s32SdrDisplayMaxLuminance;
	HI_S32  s32SdrDisplayMinLuminance;
	HI_S32  s32MatrixCoefficient[4];
	HI_S32  s32ChromaToLumaInjection[2];
	HI_S32  s32KCoefficient[3];
	union
	{
		 HI_SL_HDR_Metadata_Var_S variables;
		 //HI_SL_HDR_Metadata_Tab_S tables;
	} u;
} HI_SL_HDR_Metadata_S;

typedef struct hiSL_HDR_Metadata_STREAM_S
{
    HI_U32 u32Addr;
    HI_U32 u32Length;
} HI_SL_HDR_Metadata_STREAM_S;

typedef struct hiDRV_VIDEO_AVS_SL_HDR_INFO_S
{
	HI_U8  u8staticMetadataAvailable;
	HI_U8  u8dynamicMetadataAvailable;
	HI_U8  u8TransferCharacteristics;
	HI_DRV_VIDEO_HDR10_INFO_S staticMetadata;
	HI_SL_HDR_Metadata_STREAM_S dynamicMetadata;
} HI_DRV_VIDEO_JTP_SL_HDR_INFO_S;
...
typedef struct hiDRV_VIDEO_HDR10PLUS_DYNAMIC_METADATA_S
{
    /************Abbreviations*************/
    /*targeted system display -> T display*/
    /*mastering display       -> M display*/
    /*distribution            -> dist     */
    /**************************************/
    HI_U8   u8ItuCountryCode;
    HI_U16  u16ItuTerminalProviderCode;
    HI_U16  u16ItuTerminalProviderOrientedCode;
    HI_U8   u8ApplicationIdentifier;
    HI_U8   u8ApplicationVersion;
    HI_U8   u8NumWindows;

    HI_U16  u16WindowUpperLeftCornerX[MAX_NUM_WINDOWS];
    HI_U16  u16WindowUpperLeftCornerY[MAX_NUM_WINDOWS];
    HI_U16  u16WindowLowerRightCornerX[MAX_NUM_WINDOWS];
    HI_U16  u16WindowLowerRightCornerY[MAX_NUM_WINDOWS];
    HI_U16  u16CenterOfEllipseX[MAX_NUM_WINDOWS];
    HI_U16  u16CenterOfEllipseY[MAX_NUM_WINDOWS];
    HI_U8   u8RotationAngle[MAX_NUM_WINDOWS];
    HI_U16  u16SemimajorAxisInternalEllipse[MAX_NUM_WINDOWS];
    HI_U16  u16SemimajorAxisExternalEllipse[MAX_NUM_WINDOWS];
    HI_U16  u16SemiminorAxisExternalEllipse[MAX_NUM_WINDOWS];
    HI_U8   u8OverlapProcessOption[MAX_NUM_WINDOWS];

    HI_U32  u32_T_DisplayMaximumLuminance;

    HI_BOOL b_T_Display_APL_Flag;
    HI_U8   u8NumRows_T_Display_APL;
    HI_U8   u8NumCols_T_Display_APL;
    HI_U8   u8_T_display_APL[MAX_NUM_ROWS_T_DISPLAY_APL][MAX_NUM_COLS_T_DISPLAY_APL];

    HI_U32  u32MaxScl[MAX_NUM_WINDOWS][3];
    HI_U32  u32AverageMaxrgb[MAX_NUM_WINDOWS];
    HI_U8   u8NumDistMaxrgbPercentiles[MAX_NUM_WINDOWS];
    HI_U8   u8DistMaxrgbPercentages[MAX_NUM_WINDOWS][MAX_NUM_DIST_MAXRGB_PERCENTILES];
    HI_U32  u32DistMaxrgbPercentiles[MAX_NUM_WINDOWS][MAX_NUM_DIST_MAXRGB_PERCENTILES];
    HI_U16  u16FractionBrightPixels[MAX_NUM_WINDOWS];

    HI_BOOL b_M_Display_APL_Flag;
    HI_U8   u8NumRows_M_Display_APL;
    HI_U8   u8NumCols_M_Display_APL;
    HI_U8   u8_M_Display_APL[MAX_NUM_ROWS_M_DISPLAY_APL][MAX_NUM_COLS_M_DISPLAY_APL];

    HI_BOOL bToneMappingFlag[MAX_NUM_WINDOWS];
    HI_U16  u16KneePointX[MAX_NUM_WINDOWS];
    HI_U16  u16KneePointY[MAX_NUM_WINDOWS];
    HI_U8   u8NumBezierCurveAnchors[MAX_NUM_WINDOWS];
    HI_U16  u16bezierCurveAnchors[MAX_NUM_WINDOWS][MAX_NUM_BEZIER_CURVE_ANCHORS];

    HI_BOOL bColorSaturationMappingFlag[MAX_NUM_WINDOWS];
    HI_U8   u8ColorSaturationWeight[MAX_NUM_WINDOWS];
    HI_BOOL graphics_overlay_flag;
    HI_BOOL no_delay_flag;
} HI_DRV_VIDEO_HDR10PLUS_DYNAMIC_METADATA_S;


typedef struct hiDRV_VIDEO_HDR10PLUS_DYNAMIC_METADATA_STREAM_S
{
    HI_U32 u32Addr;
    HI_U32 u32Length;
} HI_DRV_VIDEO_HDR10PLUS_DYNAMIC_METADATA_STREAM_S;

typedef struct hiDRV_VIDEO_HDR10PLUS_INFO_S
{
    HI_BOOL                                          bStaticMetadataValid;
    HI_BOOL                                          bDynamicMetadataValid;
    HI_DRV_VIDEO_HDR10_INFO_S                        stStaticMetadata;
    HI_DRV_VIDEO_HDR10PLUS_DYNAMIC_METADATA_STREAM_S stDynamicMetadata;
} HI_DRV_VIDEO_HDR10PLUS_INFO_S;
#endif
...
typedef enum hiDRV_VIDEO_FRAME_TYPE_E
{
    ...
    HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR,   /*JTP_SL_HDR frame.*/
#ifdef HI_HDR10PLUS_SUPPORT
    HI_DRV_VIDEO_FRAME_TYPE_HDR10PLUS,      /* HDR10 Plus. */
#endif
    HI_DRV_VIDEO_FRAME_TYPE_BUTT
}HI_DRV_VIDEO_FRAME_TYPE_E;


typedef struct hiWinPLAYINFOR_S
{
    HI_U32  u32PlayCnt;
    HI_U32  u32FrameIndex;
} HI_FRAMEPLAYINFOR_S;

typedef enum hiDRV_VIDEO_AFD_TYPE_E
{
    ...
    HI_DRV_AFD_TYPE_BUTT
} HI_DRV_VIDEO_AFD_TYPE_E;

typedef struct hiDRV_DEI_MEM_INFO_S
{
    HI_U32 u32Width;
    HI_U32 u32Height;
    HI_U32 u32BitDepth;
    HI_U32 u32DeiMemSize;
    HI_U32 u32DeiMemPhyAddr;
} HI_DRV_DEI_MEM_INFO_S;

typedef enum hi_DRV_FASTPLAY_FLAG_E{
    HI_DRV_FASTPLAY_1X_SPEED = 0,
    HI_DRV_FASTPLAY_2X_SPEED,
    HI_DRV_FASTPLAY_4X_SPEED,
    HI_DRV_FASTPLAY_8X_SPEED,
    HI_DRV_FASTPLAY_16X_SPEED,
    HI_DRV_FASTPLAY_32X_SPEED,
    HI_DRV_FASTPLAY_FASTSYNC,
    HI_DRV_FASTSYNC_BUTT
}HI_DRV_FASTPLAY_FLAG_E ;

typedef enum hi_DRV_VIDEOPATH_PLAYCTRL_TYPE_E{
    HI_DRV_VIDEOPATH_FASTPLAY_CTRL = 0,
    HI_DRV_VIDEOPATH_PROGRESSIVE_INTERLEAVE_REVISE_CTRL,
    HI_DRV_VIDEOPATH_BUTT
}HI_DRV_VIDEOPATH_PLAYCTRL_TYPE_E;


typedef struct hi_DRV_PLAYCTRL_OPERATION_INFOR_S{
    HI_DRV_VIDEOPATH_PLAYCTRL_TYPE_E ePlayctrlType;
    union
    {
        HI_DRV_FASTPLAY_FLAG_E eFastPlayCtrl;
    } uPlayCtrlInfor;

}HI_DRV_PLAYCTRL_OPERATION_INFOR_S ;

/* last video frame flag */
typedef enum hiDRV_LAST_FRAME_FLAG_E
{
    HI_DRV_LAST_ERROR_FLAG = 0xff00,
    HI_DRV_LAST_FAKE_FLAG  = 0xff11,
    HI_DRV_LAST_FRAME_FLAG = 0xffee,
    HI_DRV_LAST_FLAG_BUTT
} HI_DRV_LAST_FRAME_FLAG_E;

typedef union hiDRV_VIDEO_HDR_INFO_U
{
    ...
    HI_DRV_VIDEO_JTP_SL_HDR_INFO_S stJTPSlHDRInfo;
#ifdef HI_HDR10PLUS_SUPPORT
    HI_DRV_VIDEO_HDR10PLUS_INFO_S stHDR10PlusInfo;  /* HDR10 Plus frame info */
#endif
} HI_DRV_VIDEO_HDR_INFO_U;
...
typedef struct hiDRV_VIDEO_FRAME_S
{
    ...
    HI_FRAMEPLAYINFOR_S              stPlayInfor;
    HI_DRV_LOWDELAY_STAT_INFO_S stLowdelayStat;
    HI_BOOL active_format_flag;
    HI_DRV_VIDEO_AFD_TYPE_E active_format;
