...
extern HI_U32   g_ReTry;


/*=================== MACRO ====================*/
#define OMXVDEC_VERSION (2018102400)
...
#define DBG_CMD_SET_RETRY                  "set_retry"
...
static HI_S32 omxvdec_ioctl_chan_create(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_from_user(&chan_cfg, (HI_VOID *)(HI_SIZE_T)pMsg->in, sizeof(OMXVDEC_DRV_CFG)))
    {
    ...
    if (copy_to_user((HI_VOID *)(HI_SIZE_T)pMsg->out, &chan_id, sizeof(HI_S32)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
...
static HI_S32 omxvdec_ioctl_chan_alloc_buffer(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_from_user(&user_buf, (HI_VOID *)(HI_SIZE_T)pMsg->in, sizeof(OMXVDEC_BUF_DESC)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
    ...
    if (copy_to_user((HI_VOID *)(HI_SIZE_T)pMsg->out, &user_buf, sizeof(OMXVDEC_BUF_DESC)))
    {
...
static HI_S32 omxvdec_ioctl_flush_port(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_from_user(&flush_dir, (HI_VOID *)(HI_SIZE_T)pMsg->in, sizeof(ePORT_DIR)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
...
static HI_S32 omxvdec_ioctl_chan_get_msg(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_to_user((HI_VOID *)(HI_SIZE_T)pMsg->out, &msg, sizeof(OMXVDEC_MSG_INFO)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
...
static HI_S32 omxvdec_ioctl_chan_port_enable(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_from_user(&bPortEnable, (HI_VOID *)(HI_SIZE_T)pMsg->in, sizeof(HI_BOOL)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
    ...
    ret = channel_set_port(pchan, bPortEnable);
...
static HI_S32 omxvdec_ioctl_chan_get_bypass_info(struct file *fd, OMXVDEC_IOCTL_MSG *pMsg)
{
    ...
    if (copy_to_user((HI_VOID *)(HI_SIZE_T)pMsg->out, &VpssBypassEn, sizeof(HI_BOOL)))
    {
        OmxPrint(OMX_FATAL, "%s: call copy_from_user failed!\n", __func__);
...
static HI_S32 omxvdec_proc_cmd_set_retrynum(HI_CHAR CmdStr[DBG_CMD_NUM][DBG_CMD_LEN])
{
    HI_U32 Data = 0;
...
static const PROC_COMMAND_NODE g_ProcCommandTable[] =
{
    ...
    {DBG_CMD_SET_RETRY,                omxvdec_proc_cmd_set_retrynum},
