...
static CAPTION_TTX_KEY_MAP_S s_stTtxKeyMap[] =
{
    ...
    {CAPTION_TTX_TTX_KEY_ZOOM, HI_UNF_TTX_KEY_ZOOM},
    {CAPTION_TTX_TTX_KEY_SUBPAGE, HI_UNF_TTX_KEY_SUBPAGE},
    {CAPTION_TTX_TTX_KEY_CANCEL, HI_UNF_TTX_KEY_CANCEL},
    {CAPTION_TTX_TTX_KEY_TIME, HI_UNF_TTX_KEY_TIME},
};
...
static S32 caption_subt_get_pts(U32 u32UserData, HI_S64* ps64CurrentPts)
{
    ...
    {
        HAL_ERROR("[%s, %d] status error!! subtitle status = 0x%08x\n", __FUNCTION__, __LINE__, s_stCaptionModule.stSubtInfo.enStatus);
        return FAILURE;
    }
...
static S32 caption_subt_inject_data(U8* pu8Data, U32 u32DataSize)
{
    ...
    //HAL_DEBUG("[%s, %d]  u32DataSize(%d)\n", __FUNCTION__, __LINE__, u32DataSize);
...
static S32 caption_ttx_inject_data(U8* pu8Data, U32 u32DataSize)
{
    ...
    {
        HAL_DEBUG("[%s, %d] status error!! ttx status = 0x%08x\n", __FUNCTION__, __LINE__, s_stCaptionModule.stTtxInfo.enStatus);
        return SUCCESS;
    }
...
static S32 caption_cc_inject_data(U8* pu8Data, U32 u32DataSize)
{
    ...
    {
        //HAL_ERROR("[%s, %d] status error!! cc status = 0x%08x\n", __FUNCTION__, __LINE__, s_stCaptionModule.stCCInfo.enStatus);
        return FAILURE;
    }
    ...
    switch (enCCDataType)
    {
        ...
        case CAPTION_CC_DATA_ARIB:
        {
            ...
            if (SUCCESS != s32Ret)
            {
                HAL_ERROR("[%s, %d] error!! s32Ret(0x%08x), hCC = 0x%x\n", __FUNCTION__, __LINE__, s32Ret, hCC);
                return FAILURE;
            }
        ...
        default:
        {
            HAL_ERROR("[%s, %d] error!! type(0x%08x)\n", __FUNCTION__, __LINE__, enCCDataType);
            return ERROR_INVALID_PARAM;
        }
    }
...
static S32 caption_subt_start(CAPTION_SUBT_PARAM_S* pstSubtParam)
{
    ...
    {
        HAL_ERROR("[%s, %d] error!! enSubtDataType(0x%08x)\n", __FUNCTION__, __LINE__, pstSubtParam->enSubtDataType);
        return ERROR_INVALID_PARAM;
    }
...
static S32 caption_subt_hide(VOID)
{
    ...
    s32Ret = caption_subt_output_clear(0, (VOID*)&stClear);
...
static S32 caption_subt_stop()
{
    ...
    if (CAPTION_STOPPED == s_stCaptionModule.stSubtInfo.enStatus)
    {
        HAL_DEBUG("[%s, %d] subt has stooped!! \n", __FUNCTION__, __LINE__);
        return SUCCESS;
    }
...
static S32 caption_ttx_callback(HI_HANDLE hTTX, HI_UNF_TTX_CB_E enCB, HI_VOID* pvCBParam)
{
    ...
    if ((HI_UNF_TTX_CB_APP_FILLRECT == enCB)
        || (HI_UNF_TTX_CB_APP_DRAWCHAR == enCB)
        || (HI_UNF_TTX_CB_APP_DRAWDRCSCHAR == enCB)
        || (HI_UNF_TTX_CB_APP_REFRESH == enCB))
    {
        if (CAPTION_HIDE == s_stCaptionModule.stTtxInfo.enStatus)
        {
            caption_ttx_output_clear();
            return HI_SUCCESS;
        }
    }
    ...
    {
        if (s_stCaptionModule.stTtxInfo.enStatus == CAPTION_HIDE && enCB == HI_UNF_TTX_CB_APP_REFRESH)
        {
            //not draw
        }
        else
...
static VOID caption_ttx_map_outtype_tounf(CAPTION_TTX_OUTPUT_TYPE_E enOutputType, HI_UNF_TTX_OUTPUT_E* penUnfOutputType)
{
    ...
    *penUnfOutputType = enOutput;
    ...
    HAL_DEBUG("[%s, %d]!! outtype(0x%08x)\n", __FUNCTION__, __LINE__, enOutput);
...
static S32 caption_ttx_start(CAPTION_TTX_PARAM_S* pstTTXParam)
{
    ...
    {
        HAL_ERROR("[%s, %d] error!! enDataSource(0x%08x)\n", __FUNCTION__, __LINE__,  pstTTXParam->enDataSource);
        return FAILURE;
    }
...
static S32 caption_ttx_switch(CAPTION_TTX_CONTENT_S* pstTTXContentParam)
{
    ...
    {
        HAL_ERROR("[%s, %d] status error!! ttx status = 0x%08x\n", __FUNCTION__, __LINE__, s_stCaptionModule.stTtxInfo.enStatus);
        return FAILURE;
    }
...
static S32 caption_ttx_hide(CAPTION_TTX_OUTPUT_TYPE_E enOutputType)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error false!! s32Ret(0x%08x),hTTX(0x%08x), enOutput(0x%08x)\n", __FUNCTION__, __LINE__, hTTX, s32Ret, enOutput);
        return FAILURE;
    }

#if 1
    caption_output_clear_surface();
...
static S32 caption_cc_map_608attr_tounf(CAPTION_CC608_ATTR_S st608Attr, HI_UNF_CC_608_CONFIGPARAM_S* pstUnf608Attr)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608DataType(0x%08x)\n", __FUNCTION__, __LINE__, st608Attr.enCC608DataType);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608TextOpac(0x%08x)\n", __FUNCTION__, __LINE__, st608Attr.enCC608TextOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608BgOpac(0x%08x)\n", __FUNCTION__, __LINE__, st608Attr.enCC608BgOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608FontStyle(0x%08x)\n", __FUNCTION__, __LINE__, st608Attr.enCC608FontStyle);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608DispFormat(0x%08x)\n", __FUNCTION__, __LINE__, st608Attr.enCC608DispFormat);
        return ERROR_INVALID_PARAM;
    }
...
static S32 caption_cc_map_708attr_tounf(CAPTION_CC708_ATTR_S st708Attr, HI_UNF_CC_708_CONFIGPARAM_S* pstUnf708Attr)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708ServiceNum(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708ServiceNum);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708FontName(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708FontName);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("caption_cc_map_fontstyle_tounf failed. enCC708FontStyle(0x%08x)\n", st708Attr.enCC708FontStyle);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708FontSize(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708FontSize);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708TextOpac(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708TextOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708BgOpac(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708BgOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708WinOpac(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708WinOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708TextEdgeType(0x%08x)\n", __FUNCTION__, __LINE__, st708Attr.enCC708TextEdgeType);
        return ERROR_INVALID_PARAM;
    }
...
static S32 caption_cc_map_608attr_tohal(HI_UNF_CC_608_CONFIGPARAM_S stUnf608Attr, CAPTION_CC608_ATTR_S* pst608Attr)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608TextOpac(0x%08x)\n", __FUNCTION__, __LINE__, stUnf608Attr.enCC608TextOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608BgOpac(0x%08x)\n", __FUNCTION__, __LINE__, stUnf608Attr.enCC608BgOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608FontStyle(0x%08x)\n", __FUNCTION__, __LINE__, stUnf608Attr.enCC608FontStyle);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC608DispFormat(0x%08x)\n", __FUNCTION__, __LINE__, stUnf608Attr.enCC608DispFormat);
        return ERROR_INVALID_PARAM;
    }
...
static S32 caption_cc_map_708attr_tohal(HI_UNF_CC_708_CONFIGPARAM_S stUnf708Attr, CAPTION_CC708_ATTR_S* pst708Attr)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708ServiceNum(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708ServiceNum);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708FontName(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708FontName);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708FontStyle(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708FontStyle);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708FontSize(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708FontSize);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708TextOpac(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708TextOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708BgOpac(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708BgOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708WinOpac(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708WinOpac);
        return ERROR_INVALID_PARAM;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCC708TextEdgeType(0x%08x)\n", __FUNCTION__, __LINE__, stUnf708Attr.enCC708TextEdgeType);
        return ERROR_INVALID_PARAM;
    }
...
static S32 caption_cc_stop(VOID)
{
    ...
    {
        HAL_DEBUG("[%s, %d] error!! cc already stopped\n", __FUNCTION__, __LINE__);
        return SUCCESS;
    }
...
static S32 caption_common_injectData(CAPTION_DEVICE_S* pstDev, CAPTION_TYPE_E enCaptionType, U8* pu8Data, U32 u32DataSize)
{
    ...
    switch (enCaptionType)
    {
        ...
        default:
        {
            HAL_ERROR("[%s, %d] error!! enCaptionType(0x%08x)\n", __FUNCTION__, __LINE__, enCaptionType);
            return ERROR_INVALID_PARAM;
        }
    }
...
static S32 caption_set_window(CAPTION_DEVICE_S* pstDev, U32 u32PosX, U32 u32PosY, U32 u32PosWidth, U32 u32PosHeight)
{
    ...
    {
        HAL_ERROR("[%s, %d] error!! ERROR_NOT_INITED\n", __FUNCTION__, __LINE__);
        return ERROR_NOT_INITED;
    }
...
#ifndef LINUX_OS //refer to caption_setsurface
...
S32 caption_setsurface(VOID* pvSurfaceToHal)
{
    return caption_output_set_surface(pvSurfaceToHal);
}
...
static S32 ttx_show(CAPTION_DEVICE_S* pstDev, CAPTION_TTX_OUTPUT_TYPE_E enOutputType)
{
    ...
    if ((CAPTION_BUTT == s_stCaptionModule.stTtxInfo.enStatus) ||
        (CAPTION_STOPPED == s_stCaptionModule.stTtxInfo.enStatus))
    {
        //HAL_ERROR("status error!! ttx status = 0x%08x\n", s_stCaptionModule.stTtxInfo.enStatus);
        return FAILURE;
    }
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! s32Ret(0x%08x),hTTX(0x%08x), enOutput(0x%08x)\n", __FUNCTION__, __LINE__, s32Ret, hTTX, enOutput);
        return FAILURE;
    }
...
static S32 cc_creat(CAPTION_DEVICE_S* pstDev, CAPTION_CC_PARAM_S* pstCCParam)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! s32Ret(0x%08x), enCCDataType(0x%08x)\n", __FUNCTION__, __LINE__, s32Ret, pstCCParam->enCCDataType);
        return FAILURE;
    }
...
static S32 cc_start(CAPTION_DEVICE_S* pstDev)
{
    ...
    if (INVALID_CAPTION_HANDLE == hCC)
    {
        HAL_ERROR("[%s, %d] error!! cc is not create.\n", __FUNCTION__, __LINE__);
        return FAILURE;
    }
...
static S32 cc_getAttr(CAPTION_DEVICE_S* pstDev, CAPTION_CC_ATTR_S* pstCCAttr)
{
    ...
    if (SUCCESS != s32Ret)
    {
        HAL_ERROR("[%s, %d] error!! enCCDataType(0x%08x)\n", __FUNCTION__, __LINE__, pstCCAttr->enCCDataType);
        return FAILURE;
    }
...
stCommon:
    ...
    tag:
        ...
        version_minor: 0,
    id:
        CAPTION_HARDWARE_MODULE_ID,
    name:          "Sample hwcomposer module"
        ,
    author:        "The Android Open Source Project"
        ,
    methods:
        & caption_module_methods,
    dso:
        NULL,
    reserved:
        {0},
