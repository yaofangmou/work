...
#define WIN_FRAME_MAX_HEIGHT 1088
...
#define WIN_ALG_MMZ_NUM            4
#define WIN_ALG_MMZ_SIZE           0x40000

#define FHD_WIDTH_VALUE            1920
#define FHD_HEIGHT_VALUE           1088
...
#define WIN_MAX_FRAME_PLAY_TIME  100 //20 time is enough for the normal situation, but we need to add some allowance here.
...
#define CHECK_LOOP_REACH_MAX(a,b)\
 do{\
    if(a > b)\
    {\
        WIN_FATAL("Abnormal, loop reach max:%d,may cause lock up!\n", b);\
        break;\
    }\
    a ++;\
}while(0)
...
typedef struct hiDRV_VIDEO_ALG_MMZ_S
{
    ...
    HI_BOOL   bReady;
    HI_U32    u32FrameYAddr;
    HI_U32    u32FrameIndex;
}HI_DRV_VIDEO_ALG_MMZ_S;

typedef struct tagCRITICAL_FrameInfor_S
{
    HI_RECT_S             stInSize;
    HI_RECT_S             stFrameSize;
    HI_RECT_S             stVideoSize;
    HI_RECT_S             stDispSize;
    HI_DRV_FIELD_MODE_E   enFieldMode;
    HI_BOOL               bTopFieldFirst;
    HI_BOOL               bProgressive;
    HI_BOOL               bLastFrame;
    HI_BOOL               bMask;
    HI_BOOL               bUndergoSuspend;
    HI_BOOL               bUndergoSuspend_ForQueue;
    HI_U32                u32LastQueueFrameAddr;
}WIN_CRITICAL_FrameInfor_S;

typedef struct tagWIN_Frame_HardwareConfig_INFOR_S
{
    DISP_MMZ_BUF_S           stDeiMmzBuf;
    DISP_MMZ_BUF_S           stVpssoutMmzBuf;
#ifdef XDP_SUPPORT_MEM_MAGIC
    hiXDP_OSAL_MEM_S         stDeiMmzBufMagicInfo;
#endif
    HI_DRV_VIDEO_ALG_MMZ_S   stNodeMMZ[WIN_ALG_MMZ_NUM];
    HI_DRV_VIDEO_ALG_MMZ_S   stVPSSMMZ[WIN_ALG_MMZ_NUM];
    HW_NODE_PRIVATE_DEBUG_INFOR_S *pstNodePrivateInfor[WIN_ALG_MMZ_NUM];
    HI_BOOL                  bVPSSAlgMMZBuf;
} WIN_Frame_HardwareConfig_INFOR_S;

typedef struct tagWIN_STATUS_S
{
    HI_BOOL bInQueueMode;
    HI_BOOL bInResetMode;
    HI_BOOL bInMaskMode;

    HI_BOOL                   bResetOccur;
    HI_DRV_WIN_SWITCH_E       enResetEventMode;
...
typedef struct tagWINDOW_S
{
    ...
    WIN_STATUS           stWinStatus;

    DISP_OSAL_SPIN  Spin;
    DISP_OSAL_LIST  ListHead;
    WIN_CRITICAL_FrameInfor_S stPlayInfor;
    WIN_Frame_HardwareConfig_INFOR_S stHardwareConfig;
    WIN_CRITICAL_PlayCtrl_S   stPlayCtrl;
    XDP_RWZB_S *pstRwzb;
} WINDOW_S;
