...
HI_S32 TdeHalInit(HI_U32 u32BaseAddr)
{
    ...
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, wmeminit, FAILURE_TAG);
        goto TDE_INIT_ERR;
    }
    ...
    {
        wmemterm();
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, TdeHalInitParaTable, FAILURE_TAG);
        goto TDE_INIT_ERR;
    }
...
HI_S32 TdeHalNodeInitNd(TDE_HWNode_S **pstHWNode)
{
    ...
    {
        wgetfreenum();
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, sizeof(TDE_HWNode_S) + TDE_NODE_HEAD_BYTE + TDE_NODE_TAIL_BYTE);
...
HI_S32 TdeHalNodeExecute(HI_U32 u32NodePhyAddr, HI_U64 u64Update, HI_BOOL bAqUseBuff)
{
    ...
    if (TdeHalCtlIsIdleSafely())
    {
        ...
        if (HI_TRUE ==  bAqUseBuff)
        {
            ...
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, TDE_AQ_CTRL_COMP_LIST);
            HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "Aq Ctrl use comp list mode");
        }
        else
        {
            unAqCtrl.stBits.u32AqOperMode = TDE_AQ_CTRL_COMP_LINE;
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, TDE_AQ_CTRL_COMP_LINE);
            HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "Aq Ctrl use comp node line mode");
        }
...
HI_VOID TdeHalNodeSetSrcMbY(TDE_HWNode_S *pHWNode, TDE_DRV_SURFACE_S *pDrvMbY, TDE_DRV_MB_OPT_MODE_E enMbOpt)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pHWNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pDrvMbY);
...
HI_VOID TdeHalNodeSetSrcMbCbCr(TDE_HWNode_S *pHWNode, TDE_DRV_SURFACE_S *pDrvMbCbCr, TDE_DRV_MB_OPT_MODE_E enMbOpt)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pHWNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pDrvMbCbCr);
...
static HI_VOID TdeHalNodeSetYcbcrColorKeyPara(TDE_HWNode_S *pHWNode, TDE_DRV_COLORKEY_CMD_S *pColorKey, TDE_ALU_U *unAluMode)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(unAluMode);
...
HI_S32 TdeHalNodeSetFlicker(TDE_HWNode_S *pHWNode, TDE_DRV_FLICKER_CMD_S *pFlicker)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pHWNode, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pFlicker, HI_FAILURE);

    TdeHalGetCapability(&u32Capability);

    if (!(u32Capability & DEFLICKER))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, DEFLICKER);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32Capability);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "It deos not support Deflicker");
        return HI_ERR_TDE_UNSUPPORTED_OPERATION;
    }
...
HI_S32 TdeHalNodeSetResize(TDE_HWNode_S *pHWNode, TDE_FILTER_OPT *pstFilterOpt, TDE_NODE_SUBM_TYPE_E enNodeType)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pHWNode, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstFilterOpt, HI_FAILURE);

    TdeHalGetCapability(&u32Capability);
    if (!(u32Capability & RESIZE))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, RESIZE);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32Capability);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "It deos not support Resize");
        return HI_ERR_TDE_UNSUPPORTED_OPERATION;
    }
...
HI_VOID TdeHalNodeAddChild(TDE_HWNode_S *pHWNode, TDE_CHILD_INFO *pChildInfo)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pHWNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pChildInfo);
...
HI_VOID TdeHalNodeSetMbMode(TDE_HWNode_S *pHWNode, TDE_DRV_MB_CMD_S *pMbCmd)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pHWNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pMbCmd);
...
HI_S32 TdeHalNodeSetCsc(TDE_HWNode_S *pHWNode, TDE2_CSC_OPT_S stCscOpt)
{
    ...
    if (unIns.stBits.u32IcsConv)
    {
        ...
        else
        {
            ...
            HI_GFX_CHECK_LEFT_EQUAL_RIGHT_RETURN_VALUE(0, pHWNode->u32TDE_ICSC_ADDR, HI_ERR_TDE_NO_MEM);
    ...
    if (unIns.stBits.u32OcsConv)
    {
        ...
        else
        {
            ...
            HI_GFX_CHECK_LEFT_EQUAL_RIGHT_RETURN_VALUE(0, pHWNode->u32TDE_OCSC_ADDR, HI_ERR_TDE_NO_MEM);
...
HI_S32 TDE_OSR_Init(HI_VOID)
{
    ...
    {
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "THE hal init failed");
        return -1;
    }
...
HI_VOID TdeHalNodePrintInfo(struct seq_file *p, HI_U32 *pu32CurNode)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pu32CurNode);
