...
static HI_S32 VDEC_DRV_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (pstChan)
    {
        ...
        PROC_PRINT(p, "-----------------------Alloc/Free Frame time-----------------------\n");
        PROC_PRINT(p, "Frame_num: %d\n", pstChan->stRecordFrameTime.u32RecordNum);
        for(i=0; i<pstChan->stRecordFrameTime.u32RecordNum; i++)
        {
           PROC_PRINT(p, "%2d --- alloc, %d, %d, %d ms\n", i, pstChan->stRecordFrameTime.u32AllocStartSysTime[i],
            pstChan->stRecordFrameTime.u32AllocEndSysTime[i],
            pstChan->stRecordFrameTime.u32AllocEndSysTime[i] - pstChan->stRecordFrameTime.u32AllocStartSysTime[i]);
        }
        for(i=0; i<pstChan->stRecordFrameTime.u32FreeNum; i++)
        {
           PROC_PRINT(p, "%2d --- free, %d, %d, %d ms\n", i, pstChan->stRecordFrameTime.u32FreeStartTime[i],
           pstChan->stRecordFrameTime.u32FreeEndTime[i],
           pstChan->stRecordFrameTime.u32FreeEndTime[i] - pstChan->stRecordFrameTime.u32FreeStartTime[i]);
        }
        PROC_PRINT(p, "FirstI cfg - report %d ms\n", pstChan->stRecordFrameTime.u32ConfigTime - pstChan->stRecordFrameTime.u32ReportTime);
        PROC_PRINT(p, "allocFailed: %d, Waittime: %d\n", pstChan->stDfsInfo.u32AllocFailed, pstChan->stDfsInfo.u32WaitTime);
        PROC_PRINT(p, "vfmw->vpss: %d ms, vpss->avplay: %d ms\n", pstChan->stRecordFrameTime.u32VpssDelayTime, pstChan->stRecordFrameTime.u32AvplayDelayTime);

      if(pstChan->stRecordFrameTime.u32RecvNum > 0)
      {
        PROC_PRINT(p, "--------------------------Recv Frame time---------------------------\n");
        PROC_PRINT(p, "%2d frame: %d ms\n", 0, pstChan->stRecordFrameTime.u32VpssDelayTime);
        for(i = 0; i < pstChan->stRecordFrameTime.u32RecvNum - 1; i++)
        {
            PROC_PRINT(p, "%2d frame: %d ms\n", i + 1, pstChan->stRecordFrameTime.u32RecvTime[i+1] - pstChan->stRecordFrameTime.u32RecvTime[i]);
        }
        PROC_PRINT(p, "=======================================================================\n");
      }
    }
