...
void __list_add(struct list_head *pnew,struct list_head *pprev,struct list_head *pnext)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pnew);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pprev);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pnext);
        return;
    }
    pnext->prev = pnew;
    pnew->next  = pnext;
...
void list_del(struct list_head *pentry)
{
    if (NULL == pentry)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pentry);
        return;
    }
...
int list_empty(struct list_head *pHead)
{
    if (NULL == pHead)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pHead);
        return -1;
    }
