...
static HI_BOOL EngineIsSystemTrackAlone(HI_VOID)
{
    ...
    for (enSourceID = SOURCE_ID_MAIN; enSourceID < SOURCE_ID_BUTT; enSourceID++)
    {
        ...
        {
            HI_INFO_AO("enState:%d bDataValid:%d\n", pstSource->enState, pstSource->bDataValid);
...
static HI_VOID EngineUpdateStreamInfo(SOURCE_MANAGER_S* pstSourceManager, HA_MS12_STREAM_S* pstMS12DecodeStream, HA_MS12_PROC_S* pstProc)
{
    ...
    for (enSourceID = SOURCE_ID_MAIN; enSourceID <= SOURCE_ID_MAIN; enSourceID++)
    {
        ...
        pstSource->stRenderEvent.stStreamInfo.enSampleRate = pstProc->stStreamProc[enSourceID].u32SampleRate;
        pstSource->stRenderEvent.stStreamInfo.bLfePresent  = pstProc->stStreamProc[enSourceID].bLfeExist;
    ...
    for (enSourceID = SOURCE_ID_MAIN; enSourceID < SOURCE_ID_BUTT; enSourceID++)
    {
        ...
        if (u32ConsumedByte > 0)
        {
            if (SOURCE_ID_EXTDEC == enSourceID)
            {
                ...
                pstSource->u32DecoderPcmDelay = pstMS12DecodeStream->stOutput.u32PcmDelayMs[enSourceID];//update pcm delay, for get delay ms
            }
            else
            {
...
HI_S32 Engine_SetTrackChannelMode(HI_UNF_TRACK_MODE_E enMode)
{
    return Sink_SetTrackChannelMode(enMode);
}

HI_S32 Engine_GetTrackChannelMode(HI_UNF_TRACK_MODE_E* penMode)
{
    return Sink_GetTrackChannelMode(penMode);
}
