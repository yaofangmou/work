...
#ifdef VPSS_SUPPORT_PROC_V2
        pstInstance->aulSrcReadyTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
        return HI_SUCCESS;
    }
    else
...
HI_S32 VPSS_CTRL_SetHalBufManageMode(VPSS_INSTANCE_S *pstInstance, VPSS_HAL_INFO_S *pstHalInfo)
{
    HI_BOOL bIsPriMode = HI_TRUE;
    HI_U32 u32PortId;
    VPSS_PORT_S *pstPortInfo = NULL;
    VPSS_CHECK_NULL(pstInstance);
    VPSS_CHECK_NULL(pstHalInfo);
    for (u32PortId = 0; u32PortId < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32PortId++)
    {
        pstPortInfo = &(pstInstance->stPort[u32PortId]);
        if (pstPortInfo->bEnble == HI_TRUE)
        {
            if (pstPortInfo->stFrmInfo.stBufListCfg.eBufType == HI_DRV_VPSS_BUF_USER_ALLOC_MANAGE)
            {
                bIsPriMode = HI_FALSE;
                break;
            }
    ...
    pstHalInfo->bIsBufPrivManage =  bIsPriMode;
    return HI_SUCCESS;
}
#endif
...
HI_S32 VPSS_CTRL_FixTask(VPSS_IP_E enIp, HI_DRV_BUF_ADDR_E enLR, VPSS_INSTANCE_S *pstInstance)
{
    ...
    pstHalInfo->bAndroidCts = pstInstance->bAndroidCts;
...
#endif
    {
        ...
        stVpssWbcInfo.pstVPSSWbcReg = (HI_VOID *)(&(pstInstance->stPqWbcReg[0]));
        ...
        stVpssCfgInfo.s32FieldOrder = VPSS_FIELDORDER_UNKNOWN;
...
#ifdef VPSS_RWZB_FASTCHECK_MODE
    VPSS_CTRL_SetHalBufManageMode(pstInstance, pstHalInfo);
#endif
#if !defined(HI_NVR_SUPPORT)
    {
        HI_S32          s32SrcId;
...
HI_VOID VPSS_CTRL_CopyWBCStatistic(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if (HI_NULL != pstInstance->stVpssHalInfo.pu8stt_w_vir_addr)
    {
        VPSS_SAFE_MEMCPY(&(pstInstance->stPqWbcReg),
                         (S_VPSS_STT_REGS_TYPE *)pstInstance->stVpssHalInfo.pu8stt_w_vir_addr,
                         sizeof(S_VPSS_STT_REGS_TYPE));
    }
#endif
    ...
    */
    ...
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
    {
            ...
            pstPort->bEnble == HI_TRUE)
        {
            ...
            {
                HI_RECT_S stInRect;
                VPSS_INST_GetInCropFromSrcWH(pstInstance, u32Count, &stInRect);
...
HI_S32 VPSS_CTRL_UpdatePqWbcData(VPSS_INSTANCE_S *pstInstance, S_VPSS_STT_REGS_TYPE *pstLocalWbcReg)
{
    ...
    stPqWbcInfo.pstVPSSWbcReg = (HI_VOID *)(&pstInstance->stPqWbcReg[0]);
    ...
    stPqWbcInfo.u32FrameIndex = pstInstance->stVpssHalInfo.u32DetileFieldIdx;
...
HI_S32 VPSS_CTRL_CompleteTask(VPSS_IP_E enIp, VPSS_INSTANCE_S *pstInstance)
{
...
#if !defined(HI_NVR_SUPPORT)
#ifdef VPSS_HAL_WITH_CBB
    VPSS_CTRL_UpdatePqWbcData(pstInstance, (S_VPSS_STT_REGS_TYPE *)pstInstance->stVpssHalInfo.stSttAddr.pu8stt_w_vir_addr);
#else
    VPSS_CTRL_UpdatePqWbcData(pstInstance, (S_VPSS_STT_REGS_TYPE *)pstInstance->stVpssHalInfo.pu8stt_w_vir_addr);
#endif
#endif
    ...
    if (pstInstance->u32TimeStampIndex == 0)
    {
        //reserve node0 for first frame
        pstInstance->u32TimeStampIndex = (pstInstance->u32TimeStampIndex + 1) % VPSS_PROC_TIMESTAMP_MAXCNT;
    }
#endif
    /* Clear new image flag to avoid call twice DRV_PQ_UpdateVpssPQ() in
     VPSS_INST_CheckUndoImage() when play interlace video*/
    pstInstance->stInEntity.stStreamInfo.u32IsNewImage = HI_FALSE;
    return HI_SUCCESS;
}
...
#if 0
                    if (pstTmpFrame->u32FrameIndex < 4)
                    {
                        char chFile[128] = {0};
                        HI_OSAL_Snprintf(chFile, DEF_FILE_NAMELENGTH, "vpss_%dx%d_%d.yuv",
                                         pstTmpFrame->u32Width, pstTmpFrame->u32Height, pstTmpFrame->u32FrameIndex);
                        VPSS_OSAL_WRITEYUV(pstTmpFrame, chFile);
                    }
#endif
                }
                else
...
#ifdef VPSS_SUPPORT_PROC_V2
            pstInstance->aulOutBufReadyTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
...
#ifdef VPSS_SUPPORT_PROC_V2
                pstInstance->aulInstReadyTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
