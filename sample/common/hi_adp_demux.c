...
HI_S32 DMX_SectionStartDataFilterExt(HI_U32 u32DmxId, DMX_DATA_FILTER_S * pstDataFilter, HI_BOOL bTeeEnable)
{
    HI_U32 i;
    HI_U8 *p;
    ...
    if (HI_TRUE == bTeeEnable)
    {
        stChanAttr.u32BufSize    = 2 * 1024 * 1024;
        stChanAttr.enSecureMode  = HI_UNF_DMX_SECURE_MODE_TEE;
    }
    else
    {
...
{
    return DMX_SectionStartDataFilterExt(u32DmxId, pstDataFilter, HI_FALSE);
}
