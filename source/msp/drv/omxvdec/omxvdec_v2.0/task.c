...
#define MIN_LIMIT_MEM (150 * 1024 * 1024)
...
static HI_S32 task_alloc_buf_for_slot(OMXVDEC_CHAN_CTX *pchan, DSF_SINGLE_BUF_S *pBufSlot, FRAME_BUF_S *pstFrameBuf, ALLOC_BUF_TYPE_E type)
{
    ...
    HI_U32 AllocStartTime = 0;
    HI_U32 AllocEndTime = 0;
    HI_U32 WaitStartTime = 0;
    HI_U32 WaitEndTime = 0;
    //unsigned long FreeMemSize = 0;
    //unsigned long LimitMemSize = 0;
    ...
    spin_unlock_irqrestore(&pchan->dfs.dfs_lock, flags);
#if 0
    HI_DRV_SYS_GetTimeStampMs(&WaitStartTime);

    while (pchan->dfs.retry_count < g_ReTry)
    {
        ret = HI_DRV_SMMU_GetSysPoolSize(&FreeMemSize, &LimitMemSize);
        if (ret != HI_SUCCESS)
        {
            OmxPrint(OMX_ERR, "%s call HI_DRV_SMMU_GetSysPoolSize failed\n", __func__);
        }
        else
        {
            OmxPrint(OMX_ERR, "%s LimitMemSize:%ld FreeMemSize:%ld\n", __func__, LimitMemSize, FreeMemSize);
        }

        if ((FreeMemSize > NeededAllocSize) || (LimitMemSize < MIN_LIMIT_MEM))
        {
            break;
        }
        ...
        pchan->dfs.retry_count++;
    }
    HI_DRV_SYS_GetTimeStampMs(&WaitEndTime);

    if (pchan->dfs.retry_count == g_ReTry)
    {
        pchan->dfs.alloc_from_cached_failed++;
    }

    pchan->dfs.retry_count = 0;
#endif

    HI_DRV_SYS_GetTimeStampMs(&AllocStartTime);
    ...
    HI_DRV_SYS_GetTimeStampMs(&AllocEndTime);
    ...
    if (type == ALLOC_FRM)
    {
        ...
        pBufSlot->frame_wait_time = WaitEndTime - WaitStartTime;
        pBufSlot->alloc_frame_time = AllocEndTime - AllocStartTime;
    ...
    else
    {
        ...
        pBufSlot->pmv_wait_time = WaitEndTime - WaitStartTime;
        pBufSlot->alloc_pmv_time = AllocEndTime - AllocStartTime;
