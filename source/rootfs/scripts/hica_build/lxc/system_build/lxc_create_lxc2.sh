...
then
    ...
    echo -e "        for example: ./lxc_create_lxc2.sh DEBUG /home/user/HiSTBLinuxV100R002/out/hi3716mv410/hi3716m41dma_ca_debug/rootbox stb arm-histbv300-linux"
    exit 1
fi

#===========prepare environment=================================================
#fetch configure
source $5/lxc2_cas_service.config
...
{
    echo  "=========$CUR_PATH/lxc2_cas_service.config=============="
...
cak:x:1003:1003:Linux User,,,:/home:/bin/sh
EOF
...
iptables -A INPUT -i eth0 -p tcp -m tcp --sport 8888 -d $LXC2_ADDR/32 -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A INPUT -i eth0 -p tcp -m tcp --sport 80 -d $LXC2_ADDR/32 -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
...
iptables -A OUTPUT -o eth0 -p tcp -m tcp --dport 8888  -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
...
EOF
    ...
    if [[ $LXC_DEV_ETHERNET_ENABLE == y* ]]; then
        ...
        echo "lxc.network.hwaddr = 4a:49:43:50:79:bf" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.ipv4 = 192.168.2.2/24" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.name = eth0" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.ipv4.gateway = 192.168.2.3" >> $LXC_CONFIGURE_FILE
    fi
