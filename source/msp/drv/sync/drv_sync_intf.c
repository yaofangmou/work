...
HI_VOID SYNC_PrintDebugInfo(SYNC_S *pSync, HI_VOID *pPara)
{
    HI_CHAR *SyncProcMode;
    struct list_head *pos = HI_NULL;
    struct list_head *n = HI_NULL;
    SYNC_DBGINFO_S   *pSyncDbgInfo = HI_NULL;

    struct seq_file *p = (struct seq_file *)pPara;
    ...
    PROC_PRINT(p, "%8s |"
                  ...
                  "VidPts",     "AudPts",
                  "VidAudDiff",
                  "VidLocTime", "AudLocTime",
                  "PcrLocTime", "PcrLast",
                  "VidPcrDiff", "AudPcrDiff",
                  "VidDelay",
                  "AudDelay",
                  "VidFrmTime", "AudFrmTime",
                  "VidSrcPts",  "AudSrcPts",
                  "PcrDelt",
                  "DispCnt",
                  "Index"
    ...
    list_for_each_safe(pos, n, &pSync->SyncDbgInfoListHead)
    {
        ...
        switch (pSyncDbgInfo->SyncProc)
        {
            case SYNC_PROC_DISCARD :
                SyncProcMode = "DISCARD";
                break;
        ...
        PROC_PRINT(p, "%8s |"
                      ...
                      "%10d |\n",
            SyncProcMode,
            pSyncDbgInfo->SystemTime,
            pSyncDbgInfo->VidPts, pSyncDbgInfo->AudPts,
            pSyncDbgInfo->VidAudDiff,
            pSyncDbgInfo->VidLocalTime, pSyncDbgInfo->AudLocalTime,
            pSyncDbgInfo->PcrLocalTime, pSyncDbgInfo->PcrLast,
            pSyncDbgInfo->VidPcrDiff,   pSyncDbgInfo->AudPcrDiff,
            pSyncDbgInfo->VidDelayTime,
            pSyncDbgInfo->AudBufTime,
            pSyncDbgInfo->VidFrameTime, pSyncDbgInfo->AudFrameTime,
            pSyncDbgInfo->VidSrcPts,    pSyncDbgInfo->AudSrcPts,
            pSyncDbgInfo->PcrDeltaTime,
            pSyncDbgInfo->DispTime,
            pSyncDbgInfo->Idx);
    }
...
static HI_S32 SYNC_ProcRead(struct seq_file *p, HI_VOID *v)
{
    ...
    if ((pSync->PcrSyncInfo.enPcrAdjust == SYNC_PCR_ADJUST_SCR) && (pSync->SyncAttr.enSyncRef == HI_UNF_SYNC_REF_PCR))
    {
...
#endif
#ifdef SYNC_DEBUG_DETAIL
    if (HI_TRUE == pSync->IsShowDebugDetail)
    {
        PROC_PRINT(p,"____________________________SYNC Adjust Detail____________________________\n");
        SYNC_PrintDebugInfo(pSync, p);
    }
#endif
    return HI_SUCCESS;
}
...
#ifdef SYNC_DEBUG_DETAIL
    HI_DRV_PROC_EchoHelper("echo SyncLogDepth = xxx > /proc/msp/syncxx\n");
    HI_DRV_PROC_EchoHelper("echo SyncLogDetail = on|off > /proc/msp/syncxx\n");
#endif
}
...
static HI_S32 SYNC_ProcWrite(struct file * file, const char __user * buf, size_t count, loff_t *ppos)
{
    ...
    HI_CHAR           *pItem = HI_NULL,*pValue = HI_NULL;
    HI_S32            Ret;
...
#ifdef SYNC_DEBUG_DETAIL
    else if (0 == HI_OSAL_Strncmp(pItem, "SyncLogDetail", strlen("SyncLogDetail")))
    {
        if (0 == HI_OSAL_Strncmp(pValue, "on", strlen("on")))
        {
            pSync->IsShowDebugDetail = HI_TRUE;
        }
        else if (0 == HI_OSAL_Strncmp(pValue, "off", strlen("off")))
        {
            pSync->IsShowDebugDetail = HI_FALSE;
        }
    }
    else if (0 == HI_OSAL_Strncmp(pItem, "SyncLogDepth", strlen("SyncLogDepth")))
    {
        HI_U32 ListSize = simple_strtol(pValue, NULL, 10);

        SYNC_ProcDebugInfo(pSync, SYNC_DBG_INFO_LIST_SIZE, (HI_VOID *)&ListSize);
    }
#endif
    else
    {
