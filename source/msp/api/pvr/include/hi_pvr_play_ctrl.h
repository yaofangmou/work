...
#define PVR_TRICK_WITH_AUDIO_SUPPORT
#endif

#define PVR_TRICK_WITH_AUDIO_ENABLE_MASK 0x0F000000
...
#define  PVR_PLAY_READ_FILE(pu8Addr, offset, size, pChnAttr) \
    ...
    {\
        ssize_t  n;\
        if ((n = PVR_PREAD64(pu8Addr, (size), pChnAttr->s32DataFile, (offset))) == -1)\
        {\
            if (NULL != &errno)\
            {\
                if (EINTR == errno)\
...
typedef struct hiPVR_PLAY_CHN_S
{
    ...
    HI_BOOL                 bChangePID;             /*Determine whether the current playback channel is switching PID*/
#endif
...
} PVR_PLAY_CHN_S, *PVR_PLAY_HANDLE;
