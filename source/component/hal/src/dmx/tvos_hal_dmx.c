...
#define HAL_DMX_CHK_IV_ID(dmxId) \
    do                                                      \
    {                                                       \
        if (dmxId != (U32)DMX_IV_IDLEN)                                  \
        {                                                   \
            DMX_LOG_ERROR("INVALID IV ID: %d, TRUE IV ID: %d.\n", dmxId, DMX_IV_IDLEN);  \
            return ERROR_INVALID_PARAM;                          \
        };                                                  \
    } while (0)

#define HAL_DMX_CHK_DEMUX_ID(dmxId) \
    do                                                      \
    {                                                       \
        if ((U32)dmxId >= DMX_ID_BUTT)                                  \
        {                                                   \
            DMX_LOG_ERROR("INVALID DMX ID %d, Total Dmx Num %d.", dmxId, s_u32DmxNum);  \
            return ERROR_DMX_INVALID_DMX_ID;                          \
        };                                                  \
...
DMX_KEY_S*  dmxNewKeyInfo(const DMX_ID_E enDemuxId, const HI_HANDLE hKey, const DMX_DESC_ASSOCIATE_MODE_E enMode)
{
    ...
    *p = q;
    ...
    {
        q->stAssocviateInfo.DMX_ASSOCIATE_U.u16Pid =  DMX_INVALID_PID;
    }
    else
    {
        q->stAssocviateInfo.DMX_ASSOCIATE_U.u32ChannelId =  DMX_INVALID_CHN_ID;
    }
...
S32 dmx_disconnect(struct _DEMUX_DEVICE_S* pstDev, const DMX_ID_E enDemuxId)
{
    ...
    HAL_DMX_CHK_INIT();
    HAL_DMX_CHK_DEMUX_ID(enDemuxId);
...
S32 dmx_channel_open(struct _DEMUX_DEVICE_S* pstDev, const DMX_ID_E enDemuxId, U32* const pu32ChannelId, const DMX_CHANNEL_OPEN_PARAM_S* const pstOpenParams)
{
    ...
    if (pstOpenParams->u32Pid < DMX_INVALID_PID)
    {
...
S32 dmx_channel_set_pid(struct _DEMUX_DEVICE_S* pstDev, const U32 u32ChannelId, const U16 u16Pid)
{
    ...
    if (u16Pid > DMX_INVALID_PID)
    {
        u32TempPid = DMX_INVALID_PID;
...
S32 dmx_avfilter_open(struct _DEMUX_DEVICE_S* pstDev, const DMX_ID_E enDemuxId, U32* const pu32AVFilterId, const DMX_PARSER_FILTER_OPEN_PARAM_S* const pstFilterOpenPara)
{
    ...
    if (TRUE == pstFilterOpenPara->bSecureChannel)
    {
        stChnAttr.enSecureMode = HI_UNF_DMX_SECURE_MODE_TEE;
    }
    else
    {
        stChnAttr.enSecureMode = HI_UNF_DMX_SECURE_MODE_NONE;
    }
    ...
    if (DMX_INVALID_PID == pstFilterOpenPara->u32Pid)
    {
...
S32 dmx_avfilter_get_esframe(struct _DEMUX_DEVICE_S* pstDev, const U32 u32AVFilterId, DMX_ESFRAME_INFO_S* pstFrameInfo)
{
    ...
    pstFrameInfo->u64BufKerVirAddr = stEsBuf.stPriEsBufAddr.u64BufKerVirAddr;
    pstFrameInfo->u64BufPhyAddr = stEsBuf.stPriEsBufAddr.u64BufPhyAddr;

    return SUCCESS;
}

S32 dmx_avfilter_get_info(struct _DEMUX_DEVICE_S* pstDev, const U32 u32AVFilterId, DMX_AVFILTER_INFO_S* pstAvFilterInfo)
{
    S32 s32Ret = FAILURE;
    HI_UNF_DMX_CHAN_ATTR_S stChAttr = {0};

    HAL_DMX_CHK_INIT();
    HAL_DMX_CHK_PARA(pstAvFilterInfo);

    s32Ret = HI_UNF_DMX_GetChannelAttr(u32AVFilterId, &stChAttr);
    if (SUCCESS != s32Ret)
    {
        DMX_LOG_ERROR("HI_UNF_DMX_GetChannelAttr error ret = %#x \n", s32Ret);
        return s32Ret;
    ...
    if (stChAttr.enSecureMode != HI_UNF_DMX_SECURE_MODE_TEE)
    {
        pstAvFilterInfo->bSecureChannel = FALSE;
    }
    else
    {
        pstAvFilterInfo->bSecureChannel = TRUE;
    }

    pstAvFilterInfo->u32BufSize = stChAttr.u32BufSize;
    pstAvFilterInfo->enChannelType = dmxConvert2HalChnType(stChAttr.enChannelType);
...
S32 dmx_avfilter_release_esframe(struct _DEMUX_DEVICE_S* pstDev, const U32 u32AVFilterId, DMX_ESFRAME_INFO_S* pstFrameInfo)
{
    ...
    stEsBuf.stPriEsBufAddr.u64BufKerVirAddr = pstFrameInfo->u64BufKerVirAddr;
    stEsBuf.stPriEsBufAddr.u64BufPhyAddr = pstFrameInfo->u64BufPhyAddr;
                           ...
                           struct hw_device_t** device)
{
    ...
    pstDev->dmx_avfilter_get_info = dmx_avfilter_get_info;
...
static DEMUX_DEVICE_S s_stDmxDev=
{
    ...
    .dmx_avfilter_get_info                   = dmx_avfilter_get_info,
