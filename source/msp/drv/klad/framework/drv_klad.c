...
irqreturn_t DRV_KLAD_ISR(HI_S32 irq, HI_VOID *devId);
#endif
...
#endif
#ifdef HI_KLAD_INT_SUPPORT
    init_waitqueue_head(&(klad_session_list.klad_wait_queue));
    klad_session_list.wake_up = HI_FALSE;
    /* request irq */
    if(0 != osal_request_irq(HKL_IRQ_NUMBER,(irq_handler_t) DRV_KLAD_ISR, IRQF_SHARED, HKL_IRQ_NAME, (HI_VOID *)&klad_session_list))
    {
        KLAD_PrintErrorCode(HI_ERR_KLAD_REQUIRE_IRQ_FAILED);
        return HI_ERR_KLAD_REQUIRE_IRQ_FAILED;
    }
    HAL_KLAD_IntEn();
#endif
    KLAD_FUNC_EXIT();
...
{
    HI_S32 int_stat = 0;
    DRV_KLAD_SESSION_LIST *session_list = (DRV_KLAD_SESSION_LIST*)devId;

    int_stat = HAL_KLAD_GetIntSta();
    HAL_KLAD_IntDis();
    HAL_KLAD_IntClr();
    /*if dma obuf int happen*/
    if ( int_stat & 0x1 )
    {
        klad_session_list.wake_up = HI_TRUE;
        wake_up_interruptible(&(session_list->klad_wait_queue));
    }
    HAL_KLAD_IntEn();
    return IRQ_HANDLED;
}
...
HI_S32 DRV_KLAD_Exit(HI_VOID)
{
    ...
    KLAD_FUNC_ENTER();
#ifdef HI_KLAD_INT_SUPPORT
    osal_free_irq(HKL_IRQ_NUMBER, HKL_IRQ_NAME, (HI_VOID *)&klad_session_list);
#endif
...
HI_S32 DRV_KLAD_Ioctl(struct inode* inode, struct file* file, unsigned int cmd, void* arg)
{
    ...
    switch (cmd)
    {
        ...
        case CMD_KLAD_SET_CONTENT_KEY:
            ...
            HI_ERR_KLAD("keyladder enter:%dus->exit:%dus==>time:%dus.\n", tv.tv_usec, tv_1.tv_usec, (tv_1.tv_usec + 1000000 - tv.tv_usec)%1000000);
#endif
            break;
        default:
            ret = DRV_KLAD_RunCACallBack(cmd, arg);
...
#ifdef HI_CAS_OPTIMIZE_REPEAT_SET
static HI_S32 EXT_KLAD_KeyStore(EXT_CLEAR_CW_KLAD_PARA_S *stClearKey)
{
    DRV_KLAD_SESSION    klad_session;

    memset(&klad_session, 0x00, sizeof(DRV_KLAD_SESSION));

    KLAD_CHECK_POINTER(stClearKey);
    klad_session.instance.target = stClearKey->handle;
    klad_session.instance.attr_flag = HI_TRUE;
    klad_session.instance.attr.klad_type = HI_UNF_KLAD_TYPE_CLEARCW;
    klad_session.instance.attr.engine = stClearKey->enEngine;
    klad_session.instance.content_key_flag = HI_TRUE;
    klad_session.instance.content_key.odd = stClearKey->bOdd;
    memcpy(klad_session.instance.content_key.key, stClearKey->u8Key, HI_UNF_KLAD_MAX_KEY_LEN);
...
#ifdef HI_CAS_OPTIMIZE_REPEAT_SET
    ret = EXT_KLAD_KeyStore(stClearKey);
    if(HI_SUCCESS != ret)
    {
        KLAD_PrintErrorFunc(EXT_KLAD_KeyStore,ret);
    }
#endif
