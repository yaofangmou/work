...
static MXL661_XTAL_SPUR_CFG_T MxL661xtalSpurCfg[MAX_MXL661_DEVICES] ;

static MXL661_IF_FCW_CFG_T MxL661ifFcwCfg[MAX_MXL661_DEVICES] ;
...
static MXL_STATUS MxL661_ConfigDevReset(PMXL661_RESET_CFG_T resetCfg)
{
  ...
  status = MxLWare661_OEM_WriteRegister(resetCfg->I2cSlaveAddr, AIC_RESET_REG, 0x00);
   if(MXL_TRUE != status)
  {
      MxL_DLL_DEBUG0("=====Call MxLWare661_OEM_WriteRegister failure!\n");
     // goto TUNER_CLOSE;
  }
    ...
    switch(modeSetPtr->SignalMode)
    {
      ...
      case MXL661_DIG_DVB_T_DTMB:
        ...
        if (modeSetPtr->IFOutFreqinKHz < HIGH_IF_35250_KHZ)
        {
          // Low power IF dependent settings
          status |= MxLWare661_OEM_WriteRegister(modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_0, 0xFE);
          status |= MxLWare661_OEM_WriteRegister(modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_CFG_1, 0x10/*0x18*/);
          status |= MxLWare661_OEM_WriteRegister(modeSetPtr->I2cSlaveAddr, DIG_ANA_IF_PWR, 0xB1/*0xF1*/);
        }
        else
        {
          // High power IF dependent settings
...
static MXL_STATUS MxL661_ConfigTunerChanTune(PMXL661_TUNER_TUNE_CFG_T tuneParamPtr)
{
  ...
  if ((tuneParamPtr->SignalMode <= MXL661_DIG_DVB_C) && (tuneParamPtr->TuneType == MXL661_VIEW_MODE))
  {
    ...
    switch(tuneParamPtr->SignalMode)
    {
      ...
      case MXL661_DIG_J83B:
        if ((tuneParamPtr->BandWidth == MXL661_DIG_TERR_BW_6MHz) || (tuneParamPtr->BandWidth == MXL661_DIG_TERR_BW_8MHz)
            || (tuneParamPtr->BandWidth == MXL661_ANA_TV_DIG_CABLE_BW_8MHz))
        {
            if (tuneParamPtr->BandWidth == MXL661_DIG_TERR_BW_6MHz)
            {
                freqLutPtr = MXL661_DIG_CABLE_FREQ_LUT_BW_6MHZ;
            }
            else
            {
                freqLutPtr = MXL661_DIG_CABLE_FREQ_LUT_BW_8MHZ;
            }
...
MXL_STATUS MxLWare661_API_ConfigTuner(MXL661_COMMAND_T *ParamPtr)
{
  ...
  MxL_DLL_DEBUG0("MxLWare661_API_ConfigTuner : cmdType =%d\n", cmdType);
