...
#if defined(CHIP_TYPE_hi3716mv410) || defined(CHIP_TYPE_hi3716mv420) || defined(CHIP_TYPE_hi3716mv430) || defined(CHIP_TYPE_hi3716mv450)
...
{
    HI_S32 s32Ret = HI_FAILURE;
    HI_SYS_IO_REG_S para = {0};

    para.u32RegValue = u32Value;
    para.u32BaseAddr= u32RegAddr;
    ...
    s32Ret = ioctl(s_s32SysFd, SYS_WRITE_REGISTER, &para);
...
HI_S32 HI_SYS_ReadRegister(HI_U32 u32RegAddr, HI_U32 *pu32Value)
{
    ...
    s32Ret = ioctl(s_s32SysFd, SYS_READ_REGISTER, &para);
    if(HI_SUCCESS == s32Ret)
    {
        *pu32Value = para.u32RegValue;
    }
...
{
    return MPI_UPROC_AddDir(pszName, HI_MPI_UPROC_PARENT_DEFAULT);
}

HI_S32 HI_PROC_RemoveDir(const HI_CHAR *pszName)
{
    return MPI_UPROC_RemoveDir(pszName, HI_MPI_UPROC_PARENT_DEFAULT);
}
