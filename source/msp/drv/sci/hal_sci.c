...
volatile SCI_LDO_REG             *g_PSciLdoReg[HI_UNF_SCI_PORT_BUTT];
volatile SCI_LDO_IRQ_REG         *g_PSciLdoIrqReg[HI_UNF_SCI_PORT_BUTT];

HI_VOID SCI_HAL_LDO_Init(HI_UNF_SCI_PORT_E enSciPort, SCI_LDO_REG *PSciLdoReg)
{
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        g_PSciLdoReg[enSciPort] = PSciLdoReg;
    }

    return;
}

HI_VOID SCI_HAL_LDO_IRQ_Init(HI_UNF_SCI_PORT_E enSciPort, SCI_LDO_IRQ_REG *PSciLdoIrqReg)
{
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        g_PSciLdoIrqReg[enSciPort] = PSciLdoIrqReg;
    }
...
HI_VOID SCI_HAL_SetVcc(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_LEVEL_E enSciVcc)
{
    ...
    if (HI_UNF_SCI_LEVEL_LOW == enSciVcc)
    {
        SciCr0.Bits.vccen_inv = 1;
    }
    else
    {
        SciCr0.Bits.vccen_inv = 0;
    }
...
HI_VOID SCI_HAL_SetDetect(HI_UNF_SCI_PORT_E enSciPort, HI_UNF_SCI_LEVEL_E enSciDetect)
{
    ...
    if (HI_UNF_SCI_LEVEL_LOW == enSciDetect)
    {
        SciCr0.Bits.detect_inv = 1;
    }
    else
    {
        SciCr0.Bits.detect_inv = 0;
    }
...
HI_VOID SCI_HAL_DisableAllInt(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        g_PSciLdoIrqReg[enSciPort]->SciLdoImsc.Value = 0;
    }
#endif
    return;
}
...
HI_VOID SCI_HAL_EnableInt(HI_UNF_SCI_PORT_E enSciPort, SCI_INT_E SciIntType)
{
    ...
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        SCI_LDO_IMSC SciLdoImsc;
        SciLdoImsc.Value = g_PSciLdoIrqReg[enSciPort]->SciLdoImsc.Value;
        SciLdoImsc.Value |= (SciIntType & (~0xffff)) >> 16;
        g_PSciLdoIrqReg[enSciPort]->SciLdoImsc.Value = SciLdoImsc.Value;
        SciIntType = SciIntType & 0xffff;
    }
#endif
...
#if (defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT))
HI_U32 SCI_HAL_GetLdoIntState(HI_UNF_SCI_PORT_E enSciPort)
{
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        return g_PSciLdoIrqReg[enSciPort]->SciLdoMis.Value;
    }
...
HI_VOID SCI_HAL_ClearAllInt(HI_UNF_SCI_PORT_E enSciPort)
{
    ...
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        g_PSciLdoIrqReg[enSciPort]->SciLdoIcr.Value = 0x1;
    }
#endif
...
HI_VOID SCI_HAL_ClearInt(HI_UNF_SCI_PORT_E enSciPort, SCI_INT_E SciIntType)
{
    ...
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        SCI_INT_E SciLdoIntType;
        SciLdoIntType = (SciIntType & (~0xffff)) >> 16;
        g_PSciLdoIrqReg[enSciPort]->SciLdoIcr.Value = SciLdoIntType;
        SciIntType = SciIntType & 0xffff;
    }
#endif
...
HI_VOID SCI_HAL_EnableDelayTime(HI_UNF_SCI_PORT_E enSciPort)
{
    SCI_SCR0_REG SciCr0;

    SciCr0.Value = g_pSciReg[enSciPort]->SciCr0.Value;

    SciCr0.Bits.data_ini = 1;
...
HI_VOID SCI_HAL_SetSciItime(HI_UNF_SCI_PORT_E enSciPort, HI_U32 SciItime)
{
    g_pSciReg[enSciPort]->SciITime.Value = SciItime;

    return;
}

HI_VOID SCI_HAL_SetSciCtime(HI_UNF_SCI_PORT_E enSciPort, HI_U32 SciCtime)
{
    g_pSciReg[enSciPort]->SciCTime.Value = SciCtime;
...
#if ((defined(CHIP_TYPE_hi3716mv430) && defined(HI_SCI_VCC_AUTO_MATCH_SUPPORT)))
HI_VOID SCI_HAL_SetLdoenhz(HI_UNF_SCI_PORT_E enSciPort, HI_BOOL bLDOEnable)
{
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        SCI_LDO_CTRL SciLdoCtrl;
        SciLdoCtrl.Value = g_PSciLdoReg[enSciPort]->SciLdoCtrl.Value;
        SciLdoCtrl.Bits.ldo_en_hz = bLDOEnable;
        g_PSciLdoReg[enSciPort]->SciLdoCtrl.Value = SciLdoCtrl.Value;
    }
    return;
}

HI_VOID SCI_HAL_SetLdoVcc(HI_UNF_SCI_PORT_E enSciPort, HI_BOOL bHighVcc)
{
    if (HI_UNF_SCI_PORT0 == enSciPort)
    {
        SCI_LDO_CTRL SciLdoCtrl;
        SciLdoCtrl.Value = g_PSciLdoReg[enSciPort]->SciLdoCtrl.Value;
        SciLdoCtrl.Bits.ldo_sw = bHighVcc;
        g_PSciLdoReg[enSciPort]->SciLdoCtrl.Value = SciLdoCtrl.Value;
    }
    return;
}
