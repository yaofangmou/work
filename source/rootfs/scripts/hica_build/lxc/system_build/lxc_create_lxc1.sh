...
then
    ...
    echo -e "        for example: ./lxc_create_lxc1.sh DEBUG /home/user/HiSTBLinuxV100R002/out/hi3716mv410/hi3716m41dma_ca_debug/rootbox stb arm-histbv300-linux"
    exit 1
fi

#===========prepare environment=================================================
#fetch configure
source $5/lxc1_main_app.config
...
{
    echo  "=========$CUR_PATH/lxc1_main_app.config=============="
...
main:x:1002:1002:Linux User,,,:/home:/bin/sh
EOF
...
iptables -A INPUT -i eth0 -p tcp -m tcp --sport 4444 -d $LXC1_ADDR/32 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
iptables -A INPUT -i eth0 -p tcp -m tcp --sport 80 -d $LXC1_ADDR/32 -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
...
iptables -A OUTPUT -o eth0 -p tcp -m tcp --dport 4444  -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
iptables -A OUTPUT -o eth0 -p tcp -m tcp --dport 80  -m conntrack --ctstate NEW,ESTABLISHED,RELATED -j ACCEPT
EOF
}
...
#    cp -f $SAMPLE_OUT_PATH/lxc/other/lxc_mount       $PREFIX/$lxc_rootfs_dir/home/ && \
...
#
    cp -f $SAMPLE_OUT_PATH/lxc/ipc/msg_snd_lxc1      $PREFIX/$lxc_rootfs_dir/home/ && \
    $STRIP $PREFIX/$lxc_rootfs_dir/home/msg_snd_lxc1
#
#    cp -fr $SAMPLE_OUT_PATH/lxc/ipc/shm_write_lxc1      $PREFIX/$lxc_rootfs_dir/home/ && \
#    $STRIP $PREFIX/$lxc_rootfs_dir/home/shm_write_lxc1
#
#    cp -fr $SAMPLE_OUT_PATH/lxc/ipc/socket_client_lxc1  $PREFIX/$lxc_rootfs_dir/home/ && \
#    $STRIP $PREFIX/$lxc_rootfs_dir/home/socket_client_lxc1
#
#    cp -fr $SAMPLE_OUT_PATH/lxc/other/lxc_capset          $PREFIX/$lxc_rootfs_dir/home/
#    $STRIP $PREFIX/$lxc_rootfs_dir/home/lxc_capset
#
    cp -fr $SAMPLE_OUT_PATH/lxc/refapp/ref_server          $PREFIX/$lxc_rootfs_dir/home/
    $STRIP $PREFIX/$lxc_rootfs_dir/home/ref_server
...
/home/msg_snd_lxc1 &
#/home/shm_write_lxc1 &
#/home/socket_client_lxc1 &
#/home/lxc_capset &
#/home/sample_dvbplay 714
/home/ref_server 714
EOF
...
EOF
    ...
    if [[ $LXC_DEV_USB_ENABLE == y* ]]; then
     ...
     echo "lxc.mount.entry = /dev/mmcblk0 dev/mmcblk0 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE 
     echo "lxc.mount.entry = /dev/mmcblk0p10 dev/mmcblk0p10 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p11 dev/mmcblk0p11 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p12 dev/mmcblk0p12 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p13 dev/mmcblk0p13 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p14 dev/mmcblk0p14 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p15 dev/mmcblk0p15 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p16 dev/mmcblk0p16 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p17 dev/mmcblk0p17 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p18 dev/mmcblk0p18 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p19 dev/mmcblk0p19 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE

     echo "lxc.mount.entry = /dev/mmcblk0p2 dev/mmcblk0p2 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p20 dev/mmcblk0p20 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p21 dev/mmcblk0p21 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p22 dev/mmcblk0p22 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p23 dev/mmcblk0p23 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p24 dev/mmcblk0p24 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p25 dev/mmcblk0p25 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p26 dev/mmcblk0p26 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p27 dev/mmcblk0p27 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE

     echo "lxc.mount.entry = /dev/mmcblk0p3 dev/mmcblk0p3 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p4 dev/mmcblk0p4 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p5 dev/mmcblk0p5 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p6 dev/mmcblk0p6 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p7 dev/mmcblk0p7 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p8 dev/mmcblk0p8 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mmcblk0p9 dev/mmcblk0p9 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE

     echo "lxc.mount.entry = /dev/mtd0 dev/mtd0 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mtd1 dev/mtd1 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mtd0ro dev/mtd0ro none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mtd1ro dev/mtd1ro none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mtdblock0 dev/mtdblock0 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     echo "lxc.mount.entry = /dev/mtdblock1 dev/mtdblock1 none bind,optional,create=file 0 0" >> $LXC_CONFIGURE_FILE
     
    echo "" >> $LXC_CONFIGURE_FILE
    if [[ $LXC_DEV_ETHERNET_ENABLE == y* ]]; then
        echo "#config network to empty" >> $LXC_CONFIGURE_FILE
        echo "# lxc.network.type = empty" >> $LXC_CONFIGURE_FILE
        echo "#config virtual eth0" >> $LXC_CONFIGURE_FILE
        ...
        echo "lxc.network.hwaddr = 4a:49:43:49:79:be" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.ipv4 = 192.168.1.2/24" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.name = eth0" >> $LXC_CONFIGURE_FILE
        echo "lxc.network.ipv4.gateway = 192.168.1.3" >> $LXC_CONFIGURE_FILE
    fi
	...
	echo "lxc.cgroup.devices.allow = c 204:* rwm" >> $LXC_CONFIGURE_FILE
        ...
        echo "lxc.cgroup.devices.allow = b 179:* rwm"  >> $LXC_CONFIGURE_FILE
	echo "lxc.cgroup.devices.allow = b 259:* rwm"  >> $LXC_CONFIGURE_FILE
...
#    file_to_del+=" $PREFIX/$lxc_rootfs_dir/usr/lib/libz* "
    file_to_del+=" $PREFIX/$lxc_rootfs_dir/sbin/udhcpc "
    file_to_del+=" $PREFIX/$lxc_rootfs_dir/bin/mkdir "
    file_to_del+=" $PREFIX/$lxc_rootfs_dir/usr/sbin/arping "
    file_to_del+=" $PREFIX/$lxc_rootfs_dir/usr/bin/ping "
