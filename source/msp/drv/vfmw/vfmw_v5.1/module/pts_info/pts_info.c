...
static UINT64 PTS_Info_GetFrameTime_User(PTS_CHAN_INFO_S *pstPtsInfo, IMAGE *pstImage)
{
    ...
    if (pstImage->frame_rate >= 1000 && u64FrameTime > (1000000 / 12 * 2)
        && u64FrameTime > 1000000 / (pstImage->frame_rate / 1000) * 2)
    {
        u64FrameTime = 1000000 / (pstImage->frame_rate / 1000);
    }
...
static VOID PTS_Info_GetRoundFrameRate(SINT32 ChanID, IMAGE *pstImage)
{
    ...
    PTS_FRMRATE_TYPE_E frame_rate_type = g_PtsChan[ChanID].stUserFrameRate.enFrmRateType;

    if (((pstImage->optm_inf.Rwzb == 0)
        && (frame_rate_type != PTS_FRMRATE_TYPE_STREAM)
        && (frame_rate_type != PTS_FRMRATE_TYPE_USER))
        || ((frame_rate_type == PTS_FRMRATE_TYPE_STREAM) && (pstImage->frame_rate == 0)))
    {
...
SINT32 PTS_Info_Convert(SINT32 ChanID, IMAGE *pstImage)
{
    ...
    dprint(PRN_PTS, "Chan:%d PTSInfo: Width %d, Height %d, PtsUnit %d, PtsInfo{SrcPTS, InPts, OutPts, InFrmRate, OutFrmRate, PtsDelta} = {%lld, %lld, %lld, %d, %d, %d},\n",
           ChanID, pstImage->image_width, pstImage->image_height, pstImage->format.pts_unit,
