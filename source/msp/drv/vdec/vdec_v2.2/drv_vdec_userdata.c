...
HI_S32 USRDATA_Alloc(HI_HANDLE hHandle, HI_DRV_VDEC_USERDATABUF_S *pstBuf)
{
    ...
    if ((hHandle >= HI_VDEC_MAX_INSTANCE_NEW)
        || (pstBuf == HI_NULL)
        || (pstBuf->UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT)
    ...
    hBuf = hHandle * VDEC_USRDATA_TYPE_INDEX_BUTT + HI_VDEC_MAX_INSTANCE_NEW + pstBuf->UserDataType;
...
{
    if (hHandle >= HI_VDEC_MAX_INSTANCE_NEW || UserDataType >= VDEC_USRDATA_TYPE_INDEX_BUTT || (u64Addr == 0))
    {
        return HI_FAILURE;
    }
...
HI_S32 USRDATA_Put(HI_HANDLE hHandle, USRDAT *pstUsrData, HI_UNF_VIDEO_USERDATA_TYPE_E enType)
{
    ...
    pstPutData->enPositionInStream = VDEC_Convert_Userdata_SourceType(pstUsrData->from);
