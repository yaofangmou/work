...
typedef struct
{
    ...
    union
    {
        ...
        HI_DRV_VIDEO_JTP_SL_HDR_INFO_S stAvsSlHDRInfo;
    } unHDRInfo;
}HDR_OUTPUT_S;
...
static VOID HDR_GetMetadata_AVS2(HDR_INPUT_S *pstHDRInput, HDR_OUTPUT_S *pstHDROutput)
{
    ...
    {
        pstHDROutput->unHDRInfo.stSLFInfo.u8TransferCharacteristics = pstHDRInput->ColourInfo.TransferCharacteristics;
    ...
    {
        pstHDROutput->unHDRInfo.stAvsSlHDRInfo.u8TransferCharacteristics = pstHDRInput->ColourInfo.TransferCharacteristics;
        pstHDROutput->unHDRInfo.stAvsSlHDRInfo.u8staticMetadataAvailable = pstHDRInput->MasteringAvailable;
        pstHDROutput->unHDRInfo.stAvsSlHDRInfo.u8dynamicMetadataAvailable = pstHDRInput->SlHDR1MetadataAvailable;
        pstHDROutput->unHDRInfo.stAvsSlHDRInfo.staticMetadata.u8TransferCharacteristics = pstHDRInput->ColourInfo.TransferCharacteristics;

        if (pstHDRInput->MasteringAvailable == 1)
        {
            pstHDROutput->unHDRInfo.stAvsSlHDRInfo.staticMetadata.u8MasteringAvailable = pstHDRInput->MasteringAvailable;
            vfmw_Osal_Func_Ptr_S.pfun_Osal_MemCpy(&(pstHDROutput->unHDRInfo.stAvsSlHDRInfo.staticMetadata.stMasteringInfo),
                   &(pstHDRInput->MasteringDisplayColourVolume),
                   sizeof(MASTERING_DISPLAY_COLOUR_VOLUME_S));
        }
        ...
        if (pstHDRInput->ContentAvailable == 1)
        {
            pstHDROutput->unHDRInfo.stAvsSlHDRInfo.staticMetadata.u8ContentAvailable = pstHDRInput->ContentAvailable;
            vfmw_Osal_Func_Ptr_S.pfun_Osal_MemCpy(&(pstHDROutput->unHDRInfo.stAvsSlHDRInfo.staticMetadata.stContentInfo),
                   &(pstHDRInput->ContentLightLevel),
                   sizeof(CONTENT_LIGHT_LEVEL_INFO_S));
        }
        ...
        if (pstHDRInput->SlHDR1MetadataAvailable == 1)
        {
            pstHDROutput->unHDRInfo.stAvsSlHDRInfo.dynamicMetadata.u32Length = pstHDRInput->HDR_Metadata_size;
            pstHDROutput->unHDRInfo.stAvsSlHDRInfo.dynamicMetadata.u32Addr   = pstHDRInput->HDR_Metadata_phy_addr;
        }
    }
...
static VOID HDR_ReadSLHDRMetadataInfo(struct seq_file *p, HI_DRV_VIDEO_JTP_SL_HDR_INFO_S *pstSLHDRInfo)
{
    HI_SL_HDR_Metadata_S *pdynamicMetadata;
    if (p == HI_NULL || pstSLHDRInfo == HI_NULL)
    {
        return;
    }

    PROC_PRINT(p, "%-35s : %d\n", "staticMetadataAvailable", pstSLHDRInfo->u8staticMetadataAvailable);
    PROC_PRINT(p, "%-35s : %d\n", "dynamicMetadataAvailable", pstSLHDRInfo->u8dynamicMetadataAvailable);
    PROC_PRINT(p, "%-35s : %d\n", "TransferCharacteristics", pstSLHDRInfo->u8TransferCharacteristics);

    if (pstSLHDRInfo->u8staticMetadataAvailable == 1)
    {
        HDR_ReadHDR10MetadataInfo(p, &(pstSLHDRInfo->staticMetadata));
    }

    if (pstSLHDRInfo->u8dynamicMetadataAvailable == 1)
    {
        pdynamicMetadata = (HI_SL_HDR_Metadata_S *)MEM_Phy2Vir(pstSLHDRInfo->dynamicMetadata.u32Addr);

        if (pdynamicMetadata == HI_NULL)
        {
            dprint(PRN_ERROR, "[%s %d] pdynamicMetadata is null\n", __func__,__LINE__);
            return;
        }

        PROC_PRINT(p, "%-35s : %d\n", "PartID", pdynamicMetadata->s32PartID);
        PROC_PRINT(p, "%-35s : %d\n", "MajorSpecVersionID", pdynamicMetadata->s32MajorSpecVersionID);
        PROC_PRINT(p, "%-35s : %d\n", "MinorSpecVersionID", pdynamicMetadata->s32MinorSpecVersionID);
        PROC_PRINT(p, "%-35s : %d\n", "PayloadMode", pdynamicMetadata->s32PayloadMode);

        PROC_PRINT(p, "%-35s : %d\n", "HdrPicColourSpace", pdynamicMetadata->s32HdrPicColourSpace);
        PROC_PRINT(p, "%-35s : %d\n", "HdrDisplayColourSpace", pdynamicMetadata->s32HdrDisplayColourSpace);
        PROC_PRINT(p, "%-35s : %d\n", "HdrDisplayMaxLuminance", pdynamicMetadata->s32HdrDisplayMaxLuminance);
        PROC_PRINT(p, "%-35s : %d\n", "HdrDisplayMinLuminance", pdynamicMetadata->s32HdrDisplayMinLuminance);

        PROC_PRINT(p, "%-35s : %d\n", "SdrPicColourSpace", pdynamicMetadata->s32SdrPicColourSpace);
        PROC_PRINT(p, "%-35s : %d\n", "SdrDisplayColourSpace", pdynamicMetadata->s32SdrDisplayColourSpace);
        PROC_PRINT(p, "%-35s : %d\n", "SdrDisplayMaxLuminance", pdynamicMetadata->s32SdrDisplayMaxLuminance);
        PROC_PRINT(p, "%-35s : %d\n", "SdrDisplayMinLuminance", pdynamicMetadata->s32SdrDisplayMinLuminance);

        PROC_PRINT(p, "%-35s : (%d, %d, %d)\n", "MatrixCoefficient",
                      pdynamicMetadata->s32MatrixCoefficient[0],
                      pdynamicMetadata->s32MatrixCoefficient[1],
                      pdynamicMetadata->s32MatrixCoefficient[2]);

        PROC_PRINT(p, "%-35s : (%d, %d)\n", "ChromaToLumaInjection",
                      pdynamicMetadata->s32ChromaToLumaInjection[0],
                      pdynamicMetadata->s32ChromaToLumaInjection[1]);

        PROC_PRINT(p, "%-35s : (%d, %d, %d)\n", "MatrixCoefficient",
                      pdynamicMetadata->s32KCoefficient[0],
                      pdynamicMetadata->s32KCoefficient[1],
                      pdynamicMetadata->s32KCoefficient[2]);

        PROC_PRINT(p, "%-35s : %d\n", "TmInputSignalBlackLevelOffset", pdynamicMetadata->u.variables.s32TmInputSignalBlackLevelOffset);

        PROC_PRINT(p, "%-35s : %d\n", "TmInputSignalWhiteLevelOffset", pdynamicMetadata->u.variables.s32TmInputSignalWhiteLevelOffset);
        PROC_PRINT(p, "%-35s : %d\n", "ShadowGain", pdynamicMetadata->u.variables.s32ShadowGain);
        PROC_PRINT(p, "%-35s : %d\n", "HighlightGain", pdynamicMetadata->u.variables.s32HighlightGain);
        PROC_PRINT(p, "%-35s : %d\n", "MidToneWidthAdjFactor", pdynamicMetadata->u.variables.s32MidToneWidthAdjFactor);
        PROC_PRINT(p, "%-35s : %d\n", "TmOutputFineTuningNumVal", pdynamicMetadata->u.variables.s32TmOutputFineTuningNumVal);
        PROC_PRINT(p, "%-35s : %d\n", "SaturationGainNumVal", pdynamicMetadata->u.variables.s32SaturationGainNumVal);
    }
...
static VOID HDR_ReadOutputHDRInfo(struct seq_file *p, HDR_OUTPUT_S *pstHDROutput)
{
    ...
    switch (pstHDROutput->enSrcFrameType)
    {
        ...
        case HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR:
            HDR_ReadSLHDRMetadataInfo(p, &(pstHDROutput->unHDRInfo.stAvsSlHDRInfo));
            break;
