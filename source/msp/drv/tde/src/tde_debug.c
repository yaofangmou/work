...
HI_VOID DRV_TDE_OutputMessage(const HI_CHAR* pFunc, const HI_U32 Line, const HI_GFX_MSG_TYPE_E eMsgType, const HI_U32 Format, ...)
{
    ...
    if (NULL == pFunc)
    {
        HI_GFX_Print("[module-gfx-tde][err] : input func pointer %p\n",pFunc);
        return;
    }

    if ((HI_GFX_MSG_BEG == eMsgType) || (HI_GFX_MSG_END == eMsgType))
    {
        FormatMessage("tde", pFunc, Line, eMsgType, TmpFormat, &LogErr, &LogFunc, &LogValue);
        return;
    }
