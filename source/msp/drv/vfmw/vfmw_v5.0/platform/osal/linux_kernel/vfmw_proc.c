...
VOID vfmw_read_proc_shvc(struct seq_file *p, VFMW_CHAN_S *pChan)
{
    ...
    {
        ret = HEVC_CheckVpsExtOverlap(&(pCtx->pVPS[i].vps_extension), &OverLap);
        if (ret == FMW_OK && OverLap != 0)
        {
    ...
    {
        ret = HEVC_CheckPpsExtOverlap(&(pCtx->pPPS[i].pps_extension), &OverLap);
        if (ret == FMW_OK && OverLap != 0)
        {
    ...
    ret = HEVC_CheckSliceExtOverlap(&(pCtx->CurrSlice.slice_extension), &OverLap);
    if (ret == FMW_OK && OverLap != 0)
    {
...
static VOID vfmw_read_proc_size_change(SINT32 ChanID, struct seq_file *p, VOID *v)
{
    SIZE_CHANGE_STATE_ST *pScStat = &(g_VfmwChanStat[ChanID].ScStat);

    seq_printf(p, "----------------------- SIZE CHANGE INFO ----------------------\n");

    seq_printf(p, "%-40s :%d\n",       "SizeChangeState",  pScStat->eSizeChangeState);
    seq_printf(p, "%-40s :%dx%d\n",    "OldSize",          pScStat->u32OldWidth, pScStat->u32OldHeight);
    seq_printf(p, "%-40s :%dx%d\n",    "NewSize",          pScStat->u32NewWidth, pScStat->u32NewHeight);
    seq_printf(p, "%-40s :%dms\n",     "ReportUseTime",    pScStat->u32ReportTime-pScStat->u32SizeChangeTime);
    seq_printf(p, "%-40s :%dms\n",     "WaitFsUseTime",    pScStat->u32BindTime-pScStat->u32ReportTime);
    seq_printf(p, "%-40s :%dms\n",     "GetFsUseTime",     pScStat->u32GetFsTime-pScStat->u32BindTime);
    seq_printf(p, "%-40s :%dms\n",     "ParseUseTime",     pScStat->u32StartDecTime-pScStat->u32GetFsTime);
    seq_printf(p, "%-40s :%dms\n",     "DecUseTime",       pScStat->u32FinishDecTime-pScStat->u32StartDecTime);
    seq_printf(p, "%-40s :%dms\n",     "PostProcUseTime",  pScStat->u32OutputTime-pScStat->u32FinishDecTime);
    seq_printf(p, "%-40s :%dms\n",     "TotalUseTime",     pScStat->u32OutputTime-pScStat->u32SizeChangeTime);
...
SINT32 vfmw_read_proc_scd(struct seq_file *p, VOID *v)
{
    ...
    seq_printf(p, "%-40s :%d\n",      "Is tunnel",         pInst->Config.IsTunneledPath);
...
SINT32 vfmw_read_proc_chn(struct seq_file *p, VOID *v)
{
    ...
    vfmw_read_proc_size_change(ChanId, p, v);
