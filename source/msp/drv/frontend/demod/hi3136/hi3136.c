...
static HI3136_CFG_ITEM_S hi3136_cfg[TUNER_NUM];
...
static HI_S32 hi3136_setlpf(HI_U32 u32TunerPort, HI_U32 u32SymbolRate/*unit KHz , neither BAUD, nor M*/)
{
    HI_U32 u32LPFKHz = u32SymbolRate;

    if (u32LPFKHz >= hi3136_cfg[u32TunerPort].u16TunerLPF_100Khz * 100)
    {
        u32LPFKHz = hi3136_cfg[u32TunerPort].u16TunerLPF_100Khz * 100;
    }
...
HI_S32 hi3136_lock_TP_BS(HI_U32 u32TunerPort, TUNER_ACC_QAM_PARAMS_S *tp)
{
    ...
    HI_U32 fs_cur = 0, time_cnt = 0, fec_ok_wait = 0, max_reliablity = 0, reliablity = 0, cbs_fs_sel = 0, u32LPFKHz;
    ...
    center_shift_KHz = (HI_UNF_TUNER_IQSPECTRUM_INVERT == hi3136_cfg[u32TunerPort].enIQSpectrum) ? (-fc_true) : fc_true;
    ...
    {
        u32LPFKHz = fs_cur / 10 * 7 / 1000 + abs(center_shift_KHz)  + 5000;
        u32LPFKHz = u32LPFKHz * 75 / 100 + 4000;
        hi3136_setlpf(u32TunerPort, u32LPFKHz);
    }
...
{
    HI_U32  TP_Num = 0, time_cnt = 0, u32LPFKHz;
    HI_U8   i;
    ...
    {
        u32LPFKHz = Lpf_BW_window * 75 / 100 + 4000;
        hi3136_setlpf(u32TunerPort, u32LPFKHz);
    }
    ...
    if(TP_Num != 0)
    {
        ...
        for(i = 0; i < TP_Num; i++)
        {
            ...
            if(HI_UNF_TUNER_IQSPECTRUM_INVERT == hi3136_cfg[u32TunerPort].enIQSpectrum)
            {
...
HI_S32 hi3136_init(HI_U32 u32TunerPort, HI_U8 enI2cChannel, HI_UNF_TUNER_DEV_TYPE_E enTunerDevType)
{
    ...
    //设置盲扫的搜索范围
    qam_write_byte(u32TunerPort, FS_SPAN, 0x03);
...
HI_S32 tp_verify_second_check_lock_status(HI_U32 u32TunerPort, TP_VERIFY_INFO_ST* pstTpVerifyInfo)
{
    ...
    HI_S16 cr_cn = 0;
    HI_U8 tim_ok_bit, u8Temp = 0, m, scan_num = pstTpVerifyInfo->ScanNum;
    HI_U8 modcod = 0;
...
HI_S32 hi3136_get_agc(HI_U32 u32TunerPort, HI_U32 Fcenter, HI_U32 *pu32Agc)
{
    ...
    hi3136_setlpf(u32TunerPort, 15000);
