                                       ...
                                       HI_U32 u32ValidDataLen, HI_U32 u32Pts)
{
    ...
    HI_BOOL IsRandSupport = HI_FALSE;

    s32Ret = HI_MPI_AVPLAY_CheckIfSupportRandomStream(hAvplay, &IsRandSupport);
    if (s32Ret != HI_SUCCESS)
    {
        return s32Ret;
    }

    stExOpt.bEndOfFrm = (HI_TRUE == IsRandSupport) ? HI_FALSE : HI_TRUE;
...
{
    return HI_MPI_AVPLAY_GetDmxAdAudChnHandle(hAvplay, phDmxAdAudChn);
}
