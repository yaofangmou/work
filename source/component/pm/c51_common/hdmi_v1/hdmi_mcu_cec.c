...
static HI_VOID CecPhyAddrReport(HI_VOID)
{
    ...
    stFrame.stOperand.u8Data[2] = CEC_LOGICALADD_PLAYDEV_1;
#else
    stFrame.stOperand.u8Data[2] = CEC_LOGICALADD_TUNER_1;
#endif
...
#ifdef HDMI_PRIORITY_PLAYBACK
    if( !CecLogicAddr.bits.bit01_playback_dev1 )
...
#ifdef HDMI_PRIORITY_PLAYBACK
            pstCecInfo->enMyLogicAddr               = CEC_LOGICALADD_PLAYDEV_1;
#else
...
#if HDMI_MCU_DEBUG
    HDMI_MCU_ComPuts("T:");
    HDMI_MCU_ComPutU8Hex(pstFrame->enSrcAddr);
    HDMI_MCU_ComPutU8Hex(pstFrame->enDestAddr);
    HDMI_MCU_ComPutU8Hex(pstFrame->u8Opcode);
    for( u8Timeout = 0; (u8Timeout<pstFrame->stOperand.u8Length) && (u8Timeout<15); u8Timeout++)
    {
        HDMI_MCU_ComPutU8Hex(pstFrame->stOperand.u8Data[u8Timeout]);
    }
    HDMI_MCU_ComPuts("\n\r");
#endif
...
HI_S16 MCU_HDMI_CecFrameRx(CEC_MSG_FRAME_S *pstFrame)
{
    ...
    HDMI_MCU_ComPutU8Hex(pstFrame->enDestAddr);
    HDMI_MCU_ComPutU8Hex(pstFrame->u8Opcode);
    for( i = 0; (i<pstFrame->stOperand.u8Length) && (i<15); i++)
    {
        HDMI_MCU_ComPutU8Hex(pstFrame->stOperand.u8Data[i]);
    }
    HDMI_MCU_ComPuts("\n\r");
#endif
