                                 ...
                                 ALG_ASP_PARA_S *pstAspPara)
{
    ...
    if (v_w <= pOutWnd->s32Width)
    {
        ...
        pOutWnd->s32X = (pOutWnd->s32X + (pOutWnd->s32Width - v_w) / 2);
        /* if pOutWnd->s32X is odd, then adjust zme out Width and make lbx X to 2 align */
        if (0 != ((HI_U32)(pOutWnd->s32X) & 0x1))
        {
            pOutWnd->s32X -= 1;
            v_w += 2;
        }
    ...
    else
    {
        ...
        /* if pOutWnd->s32Y is odd, then adjust zme out Height and make lbx Y to 2 align */
        pOutWnd->s32Y = (pOutWnd->s32Y + (pOutWnd->s32Height - v_h) / 2);
        if (0 != ((HI_U32)(pOutWnd->s32Y) & 0x1))
        {
            pOutWnd->s32Y -= 1;
            v_h += 2;
        }
