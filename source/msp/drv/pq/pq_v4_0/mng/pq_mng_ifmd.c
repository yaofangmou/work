...
*****************************************************************************/
ALG_FMD_RTL_OUTPARA_S g_stFmdRtlOutPara;
ALG_DEI_DRV_PARA_S    g_stDeiDrvPara;
...
static HI_S32 PQ_MNG_ALG_GetIFMDPlayBackConfig(IFMD_TotalHardPara *pIfmdTotalHardPara, PQ_IFMD_CALC_S *pstIfmdCalc, REG_PLAYBACK_CONFIG_S *pstIfmdResult)
{
    ...
    memset(&g_stFmdRtlOutPara, 0, sizeof(ALG_FMD_RTL_OUTPARA_S));
    memset(&g_stDeiDrvPara, 0, sizeof(ALG_DEI_DRV_PARA_S));

    memcpy(&(g_stDeiDrvPara.stFmdRtlStatPara), &(pIfmdTotalHardPara->stFmdRtlStatPara), sizeof(ALG_FMD_RTL_STATPARA_S));
    memcpy(&(g_stDeiDrvPara.stMadRtlStatPara), &(pIfmdTotalHardPara->stMadRtlStatPara), sizeof(ALG_MAD_RTL_STATPARA_S));
    memcpy(&(g_stDeiDrvPara.stVdecInfo), &(pstIfmdCalc->stVdecInfo), sizeof(ALG_VDEC_INFO_S));
    g_stDeiDrvPara.stVdecInfo.RealFrmRate = pstIfmdCalc->u32FrameRate / 2;

    g_stDeiDrvPara.s32DeiMdLum  = ALG_DEI_MODE_4FLD;
    g_stDeiDrvPara.s32DeiMdChr  = ALG_DEI_MODE_4FLD;

    g_stDeiDrvPara.s32FrmWidth  = pstIfmdCalc->u32WidthY;
    g_stDeiDrvPara.s32FrmHeight = pstIfmdCalc->u32HeightY;
    g_stDeiDrvPara.BtMode       = pstIfmdCalc->s32FieldOrder;
    g_stDeiDrvPara.RefFld       = pstIfmdCalc->s32FieldMode;
    g_stDeiDrvPara.bPreInfo     = pstIfmdCalc->bPreInfo;
    g_stDeiDrvPara.bOfflineMode = HI_TRUE;
    g_stDeiDrvPara.FodEnable    = (HI_S32)g_enFodMode;
    g_stDeiDrvPara.Pld22Enable  = HI_TRUE;
    g_stDeiDrvPara.Pld32Enable  = HI_TRUE;
    g_stDeiDrvPara.s32Drop      = 0;
    g_stDeiDrvPara.s32Repeat    = 0;
    g_stDeiDrvPara.EdgeSmoothEn = HI_FALSE;

    /* 利用从硬件得到的数据，进行软件层的判断 */
    PQ_MNG_ALG_FmdSet(pstFmdSoftInfo, &g_stDeiDrvPara, &g_stFmdRtlOutPara);

    pstIfmdResult->die_out_sel     = g_stFmdRtlOutPara.DieOutSelLum;
    pstIfmdResult->dir_mch         = g_stFmdRtlOutPara.DirMch;
    pstIfmdResult->s32FieldOrder   = g_stFmdRtlOutPara.s32FieldOrder;
    pstIfmdResult->u32EdgeSmoothEn = g_stFmdRtlOutPara.EdgeSmoothEn;

    g_s32DeiMdLum = g_stDeiDrvPara.s32DeiMdLum;
