...
{
    LNBCTRL_DEV_PARAM_S* pstParam = NULL;
#if defined(DEMOD_DEV_TYPE_AVL6211) || defined(DEMOD_DEV_TYPE_HI3136) || defined(HISDTV100_SAT_SUPPORT)
    HI_S32 s32Ret = HI_FAILURE;
#endif
        ...
        case HI_UNF_DEMOD_DEV_TYPE_INTERNAL0:
            ...
            if (HI_CHIP_TYPE_HI3716M == enChipType && HI_CHIP_VERSION_V430 == enChipVersion)
            {
                ...
                if (TUNER_LNB_OUT_0V != enOut)
                {
                    ...
                    s32Ret = HiSDTV100_ADP_SetLnbOut(u32TunerPort, enOut);
                    if (HI_SUCCESS != s32Ret)
                    {
                        HI_ERR_TUNER("HiSDTV100_ADP_SetLnbOut failed.\n");
                        return s32Ret;
                    }
#endif
                }
