...
HI_VOID VPSS_Hal_PrintHalInfo(struct seq_file *p, S_VPSS_REGS_TYPE *pstNodeReg)
{
    ...
    PROC_PRINT(p,
               ...
               VPSS_PROC_LEVEL5"        yaddr_lb:%08x ystride_lb:%u "
               ...
               pstNodeReg->VPSS_NX2Y_LB_ADDR.u32,
               pstNodeReg->VPSS_NX2_LB_STRIDE.u32,
    ...
    for (u32PortId = 0; u32PortId < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32PortId++)
    {
        ...
        if ((pstHalPort->bEnable)  && (pstHalPort->bConfig == HI_FALSE))
        {
            ...
            */
            ...
            {
                if (HI_FALSE == stFuncCropCfg.bEnable)
                {
...
HI_S32 VPSS_HAL_Set_2D_UHDFrame_Node(VPSS_IP_E enIP, VPSS_HAL_INFO_S *pstHalInfo, HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    if (HI_TRUE == pstHalInfo->bAndroidCts)
    {
        if ((pstHalInfo->stInInfo.u32Height & 0x2) != 0)
        {
            stFunc4PixelCfg.bEnable = HI_FALSE;
        }
    }
    VPSS_CBB_FUNC_SetConfig(stRegAddr, CBB_FUNC_SPEED_UP_4PIXEL, (HI_VOID *)&stFunc4PixelCfg, HI_NULL);
...
static CBB_FRAME_S g_stVPSS_H265Step1FrameCfg; //do this to reduce stack of the function below
        ...
        HI_U32 *pu32AppVir, HI_U32 u32AppPhy)
{
    ...
    VPSS_MEM_CLEAR(g_stVPSS_H265Step1FrameCfg);
    VPSS_MEM_CLEAR(stFlipInfo);
    VPSS_MEM_CLEAR(stFuncCropCfg);
    VPSS_MEM_CLEAR(stFuncZmeCfg);
    VPSS_MEM_CLEAR(stLBox);
    ...
    if (pstHalPort->bEnable && pstHalPort->bConfig == HI_FALSE)
    {
        ...
        */
        ...
        g_stVPSS_H265Step1FrameCfg.enBitWidth = pstOutFrm->enBitWidth;
        g_stVPSS_H265Step1FrameCfg.enPixelFmt = HI_DRV_PIX_FMT_NV21;
        g_stVPSS_H265Step1FrameCfg.u32Width   = pstOutFrm->u32Width;
        g_stVPSS_H265Step1FrameCfg.u32Height  = pstOutFrm->u32Height;
        g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32PhyAddr_Y = pstOutAddr->u32PhyAddr_Y;
        g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32PhyAddr_C = pstOutAddr->u32PhyAddr_C;
        g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32Stride_Y  = pstOutAddr->u32Stride_Y * 2;
        g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32Stride_C  = pstOutAddr->u32Stride_C * 2;

        VPSS_DBG_INFO(
            "Vhd0 Y C StrideY StrideC W H %08x %08x %d %d %d %d\n",
            g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32PhyAddr_Y,
            g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32PhyAddr_C,
            g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32Stride_Y,
            g_stVPSS_H265Step1FrameCfg.stAddr[CBB_FREAM_BUF_8BIT].u32Stride_C,
            g_stVPSS_H265Step1FrameCfg.u32Width,
            g_stVPSS_H265Step1FrameCfg.u32Height);

        VPSS_CBB_FRAME_SetConfig(stRegAddr, CBB_FRAME_WRITE_VHD0, (HI_VOID *)&g_stVPSS_H265Step1FrameCfg, HI_NULL);
