...
HI_U16  u16LastMonitorDownScale_w = INVALID_INTERMEDIARect;
HI_U16  u16LastMonitorDownScale_h = INVALID_INTERMEDIARect;
HI_VOID Win_Policy_GetPQDegradingConfigFromTEE(HI_BOOL bTEEMode,
        ...
        HI_BOOL   *bMute,
        HI_BOOL   *blastNoFrameMonitorMute,
        HI_U32    *pu32StandMuteCount,
        HI_RECT_S *pstHdcpIntermediateRect)
{
    HI_U16  u16DownScale_w;
    HI_U16  u16DownScale_h;

    DRV_GetTeeVideoMonitorCtrl(&u16DownScale_w, &u16DownScale_h);
        ...
        || (HI_FALSE == bCfgFrame))
    {
        ...
        *pbHdcpDegradePQ = HI_FALSE;
        if ((HI_FALSE == bCfgFrame)
            && ((pstOutRect->s32Width >  u16DownScale_w) || (pstOutRect->s32Height > u16DownScale_h)))
        {
            *blastNoFrameMonitorMute = HI_TRUE;
        }
        return;
    }
    if ((u16DownScale_w == 0) || (u16DownScale_h == 0))
    {
        u16DownScale_w = INVALID_INTERMEDIARect;
        u16DownScale_h = INVALID_INTERMEDIARect;
    }

    if ((pstFrameIntermediateRect->s32Width == 0) || (pstFrameIntermediateRect->s32Height == 0))
    {
        pstFrameIntermediateRect->s32Width = INVALID_INTERMEDIARect;
        pstFrameIntermediateRect->s32Height = INVALID_INTERMEDIARect;
    }

    if (*pu32StandMuteCount > 0)
    {
        *pu32StandMuteCount = *pu32StandMuteCount - 1;
        *bMute = HI_TRUE;
    }
    else if ((MUTE_INTERMEDIARect == u16DownScale_w)
             || (MUTE_INTERMEDIARect == u16DownScale_h)
    ...
    {
        /*monitor  ffff and normal*/

        if ((INVALID_INTERMEDIARect != u16DownScale_w) && (INVALID_INTERMEDIARect != u16DownScale_h)
            && ((pstOutRect->s32Width >  u16DownScale_w) || (pstOutRect->s32Height >  u16DownScale_h)))
        {
            if (((INVALID_INTERMEDIARect == u16LastMonitorDownScale_w) || (INVALID_INTERMEDIARect == u16LastMonitorDownScale_h))
                || ((u16DownScale_w < u16LastMonitorDownScale_w) || (u16DownScale_h < u16LastMonitorDownScale_h)))
            {

                *pu32StandMuteCount = 3;
                *bMute = HI_TRUE;
            }
            else
            {
    ...
    /*from no frame to has frame*/
    if (HI_TRUE == *blastNoFrameMonitorMute )
    {
        *pu32StandMuteCount = 3;
        *bMute = HI_TRUE;
        *blastNoFrameMonitorMute = HI_FALSE;
    }

    u16LastMonitorDownScale_w = u16DownScale_w;
    u16LastMonitorDownScale_h = u16DownScale_h;

    if ((MUTE_INTERMEDIARect == u16DownScale_w) || (MUTE_INTERMEDIARect == u16DownScale_h))
    {
        pstHdcpIntermediateRect->s32X      = 0;
        pstHdcpIntermediateRect->s32Y      = 0;
        pstHdcpIntermediateRect->s32Width  = 0;
    ...
    {
        if ((INVALID_INTERMEDIARect == u16DownScale_w)
            || (INVALID_INTERMEDIARect == u16DownScale_h))
        {
            if ((INVALID_INTERMEDIARect == pstFrameIntermediateRect->s32Width)
            ...
            {
                pstHdcpIntermediateRect->s32Width  =  pstFrameIntermediateRect->s32Width;
                pstHdcpIntermediateRect->s32Height =  pstFrameIntermediateRect->s32Height;
                *pbHdcpDegradePQ = HI_TRUE;
            }
        }
        else
        {
            if ((MUTE_INTERMEDIARect == pstFrameIntermediateRect->s32Width)
                || (MUTE_INTERMEDIARect == pstFrameIntermediateRect->s32Height))
            {
                pstHdcpIntermediateRect->s32Width = u16DownScale_w;
                pstHdcpIntermediateRect->s32Height = u16DownScale_h;
            }
            else
            {
                pstHdcpIntermediateRect->s32Width = (u16DownScale_w > pstFrameIntermediateRect->s32Width) ? pstFrameIntermediateRect->s32Width : u16DownScale_w;
                pstHdcpIntermediateRect->s32Height = (u16DownScale_h > pstFrameIntermediateRect->s32Height) ? pstFrameIntermediateRect->s32Height : u16DownScale_h;
            }
                                        ...
                                        HI_DRV_DISP_XDR_ENGINE_E  *penXdrEngine)
{
    ...
    s32Ret = DISP_GetFormat(HI_DRV_DISPLAY_1, &enStereo, &enEncFmt);
    if (HI_SUCCESS != s32Ret)
    {
        return s32Ret;
    }

    if ((enEncFmt >= HI_DRV_DISP_FMT_VESA_800X600_60)
        && (enEncFmt <= HI_DRV_DISP_FMT_VESA_2560X1600_60_RB))
    {
        enTempOutType = HI_DRV_DISP_TYPE_NORMAL;
    }
...
static HI_VOID WinFixAutoFollowFirstFrmOutStatus(HI_DRV_VIDEO_FRAME_TYPE_E enFrameType,
                                            HI_DRV_DISP_OUT_TYPE_E *penOutType,
                                            HI_DRV_COLOR_SPACE_E enInColorSpace,
                                            HI_DRV_COLOR_SPACE_E *penOutColorSpace)
{
    HI_DRV_COLOR_SPACE_E   enFormatColorSpace = HI_DRV_CS_UNKNOWN;
    HI_DRV_DISP_SINK_CAPABILITY_S stSinkCap = {0};
    ...
    /* get disp status. */
    (HI_VOID)DISP_GetDisplayColorSpaceWithoutVideo(HI_DRV_DISPLAY_1, &enFormatColorSpace);
    if (enFormatColorSpace != *penOutColorSpace)
    {
        /* out colorspace must obey to PAL/VESA format. */
        if ((HI_DRV_CS_BT601_YUV_LIMITED == enFormatColorSpace)
            || (HI_DRV_CS_BT709_RGB_FULL == enFormatColorSpace))
        {
            *penOutColorSpace = enFormatColorSpace;
        }

        if ((HI_DRV_CS_BT601_YUV_LIMITED == *penOutColorSpace)
            || (HI_DRV_CS_BT709_RGB_FULL == *penOutColorSpace))
        {
            /* re-generate output colorspace when switch format. */
            (HI_VOID)DISP_GenerateFrameOutColorSpace(HI_DRV_DISPLAY_1,
                                                     enFrameType,
                                                     *penOutType,
                                                     enInColorSpace,
                                                     penOutColorSpace);
        ...
        /* adapter to sink capbility when hdmi hot-plug happened. */
                                             ...
                                             HI_DRV_COLOR_SPACE_E *penOutColorSpace)
{
    ...
    if (HI_TRUE == stFirstFrmStatus.bSenceChange)
    {
        ...
        *penOutColorSpace = stFirstFrmStatus.enOutColorSpace;
        ...
        WinFixAutoFollowFirstFrmOutStatus(pstXdrFrame->stBaseFrm.enSrcFrameType,
                                         &pstXdrFrame->enDispOutType,
                                         enInColorSpace,
                                         penOutColorSpace);
    }
    else
