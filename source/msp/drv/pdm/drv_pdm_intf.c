...
static HI_VOID PDM_ProcReserveMem(struct seq_file *p)
{
    HI_U32 i = 0;

    PROC_PRINT(p, "\n-------------------- Reserve Mem Info -------------------------\n");
    PROC_PRINT(p, "Name              phyaddr     size(Byte)       IsRelease\n");

    for (i = 0; i < g_PdmGlobal.u32BufNum; i++)
    {
        PROC_PRINT(p, "%-16s  0x%-8p  %-16d  %d\n",
            g_PdmGlobal.stBufInfo[i].as8BufName,
            g_PdmGlobal.stBufInfo[i].pPhyAddr,
            g_PdmGlobal.stBufInfo[i].u32Lenth,
            g_PdmGlobal.stBufInfo[i].bRelease);
    }
}
