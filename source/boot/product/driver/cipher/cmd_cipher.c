...
int sm2_test_sign(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    HI_S32 ret;
    ...
    if (ret != HI_SUCCESS)
    {
       HI_ERR_CIPHER("HI_UNF_CIPHER_Sm2Sign Failed!\n");
       HI_UNF_CIPHER_DeInit();
       return HI_FAILURE;
    }
    time_start = get_timer(time_start);
    printf("SM2 Sign Use Clock Cycle: %u, Clock Cycle for 1S: %lu\n", time_start, CONFIG_SYS_HZ);

    Cipher_printBuffer("r", u8R, SM2_LEN_IN_BYTE);
    Cipher_printBuffer("s", u8S, SM2_LEN_IN_BYTE);

    HI_ERR_CIPHER("Verify...\n");
    time_start = get_timer(0);
    ret = HI_UNF_CIPHER_Sm2Verify(&stSm2Verify, u8M, sizeof(u8M), u8R, u8S);
    if (ret != HI_SUCCESS)
    {
       HI_ERR_CIPHER("HI_UNF_CIPHER_Sm2Verify Failed!\n");
       HI_UNF_CIPHER_DeInit();
       return HI_FAILURE;
    }
    time_start = get_timer(time_start);
    printf("SM2 Verify Use Clock Cycle: %u, Clock Cycle for 1S: %lu\n", time_start, CONFIG_SYS_HZ);
...
U_BOOT_CMD(sm2_test_sign, 1, 1, sm2_test_sign, "Calcluate sm2 sign verify", "for example: sm2_test_sign");

/*SM2 encrypt and decrypt, use gold data*/
int sm2_test_enc(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    HI_S32 ret;
    ...
    ret = HI_UNF_CIPHER_Sm2GenKey(&stSm2Key);;
    if (ret != HI_SUCCESS)
    {
        ret = HI_FAILURE;
        goto EXIT;
    ...
    Cipher_printBuffer("d", (HI_U8*)stSm2Key.u32d, SM2_LEN_IN_BYTE);
    Cipher_printBuffer("Px", (HI_U8*)stSm2Key.u32Px, SM2_LEN_IN_BYTE);
    Cipher_printBuffer("Py", (HI_U8*)stSm2Key.u32Py, SM2_LEN_IN_BYTE);

    HI_ERR_CIPHER("--------------------------Encrypt-------------------------\n");
    ...
    Cipher_printBuffer("C", u8C, u32Clen);

    HI_ERR_CIPHER("--------------------------Decrypt-------------------------\n");
    ...
    Cipher_printBuffer("MO", u8MO, u32Mlen);
...
U_BOOT_CMD(sm2_test_enc, 1, 1, sm2_test_enc, "Calcluate sm2 encrypt/decrypt", "for example: sm2_test_enc");
#endif
