						...
						TEEC_Operation *operation, uint32_t *returnOrigin)
{
	...
	{
		tlogd("session->context is null point.\n");
...
TEEC_Result TEEC_EXT_WakeUpEvent(uint32_t agent_id, int dev_fd)
{
	int ret;
	ret = ioctl(dev_fd, (int)TC_NS_CLIENT_IOCTL_WAKE_UP_EVENT, agent_id);
	if(ret) {
		tlogd("Agent %d failed to wakeupevent, ret is %d!\n", agent_id, ret);
		return (TEEC_Result)TEEC_ERROR_GENERIC;
	}
	return TEEC_SUCCESS;
}
