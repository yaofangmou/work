...
extern HI_VOID EngineLock(HI_VOID);
extern HI_VOID EngineUnLock(HI_VOID);
...
static HI_VOID IPCServerForceFlushTrack(HI_HANDLE hTrack);
...
static HI_VOID IPCServerRecordListDestoryAllTrack(HI_S32 s32SocketFd)
{
    ...
    if (HI_TRUE == g_stServer.stRecordList[i].bContinueCtrl)
    {
        ...
        s32Ret = Engine_SetTrackConfig(SOURCE_ID_MAIN, &stTrackConfig);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_AO("call Engine_SetTrackConfig(CONFIG_SETCONTINUOUSOUTPUTSTATUS) failed(0x%x)\n", s32Ret);
        }
        g_stServer.stRecordList[i].bContinueCtrl = HI_FALSE;
    }
    ...
    for (j = 0; j < SOCKET_TRACK_MAX_NUM; j++)
    {
        ...
        if (HI_INVALID_HANDLE != hTrack)
        {
            ...
            IPCServerForceFlushTrack(hTrack);
    ...
    //ATMOS unLock
    {
        HI_BOOL bATMOSLock = HI_FALSE;
        g_stServer.stRecordList[i].bATMOSLock = HI_FALSE;

        for (i = 0; i < SOCKET_CONNECT_MAX_NUM; i++)
        {
            if (INVALID_SOCKET_VALUE != g_stServer.stRecordList[i].s32SocketFd)
            {
               if(HI_TRUE == g_stServer.stRecordList[i].bATMOSLock)
               {
                   bATMOSLock = HI_TRUE;
                   break;
               }
        ...
        if(HI_FALSE == bATMOSLock)
        {
            HI_UNF_AUDIOTRACK_CONFIG_S stConfig;
            SETATMOSLOCK_CONFIG_S stAtmosLock;

            stAtmosLock.bEnable = HI_FALSE;
            stConfig.u32ConfigType = CONFIG_SETATMOSLOCKOUTPUT;
            stConfig.pConfigData = (HI_VOID *)&stAtmosLock;

            s32Ret = Engine_SetTrackConfig(SOURCE_ID_MAIN, &stConfig);
            if (HI_SUCCESS != s32Ret)
            {
                ELOG("CONFIG_SETATMOSLOCKOUTPUT failed(0x%x)\n", s32Ret);
            }
        }
    }
...
static HI_VOID IPCServerSetChannelMode(IPC_CMM* pstCmmDate)
{
    HI_S32 s32Ret;
    HI_HANDLE hTrack;
    SOURCE_ID_E enSourceID = SOURCE_ID_BUTT;
    HI_UNF_TRACK_MODE_E enMode = HI_UNF_TRACK_MODE_BUTT;
    ...
    enMode = (HI_UNF_TRACK_MODE_E)pstCmmDate->aPayload[0];

    s32Ret = Engine_SetTrackChannelMode(enMode);
    if (HI_SUCCESS != s32Ret)
    {
        ELOG("Engine_SetTrackChannelMode(enMode:%d) failed(0x%x)\n", enMode, s32Ret);
    }
...
static HI_VOID IPCServerGetChannelMode(IPC_CMM* pstCmmDate)
{
    HI_S32 s32Ret;
    HI_HANDLE hTrack;
    SOURCE_ID_E enSourceID = SOURCE_ID_BUTT;
    ...
    s32Ret = Engine_GetTrackChannelMode(&enMode);
    if (HI_SUCCESS != s32Ret)
    {
        ELOG("Engine_GetTrackChannelMode(enMode:%d) failed(0x%x)\n", enMode, s32Ret);
    }
...
static HI_VOID IPCServerSetTrackPrescale(IPC_CMM* pstCmmDate)
{
    ...
    HI_WARN_AO("enSourceID:%d int:%d dec:%d\n", enSourceID, stPrescale.s32IntegerGain, stPrescale.s32DecimalGain);
...
static HI_VOID IPCServerForceFlushTrack(HI_HANDLE hTrack)
{
    ...
    DLOG("Force Source_Server_FlushTrack hTrack(0x%x) Ret(0x%x)\n", hTrack, s32Ret);
...
static HI_VOID IPCServerSetAtmosLockEnable(HI_S32 s32SocketFd, IPC_CMM* pstCmmDate)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 i;
    HI_S32 s32CurIndex = 0;
    HI_UNF_AUDIOTRACK_CONFIG_S stConfig = {0};
    SETATMOSLOCK_CONFIG_S stAtmosLock = {0};

    //Search for current SocketFD index
    for (s32CurIndex = 0; s32CurIndex < SOCKET_CONNECT_MAX_NUM; s32CurIndex++)
    {
        if (s32SocketFd == g_stServer.stRecordList[s32CurIndex].s32SocketFd)
        {
            break;
        }
    }
    if (SOCKET_CONNECT_MAX_NUM == s32CurIndex)
    {
        DLOG("Socket(0x%x) is not created\n", s32SocketFd);
        s32Ret = HI_FAILURE;
        goto EXIT;
    ...
    // if not all false, exit
    if(HI_FALSE == pstCmmDate->aPayload[0])
    {
        for (i = 0; i < SOCKET_CONNECT_MAX_NUM; i++)
        {
            if (INVALID_SOCKET_VALUE != g_stServer.stRecordList[i].s32SocketFd && i != s32CurIndex)
            {
               if(HI_TRUE == g_stServer.stRecordList[i].bATMOSLock)
               {
                   break;
        ...
        {
            g_stServer.stRecordList[s32CurIndex].bATMOSLock = (HI_BOOL)pstCmmDate->aPayload[0];
            goto EXIT;
        }
    }

    stAtmosLock.bEnable = (HI_BOOL)pstCmmDate->aPayload[0];
    stConfig.u32ConfigType = CONFIG_SETATMOSLOCKOUTPUT;
    stConfig.pConfigData = (HI_VOID *)&stAtmosLock;
    ...
    g_stServer.stRecordList[s32CurIndex].bATMOSLock = stAtmosLock.bEnable;
...
static HI_VOID IPCServerGetAtmosLockEnable(HI_S32 s32SocketFd, IPC_CMM* pstCmmDate)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 s32CurIndex = 0;
    ...
    pstCmmDate->aPayload[0] = (HI_BOOL)g_stServer.stRecordList[s32CurIndex].bATMOSLock;
...
static HI_VOID IPCServerCmdProcess(HI_S32 s32SocketFd, IPC_CMM* pstCmmDate)
{
    ...
    switch (pstCmmDate->u32Command)
    {
        ...
        case CMD_SOURCE_GETSTREAMINFO:
            ...
            break;
        case CMD_SOURCE_SETCHANNELMODE:
            HI_WARN_AO("process SetChannelMode\n");
            IPCServerSetChannelMode(pstCmmDate);
            break;
        case CMD_SOURCE_GETCHANNELMODE:
            HI_WARN_AO("process GetChannelMode\n");
            IPCServerGetChannelMode(pstCmmDate);
            break;
        case CMD_SOURCE_SETATMOSLOCKEnable:
            HI_WARN_AO("process SETATMOSLOCK\n");
            IPCServerSetAtmosLockEnable(s32SocketFd, pstCmmDate);
            break;
        case CMD_SOURCE_GETATMOSLOCKEnable:
            HI_WARN_AO("process GETATMOSLOCK\n");
            IPCServerGetAtmosLockEnable(s32SocketFd, pstCmmDate);
            break;
        default:
...
static HI_VOID* HiClientIpcthread(HI_VOID* pClientFd)
{
    ...
    while (1)
    {
        ...
        s32DoneBytes = read(s32SocketFd, &stCmmDate, u32CmmBytes);
        if (s32DoneBytes <= 0)
        {
