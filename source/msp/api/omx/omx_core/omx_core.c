...
#ifdef HI_LOWRAM_SUPPORT
#define MAX_SECURE_INSTANCES                (1) // total instances of all components
#else
#define MAX_SECURE_INSTANCES                (3)
#endif
...
static OMX_U32 g_SecDecCompCnt = 0;
static pthread_mutex_t g_Mutex = PTHREAD_MUTEX_INITIALIZER;

static OMX_PTR pModules[MAX_INSTANCES] = {0};
static OMX_PTR pSecureModules[MAX_INSTANCES] = {0};
        ...
        OMX_CALLBACKTYPE *pCallBacks)
{
    ...
    OMX_U32 i = 0;
    OMX_BOOL bSecureComponent = OMX_FALSE;
    ...
    DEBUG_PRINT("%s :: cComponentName= %s\n", __func__, cComponentName);

    if (!strncmp(cComponentName, OMX_VDEC_SECURE_COMPONENT_NAME, MAX(strlen(cComponentName), strlen(OMX_VDEC_SECURE_COMPONENT_NAME))))
    {
        bSecureComponent = OMX_TRUE;
        DEBUG_PRINT_ERROR("%s :: cComponentName= %s is secure\n", __func__, cComponentName);
    }

    DEBUG_PRINT("%s :: g_SecDecCompCnt = %ld \n", __func__, g_SecDecCompCnt);

    if (i >= COUNTOF(pModules) || (g_SecDecCompCnt >= MAX_SECURE_INSTANCES && bSecureComponent == OMX_TRUE))
    {
    ...
    if (bSecureComponent == OMX_TRUE)
    {
        pSecureModules[i] = pModules[i];
        g_SecDecCompCnt++;
    }
...
CLEAN_UP:
    ...
    if (pSecureModules[i] != NULL)
    {
        if (g_SecDecCompCnt > 0)
        {
            g_SecDecCompCnt--;
        }
        else
        {
            DEBUG_PRINT_ERROR("%s :: Secure Component count = 0\n", __func__);
        }
    }
    pSecureModules[i] = NULL;
#endif
...
OMX_ERRORTYPE OMX_FreeHandle(OMX_HANDLETYPE hComponent)
{
    ...
    DEBUG_PRINT_ERROR("%s :%d: i:%ld pSecureModules[i]:%p g_SecDecCompCnt:%ld\n", __func__, __LINE__, i, pSecureModules[i], g_SecDecCompCnt);
