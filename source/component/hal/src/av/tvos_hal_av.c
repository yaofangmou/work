...
#else
#define AV_VIDEO_BUFF_SIZE          (5 * 1024 * 1024)
#endif
#endif
...
static inline BOOL AV_SDK_BOOL_TO_HAL(HI_BOOL bUnfVal)
{
    return (BOOL)((HI_TRUE == bUnfVal) ? TRUE : FALSE);
}
    ...
    *penSdkEvent2 = HI_UNF_AVPLAY_EVENT_BUTT;
    ...
    switch (enHalEvt)
    {
        ...
        case AV_VID_EVT_PICTURE_DECODING_ERROR:
            *penSdkEvent2 =  HI_UNF_AVPLAY_EVENT_VID_ERR_RATIO;
            break;
...
static S32 AV_SDK_GetStatus(HANDLE hAvPlayerSdk, const AV_HAL_INFO_S* pstAvInfo, AV_STATUS_S* pstStatus)
{
    ...
    if (AV_AUD_STREAM_TYPE_BUTT != pstAvInfo->stAvSettings.enAudStreamType)
    {
        ...
        pstStatus->stAudStatus.u32Chanel            = stStreaminfo.stAudStreamInfo.u32Channel;
#endif
                              ...
                              VOID *pPara)
{
    ...
    switch (enEvent)
    {
        ...
        case HI_UNF_AVPLAY_EVENT_VID_FRAME_INFO:
            ...
            stUvmosFrameInfo.u32DecoderBufferState = pstFrameInfo->u32DecoderBuffer;
                           ...
                           S64 s64Pts)
{
    ...
    HAL_CHK_RETURN((0 == u32HeaderLen && 0 == u32EsLength && 0 == u32PrivateLen), ERROR_INVALID_PARAM, "parameter is invalid.");
    HAL_CHK_RETURN((NULL == pHeader && NULL == pEsData && NULL == pvPrivate), ERROR_NULL_PTR, "parameter is NULL.");
    ...
    if (HI_UNF_AVPLAY_BUF_ID_ES_VID == enAvBufId)
    {
        ...
        HI_UNF_AVPLAY_STATUS_INFO_S stStatusInfo;
        HI_U32 u32CurrentAvailableSize = 0;

        s32Ret = HI_MPI_AVPLAY_CheckIfSupportRandomStream(hAvHandleSdk,&bEofSupport);
        HAL_CHK_RETURN_NO_PRINT((SUCCESS != s32Ret), FAILURE);

        if (bEofSupport)
        {
            s32Ret = HI_UNF_AVPLAY_GetStatusInfo(hAvHandleSdk, &stStatusInfo);
            HAL_CHK_RETURN_NO_PRINT((SUCCESS != s32Ret), FAILURE);

            u32CurrentAvailableSize = stStatusInfo.stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_VID].u32CurrentAvailableSize;
            if ( u32CurrentAvailableSize > 0 && u32CurrentAvailableSize < u32ReqLen)
            {
                u32ReqLen = stStatusInfo.stBufStatus[HI_UNF_AVPLAY_BUF_ID_ES_VID].u32CurrentAvailableSize;
            }
        }
    }
                              ...
                              DOLBYPLUS_DECODE_OPENCONFIG_S* pDolbyDecConfig)
{
    ...
    switch (stAdecAttr.enType)
    {
        case HA_AUDIO_ID_PCM: /*lint !e778*/
            ...
            {
                if (NULL != pstAdecParam->pu8Extradata)
                {
                     ...
                     const AV_CREATE_PARAMS_S* const pstCreateParams)
{
    ...
    *phAvHandle = s_stAvHandleInfo[u32Index].pstAvHalInfo->hAvPlayerSdk; /*av handle is the same as avplay handle*/
                             ...
                             AV_CAPABILITY_S* pstCapability)
{
    ...
    HAL_MEMSET_NO_VERIFY(&stChipAttr, 0, sizeof(HI_SYS_CHIP_ATTR_S));
    ...
    s32Ret = HI_SYS_GetChipAttr(&stChipAttr);

    if (HI_SUCCESS == s32Ret)
    {
        pstCapability->stDecoderCapability.au32AudDecode[AV_AUD_STREAM_TYPE_DTS]  = AV_SDK_BOOL_TO_HAL(stChipAttr.bDTSSupport);
        pstCapability->stDecoderCapability.au32AudDecode[AV_AUD_STREAM_TYPE_AC3]  = AV_SDK_BOOL_TO_HAL(stChipAttr.bDolbySupport);
        pstCapability->stDecoderCapability.au32AudDecode[AV_AUD_STREAM_TYPE_EAC3] = AV_SDK_BOOL_TO_HAL(stChipAttr.bDolbySupport);
    }
...
#ifdef HAL_HISI_EXTEND_H
static S32 av_inject_get_freebuf_ex(struct _AV_DEVICE_S* pstDev, const HANDLE hInjecter, U32 u32ReqLen,
                                AV_INJECT_BUF_TYPE_E eReqBufType, VOID** ppvBufFree, U32* pu32FreeSize)
{
    HAL_CHK_RETURN((0 == AV_IS_Init()), ERROR_NOT_INITED, "AV not init.");
    HAL_CHK_RETURN(((NULL == ppvBufFree) || (NULL == pu32FreeSize)), ERROR_NULL_PTR, "input exist null ptr");
    HAL_CHK_RETURN((AV_INVALID_HANDLE == hInjecter), ERROR_AV_INJECT_NOT_OPENED, "AV hInjecter is not opened.");
    HAL_CHK_RETURN(((AV_INJECT_BUF_TYPE_COMMON > eReqBufType) ||
                    (AV_BUFFER_TYPE_BUTT <= eReqBufType)), ERROR_AV_PARAMETER_INVALID, "eReqBufType is invalid.");
...
{
    HAL_CHK_RETURN((0 == AV_IS_Init()), ERROR_NOT_INITED, "AV not init.");
    HAL_CHK_RETURN((NULL == pstHDR10Info), ERROR_NULL_PTR, "input exist null ptr.");
    HAL_CHK_RETURN((AV_INVALID_HANDLE == hAvHandle), ERROR_AV_PLAYER_NOT_CREATED, "input invalid av handle.");
...
S32 av_set_metadata_info(struct _AV_DEVICE_S* pstDev, const HANDLE hAvHandle, const AV_VIDEO_METADATA_INFO_S* pstMetaDataInfo)
{
    ...
    HAL_CHK_RETURN((NULL == pstMetaDataInfo), ERROR_NULL_PTR, "input exist null ptr");
    HAL_CHK_RETURN((0 == AV_IS_Init()), ERROR_NOT_INITED, "AV not init.");
    HAL_CHK_RETURN((AV_INVALID_HANDLE == hAvHandle), ERROR_AV_PLAYER_NOT_CREATED, "input invalid av handle.");
...
static int av_dev_open(const struct hw_module_t *module, const char *name, struct hw_device_t **device)
{
...
#ifdef HAL_HISI_EXTEND_H
    pstAvDev->av_inject_get_freebuf_ex    = av_inject_get_freebuf_ex;
    ...
    pstAvDev->av_set_hdr10_info              = av_set_hdr10_info;
...
static AV_DEVICE_S s_stDevice_av =
{
...
#ifdef HAL_HISI_EXTEND_H
    .av_inject_get_freebuf_ex       = av_inject_get_freebuf_ex,
    ...
    .av_invoke                      = av_invoke,
    .av_set_hdr10_info              = av_set_hdr10_info,
