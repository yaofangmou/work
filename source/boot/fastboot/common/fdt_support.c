...
static int fdt_fixup_stdout(void *fdt, int chosenoff)
{
	...
	snprintf(sername, sizeof(sername), "serial%d", CONFIG_CONS_INDEX - 1);
...
void fdt_fixup_ethernet(void *fdt)
{
	...
	for (prop = 0; ; prop++) {
		...
		if (!strncmp(name, "ethernet", 8)) {
			...
			if (!strncmp(name, "ethernet", sizeof("ethernet"))
#ifdef FDT_SEQ_MACADDR_FROM_ENV
			 || !strncmp(name, "ethernet0", sizeof("ethernet0"))
#endif
			)
				i = 0;
			...
			if (i != -1) {
				if (i == 0)
					strncpy(mac, "ethaddr", sizeof(mac));
				else
			...
			fdt_prop = fdt_get_property(fdt, nodeoff, "status",
						    NULL);
			if (fdt_prop && !strncmp(fdt_prop->data, "disabled", sizeof("disabled")))
				continue;
			i++;
#endif
			tmp = getenv(mac);
...
static int of_bus_isa_match(const void *blob, int parentoffset)
{
	...
	return !strncmp(name, "isa", sizeof("isa"));
}
			...
			enum fdt_status status, unsigned int error_code)
{
	...
	case FDT_STATUS_FAIL_ERROR_CODE:
		snprintf(buf, sizeof(buf), "fail-%d", error_code);
