...
#ifdef HI_VDP_TECHNICOLOR_SUPPORT
/*===============================================================================
 Tecnicolor source
 ===============================================================================*/
#define TECHNICOLOR_OETF_BT1886         0
#define TECHNICOLOR_OETF_PQ             1
#define TECHNICOLOR_OETF_LINEAR         2
#define TECHNICOLOR_YUV_FULL_RANGE      1
#define TECHNICOLOR_YUV_LIMITED_RANGE   0
#define TECHNICOLOR_SDR_BRIGHTNESS      100
#define TECHNICOLOR_HDR_BRIGHTNESS      1000

#ifdef ANDROID
static HI_CHAR* pszTecnicolorDllName = "/system/lib/libtechnicolor.so";
#else
static HI_CHAR* pszTecnicolorDllName = "/usr/lib/libtechnicolor.so";
#endif

static HI_VOID* pTechnicolorDllModule               = HI_NULL;
TECHNICOLOR_UNIVERSAL_FUN_S s_TechnicolorFuncEntry  = {0};
HI_TECHNICOLOR_PS_CFG_S g_stTechnicolorPsCfg;
#endif
...
HI_S32 HI_MPI_WIN_AllocateBuf(const HI_CHAR *szBufName, HI_U32 u32BufSize, HI_MMZ_BUF_S *pstBuf)
{
    if (HI_NULL == pstBuf)
    {
        return HI_ERR_VO_NULL_PTR;
    ...
    if (HI_NULL != pstBuf->phyaddr)
    {
        HI_ERR_WIN("buffer has been allocated!\n");
        return HI_ERR_VO_MALLOC_FAILED;
    }

    memset(pstBuf,0x0,sizeof(HI_MMZ_BUF_S));

    strncpy(pstBuf->bufname, szBufName, sizeof(pstBuf->bufname) - 1);
    pstBuf->bufname[sizeof(pstBuf->bufname)-1] = '\0';
    pstBuf->bufsize = u32BufSize;

#ifdef HI_SMMU_SUPPORT
    pstBuf->phyaddr = HI_MPI_SMMU_New(pstBuf->bufname, pstBuf->bufsize);
    if (HI_NULL == pstBuf->phyaddr )
    {
        HI_FATAL_WIN("HI_MPI_SMMU_New size=%d failed!\n", pstBuf->bufsize);
        return HI_ERR_VO_MALLOC_FAILED;
    }

    pstBuf->user_viraddr = (HI_U8*)HI_MPI_SMMU_Map(pstBuf->phyaddr, HI_FALSE);
    if (HI_NULL == pstBuf->user_viraddr )
    {
        HI_FATAL_WIN("HI_MPI_SMMU_Map size=%d failed!\n", pstBuf->bufsize);
        return HI_ERR_VO_NULL_PTR;
    }
#else
    if (HI_MPI_MMZ_Malloc(pstBuf) != HI_SUCCESS)
    {
        HI_FATAL_WIN("HI_MPI_MMZ_Malloc size=%d failed!\n", pstBuf->bufsize);
...
HI_VOID HI_MPI_WIN_ReleaseBuf(HI_MMZ_BUF_S *pstBuf)
{
    HI_S32 ret = 0;
    ...
    if (HI_NULL != pstBuf->phyaddr)
    {
        if(HI_NULL != pstBuf->user_viraddr)
        {
            ret = HI_MPI_SMMU_Unmap(pstBuf->phyaddr);
            if (HI_SUCCESS != ret)
            {
                HI_ERR_WIN("HI_MPI_SMMU_Unmap failed\n");
                return;
            }
        }
        HI_MPI_SMMU_Delete(pstBuf->phyaddr);
    }
#else
    if (HI_NULL != pstBuf->phyaddr)
    {
        HI_MPI_MMZ_Free(pstBuf);
    }
#endif
    memset(pstBuf, 0x0, sizeof(HI_MMZ_BUF_S));
...
{
    return HI_MPI_WIN_AllocateBuf("VDP_DolbyVisionBuf", DOLBY_BUF_SIZE, pstDolbyVisionBuf);
}

HI_VOID HI_MPI_WIN_ReleaseDolbyBuf(HI_MMZ_BUF_S *pstDolbyVisionBuf)
{
    HI_MPI_WIN_ReleaseBuf(pstDolbyVisionBuf);
    return;
}

/*
HI_S32  HI_MPI_WIN_AllocateTechnicolorBuf(HI_MMZ_BUF_S *pstTechnicolorBuf)
{
    return HI_MPI_WIN_AllocateBuf("VDP_TechnicolorVisionBuf",
                   sizeof(HI_TECHNICOLOR_PS_CFG_S), pstTechnicolorBuf);
}

HI_VOID HI_MPI_WIN_ReleaseTechnicolorBuf(HI_MMZ_BUF_S *pstTechnicolorBuf)
{
    HI_MPI_WIN_ReleaseBuf(pstTechnicolorBuf);
    return;
}
*/
...
#ifdef HI_VDP_TECHNICOLOR_SUPPORT
HI_S32 HI_MPI_WIN_RegisterTechnicolorLib(HI_VOID)
{
    HI_S32  s32Ret = HI_FAILURE;
    WIN_TECHNICOLOR_LIB_INFO_S stTechnicolorLibInfo;
    technicolor_version_info_s technicolor_version_info;

    memset(&stTechnicolorLibInfo, 0x0, sizeof(stTechnicolorLibInfo));
    memset(&technicolor_version_info, 0x0, sizeof(technicolor_version_info));

    pTechnicolorDllModule = HI_NULL;
    pTechnicolorDllModule = dlopen(pszTecnicolorDllName, RTLD_LAZY | RTLD_GLOBAL);
    if (HI_NULL == pTechnicolorDllModule)
    {
        HI_WARN_WIN("Register %s fail:%s.\n", pszTecnicolorDllName, dlerror());
        memset(&s_TechnicolorFuncEntry, 0x0, sizeof(s_TechnicolorFuncEntry));
        return HI_FAILURE;
    }

    s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface = dlsym(pTechnicolorDllModule, "technicolor_lib_wrap_interface");

    if (HI_NULL == s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface)
    {
        HI_ERR_WIN("Open s_TechnicolorFuncEntry functions fail.\n");
        memset(&s_TechnicolorFuncEntry, 0x0, sizeof(s_TechnicolorFuncEntry));
        return HI_FAILURE;
    }

    s32Ret = s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface(CMD_TECHNICOLOR_GET_VERSION_INFO, NULL, &technicolor_version_info);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_WIN("Get technicolorlib info fail.\n");
        memset(&s_TechnicolorFuncEntry, 0x0, sizeof(s_TechnicolorFuncEntry));
        return HI_FAILURE;
    }

    memset(stTechnicolorLibInfo.cVersion, 0, sizeof(stTechnicolorLibInfo.cVersion));
    memset(stTechnicolorLibInfo.cBuildtime, 0, sizeof(stTechnicolorLibInfo.cBuildtime));

    strncpy(stTechnicolorLibInfo.cVersion, technicolor_version_info.pversion, sizeof(stTechnicolorLibInfo.cVersion) - 1);
    strncpy(stTechnicolorLibInfo.cBuildtime, technicolor_version_info.pbuild_time, sizeof(stTechnicolorLibInfo.cBuildtime) - 1);

    stTechnicolorLibInfo.cVersion[sizeof(stTechnicolorLibInfo.cVersion)-1] = '\0';
    stTechnicolorLibInfo.cBuildtime[sizeof(stTechnicolorLibInfo.cBuildtime)-1] = '\0';

    stTechnicolorLibInfo.bValid = HI_TRUE;

    s32Ret = HI_MPI_WIN_SetTechnicolorLibInfo(&stTechnicolorLibInfo);

    return s32Ret;
}


HI_VOID HI_MPI_WIN_UnRegisterTechnicolorLib(HI_VOID)
{
    WIN_TECHNICOLOR_LIB_INFO_S stTechnicolorLibInfo;
    memset(&stTechnicolorLibInfo, 0x0, sizeof(stTechnicolorLibInfo));
    ...
    //Reset lib info
    (HI_VOID)HI_MPI_WIN_SetTechnicolorLibInfo(&stTechnicolorLibInfo);

    return;
}

static HI_BOOL HI_MPI_WIN_IsTechnicolorScenes(HI_DRV_VIDEO_FRAME_TYPE_E enSrcFrameType)
{
    /* Not technicolor scenes as following condition :
        1?¡éthere is no  technicolor dll lib
        2?¡écall technicolor lib function  failed
        3?¡énot technicolor frame type */
    if (HI_NULL == pTechnicolorDllModule
        || HI_NULL == s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface
        || HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR != enSrcFrameType)
    {
        return HI_FALSE;
    }
...
static HI_S32 HI_MPI_WIN_TechnicolorGetPsCfg(WIN_XDR_FRAME_S *pXdrFrame, HI_TECHNICOLOR_PS_CFG_S *pstPsCfg)
{
    HI_S32 s32Ret                           = HI_FAILURE;
    HI_BOOL bHdrOutput                      = HI_FALSE;
    HI_U32 u32SlMetadataPhyAddr             = HI_NULL;
    HI_S32 s32TechniDispBright              = 0;
    HI_TECHNICOLOR_PS_PARA_IN_S stPsParaIn  = {0};

    if (HI_NULL == pTechnicolorDllModule
        || HI_NULL == s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface)
    {
        HI_FATAL_WIN("pTechnicolorDllModule or pfn_technicolor_lib_wrap_interface is null .\n");
        return HI_ERR_VO_NULL_PTR;
    }

    /* Check frame type */
    if (HI_DRV_DISP_TYPE_NORMAL == pXdrFrame->enDispOutType)
    {
        bHdrOutput = HI_FALSE;
    }
    else if (HI_DRV_DISP_TYPE_HDR10 == pXdrFrame->enDispOutType)
    {
        bHdrOutput = HI_TRUE;
    }
    else
    {
        HI_FATAL_WIN("enDispOutType=%d is out of range .\n", pXdrFrame->enDispOutType);
        return HI_FAILURE;
    }

    s32Ret = HI_MPI_WIN_GetTechniOutputBright(&s32TechniDispBright);
    if (HI_SUCCESS != s32Ret)
    {
        HI_FATAL_WIN("Call HI_MPI_WIN_GetTechniOutputBright failed .\n", pXdrFrame->enDispOutType);
        return s32Ret;
    }

    /* Brightness filled with defualt value when setted 0 */
    if (0 == s32TechniDispBright)
    {
        stPsParaIn.stAvsSlHdrDispSet.s32Display_Brightness
                = bHdrOutput ? TECHNICOLOR_HDR_BRIGHTNESS : TECHNICOLOR_SDR_BRIGHTNESS;
    }
    else
    {
        stPsParaIn.stAvsSlHdrDispSet.s32Display_Brightness = s32TechniDispBright;
    }

    /* Dynamic metadata trans by phy-addr */
    u32SlMetadataPhyAddr = pXdrFrame->stBaseFrm.unHDRInfo.stJTPSlHDRInfo.dynamicMetadata.u32Addr;
    if (HI_NULL == u32SlMetadataPhyAddr)
    {
        HI_FATAL_WIN("u32SlMetadataPhyAddr is null .\n");
        return HI_ERR_VO_NULL_PTR;
    }
#ifdef HI_SMMU_SUPPORT
    stPsParaIn.pstAvsSlHdrMetadata = (HI_SL_HDR_Metadata_S *)HI_MPI_SMMU_Map(u32SlMetadataPhyAddr, HI_FALSE);
#else
    stPsParaIn.pstAvsSlHdrMetadata = (HI_SL_HDR_Metadata_S *)HI_MPI_MMZ_Map(u32SlMetadataPhyAddr, HI_FALSE);
#endif

    /* Only support HDR and SDR output */
    stPsParaIn.stAvsSlHdrDispSet.s32Display_OETF
            = bHdrOutput ? TECHNICOLOR_OETF_PQ : TECHNICOLOR_OETF_BT1886;

    /* SRC YUV value */
    if (1 == pXdrFrame->stBaseFrm.stColourInfo.u8FullRangeFlag)
    {
        stPsParaIn.s32YuvRange = TECHNICOLOR_YUV_FULL_RANGE;
    }
    else
    {
        stPsParaIn.s32YuvRange = TECHNICOLOR_YUV_LIMITED_RANGE;
    }

    /* Call dynamic lib */
    s32Ret = s_TechnicolorFuncEntry.pfn_technicolor_lib_wrap_interface(CMD_TECHNICOLOR_GENERATE_CONFIG, &stPsParaIn, pstPsCfg);
    if (HI_SUCCESS != s32Ret)
    {
#ifdef HI_SMMU_SUPPORT
        s32Ret = HI_MPI_SMMU_Unmap(u32SlMetadataPhyAddr);
#else
        s32Ret = HI_MPI_MMZ_Unmap(u32SlMetadataPhyAddr);
#endif
        if (HI_SUCCESS != s32Ret)
        {
            return s32Ret;
...
#ifdef HI_VDP_TECHNICOLOR_SUPPORT
    if (HI_SUCCESS != HI_MPI_WIN_RegisterTechnicolorLib())
    {
        HI_MPI_WIN_UnRegisterTechnicolorLib();
    }
#endif
...
#ifdef HI_VDP_TECHNICOLOR_SUPPORT
static HI_S32 HI_MPI_WIN_TechnicolorLibProcess(WIN_XDR_FRAME_S *pstWinXdrFrm)
{
    HI_S32 s32Ret = HI_FAILURE;

    if (HI_TRUE != HI_MPI_WIN_IsTechnicolorScenes(pstWinXdrFrm->stBaseFrm.enSrcFrameType))
    {
        return HI_SUCCESS;
    }

    pstWinXdrFrm->enXdrEngine = DRV_XDR_ENGINE_JTP;
    pstWinXdrFrm->u64StSlHdrPsParamAddr = (uintptr_t)&g_stTechnicolorPsCfg;

    /* Get PS ip config para from dynamic lib. Only support single window now*/
    s32Ret = HI_MPI_WIN_TechnicolorGetPsCfg(pstWinXdrFrm, &g_stTechnicolorPsCfg);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_WIN("Call HI_MPI_WIN_TechnicolorGetPsCfg failed .\n");
        return s32Ret;
    }
...
#ifdef HI_VDP_TECHNICOLOR_SUPPORT
    s32Ret = HI_MPI_WIN_TechnicolorLibProcess(pstWinXdrFrame);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_WIN("TechnicolorLib process fail.\n");
        return s32Ret;
    }
#endif
...
HI_S32 HI_MPI_WIN_QueueSyncFrame(HI_HANDLE hWindow, HI_DRV_VIDEO_FRAME_S *pFrame,HI_U32 *pu32FenceFd)
{
    ...
    memset(&stSyncFrame, 0x0, sizeof(stSyncFrame));

    stSyncFrame.stXdrFrame.hWindow = hWindow;
    stSyncFrame.stXdrFrame.bEnhanceValid = HI_FALSE;
    stSyncFrame.stXdrFrame.stBaseFrm = *pFrame;

    if (pu32FenceFd == HI_NULL)
    {
        stSyncFrame.u32FenceFd = 0x0;
    }
    else
    {
        stSyncFrame.u32FenceFd = 0xffffffff;
    }

    Ret = HI_MPI_WIN_GenerateFrmCfg(&stSyncFrame.stXdrFrame);
    if (HI_SUCCESS != Ret)
    {
        return Ret;
    }

    Ret = ioctl(g_VoDevFd, CMD_WIN_QUSYNC_FRAME, &stSyncFrame);
    if ((HI_SUCCESS == Ret) && (pu32FenceFd != HI_NULL))
    {
        *pu32FenceFd = stSyncFrame.u32FenceFd;
    }
...
HI_S32 HI_MPI_WIN_SetTechnicolorLibInfo(WIN_TECHNICOLOR_LIB_INFO_S *pstTechnicolorLibInfo)
{
    HI_S32  Ret = HI_SUCCESS;

    CHECK_VO_NULL_PTR(pstTechnicolorLibInfo);

    Ret = ioctl(g_VoDevFd, CMD_WIN_SET_TECHNI_LIB_STATUS, pstTechnicolorLibInfo);
    if (Ret != HI_SUCCESS)
    {
        HI_ERR_WIN("HI_MPI_WIN_SetTechnicolorLibInfo fail \r\n");
        return Ret;
    }
...
HI_S32 HI_MPI_WIN_SetFrameProgInterleaveMode(HI_HANDLE hWindow,
                                                             HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E eForceFrameProgFlag)
{
    WIN_FORCEFRAME_FLAG_S stWinForceFrameFlag;
    ...
    if (eForceFrameProgFlag >= WIN_FRAME_FORCE_BUTT)
    {
        HI_ERR_WIN("para frame prog interleave is invalid.\n");
        return HI_ERR_VO_INVALID_PARA;
    }

    CHECK_VO_INIT();

    stWinForceFrameFlag.hWindow             = hWindow;
    stWinForceFrameFlag.eForceFrameProgFlag = eForceFrameProgFlag;

    return ioctl(g_VoDevFd, CMD_WIN_SET_FRAME_PROGINTER, &stWinForceFrameFlag);
}

HI_S32 HI_MPI_WIN_GetFrameProgInterleaveMode(HI_HANDLE hWindow,
                                                              HI_DRV_WIN_FORCEFRAMEPROG_FLAG_E *peForceFrameProgFlag)
{
    WIN_FORCEFRAME_FLAG_S stWinForceFrameFlag = {0};
    HI_S32 ret = HI_FAILURE;
    ...
    if (peForceFrameProgFlag == HI_NULL)
    {
        HI_ERR_WIN("para frame prog interleave is invalid.\n");
        return HI_ERR_VO_INVALID_PARA;
    }
    ...
    ret = ioctl(g_VoDevFd, CMD_WIN_GET_FRAME_PROGINTER, &stWinForceFrameFlag);
    if (HI_SUCCESS == ret)
    {
        *peForceFrameProgFlag = stWinForceFrameFlag.eForceFrameProgFlag;
    }

    return ret;
}

HI_S32 HI_MPI_WIN_GetTechniOutputBright(HI_S32 *ps32TechniDispBright)
{
    HI_S32 s32Ret = HI_FAILURE;
    WIN_TECHNI_DISP_BRIGHT stTechniDispBright = {0};

    CHECK_VO_NULL_PTR(ps32TechniDispBright);

    s32Ret = ioctl(g_VoDevFd, CMD_WIN_GET_TECHNI_OUT_BRIGHT, &stTechniDispBright);
    if (s32Ret != HI_SUCCESS)
    {
        return s32Ret;
    }

    *ps32TechniDispBright = stTechniDispBright.u32TechniDispBright;
