...
#ifdef CFG_DTBO_ADDR
	filesize = usb_file_read(CONFIG_FILE_DTB_NAME,(void*)CONFIG_DTB_LOAD_ADDR, 0, CONFIG_DTB_MAX_SIZE);
	memset(buf, 0 ,40);
	if (filesize > 0)
	{
		snprintf(buf, 40, "bootm 0x%x - 0x%x", CONFIG_FILE_READ_ADDR, CONFIG_DTB_LOAD_ADDR + CONFIG_DTB_HEADER_OFFSET);
	}
	else
#endif
	{
		snprintf(buf, 40, "bootm 0x%x", CONFIG_FILE_READ_ADDR);
	}
...
void download_boot(int (*handle)(void))
{
	if (_HI3798C_V200 == get_chipid() || _HI3798M_V200 == get_chipid()
		...
		|| _HI3798M_V300_H == get_chipid() || _HI3716M_V430 == get_chipid()
		|| _HI3798M_V200_H == get_chipid() || _HI3716D_V110_H == get_chipid())
