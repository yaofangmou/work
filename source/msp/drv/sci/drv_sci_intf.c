...
HI_S32 SCI_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    {
        SCI_SECURITY_MODE_S *pSciSecurityMode;

        pSciSecurityMode = (SCI_SECURITY_MODE_S *)arg;

        Ret = SCI_SetSecurityMode(pSciSecurityMode->enSciPort, pSciSecurityMode->SecurityMode);
        break;
    }
#endif
    case CMD_SCI_OPEN:
