...
extern const HDMI_AUDIO_CTS_N_S s_astAudioCtsN[];
#define CONFIRM_CTS_TRESHOLD 5

HI_U8 ResChangeTimeOut = 0;
Bool F_ResChanged = HI_FALSE;
//CmdType * CmdPtr;
...
void SI_Set_AudioInfoFramePacket ( HI_U8 * abAudioPath, HI_U8 bChSpkAlloc, HI_U8 bDM_LSV )
{
	...
	SI_CHECK_POINTER_NO_RET(abAudioPath);
    memset(abData, 0, 10);
...
HI_U32 SI_HDMI_CtsValueGet(HI_U32 u32SampleRate,HI_U32 u32TmdsClk)
{
    HI_U32              i = 0;
    HI_U32              u32TmpCts = 0xffffffff;
    HI_U32              u32TmpN = 0xffffffff;
    HDMI_AUDIO_CTS_N_S *pstAudioCtsN = (HDMI_AUDIO_CTS_N_S *)&s_astAudioCtsN[0];
    for (i = 0;i < 180; i++,pstAudioCtsN++)
    {
        if ( (pstAudioCtsN->u32AudioSmpRate == u32SampleRate) && (pstAudioCtsN->u32TmdsClk == u32TmdsClk) )
        {
            ...
            && (pstAudioCtsN->u32TmdsClk >= (u32TmdsClk - 20))
            && (pstAudioCtsN->u32TmdsClk <= (u32TmdsClk + 20)) )
        {
            u32TmpCts = pstAudioCtsN->u32CtsValue;
            u32TmpN = pstAudioCtsN->u32NValue;
    ...
    else if (u32TmpCts == 0xffffffff && u32SampleRate) /* can't find , N default 48k samprate*/
    {
