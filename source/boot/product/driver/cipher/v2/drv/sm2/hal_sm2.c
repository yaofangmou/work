...
HI_VOID HAL_SM2_Enable(HI_VOID)
{
    ...
    HAL_SET_BIT(Value, CIPHER_SM2_CRG_CLK_BIT);   /* clock opened */
    HAL_CIPHER_WriteReg(pu32Crg, Value);

    cipher_msleep(1);

    HAL_CLEAR_BIT(Value, CIPHER_SM2_CRG_RST_BIT); /* cancel reset */
    HAL_CIPHER_WriteReg(pu32Crg, Value);
...
HI_VOID HAL_SM2_Disable(HI_VOID)
{
    ...
    HAL_CIPHER_ReadReg(pu32Crg, &Value);
    HAL_SET_BIT(Value, CIPHER_SM2_CRG_RST_BIT); /* reset */
    HAL_CIPHER_WriteReg(pu32Crg,Value);
    cipher_msleep(1);

    HAL_CIPHER_ReadReg(pu32Crg, &Value);
    HAL_CLEAR_BIT(Value, CIPHER_SM2_CRG_CLK_BIT);   /* clock close */
    HAL_CIPHER_WriteReg(pu32Crg,Value);
}
