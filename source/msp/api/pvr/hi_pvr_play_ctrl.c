...
volatile PVR_PLAY_HANDLE PVR_GetPlayChns(HI_U32 ChnId)
{
    HI_PVR_LOG_ENTER();
    return &g_stPvrPlayChns[ChnId];
    HI_PVR_LOG_EXIT();
}
...
static void PVRPlayCalcTime(union sigval unSig)
{
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
        ...
        if ((HI_UNF_PVR_PLAY_STATE_STOP != pChnAttr->enState) && (HI_UNF_PVR_PLAY_STATE_INVALID != pChnAttr->enState))
        {
            /*trickPlay with audio*/
            if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pChnAttr->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
            {
                /*set base value,设置基准值，使倍速整数化*/
                s32SpeedRatio = 10;
                if (HI_UNF_PVR_PLAY_SPEED_0_5X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 5;
                }
                if (HI_UNF_PVR_PLAY_SPEED_0_8X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 8;
                }
                if (HI_UNF_PVR_PLAY_SPEED_0_9X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 9;
                }
                if (HI_UNF_PVR_PLAY_SPEED_1_1X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 11;
                }
                if (HI_UNF_PVR_PLAY_SPEED_1_2X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 12;
                }
                if (HI_UNF_PVR_PLAY_SPEED_1_5X_WITH_AUDIO == pChnAttr->enSpeed)
                {
                    s32SpeedRatio = 15;
                }
                pChnAttr->u32CurPlayTimeMs += (s32SpeedRatio*((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS))/10000000;
                pChnAttr->u32PlaySyncTime += (s32SpeedRatio*((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS))/10000000;
            }
            else
            {
                /* normal or fast mode */
                ...
                {
                    s32SpeedRatio = pChnAttr->enSpeed/HI_UNF_PVR_PLAY_SPEED_NORMAL;
                    pChnAttr->u32CurPlayTimeMs += (s32SpeedRatio*((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS))/1000000;
                    pChnAttr->u32PlaySyncTime += (s32SpeedRatio*((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS))/1000000;
                }
                else/* slow mode */
                {
                    s32SpeedRatio = (HI_S32)HI_UNF_PVR_PLAY_SPEED_NORMAL/pChnAttr->enSpeed;
                    pChnAttr->u32CurPlayTimeMs += (((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS)/s32SpeedRatio)/1000000;
                    pChnAttr->u32PlaySyncTime += (((HI_S32)PVR_TIME_CTRL_TIMEBASE_NS)/s32SpeedRatio)/1000000;
                }
            }
        }
...
static HI_S32 PVRTplayShowProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData, PVR_PLAY_CHN_S* pChnAttr)
{
        ...
        || (HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState))
    {
        ...
        if (HI_TRUE == pstFBAttr->bPVRExtract)
        {
...
static HI_S32 PVRPlayShowProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData)
{
    ...
    HI_S8 pPlayStats[PVR_PLAY_MAX_STATUS_NUM][16] = { "INVALID", "INIT", "PLAY", "PAUSE", "FF", "FB", "SF", "SB", "STEPF",
            "STEPB", "STOP", "BUTT"};
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    HI_BOOL bTrickPlayWithAudio = HI_FALSE;
#endif
    HI_S32 s32PlaySpeed = HI_UNF_PVR_PLAY_SPEED_NORMAL;
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
            ...
            && (pChnAttr->enState != HI_UNF_PVR_PLAY_STATE_BUTT))
        {
            ...
            s32PlaySpeed = (HI_S32)pChnAttr->enSpeed;
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
            bTrickPlayWithAudio = (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pChnAttr->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK));
            if (HI_TRUE == bTrickPlayWithAudio)
            {
                s32PlaySpeed = abs(s32PlaySpeed) & (~HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE);
            }
#endif
            ...
            if (HI_NULL != HI_PVR_SmoothGetVersion())
            {
                HI_PROC_Printf(pstBuf, "\t%s\n", HI_PVR_SmoothGetVersion());
            }
...
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
            HI_PROC_Printf(pstBuf, "\tPlay audio        \t:%d\n", bTrickPlayWithAudio);
#endif
            ...
            if (1 == pChnAttr->IndexHandle->pstPidCtrl->u32IndexType)
            {
                HI_PROC_Printf(pstBuf, "\t  IndexType       \t:%s\n","Video");
            }
            else if (2 == pChnAttr->IndexHandle->pstPidCtrl->u32IndexType)
            {
                HI_PROC_Printf(pstBuf, "\t  IndexType       \t:%s\n","Audio");
            }
...
static INLINE HI_S32 PVRPlayReadDataFromFile(PVR_PLAY_CHN_S* pstChn, HI_VOID* pBuf, HI_U32 u32BytesSend, HI_U64 u64Offset)
{
    HI_U64 u64Offset1 = 0;
    HI_U64 u64Offset2 = 0;
    HI_U32 u32Size1 = 0;
    ...
    {
        if (HI_TRUE == PVR_INDEX_IS_REWIND(pstChn->IndexHandle))
        {
    ...
    if (0 == u32Size1)
    {
        s32Ret = PVR_PREAD64(pBuf, u32BytesSend, pstChn->s32DataFile, u64Offset);
                                           ...
                                           HI_U32 u32HeadToAdd)
{
    ...
    if (HI_TRUE == pstChn->bPlayMainThreadStop)
    {
        return 0;
    }
    ...
    s32Ret = PVRPlayReadDataFromFile(pstChn, pBuf, u32BytesSend, u64Offset);
    if (u32BytesSend != s32Ret)
    {
        HI_ERR_PVR("Read data form file:u32BytesSend != s32Ret, u32BytesSend=%u, s32Ret=%u\n", u32BytesSend, s32Ret);
        return HI_ERR_PVR_PLAY_INDEX_BEYOND_TS;
    }
...
STATIC INLINE PVR_PLAY_CHN_S* PVRPlayFindFreeChn(HI_VOID)
{
    ...
    pChnAttr = PVR_GetPlayChns(ChanId);
...
STATIC INLINE HI_S32 PVRPlayCheckUserCfg(HI_UNF_PVR_PLAY_ATTR_S* pUserCfg, HI_HANDLE hAvplay, HI_HANDLE hTsBuffer)
{
    ...
    if ((HI_TRUE == pUserCfg->stDecryptCfg.bDoCipher) && (HI_UNF_CIPHER_ALG_DES == pUserCfg->stDecryptCfg.enType))
    {
        HI_ERR_PVR("PVR not support cipher algorithm(%d),only support AES,3DES!\n", pUserCfg->stDecryptCfg.enType);
    ...
    if (!((pUserCfg->u32FileNameLen > 0) && (strlen(pUserCfg->szFileName) == pUserCfg->u32FileNameLen)))
    {
        HI_ERR_PVR("Invalid file name!\n");
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }

    ret = PVR_GetTheStandardFileName(pUserCfg->szFileName);
    if(HI_SUCCESS != ret)
    {
        HI_ERR_PVR("get real fileName failed! szFileName: %s, ret= 0x%x\n", pUserCfg->szFileName, ret);
        return ret;
    }

    pUserCfg->u32FileNameLen = strlen(pUserCfg->szFileName);
    /* check if stream exist!                                               */
    if (!PVR_CHECK_FILE_EXIST(pUserCfg->szFileName))
    {
    ...
    if (HI_UNF_AVPLAY_STATUS_STOP != StatusInfo.enRunStatus)
    {
...
STATIC HI_VOID PVRPlaySetTsHead(HI_U8* pTsHead, HI_U32 dataStartPos)
{
    HI_U8 adapt_flag = 0;
...
*------------------------------------------------------------
*/
STATIC HI_VOID PVRPlayAddTsEnd(HI_U8* pBufAddr, HI_U32 dataEnd,  HI_U32 u32EndToAdd)
{
    HI_U8* pLastTsHead = HI_NULL;
    HI_U8  adapt_flag = 0;
    HI_U32 dataInLastTs  = 0;

    HI_PVR_DEBUG_ENTER();

    if (0 == u32EndToAdd)
    {
        HI_INFO_PVR("parameter error!\n");
        return;
    }
    ...
    RVRPlaySetAdptFiled(pLastTsHead,
                        ((u32EndToAdd + PVR_TS_HEAD_SIZE) == PVR_PACKET_LEN_188) ? PVR_TS_ADAPT_ADAPT_ONLY : PVR_TS_ADAPT_BOTH);
                                     ...
                                     HI_U32 u32BytesSend)
{
    ...
    PVR_Index_GetStartAndEndPosByIndex(pChnAttr->IndexHandle, &stDataAttr);
    ...
    if ((HI_TRUE == pChnAttr->bSecTsBuffer) && (HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode))
    {
        ...
        {
            s32Ret = pChnAttr->readCallBack(&stDataAttr, pChnAttr->stReeBuf.pUserAddr, pChnAttr->stReeBuf.u32SmmuAddr, (HI_U32)u64ReadOffset, u32BytesSend);
...
STATIC INLINE HI_U32 PVRDealAppendPointData(PVR_PLAY_CHN_S *pChnAttr, HI_U8 *pu8Data,
                                            HI_U32 u32BytesSend, HI_U64 u64GlobalOffset,
                                            HI_U32 u32CipherHead, HI_U32 u32CipherEnd,
                                            HI_U32 u32HeadToAdd, HI_U32 u32EndToAdd)
{
    ...
    PVR_INDEX_HANDLE IndexHandle = HI_NULL;
    ...
    u32CurFrameNum = PVR_Index_GetPrevFrameNum(IndexHandle->u32ReadFrame, &IndexHandle->stCycMgr);

    /*如果当前所送帧为start帧,则不进行边界判断*/
    if (u32CurFrameNum != IndexHandle->stCycMgr.u32StartFrame)
    {
        ret = PVR_Index_GetFrameByNum(IndexHandle, &stPrevFrame, PVR_Index_GetPrevFrameNum(u32CurFrameNum, &IndexHandle->stCycMgr));
        ...
        {
            ret = PVR_Index_GetFrameByNum(IndexHandle, &stNextFrame, PVR_Index_GetNextFrameNum(u32CurFrameNum, &IndexHandle->stCycMgr));
            PVR_Index_GetIndexInfo(IndexHandle, &stNextFrame, PVR_INDEX_GLOBAL_OFFSET, &u64FrameGlobalOffset);
...
STATIC INLINE HI_S32 PVRPlaySendToTsBuffer(PVR_PLAY_CHN_S* pChnAttr, HI_U64 u64ReadOffset,
                                           HI_U64 u64GlobalOffset, HI_U32 u32BytesSend,
                                           HI_U32 u32CipherHead, HI_U32 u32CipherEnd,
                                           HI_U32 u32HeadToAdd, HI_U32 u32EndToAdd)
{
    HI_S32   ret = HI_SUCCESS;
    ...
    {
        HI_INFO_PVR("cipher decrypt failed!\n");
        return ret;
    }
...
STATIC INLINE HI_S32 PVRPlaySendData(PVR_PLAY_CHN_S* pChnAttr, HI_U64 u64OffSet, HI_U32 u32BytesToSend, HI_U64 u64GlobalOffset)
{
    ...
    if ((u64GlobalOffset > u32HeadToAdd + PVR_CIPHER_PKG_LEN + u32CipherHead) && (64 >= u32CipherHead))
    {
        HI_INFO_PVR("globaloffset:0x%08llx;u32HeadToAdd:0x%08x; u32CipherHead:0x%08x\n", u64GlobalOffset, u32HeadToAdd, u32CipherHead);
    ...
    if ((HI_UNF_PVR_SECURE_MODE_TEE == pChnAttr->stUserCfg.enSecureMode) && ((0 != u32HeadToAdd) || (0 != u32EndToAdd)))
    {
...
STATIC HI_VOID PVRPlayPreparePrivatePacket(PVR_PLAY_CHN_S* pChnAttr,
                                           ...
                                           HI_U32 u32Size)
{
    HI_U32* pu32DisplayTime = HI_NULL;
    ...
    if (HI_SUCCESS != ret)
    {
        if ((HI_ERR_DMX_NOAVAILABLE_BUF == ret) || (HI_ERR_DMX_TIMEOUT == ret))
        {
            while (HI_SUCCESS != ret)
            {
                ...
                {
                    HI_WARN_PVR("Play main thread stop!\n");
                    return HI_FAILURE;
                }
 ...
 *****************************************************************************/
STATIC HI_VOID PVRPlayCheckError(PVR_PLAY_CHN_S*  pChnAttr,  HI_S32 ret)
{
    HI_PVR_DEBUG_ENTER();
...
STATIC INLINE HI_S32 PVRPlaySeekToCurFrame(PVR_INDEX_HANDLE handle, PVR_PLAY_CHN_S* pChnAttr, HI_UNF_PVR_PLAY_STATE_E enCurState)
{
    HI_S32 ret = HI_SUCCESS;
    ...
    if (HI_TRUE == pChnAttr->bEndOfFile)
    {
        ...
        {
            pChnAttr->u32CurPlayTimeMs = u32SeekToTime;
        }
        return HI_SUCCESS;
    }
...
STATIC HI_S32 PVRPlayResetToCurFrame(PVR_PLAY_CHN_S*  pChnAttr)
{
    HI_S32 ret = HI_SUCCESS;
...
STATIC INLINE HI_S32 PVRPlayDealWithRecReachPlay(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    if (HI_FALSE == pChnAttr->IndexHandle->bIsRec)
    {
        HI_PVR_DEBUG_EXIT();
        return HI_SUCCESS;
    }
...
STATIC INLINE HI_BOOL PVRPlayCheckTimeShiftForwardTillEnd(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    if ((HI_FALSE == pChnAttr->IndexHandle->bIsRec) || (0 > pChnAttr->enSpeed))
    {
        HI_INFO_PVR("is not record or speed less than zero!\n");
        return HI_FALSE;
    }
    ...
    HI_INFO_PVR("PVRPlayCheckTimeShiftForwardTillEnd failed!\n");
    return HI_FALSE;
}
...
STATIC PVR_PLAY_CHN_S* PVRPlayGetChnAttrByName(const HI_CHAR* pFileName)
{
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
        ...
        {
            HI_PVR_LOG_EXIT();
            return (PVR_PLAY_CHN_S*)(pChnAttr);
        }
    }

    HI_WARN_PVR("PVRPlayGetChnAttrByName failed!\n");
...
STATIC HI_UNF_SYNC_PROC_E PVRPlaySyncPlayTimeFF(PVR_PLAY_CHN_S* pChnAttr, HI_U32 u32CurPlayTimeMs, HI_U32 u32Pts)
{
    ...
    HI_U32 u32ReasonableDelValue = 1000*abs(pChnAttr->enSpeed)/HI_UNF_PVR_PLAY_SPEED_NORMAL;
    ...
    if (pChnAttr->u32PlaySyncTime + u32ReasonableDelValue < u32CurPlayTimeMs)
    {
        ...
        if (pChnAttr->u32RepeatFrameNum > 3)
        {
    ...
    else if (pChnAttr->u32PlaySyncTime > u32CurPlayTimeMs + u32ReasonableDelValue)
    {
        ...
        if (pChnAttr->u32DiscardFrameNum > 3)
        {
            enSyncResult = HI_UNF_SYNC_PROC_PLAY;
            pChnAttr->u32DiscardFrameNum = 0;
        }
...
STATIC HI_S32 PVRPlaySetFrmRateAutoDetect(PVR_PLAY_CHN_S*  pChnAttr)
{
    ...
    {
        s32Ret = PVRPlaySetFrmRate(pChnAttr->hAvplay, HI_UNF_AVPLAY_FRMRATE_TYPE_PTS, 0, 0);
        HI_PVR_DEBUG_EXIT();
        return s32Ret;
    }
...
STATIC HI_VOID PVRPlayGetTplayAttrBySpeed(HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed, HI_UNF_AVPLAY_TPLAY_OPT_S* pstTPlayOpt)
{
    pstTPlayOpt->enTplayDirect = (enPlaySpeed >= 0) ? HI_UNF_AVPLAY_TPLAY_DIRECT_FORWARD : HI_UNF_AVPLAY_TPLAY_DIRECT_BACKWARD;
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(enPlaySpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
    {
        /*带声音播放的倍速*/
        if (HI_UNF_PVR_PLAY_SPEED_0_5X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 0;
            pstTPlayOpt->u32SpeedDecimal = 500;
        }
        else if (HI_UNF_PVR_PLAY_SPEED_0_8X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 0;
            pstTPlayOpt->u32SpeedDecimal = 800;
        }
        else if (HI_UNF_PVR_PLAY_SPEED_0_9X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 0;
            pstTPlayOpt->u32SpeedDecimal = 900;
        }
        else if (HI_UNF_PVR_PLAY_SPEED_1_1X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 1;
            pstTPlayOpt->u32SpeedDecimal = 100;
        }
        else if (HI_UNF_PVR_PLAY_SPEED_1_2X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 1;
            pstTPlayOpt->u32SpeedDecimal = 200;
        }
        else if (HI_UNF_PVR_PLAY_SPEED_1_5X_WITH_AUDIO == enPlaySpeed)
        {
            pstTPlayOpt->u32SpeedInteger = 1;
            pstTPlayOpt->u32SpeedDecimal = 500;
        }
...
#endif
    {
        ...
        {
            pstTPlayOpt->u32SpeedInteger = abs(enPlaySpeed / HI_UNF_PVR_PLAY_SPEED_NORMAL);
            pstTPlayOpt->u32SpeedDecimal = 0;
        }
        else
        {
            pstTPlayOpt->u32SpeedInteger = 0;
            pstTPlayOpt->u32SpeedDecimal = (abs(enPlaySpeed) * 1000) / HI_UNF_PVR_PLAY_SPEED_NORMAL;
        }
    }
...
STATIC HI_S32 PVRPlaySetDecoderSpeed(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    ...
    memset(&stTPlayOpt, 0x00, sizeof(stTPlayOpt));
    if (HI_TRUE == pChnAttr->bRecordedVideoExist)
    {
#if defined(CHIP_TYPE_hi3716mv310) || defined(CHIP_TYPE_hi3716h) || defined(CHIP_TYPE_hi3716c) || defined(CHIP_TYPE_hi3716m)
...
#else
        (HI_VOID)PVRPlayGetTplayAttrBySpeed(enPlaySpeed, &stTPlayOpt);
...
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    /*如果当前播放速度为trickWithAudio，则关闭avplay音频Tplay使能*/
    if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pChnAttr->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
    {
        s32Ret = HI_UNF_AVPLAY_Invoke(pChnAttr->hAvplay, HI_UNF_AVPLAY_INVOKE_SET_TPLAY_AUDIO_ENABLE, (void*)&bAudioEnable);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("Set decoder speed failed! ret=%#x\n", s32Ret);
            return s32Ret;
...
STATIC HI_S32 PVRPlaySetNormalMode(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_UNF_PVR_PLAY_STATE_E enCurPlayState = HI_UNF_PVR_PLAY_STATE_BUTT;
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    HI_BOOL bAudioEnable = HI_TRUE;
#endif

    HI_PVR_LOG_ENTER();

    enCurPlayState = pChnAttr->enState;
    if ((enPlaySpeed == pChnAttr->enSpeed) && (HI_UNF_PVR_PLAY_STATE_PLAY == enCurPlayState))
    {
        HI_INFO_PVR("Have no use for change play speed! curspeed=%d, wantSpeed=%d\n", pChnAttr->enSpeed, enPlaySpeed);
        return s32Ret;
    }
    ...
    switch (enCurPlayState)
    {
        ...
        {
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
            /*如果是1倍速，则关闭Tplay使能，否则则打开avplay音频Tplay使能*/
            if (HI_UNF_PVR_PLAY_SPEED_NORMAL != enPlaySpeed)
            {
                bAudioEnable = HI_TRUE;
                s32Ret = HI_UNF_AVPLAY_Invoke(pChnAttr->hAvplay, HI_UNF_AVPLAY_INVOKE_SET_TPLAY_AUDIO_ENABLE, (void*)&bAudioEnable);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_ERR_PVR("Set decoder speed failed! ret=%#x\n", s32Ret);
...
STATIC HI_S32 PVRPlayCheckTrickParaIsValid(PVR_PLAY_CHN_S*  pChnAttr, const HI_UNF_PVR_PLAY_MODE_S* pstTrickMode)
{
#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32SpeedRate = 0;
    HI_UNF_AVPLAY_STREAM_INFO_S stStreamInfo;
    HI_U32 u32LeftDeAbility = 0;
    HI_U32 u32NeedDecodeAbility = 0;
    ...
    switch(pstTrickMode->enSpeed)
    {
        ...
        default:
        {
            HI_ERR_PVR("Doesn't support current speed playback!  speed: %d\n", pstTrickMode->enSpeed);
            return HI_ERR_PVR_NOT_SUPPORT;
        }
    }

#ifdef PVR_TRICK_WITH_AUDIO_SUPPORT
    if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pstTrickMode->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
    {
        if (0 == pChnAttr->stFBAttr.u32Width * pChnAttr->stFBAttr.u32Heigth)
        {
            s32Ret = HI_UNF_AVPLAY_GetStreamInfo(pChnAttr->hAvplay, &stStreamInfo);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_PVR("get stream info failed! ret = 0x%x\n", s32Ret);
                return s32Ret;
            }
            else if (0 == stStreamInfo.stVidStreamInfo.u32Width * stStreamInfo.stVidStreamInfo.u32Height)
            {
                HI_ERR_PVR("invalid stream info(W, H) = (%u, %u)\n", stStreamInfo.stVidStreamInfo.u32Width, stStreamInfo.stVidStreamInfo.u32Height);
                return HI_ERR_PVR_NOT_SUPPORT;
            }
        }
        else
        {
            stStreamInfo.stVidStreamInfo.u32Width = pChnAttr->stFBAttr.u32Width;
            stStreamInfo.stVidStreamInfo.u32Height = pChnAttr->stFBAttr.u32Heigth;
        }

        /*最大支持之4kp60的进行带音频特技播放*/
        if ((pstTrickMode->enSpeed & (~HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE)) > HI_UNF_PVR_PLAY_SPEED_NORMAL)
        {
            ...
            u32NeedDecodeAbility = (stStreamInfo.stVidStreamInfo.u32Width * stStreamInfo.stVidStreamInfo.u32Height
                                            * stStreamInfo.stVidStreamInfo.u32fpsInteger * u32SpeedRate) / 10;
            if (u32NeedDecodeAbility > (u32LeftDeAbility + pChnAttr->u32UseDecodeAbility))
            {
                HI_ERR_PVR("The current decoding capability does not support the speed to paly. u32NeedDecodeAbility: %u, TotalAbility: %u\n",
                            u32NeedDecodeAbility, u32LeftDeAbility + pChnAttr->u32UseDecodeAbility);
                return HI_ERR_PVR_NOT_SUPPORT;
            }
        }
    }
...
STATIC HI_S32 PVRPlaySetTrickMode(PVR_PLAY_CHN_S*  pChnAttr, HI_UNF_PVR_PLAY_SPEED_E enPlaySpeed)
{
    ...
    HI_UNF_PVR_PLAY_STATE_E enCurPlayState = pChnAttr->enState;
    ...
    if (HI_UNF_PVR_PLAY_STATE_PLAY == enPlayStateTmp)
    {
        s32Ret = PVRPlaySetNormalMode(pChnAttr, enPlaySpeed);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("Resume avplay failed! ret=%#x\n", s32Ret);
        }
    ...
    switch (enCurPlayState)
    {
        ...
        case HI_UNF_PVR_PLAY_STATE_FF:
        {
            if ((enPlayStateTmp != HI_UNF_PVR_PLAY_STATE_FB) && (enPlayStateTmp != HI_UNF_PVR_PLAY_STATE_FF))
            {
                HI_PVR_SmoothExit(&pChnAttr->stFBAttr);
            }
...
STATIC HI_S32 PVRPlaySetStepMode(PVR_PLAY_CHN_S*  pChnAttr, HI_S32 s32Direction)
{
    ...
    if ((HI_UNF_PVR_PLAY_STATE_PAUSE == enCurPlayState) || (HI_UNF_PVR_PLAY_STATE_PLAY == enCurPlayState))
    {
        HI_UNF_SYNC_ATTR_S stSyncAttr = {0};
        s32Ret = HI_UNF_AVPLAY_GetAttr(pChnAttr->hAvplay, HI_UNF_AVPLAY_ATTR_ID_SYNC, &stSyncAttr);
        if (HI_SUCCESS != s32Ret)
...
STATIC HI_S32 PVRPlayControl(PVR_PLAY_CHN_S* pChnAttr, PVR_INDEX_ENTRY_NEW_S* pstIdxEntry)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_INFO_PVR("deal with record reach play failed!\n");
        return s32Ret;
    }
    ...
    switch (pChnAttr->enState)
    {
        ...
        case HI_UNF_PVR_PLAY_STATE_FB:
        {
            ...
            memcpy((void*)pstIdxEntry, (void*)&(pChnAttr->IndexHandle->stCurPlayFrame), pChnAttr->IndexHandle->u32EntryLength);
...
static HI_S32 PVRPlaySendAframe(PVR_PLAY_CHN_S* pChnAttr, const PVR_INDEX_ENTRY_NEW_S* pstFrame)
{
    ...
    {
        HI_INFO_PVR("invalid frame type, we don't decode them!\n");
        return HI_SUCCESS;
    }
#ifdef HI_PVR_HEVC_SUPPORT
...
STATIC HI_S32 PVRPlayProcDecodeAbility(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    if (0 == pChnAttr->u32UseDecodeAbility)
    {
        s32Ret = HI_UNF_AVPLAY_GetStreamInfo(pChnAttr->hAvplay, &stStreamInfo);
        if (HI_SUCCESS != s32Ret)
        {
        ...
        if ((0 != stStreamInfo.stVidStreamInfo.u32Height)
            && (0 != stStreamInfo.stVidStreamInfo.u32Width))
        {
    ...
    {
        if ((HI_UNF_PVR_PLAY_STATE_FF == pChnAttr->enState) || (HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState))
        {
            s32Ret = ioctl(g_s32PvrFd, CMD_PVR_GET_DECODE_ABILITY, &u32LeftDeAbility);
            if (HI_SUCCESS != s32Ret)
            {
...
STATIC HI_S32 PVRPlaySmoothInit(PVR_PLAY_CHN_S** ppChnAttr)
{
    ...
    pstFBAttr->u32VoUsedFrmBufNum = PVR_Index_GetFrameBufUsed(HI_FALSE);
...
STATIC HI_VOID PVRPlayDestroyTimer(HI_VOID)
{
    ...
    for (i = 0; i < PVR_PLAY_MAX_CHN_NUM; i++)
    {
        ...
        if ((HI_UNF_PVR_PLAY_STATE_STOP != pChnAttr->enState)
            ...
            && (HI_UNF_PVR_PLAY_STATE_BUTT != pChnAttr->enState))
        {
            break;
        }
    }
...
STATIC HI_S32 PVRPlayCheckSeekCondition(PVR_PLAY_CHN_S*  pChnAttr, const HI_UNF_PVR_PLAY_POSITION_S* pPosition)
{
    ...
    if (HI_FALSE == pChnAttr->stUserCfg.bIsClearStream)
    {
    ...
    if ((SEEK_END == pPosition->s32Whence) && (pPosition->s64Offset > 0))
    {
        HI_ERR_PVR("seek from end, offset error: %d!\n", pPosition->s64Offset);
        return HI_ERR_PVR_INVALID_PARA;
    }
    if ((SEEK_CUR == pPosition->s32Whence) && (0 == pPosition->s64Offset))
    {
...
STATIC HI_VOID PVRPlayCheckToSwitchNormal(PVR_PLAY_CHN_S *pChnAttr)
{
            ...
            ||(HI_UNF_PVR_PLAY_STATE_FB == pChnAttr->enState)))
    {
        ...
        s32Ret = PVRPlaySetNormalMode(pChnAttr, HI_UNF_PVR_PLAY_SPEED_NORMAL);
        if(HI_SUCCESS != s32Ret)
        {
...
STATIC HI_S32 PVRPlaySeekBeforeStart(PVR_PLAY_CHN_S *pstChnAttr, const HI_UNF_PVR_PLAY_POSITION_S *pPosition)
{
    ...
    if ((SEEK_CUR == pPosition->s32Whence) && (pPosition->s64Offset <= 0))
    {
    ...
    while (!pChnAttr->bPlayMainThreadStop)
    {
        ...
        if (HI_SUCCESS == s32Ret)
        {
            ...
            if (HI_SUCCESS != s32Ret)
            {
                ...
                {
                    if (HI_FALSE == pChnAttr->bStartOfFile)
                    {
                        if (HI_FALSE == pChnAttr->bFlushingFlag)
                        {
                            (HI_VOID)HI_UNF_AVPLAY_FlushStream(pChnAttr->hAvplay, HI_NULL);
        ...
        else if ((HI_ERR_PVR_FILE_TILL_END == s32Ret) || (HI_ERR_PVR_NOT_SUPPORT == s32Ret))
        {
            PVRPlayCheck(pChnAttr, s32Ret);
        }
        PVR_UNLOCK(&(pChnAttr->stPlayChnMutex));
...
STATIC HI_S32 PVRPlayWaitCurrentDataConsume(PVR_PLAY_CHN_S *pChnAttr)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 i = 0;
    HI_U32 u32CurPlayFrmDisp = pChnAttr->u32CurPlayTimeMs;
    HI_U32 u32CurPlayFrmPts = PVR_INDEX_INVALID_PTSMS;
    HI_U32 u32SpeedRate = 1;
    ...
    HI_U32 u32CurSendFrmDispTimeMs = PVR_INDEX_INVALID_PTSMS;
    HI_U32 u32CurSendFrmPtsMs = PVR_INDEX_INVALID_PTSMS;
    HI_U32 u32WaitPlayTimeMs = 0;
    HI_U32 u32WaitPtsPlayTimeMs = 0;
    HI_U32 u32SourceFrmRate = 0;

    HI_PVR_LOG_ENTER();

    while(HI_TRUE == pChnAttr->bTimeShiftStartFlg)
    {
        memset(&stStatusInfo, 0x00, sizeof(stStatusInfo));
        s32Ret = HI_UNF_AVPLAY_GetStatusInfo(pChnAttr->hAvplay, &stStatusInfo);
        if ((HI_SUCCESS == s32Ret) && (-1 != stStatusInfo.stSyncStatus.u32LastVidPts))
        {
            break;
        }
    ...
    if (PVR_INDEX_INVALID_PTSMS == stStatusInfo.stSyncStatus.u32LastVidPts)
    {
        u32CurPlayFrmPts = PVR_INDEX_INVALID_PTSMS;
    }

    if (HI_SUCCESS == HI_UNF_AVPLAY_Invoke(pChnAttr->hAvplay, HI_UNF_AVPLAY_INVOKE_GET_PRIV_PLAYINFO, (void*)&stAvplayPrivInfo))
    {
        if ((PVR_INDEX_INVALID_PTSMS != stAvplayPrivInfo.u32LastPlayTime) && (HI_FALSE == pChnAttr->bTimeShiftStartFlg))
        {
            pChnAttr->u32CurPlayTimeMs = stAvplayPrivInfo.u32LastPlayTime;
            pChnAttr->u32PlaySyncTime = stAvplayPrivInfo.u32LastPlayTime;
            u32CurPlayFrmDisp = stAvplayPrivInfo.u32LastPlayTime;
        }
    }

    if ((PVR_INDEX_INVALID_PTSMS == u32CurPlayFrmPts) && (PVR_INDEX_INVALID_PTSMS == u32CurPlayFrmDisp))
    {
        HI_ERR_PVR("invalid current playTime(0x%0x) of Pts(0x%x)!\n", u32CurPlayFrmDisp, u32CurPlayFrmPts);
        return HI_FAILURE;
    }

    HI_INFO_PVR("i: %u,u32LastVidPts: 0x%x, u32FirstVidPts: 0x%x,u32CurPlayTimeMs: %u, avplayPrivTime: %u\n", i,
                stStatusInfo.stSyncStatus.u32LastVidPts, stStatusInfo.stSyncStatus.u32FirstVidPts, pChnAttr->u32CurPlayTimeMs,
                stAvplayPrivInfo.u32LastPlayTime);

    /*获取当前播放帧的信息*/
    PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, &hIndex->stCurPlayFrame, PVR_INDEX_DISP_TIME, &u32CurSendFrmDispTimeMs);
    PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, &hIndex->stCurPlayFrame, PVR_INDEX_PTS, &u32CurSendFrmPtsMs);

    /*计算当前播放PID中未播放完的时间长度,以Pts变化*/
    if ((abs(pChnAttr->enSpeed) & (~HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE)) >= HI_UNF_PVR_PLAY_SPEED_NORMAL)
    {/*快进快退或正常播放*/
        /*set base value,  11/10,6/5,3/2 整数化的最小公倍数*/
        u32SpeedRate = 10;
        if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pChnAttr->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
        {
            if (HI_UNF_PVR_PLAY_SPEED_1_1X_WITH_AUDIO == pChnAttr->enSpeed)
        ...
        {
            u32SpeedRate = abs(pChnAttr->enSpeed) * 10 / HI_UNF_PVR_PLAY_SPEED_NORMAL;
        }
        u32WaitPlayTimeMs = abs(u32CurSendFrmDispTimeMs - u32CurPlayFrmDisp) * 10 / u32SpeedRate;
        u32TimeOutMinMs = u32TimeOutMinMs * 10 / u32SpeedRate;
    }
    else
    {/*慢放*/
        /*set base value,  2,5/4,10/9 整数化的最小公倍数*/
        u32SpeedRate = 36;
        if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pChnAttr->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
        {
            if (HI_UNF_PVR_PLAY_SPEED_0_5X_WITH_AUDIO == pChnAttr->enSpeed)
        ...
        {
            u32SpeedRate = HI_UNF_PVR_PLAY_SPEED_NORMAL / abs(pChnAttr->enSpeed) * 36;
        }
        u32WaitPlayTimeMs = abs(u32CurSendFrmDispTimeMs - u32CurPlayFrmDisp) * abs(u32SpeedRate) / 36;
        u32TimeOutMinMs = u32TimeOutMinMs * abs(u32SpeedRate) / 36;
    }

    /*计算码流中平均一个GOP的总时间,将此时间作为水平线时间,若播放剩余时间小于等于此值，则可进行PID变更*/
    PVR_Index_GetFrameRate(pChnAttr->IndexHandle, &u32SourceFrmRate);
    if (0 == u32SourceFrmRate)
    {
        u32SourceFrmRate = 25;
    }
    if (0 == pChnAttr->IndexHandle->stCurPlayPidInfo.stGopInfo.u32GopTotalNum)
    {
       return HI_SUCCESS;
    }
    u32WaitPtsPlayTimeMs = (pChnAttr->IndexHandle->stCurPlayPidInfo.stGopInfo.u32FrameTotalNum * 1000 / pChnAttr->IndexHandle->stCurPlayPidInfo.stGopInfo.u32GopTotalNum) / u32SourceFrmRate;
    HI_INFO_PVR("sourceRate: %u, FrameTotalNum: %u, GopNum: %u\n", u32SourceFrmRate, pChnAttr->IndexHandle->stCurPlayPidInfo.stGopInfo.u32FrameTotalNum,
                pChnAttr->IndexHandle->stCurPlayPidInfo.stGopInfo.u32GopTotalNum);

    /*水平线余量会由于HI_UNF_AVPLAY_GetStatusInfo 接口耗时带来的误差导致切换时出现过长时间播放停止的效果，
        * 此处根据多个码流测试发现该接口耗时占比，故该改动可减小误差导致的影响*/
    if (u32WaitPlayTimeMs > u32TimeOutMinMs)
    {
        u32TimeOutMinMs = u32TimeOutMinMs + (u32WaitPlayTimeMs - u32TimeOutMinMs) * 17 / 100;
    }

    while (u32WaitPlayTimeMs > u32TimeOutMinMs)
    {
        if (HI_TRUE == pChnAttr->bPlayMainThreadStop)
        {
            HI_WARN_PVR("stop channel Already!\n");
            return HI_SUCCESS;
        }
        ...
        if (abs(stStatusInfo.stSyncStatus.u32LastVidPts - u32CurSendFrmPtsMs) <= u32WaitPtsPlayTimeMs)
        {
            HI_INFO_PVR("u32LastVidPts: 0x%x, u32WaitPlayTimeMs: %u, u32CurSendFrmPtsMs: 0x%x, u32WaitPlayTimeMs: %u\n",
                        stStatusInfo.stSyncStatus.u32LastVidPts, u32WaitPtsPlayTimeMs, u32CurSendFrmPtsMs, u32WaitPlayTimeMs);
            return HI_SUCCESS;
        }
...
STATIC HI_S32 PVRPlayPidSwitchProcess(PVR_PLAY_CHN_S *pChnAttr, HI_U32 u32NextIdx)
{
    ...
    PVR_INDEX_PID_INFO_S* pstNextPlayPidInfo = HI_NULL;
    ...
    /*如果当前仅仅是add/del  PID，则不用reset，仅需等待数据消耗到一定水平线时再切换即可*/
    pstCurPlayPidInfo = PVR_INDEX_GET_CURRENT_PLAY_PID_INFO(pChnAttr->IndexHandle);
    pstNextPlayPidInfo = PVR_Index_GetPidInfoByPidIdx(pChnAttr->IndexHandle, pChnAttr->IndexHandle->u32CurPlayIdx+1);
    if(HI_NULL == pstNextPlayPidInfo || (HI_NULL == pstCurPlayPidInfo))
    {
        HI_ERR_PVR("pid change is failed!\n");
        return HI_FAILURE;
    }
    if ((pstCurPlayPidInfo->u32IndexPid == pstNextPlayPidInfo->u32IndexPid)
         && (pstCurPlayPidInfo->u32IndexType == pstNextPlayPidInfo->u32IndexType))
    {
...
HI_S32 PVRPlaySetProc(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_U32 u32Argc, HI_U8* pu8Argv[], HI_VOID* pPrivData)
{
    ...
    PVR_PLAY_CHN_S* pChnAttr = &g_stPvrPlayChns[0];
        ...
        || (HI_UNF_PVR_PLAY_STATE_FB == pChnAttr[u32ChnNum].enState))
    {
        ...
        if (1 == u32PrintFlg)
        {
            pChnAttr[u32ChnNum].stFBAttr.bPrintFlg = HI_TRUE;
        }
        else if (0 == u32PrintFlg)
        {
            pChnAttr[u32ChnNum].stFBAttr.bPrintFlg = HI_FALSE;
        }
    }
...
void PVRPlaySyncTrickPlayTime(PVR_PLAY_CHN_S* pChnAttr)
{
    ...
    if (HI_FALSE == pChnAttr->bRecordedVideoExist)
    {
...
HI_BOOL PVRPlayCheckSlowPauseBack(const HI_CHAR* pFileName)
{
    ...
    {
        pChnAttr = PVR_GetPlayChns(i);
        if (0 == strncmp(pChnAttr->stUserCfg.szFileName, pFileName, strlen(pFileName)))
        {
            break;
        }
    }
    ...
    else if (HI_UNF_PVR_PLAY_STATE_FB == enPlayStatus)
    {
        /*if alreadly rewind*/
...
HI_S32 HI_PVR_PlayInit(HI_VOID)
{
    ...
    else
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_ERR_PVR("play dev init failed!\n");
            return ret;
        }
        ...
        {
            close(g_s32PvrFd);
            HI_ERR_PVR("play init event failed!\n");
            return ret;
        }
        ...
        if (HI_FALSE == PVRRecCheckInitStat())
        {
...
HI_S32 HI_PVR_PlayDeInit(HI_VOID)
{
    ...
    else
    {
        ...
        for (i = 0 ; i < PVR_PLAY_MAX_CHN_NUM; i++)
        {
            ...
            (HI_VOID)pthread_mutex_destroy(&(pChnAttr->stPlayChnMutex));
        }
        ...
        if (HI_FALSE == PVRRecCheckInitStat())
        {
            MPI_UPROC_RemoveDir(g_stPvrPlayProcEntry.pszDirectory, HI_MPI_UPROC_PARENT_MSP);
...
HI_S32 HI_PVR_PlayCreateChn(HI_U32* pChn, const HI_UNF_PVR_PLAY_ATTR_S* pAttr, HI_HANDLE hAvplay, HI_HANDLE hTsBuffer)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("PVR Play Check User Cfg failed!\n");
        return ret;
    }
    ...
    ret = HI_PVR_PrepareCipher(&(pChnAttr->stUserCfg.stDecryptCfg), &pChnAttr->hCipher);
    if (HI_SUCCESS != ret)
    {
        goto ErrorExit;
    }
...
ErrorExit:
    ...
    if (HI_NULL != pChnAttr->IndexHandle)
    {
...
HI_S32 HI_PVR_PlayDestroyChn(HI_U32 u32Chn)
{
    ...
    pChnAttr = PVR_GetPlayChns(u32Chn);
    PVR_LOCK(&(pChnAttr->stPlayChnMutex));
    PVR_PLAY_CHECK_CHN_INIT_UNLOCK(pChnAttr);

    /* check channel state */
    if ((HI_UNF_PVR_PLAY_STATE_STOP != pChnAttr->enState) && (HI_UNF_PVR_PLAY_STATE_INIT != pChnAttr->enState))
    {
...
HI_S32  HI_PVR_PlaySetChn(HI_U32 u32ChnID, const HI_UNF_PVR_PLAY_ATTR_S* pstPlayAttr)
{
    ...
    pChnAttr = PVR_GetPlayChns(u32ChnID);
...
HI_S32 HI_PVR_PlayStartChn(HI_U32 u32ChnID) /* pause when end of file */
{
    ...
    HI_BOOL bAvplayResetEnable = HI_FALSE;
    ...
    s32Ret = HI_UNF_AVPLAY_Invoke(pChnAttr->hAvplay, HI_UNF_AVPLAY_INVOKE_SET_TPLAY_RESET_ENABLE, &bAvplayResetEnable);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("set avplay reset enable failed! ret=%#x \n", s32Ret);
    }
...
HI_S32 HI_PVR_PlayStartTimeShift(HI_U32* pu32PlayChnID, HI_U32 u32RecChnID, HI_HANDLE hAvplay, HI_HANDLE hTsBuffer)
{
    ...
    if (HI_SUCCESS != ret)
    {
        HI_WARN_PVR("pvr play get record channel attribute failed!\n");
        return ret;
    }
    ...
    if (HI_SUCCESS != ret)
    {
        HI_WARN_PVR("pvr play create chnnel failed!\n");
        return ret;
    }
    ...
    if (HI_SUCCESS != ret)
    {
        ...
        HI_WARN_PVR("pvr play start chnnel failed!\n");
        return ret;
    }
...
HI_S32 HI_PVR_PlayStopTimeShift(HI_U32 u32PlayChnID, const HI_UNF_AVPLAY_STOP_OPT_S* pstStopOpt)
{
    ...
    pChnAttr = PVR_GetPlayChns(u32PlayChnID);
    PVR_PLAY_CHECK_CHN_INIT(pChnAttr->enState);
...
HI_S32 HI_PVR_PlayPauseChn(HI_U32 u32Chn)
{
    ...
    if (HI_TRUE == PVRRecCheckChnRecording(u32Chn))
    {
        s32Ret = PVRRecMarkPausePos(u32Chn);
        HI_PVR_LOG_EXIT();
        return s32Ret;
    }
...
HI_S32 HI_PVR_PlayTrickMode(HI_U32 u32Chn, const HI_UNF_PVR_PLAY_MODE_S* pTrickMode)
{
    HI_S32 s32Ret = HI_SUCCESS;
    PVR_PLAY_CHN_S*  pChnAttr = HI_NULL;
    HI_BOOL bTrickWithAudioEnable = HI_FALSE;
    HI_UNF_PVR_PLAY_STATE_E enCurPlayState = HI_UNF_PVR_PLAY_STATE_BUTT;
    HI_UNF_PVR_PLAY_STATE_E enPlayStateTmp = HI_UNF_PVR_PLAY_STATE_BUTT;
    ...
    if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (abs(pTrickMode->enSpeed) & PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
    {
        bTrickWithAudioEnable = HI_TRUE;
    }
    s32Ret = PVRPlayCheckTrickParaIsValid(pChnAttr, pTrickMode);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("invalid trick parameter! ret = 0x%x\n", s32Ret);
        return s32Ret;
    }
    ...
    if ((HI_TRUE != bTrickWithAudioEnable) && (PVR_INDEX_IS_TYPE_AUDIO(pChnAttr->IndexHandle) || (HI_FALSE == pChnAttr->bRecordedVideoExist)))
    {
    ...
    if ((HI_TRUE != bTrickWithAudioEnable) && (HI_TRUE != pChnAttr->stUserCfg.bIsClearStream))
    {
    ...
    {
        PVR_GET_STATE_BY_SPEED(enPlayStateTmp, pTrickMode->enSpeed);
        if ((HI_UNF_PVR_PLAY_STATE_PLAY != enCurPlayState) || (HI_UNF_PVR_PLAY_STATE_PLAY != enPlayStateTmp))
        {
            pChnAttr->bTimeShiftStartFlg = HI_TRUE;
        }
    }
...
HI_S32 HI_PVR_PlayStep(HI_U32 u32Chn, HI_S32 direction)
{
    ...
    if (HI_TRUE == PVR_INDEX_IS_TYPE_AUDIO(pChnAttr->IndexHandle))
    {
...
HI_S32 HI_PVR_PlayGetStatus(HI_U32 u32Chn, HI_UNF_PVR_PLAY_STATUS_S* pStatus)
{
    ...
    s32Ratio = (abs(pChnAttr->enSpeed) & (~HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE))/HI_UNF_PVR_PLAY_SPEED_NORMAL;
    if ((HI_UNF_PVR_PLAY_STATE_STEPF == pChnAttr->enState)
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        PVR_Index_GetIndexInfo(pChnAttr->IndexHandle, &stCurPlayFrame, PVR_INDEX_DISP_TIME, &u32PtsPos);
        if (abs((HI_S32)pStatus->u32CurPlayTimeInMs - (HI_S32)u32PtsPos) > s32Ratio + 1000)
        {
            pStatus->u32CurPlayTimeInMs = u32PtsPos;
        }
    }
    else
    {
...
{
    HI_S32 ret = HI_SUCCESS;
    HI_CHAR szRealFileName[PVR_MAX_FILENAME_LEN] = {0};
    ...
    if (PVR_MAX_FILENAME_LEN <= strlen(pFileName))
    {
        HI_ERR_PVR("invalided filename:length:%u; max is %u\n", strlen(pFileName), PVR_MAX_FILENAME_LEN);
        return HI_ERR_PVR_FILE_INVALID_FNAME;
    }

    strncpy(szRealFileName, pFileName, sizeof(szRealFileName));
    szRealFileName[PVR_MAX_FILENAME_LEN - 1] = '\0';
    ret = PVR_GetTheStandardFileName(szRealFileName);
    if(HI_SUCCESS != ret)
    {
        HI_ERR_PVR("get real fileName failed! szFileName: %s, ret= 0x%x\n", szRealFileName, ret);
        return ret;
    ...
    if (HI_TRUE == PVRRecCheckFileRecording(szRealFileName))
    {
        pstRecChnAttr = PVRRecGetChnAttrByName(szRealFileName);

        if (NULL != pstRecChnAttr)
        {
