...
static HI_DRV_VPSS_FRMINFO_S  g_stFrmInfo;
        ...
        if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
        {
            pstPortCfg = &(pstInstance->stUsrPortCfg[u32Count]);
            {
                ...
                if (!VPSS_HDR_TYPE_NEED_DETILE_NOALG(pstStreamInfo->eStreamType))
                {
                    ...
                    if (pstPort->enOutBitWidth == HI_DRV_PIXEL_BITWIDTH_10BIT_CTS)
                    {
...
HI_S32 VPSS_INST_SyncUsrCfg(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if (pstInstance->bCfgNew)
    {
        ...
        pstInstance->bAndroidCts = pstInstUsrcCfg->bAndroidCts;
        pstInstance->stInEntity.bAndroidCts = pstInstUsrcCfg->bAndroidCts;
        VPSS_DBG_ReplyDbgCmd(&(pstInstance->stDbgCtrl),
                             DBG_SET_UHD,
             ...
             u32Count ++)
        {
            ...
            if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
            {
                ...
                pstPort->bFixdOmxBuf = pstPortCfg->bFixdOmxBuf;
                pstPort->u32OmxBufWidth = pstPortCfg->u32OmxBufWidth;
                pstPort->u32OmxBufHeight = pstPortCfg->u32OmxBufHeight;
            }
        }
...
HI_S32 VPSS_INST_Reset(VPSS_INSTANCE_S *pstInstance)
{
    ...
    for (u32Count = 0; u32Count < VPSS_PROC_TIMESTAMP_MAXCNT; u32Count++)
    {
        ...
        pstInstance->aulTrySrcTime[u32Count] = 0;
        pstInstance->aulGetSrcTime[u32Count] = 0;
        pstInstance->aulInBufDoneTime[u32Count] = 0;
        pstInstance->aulPQInitStartTime[u32Count] = 0;
        pstInstance->aulPQInitEndTime[u32Count] = 0;
        pstInstance->aulSrcReadyTime[u32Count] = 0;
        pstInstance->aulOutBufReadyTime[u32Count] = 0;
        pstInstance->aulInstReadyTime[u32Count] = 0;
        pstInstance->aulLogicStartTime[u32Count] = 0;
        pstInstance->aulTunnelOutTime[u32Count] = 0;
        pstInstance->aulSendOutTime[u32Count] = 0;
...
HI_S32 VPSS_INST_SetState(VPSS_INSTANCE_S *pstInstance, VPSS_INSTANCE_STATE_E enState)
{
    ...
    while (u32SleepCnt < 100)
    {
        msleep(1);
        VPSS_OSAL_DownLock(&(pstInstance->stInstLock));
        if (pstInstance->enState == enState)
        {
            VPSS_OSAL_UpLock(&(pstInstance->stInstLock));
            break;
        }
    ...
    if (u32SleepCnt >= 100)
    {
        VPSS_ERROR("Vpss set state: %d timeout! CurState:%d LastState:%d \n",
                   enState, pstInstance->enState, pstInstance->enLastState);
    }
...
HI_S32 VPSS_INST_Init(VPSS_INSTANCE_S *pstInstance, HI_DRV_VPSS_CFG_S *pstVpssCfg)
{
    ...
    pstInstance->stInEntity.pstInstance = pstInstance;
...
HI_S32 VPSS_INST_GetDefInstCfg(HI_DRV_VPSS_CFG_S *pstVpssCfg)
{
    ...
    pstVpssCfg->bAndroidCts = HI_FALSE;
...
HI_U32 VPSS_INST_GetDefPortCfg(HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    pstPortCfg->bFixdOmxBuf = HI_FALSE;
    pstPortCfg->u32OmxBufWidth = HI_FALSE;
    pstPortCfg->u32OmxBufHeight = HI_FALSE;
                            ...
                            HI_DRV_VPSS_PORT_CFG_S *pstPortCfg)
{
    ...
    //VPSS_OSAL_UpLock(&(pstInstance->stInstLock));
    if (HI_TRUE == pstUsrPortCfg->bFixdOmxBuf)
    {
        HI_PRINT("Set port cfg bFixdOmxBuf:%d wh:%u %u \n",
                 pstUsrPortCfg->bFixdOmxBuf,
                 pstUsrPortCfg->u32OmxBufWidth,
                 pstUsrPortCfg->u32OmxBufHeight);
...
#ifdef VPSS_SUPPORT_PROC_V2
        pstInstance->aulPQInitStartTime[pstInstance->u32TimeStampIndex] = jiffies;
        (HI_VOID)DRV_PQ_UpdateVpssPQ(pstInstance->ID, &stVpssPqInfo, (HI_VOID *)(&pstInstance->stPqRegData), (HI_VOID *)(&pstInstance->stPqWbcReg[0]), &pstInstance->stPQModule);
        pstInstance->aulPQInitEndTime[pstInstance->u32TimeStampIndex] = jiffies;
#else
        (HI_VOID)DRV_PQ_UpdateVpssPQ(pstInstance->ID, &stVpssPqInfo, (HI_VOID *)(&pstInstance->stPqRegData), (HI_VOID *)(&pstInstance->stPqWbcReg[0]), &pstInstance->stPQModule);
#endif
    }
                              ...
                              VPSS_HANDLE  hPort, HI_DRV_VIDEO_FRAME_S *pstFrm)
{
    ...
    g_stFrmInfo.hPort = hPort;
    VPSS_SAFE_MEMCPY(&(g_stFrmInfo.stFrame), pstFrm, sizeof(HI_DRV_VIDEO_FRAME_S));
#ifdef VPSS_SUPPORT_PROC_V2
    pstInstance->aulSendOutTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif

    s32Ret = pstInstance->pfUserCallBack(pstInstance->hDst, VPSS_EVENT_NEW_FRAME, &g_stFrmInfo);
                                  ...
                                  HI_DRV_VIDEO_FRAME_S *pstFrm, HI_DRV_BUF_ADDR_E enBufLR)
{
    ...
    VPSS_PORT_S *pstPort;
    HI_U32 u32PhyAddr, u32Stride, u32Height;
    VPSS_IN_INTF_S stIntf = {0};
    ...
    {
        HI_RECT_S stInRect;
        VPSS_INST_GetInCropFromSrcWH(pstInst, PortId, &stInRect);
    ...
    if (HI_TRUE == pstPort->bFixdOmxBuf)
    {
        u32Height = pstPort->u32OmxBufHeight;
    }
    else
    {
        u32Height = pstFrm->u32Height;
    }
       ...
       )
    {
        ...
        pstFrm->stBufAddr[enBufLR].u32PhyAddr_Y =
            pstFrm->stBufAddr[enBufLR].u32PhyAddr_YHead + u32Height * 16;

        pstFrm->stBufAddr[enBufLR].u32PhyAddr_CHead =
            pstFrm->stBufAddr[enBufLR].u32PhyAddr_Y + u32Stride * u32Height;
        pstFrm->stBufAddr[enBufLR].u32PhyAddr_C =
            pstFrm->stBufAddr[enBufLR].u32PhyAddr_CHead + u32Height * 16;
    }
              ...
              || pstFrm->ePixFormat == HI_DRV_PIX_FMT_NV12_CMP)
    {
        if (HI_TRUE == pstInst->enCapability.bits.hi3716mv410)
        {
            ...
            pstFrm->stBufAddr[enBufLR].u32PhyAddr_C =  u32PhyAddr + u32Stride * u32Height;
        }
    }
             ...
             || pstFrm->ePixFormat == HI_DRV_PIX_FMT_NV16_2X1)
    {
        ...
        pstFrm->stBufAddr[enBufLR].u32PhyAddr_C =  u32PhyAddr + u32Stride * u32Height;
        pstFrm->stBufAddr[enBufLR].vir_addr_c = (HI_U64)(HI_SIZE_T)(pstBuf->stMemBuf.pu8StartVirAddr + u32Stride * u32Height);
    }
...
HI_VOID VPSS_INST_GetInCrop(VPSS_INSTANCE_S *pstInst, HI_U32 PortId, HI_RECT_S *pstInCropRect)
{
    ...
    u32SrcW = stInfo.stDispRect.s32Width;
    u32SrcH = stInfo.stDispRect.s32Height;
    ...
    pstInCropRect->s32X = stInfo.stDispRect.s32X;
    pstInCropRect->s32Y = stInfo.stDispRect.s32Y;
...
HI_VOID VPSS_INST_GetInCropFromSrcWH(VPSS_INSTANCE_S *pstInst, HI_U32 PortId, HI_RECT_S *pstInCropRect)
{
    HI_U32 u32SrcW;
    HI_U32 u32SrcH;
    VPSS_IN_STREAM_INFO_S stInfo;
                               ...
                               HI_RECT_S *pstOutCropRect)
{
    ...
    if ((u32DstW == 0) || u32DstH == 0)
    {
        HI_RECT_S stSrcInCropRect;
        VPSS_INST_GetInCropFromSrcWH(pstInst, PortId, &stSrcInCropRect);
...
#endif
        {
            u32DstW = stSrcInCropRect.s32Height;
            u32DstH = stSrcInCropRect.s32Width;

            VPSS_DBG_INFO("Dst w h %d %d\n", u32DstW, u32DstH);
        }
        else
        {
            u32DstW = stSrcInCropRect.s32Width;
            u32DstH = stSrcInCropRect.s32Height;
        }
...
HI_BOOL VPSS_INST_CheckPassThroughForOMX(VPSS_INSTANCE_S *pstInstance, HI_DRV_VIDEO_FRAME_S *pstInImage)
{
    ...
    {
        pstPort = &(pstInstance->stPort[u32Count]);
        if (HI_INVALID_HANDLE != pstPort->s32PortId) //ignore port enable state
        {
            if ((VPSS_FORCE_BYPASS == pstPort->bPassThrough)
                || (VPSS_BYPASS_AUTOBYVPSS == pstPort->bPassThrough))  //omx do not force vpss to work, vpss can make choose by resolutation etc..
            {
                bPassThrough = HI_TRUE;
            }
            else if (VPSS_FORCE_NOT_BYPASS == pstPort->bPassThrough)
            {
                bPassThrough = HI_FALSE;
            }
            else
