...
static HI_S32 getstring(FILE **fp, const HI_CHAR *section, const HI_CHAR *key, HI_CHAR *buffer, HI_S32 size)
{
    ...
    if (len > 0)
    {
        do
        {
            if (!HIINI_Read(buf, HI_INI_BUFSIZE, fp))
            {
                return 0;
            }
            start = skiphead(buf);
    ...
    do
    {
        if (!HIINI_Read(buf,HI_INI_BUFSIZE,fp) || *(start = skiphead(buf)) == '[')
        {
            return 0;
        }
        start = skiphead(buf);
...
HI_S32 HIINI_GetString(const HI_CHAR *section, const HI_CHAR *key, const HI_CHAR *default_value,
             HI_CHAR *buffer, HI_S32 size, const HI_CHAR *file)
{
    ...
    else
    {
        ...
        {
            printf("file dir not exit\n");
            return HI_FAILURE;
        }

        fp = fopen(strrchr(file, '/'),"rb");
        if (NULL != fp)
        {
            Ret = getstring(&fp, section, key, buffer, size);
            (void)fclose(fp);
...
long HIINI_GetLong(const HI_CHAR *section, const HI_CHAR *key, long default_value, const HI_CHAR *file)
{
    ...
    HI_S32 len = HIINI_GetString(section, key, "", buf, sizeof(buf)/sizeof(buf[0]), file);
...
float HIINI_GetFloat(const HI_CHAR *section, const HI_CHAR *key, float default_value, const HI_CHAR *file)
{
    ...
    HI_S32 len = HIINI_GetString(section, key, "", buf, sizeof(buf)/sizeof(buf[0]), file);
    return (len == 0) ? default_value : HIINI_Atof(buf);
}
...
HI_BOOL HIINI_GetBool(const HI_CHAR *section, const HI_CHAR *key, HI_BOOL default_value, const HI_CHAR *file)
{
    ...
    HIINI_GetString(section, key, "", buf, sizeof(buf)/sizeof(buf[0]), file);
