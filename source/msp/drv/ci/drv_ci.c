                       ...
                       const HI_U8 *pu8Buffer, HI_U32 u32WriteLen, HI_U32 *pu32WriteOKLen)
{
    ...
    pu8Buff = s_astCIDrvParam[enCIPort].astCardParam[enCardId].pu8Buffer;

    /* If registe IOWrite function, use it to write directly */
    if (s_astCIDrvParam[enCIPort].stOP.ci_pccd_io_write)
    {
        s32Ret = s_astCIDrvParam[enCIPort].stOP.ci_pccd_io_write(enCIPort, enCardId, pu8Buff, u32WriteLen);
        if (HI_SUCCESS != s32Ret)
        {
            CIPORT_UNLOCK(enCIPort);
    ...
    else
    {
        ...
        if (u8Status & PCCD_FR)
        {
            ...
            s_astCIDrvParam[enCIPort].stOP.ci_pccd_write_byte(enCIPort, enCardId, SIZE_L_REG, (HI_U8)(u32WriteLen & 0xff));
            ...
            {
                s_astCIDrvParam[enCIPort].stOP.ci_pccd_write_byte(enCIPort, enCardId, DATA_REG, pu8Buff[u16Counter]);
                u32WriteOKLen++;
