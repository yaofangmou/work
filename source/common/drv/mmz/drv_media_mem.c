...
int hil_mmb_free(hil_mmb_t *mmb)
{
	...
	if (!found) {
		pr_debug("%s :cannot found mem block,maybe free twice.  \n", __func__);
...
unsigned long get_system_pool_size(unsigned long *pool_limit)
{
	unsigned long total_size = 0;
	struct mmz_iommu *common = &mmz_iommu;

	total_size = get_heap_pool_size(common->client, ION_HEAP_SYSTEM_MASK, pool_limit);
...
int sec_mem_source_query(u32 sec_iommu, int *source)
{
	...
	*source = 1;
	...
	if (!sec_iommu) {
		HI_MMZ_WARN("%s sec_iommu:0x%x err args\n", __func__, sec_iommu);
		return HI_FAILURE;
	}
