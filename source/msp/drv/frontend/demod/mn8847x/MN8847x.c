...
/*only used in DVB-T2*/
HI_S32 MN8847x_get_plp_info(HI_U32 u32TunerPort, HI_U32 u32Index, HI_UNF_TUNER_TER_PLP_INFO_S *pstPLPPara)
{
    DMD_PARAMETER_t* pstParaMeter;

    HI_ASSERT(HI_NULL != pstPLPPara);
    /* Check tuner port and init. */
    if (u32TunerPort >= TUNER_NUM)
    {
        HI_ERR_TUNER("Bad u32TunerPort\n");
    ...
    {
        /*get the number of PLPs*/
        MN8847X_DMD_set_info( pstParaMeter , DMD_E_INFO_DVBT2_SELECTED_PLP , u32Index & 0xFF );
        MN8847X_DMD_device_reset(pstParaMeter);
        MN8847X_DMD_device_scan(pstParaMeter);
        MN8847X_DMD_get_info( pstParaMeter , DMD_E_INFO_DVBT2_SELECTED_PLP);
        pstPLPPara->u8PlpId = pstParaMeter->info[DMD_E_INFO_DVBT2_SELECTED_PLP];
        MN8847X_DMD_get_info( pstParaMeter, DMD_E_INFO_DVBT2_DAT_PLP_TYPE );
        pstPLPPara->enPlpType = pstParaMeter->info[DMD_E_INFO_DVBT2_DAT_PLP_TYPE];
        return HI_SUCCESS;
    }
...
/*only used in DVB-T2*/
HI_S32 MN8847x_set_plp_para(HI_U32 u32TunerPort, HI_UNF_TUNER_TER_PLP_PARA_S *pstPLPPara)
{
    DMD_PARAMETER_t* pstParaMeter;
    ...
    MN8847X_DMD_set_info( pstParaMeter , DMD_E_INFO_DVBT2_SELECTED_PLP , pstPLPPara->u8PlpId & 0xFF );
    return HI_SUCCESS;
}
