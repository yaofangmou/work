...
void gpufreq_interactive_input_event(struct input_handle *handle, unsigned int type, unsigned int code, int value);
int gpufreq_interactive_input_connect(struct input_handler *handler, struct input_dev *dev, const struct input_device_id *id);
void gpufreq_interactive_input_disconnect(struct input_handle *handle);
...
static const struct input_device_id gpufreq_interactive_ids[] =
{
	{ .driver_info = 1 },       /* Matches all devices */
	{ },                        /* Terminating zero entry */
};

static struct input_handler gpufreq_interactive_input_handler =
{
	.event = gpufreq_interactive_input_event,
	.connect = gpufreq_interactive_input_connect,
	.disconnect = gpufreq_interactive_input_disconnect,
	.name = "gpufreq_interactive",
	.id_table = gpufreq_interactive_ids,
};
...
_mali_osk_errcode_t mali_adp_platform_init(struct device *device)
{
	...
	ret = HI_DRV_MODULE_Register(HI_ID_GPU, "MALI", (HI_VOID*)&mali_export_function);
	if (ret) {
		HISI_ERROR_INFO();
	}

	ret = input_register_handler(&gpufreq_interactive_input_handler);
	if (ret) {
		HISI_ERROR_INFO();
	}
...
_mali_osk_errcode_t mali_adp_platform_deinit(struct device *device)
{
	...
	input_unregister_handler(&gpufreq_interactive_input_handler);
	...
	case MALI_POWER_MODE_DEEP_SLEEP:
		if (s_PowerOff == 0) {
			...
			if (mali_dvfs_enable == 1) {
				...
				s_MaliConfiguration.input_ctrl = 0;     /* if power down, disable input event */
			}
...
static int mali_adp_suspend(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->suspend)) {
...
static int mali_adp_resume(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->resume)) {
...
static int mali_adp_freeze(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->freeze)) {
...
static int mali_adp_thaw(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->thaw)) {
...
static int mali_adp_runtime_suspend(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->runtime_suspend)) {
...
static int mali_adp_runtime_resume(struct device *device)
{
	...
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->runtime_resume)) {
...
static int mali_adp_runtime_idle(struct device *device)
{
	if ((NULL != device->driver) && (NULL != device->driver->pm) && (NULL != device->driver->pm->runtime_idle)) {
...
{
    struct mali_device *mdev = dev_get_drvdata(mali_adp_get_device());
    int polling_ms = mdev->devfreq->profile->polling_ms;

    if ((type == EV_SYN) && (code == SYN_REPORT)) {
        s_MaliConfiguration.input_ctrl = 2000 / polling_ms;  /* 2s */
    }
...
int gpufreq_interactive_input_connect(struct input_handler *handler, struct input_dev *dev, const struct input_device_id *id)
{
	...
	handle->name = "gpufreq_interactive";
