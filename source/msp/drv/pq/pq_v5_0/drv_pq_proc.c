...
#define  PQ_MODIFY_TIME     "(20181020)"
...
static HI_PQ_ZME_VID_COEF_SEL_S stProcZmeVidCoefSel = {{0}};
static HI_PQ_ZME_WBC_COEF_SEL_S stProcZmeWbcCoefSel = {{0}};
...
static HI_S32 drv_pq_ProcSetVdpZmeVidCoefSel(HI_CHAR *pArg1, HI_CHAR *pArg2);
static HI_S32 drv_pq_ProcSetVdpZmeWbcCoefSel(HI_CHAR *pArg1, HI_CHAR *pArg2);
...
static PQ_PROC_FUN_S g_stPQProcInfo[] =
{
    ...
    {"vdpzmevidcoefsel", drv_pq_ProcSetVdpZmeVidCoefSel},
    {"vdpzmewbccoefsel", drv_pq_ProcSetVdpZmeWbcCoefSel},
};
...
{
#ifdef PQ_ALG_CSC
    HI_PQ_VDP_CSC_S stCSC[HI_PQ_CSC_TYPE_BUTT];
...
static HI_S32 drv_pq_ProcReadZME(struct seq_file *s)
{
    ...
    HI_PQ_ZME_VID_COEF_SEL_S stProcZmeVidCoefSel = {{0}};
    HI_PQ_ZME_WBC_COEF_SEL_S stProcZmeWbcCoefSel = {{0}};
    ...
    if (PQ_PROC_PRINT_LEVEL3 == sg_enPqProcPrintLevel)
    {
        ...
        if (GET_ALG_FUN(HI_PQ_MODULE_ZME)->GetProcVdpZmeReso)
        {
            ...
            PROC_PRINT(s, "%-20s | %-8d: %-8d \n",  "vdp  hzme in  : width, height", stProcVdpZmeReso.stVdpZme1Inreso.s32Width, stProcVdpZmeReso.stVdpZme1Inreso.s32Height);
            PROC_PRINT(s, "%-20s | %-8d: %-8d \n",  "vdp  hzme out : width, height", stProcVdpZmeReso.stVdpZme1Outreso.s32Width, stProcVdpZmeReso.stVdpZme1Outreso.s32Height);

            PROC_PRINT(s, "%-20s | %-8d: %-8d \n",  "vdp  zme in   : width, height", stProcVdpZmeReso.stVdpZme2Inreso.s32Width, stProcVdpZmeReso.stVdpZme2Inreso.s32Height);
            PROC_PRINT(s, "%-20s | %-8d: %-8d \n",  "vdp  zme out  : width, height", stProcVdpZmeReso.stVdpZme2Outreso.s32Width, stProcVdpZmeReso.stVdpZme2Outreso.s32Height);
        }
        ...
        /* zme vid coef sel */
        {
            ...
            if (GET_ALG_FUN(HI_PQ_MODULE_ZME)->GetProcVdpZmeVidCoefSel)
            {
                GET_ALG_FUN(HI_PQ_MODULE_ZME)->GetProcVdpZmeVidCoefSel(&stProcZmeVidCoefSel);
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme vid hl reale ratio", stProcVdpZmeStrategy.u32VidRatio[HI_PQ_ZME_COEF_TYPE_LH]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme vid HL", i, stProcZmeVidCoefSel.u32HLSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme vid hc reale ratio", stProcVdpZmeStrategy.u32VidRatio[HI_PQ_ZME_COEF_TYPE_CH]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme vid HC", i, stProcZmeVidCoefSel.u32HCSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme vid vl reale ratio", stProcVdpZmeStrategy.u32VidRatio[HI_PQ_ZME_COEF_TYPE_LV]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme vid VL", i, stProcZmeVidCoefSel.u32VLSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme vid vc reale ratio", stProcVdpZmeStrategy.u32VidRatio[HI_PQ_ZME_COEF_TYPE_CV]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme vid VC", i, stProcZmeVidCoefSel.u32VCSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "\n");
                PROC_PRINT(s, "----------------------------------%s--------------------------------------\n", "zme vid coef sel end");
            }
        }

        {
            PROC_PRINT(s, "----------------------------------%s--------------------------------------\n", "zme wbc coef sel begin");

            if (GET_ALG_FUN(HI_PQ_MODULE_ZME)->GetProcVdpZmeWbcCoefSel)
            {
                GET_ALG_FUN(HI_PQ_MODULE_ZME)->GetProcVdpZmeWbcCoefSel(&stProcZmeWbcCoefSel);
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme wbc hl reale ratio", stProcVdpZmeStrategy.u32WbcRatio[HI_PQ_ZME_COEF_TYPE_LH]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme wbc HL", i, stProcZmeWbcCoefSel.u32HLSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme wbc hc reale ratio", stProcVdpZmeStrategy.u32WbcRatio[HI_PQ_ZME_COEF_TYPE_CH]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme wbc HC", i, stProcZmeWbcCoefSel.u32HCSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme wbc vl reale ratio", stProcVdpZmeStrategy.u32WbcRatio[HI_PQ_ZME_COEF_TYPE_LV]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme wbc VL", i, stProcZmeWbcCoefSel.u32VLSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
                ...
                PROC_PRINT(s, "%-20s | %-8d \n",   "vdp zme wbc vc reale ratio", stProcVdpZmeStrategy.u32WbcRatio[HI_PQ_ZME_COEF_TYPE_CV]);
                for (i = 0; i < 7; i++)
                {
                    PROC_PRINT(s, "(%s | ratio : %d | sel : %d)\t",   "vdp zme wbc VC", i, stProcZmeWbcCoefSel.u32VCSel[i]);
                    if ((0 == (i + 1) % 3))
                    {
                        PROC_PRINT(s, "\n");
                    }
...
static HI_S32 drv_pq_ProcReadFMD(struct seq_file *s)
{
    ...
    if (GET_ALG_FUN(HI_PQ_MODULE_FMD)->GetIfmdDectInfo)
    {
        ...
        PROC_PRINT(s, "%-20s: %-20s",   "field order",      stIfmdPlayBack.u32FieldOrder ? "bottom first" : "top first");
...
HI_S32 DRV_PQ_ProcRead(struct seq_file *s, HI_VOID *data)
{
    ...
    HI_U32 AlgModule = 0;
    HI_VDP_CHANNEL_TIMING_S  stHDTimingInfo = g_stPqStatus.stTimingInfo[HI_PQ_DISPLAY_HD1];
    HI_VDP_CHANNEL_TIMING_S  stSDTimingInfo = g_stPqStatus.stTimingInfo[HI_PQ_DISPLAY_SD0];
    ...
    PROC_PRINT(s, "%-20s: %s %s\n", "Driver version", "PQ_V5_0"PQ_MODIFY_TIME, "[Build Time:"__DATE__", "__TIME__"]");
...
static HI_VOID DRV_PQ_ProcPrintHelp(HI_VOID)
{
    ...
    HI_DRV_PROC_EchoHelper("echo scenechange     on/off           > /proc/msp/pq\n");
    ...
    if (PQ_PROC_PRINT_LEVEL3 == sg_enPqProcPrintLevel)
    {
        ...
        HI_DRV_PROC_EchoHelper("echo vdpzmevidcoefsel  HL/HC/VL/VC(0:HL,1:HC,VL:2,VC:3)RATIO(0,1,2,3,4,5,6)SEL(0,1,2,3,4,5,6,7) > /proc/msp/pq\n");
        HI_DRV_PROC_EchoHelper("echo vdpzmewbccoefsel  HL/HC/VL/VC(0:HL,1:HC,VL:2,VC:3)RATIO(0,1,2,3,4,5,6)SEL(0,1,2,3,4,5,6,7) > /proc/msp/pq\n");
    }
...
static HI_S32 drv_pq_ProcSetBrightness(HI_CHAR *pArg1, HI_CHAR *pArg2)
{
    ...
    DRV_PQ_SetBrightness(HI_PQ_DISPLAY_SD0, u32Data);
    DRV_PQ_SetBrightness(HI_PQ_DISPLAY_HD1, u32Data);
...
static HI_S32 drv_pq_ProcSetContrast(HI_CHAR *pArg1, HI_CHAR *pArg2)
{
    ...
    DRV_PQ_SetContrast(HI_PQ_DISPLAY_SD0, u32Data);
    DRV_PQ_SetContrast(HI_PQ_DISPLAY_HD1, u32Data);
...
static HI_S32 drv_pq_ProcSetHue(HI_CHAR *pArg1, HI_CHAR *pArg2)
{
    ...
    DRV_PQ_SetHue(HI_PQ_DISPLAY_SD0, u32Data);
    DRV_PQ_SetHue(HI_PQ_DISPLAY_HD1, u32Data);
...
static HI_S32 drv_pq_ProcSetSaturation(HI_CHAR *pArg1, HI_CHAR *pArg2)
{
    ...
    DRV_PQ_SetSaturation(HI_PQ_DISPLAY_SD0, u32Data);
    DRV_PQ_SetSaturation(HI_PQ_DISPLAY_HD1, u32Data);
...
{
    HI_CHAR achList[8] = "\0";
    HI_U32 u32ProcZmeHV = 0;
    HI_U32 u32ProcZmeRatio = 0;
    HI_U32 u32ProcZmeSel = 0;
    ...
    memcpy(achList, pArg2 , 1);
    u32ProcZmeHV = (HI_U32)simple_strtol(achList, NULL, 10);

    memcpy(achList, pArg2 + 1 , 1);
    u32ProcZmeRatio = (HI_U32)simple_strtol(achList, NULL, 10);

    memcpy(achList, pArg2 + 2 , 1);
    u32ProcZmeSel = (HI_U32)simple_strtol(achList, NULL, 10);

    if ((u32ProcZmeHV < 5) || (u32ProcZmeRatio < 7) || (u32ProcZmeSel < 7))
    {
        if (0 == u32ProcZmeHV)
        {
            stProcZmeVidCoefSel.u32HLSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (1 == u32ProcZmeHV)
        {
            stProcZmeVidCoefSel.u32HCSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (2 == u32ProcZmeHV)
        {
            stProcZmeVidCoefSel.u32VLSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (3 == u32ProcZmeHV)
        {
            stProcZmeVidCoefSel.u32VCSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }

        if (PQ_FUNC_CALL(HI_PQ_MODULE_ZME, SetProcVdpZmeVidCoefSel))
        {
            GET_ALG_FUN(HI_PQ_MODULE_ZME)->SetProcVdpZmeVidCoefSel(stProcZmeVidCoefSel);
        }
    ...
    {
#ifdef PQ_PROC_CTRL_SUPPORT
        HI_DRV_PROC_EchoHelper("vdpzmevidcoefsel: zme u32ProcZmeHV=%d, u32ProcZmeRatio=%d, u32ProcZmeSel=%d\n", u32ProcZmeHV, u32ProcZmeRatio, u32ProcZmeSel);
#endif
        return HI_FAILURE;
    }
}
...
static HI_S32 drv_pq_ProcSetVdpZmeWbcCoefSel(HI_CHAR *pArg1, HI_CHAR *pArg2)
{
    ...
    if ((u32ProcZmeHV < 5) || (u32ProcZmeRatio < 7) || (u32ProcZmeSel < 7))
    {
        if (0 == u32ProcZmeHV)
        {
            stProcZmeWbcCoefSel.u32HLSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (1 == u32ProcZmeHV)
        {
            stProcZmeWbcCoefSel.u32HCSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (2 == u32ProcZmeHV)
        {
            stProcZmeWbcCoefSel.u32VLSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }
        else if (3 == u32ProcZmeHV)
        {
            stProcZmeWbcCoefSel.u32VCSel[u32ProcZmeRatio] = u32ProcZmeSel;
        }

        if (PQ_FUNC_CALL(HI_PQ_MODULE_ZME, SetProcVdpZmeWbcCoefSel))
        {
            GET_ALG_FUN(HI_PQ_MODULE_ZME)->SetProcVdpZmeWbcCoefSel(stProcZmeWbcCoefSel);
        }
    ...
    {
#ifdef PQ_PROC_CTRL_SUPPORT
        HI_DRV_PROC_EchoHelper("SetProcVdpZmeWbcCoefSel: zme u32ProcZmeHV=%d, u32ProcZmeRatio=%d, u32ProcZmeSel=%d\n", u32ProcZmeHV, u32ProcZmeRatio, u32ProcZmeSel);
#endif
        return HI_FAILURE;
    }
}
