...
HI_S32 RDA5815_init_tuner(HI_U32 u32TunerPort)
{
    ...
    {
        RDA5815WriteReg(u32TunerPort, 0x38, 0x93); //sdk modify 2012.04.17
    }
    ...
    g_stTunerOps[u32TunerPort].stTunerCfg.u32XtalFreqKHz = 27000;
    ...
    g_stTunerOps[u32TunerPort].stTunerCfg.u32XtalFreqKHz = 24000;
...
#if defined (RDA5815_XTAL_27MHz)
    fPLL = u32PuRF / 40;//原始代码为除以1000，现在分两次除，先除40，再除25。2150000/40*77672=4174870000，基本接近HI_U32的上限。
#elif defined (RDA5815_XTAL_24MHz)
    fPLL = u32PuRF / 50;//原始代码为除以1000，现在分两次除，先除50，再除20。2150000/50*87381=3757383000，基本接近HI_U32的上限。
#endif
...
#if defined (RDA5815_XTAL_27MHz)
    //set frequency start
    u32Temp_value = (HI_U32)fPLL* 77672/25;//((2<<21) / RDA5815_XTALFREQ);
#elif defined (RDA5815_XTAL_24MHz)
    u32Temp_value = (HI_U32)fPLL* 87381/20;//((2<<21) / RDA5815_XTALFREQ);
#endif
...
/********************************************************************************/
HI_S32 RDA5815m_get_signal_strength(HI_U32 u32TunerPort, HI_U32 *pu32SignalStrength)
{
    HI_U8 freq_band;
    //unsigned int buf_reg16, buf_reg17;
    HI_U32 i2v,filter,st1,st2,pre,post;
    HI_U8 buffer;
    HI_U32 buf_vga,gain_stage,stage_code;
    HI_U32 max_vga = 30;     //unit : 0.1dB
    HI_U32 fPLL = g_stTunerOps[u32TunerPort].stCurrPara.u32Frequency / 1000;
    HI_U32 total_gain;
...
#define RDA5815m_Gain_Stage_15 0xfff /* '11   11    11  11   11  11 ' */
  ...
  HI_S8 gain[13][16] = {
    ...
    RDA5815ReadReg(u32TunerPort,0x16,&buffer);  //i2v_gain_bit_0,   Filter_gain_bit_0,      lna_gain_st1_0,     lna_gain_st2_0
    ...
    RDA5815ReadReg(u32TunerPort,0x17,&buffer);  //Reserved,                                 lna_gain_pre_0,     lna_gain_post_0
    pre     = (unsigned int)(buffer&0x0c)>>2;
    post    = (unsigned int)(buffer&0x03);

    RDA5815ReadReg(u32TunerPort,0xB7,&buffer);  //vga_gain_bit_stage_0_start[8:1]
