...
#ifdef DFB_HWCBLEND_SUPPORT
#include "hi_gfx2d_api.h"

static int check_update_region_hwc(SaWMan          *sawman,
                SaWManTier      *tier,
                CardState       *state,
                int              start,
                int              x1,
                ...
                bool             right_eye)
{
    ...
    fusion_vector_foreach (sawwin, i, sawman->layout) {
          ...
          if (SAWMAN_VISIBLE_WINDOW( window ) && (tier->classes & (1 << window->config.stacking))) {
                if  (window->surface && window->surface->config.format != DSPF_ARGB) {
                    /*Ö»Ö§³ÖDSPF_ARGB¸ñÊ½*/
                    D_ERROR("[%s:%d]\n", __func__, __LINE__);
                    return DFB_FAILURE;
                }
...
static int update_region_hwc(SaWMan          *sawman,
                SaWManTier      *tier,
                CardState       *state,
                int              start,
                int              x1,
                ...
                bool             right_eye )
{
    ...
    SaWManWindow *sawwin;
    CoreWindowStack *stack;
    HI_GFX2D_COMPOSE_LIST_S stComposeList = {0};
    HI_GFX2D_SURFACE_S stDst = {0};
    CoreSurfaceBufferLock dst = {0};
    CoreSurfaceBufferLock *pSrc;

    ret = check_update_region_hwc(sawman, tier, state, start, x1, y1, x2, y2, right_eye);
    if (ret) {
        return ret;
    }
    ...
    if (stack->bg.mode == DLBM_IMAGE) {
          stComposeList.u32ComposeCnt++;
    }
    ...
    fusion_vector_foreach (sawwin, i, sawman->layout) {
     ...
     if (0 == stComposeList.u32ComposeCnt) {
          return DFB_OK;
     }

     stComposeList.pstCompose = malloc(sizeof(HI_GFX2D_COMPOSE_S) * stComposeList.u32ComposeCnt);
     if (NULL == stComposeList.pstCompose) {
          return;
     }

     pSrc = malloc(sizeof(CoreSurfaceBufferLock) * stComposeList.u32ComposeCnt);
     if (NULL == pSrc) {
          free(stComposeList.pstCompose);
          return;
     }

     memset(stComposeList.pstCompose, 0, sizeof(HI_GFX2D_COMPOSE_S) * stComposeList.u32ComposeCnt);

     ret = dfb_surface_lock_buffer2(state->destination, CSBR_BACK, state->destination->flips,
                                               DSSE_LEFT,
                                               CSAID_GPU, CSAF_WRITE, &dst );

     stDst.u32Width = dst.buffer->config.size.w;
     stDst.u32Height = dst.buffer->config.size.h;

     stDst.enFmt = HI_GFX2D_FMT_ARGB8888;

     stDst.u32Phyaddr[0] = dst.phys;



     stDst.u32Stride[0] = dst.pitch;

     stComposeList.u32ComposeCnt = 0;

     if (stack->bg.mode == DLBM_IMAGE) {
             CoreSurface *bg = stack->bg.image;

             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.s32XPos = 0;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.s32YPos = 0;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Width = bg->config.size.w;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Height = bg->config.size.h;

             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.s32XPos = 0;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.s32YPos = 0;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Width = stack->width;
             stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Height = stack->height;

             if ((bg->config.size.w != stack->width) || (bg->config.size.h != stack->height)) {
                  stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOpt.stResize.bResizeEnable = HI_TRUE;
             }

             ret = dfb_surface_lock_buffer2( bg, CSBR_FRONT, bg->flips,
                                                   DSSE_LEFT,
                                                   CSAID_GPU, CSAF_READ, &pSrc[stComposeList.u32ComposeCnt] );
             if (ret != DFB_OK) {
                  D_ERROR("[%s:%d]\n", __func__, __LINE__);
             }


           stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Width = bg->config.size.w;
           stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Height = bg->config.size.h;

           stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.enFmt = HI_GFX2D_FMT_ARGB8888;

           stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Phyaddr[0] = pSrc[stComposeList.u32ComposeCnt].phys;
           stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Stride[0] = pSrc[stComposeList.u32ComposeCnt].pitch;

           stComposeList.u32ComposeCnt++;
    }
    else if (stack->bg.mode == DLBM_COLOR) {
         stComposeList.u32BgColor = ((HI_U32)stack->bg.color.a << 24) | ((HI_U32)stack->bg.color.r << 16) | ((HI_U32)stack->bg.color.g << 8) | ((HI_U32)stack->bg.color.b);
    }
     ...
     fusion_vector_foreach (sawwin, i, sawman->layout) {
          ...
          if (SAWMAN_VISIBLE_WINDOW( window ) && (tier->classes & (1 << window->config.stacking))) {
                   /*todo:ÐèÒª¸ù¾ÝoptÓëÄ¿±êµÄ½»¼¯·´ËãÔ´µÄ¾ØÐÎ,ÕâÀïÏÈ°´ÕÕÈ«ÆÁ´°¿Ú
                   ¼ò»¯´¦Àí*/
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.s32XPos = 0;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.s32YPos = 0;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Width = window->surface->config.size.w;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Height = window->surface->config.size.h;

                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.s32XPos = window->config.bounds.x;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.s32YPos = window->config.bounds.y;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Width = window->config.bounds.w;
                   stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Height = window->config.bounds.h;

                   if (stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Width != stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Width
                    || stComposeList.pstCompose[stComposeList.u32ComposeCnt].stInRect.u32Height != stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOutRect.u32Height) {
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOpt.stResize.bResizeEnable = HI_TRUE;
                   }

                   if (window->surface) {
                       ret = dfb_surface_lock_buffer2( window->surface, CSBR_FRONT, window->surface->flips,
                                                       DSSE_LEFT,
                                                       CSAID_GPU, CSAF_READ, &pSrc[stComposeList.u32ComposeCnt] );
                       if (ret != DFB_OK) {
                           D_ERROR("[%s:%d]\n", __func__, __LINE__);
                       ...
                       stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Width = window->surface->config.size.w;
                       stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Height = window->surface->config.size.h;

                       stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Phyaddr[0] = pSrc[stComposeList.u32ComposeCnt].phys;
                       stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Stride[0] = pSrc[stComposeList.u32ComposeCnt].pitch;
                    }
                    else if (window->caps & DWCAPS_COLOR) {
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.enType = HI_GFX2D_SURFACE_TYPE_COLOR;
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Color =
                            ((HI_U32)window->config.color.a << 24) | ((HI_U32)window->config.color.r << 16)
                            | ((HI_U32)window->config.color.g << 8) | ((HI_U32)window->config.color.b);
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Width = window->config.bounds.w;
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.u32Height = window->config.bounds.h;
                    }
                    else {
                        continue;
                    }
                   ...
                   if (window->config.options & DWOP_ALPHACHANNEL) {
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOpt.stBlend.bPixelAlphaEnable = HI_TRUE;
                   }

                   if (window->config.opacity != 0xFF) {
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOpt.stBlend.bGlobalAlphaEnable = HI_TRUE;
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stOpt.stBlend.u8GlobalAlpha = window->config.opacity;
                   }

                   if (window->surface->config.caps & DSCAPS_PREMULTIPLIED) {
                        stComposeList.pstCompose[stComposeList.u32ComposeCnt].stSurface.bPremulti = HI_TRUE;
                   }
     ...
     if (stComposeList.u32ComposeCnt) {
        ret = HI_GFX2D_Open(HI_GFX2D_DEV_ID_0);
        if (ret) {
            D_ERROR("[%s:%d]0x%x\n", __func__, __LINE__, ret);
        }

        ret |= HI_GFX2D_ComposeSync(HI_GFX2D_DEV_ID_0, &stComposeList, &stDst, 100);
        //ret |= HI_GFX2D_ComposeAsync(HI_GFX2D_DEV_ID_0, &stComposeList, &stDst);
        if (ret) {
            D_ERROR("[%s:%d]0x%x\n", __func__, __LINE__, ret);
        }

        ret |= HI_GFX2D_Close(HI_GFX2D_DEV_ID_0);
        if (ret) {
            D_ERROR("[%s:%d]0x%x\n", __func__, __LINE__, ret);
        }
     }

     dfb_surface_unlock_buffer(state->destination, &dst);
     ...
     if (stack->bg.mode == DLBM_IMAGE) {
         ...
         dfb_surface_unlock_buffer(bg, &pSrc[stComposeList.u32ComposeCnt]);
         stComposeList.u32ComposeCnt++;
     }
     ...
     fusion_vector_foreach (sawwin, i, sawman->layout) {
          ...
          if (SAWMAN_VISIBLE_WINDOW( window ) && (tier->classes & (1 << window->config.stacking))) {
                if (window->surface) {
                    dfb_surface_unlock_buffer(window->surface, &pSrc[stComposeList.u32ComposeCnt]);
                }

                if ((NULL == window->surface) && !(window->caps & DWCAPS_COLOR)) {
                    continue;
                }
                stComposeList.u32ComposeCnt++;
          }
...
#ifdef DFB_HWCBLEND_SUPPORT
     ret = update_region_hwc(sawman, tier, state, start, x1, y1, x2, y2, right_eye);
     if (ret == DFB_OK) {
          return;
     }
#endif
