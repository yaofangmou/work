...
static HI_BOOL parse_dri_marker(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    if (s32Len != 4)
    {
      HI_GFX_Print("the dri len is bad\n");
      return HI_FALSE;
    }
...
static HI_BOOL parse_sofn_marker(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    {/**< 精度硬件不支持 >**/
        HI_GFX_Print("the data precison is %d,hard dec not support\n",s32DataPrecision);
        return HI_FALSE;
    }
    ...
    {
       HI_GFX_Print("the s32Len is larger\n");
       return HI_FALSE;
    }
...
static inline HI_BOOL DRV_JPEG_CheckIsSupportSofInfo(JPEG_IMG_INFO_S *pstImgInfo)
{
    ...
    {
        HI_GFX_Print("the s32NumComponent is larger,not support\n");
        return HI_FALSE;
    }

    if ((pstImgInfo->image_width > 8095) || (pstImgInfo->image_height > 8095))
    {
        HI_GFX_Print("the image w and h is to large\n");
        return HI_FALSE;
    }
...
static HI_BOOL parse_sos_marker(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    {
        HI_GFX_Print("not has sof befor sos\n");
        return HI_FALSE;
    }
    ...
    {
        HI_GFX_Print("the len is larger\n");
        return HI_FALSE;
    }
    ...
    {
        HI_GFX_Print("the component num is larger\n");
        return HI_FALSE;
    }
    ...
    for (s32Cnt = 0; s32Cnt < s32NumComponent; s32Cnt++)
    {
        ...
        HI_GFX_Print("can not find component id\n");
        return HI_FALSE;
...
static HI_BOOL parse_dht_marker(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    while (s32Len > 16)
    {
        ...
        #if 0
        HI_GFX_Print("bits1->bits8[%d %d %d %d %d %d %d %d]\n",
             bits[1], bits[2], bits[3], bits[4],
             bits[5], bits[6], bits[7], bits[8]);
        HI_GFX_Print("bits9->bits16[%d %d %d %d %d %d %d %d]\n",
        ...
        {/** AC table definition **/
              ...
              if(s32HTIndex < 0 || s32HTIndex >= MAX_NUM_HUFF_TBLS)
                {/** deal codecc warning **/
                  HI_GFX_Print("the dht index is error\n");
                  return HI_FALSE;
                }
    ...
    if (s32Len != 0)
    {
       HI_GFX_Print("the dht len is error\n");
       return HI_FALSE;
    }
    ...
    {
      HI_GFX_Print("the dri is not support\n");
      return HI_FALSE;
    }
#endif
...
static HI_BOOL parse_dqt_marker(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    while (s32Len > 0)
    {
        ...
        {
            HI_GFX_Print("the qt table is to larger\n");
            return HI_FALSE;
        }
        ...
        for (s32Cnt = 0; s32Cnt < HI_DCT_SIZE2; s32Cnt++)
        {
            ...
            {
                 HI_GFX_Print("quantable[%d %d %d %d %d %d %d %d]\n",
    ...
    if (s32Len != 0)
    {
       HI_GFX_Print("the dqt len is error\n");
       return HI_FALSE;
    }
...
static inline HI_S32 DRV_JPEG_ParseMarkInfo(HI_S32 DataMark,JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_Print("is not others mark\n");
        return JPEG_PARSE_ERROR;
    }
    ...
    switch (DataMark)
    {
        ...
        case M_JPEG_SOF1:
              ...
              {
                 HI_GFX_Print("parse sof failure\n");
                 return JPEG_PARSE_ERROR;
              }
        ...
        case M_JPEG_SOS:
              ...
              {
                 HI_GFX_Print("parse sos failure\n");
                 return JPEG_PARSE_ERROR;
              }
        ...
        case M_JPEG_DHT:
              ...
              {
                 HI_GFX_Print("parse dht failure\n");
                 return JPEG_PARSE_ERROR;
              }
              break;
        ...
        case M_JPEG_DQT:
              ...
              {
                 HI_GFX_Print("parse dqt failure\n");
                 return JPEG_PARSE_ERROR;
              }
              break;
        case M_JPEG_DRI:
              ...
              {
                 HI_GFX_Print("parse dri failure\n");
                 return JPEG_PARSE_ERROR;
              }
              break;
        case M_JPEG_COM:
              ...
              {
                 HI_GFX_Print("parse com failure\n");
                 return JPEG_PARSE_ERROR;
              }
              break;
        case M_JPEG_EOI:
              HI_GFX_Print("is end jpeg\n");
              return JPEG_PARSE_ERROR;
        default:
