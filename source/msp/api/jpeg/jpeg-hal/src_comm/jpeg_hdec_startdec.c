...
DEC_FAILURE:
    ...
    if (HI_TRUE == SupportSeekToSoft)
    {
        pJpegHandle->stProcInfo.SupportSeekToSoft = HI_TRUE;
    }
...
static inline HI_S32 JPEG_HDEC_OnDecInputUnPhyMemData(JPEG_HDEC_HANDLE_S_PTR pJpegHandle, HI_BOOL HasBeenStartDec, JPEG_INTTYPE_E *pIntStatus)
{
    ...
    {
        HI_GFX_Print("+++++warning: decode timeout %d\n",TotalTimes);
    }
    ...
    //HI_GFX_Print("+++++++++++++*pIntStatus = %d JPG_INTTYPE_ERROR = %d\n",*pIntStatus,JPG_INTTYPE_ERROR);
    HI_GFX_Print("+++++++input stream err with eof data [0x%x 0x%x],seek to soft dec\n",pStremBuf[DataSize - 2],pStremBuf[DataSize - 1]);
