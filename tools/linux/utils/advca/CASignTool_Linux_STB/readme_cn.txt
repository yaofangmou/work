...
4.使用方式如下：
  ...
  制作未签名的安全Boot：
  不设置ROOT_RSA_Private_Key，设置External_public_key_file和External_private_key_file，生成各区镜像和除Key区以外其他区签名
  不设置ROOT_RSA_Private_Key，External_private_key_file和External_public_key_file，生成各区镜像，不做签名，Finalboot中的各区签名填充为全0或全FF。
  不设置ROOT_RSA_Private_Key，External_private_key_file，设置External_public_key_file，生成除Key区以外的各区镜像，不做签名，Finalboot中的各区签名填充为全0或全FF。
  
  签名除Key区以外的所有区： 不设置ROOT_RSA_Private_Key和External_public_key_file，仅设置External_private_key_file，生成其他区域的镜像及签名
  ...
  非boot镜像M方式第一步签名。
  支持用-K, -R 和-O指定Key文件、源文件和目标文件所在路径，用-sw指定软件号，可选用-c指定芯片号，-P指定私有数据所在路径。-crypto指定签名加密方式使用RSA还是国密，若不指定或指定为0，则为RSA，指定为1则为国密。

  9>./CASignTool 11 -K keyFilePath -R srcFilePath -O destFilePath [-SWPK SWPKFile] [-c ChipType] [-crypto 0/1]
  非boot镜像M方式第二步签名。
  支持用-K, -R 和 -O指定Key文件、源文件和目标文件所在路径，用-swpk指定加密密钥文件（.txt格式）， -crypto指定签名加密方式使用RSA还是国密，若不指定或指定为0，则为RSA，指定为1则为国密。可用-c指定芯片号。
