...
HI_S32 loaderCheckPolicy(LDDB_LOADER_INFO_S* pstLoaderDBInfo)
{
    ...
    if (loaderCheckEnforceType(pstLoaderDBInfo))
    {
        ...
        COM_StrNCpy((HI_CHAR *)pstLoaderDBInfo->stUPDParamData.unParam.stUSBParam.as8FileName, COM_CFG_D_UPGRADE_FILENAME, HI_LOADER_FILENAME_LEN - 1);
...
static HI_S32 loaderShowUpgradeResult(HI_S32 s32Ret)
{
    if (HI_SUCCESS != s32Ret)
    {
        switch (s32Ret)
        {
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_SIG_EXPT, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_DOWNLOAD_TIMEOUT, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_NOT_FIND_USB_FILE, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_NO_RESP, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_IP_NOT_FIND_FILE, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_NET_EXPT, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_SET_IP_ERR, COM_CFG_D_HINT_TIME);
            ...
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_NET_TIMEOUT, COM_CFG_D_HINT_TIME);
            ...
            default:
            {
                OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FAIL_DOWNLOAD, COM_CFG_D_HINT_TIME);
    ...
    {
        OSD_ShowMsgBox(UI_LAN_NOTICE, UI_LAN_PT_SUCC, COM_CFG_D_HINT_TIME);
...
static HI_S32 loaderDoPartitionReset(HI_VOID)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ...
        COM_Delayms(COM_CFG_D_HINT_TIME);
        OSD_ShowMsgBox(UI_LAN_WARNING, UI_LAN_FACTORY_FAILED, COM_CFG_D_HINT_TIME);
        return s32Ret;
    }
    ...
    COM_Delayms(COM_CFG_D_HINT_TIME);
    COM_CHECK(UI_KEYLED_Display("Succ"));
    COM_CHECK(OSD_ShowMsgBox(UI_LAN_NOTICE, UI_LAN_FACTORY_SUCCESS, COM_CFG_D_HINT_TIME));
...
static HI_S32 loaderDoUpgradeProcedure(HI_VOID)
{
    ...
    COM_CHECK(UI_KEYLED_Display("Succ"));
    COM_CHECK(OSD_ShowMsgBox(UI_LAN_NOTICE, UI_LAN_PT_SUCC, COM_CFG_D_HINT_TIME));
...
#ifdef HI_LOADER_APPLOADER
static HI_VOID loaderUpgradeDone(HI_VOID)
{
    sync();
    reboot(RB_AUTOBOOT);
