...
static HI_VOID LoaderGetDefDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    if (enDisp == HI_UNF_DISPLAY1)
    {
        ...
        pstDispParam->u32VirtScreenWidth = 1280;
        pstDispParam->u32VirtScreenHeight = 720;
        pstDispParam->enDeepColorMode = HI_UNF_HDMI_DEEP_COLOR_24BIT;
...
HI_BOOL g_bGotDisp0Param = HI_FALSE;
HI_BOOL g_bGotDisp1Param = HI_FALSE;
HI_DISP_PARAM_S g_Disp0BaseParam = {0};
HI_DISP_PARAM_S g_Disp1BaseParam = {0};
...
HI_S32 HI_DRV_PDM_GetDispParam(HI_UNF_DISP_E enDisp, HI_DISP_PARAM_S *pstDispParam)
{
    ...
    if ((HI_TRUE == g_bGotDisp0Param) && (HI_UNF_DISPLAY0 == enDisp))
    {
        *pstDispParam = g_Disp0BaseParam;
        return HI_SUCCESS;
    }

    if ((HI_TRUE == g_bGotDisp1Param) && (HI_UNF_DISPLAY1 == enDisp))
    {
        *pstDispParam = g_Disp1BaseParam;
        return HI_SUCCESS;
    }
    ...
    if (HI_UNF_DISPLAY0 == enDisp)
    {
        g_bGotDisp0Param = HI_TRUE;
        g_Disp0BaseParam = *pstDispParam;
    }

    if (HI_UNF_DISPLAY1 == enDisp)
    {
        g_bGotDisp1Param = HI_TRUE;
        g_Disp1BaseParam = *pstDispParam;
    }
...
HI_S32 HI_DRV_PDM_ReleaseLogoData(HI_U32 LogoDataAddr)
{
    HI_VOID *DataAddr = (HI_VOID *)LogoDataAddr;
    ...
    if (LogoDataAddr <= MCE_DEF_LOGOPARAM_SIZE)
    {
        HI_ERR_PDM("free addr must be greater than %u\n", MCE_DEF_LOGOPARAM_SIZE);
        return HI_FAILURE;
    }

    /*The address read in the function PDM_ReadLogoData is offset by the size of MCE_DEF_LOGOPARAM_SIZE, so the address is rolled back there.*/
    DataAddr -= MCE_DEF_LOGOPARAM_SIZE;
    free(DataAddr);
    return HI_SUCCESS;
}
...
HI_S32 HI_DRV_PDM_AllocReserveMem(const HI_CHAR *BufName, HI_U32 u32Len, HI_U32 *pu32PhyAddr)
{
    ...
    *pu32PhyAddr = (HI_U32)reserve_mem_alloc(u32Len, &AlignSize);
    ...
    NameSize = sizeof(g_stResBuf[g_u32ResBufNum].Name);
    BufNameSize = strlen(BufName);
    CpyLenth = NameSize > BufNameSize ? BufNameSize : NameSize;
    strncpy(g_stResBuf[g_u32ResBufNum].Name, BufName, CpyLenth);
...
HI_S32 PDM_UpdatePmocParam(HI_PDM_PMOCPARAM_S *pstPmocParam, HI_U8 *pBuf)
{
    HI_S32 Ret;
    HI_DB_S stBaseDB = {0};
    HI_DB_TABLE_S stTable;
...
HI_S32 HI_DRV_PDM_UpdatePmocParam(HI_PDM_PMOCPARAM_S *pPmocParam)
{
    HI_S32                Ret = HI_SUCCESS;
    HI_S32                Size = 0;
    HI_HANDLE             hFlash = HI_INVALID_HANDLE;
    ...
    HI_CHAR               Bootarg[BOOT_ARGS_SIZE] = {0};
    HI_Flash_InterInfo_S  stFlashInfo;
    HI_U8                *pFlashDataBuf = HI_NULL;
    HI_U32                u32ReadLenth = 0;
    HI_MCE_MTD_INFO_S     MtdInfo;

    if (pPmocParam == HI_NULL)
    {
        HI_ERR_PDM("ERR: param is invalid!");
        return HI_FAILURE;
    }

    memset(&MtdInfo, 0, sizeof(HI_MCE_MTD_INFO_S));
    strncpy(MtdInfo.stBase.MtdName, PDM_BASE_DEF_NAME, sizeof(MtdInfo.stBase.MtdName) - 1);

    hFlash = HI_Flash_OpenByName(MtdInfo.stBase.MtdName);
    if (HI_INVALID_HANDLE == hFlash)
    {
        pTmp = getenv("bootargs");
        ...
        Ret = PDM_GetMtdInfo(PDM_BASE_DEF_NAME, Bootarg, &MtdInfo.stBase);
        if (HI_SUCCESS == Ret)
        {
            hFlash = HI_Flash_OpenByName(MtdInfo.stBase.MtdName);
            if (HI_INVALID_HANDLE == hFlash)
    ...
    {
        MtdInfo.stBase.u32Offset = 0;
        (HI_VOID)HI_Flash_GetInfo(hFlash, &stFlashInfo);
        MtdInfo.stBase.u32Size = stFlashInfo.PartSize;
    }

    u32ReadLenth = stFlashInfo.PageSize > MCE_DEF_BASEPARAM_SIZE ? stFlashInfo.PageSize : MCE_DEF_BASEPARAM_SIZE;

    pFlashDataBuf = (HI_U8 *)malloc(u32ReadLenth);
    if (HI_NULL == pFlashDataBuf)
    {
        HI_ERR_PDM("malloc baseparam part size fail!");
        goto FLASH_CLOSE;
    }

    Size = HI_Flash_Read(hFlash, (HI_U64)MtdInfo.stBase.u32Offset, (HI_U8 *)pFlashDataBuf, u32ReadLenth,
                            HI_FLASH_RW_FLAG_RAW);
    if (Size < 0)
    {
        HI_INFO_PDM("ERR: HI_Flash_Read!");
    ...
    Ret = PDM_UpdatePmocParam(pPmocParam, pFlashDataBuf);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PDM("PDM_UpdateBaseByType fail 0x%x\n", Ret);
        goto MEM_FREE;
    }

    if (HI_FLASH_TYPE_EMMC_0 != stFlashInfo.FlashType)
    {
        Size = HI_Flash_Erase(hFlash, (HI_U64)MtdInfo.stBase.u32Offset, u32ReadLenth);
        if (Size <= 0)
        {
    ...
    Size = HI_Flash_Write(hFlash, (HI_U64)MtdInfo.stBase.u32Offset, (HI_U8 *)pFlashDataBuf, u32ReadLenth,
                            HI_FLASH_RW_FLAG_RAW);
    if (Size <= 0)
    {
