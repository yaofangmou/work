...
IMAGE *pRcvImage = HI_NULL;
IMAGE *pRlsImage = HI_NULL;
...
HI_S32 BUFMNG_InitSpinLock(BUFMNG_VPSS_IRQ_LOCK_S *pLock)
{
    if (HI_NULL == pLock)
    {
        return HI_FAILURE;
...
HI_S32 BUFMNG_SpinLockIRQ(BUFMNG_VPSS_IRQ_LOCK_S *pLock)
{
    if (HI_NULL == pLock)
    {
        return HI_FAILURE;
...
HI_S32 BUFMNG_SpinUnLockIRQ(BUFMNG_VPSS_IRQ_LOCK_S *pLock)
{
    if (HI_NULL == pLock)
    {
        return HI_FAILURE;
                                ...
                                HI_VOID *pstArgs)
{
    ...
    if (s_stVdecDrv.astChanEntity[hHandle].pstChan->stStatInfo.u32VpssReportTime == 0)
    {
        s_stVdecDrv.astChanEntity[hHandle].pstChan->stStatInfo.u32VpssReportTime = VDEC_GetTimeInMs();
    }
...
HI_S32 HI_DRV_VDEC_SetEosFlag(HI_HANDLE hHandle)
{
    ...
    pstChan->bReceiveEosFlag = HI_TRUE;
...
error:
    if (pstVpssFrame != HI_NULL)
    {
        ...
        s32Ret = VDEC_ConvertFrame2Image(pstVpssFrame, pImage);
        if (s32Ret != HI_SUCCESS)
        {
...
static HI_S32 VDEC_RlsFrm(VDEC_CHANNEL_S *pstChan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    VDEC_SpinLockIRQ(&(s_stVdecDrv.stLock));
    pImage = pRlsImage + (pstChan->hVdec & 0xff);

    memset(pImage, 0, sizeof(IMAGE));
    pImage->image_width   = pstFrame->u32Width;
    pImage->image_height = pstFrame->u32Height;
    pImage->image_id             = pstPrivInfo->image_id;
    pImage->image_id_1           = pstPrivInfo->image_id_1;
    pImage->stDispInfo.luma_phy_addr = pstFrame->stBufAddr[0].u32PhyAddr_Y - pstFrame->stBufAddr[0].u32Head_Size;
    pImage->stHDRInput.HDR_Metadata_phy_addr = pstVideoPrivate->HdrPhyAddr;

    if (pstPrivInfo->image_id_1 != -1)
    {
        pImage->stDispInfo.luma_phy_addr_1 = pstFrame->stBufAddr[1].u32PhyAddr_YHead;
    }

    pstStatInfo->u32VdecRlsFrameTry++;

    s32Ret = RlsImage(pstChan, pstImgInft->image_provider_inst_id, pImage);
    ...
    if (VDEC_OK != s32Ret)
    {
        ...
        VDEC_SpinUnLockIRQ(&(s_stVdecDrv.stLock));
        return HI_FAILURE;
    }
...
static HI_S32 VDEC_RecvFrm(HI_HANDLE hHandle, VDEC_CHANNEL_S *pstChan, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    ...
    pImage->pOutFrame = (HI_U64)(HI_SIZE_T)pstFrame;

    if (s_stVdecDrv.astChanEntity[hHandle].stVpssChan.s32Speed < 0)
    {
        s32Ret = VDEC_RecvFastBackFrm(hHandle, pstImgInft, pImage);
    }
    else
    {
        s32Ret = GetImage(s_stVdecDrv.astChanEntity[hHandle].pstChan, pstImgInft->image_provider_inst_id, pImage);
    }
    ...
    pstPrivInfo->s32InterPtsDelta = pImage->InterPtsDelta;

    for (u32UserdataId = 0; u32UserdataId < 4; u32UserdataId++)
    {
        pstChan->pu8UsrDataForWaterMark[u32UserdataId] = (VOID *)(HI_SIZE_T)(pImage->p_usrdat[u32UserdataId]);
    }
    ...
    if (pImage->p_usrdat != HI_NULL)
    {
        USRDATA_Arrange(hHandle, (VOID *)(HI_SIZE_T)(pImage->p_usrdat[0]));
    }

#endif

    VDEC_ConvertFrm(pstChan->stCurCfg.enType, pstChan, pImage, pstFrame);
    ...
    {
        RecvFrm_FramePackTypeProcess(hHandle, pImage, pstVpssChan, pstFrame);
    }
    ...
    if (pImage->image_id % 100 == pstChan->u32LastFrmId)
    {
...
#if (1 == HI_VDEC_USERDATA_CC_SUPPORT)
#if (1 == HI_VDEC_CC_FROM_IMAGE)
    RecvFrm_UserDataProcess(hHandle, pImage);
#endif
#endif
    ...
    if (pstChan->stStatInfo.u32VpssReadImageTime == 0)
    {
        pstChan->stStatInfo.u32VpssReadImageTime = VDEC_GetTimeInMs();
    }

    /* save yuv data */
    BUFMNG_SaveYuv(hHandle, pstChan->stCurCfg.enType, (HI_VOID *)pImage, pstFrame);
...
static HI_S32 VDEC_Chan_VpssRlsFrmBuf(VPSS_HANDLE hVpss, HI_DRV_VIDEO_FRAME_S *pstFrm)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        ...
        s32Ret = VDEC_ConvertFrame2Image(pstFrm, pImage);
...
static HI_S32 VDEC_Chan_InitParam(HI_HANDLE hHandle, HI_UNF_AVPLAY_OPEN_OPT_S *pstCapParam)
{
    ...
    pstChan->bSetEosFlag = HI_FALSE;
    pstChan->bReceiveEosFlag = HI_FALSE;
...
HI_S32 HI_DRV_VDEC_ChanStart(HI_HANDLE hHandle)
{
    ...
    StartVpssTime = VDEC_GetTimeInMs();

    g_KpiInfo.StartDecoder = StartVpssTime - StartDecoderTime;
    ...
    g_KpiInfo.StartVpss = VDEC_GetTimeInMs() - StartVpssTime;
...
HI_S32 HI_DRV_VDEC_ChanStop(HI_HANDLE hHandle)
{
    ...
    g_KpiInfo.StopVpss = VDEC_GetTimeInMs() - StartTime;
...
HI_S32 DRV_VDEC_SetChanAttr(HI_HANDLE hHandle, VDEC_VCODEC_ATTR_S *pstCfgParam)
{
    ...
    pstChan->stCurCfg.u32ErrCover    = (g_CfgErrRatio != -1) ? g_CfgErrRatio : pstCfgParam->u32ErrCover;
...
HI_S32 HI_DRV_VDEC_GetChanStatusInfo(HI_HANDLE hHandle, VDEC_STATUSINFO_S *pstStatus)
{
    ...
    if (pstStatus->bEndOfStream  && bAllPortCompleteFrm)
    {
        pstChan->bReportEos = HI_TRUE;
    }
...
static HI_S32 VDEC_Ioctl_RcvVpssFrm(struct file *filp, unsigned int cmd, void *arg)
{
    ...
    if (s32Ret == HI_SUCCESS)
    {
        ...
        pstStatInfo = &s_stVdecDrv.astChanEntity[hVdec].pstChan->stStatInfo;
        if ((s32Ret == HI_SUCCESS) && (pstStatInfo->u32ReceiveFrameTime == 0))
        {
            pstStatInfo->u32ReceiveFrameTime = VDEC_GetTimeInMs();
            g_KpiInfo.ReceiveFirstFrameTime = pstStatInfo->u32ReceiveFrameTime;
            if (pstStatInfo->u32VpssReportTime == 0)
            {
                pstStatInfo->u32VpssReportTime = pstStatInfo->u32ReceiveFrameTime;
            }
        }
    }
...
static HI_S32 VDEC_Ioctl_CreateVpss(struct file *filp, unsigned int cmd, void *arg)
{
    ...
    g_KpiInfo.CreatVpss = VDEC_GetTimeInMs() - StartTime;
...
static HI_S32 VDEC_Ioctl_DestroyVpss(struct file *filp, unsigned int cmd, void *arg)
{
    ...
    g_KpiInfo.DestroyVpss = VDEC_GetTimeInMs() - StartTime;
...
static HI_S32 VDEC_Ioctl_ResetVpss(struct file *filp, unsigned int cmd, void *arg)
{
    ...
    g_KpiInfo.ResetVpss = VDEC_GetTimeInMs() - StartTime;
...
HI_S32 VDEC_DRV_Init(HI_VOID)
{
    ...
    pRcvImage = HI_VMALLOC_VDEC(HI_VDEC_MAX_INSTANCE_NEW * sizeof(IMAGE));
    if (!pRcvImage)
    {
        HI_FATAL_VDEC("Vmalloc Receive image failed!\n");
        return HI_FAILURE;
    }

    pRlsImage = HI_VMALLOC_VDEC(HI_VDEC_MAX_INSTANCE_NEW * sizeof(IMAGE));
    if (!pRlsImage)
    {
        HI_VFREE_VDEC(pRcvImage);
        pRcvImage = HI_NULL;
        HI_FATAL_VDEC("Vmalloc Receive image failed!\n");
        return HI_FAILURE;
    }
    ...
    ret = VDEC_InitSpinLock(&(s_stVdecDrv.stLock));
    if (ret != HI_SUCCESS)
    {
        HI_ERR_VDEC("VDEC_InitSpinLock failed: ret = %d\n", ret);
        return HI_FAILURE;
    }
