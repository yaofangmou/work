...
HI_S32 PQ_HAL_GetHDRCfg(HI_PQ_XDR_FRAME_INFO *pstXdrFrameInfo, HI_PQ_HDR_CFG *pstPqHdrCfg)
{
    ...
    if (HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR == pstXdrFrameInfo->enSrcFrameType)
    {
        if (pstXdrFrameInfo->bTechnilolorEn)
        {
            if (HI_PQ_DISP_TYPE_HDR10 == pstXdrFrameInfo->enDispType)
            {
                pstXdrFrameInfo->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_HDR10;
            }
            else if (HI_PQ_DISP_TYPE_NORMAL == pstXdrFrameInfo->enDispType)
            {
                pstXdrFrameInfo->enSrcFrameType = HI_DRV_VIDEO_FRAME_TYPE_SDR;
            }
        }
        else
        {
