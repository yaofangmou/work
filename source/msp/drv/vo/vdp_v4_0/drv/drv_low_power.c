...
static HI_S32 LowPower_AddMsg(REG_NAME_E eRegister, HI_BOOL bEnable)
{
    ...
    msg = VDP_ASYNC_GetMsgPointer(ASYNC_QUEUE_IDX_LOWPOWER);
    if (HI_NULL == msg)
    {
        return HI_FAILURE;
    }
    ...
    {
        HI_KFREE(HI_ID_VO, msg);
        return HI_FAILURE;
    }

    msg->ulExpectedTime = VDP_ASYNC_GenExpectTime(u32CycleTime);
    msg->enAsyncType = ASYNC_MSG_TYPE_LOWPOWER;
    msg->unMsgItem.AsyncMsgLowPower.enRegName = eRegister;
    msg->unMsgItem.AsyncMsgLowPower.bRegStatus = bEnable;
    if (HI_SUCCESS != VDP_ASYNC_PushMsg(msg, ASYNC_QUEUE_IDX_LOWPOWER))
    {
        HI_KFREE(HI_ID_VO, msg);
        return HI_FAILURE;
    }
...
static HI_VOID LowPower_DelMsg(REG_NAME_E reg_name, HI_BOOL bEnable)
{
    ...
    msg.enAsyncType = ASYNC_MSG_TYPE_LOWPOWER;
    msg.unMsgItem.AsyncMsgLowPower.enRegName = reg_name;
    msg.unMsgItem.AsyncMsgLowPower.bRegStatus = bEnable;
    VDP_ASYNC_DelMsg(&msg, ASYNC_QUEUE_IDX_LOWPOWER);
...
static HI_S32 LowPower_Callback(const ASYNC_MSG_S *pAsyncMsg)
{
    ...
    msg = pAsyncMsg->unMsgItem.AsyncMsgLowPower;
...
HI_S32 LowPower_Init(HI_VOID)
{
    ...
    if (HI_SUCCESS != VDP_ASYNC_ResetMsgQueue(ASYNC_QUEUE_IDX_LOWPOWER))
    {
        return HI_FAILURE;
    }
...
HI_S32 LowPower_DeInit(HI_VOID)
{
    ...
    if (HI_SUCCESS != VDP_ASYNC_DestroyMsgQueue(ASYNC_QUEUE_IDX_LOWPOWER))
    {
        return HI_FAILURE;
    }
