...
# if defined(CONFIG_SMCU_LOAD_SIZE)
static void set_smcu_param_tag(bd_t *bd);
#endif
...
void start_tee_atf(uint32_t arch, char *buf, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
	...
	if(arch == BL33_IMG_ARM64) {
		...
		bl33_ep.args.arg0 = (uint64_t)(arg0);
		...
		if (arg0) {
			bl33_ep.args.arg3 = (uint64_t)(arg0);
		}
...
int do_bootm_linux(int flag, int argc, char *argv[], bootm_headers_t *images)
{
	...
	if (IMAGE_ENABLE_OF_LIBFDT && images->ft_len) {
		...
		if (fdt_check_header(fdt)) {
			printf("Invalid FDT at %p, hdr at %p\n", fdt, hdr);
			return 1;
		}
	}
    ...
    writel((u32)r2, REG_BASE_SCTL + REG_SC_GEN41);
#endif
	...
	if (is_tee_type()) {
		...
		if (!notee) {
			if (image_check_arch (hdr, IH_ARCH_ARM64)) {
				...
				bl33_ep_info->pc = (uint64_t)(image_get_ep(hdr));
			...
			} else if(image_check_arch (hdr, IH_ARCH_ARM)) {
		...
		if(!notee) {
			if (image_check_arch (hdr, IH_ARCH_ARM)) {
				printf("Start TEE with aarch32 kernel...\n");
				start_tee_atf(BL33_IMG_ARM32, (char *)hdr, (uint32_t)fdt, 0, 0, 0);
			}
...
static void set_smcu_param_tag(bd_t *bd)
{
	params->hdr.tag = ATAG_SMCU_PARAM;
	params->hdr.size = 4;
	*((u32*)&params->u) = get_smcu_reserve_mem();
	params = tag_next(params);
}
#endif
