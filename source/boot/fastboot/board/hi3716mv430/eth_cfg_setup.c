...
static int hi3716mv430_phyinf[] = {/* array index is the hw config */
	[0] = ETH_PHY_MII,
	[1] = ETH_PHY_RMII,
	[2] = ETH_PHY_UNKNOWN,
};
...
int get_eth_phymdio(int index, int defval)
{
	...
	if ((phymdio != 0 && phymdio != 1) && (phymdio != 0x1f)) {
...
static void set_hi3716mv430_eth_phyintf(u32 regval)
{
	u32 phy0intf;
	u32 phy1intf;
	...
	if (phy1intf != 0 && phy1intf != 1) {
		printf("MAC1_IF register should be 0(MII), 1(RMII), "
		       "but current is %d\n",
		       phy1intf);
		phy1intf = 1;
	}
	...
	snprintf(eth_config.phyintf, sizeof(eth_config.phyintf), "%s,%s",
		 ethphy_intfname(hi3716mv430_phyinf[phy0intf]),
		 ethphy_intfname(hi3716mv430_phyinf[phy1intf]));
}
...
void eth_config_init(void)
{
	...
	set_hi3716mv430_eth_phyintf(phyaddrmdio_val);
}
