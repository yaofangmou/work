...
#else
    if (CRYPTO_CAPACITY_SUPPORT == tdes_capacity)
    {
        func.mode = SYMC_MODE_ECB;
        func.alg = SYMC_ALG_NULL_CIPHER;
        func.create = cryp_symc_create;
...
void cryp_get_symc_alg(u8 *alg, u8 *mode, u32 *count)
{
    u32 i = 0;
    u32 id = 0;
    ...
    for(i=0; i<SYMC_FUNC_TAB_SIZE; i++)
    {
        if (symc_descriptor[i].valid)
        {
            alg[id]  = (u8)symc_descriptor[i].alg;
            mode[id] = (u8)symc_descriptor[i].mode;
            id++;
        }
    }
