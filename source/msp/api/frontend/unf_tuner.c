...
TUNER_STATUS_SAT_S s_stSatPara[UNF_TUNER_NUM];
                    ...
                    HI_UNF_TUNER_FE_LNB_22K_E LNB22K, HI_UNF_TUNER_FE_POLARIZATION_E polar, HI_S32 startFreqMHz, HI_S32 stopFreqMHz)
{
    ...
    if((HI_UNF_TUNER_FE_LNB_UNICABLE != s_stSatPara[u32TunerId].stLNBConfig.enLNBType)
        && (HI_UNF_TUNER_FE_LNB_UNICABLE2 != s_stSatPara[u32TunerId].stLNBConfig.enLNBType))
    {
                                      ...
                                      HI_U32* pu32IF, HI_UNF_TUNER_FE_LNB_22K_E *penLNB22K)
{
    ...
    /*unicable LNB*/
        ...
        || (HI_UNF_TUNER_FE_LNB_UNICABLE2 == pstLNBConfig->enLNBType))
    {
...
static HI_VOID* TUNER_BlindScanThread(HI_VOID* pstBlindScan)
{
    ...
    if (HI_UNF_DEMOD_DEV_TYPE_AVL6211 == s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
        ...
        || (HI_UNF_DEMOD_DEV_TYPE_INTERNAL0 == s_strCurTunerAttr[u32TunerId].enDemodDevType))
    {
...
HI_S32 TUNER_DISEQC_Stop22K(HI_U32 u32TunerId)
{
    HI_S32 s32Ret = HI_FAILURE;
    TUNER_DATA_S stTunerData;
...
HI_S32 TUNER_DISEQC_Resume22K(HI_U32 u32TunerId)
{
    /* Resume */
...
HI_S32 HI_UNF_TUNER_SetAttr(HI_U32  u32tunerId , const HI_UNF_TUNER_ATTR_S *pstTunerAttr)
{
    ...
    s_bSigAttrInit[u32tunerId] = HI_FALSE;
...
HI_S32 UNIC_SCRxSignal_ON(HI_U32 u32TunerId)
{
    ...
    u8ChanByte[0] = ((u8SCRNO & 7) << 5) | (u8SubFunc & 0x1f) ;
...
static HI_VOID* TUNER_TerSignalDetect(HI_VOID *pstSignal)
{
    ...
    stTunerSignal.stSignal.enSyncLockStatus = HI_UNF_TUNER_SIGNAL_BUTT;
    ...
    else if (stTunerSignal.stSignal.enSyncLockStatus == HI_UNF_TUNER_SIGNAL_DROPPED)
    {
        HI_INFO_TUNER("Sync is not ok,so tell the locked status immediately.\n");
    }
    else
    {
...
static HI_S32 TUNER_Connect_C(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    if ((pstCab->u32SymbolRate < 900000) || (pstCab->u32SymbolRate > 7270000))
    {
    ...
    stTunerPara.enSyncLockStatus = HI_UNF_TUNER_SIGNAL_BUTT;
    ...
    if (stTunerSignal.stSignal.enSyncLockStatus == HI_UNF_TUNER_SIGNAL_DROPPED)
    {
        HI_INFO_TUNER("Sync is not ok,so tell the locked status immediately.\n");
...
static HI_S32 TUNER_Connect_S(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    HI_S32 i = 0;
    HI_U32 u32StartTime = 0;
#if defined (DISEQC_SUPPORT)
    TUNER_DATA_S stSigStat;
    TUNER_DATA_S stClrSigChange;
    TUNER_DATABUF_S stSigchange;
    TUNER_SIGNALSTAT_E enSignalStat = TUNER_SIGNAL_BAD;
    HI_BOOL bChanChanged = HI_FALSE;
...
#if defined (DISEQC_SUPPORT)
    else if(HI_UNF_TUNER_FE_LNB_UNICABLE2 == s_stSatPara[u32tunerId].stLNBConfig.enLNBType)
    {
#if CONFLICT_CHECK
        //Get Signal Stat:good, normal or bad
        stSigStat.u32Port = u32tunerId;
        (HI_VOID)ioctl(s_s32TunerFd, TUNER_GET_SIG_STAT_CMD, (unsigned long)&stSigStat);
        enSignalStat = (TUNER_SIGNALSTAT_E)stSigStat.u32Data;

        HI_UNICABLE_TRACE("[usr]%d:Last Signal is %s.\n", __LINE__, stSigStat.u32Data==0?"BAD":(stSigStat.u32Data==1?"NORMAL":"GOOD"));

        //Store the NOW Signal Status.
        stClrSigChange.u32Port = u32tunerId;
        s32Ret = ioctl(s_s32TunerFd, TUNER_CLEAR_CHANGE_FLAG_CMD, (unsigned long)&stClrSigChange);
        if (s32Ret != HI_SUCCESS)
        {
            HI_ERR_TUNER("TUNER_CLEAR_CHANGE_FLAG_CMD error %d\n\n", s32Ret);
            return HI_FAILURE;
        }
#endif
        s32Ret = UNIC2_ChanChange(u32tunerId, pstConnectPara->unConnectPara.stSat.u32Freq / 1000, pstConnectPara->unConnectPara.stSat.enPolar);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_TUNER("Send ODU_Channel_change cmd fail.\n");
        }
#if CONFLICT_CHECK
        //Check whether the channel changed or not.
        if (enSignalStat == TUNER_SIGNAL_GOOD)
        {
            stSigchange.u32Port = u32tunerId;
            stSigchange.u32DataBuf[0] = enSignalStat;
            s32Ret = ioctl(s_s32TunerFd, TUNER_GET_CHAN_CHANGE_CMD, (unsigned long)&stSigchange);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_TUNER("TUNER_GET_CHAN_CHANGE_CMD error %d\n\n", s32Ret);
                return HI_FAILURE;
            }

            bChanChanged = stSigchange.u32DataBuf[0];
            HI_UNICABLE_TRACE("[usr]%d,Original signal is good,Signal changed: %d\n", __LINE__,bChanChanged);
            if (!bChanChanged)
            {
                u32RandVal = UNIC2_rand();
                HI_UNICABLE_TRACE("[usr]%d,u32RandVal:%d us.\n",__LINE__,u32RandVal);
                stSlpTm.tv_sec = 0;
                stSlpTm.tv_nsec = u32RandVal * 1000;      //1000 ns = 1 us
                s32Ret = nanosleep(&stSlpTm, NULL);
                s32Ret = UNIC2_ChanChange(u32tunerId, pstConnectPara->unConnectPara.stSat.u32Freq / 1000, pstConnectPara->unConnectPara.stSat.enPolar);
                if (HI_SUCCESS != s32Ret)
                {
                    HI_ERR_TUNER("Send UNIC2_ChanChange failed.\n");
                    return HI_FAILURE;
                }
            }
        }
        else if (enSignalStat == TUNER_SIGNAL_NORMAL)
        {
            stSigchange.u32Port = u32tunerId;
            stSigchange.u32DataBuf[0] = enSignalStat;
            stSigchange.u32DataBuf[1] = TUNER_NORMALSIGNAL_FIRST;
            stSigchange.u32DataBuf[2] = pstConnectPara->unConnectPara.stSat.u32SymbolRate;
            s32Ret = ioctl(s_s32TunerFd, TUNER_GET_CHAN_CHANGE_CMD, (unsigned long)&stSigchange);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_TUNER("TUNER_GET_CHAN_CHANGE_CMD error %d\n\n", s32Ret);
            ...
            bChanLocked = stSigchange.u32DataBuf[0];
            HI_UNICABLE_TRACE("[usr]%d,Original signal is normal,First checking signal locked: %d\n", __LINE__,bChanLocked);
        }
#endif
    }
#endif
...
#if defined (DISEQC_SUPPORT)
#if CONFLICT_CHECK
    if (HI_UNF_TUNER_FE_LNB_UNICABLE2 == s_stSatPara[u32tunerId].stLNBConfig.enLNBType)
    {
        if (enSignalStat == TUNER_SIGNAL_BAD)
        {
            stSigchange.u32Port = u32tunerId;
            stSigchange.u32DataBuf[0] = TUNER_SIGNAL_BAD;

            s32Ret = ioctl(s_s32TunerFd, TUNER_GET_CHAN_CHANGE_CMD, (unsigned long)&stSigchange);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_TUNER("[usr]TUNER_GET_CHAN_CHANGE_CMD error %d\n\n", s32Ret);
                return HI_FAILURE;
            }
            bChanChanged = stSigchange.u32DataBuf[0];
            HI_UNICABLE_TRACE("[usr]%d, Original signal is bad ,Signal changed: %d.\n", __LINE__,bChanChanged);
            if (!bChanChanged)
            {
                u32RandVal = UNIC2_rand();
                HI_UNICABLE_TRACE("[usr]%d,u32RandVal:%d us.\n",__LINE__,u32RandVal);
                ...
                if (HI_SUCCESS != s32Ret)
                {
                    HI_ERR_TUNER("Send ODU_ChannelChange2 failed.\n");
                    return HI_FAILURE;
                }
            }
        }
        else if (enSignalStat == TUNER_SIGNAL_NORMAL)
        {
            stSigchange.u32Port = u32tunerId;
            stSigchange.u32DataBuf[0] = enSignalStat;
            stSigchange.u32DataBuf[1] = TUNER_NORMALSIGNAL_SECOND;
            stSigchange.u32DataBuf[2] = bChanLocked;
            s32Ret = ioctl(s_s32TunerFd, TUNER_GET_CHAN_CHANGE_CMD, (unsigned long)&stSigchange);
            if (s32Ret != HI_SUCCESS)
            {
                HI_ERR_TUNER("[usr]TUNER_GET_CHAN_CHANGE_CMD error %d\n\n", s32Ret);
            ...
            bChanChanged = stSigchange.u32DataBuf[0];
            HI_UNICABLE_TRACE("[usr]%d,Original signal is normal,Second checking signal changed: %d\n", __LINE__,bChanLocked);
            if (!bChanChanged)
            {
                u32RandVal = UNIC2_rand();
                HI_UNICABLE_TRACE("[usr]%d,u32RandVal:%d us.\n",__LINE__,u32RandVal);
    ...
    else
    {
        ...
        {
            s32Ret = HI_ERR_TUNER_FAILED_CONNECT;/*return failed when connect timeout.*/
        }
    }
...
HI_S32 HI_UNF_TUNER_Connect(HI_U32 u32tunerId, const HI_UNF_TUNER_CONNECT_PARA_S *pstConnectPara, HI_U32 u32TimeOut)
{
    ...
    if ((HI_UNF_TUNER_SIG_TYPE_CAB == pstConnectPara->enSigType) || (HI_UNF_TUNER_SIG_TYPE_J83B == pstConnectPara->enSigType))
    {
        ...
        if(HI_SUCCESS != s32Ret && HI_ERR_TUNER_FAILED_CONNECT != s32Ret)
        {
...
static HI_S32 TUNER_GetBER_C(HI_U32 u32tunerId, const TUNER_DATABUF_S *pstTunerDataBuf, HI_U32 *pu32BER)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_INTERNAL0:
            ...
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_TUNER("HI_SYS_GetVersion error\n");
                return s32Ret;
            }
            if (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeHardWare && HI_CHIP_VERSION_V430 == stVersion.enChipVersion)
            {
                dBer = ((HI_DOUBLE)(pstTunerDataBuf->u32DataBuf[0])/16384);
            ...
            {
                HI_ERR_TUNER("Unknown internal demod type\n");
                return HI_FAILURE;
            }
            break;
...
static HI_S32 TUNER_GetSNR_C(HI_U32 u32tunerId, const TUNER_DATA_S *pu32StTunerData, HI_DOUBLE *pdoubleDSnr)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_INTERNAL0:
            ...
            if (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeHardWare && HI_CHIP_VERSION_V430 == stVersion.enChipVersion)
            {
                if (0 == pu32StTunerData->u32Data)
                {
                    *pdoubleDSnr = 0;
                    HI_ERR_TUNER("Get SNR failed.\n");
                    return HI_FAILURE;
                }
                switch (s_stCurrentSignal[u32tunerId].enQamType)
                {
                    case QAM_TYPE_16:
                        *pdoubleDSnr = 10.0 * log10(161.0 * 2048 / pu32StTunerData->u32Data);
                        break;

                    case QAM_TYPE_32:
                        *pdoubleDSnr = 10.0 * log10(180.0 * 2048 / pu32StTunerData->u32Data);
                        break;

                    case QAM_TYPE_64:
                        *pdoubleDSnr = 10.0 * log10(168.0 * 2048 / pu32StTunerData->u32Data);
                        break;

                    case QAM_TYPE_128:
                        *pdoubleDSnr = 10.0 * log10(184.0 * 2048 / pu32StTunerData->u32Data);
                        break;

                    case QAM_TYPE_256:
                        *pdoubleDSnr = 10.0 * log10(170.0 * 2048 / pu32StTunerData->u32Data);
                        break;
...
static HI_S32 TUNER_GetSignalStrength_S_His(HI_U32 u32tunerId, HI_U32 *pu32SignalStrength)
{
    ...
    switch (s_strCurTunerAttr[u32tunerId].enTunerDevType)
    {
        ...
        {
            if (s_strCurTunerAttr[u32tunerId].enDemodDevType == HI_UNF_DEMOD_DEV_TYPE_INTERNAL0)
            {
                *pu32SignalStrength = s32Agc / 10;
            }
            else
            {
...
static HI_S32 TUNER_GetSignalQuality_C_BerSnr(HI_U32 u32TunerId, HI_DOUBLE *pdBer, HI_DOUBLE *pdSnrEva)
{
    ...
    switch (s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_INTERNAL0:
            s32Result = HI_SYS_GetVersion(&stVersion);
            if (HI_SUCCESS != s32Result)
            {
                HI_ERR_TUNER("HI_SYS_GetVersion error\n");
                return s32Result;
            ...
            if (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeHardWare && HI_CHIP_VERSION_V430 == stVersion.enChipVersion)
            {
                ...
                {
                    case QAM_TYPE_16:
                        dSnrEva = 10.0 * log10(161.0 * 2048 / stTunerData.u32Data);
                        break;

                    case QAM_TYPE_32:
                        dSnrEva = 10.0 * log10(180.0 * 2048 / stTunerData.u32Data);
                        break;

                    case QAM_TYPE_64:
                        dSnrEva = 10.0 * log10(168.0 * 2048 / stTunerData.u32Data);
                        break;

                    case QAM_TYPE_128:
                        dSnrEva = 10.0 * log10(184.0 * 2048 / stTunerData.u32Data);
                        break;

                    case QAM_TYPE_256:
                        dSnrEva = 10.0 * log10(170.0 * 2048 / stTunerData.u32Data);
                        break;

                    default:
                        return HI_FAILURE;
                }
                dBer = ((HI_DOUBLE)(stTunerDataBuf.u32DataBuf[0])/16384);
                dBer = dBer*10*10*4/204/8;  /*change PER to BER*/
            }
            else
            {
                HI_ERR_TUNER("Unknown internal demod type\n");
...
static HI_VOID TUNER_GetSignalQuality_T_His_GetNordig(HI_U32 u32TunerId, TUNER_SIGNALINFO_S *pstTunerSignalInfo, HI_U32 *pu32SnrNordig)
{
    ...
    else if (HI_UNF_TUNER_SIG_TYPE_ISDB_T == pstTunerSignalInfo->stInfo.enSigType)
    {
        static const HI_U32 snrNordigP1dB1000[4][5] = {
            /*1/2*/    /*2/3*/   /*3/4*/   /*5/6*/    /*7/8*/
            {5100+3000, 6900+3000, 7900+3000, 8900+3000, 9700+3000},  /*DQPSK*/
            {5100,      6900,      7900,      8900,      9700},       /*QPSK*/
            {10800,     13100,     14600,     15600,     16000},      /*QAM16*/
            {16500,     18700,     20200,     21600,     21500}       /*AQM64*/
        };
        HI_UNF_MODULATION_TYPE_E       enIsdbTModType;
        HI_UNF_TUNER_FE_FECRATE_E      enIsdbTFECRate;
        if (pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.u8LayerSegNum
            && pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.unISDBTLayers.bits.u8LayerCExist)
        {
            enIsdbTModType = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.enLayerModType;
            enIsdbTFECRate = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersCInfoBits.enLayerFECRate;
        }
        else if (pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.u8LayerSegNum
            && pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.unISDBTLayers.bits.u8LayerBExist)
        {
            enIsdbTModType = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.enLayerModType;
            enIsdbTFECRate = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersBInfoBits.enLayerFECRate;
        }
        else
        {
            enIsdbTModType = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.enLayerModType;
            enIsdbTFECRate = pstTunerSignalInfo->stInfo.unSignalInfo.stIsdbT.stISDBTTMCCInfo.stIsdbtLayersAInfoBits.enLayerFECRate;
        }

        switch(enIsdbTModType)
        {
...
static HI_S32 TUNER_GetSignalQuality_T_His(HI_U32 u32TunerId, HI_U32 *pu32SignalQuality)
{
    ...
    if (HI_UNF_TUNER_SIG_TYPE_DVB_T == stTunerSignalInfo.stInfo.enSigType
        || HI_UNF_TUNER_SIG_TYPE_ISDB_T == stTunerSignalInfo.stInfo.enSigType)
    {
    ...
    switch (s_strCurTunerAttr[u32TunerId].enDemodDevType)
    {
        ...
        case HI_UNF_DEMOD_DEV_TYPE_INTERNAL0:
            ...
            if (HI_CHIP_TYPE_HI3716M == stVersion.enChipTypeHardWare && HI_CHIP_VERSION_V430 == stVersion.enChipVersion)
            {
                ...
                *pu32Freq = (HI_U32)( dTmpFreq ) + pstTunerDataBuf->u32DataBuf[0];
                ...
                {
                    dTmpSymb = pstTunerDataBuf->u32DataBuf[1];
                }
...
HI_S32 HI_UNF_TUNER_SetLNBConfig(HI_U32 u32TunerId, HI_UNF_TUNER_FE_LNB_CONFIG_S *pstLNB)
{
    ...
    if ((HI_UNF_TUNER_FE_LNB_UNICABLE == pstLNB->enLNBType)
        || (HI_UNF_TUNER_FE_LNB_UNICABLE2 == pstLNB->enLNBType))
    {
...
{
    TUNER_SETPLPPARA_S stTunerPLPPara = {0};
    HI_S32 s32Ret;
    ...
    if (HI_NULL == pstPLPPara)
    {
        HI_ERR_TUNER("Input parameter(pstPLPPara) invalid\n");
        return HI_ERR_TUNER_INVALID_POINT;
    }

    stTunerPLPPara.u32Port = u32TunerId;
    stTunerPLPPara.stPLPPara = *pstPLPPara;

    s32Ret = ioctl(s_s32TunerFd, TUNER_SET_PLP_PARA_CMD, &stTunerPLPPara);
    if (HI_SUCCESS != s32Ret)
    {
...
{
    TUNER_GETPLPINFO_S stTunerPLPInfo = {0};
    HI_S32 s32Ret;
    ...
    if (HI_NULL == pstPLPInfo)
    {
        HI_ERR_TUNER("Input parameter(pstPLPPara) invalid\n");
        return HI_ERR_TUNER_INVALID_POINT;
    }

    stTunerPLPInfo.u32Port = u32TunerId;
    stTunerPLPInfo.u32Index = u32PLPIndex;

    s32Ret = ioctl(s_s32TunerFd, TUNER_GET_PLP_INFO_CMD, &stTunerPLPInfo);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_TUNER("Get PLP group id fail.\n");
        return s32Ret;
    ...
    memcpy(pstPLPInfo, &stTunerPLPInfo.stPLPInfo, sizeof(HI_UNF_TUNER_TER_PLP_INFO_S));
