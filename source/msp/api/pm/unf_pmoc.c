...
#define LOG_D_MODULE_ID     HI_ID_PM
#define LOG_D_UNFTRACE      (1)
...
HI_S32 HI_UNF_PMOC_Init(HI_VOID)
{
    ...
    if (g_s32C51Fd < 0)
    {
        ...
        if (g_s32C51Fd < 0)
        {
            HI_ERR_PrintFuncResult(open, HI_ERR_PMOC_FAILED_INIT);
    ...
    if (g_Netfd < 0)
    {
        ...
        if (g_Netfd < 0)
        {
            HI_WARN_PrintFuncResult(socket, HI_FAILURE);
        }
    }
...
HI_S32 HI_UNF_PMOC_DeInit(HI_VOID)
{
    ...
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
    ...
    {
        HI_ERR_PrintFuncResult(ioctl, s32Ret);
        return HI_FAILURE;
    }
    ...
    if (0 == g_u32PmocInitCount)
    {
        ...
        {
            HI_ERR_PrintFuncResult(close, s32Ret);
            return HI_FAILURE;
        }
        g_s32C51Fd = -1;
...
HI_S32 HI_UNF_PMOC_SetStandbyDispMode(HI_UNF_PMOC_STANDBY_MODE_S_PTR pstStandbyMode)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pstStandbyMode, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
    HI_CHECK_PARAM(pstStandbyMode->u32Mode > DISPLAY_MODE_TIME, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
    ...
    if (pstStandbyMode->u32Mode == DISPLAY_MODE_TIME)
    {
        ...
        {
            HI_ERR_PrintInfo("time parameter is illegal \n");
    ...
    else if (pstStandbyMode->u32Mode == DISPLAY_MODE_DIGIT)
    {
        ...
        {
            HI_ERR_PrintInfo("chan num is illegal \n");
            return HI_ERR_PMOC_INVALID_PARA;
        }
    ...
    if (ret != HI_SUCCESS)
    {
        HI_ERR_PrintFuncResult(ioctl, ret);
...
HI_S32 HI_UNF_PMOC_SetWakeUpAttr(HI_UNF_PMOC_WKUP_S_PTR pstAttr)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pstAttr, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
    HI_CHECK_PARAM(pstAttr->u32IrPmocNum > PMOC_WKUP_IRKEY_MAXNUM, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
    HI_CHECK_PARAM(pstAttr->stNetwork.enEthIndex >= HI_UNF_PMOC_ETH_BUTT, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
    ...
    if (pstAttr->bMouseKeyboardEnable == HI_TRUE)
    {
        ...
        if (ret == HI_FAILURE)
        {
            HI_ERR_PrintFuncResult(get_remotewakeup_devnum, ret);
            return HI_ERR_PMOC_FAILED_SETWAKEUPVAL;
        }
        HI_INFO_PrintS32(ret);
        HI_INFO_PrintS32(u8IntMask);

        ret = set_remotewakeup();
        if (ret == HI_FAILURE)
        {
            HI_ERR_PrintFuncResult(set_remotewakeup, ret);
            return HI_ERR_PMOC_FAILED_SETWAKEUPVAL;
        }
            ...
            || pstAttr->stNetwork.bWakeupFrameEnable)
    {
        ...
        if (pm.wakeup_pkts_enable == 1)
        {
            for (u8FilterNumber = 0; u8FilterNumber < FILTER_COUNT; u8FilterNumber++)
            {
                ...
                {
                    HI_ERR_PrintInfo(" Filter Offset less than 12, force it to be 12 \n");
            ...
            for (u8FilterNumber = 0; u8FilterNumber < FILTER_COUNT; u8FilterNumber++)
            {
                HI_INFO_PrintS32(u8FilterNumber);
                HI_INFO_PrintS32(pm.filter[u8FilterNumber].valid);
                HI_INFO_PrintS32(pm.filter[u8FilterNumber].offset);
                HI_INFO_PrintH32(pm.filter[u8FilterNumber].mask_bytes);
...
HI_S32 HI_UNF_PMOC_ReadSystemMode(HI_UNF_PMOC_MODE_E * penSystemMode)
{
    ...
    HI_CHECK_PARAM(HI_NULL == penSystemMode, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
...
HI_S32 HI_UNF_PMOC_SwitchSystemMode(HI_UNF_PMOC_MODE_E enSystemMode, HI_UNF_PMOC_ACTUAL_WKUP_E * pu32WakeUpStatus)
{
    ...
    if (fp == NULL)
    {
        HI_ERR_PrintFuncResult(fopen, fp);
        return HI_ERR_PMOC_FAILED_STANDBY;
    }
    ...
    if (cnt >= MAX_COUNT)
    {
        HI_ERR_PrintInfo(" fail to get wake up mode after resume \n");
        return HI_ERR_PMOC_FAILED_STANDBY;
    }

    HI_INFO_PrintS32(wakeupmode.enWakeUpMode);
...
HI_S32 HI_UNF_PMOC_SetScene(HI_UNF_PMOC_SCENE_E eScene)
{
    ...
    HI_CHECK_PARAM(eScene != HI_UNF_PMOC_SCENE_STANDARD, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
...
HI_S32 HI_UNF_PMOC_SetDevType(HI_UNF_PMOC_DEV_TYPE_S_PTR pdevType)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pdevType, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);

    HI_INFO_PrintS32(pdevType->irtype);
    HI_CHECK_PARAM(pdevType->irtype >= HI_UNF_IR_CODE_BUTT, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);

    dev.irtype = pdevType->irtype;
    HI_INFO_PrintS32(dev.irtype);

#ifdef HI_KEYLED_SUPPORT
    HI_INFO_PrintS32(pdevType->kltype);
    HI_CHECK_PARAM(pdevType->kltype >= HI_UNF_KEYLED_TYPE_BUTT, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
...
HI_S32 HI_UNF_PMOC_LoadMcuCode(HI_U8 *pMcuCode, HI_U32 CodeSize)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pMcuCode, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);

    HI_INFO_PrintH32(CodeSize);
    HI_INFO_PrintH32(MCU_CODE_MAXSIZE);
    HI_CHECK_PARAM(CodeSize > MCU_CODE_MAXSIZE, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
...
HI_S32 HI_UNF_PMOC_SetPwrOffGpio(HI_U32 u32GpioNo,HI_BOOL bHighOrLow)
{
    ...
    HI_INFO_PrintS32(u32GpioNo);
    HI_CHECK_PARAM((u32GpioNo < GPIO5_NO_MIN) && (0 != u32GpioNo), HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
    HI_CHECK_PARAM(u32GpioNo > GPIO5_NO_MAX, HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);

    HI_INFO_PrintBool(bHighOrLow);
    HI_CHECK_PARAM((bHighOrLow != HI_TRUE) && (bHighOrLow != HI_FALSE), HI_ERR_PMOC_INVALID_PARA, HI_ERR_PMOC_INVALID_PARA);
...
HI_S32 HI_UNF_PMOC_GetStandbyPeriod(HI_U32 *pu32Period)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pu32Period, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
...
HI_S32 HI_UNF_PMOC_EnterSmartStandby(HI_U32 u32HoldModules)
{
    ...
    {
        HI_ERR_PrintInfo("can't re-enter standby \n");
        return HI_FAILURE;
    }
    ...
    return HI_SUCCESS;
#else
    HI_ERR_PrintInfo("smart standby is not support \n");
    return HI_FAILURE;
#endif
...
HI_S32 HI_UNF_PMOC_QuitSmartStandby(HI_VOID)
{
    ...
    {
        HI_ERR_PrintInfo("can't re-enter resume \n");
        return HI_FAILURE;
    }
...
HI_S32 HI_UNF_PMOC_GetWakeUpMode(HI_UNF_PMOC_WAKEUP_MODE_S_PTR pstAttr)
{
    ...
    pstAttr->u32GPIONo = wakeupmode.u32GPIONo;
...
HI_S32 HI_UNF_PMOC_GetChipTemp(HI_UNF_PMOC_CHIP_TEMP_S *pstChipTemp)
{
    ...
    HI_CHECK_PARAM(HI_NULL == pstChipTemp, HI_ERR_PMOC_INVALID_POINT, HI_ERR_PMOC_INVALID_POINT);
    HI_CHECK_PARAM(g_s32C51Fd < 0, HI_ERR_PMOC_NOT_INIT, HI_ERR_PMOC_NOT_INIT);
