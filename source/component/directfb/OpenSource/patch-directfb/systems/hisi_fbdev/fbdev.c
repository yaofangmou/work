...
static int fbdev_ioctl( int request, void *arg, int arg_size, int num );

#define FBDEV_IOCTL(request,arg,layerindex)   fbdev_ioctl( request, arg, sizeof(*(arg)),layerindex )

FBDev *dfb_fbdev[CONFIG_MAX_LAYER_NUM] = {NULL};
...
DFBResult primaryGetLevel (CoreLayer *layer,
                                                        void *driver_data,
                                                        void *layer_data,
                                                        int *level )
{
...
static DFBResult dfb_fbdev_read_modes( int num );
static DFBResult dfb_fbdev_set_gamma_ramp( DFBSurfacePixelFormat format, int num );
static DFBResult dfb_fbdev_set_palette( CorePalette *palette, int layerindex );
static DFBResult dfb_fbdev_set_rgb332_palette( int num );
static DFBResult dfb_fbdev_pan( int xoffset, int yoffset, bool onsync, int laylevel );
...
static DFBResult dfb_fbdev_test_mode       ( const VideoMode             *mode,
                                             ...
                                             int                          layerindex);
static DFBResult dfb_fbdev_test_mode_simple( const VideoMode             *mode, FBDevShared   *shared, int num );

static DFBResult dfb_fbdev_set_mode        ( const VideoMode             *mode,
                                             CoreSurface                 *surface,
                                             unsigned int                 xoffset,
                                             unsigned int                 yoffset, int num );
...
static DFBResult dfb_fbdev_open( char *pchpath, int nlayernum )
{
     DFBResult error_result = DFB_FAILURE;

     if ((nlayernum >= CONFIG_MAX_LAYER_NUM) || (nlayernum < 0))
     {
          goto error;
     }

     if (dfb_config->fb_device) {
          dfb_fbdev[nlayernum]->fd = open( dfb_config->fb_device, O_RDWR );
          if (dfb_fbdev[nlayernum]->fd < 0) {
          ...
          dfb_fbdev[nlayernum]->fd = open( getenv ("FRAMEBUFFER"), O_RDWR );
          if (dfb_fbdev[nlayernum]->fd < 0) {
               D_PERROR( "DirectFB/FBDev: Error opening '%s'!\n",
                          getenv ("FRAMEBUFFER"));
          ...
          dfb_fbdev[nlayernum]->fd = direct_try_open( pchpath, "/dev/fb/0", O_RDWR, true );
          if (dfb_fbdev[nlayernum]->fd < 0) {
     ...
     if (fcntl( dfb_fbdev[nlayernum]->fd, F_SETFD, FD_CLOEXEC ) < 0)
     {
...
static DFBResult system_fence_init(bool shutdown, int layer_id)
{
    ...
    if ((layer_id >= CONFIG_MAX_LAYER_NUM) || (layer_id < 0))
    {
        return DFB_OK;
    }

    if (NULL == dfb_fbdev[layer_id])
    {
       D_WARN("dfb_fbdev handle is null\n");
       return DFB_OK;
    }

    dfb_fbdev[layer_id]->current_fence_index = 0;

    if (false == shutdown)
    {
       for (num_index = 0; num_index < MAX_DISPLAY_BUFFER_NUM; num_index++)
       {
          dfb_fbdev[layer_id]->fence_fd[num_index] = -1;
       }
    ...
    for (num_index = 0; num_index < MAX_DISPLAY_BUFFER_NUM; num_index++)
    {
       if (dfb_fbdev[layer_id]->fence_fd[num_index] >= 0)
       {
          close(dfb_fbdev[layer_id]->fence_fd[num_index]);
       }
...
static DFBResult system_fbdev_initial(CoreDFB *core, void **data, char *pchpath, int layer_id)
{
     DFBResult            ret;
     CoreScreen          *screen;
     long                 page_size;
     ...
     D_ASSERT( dfb_fbdev[layer_id] == NULL );

     pool      = dfb_core_shmpool( core );
     pool_data = dfb_core_shmpool_data( core );

     dfb_fbdev[layer_id] = D_CALLOC( 1, sizeof(FBDev) );
     if (!dfb_fbdev[layer_id])
     {
          return D_OOM();
     }

     dfb_fbdev[layer_id]->fd = -1;

//HISILICON add begin
//tag, support fence sync
#ifdef CONFIG_DFB_SUPPORT_FENCE
     system_fence_init(false,layer_id);
#endif
//HISILICON add end
     ...
     sprintf(name, "fbdev%d", layer_id);
     core_arena_add_shared_field( core, name, shared );

     dfb_fbdev[layer_id]->core   = core;
     dfb_fbdev[layer_id]->shared = shared;

     page_size = direct_pagesize();

     shared->page_mask = page_size < 0 ? 0 : (page_size - 1);

     ret = dfb_fbdev_open(pchpath, layer_id);
     if (ret)
          goto error;

     if (dfb_config->vt) {
          ret = dfb_vt_initialize(layer_id);
          if (ret)
               goto error;
     }
     ...
     if (ioctl( dfb_fbdev[layer_id]->fd, FBIOGET_FSCREENINFO, &shared->fix ) < 0) {
     ...
     /* Map the framebuffer */
     dfb_fbdev[layer_id]->framebuffer_base = mmap( NULL, shared->fix.smem_len,
                                         PROT_READ | PROT_WRITE, MAP_SHARED,
                                         dfb_fbdev[layer_id]->fd, 0 );
     if (dfb_fbdev[layer_id]->framebuffer_base == MAP_FAILED) {
          D_PERROR( "DirectFB/FBDev: "
                    "Could not mmap the framebuffer!\n");
          dfb_fbdev[layer_id]->framebuffer_base = NULL;
          goto error;
     }

     if (ioctl( dfb_fbdev[layer_id]->fd, FBIOGET_VSCREENINFO, &shared->orig_var ) < 0) {
          D_PERROR( "DirectFB/FBDev: "
     ...
     if (ioctl( dfb_fbdev[layer_id]->fd, FBIOPUT_VSCREENINFO, &shared->current_var ) < 0) {
          D_PERROR( "DirectFB/FBDev: "
     ...
     if (ioctl( dfb_fbdev[layer_id]->fd, FBIOGETCMAP, &shared->orig_cmap ) < 0) {
          D_DEBUG( "DirectFB/FBDev: "
     ...
     if (dfb_config->agp) {
          ...
          ret = dfb_agp_initialize(layer_id);
          if (ret) {
     ...
     dfb_surface_pool_initialize( core, &fbdevSurfacePoolFuncs, &dfb_fbdev[layer_id]->shared->pool, layer_id + 2 );
     ...
     dfb_layers_register( screen, &layer_id, &primaryLayerFuncs);

     *data = dfb_fbdev[PRIMARYLAYER];
...
error:
     ...
     if (dfb_fbdev[layer_id]->framebuffer_base)
     {
          munmap( dfb_fbdev[layer_id]->framebuffer_base, shared->fix.smem_len );
     }

     if (dfb_fbdev[layer_id]->fd != -1)
     {
          close( dfb_fbdev[layer_id]->fd );
     }

     D_FREE( dfb_fbdev[layer_id] );
     dfb_fbdev[layer_id] = NULL;

     return ret;
}


static DFBResult system_initialize( CoreDFB *core, void **data)
{
    DFBResult ret = 0;
    int layer_id = 0;
    char layer_dev[256] = {'\0'};

    if (NULL == dfb_config)
    {
       D_WARN("dfb_config handle is null\n");
       return DFB_OK;
    }

    if (dfb_config->layer_num > CONFIG_MAX_LAYER_NUM)
    {
       D_ERROR( "layer_num is to more\n");
       return DFB_FAILURE;
    }

    for (layer_id = 0; layer_id < dfb_config->layer_num; layer_id++)
    {
        snprintf(layer_dev, sizeof(layer_dev), "/dev/fb%d",layer_id);

        ret = system_fbdev_initial(core, data, layer_dev,layer_id);
        if (DFB_OK != ret)
        {
            D_ERROR( "system_fbdev_initial failure\n" );
            return DFB_FAILURE;
        }
    }
...
static DFBResult system_fbdev_join(CoreDFB *core, void **data, char *pchpath, int layer_id)
{
     DFBResult   ret;
     CoreScreen *screen;
     void       *shared;
...
//HISILICON add end
     ...
     sprintf(name, "fbdev%d", layer_id);
     core_arena_get_shared_field( core, name, &shared );
     ...
     /* Map the framebuffer */
     dfb_fbdev[layer_id]->framebuffer_base = mmap( NULL, dfb_fbdev[layer_id]->shared->fix.smem_len,
                                         PROT_READ | PROT_WRITE, MAP_SHARED,
                                         dfb_fbdev[layer_id]->fd, 0 );
     if (dfb_fbdev[layer_id]->framebuffer_base == MAP_FAILED) {
          D_PERROR( "DirectFB/FBDev: "
     ...
     if (ret) {
          ...
          munmap(dfb_fbdev[layer_id]->framebuffer_base,dfb_fbdev[layer_id]->shared->fix.smem_len );
          close( dfb_fbdev[layer_id]->fd );
          D_FREE( dfb_fbdev[layer_id] );
          dfb_fbdev[layer_id] = NULL;
     ...
     dfb_surface_pool_join( core, dfb_fbdev[layer_id]->shared->pool, &fbdevSurfacePoolFuncs );
...
static DFBResult system_join(CoreDFB *core, void **data)
{
    DFBResult ret = 0;
    int layer_id = 0;
    char layer_dev[256] = {'\0'};
    ...
    for (layer_id = 0; layer_id < dfb_config->layer_num; layer_id++)
    {
        ...
        ret = system_fbdev_join( core, data, layer_dev, layer_id);
        if (DFB_OK != ret)
        {
            D_ERROR( "system_fbdev_join failure\n" );
            return DFB_FAILURE;
        }
    }
...
static DFBResult system_fbdev_shutdown(FBDev *pstrFbDev, bool emergency, int layer_id)
{
     DFBResult            ret;
     VideoMode           *m;
     FBDevShared         *shared;
...
//tag, support fence sync
#ifdef CONFIG_DFB_SUPPORT_FENCE
     system_fence_init(true,layer_id);
#endif
//HISILICON add end

     shared = pstrFbDev->shared;
     ...
     if (ioctl( pstrFbDev->fd, FBIOPUT_VSCREENINFO, &shared->orig_var ) < 0) {
          D_PERROR( "DirectFB/FBDev: "
                    "Could not restore variable screen information!\n" );
     }

     if (shared->orig_cmap.len) {
          if (ioctl( pstrFbDev->fd, FBIOPUTCMAP, &shared->orig_cmap ) < 0)
               D_DEBUG( "DirectFB/FBDev: "
     ...
     dfb_surface_pool_destroy(pstrFbDev->shared->pool);

     munmap(pstrFbDev->framebuffer_base, shared->fix.smem_len);
...
static DFBResult system_shutdown(bool emergency)
{
    ...
    for (layer_id = 0; layer_id < dfb_config->layer_num; layer_id++)
    {
        ret = system_fbdev_shutdown(dfb_fbdev[layer_id], emergency, layer_id);
        if (DFB_OK != ret)
        {
            D_ERROR( "system_fbdev_shutdown failure\n" );
            return DFB_FAILURE;
        }
...
static DFBResult system_fbdev_leave(FBDev *pstrFbDev, bool emergency, int layer_id)
{
     DFBResult ret;
...
//HISILICON add end
     ...
     dfb_surface_pool_leave(pstrFbDev->shared->pool);

     munmap(pstrFbDev->framebuffer_base,pstrFbDev->shared->fix.smem_len);
...
static DFBResult system_leave(bool emergency)
{
    ...
    for (layer_id = 0; layer_id < dfb_config->layer_num; layer_id++)
    {
        system_fbdev_leave(dfb_fbdev[layer_id], emergency, layer_id);
        if (DFB_OK != ret)
        {
            D_ERROR( "system_fbdev_shutdown failure\n" );
            return DFB_FAILURE;
...
system_get_accelerator( void )
{
     ...
     if (!strcmp( dfb_fbdev[PRIMARYLAYER]->shared->fix.id, "MATROX DH" ))
          return FB_ACCEL_MATROX_MGAG400;
#endif
#ifdef FB_ACCEL_EP9X
     if (!strcmp( dfb_fbdev[PRIMARYLAYER]->shared->fix.id, "ep9xfb" ))
     ...
     return dfb_fbdev[PRIMARYLAYER]->shared->fix.accel;
}

static VideoMode *
system_get_modes( void )
{
     return dfb_fbdev[PRIMARYLAYER]->shared->modes;
}

static VideoMode *
system_get_current_mode( void )
{
     return &dfb_fbdev[PRIMARYLAYER]->shared->current_mode;
}
...
{
     return dfb_fbdev[PRIMARYLAYER]->shared->fix.smem_start + offset;
}

static void *
system_video_memory_virtual( unsigned int offset )
{
     return(void*)((u8*)(dfb_fbdev[PRIMARYLAYER]->framebuffer_base) + offset);
}
...
system_videoram_length( void )
{
    ...
    for(i = 0; i < CONFIG_MAX_LAYER_NUM; i++)
    {
        if(dfb_fbdev[i] && dfb_fbdev[i]->shared->fix.smem_len)
        {
            break;
        }
    }
    return dfb_fbdev[i]->shared->fix.smem_len;
}

static unsigned long
system_aux_memory_physical( unsigned int offset )
{
     if (dfb_fbdev[PRIMARYLAYER]->shared->agp)
          return dfb_fbdev[PRIMARYLAYER]->shared->agp->info.aper_base + offset;
     return 0;
}
...
{
     if (dfb_fbdev[PRIMARYLAYER]->agp)
          return (void*)(u8*)dfb_fbdev[PRIMARYLAYER]->agp->base + offset;
     return NULL;
}
...
system_auxram_length( void )
{
    ...
    if (i == CONFIG_MAX_LAYER_NUM)
    {
        return 0;
    }

    if (dfb_fbdev[i]->shared->agp)
    {
        return dfb_fbdev[i]->shared->agp->agp_mem;
    }
...
{
     *ret_bus  = dfb_fbdev[PRIMARYLAYER]->shared->pci.bus;
     *ret_dev  = dfb_fbdev[PRIMARYLAYER]->shared->pci.dev;
     *ret_func = dfb_fbdev[PRIMARYLAYER]->shared->pci.func;
}
...
{
     *ret_vendor_id = dfb_fbdev[PRIMARYLAYER]->shared->device.vendor;
     *ret_device_id = dfb_fbdev[PRIMARYLAYER]->shared->device.model;
}
...
init_modes( int num )
{
     if ((num >= CONFIG_MAX_LAYER_NUM) || (num < 0))
     {
         return DFB_FAILURE;
     }

     dfb_fbdev_read_modes(num);

     if (!dfb_fbdev[num]->shared->modes) {
          /* try to use current mode*/
          dfb_fbdev[num]->shared->modes = (VideoMode*) SHCALLOC( dfb_fbdev[num]->shared->shmpool,
                                                            1, sizeof(VideoMode) );
          if (!dfb_fbdev[num]->shared->modes)
               return D_OOSHM();

          *dfb_fbdev[num]->shared->modes = dfb_fbdev[num]->shared->current_mode;

          if (dfb_fbdev_test_mode_simple(dfb_fbdev[num]->shared->modes,dfb_fbdev[num]->shared, num)) {
               ...
               D_ERROR( "DirectFB/FBDev: Current mode's pixelformat: "
                         ...
                         dfb_fbdev[num]->shared->orig_var.red.length,
                         dfb_fbdev[num]->shared->orig_var.red.offset,
                         dfb_fbdev[num]->shared->orig_var.green.length,
                         dfb_fbdev[num]->shared->orig_var.green.offset,
                         dfb_fbdev[num]->shared->orig_var.blue.length,
                         dfb_fbdev[num]->shared->orig_var.blue.offset,
                         dfb_fbdev[num]->shared->orig_var.transp.length,
                         dfb_fbdev[num]->shared->orig_var.transp.offset,
                         dfb_fbdev[num]->shared->orig_var.bits_per_pixel );
                  ...
                  void       *screen_data )
{
    ...
    if(screen)
    {
        nScreenNum = 0;//screen->shared->screen_id;
    }
     ...
     if (ioctl( dfb_fbdev[nScreenNum]->fd, FBIO_WAITFORVSYNC, &zero ))
         waitretrace();
                      ...
                      int        *ret_height )
{
    ...
    if(screen)
    {
     ...
     D_ASSERT( dfb_fbdev[nScreenNum] != NULL );
     D_ASSERT( dfb_fbdev[nScreenNum]->shared != NULL );

     *ret_width  = dfb_fbdev[nScreenNum]->shared->current_mode.xres;
     *ret_height = dfb_fbdev[nScreenNum]->shared->current_mode.yres;
                  ...
                  DFBColorAdjustment         *adjustment )
{
    ...
    if ((nLayerNum >= CONFIG_MAX_LAYER_NUM) || (nLayerNum < 0))
    {
     ...
     ret = init_modes(nLayerNum);
     if (ret)
          return ret;

     default_mode = dfb_fbdev[nLayerNum]->shared->modes;
                           ...
                           DFBColorAdjustment *adjustment )
{
     ...
     if(layer)
     {
        layerlevel=layer->shared->layer_id;
     }

     if ((layerlevel >= CONFIG_MAX_LAYER_NUM) || (layerlevel < 0))
     {
         return DFB_FAILURE;
     }

     struct fb_cmap *cmap       = &dfb_fbdev[layerlevel]->shared->current_cmap;
     struct fb_cmap *temp       = &dfb_fbdev[layerlevel]->shared->temp_cmap;
     ...
     if (dfb_fbdev[layerlevel]->shared->fix.visual != FB_VISUAL_DIRECTCOLOR)
     ...
     if (FBDEV_IOCTL( FBIOPUTCMAP, temp, layerlevel ) < 0) {
...
const VideoMode *
dfb_fbdev_find_mode( int width, int height, int layerindex )
{
     FBDevShared     *shared;
     const VideoMode *videomode;
     const VideoMode *highest   = NULL;

     if ((layerindex >= CONFIG_MAX_LAYER_NUM) || (layerindex < 0))
     {
         return NULL;
     }

     shared    = dfb_fbdev[layerindex]->shared;
                   ...
                   CoreLayerRegionConfigFlags *failed )
{
     ...
     if(layer)
     {
          numlayer = layer->shared->layer_id;
     }

     if ((numlayer >= CONFIG_MAX_LAYER_NUM) || (numlayer < 0))
     {
          return DFB_FAILURE;
     }

     FBDevShared                *shared = dfb_fbdev[numlayer]->shared;
     ...
     mode = dfb_fbdev_find_mode( config->source.w, config->source.h, numlayer );
     if (!mode) {
     ...
     if (dfb_fbdev_test_mode( mode, config, numlayer ))
                  ...
                  CoreSurfaceBufferLock      *right_lock )
{
    ...
    if(layer)
    {
        layerindex = layer->shared->layer_id;
    }
     ...
     FBDevShared *shared = dfb_fbdev[layerindex]->shared;
     ...
     if (updated & (CLRCF_SOURCE | CLRCF_WIDTH | CLRCF_HEIGHT | CLRCF_FORMAT | CLRCF_BUFFERMODE)) {
          if (updated & (CLRCF_WIDTH | CLRCF_HEIGHT | CLRCF_FORMAT | CLRCF_BUFFERMODE) ||
              ...
              config->source.h != shared->current_var.yres) {
               ...
               mode = dfb_fbdev_find_mode( config->source.w, config->source.h,layerindex );
               if (!mode) {
                    dummy = shared->current_mode;
               ...
               ret = dfb_fbdev_set_mode( mode, surface, config->source.x,
                                         left_lock->offset / left_lock->pitch + config->source.y,layerindex );
               if (ret)
                    return ret;
          }
          else {
               ret = dfb_fbdev_pan( config->source.x, left_lock->offset / left_lock->pitch + config->source.y, true, layerindex );
               if (ret)
                    return ret;
          }
     }
     ...
     if ((updated & CLRCF_PALETTE) && palette)
          dfb_fbdev_set_palette( palette,layerindex );
                   ...
                   CoreSurfaceBufferLock *right_lock )
{
     ...
     layer_num = layer->shared->layer_id;
     
     if ((layer_num >= CONFIG_MAX_LAYER_NUM) || (layer_num < 0))
     {
         return DFB_FAILURE;
     }
     ...
     ret = dfb_fbdev_pan(config->source.x,left_lock->offset / left_lock->pitch + config->source.y, false,layer_num);
     if (ret)
     {
        return ret;
     }
 ...
 */
static DFBResult
dfb_fbdev_pan( int xoffset, int yoffset, bool onsync,int numlayer )
{
     int result;
     ...
     shared = dfb_fbdev[numlayer]->shared;
...
#ifdef CONFIG_DFB_SUPPORT_FENCE
     if (false == dfb_config->fence_sync || numlayer != DLID_PRIMARY)
#endif
     {
         if (ioctl( dfb_fbdev[numlayer]->fd, FBIOPAN_DISPLAY, var ) < 0)
         {
              result = errno;
     ...
     if (ioctl( dfb_fbdev[numlayer]->fd, FBIO_HWC_REFRESH, &layer_info ) < 0)
     {
     ...
     dfb_fbdev_quene_buffer_id(0,layer_info.u32LayerAddr,&(dfb_fbdev[numlayer]->current_fence_index));
     dfb_fbdev[numlayer]->fence_fd[dfb_fbdev[numlayer]->current_fence_index] = layer_info.s32ReleaseFenceFd;
#else
     dfb_fbdev[numlayer]->fence_fd[dfb_fbdev[numlayer]->current_fence_index]  = layer_info.s32ReleaseFenceFd;

     if (DLBM_TRIPLE == config->buffermode)
     {
         dfb_fbdev[numlayer]->current_fence_index = (dfb_fbdev[numlayer]->current_fence_index + 1) % MAX_DISPLAY_BUFFER_NUM;
     }
     else
     {
         dfb_fbdev[numlayer]->current_fence_index = (dfb_fbdev[numlayer]->current_fence_index + 1) % (MAX_DISPLAY_BUFFER_NUM - 1);
     }
#endif
#endif
...
DFBResult dfb_fbdev_fence_sync_wait(int timeout, int layernum, int buffer_id)
{
    ...
    if (dfb_fbdev[layernum]->fence_fd[buffer_id] < 0)
    {
       return DFB_OK;
    }
    close(dfb_fbdev[layernum]->fence_fd[buffer_id]);
    dfb_fbdev[layernum]->fence_fd[buffer_id] = -1;
#else
    if (dfb_fbdev[layernum]->current_fence_index > (MAX_DISPLAY_BUFFER_NUM - 1))
    {
        return DFB_OK;
    }

    if (dfb_fbdev[layernum]->fence_fd[dfb_fbdev[layernum]->current_fence_index] < 0)
    {
        return DFB_OK;
    }

    ret = ioctl(dfb_fbdev[layernum]->fence_fd[dfb_fbdev[layernum]->current_fence_index], SYNC_IOC_WAIT, &to);
    if (ret < 0)
    {
        D_PERROR("dfb_fbdev_fence_sync_wait error!, wait: 0x%x\n", ret);
    }

    close(dfb_fbdev[layernum]->fence_fd[dfb_fbdev[layernum]->current_fence_index]);
    dfb_fbdev[layernum]->fence_fd[dfb_fbdev[layernum]->current_fence_index] = -1;
#endif
...
DFBResult dfb_fbdev_quene_free_buffer_id(int layer_id,int *pbufferid)
{
    ...
    HIFB_HWC_LAYERINFO_S layer_info;
    CoreLayerRegionConfig *config = &dfb_fbdev[layer_id]->shared->config;
    ...
    layer_info.eFmt = HIFB_FMT_ARGB8888;
    if (ioctl(dfb_fbdev[layer_id]->fd, FBIO_HWC_GETLAYERINFO, &layer_info) < 0)
    {
        return DFB_OK;
    }
...
DFBResult dfb_fbdev_quene_buffer_id(int layer_id,unsigned int buffer, int *pbufferid)
{
    ...
    FBDevShared  *shared = dfb_fbdev[layer_id]->shared;
...
DFBResult dfb_fbdev_quene_buffnum(int layer_id, int *pbuffer_num)
{
    ...
    if (NULL == dfb_fbdev[layer_id]->shared)
    {
       return DFB_OK;
    }

    config = &(dfb_fbdev[layer_id]->shared->config);
...
{
     if (ioctl( dfb_fbdev[PRIMARYLAYER]->fd, FBIOBLANK, level ) < 0) {
...
dfb_fbdev_mode_to_var( const VideoMode           *mode,
                       ...
                       struct fb_var_screeninfo  *ret_var, int layernum )
{
     struct fb_var_screeninfo  var;
     FBDevShared              *shared;

     if ((layernum >= CONFIG_MAX_LAYER_NUM) || (layernum < 0))
     {
         return DFB_FAILURE;
     }

     shared = dfb_fbdev[layernum]->shared;
...
dfb_fbdev_test_mode( const VideoMode             *mode,
                     const CoreLayerRegionConfig *config, int layerindex )
{
     DFBResult                  ret;
     struct fb_var_screeninfo   var;
     ...
     ret = dfb_fbdev_mode_to_var( mode, config->format, config->width, config->height,
                                  0, 0, config->buffermode, &var, layerindex );
     if (ret)
          return ret;
     ...
     if (FBDEV_IOCTL( FBIOPUT_VSCREENINFO, &var, layerindex ) < 0) {
          int erno = errno;
          dfb_gfxcard_unlock();
...
static DFBResult
dfb_fbdev_test_mode_simple( const VideoMode *mode, FBDevShared   *shared, int num  )
{
     DFBResult                ret;
     struct fb_var_screeninfo var;
     ...
     if (FBDEV_IOCTL( FBIOPUT_VSCREENINFO, &var, num ) < 0) {
          D_DEBUG_AT( FBDev_Mode, "  => FAILED!\n" );
          return errno2result( errno );
     }
                    ...
                    unsigned int             yoffset, int num )
{
    ...
    shared     = dfb_fbdev[num]->shared;
     ...
     ret = dfb_fbdev_mode_to_var( mode, config->format, config->size.w, config->size.h,
                                  xoffset, yoffset, buffermode, &var, num );
     if (ret) {
     ...
     if (FBDEV_IOCTL( FBIOPUT_VSCREENINFO, &var, num )) {
     ...
     if (FBDEV_IOCTL( FBIOGET_VSCREENINFO, &var2, num )) {
     ...
     FBDEV_IOCTL( FBIOGET_FSCREENINFO, &shared->fix, num );
     ...
     if (config->format == DSPF_RGB332)
          dfb_fbdev_set_rgb332_palette(num);
     else
          dfb_fbdev_set_gamma_ramp( config->format, num );

     /* invalidate original pan offset */
     shared->orig_var.xoffset = 0;
     shared->orig_var.yoffset = 0;

     dfb_surfacemanager_adjust_heap_offset( dfb_fbdev[num]->shared->manager,
...
dfb_fbdev_read_modes( int num )
{
     ...
     while (fgets(line,79,fp)) {
          if (sscanf(line, "mode \"%31[^\"]\"",label) == 1) {
               ...
               if (geometry && timings && !dfb_fbdev_test_mode_simple(&temp_mode, NULL, num)) {
...
static DFBResult
dfb_fbdev_set_gamma_ramp( DFBSurfacePixelFormat format, int num )
{
     int i;
    ...
    if ((num >= CONFIG_MAX_LAYER_NUM) || (num < 0))
    {
      ...
      */
     if (dfb_fbdev[num]->shared->fix.visual == FB_VISUAL_DIRECTCOLOR) {
     ...
     cmap = &dfb_fbdev[num]->shared->current_cmap;
     ...
     if (FBDEV_IOCTL( FBIOPUTCMAP, cmap, num ) < 0) {
          D_PERROR( "DirectFB/FBDev: "
...
static DFBResult
dfb_fbdev_set_palette( CorePalette *palette, int layerindex )
{
     int             i;
     struct fb_cmap *cmap;
     ...
     cmap = &dfb_fbdev[layerindex]->shared->current_cmap;
     ...
     if (FBDEV_IOCTL( FBIOPUTCMAP, cmap, layerindex ) < 0) {
          D_PERROR( "DirectFB/FBDev: Could not set the palette!\n" );
...
static DFBResult
dfb_fbdev_set_rgb332_palette( int num )
{
     ...
     pool = dfb_fbdev[num]->shared->shmpool_data;
                          ...
                          int          *ret_val )
{
     ...
     int nLayerIndex = *ret_val;

     if ((nLayerIndex >= CONFIG_MAX_LAYER_NUM) || (nLayerIndex < 0))
     {
         *ret_val = DFB_FAILURE;
         return FCHR_RETURN;
     }
     ...
     if (dfb_config->vt && !dfb_config->kd_graphics && call_arg == FBIOPUT_VSCREENINFO)
          ioctl( dfb_fbdev[nLayerIndex]->vt->fd, KDSETMODE, KD_GRAPHICS );

     ret = ioctl( dfb_fbdev[nLayerIndex]->fd, call_arg, call_ptr );
     if (ret)
          ret = errno;

     if (dfb_config->vt && !dfb_config->kd_graphics && call_arg == FBIOPUT_VSCREENINFO) {
          ioctl( dfb_fbdev[nLayerIndex]->vt->fd, KDSETMODE, KD_TEXT );
          res = write( dfb_fbdev[nLayerIndex]->vt->fd, cursoroff_str, strlen(cursoroff_str) );
          res = write( dfb_fbdev[nLayerIndex]->vt->fd, blankoff_str, strlen(blankoff_str) );
          (void)res;
     }
...
static int
fbdev_ioctl( int request, void *arg, int arg_size, int layerindex )
{
     int          ret;
     int          erno;
     void        *tmp_shm = NULL;
     ...
     D_ASSERT( dfb_fbdev[layerindex] != NULL );
     ...
     if (dfb_core_is_master( dfb_fbdev[layerindex]->core )) {
     ...
     if (arg) {
          if (!fusion_is_shared( dfb_core_world(dfb_fbdev[layerindex]->core), arg )) {
