...
{
    if (HI_NULL != g_stTrackMapSource.pstAipReg)
    {
#ifdef HI_AUDIO_DSP_SUPPORT
        DMAUnmap(&g_stTrackMapSource.stAipMap);
#else
...
#ifdef HI_AUDIO_DSP_SUPPORT
    g_stTrackMapSource.stAipMap.u32PhyAddr = g_stTrackMapSource.stParam.u32AipRegAddr;
    s32Ret = DMAMmap(&g_stTrackMapSource.stAipMap);
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_AO("DMAMmap aip register failed!\n");
        return s32Ret;
    }
    pAigReg = g_stTrackMapSource.stAipMap.pVirAddr;
#else
