...
static inline HI_U32 JPEG_DEC_Scanlines(j_decompress_ptr cinfo, HI_CHAR* LineBuf, HI_U32 MaxLines)
{
    ...
    {
        return JPEG_DEC_CscConvert((HI_ULONG)cinfo->client_data, LineBuf, MaxLines, &(cinfo->output_scanline));
    }
    ...
    if (HI_TRUE == HardDecSuccess)
    {
        ...
        HI_GFX_Print("++++++++hard dec success [%s %d]\n",__FUNCTION__,__LINE__);
#endif
        return JPEG_DEC_CscConvert((HI_ULONG)cinfo->client_data, LineBuf, MaxLines,&(cinfo->output_scanline));
    }
    ...
    {
#ifdef CONFIG_GFX_PRINT
       HI_GFX_Print("++++++++seek_input_data func is null [%s %d]\n",__FUNCTION__,__LINE__);
#endif
    ...
    {
       HI_GFX_Print("+++++++++++++seek_input_data failure [%s %d]\n",__FUNCTION__,__LINE__);
    }
