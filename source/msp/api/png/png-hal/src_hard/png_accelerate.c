...
#ifdef CONFIG_GFX_MEM_MMZ
        HI_GFX_Flush(pstTransform->u32ClutPhyaddr, bMmu);
        HI_GFX_Unmap(pstTransform->u32ClutPhyaddr, bMmu);
#else
        HI_GFX_Flush(pstHwStruct->s32MmzFd, pstTransform->u32ClutPhyaddr, pstHwStruct->pPallateMemData);
...
static HI_S32 hipng_alloc_mem(png_structp png_ptr)
{
    ...
    if ((HI_U64)u32MemSize * (HI_U64)png_ptr->height > UINT_MAX)
    {
        return HI_FAILURE;
    }
...
#ifdef CONFIG_GFX_MEM_MMZ
    HI_GFX_Flush(stInfo.u32Phyaddr, HI_TRUE);
#else
    HI_GFX_Flush(pstHwStruct->s32MmzFd, stInfo.u32Phyaddr, pstHwStruct->pTmpMemData);
#endif
    s32Ret = HI_PNG_Decode(pstHwStruct->s32Handle, &stInfo);
    if (s32Ret < 0)
    {
