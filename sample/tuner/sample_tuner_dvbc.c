...
HI_VOID hi_tuner_getspctrum(char * para)
{
    HI_S32 s32Ret = HI_FAILURE;

    FILE *fp = NULL;
    HI_U32 u32freqStartInHz = 0;
    HI_U32 u32freqStepInHz = 0;
    HI_U32 u32numOfFreqSteps = 0;
    HI_S16 s16powerData[130000] = {0};
    HI_U32 i = 0;
    ...
    s32Ret = sscanf(para, "%d" TEST_ARGS_SPLIT "%d" TEST_ARGS_SPLIT "%d", &u32freqStartInHz, &u32freqStepInHz,&u32numOfFreqSteps);

    sprintf(filename, "%d_%d_%d.txt", u32freqStartInHz,u32freqStepInHz,u32numOfFreqSteps);

    fp = fopen(filename, "wt");
    if (NULL == fp)
    {
       HI_TUNER_ERR("open fp ,line = %d\n", __LINE__);
       return;
    }

    s32Ret = HI_UNF_TUNER_GetTunerPowerSpectrumData(s_s32TunerPort,  u32freqStartInHz, u32freqStepInHz, u32numOfFreqSteps,s16powerData);
    if (HI_SUCCESS != s32Ret)
    {
        HI_TUNER_ERR("HI_UNF_TUNER_GetSignalStrength failed\n");
    }
    fprintf(fp, "freqStartInHz:%d    freqStepInHz:%d  numOfFreqSteps:%d\n", u32freqStartInHz,u32freqStepInHz,u32numOfFreqSteps);
...
TEST_FUNC_S g_testfunc[] =
{
    ...
    {"getspctrum", hi_tuner_getspctrum},
