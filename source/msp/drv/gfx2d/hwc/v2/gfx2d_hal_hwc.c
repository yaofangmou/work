...
typedef struct tagHWC_DEBUG_REG_S
{
    HI_U32 u32Status;
    HI_U32 u32Reserved[39];
    HI_U32 u32WCnt;
}HWC_DEBUG_REG_S;
...
/* debug level */
static HI_U32 gs_Gfx2dHalHwcDebugLevel = 0;
...
static volatile HWC_DEBUG_REG_S *gs_pstHWCDebugReg = HI_NULL;
#ifdef GFX2D_ALPHADETECT_SUPPORT
...
HI_S32 GFX2D_HAL_HWC_Init(HI_VOID)
{
    ...
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, InitFilterParam, s32Ret);
        return s32Ret;
    }

    u32NodeNum = GFX2D_CONFIG_GetNodeNum();
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32NodeNum);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, sizeof(HWC_NODE_S));
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, g_u32NodeSize);
...
HI_S32 GFX2D_HAL_HWC_Open(HI_VOID)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, InitDevReg, s32Ret);
        return s32Ret;
    }
...
static HWC_NODE_S *GFX2D_HAL_HWC_DuplicateNodes(HWC_NODE_S *pstNode, HI_U32 u32NodeNum)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstNode, NULL);
    ...
    for (i = 0; i < u32NodeNum; i++)
    {
        ...
        if (pstCurNode == NULL)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_MEM_Alloc,  FAILURE_TAG);
            goto ERR;
        }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstConfig);
...
static HI_VOID GFX2D_HAl_HWC_CalcSpZme(HI_U8 u8LayerId, GFX2D_HAL_BLOCKCONFIG_S *pstConfig, HWC_NODE_S *pstSplitNode)
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstConfig);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstSplitNode);
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap_luma);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xed_pos_cord_in_tap_luma);
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_chroma);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xed_pos_cord_in_chroma);

    pstConfig->hor_coffset_cfg_int = GFX2D_HAL_HWC_ComplementToTrueValue((((HI_U32)pstConfig->hor_coffset) >> GFX2D_HAL_RATIO_PRECISE), 8);
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap_chroma);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xed_pos_cord_in_tap_chroma);
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap_sp);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xed_pos_cord_in_tap_sp);

    pstConfig->node_cfg_zme_iw_sp = pstConfig->xed_pos_cord_in_tap_sp - pstConfig->xst_pos_cord_in_tap_sp + 1;

    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->node_cfg_zme_iw_sp);
...
static HI_VOID GFX2D_HAL_HWC_CalcZme(HI_U8 u8LayerId, GFX2D_HAL_BLOCKCONFIG_S *pstConfig, HWC_NODE_S *pstSplitNode)
{
    ...
    pstConfig->hor_loffset_cfg_int = GFX2D_HAL_HWC_ComplementToTrueValue(((HI_U32)(pstConfig->hor_loffset) >> GFX2D_HAL_RATIO_PRECISE), 8);
    ...
    else if (u8LayerId == 1)
    {
        ...
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xed_pos_cord);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord);
        ...
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->u32ZmeInWidth);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->u32ZmeOutWidth);
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstConfig, HI_FALSE);
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_EXIT_FUNC);
...
static HI_VOID GFX2D_HAL_HWC_CalcXStartPos(GFX2D_HAL_BLOCKCONFIG_S *pstConfig)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap_sp);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap_rgb);
...
static HI_VOID GFX2D_HAL_HWC_CalcXEndPos(GFX2D_HAL_BLOCKCONFIG_S *pstConfig)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->xst_pos_cord_in_tap);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstConfig->hpzme_en);
...
static HI_VOID GFX2D_HAL_HWC_CalcResolution(HI_U8 u8LayerId, GFX2D_HAL_BLOCKCONFIG_S *pstConfig, HWC_NODE_S *pstSplitNode)
{
    ...
    HI_GFX_CHECK_ARRAY_OVER_RETURN_NOVALUE(u8LayerId, 7);
...
static HI_VOID GFX2D_HAL_HWC_ConfigSplitNodeSrc(HWC_NODE_S *pstNode, HWC_NODE_S *pstSplitNode, HI_U32 u32DstStartPos, HI_U32 u32DstEndPos)
{
    ...
    for (u8LayerId = 0; u8LayerId < GFX2D_ARRAY_SIZE(pstNode->astLayer); u8LayerId++)
    {
        ...
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT,u32DstStartPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stBlockConfig.xfpos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32DstEndPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, stBlockConfig.xdpos);
...
static HI_S32 GFX2D_HAL_HWC_SplitNode(HWC_NODE_S *pstNode)
{
    ...
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_HAL_HWC_DuplicateNodes,  FAILURE_TAG);
...
/***************************************************************************************
* func          : GFX2D_HAL_HWC_ConfigComposeList
* description   : CNcomment: 1、将任务拆分成硬件支持的多个小任务
                             2、比如硬件支持1层缩放，用户传进来两层缩放，就要分解成两个
                                任务，做两次操作，比如支持5层，用户传进来12层，则就要分
                                3个任务分布完成
                             CNend\n
* param[in]     : HI_VOID
* retval        : NA
                                              ...
                                              HI_GFX2D_SURFACE_S *pstDstSurface)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, 0);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList->pstCompose, 0);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstNewComposeList, 0);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstDstSurface, 0);
    HI_GFX_CHECK_LEFT_LARGER_RIGHT_RETURN_VALUE(pstComposeList->u32ComposeCnt, HWC_MAX_LAYERS, 0);
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, GFX2D_HAL_MAX_NODE);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u8NodeCnt + 1);
        ...
        {
            u8ZmeLayerCnt++;
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8ZmeLayerCnt);
        }

        if (u8ZmeLayerCnt <= gs_stCapability.u8ZmeLayerNum)
        {
            u8LayerCnt++;
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8LayerCnt);
        }
        ...
        if (u8NodeCnt == 0)
        {
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8NodeCnt);
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8LayerCnt);
            HI_GFX_Memcpy(pstCompose, pstComposeList->pstCompose, sizeof(HI_GFX2D_COMPOSE_S) * u8LayerCnt);
        }
        else
        {
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8StartLayer);
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u8LayerCnt);
                                       ...
                                       HWC_NODE_S **ppstNode)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstDstSurface, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(ppstNode, HI_FAILURE);
    HI_GFX_CHECK_ARRAY_OVER_RETURN_VALUE(u8NodeCnt, (GFX2D_HAL_MAX_NODE + 1), HI_FAILURE);
    ...
    for (i = 0; i < u8NodeCnt; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckCompose,  FAILURE_TAG);
            goto ERR;
        }
        ...
        if (s32Ret < 0)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, SelectLayerId, s32Ret);
            goto ERR;
        }
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, HWC_MAX_LAYERS);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstComposeList[i].u32ComposeCnt);
            s32Ret = HI_ERR_GFX2D_NO_MEM;
            goto ERR;
        }
        ...
        if (s32Ret < 0)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_HAL_HWC_SplitNode,  FAILURE_TAG);
            goto ERR;
        }
        ...
        if (i > 0)
        {
            ...
            if (s32Ret < 0)
            {
                HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_HAL_HWC_LinkNode,  FAILURE_TAG);
                goto ERR;
            }
        }
    }
                             ...
                             HI_VOID **ppNode, GFX2D_HAL_DEV_TYPE_E *penNodeType)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(ppNode, HI_ERR_GFX2D_NULL_PTR);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(penNodeType, HI_ERR_GFX2D_NULL_PTR);

    if ((0 == pstComposeList->u32ComposeCnt) || (pstComposeList->u32ComposeCnt > gs_stCapability.u8MaxLayerNum))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstComposeList->u32ComposeCnt);
                             ...
                             HI_VOID **ppNode, GFX2D_HAL_DEV_TYPE_E *penNodeType)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstDstSurface, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(ppNode, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(penNodeType, HI_FAILURE);

    s32Ret = GFX2D_HAL_HWC_CheckCompose(enDevId, pstComposeList, pstDstSurface, ppNode, penNodeType);
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_HAL_HWC_CheckCompose, s32Ret);
        return s32Ret;
    }
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, GFX2D_HAL_MAX_NODE);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u8NodeCnt);
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, GFX2D_HAL_HWC_CreateNode,  FAILURE_TAG);
        s32Ret = HI_ERR_GFX2D_UNSUPPORT;
        goto ERR;
    }
...
static inline HI_VOID InitSmmuCrgReg(HI_VOID)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, g_pstRegCrg);
        return;
    }
...
static inline HI_S32 InitDevReg(HI_VOID)
{
    ...
    if (HI_NULL == gs_pstHWCDebugReg)
    {
        gs_pstHWCDebugReg = HI_GFX_REG_MAP((HWC_REG_BASE_ADDR + 0x4000), sizeof(HWC_DEBUG_REG_S));
        if (HI_NULL == gs_pstHWCDebugReg)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, HI_GFX_REG_MAP,  FAILURE_TAG);
            goto ERROR_EXIT;
...
ERROR_EXIT:
    ...
    if (HI_NULL != gs_pstHWCDebugReg)
    {
        HI_GFX_REG_UNMAP(gs_pstHWCDebugReg);
    }
    gs_pstHWCDebugReg = HI_NULL;
...
{
    HI_VOID *pVir = NULL;
    HI_U32 u32FiterMemSize = sizeof(gs_as32HorizonCoef) + sizeof(gs_as32VerticalCoef);
    ...
    pVir = HI_GFX_Map(gs_u32HFilterParamAddr, u32FiterMemSize, bMmu);
    if (NULL == pVir)
    {
...
static inline HI_VOID SetDst(HWC_NODE_S *pstNode, HI_GFX2D_SURFACE_S *pstDstSurface)
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstDstSurface);

    pstNode->unOutCtl.stBits.bit1Enable = 0x1;

    HI_GFX_CHECK_ARRAY_OVER_RETURN_NOVALUE(pstDstSurface->enFmt, HI_GFX2D_FMT_PLANAR444 + 1);
...
static HI_VOID GetZmeInfo(HWC_NODE_S *pstNode, HWC_LAYER_ID enLayerId, GFX2D_HAL_ZME_S *pstZme)
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstZme);
...
static inline HI_VOID SetZme(HWC_NODE_S *pstNode, HI_GFX2D_COMPOSE_S *pstCompose, HWC_LAYER_ID enLayerId)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstNode);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstCompose);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstCompose->stOutRect.u32Width);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstCompose->stOutRect.u32Height);
        return;
    }
    ...
    if (u32HStep >= (16 << GFX2D_HAL_HWC_HSTEP_FLOATLEN))
    {
        ...
        HI_GFX_CHECK_LEFT_EQUAL_RIGHT_RETURN_NOVALUE(0, HdwsmFactor);
...
static HI_VOID TraceCfg(HI_GFX2D_COMPOSE_LIST_S *pstComposeList)
{
    ...
    HI_GFX2D_COMPOSE_S *pstCompose = NULL;
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstComposeList);
    HI_GFX_CHECK_LEFT_LARGER_RIGHT_RETURN_NOVALUE(pstComposeList->u32ComposeCnt, HWC_MAX_LAYERS);
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT,i);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stSurface.enType);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOpt.stResize.bResizeEnable);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.u32Width);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.u32Height);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.u32Width);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.u32Width);
    }
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
...
{
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstSurface, HI_FALSE);
...
{
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstSurface, HI_FALSE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstOpt, HI_FALSE);
...
{
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pu8LayerRefRecord, HWC_LAYER_BUTT);
...
static HWC_LAYER_ID SelectLayer(HI_U8 *pu8LayerRefRecord)
{
    HI_U8 i = 0;
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pu8LayerRefRecord, HWC_LAYER_VHD);
...
{
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstCompose, HI_FALSE);
...
static HI_S32 SelectLayerId(HI_GFX2D_COMPOSE_LIST_S *pstComposeList, HWC_LAYER_ID *penLayerId)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(penLayerId, HI_FAILURE);
    HI_GFX_CHECK_LEFT_LARGER_RIGHT_RETURN_VALUE(pstComposeList->u32ComposeCnt, HWC_MAX_LAYERS, HI_FAILURE);
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        {/*YUV格式，必须走视频层*/
            ...
            HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, penLayerId[i]);
        }
        ...
        {/*缩放, 优先图形层G0，后视频层 */
            ...
            {
                HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, i);
                HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "unsupport");
                goto ERR;
            }
        }
        else
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, i);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, au8Ref[penLayerId[i]]);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "this layer has been seleced");
            goto ERR;
        }

        if (HI_TRUE == CheckSelectResultInvalid(penLayerId[i], pstCompose))
        {
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "this video layer no support");
            goto ERR;
        }
...
static inline HI_VOID SetGLayer(HWC_LAYER_ID enLayerId,HWC_NODE_S *pstNode,HI_GFX2D_COMPOSE_S *pstCompose)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, HWC_LAYER_G0);
    ...
    else
    {
        ...
        HI_GFX_CHECK_ARRAY_OVER_RETURN_NOVALUE(pstSurface->enFmt, HI_GFX2D_FMT_PLANAR444 + 1);
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pu8HorizonSampleFactor);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pu8VerticalSampleFactor);
...
static inline HI_VOID SetVHDLayer(HWC_LAYER_ID enLayerId,HWC_NODE_S *pstNode,HI_GFX2D_COMPOSE_S *pstCompose)
{
    ...
    {
        HI_GFX_CHECK_LEFT_EQUAL_RIGHT_RETURN_NOVALUE(0, u8VerticalSampleFactor);
        HI_GFX_CHECK_LEFT_EQUAL_RIGHT_RETURN_NOVALUE(0, u8HorizonSampleFactor);
...
{
    if ((NULL != pstNode) && (GFX2D_DEBUG_NODE_LEVEL_5 == gs_Gfx2dHalHwcDebugLevel))
    {
        HI_GFX_Print("[module-gfx-gfx2d][beg-node-info]=====================================================================\n");

        if (1 == pstNode->astLayer[0].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC1_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G0CTRL ):0x%08x |*| ", pstNode->astLayer[0].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G0ADDR      ):0x%08x |*| ", pstNode->astLayer[0].u32Phyaddr);
            HI_GFX_Print("(HWC_G0STRIDE    ):0x%08x |*| ", pstNode->astLayer[0].u32Stride);
            HI_GFX_Print("(HWC_G0CADDR     ):0x%08x |*| ", pstNode->astLayer[0].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G0CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[0].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G0IRESO     ):0x%08x |*| ", pstNode->astLayer[0].unRes.u32All);
            HI_GFX_Print("(HWC_G0DIMPARA   ):0x%08x |*| ", pstNode->astLayer[0].u32DimValue);
            HI_GFX_Print("(HWC_G0ALPHA     ):0x%08x |*| ", pstNode->astLayer[0].unAlpha.u32All);
            HI_GFX_Print("(HWC_G0PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[0].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[1].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC2_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_VHDCTRL ):0x%08x |*| ", pstNode->astLayer[1].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_VHDADDR     ):0x%08x |*| ", pstNode->astLayer[1].u32Phyaddr);
            HI_GFX_Print("(HWC_VHDSTRIDE   ):0x%08x |*| ", pstNode->astLayer[1].u32Stride);
            HI_GFX_Print("(HWC_VHDCADDR    ):0x%08x |*| ", pstNode->astLayer[1].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_VHDCSTRIDE  ):0x%08x |*| ", pstNode->astLayer[1].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_VHDIRESO    ):0x%08x |*| ", pstNode->astLayer[1].unRes.u32All);
            HI_GFX_Print("(HWC_VHDDIMPARA  ):0x%08x |*| ", pstNode->astLayer[1].u32DimValue);
            HI_GFX_Print("(HWC_VHDALPHA    ):0x%08x |*| ", pstNode->astLayer[1].unAlpha.u32All);
            HI_GFX_Print("(HWC_VHDPIXOFFSET):0x%08x |*| ", pstNode->astLayer[1].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[2].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC3_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G1CTRL ):0x%08x |*| ", pstNode->astLayer[2].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G1ADDR      ):0x%08x |*| ", pstNode->astLayer[2].u32Phyaddr);
            HI_GFX_Print("(HWC_G1STRIDE    ):0x%08x |*| ", pstNode->astLayer[2].u32Stride);
            HI_GFX_Print("(HWC_G0CADDR     ):0x%08x |*| ", pstNode->astLayer[2].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G1CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[2].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G1IRESO     ):0x%08x |*| ", pstNode->astLayer[2].unRes.u32All);
            HI_GFX_Print("(HWC_G1DIMPARA   ):0x%08x |*| ", pstNode->astLayer[2].u32DimValue);
            HI_GFX_Print("(HWC_G1ALPHA     ):0x%08x |*| ", pstNode->astLayer[2].unAlpha.u32All);
            HI_GFX_Print("(HWC_G1PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[2].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[3].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC4_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G2CTRL ):0x%08x |*| ", pstNode->astLayer[3].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G2ADDR      ):0x%08x |*| ", pstNode->astLayer[3].u32Phyaddr);
            HI_GFX_Print("(HWC_G2STRIDE    ):0x%08x |*| ", pstNode->astLayer[3].u32Stride);
            HI_GFX_Print("(HWC_G2CADDR     ):0x%08x |*| ", pstNode->astLayer[3].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G2CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[3].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G2IRESO     ):0x%08x |*| ", pstNode->astLayer[3].unRes.u32All);
            HI_GFX_Print("(HWC_G2DIMPARA   ):0x%08x |*| ", pstNode->astLayer[3].u32DimValue);
            HI_GFX_Print("(HWC_G2ALPHA     ):0x%08x |*| ", pstNode->astLayer[3].unAlpha.u32All);
            HI_GFX_Print("(HWC_G2PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[3].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[4].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC5_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G3CTRL ):0x%08x |*| ", pstNode->astLayer[4].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G3ADDR      ):0x%08x |*| ", pstNode->astLayer[4].u32Phyaddr);
            HI_GFX_Print("(HWC_G3STRIDE    ):0x%08x |*| ", pstNode->astLayer[4].u32Stride);
            HI_GFX_Print("(HWC_G3CADDR     ):0x%08x |*| ", pstNode->astLayer[4].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G3CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[4].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G3IRESO     ):0x%08x |*| ", pstNode->astLayer[4].unRes.u32All);
            HI_GFX_Print("(HWC_G3DIMPARA   ):0x%08x |*| ", pstNode->astLayer[4].u32DimValue);
            HI_GFX_Print("(HWC_G3ALPHA     ):0x%08x |*| ", pstNode->astLayer[4].unAlpha.u32All);
            HI_GFX_Print("(HWC_G3PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[4].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[5].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC6_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G4CTRL ):0x%08x |*| ", pstNode->astLayer[5].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G4ADDR      ):0x%08x |*| ", pstNode->astLayer[5].u32Phyaddr);
            HI_GFX_Print("(HWC_G4STRIDE    ):0x%08x |*| ", pstNode->astLayer[5].u32Stride);
            HI_GFX_Print("(HWC_G4CADDR     ):0x%08x |*| ", pstNode->astLayer[5].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G4CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[5].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G4IRESO     ):0x%08x |*| ", pstNode->astLayer[5].unRes.u32All);
            HI_GFX_Print("(HWC_G4DIMPARA   ):0x%08x |*| ", pstNode->astLayer[5].u32DimValue);
            HI_GFX_Print("(HWC_G4ALPHA     ):0x%08x |*| ", pstNode->astLayer[5].unAlpha.u32All);
            HI_GFX_Print("(HWC_G4PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[5].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->astLayer[6].unCtrl.stBits.bit1IsEnable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->SRC7_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_G5CTRL ):0x%08x |*| ", pstNode->astLayer[6].unCtrl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G5ADDR      ):0x%08x |*| ", pstNode->astLayer[6].u32Phyaddr);
            HI_GFX_Print("(HWC_G5STRIDE    ):0x%08x |*| ", pstNode->astLayer[6].u32Stride);
            HI_GFX_Print("(HWC_G5CADDR     ):0x%08x |*| ", pstNode->astLayer[6].u32CbcrPhyaddr);
            HI_GFX_Print("(HWC_G5CSTRIDE   ):0x%08x |*| ", pstNode->astLayer[6].u32CbcrStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_G5IRESO     ):0x%08x |*| ", pstNode->astLayer[6].unRes.u32All);
            HI_GFX_Print("(HWC_G5DIMPARA   ):0x%08x |*| ", pstNode->astLayer[6].u32DimValue);
            HI_GFX_Print("(HWC_G5ALPHA     ):0x%08x |*| ", pstNode->astLayer[6].unAlpha.u32All);
            HI_GFX_Print("(HWC_G5PIXOFFSET ):0x%08x |*| ", pstNode->astLayer[6].u32PixOffset);
            HI_GFX_Print("\n");
        }

        if (1 == pstNode->unOutCtl.stBits.bit1Enable)
        {
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("--->DES_CTRL INFO:\n");
            HI_GFX_Print("-------------------------------\n");
            HI_GFX_Print("(HWC_OUTCTRL ):0x%08x |*| ", pstNode->unOutCtl.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_OUTADDR     ):0x%08x |*| ", pstNode->u32OutPhyaddr);
            HI_GFX_Print("(HWC_OUTADDR_C   ):0x%08x |*| ", pstNode->u32OutChromPhyaddr);
            HI_GFX_Print("(HWC_OUTSTRIDE   ):0x%08x |*| ", pstNode->u32OutStride);
            HI_GFX_Print("(HWC_OUTSTRIDE_C ):0x%08x |*| ", pstNode->u32OutChromStride);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_OUTRESO     ):0x%08x |*| ", pstNode->unOutRes.u32All);
            HI_GFX_Print("(HWC_OUTALPHA    ):0x%08x |*| ", pstNode->unOutAlpha.u32All);
            HI_GFX_Print("(HWC_OUTCROPSTART):0x%08x |*| ", pstNode->unOutCropStart.u32All);
            HI_GFX_Print("(HWC_OUTCROPEND  ):0x%08x |*| ", pstNode->unOutCropEnd.u32All);
            HI_GFX_Print("\n");
            HI_GFX_Print("(HWC_MMUCTRL     ):0x%08x |*| ", pstNode->unMmuCtl.u32All);
            HI_GFX_Print("(HWC_OUTPIXOFFSET):0x%08x |*| ", pstNode->u32OutPixOffset);
            HI_GFX_Print("(HWC_OUTARADDR   ):0x%08x |*| ", pstNode->u32ARAddr);
            HI_GFX_Print("(HWC_OUTGBADDR   ):0x%08x |*| ", pstNode->u32GBAddr);
            HI_GFX_Print("\n");
        }

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->GFX_HSP INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(HWC_G0HSP       ):0x%08x |*| ", pstNode->unHsp.u32All);
        HI_GFX_Print("(HWC_G0LHOFFSET  ):0x%08x |*| ", pstNode->unHLumOffset.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_G0CHOFFSET  ):0x%08x |*| ", pstNode->unHChromaOffset.u32All);
        HI_GFX_Print("(HWC_G0VSP       ):0x%08x |*| ", pstNode->unVsp.u32All);
        HI_GFX_Print("(HWC_G0VSR       ):0x%08x |*| ", pstNode->u32Vsr);
        HI_GFX_Print("(HWC_G0VOFFSET   ):0x%08x |*| ", pstNode->u32VOffset);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_G0ZMEOUTRES ):0x%08x |*| ", pstNode->unZmeOutRes.u32All);
        HI_GFX_Print("(HWC_G0ZMEINRES  ):0x%08x |*| ", pstNode->unZmeInRes.u32All);
        HI_GFX_Print("(HWC_G0HPZME     ):0x%08x |*| ", pstNode->unHpZme.u32All);
        HI_GFX_Print("(HWC_G0HPZMERES  ):0x%08x |*| ", pstNode->unHpZmeReso.u32All);
        HI_GFX_Print("\n");

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->VIDEO_HSP INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(HWC_VHDCSCIDC0  ):0x%08x |*| ", pstNode->unVhdCscIdc0.u32All);
    #ifdef CONFIG_GFX_HWC_VERSION_3_0
        HI_GFX_Print("(HWC_VHDCSCIDC1  ):0x%08x |*| ", pstNode->unVhdCscIdc1.u32All);
    #endif
        HI_GFX_Print("(HWC_VHDCSCODC0  ):0x%08x |*| ", pstNode->unVhdCscOdc0.u32All);
    #ifdef CONFIG_GFX_HWC_VERSION_3_0
        HI_GFX_Print("(HWC_VHDCSCODC1  ):0x%08x |*| ", pstNode->unVhdCscOdc1.u32All);
    #endif
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDCSCP0    ):0x%08x |*| ", pstNode->unVhdCscParam0.u32All);
        HI_GFX_Print("(HWC_VHDCSCP1    ):0x%08x |*| ", pstNode->unVhdCscParam1.u32All);
        HI_GFX_Print("(HWC_VHDCSCP2    ):0x%08x |*| ", pstNode->unVhdCscParam2.u32All);
        HI_GFX_Print("(HWC_VHDCSCP3    ):0x%08x |*| ", pstNode->unVhdCscParam3.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDCSCP4    ):0x%08x |*| ", pstNode->unVhdCscParam4.u32All);
        HI_GFX_Print("(HWC_VHDHSP      ):0x%08x |*| ", pstNode->unVhdHsp.u32All);
        HI_GFX_Print("(HWC_VHDLHOFFSET ):0x%08x |*| ", pstNode->unVhdLumHOffset.u32All);
        HI_GFX_Print("(HWC_VHDCHOFFSET ):0x%08x |*| ", pstNode->unVhdChromaHOffset.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDVSP      ):0x%08x |*| ", pstNode->unVhdVsp.u32All);
        HI_GFX_Print("(HWC_VHDVSR      ):0x%08x |*| ", pstNode->u32VhdVsr);
        HI_GFX_Print("(HWC_VHDVOFFSET  ):0x%08x |*| ", pstNode->u32VhdVOffset);
        HI_GFX_Print("(HWC_VHDZMEOUTRES):0x%08x |*| ", pstNode->unVhdZmeOutRes.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDZMEINRES ):0x%08x |*| ", pstNode->unVhdZmeInRes.u32All);
        HI_GFX_Print("(HWC_VHDHPZME    ):0x%08x |*| ", pstNode->unVhdHpZme.u32All);
        HI_GFX_Print("(HWC_VHDHPZMERES ):0x%08x |*| ", pstNode->unVhdHpZmeReso.u32All);
        HI_GFX_Print("(HWC_CSCMUX      ):0x%08x |*| ", pstNode->unCscMux.u32All);
        HI_GFX_Print("\n");

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->OUT_CSC INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(HWC_OUTCSCIDC0  ):0x%08x |*| ", pstNode->unOutCscIdc0.u32All);
    #ifdef CONFIG_GFX_HWC_VERSION_3_0
        HI_GFX_Print("(HWC_OUTCSCODC1  ):0x%08x |*| ", pstNode->unOutCscIdc1.u32All);
    #endif
        HI_GFX_Print("(HWC_OUTCSCODC0  ):0x%08x |*| ", pstNode->unOutCscOdc0.u32All);
    #ifdef CONFIG_GFX_HWC_VERSION_3_0
        HI_GFX_Print("(HWC_OUTCSCODC1  ):0x%08x |*| ", pstNode->unOutCscOdc1.u32All);
    #endif
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_OUTCSCP0    ):0x%08x |*| ", pstNode->unOutCscParam0.u32All);
        HI_GFX_Print("(HWC_OUTCSCP1    ):0x%08x |*| ", pstNode->unOutCscParam1.u32All);
        HI_GFX_Print("(HWC_OUTCSCP2    ):0x%08x |*| ", pstNode->unOutCscParam2.u32All);
        HI_GFX_Print("(HWC_OUTCSCP3    ):0x%08x |*| ", pstNode->unOutCscParam3.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_OUTCSCP4    ):0x%08x |*| ", pstNode->unOutCscParam4.u32All);
        HI_GFX_Print("(HWC_OUTDSWM     ):0x%08x |*| ", pstNode->unOutDswm.u32All);
        HI_GFX_Print("(HWC_CBMMIX      ):0x%08x |*| ", pstNode->unCbmMix.u32All);
        HI_GFX_Print("(HWC_CBMBKGV     ):0x%08x |*| ", pstNode->u32CbmBkgValue);
        HI_GFX_Print("\n");

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->OTHERS INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(HWC_G0CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[0].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G0OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[0].unCbmStartPos.u32All);
        HI_GFX_Print("(HWC_VHDCBMPARA  ):0x%08x |*| ", pstNode->astCmbInfo[1].unCbmPara.u32All);
        HI_GFX_Print("(HWC_VHDOUTPOS   ):0x%08x |*| ", pstNode->astCmbInfo[1].unCbmStartPos.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_G1CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[2].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G1OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[2].unCbmStartPos.u32All);
        HI_GFX_Print("(HWC_G2CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[3].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G2OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[3].unCbmStartPos.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_G3CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[4].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G3OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[4].unCbmStartPos.u32All);
        HI_GFX_Print("(HWC_G4CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[5].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G4OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[5].unCbmStartPos.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_G5CBMPARA   ):0x%08x |*| ", pstNode->astCmbInfo[6].unCbmPara.u32All);
        HI_GFX_Print("(HWC_G5OUTPOS    ):0x%08x |*| ", pstNode->astCmbInfo[6].unCbmStartPos.u32All);
        HI_GFX_Print("(HWC_CMPGLBINFO  ):0x%08x |*| ", pstNode->unCmpGlbInfo.u32All);
        HI_GFX_Print("(HWC_CMPRESO     ):0x%08x |*| ", pstNode->unCmpReso.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_CMPCFG0     ):0x%08x |*| ", pstNode->unCmpCfg0.u32All);
        HI_GFX_Print("(HWC_CMPCFG1     ):0x%08x |*| ", pstNode->unCmpCfg1.u32All);
        HI_GFX_Print("(HWC_CMPCFG2     ):0x%08x |*| ", pstNode->unCmpCfg2.u32All);
        HI_GFX_Print("(HWC_CMPCFG3     ):0x%08x |*| ", pstNode->unCmpCfg3.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_CMPCFG4     ):0x%08x |*| ", pstNode->unCmpCfg4.u32All);
        HI_GFX_Print("(HWC_CMPCFG5     ):0x%08x |*| ", pstNode->unCmpCfg5.u32All);
        HI_GFX_Print("(HWC_CMPCFG6     ):0x%08x |*| ", pstNode->unCmpCfg6.u32All);
        HI_GFX_Print("(HWC_CMPCFG7     ):0x%08x |*| ", pstNode->unCmpCfg7.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_CMPCFG8     ):0x%08x |*| ", pstNode->unCmpCfg8.u32All);
        HI_GFX_Print("(HWC_CMPCFG9     ):0x%08x |*| ", pstNode->unCmpCfg9.u32All);
        HI_GFX_Print("(HWC_CMPMAXLOWREN):0x%08x |*| ", pstNode->u32CmpMaxRowLen);
        HI_GFX_Print("\n");

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->GFX_COEF INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(G0HCOEFADDR            ):0x%08x |*| ", pstNode->u32G0HCoefPhyaddr);
        HI_GFX_Print("(G0VCOEFADDR            ):0x%08x |*| ", pstNode->u32G0VCoefPhyaddr);
        HI_GFX_Print("\n");
        HI_GFX_Print("(G0HCHROMACOEFADDR      ):0x%08x |*| ", pstNode->u32G0HChromaCoefPhyaddr);
        HI_GFX_Print("(G0VCHROMACOEFADDR      ):0x%08x |*| ", pstNode->u32G0VChromaCoefPhyaddr);
        HI_GFX_Print("\n");

        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("--->VIDEO_COEF INFO:\n");
        HI_GFX_Print("-------------------------------\n");
        HI_GFX_Print("(HWC_VHDHCOEFADDR       ):0x%08x |*| ", pstNode->u32HCoefPhyaddr);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDVCOEFADDR       ):0x%08x |*| ", pstNode->u32VCoefPhyaddr);
        HI_GFX_Print("(HWC_VHDHCHROMACOEFADDR ):0x%08x |*| ", pstNode->u32HChromaCoefPhyaddr);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_VHDVCHROMACOEFADDR ):0x%08x |*| ", pstNode->u32VChromaCoefPhyaddr);
        HI_GFX_Print("(HWC_AXIID              ):0x%08x |*| ", pstNode->u32AXIID);
        HI_GFX_Print("\n");
        HI_GFX_Print("(HWC_NODEID             ):0x%08x |*| ", pstNode->u32NodeId);
        HI_GFX_Print("(HWC_INTMASK            ):0x%08x |*| ", pstNode->unIntMask.u32All);
        HI_GFX_Print("\n");
        HI_GFX_Print("[module-gfx-gfx2d][end-node-info]=====================================================================\n");
    }
    return;
}
...
HI_S32 GFX2D_HAL_HWC_SubNode(HI_GFX2D_DEV_ID_E enDevId, HI_VOID *pNode)
{
    ...
    if (HI_NULL == gs_pstHWCReg)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, gs_pstHWCReg);
        return HI_SUCCESS;
    }
    ...
    {
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO,  "gs_pstHWCReg->u32AqListHeadAddr == 0");
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pstNode);
    }
...
static HI_S32 GFX2D_HAL_HWC_LinkNode(HI_VOID *pCurNode, HI_VOID *pNextNode)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pCurNode, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pNextNode, HI_FAILURE);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstCurNode->pu32Histogram);
        return HI_FAILURE;
    }
#endif
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstCurNode->u32NextNodePhyaddr);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pstNextNode);
    }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(ppstNodeOps);
...
HI_U32 GFX2D_HAL_HWC_GetIntStatus(HI_VOID)
{
    ...
    if (u32IntStatus & 0x4)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32IntStatus);
    }
...
/** 使用二进制换算工具看哪个像素格式支持, 1的支持, 0不支持, bit同时对应枚举 **/
...
static HI_S32 CheckSurfaceType(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, bInputSurface);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enType);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_SURFACE_TYPE_COLOR);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_SURFACE_TYPE_MEM);
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");

    if (HI_GFX2D_SURFACE_TYPE_COLOR < pstSurface->enType)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstSurface->enType);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid surface type");
        return HI_ERR_GFX2D_INVALID_SURFACE_TYPE;
    }

    if (!bInputSurface && (HI_GFX2D_SURFACE_TYPE_MEM != pstSurface->enType))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, bInputSurface);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstSurface->enType);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid output surface type");
        return HI_ERR_GFX2D_INVALID_SURFACE_TYPE;
    }
...
static HI_S32 CheckSurfaceFmt(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, bInputSurface);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_FMT_YUV888);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_FMT_AYUV8888);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt / 32);
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");

    if (pstSurface->enFmt > HI_GFX2D_FMT_PLANAR444)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->enFmt);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid fmt");
        return HI_ERR_GFX2D_INVALID_SURFACE_FMT;
    }

    if (bInputSurface)
    {
        if ((pstSurface->enFmt / 32) >= GFX2D_ARRAY_SIZE(gs_stCapability.au32InputFmtBitmap))
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->enFmt);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid input fmt");
            return HI_ERR_GFX2D_INVALID_SURFACE_FMT;
        }

        if (!IS_BIT_SET((pstSurface->enFmt % 32), gs_stCapability.au32InputFmtBitmap[pstSurface->enFmt / 32]))
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->enFmt);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid input fmt");
            return HI_ERR_GFX2D_INVALID_SURFACE_FMT;
    ...
    {
        if ((pstSurface->enFmt / 32) >= GFX2D_ARRAY_SIZE(gs_stCapability.au32OutputFmtBitmap))
        {
             HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->enFmt);
             HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid output fmt");
             return HI_ERR_GFX2D_INVALID_SURFACE_FMT;
        }

        if (!IS_BIT_SET((pstSurface->enFmt % 32), gs_stCapability.au32OutputFmtBitmap[pstSurface->enFmt / 32]))
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->enFmt);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid output fmt");
            return HI_ERR_GFX2D_INVALID_SURFACE_FMT;
...
static HI_S32 CheckSurfaceAddr(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, bInputSurface);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstSurface->u32Phyaddr[0]);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstSurface->u32Phyaddr[1]);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstSurface->u32Phyaddr[2]);

    if (0 == pstSurface->u32Phyaddr[0])
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstSurface->u32Phyaddr[0]);
        return HI_ERR_GFX2D_INVALID_SURFACE_ADDR;
    }

    if ((pstSurface->enFmt <= HI_GFX2D_FMT_SEMIPLANAR400) || (pstSurface->enFmt == HI_GFX2D_FMT_PLANAR400))
    {
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_FMT_SEMIPLANAR400);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HI_GFX2D_FMT_PLANAR400);
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
        return HI_SUCCESS;
    }
    ...
    if (pstSurface->enFmt <= HI_GFX2D_FMT_PLANAR400)
    {
        HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_EXIT_FUNC);
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
        return HI_SUCCESS;
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_XINT, pstSurface->u32Phyaddr[2]);
        return HI_ERR_GFX2D_INVALID_SURFACE_ADDR;
    }
...
static HI_S32 CheckSurfaceStride(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, bInputSurface);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->u32Stride[0]);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->u32Stride[1]);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->u32Stride[2]);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Stride[0]);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Stride[1]);
        return HI_ERR_GFX2D_INVALID_SURFACE_STRIDE;
    }
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Stride[2]);
        return HI_ERR_GFX2D_INVALID_SURFACE_STRIDE;
    }
...
static HI_S32 CheckSurfaceCompress(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, bInputSurface);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->enFmt);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstSurface->stCompressInfo.enCompressType);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->stCompressInfo.enCompressType);
    ...
    else
    {
        if (HI_GFX2D_COMPRESS_NOLOSS == pstSurface->stCompressInfo.enCompressType)
        {
            ...
            {
                HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Width);
                return HI_ERR_GFX2D_UNSUPPORT;
            }
...
static HI_S32 CheckSurfaceSolution(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Width);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MinWidth);
        return HI_ERR_GFX2D_INVALID_SURFACE_RESO;
    }

    if (pstSurface->u32Height < gs_stCapability.u32MinHeight)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstSurface->u32Height);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MinHeight);
        return HI_ERR_GFX2D_INVALID_SURFACE_RESO;
    }
...
static HI_S32 CheckSurface(HI_GFX2D_SURFACE_S *pstSurface, HI_BOOL bInputSurface)
{
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceType, s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceFmt, s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceAddr, s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceStride, s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceCompress, s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurfaceSolution, s32Ret);
        return s32Ret;
    }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstRegionRect, HI_FAILURE);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT,  pstRegionRect->s32XPos);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT,  pstRegionRect->s32YPos);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRegionRect->u32Width);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRegionRect->u32Height);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRect->u32Width);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRect->u32Height);
    ...
    if ((pstRegionRect->u32Width < gs_stCapability.u32MinWidth) || (pstRegionRect->u32Width > gs_stCapability.u32MaxWidth))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRegionRect->u32Width);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MinWidth);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MaxWidth);
        return HI_ERR_GFX2D_INVALID_SURFACE_RESO;
    }
    ...
    if ((pstRegionRect->u32Height < gs_stCapability.u32MinHeight) || (pstRegionRect->u32Height > gs_stCapability.u32MaxHeight))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstRegionRect->u32Height);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MinHeight);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MaxHeight);
        return HI_ERR_GFX2D_INVALID_SURFACE_RESO;
    }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstClip, HI_FAILURE);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstRect, HI_FAILURE);

    if (pstClip->enClipMode >= HI_GFX2D_CLIP_OUTSIDE)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstClip->enClipMode);
    ...
    else if (HI_GFX2D_CLIP_INSIDE == pstClip->enClipMode)
    {
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64ClipLXPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64RXPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64LXPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64ClipTYPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64BYPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, s64TYPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstClip->stClipRect.s32XPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstClip->stClipRect.s32YPos);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstClip->stClipRect.u32Width);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstClip->stClipRect.u32Height);
...
static HI_S32 CheckResize(HI_GFX2D_COMPOSE_S *pstCompose)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstCompose, HI_FAILURE);
    ...
    if (!pstResize->bResizeEnable)
    {
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstInRect->u32Width);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstOutRect->u32Width);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstInRect->u32Height);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstOutRect->u32Height);
            return HI_ERR_GFX2D_INVALID_RECT;
        }
        return HI_SUCCESS;
    }
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, HI_GFX2D_FILTER_NONE);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_INT, pstResize->stFilter.enFilterMode);
    ...
    else if ((pstOutRect->u32Height * gs_stCapability.u32MaxVerticalZmeRatio) <= pstInRect->u32Height)
    {
        ...
        {
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, pstCompose->stSurface.u32Stride[0]);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u32MaxStride);
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32Factor);
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "unsupport");
            return HI_ERR_GFX2D_UNSUPPORT;
        }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstRop, HI_ERR_GFX2D_UNSUPPORT);
    if (pstRop->bEnable)
    {
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "unsupport");
        return HI_ERR_GFX2D_UNSUPPORT;
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstColorkey, HI_ERR_GFX2D_UNSUPPORT);
...
static HI_S32 CheckComposeOption(HI_GFX2D_COMPOSE_OPT_S *pstOpt, HI_GFX2D_RECT_S *pstOutRect)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckRop, s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckColorkey, s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckClip, s32Ret);
        return s32Ret;
    }
...
static HI_S32 CheckComposeSrc(HI_GFX2D_COMPOSE_S *pstCompose,HI_GFX2D_SURFACE_S *pstDstSurface)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurface, s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckRect, s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckComposeOption, s32Ret);
        return s32Ret;
    }
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckResize, s32Ret);
        return s32Ret;
    }
                           ...
                           HI_GFX2D_SURFACE_S *pstDstSurface)
{
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_FAILURE;
    HI_U32 u32VhdLayerNum = 0; /** 视频层 **/
    HI_U32 u32GLayerNum = 0; /** 图形层 **/
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckSurface,  FAILURE_TAG);
        return s32Ret;
    }
    HI_GFX_CHECK_LEFT_LARGER_RIGHT_RETURN_VALUE(pstComposeList->u32ComposeCnt, HWC_MAX_LAYERS, HI_FAILURE);
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        if (HI_SUCCESS != s32Ret)
        {
            HI_GFX_OutErrFuncMsg(HI_GFX_MSG_ERR_FUNC, CheckComposeSrc,  FAILURE_TAG);
            return s32Ret;
        }
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32GLayerNum);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32ZmeLayerNum);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32VhdLayerNum);
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32VhdLayerNum);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32GLayerNum);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32ZmeLayerNum);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u8MaxVLayerNum);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u8MaxGLayerNum);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, gs_stCapability.u8ZmeLayerNum);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "unsupport");
        return HI_ERR_GFX2D_UNSUPPORT;
    }
...
static HI_VOID AdjustClipRect(HI_GFX2D_RECT_S *pstInRect, HI_GFX2D_RECT_S *pstOutRect, HI_GFX2D_RECT_S *pstClipRect)
{
    ...
    HI_S32 s32RXPos = pstOutRect->s32XPos + pstOutRect->u32Width - 1;
    HI_S32 s32TYPos = pstOutRect->s32YPos;
    HI_S32 s32BYPos = pstOutRect->s32YPos + pstOutRect->u32Height - 1;
    ...
    HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstInRect->s32XPos);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstInRect->s32YPos);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstInRect->u32Width);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstInRect->u32Height);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstOutRect->s32XPos);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstOutRect->s32YPos);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstOutRect->u32Width);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstOutRect->u32Height);
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pRect);
    switch (enFmt)
    {
        ...
        case HI_GFX2D_FMT_PLANAR420:
        {
            ...
            pRect->s32YPos = (HI_U32)(pRect->s32YPos) & 0xfffffffe;
            break;
        }
                             ...
                             HI_GFX2D_SURFACE_S *pstDstSurface)
{
    ...
    for (i = 0; i < pstComposeList->u32ComposeCnt; i++)
    {
        ...
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.s32XPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.s32YPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.u32Width);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stInRect.u32Height);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.s32XPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.s32YPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.u32Width);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOutRect.u32Height);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOpt.stClip.stClipRect.s32XPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOpt.stClip.stClipRect.s32YPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOpt.stClip.stClipRect.u32Width);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, pstCompose->stOpt.stClip.stClipRect.u32Height);
...
HI_VOID GFX2D_HAL_HWC_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    PROC_PRINT(p, "++++++++++++++++++++++++++++ GFX2D Hal Info +++++++++++++++++++++++\n");
    if (HI_NULL != gs_pstHWCDebugReg)
    {
        PROC_PRINT(p, "work status\t: %s\n",  (0 == (gs_pstHWCDebugReg->u32Status & 0x1)) ? ("idle") : ("busy"));
        /**400M时钟 (1/(400 * 1000000))  *  (gs_pstHWCReg->u32TimeCnt) * 1000**/
        PROC_PRINT(p, "logical cost times\t: [%u %ums]\n",  gs_pstHWCDebugReg->u32WCnt, (1 * gs_pstHWCDebugReg->u32WCnt * 1000) / (400 * 1000000));
    }
...
HI_VOID GFX2D_HAL_HWC_SetDebugLevel(HI_U32 LevelValue)
{
    gs_Gfx2dHalHwcDebugLevel = LevelValue;
}
#endif
