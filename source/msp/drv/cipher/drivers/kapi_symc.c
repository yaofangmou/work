...
s32 kapi_symc_create(u32 *id, CRYP_CIPHER_TYPE_E type)
{
    ...
    if (HI_SUCCESS != ret)
    {
        ...
        {
            if (0x00 == (CIPHER_HARD_CHANNEL_MASK & (0x1 << chn)))
            {
                continue;
            }
    ...
    *id = HI_HANDLE_MAKEHANDLE(HI_ID_CIPHER, 0, chn);
    ...
    HI_LOG_INFO("[SYMC] create handle 0x%x, owner 0x%x\n", *id, ctx->owner);
...
s32 kapi_symc_destroy(u32 id)
{
    ...
    HI_LOG_INFO("[SYMC] destroy handle 0x%x, owner 0x%x\n", id, ctx->owner);
...
static s32 kapi_symc_load_hard_key(void *cryp_ctx, u32 handle, u32 catype, u8 *key)
{
...
#else
    symc_key_source src = SYMC_KEY_SRC_KLAD_HARD;

    CRYPTO_UNUSED(handle);
    CRYPTO_UNUSED(key);

    if (HI_UNF_CIPHER_CA_TYPE_CLEARCW == catype)
    {
        src = SYMC_KEY_SRC_KLAD_SOFT;
    }

    cryp_symc_set_key_source(cryp_ctx, src);
                    ...
                    compat_addr aad, u32 alen, u32 tlen)
{
    ...
    */
    ...
    if (HI_NULL == ctx->func)
    {
        HI_LOG_ERROR("error, nonsupport enAlg %d, enWorkMode %d\n",
            enAlg, enWorkMode);
    ...
    if (ctx->func->setkey)
    {
        if (HI_TRUE == byca)
        {
            ...
            CHECK_EXIT(kapi_symc_load_hard_key(ctx->cryp_ctx, id, catype, fkey));
        }
        else
        {
            cryp_symc_set_key_source(ctx->cryp_ctx, SYMC_KEY_SRC_CPU);
...
void kapi_symc_get_user_uuid(u32 chn_num, crypto_owner *uuid)
{
    kapi_symc_ctx *ctx = HI_NULL;

    HI_LOG_FuncEnter();

    if ((HI_NULL == uuid) || (CRYPTO_HARD_CHANNEL_MAX <= chn_num))
    {
         crypto_memset(uuid, sizeof(crypto_owner), 0, sizeof(crypto_owner));
         return;
    }

    ctx = &kapi_ctx[chn_num];
    crypto_memcpy(uuid, sizeof(crypto_owner), &ctx->owner, sizeof(crypto_owner));
