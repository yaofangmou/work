...
{
	uart_early_puts("\r\nReg Version:  ");
	uart_early_puts((char *)(regbase + 0x4));

	uart_early_puts("\r\nReg Time:     ");
	uart_early_puts((char *)(regbase + 0xc));

	uart_early_puts("\r\nReg Name:     ");
	uart_early_puts((char *)(regbase + 0x20));
	uart_early_puts("\r\n");
	return;
}
...
#define CONFIG_EXTKEY_AREA_SIG_LEN      0x100
	...
	unsigned int regbase = (unsigned int )(get_fbl_ext_area(&ext_length) + CONFIG_EXTKEY_AREA_LEN + CONFIG_EXTKEY_AREA_SIG_LEN);
	...
	uart_early_puts("Reg init 0x");
	uart_early_put_hex(regbase);
	uart_early_puts("\r\n");
