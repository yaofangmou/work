                                         ...
                                         HI_VOID **ppNode, GFX2D_HAL_DEV_TYPE_E *penNodeType)
{
    ...
    if ((0 == pstComposeList->u32ComposeCnt) || (pstComposeList->u32ComposeCnt > gs_stCapability.u8MaxLayerNum))
    {
        ...
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_DEBUG_UINT, gs_stCapability.u8MaxLayerNum);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Invalid compose cnt");
                                         ...
                                         HI_U8 u8NodeCnt)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstComposeList, 0);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(pstDstSurface, 0);
    HI_GFX_CHECK_ARRAY_OVER_RETURN_VALUE(u8NodeCnt, (GFX2D_HAL_MAX_NODE + 1), 0);
    ...
    for (i = 0; i < u8NodeCnt; i++)
    {
        ...
        if (i > 0)
        {
            ...
            {
                HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pstNode[i]);
            }
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstPara);
    switch (u32Index % 12)
    {
        case 0:
                               ...
                               HI_S32 *ps32ChmPara, HI_U32 u32ChmTap)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(pstPara);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(ps32LumPara);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(ps32ChmPara);
...
static HI_S32 InitFilterParam(HI_VOID)
{
    ...
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HWC_HORIZON_PARA_SIZE_TOTAL);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, HWC_VERTICAL_PARA_SIZE_TOTAL);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32FiterMemSize);
...
{
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_NOVALUE(punHsp);
...
{
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, enFmt);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32InWidth);
    HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_UINT, u32OutWidth);
...
HI_VOID PrintNodeInfo(HWC_NODE_S *pstNode)
{
    ...
    {
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unIntMask.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32HCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VCoefPhyaddr);
    #if HWC_VERSION >= 0x200
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32HChromaCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VChromaCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32CfgErrIntMask);
    #if HWC_VERSION == 0x400
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32G0HCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32G0VCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32G0HChromaCoefPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32G0VChromaCoefPhyaddr);
    #endif
    #endif
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCtl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdYPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdCbPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdCrPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdYSride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdCrStride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCropStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCropEndPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdRes.u32All);
    #if HWC_VERSION <= 0x300
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdDrawMode.u32All);
    #endif
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdInRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCbmPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutEndPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscIdc.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscOdc.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscParam0.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscParam1.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscParam2.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscParam3.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCscParam4.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdHsp.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdLumHOffset.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdChromaHOffset.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdVsp.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdVsr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32VhdVOffset);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdZmeOutRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdZmeInRes.u32All);
    #if HWC_VERSION == 0x400
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCropPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCropInReso.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unVhdCropOutReso.u32All);
    #endif
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unOutEndPos.u32All);
    #if HWC_VERSION == 0x400
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unG0Hsp.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unLumHOffset.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unChromaHOffset.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unVsp.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32Vsr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].u32VOffset);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unZmeOutRes);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unZmeInRes);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unCropPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unCropInReso.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[0].unCropOutReso.u32All);
    #endif
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[1].unOutEndPos.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[2].unOutEndPos.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[3].unOutEndPos.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[4].unOutEndPos.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].unCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].u32Phyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].u32Stride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].u32SfPos);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].u32DimValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].unBmpPara.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].unRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].unOutStartPos.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->astGLayer[5].unOutEndPos.u32All);

        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32CbmBkgValue);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCbmMix.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCtl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32OutPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32OutChromPhyaddr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32OutStride);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutRes.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscIdc.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscOdc.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscParam0.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscParam1.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscParam2.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscParam3.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unOutCscParam4.u32All);
    #if HWC_VERSION == 0x400
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpGlbInfo.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpReso.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg0.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg1.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg2.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg3.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg4.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg5.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg6.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg7.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg8.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCfg9.u32All);
    #else
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->unCmpCtrl.u32All);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32CmpDdr);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32CmpDataOffsetL);
        HI_GFX_OutDebugValueMsg(HI_GFX_MSG_DEBUG_XINT, pstNode->u32CmpDataOffsetC);
    #endif
        HI_GFX_OutDebugInfoMsg(HI_GFX_MSG_DEBUG_INFO, "==========================================================");
    }
    return;
...
HI_U32 GFX2D_HAL_HWC_GetIntStatus(HI_VOID)
{
    ...
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(gs_pstHWCReg,0);
    ...
    if (u32IntStatus & 0x100)
    {
        ...
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Fatal err");
    }

    if (u32IntStatus & 0x80)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32IntStatus);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Dec read err");
    }

    if (u32IntStatus & 0x40)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32IntStatus);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Slv read err");
    }

    if (u32IntStatus & 0x20)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32IntStatus);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Dec write err");
    }

    if (u32IntStatus & 0x10)
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, u32IntStatus);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "Slv write err");
    }
...
HI_VOID GFX2D_HAL_HWC_ReadProc(struct seq_file *p, HI_VOID *v)
{
    ...
    if (gs_pstHWCReg)
    {
        PROC_PRINT(p, "Status\t: %s\n", (gs_pstHWCReg->u32Status & 0x1) ? "busy" : "idle");
        PROC_PRINT(p, "logical cost times\t: [%u %ums]\n",  gs_pstHWCReg->u32WCnt, (1 * gs_pstHWCReg->u32WCnt * 1000) / (400 * 1000000));
    }
    else
    {
        PROC_PRINT(p, "Status\t\t:idle\n");
    }
