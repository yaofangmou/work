...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_Module_Ioctl);
}
...
{
    return (long)HI_DRV_UserCopy(file->f_path.dentry->d_inode, file, cmd, arg, CMPI_Module_Compat_Ioctl);
}
#endif
...
{
#ifdef CMN_MMGR_SUPPORT
    KMODULE_MemMgr_Pool_Init(u32ModuleCount);
#endif
