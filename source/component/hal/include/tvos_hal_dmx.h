...
typedef struct  _DMX_STATUS_S
{
    ...
    U32 u32TsErrPacketCount;              /**<Ts error packet number.*//**<CNcomment: 错误ts包数量 */
} DMX_STATUS_S;
...
typedef  struct _DMX_PARSER_FILTER_OPEN_PARAM_S
{
    ...
    BOOL                    bSecureChannel; /**<Channel secure flag, true means secure channel. */ /**<CNcomment:  通道安全标记,true表示是安全通道 */
...
typedef struct _DMX_ESFRAME_INFO_S
{
    ...
    U64 u64BufKerVirAddr; /**<Buffer address. */ /**<CNcomment: Es Buffer内核态虚拟地址 */
    U64 u64BufPhyAddr;  /**<Buffer address. */ /**<CNcomment: Es Buffer物理地址 */
} DMX_ESFRAME_INFO_S;

/**Demux avfilter attached channel info*/
/**CNcomment:Demux 过滤器绑定的通道信息 */
typedef struct _DMX_AVFILTER_INFO_S
{
    BOOL    bSecureChannel; /**<Channel secure flag, true means secure channel. */ /**<CNcomment: 通道安全标记,true表示是安全通道 */
    U32     u32BufSize;     /**<Channel buf total size. */ /**<CNcomment: 通道buf总大小 */
    DMX_CHANNEL_TYPE_E enChannelType; /**<Channel type */ /**<CNcomment: 通道接收数据的类型 */
} DMX_AVFILTER_INFO_S;
...
typedef struct _DEMUX_DEVICE_S
{
    ...
    /**
     \brief Set demux source. CNcomment:设置Demux数据源，并与数据源绑定 CNend
     \attention \n
    CNcomment: 设置之后处于绑定状态 CNend
    ...
    /**
     ...
     \attention \n
     The fuction deprecated now, will be abandoned, use function dmx_set_source_params instead, pass source type params of DMX_SOURCE_NONE.
     CNcomment: 本函数不赞成使用了，后续会被废弃，使用函数dmx_set_source_params替代，传入类型为DMX_SOURCE_NONE的数据源参数。CNend
     \param[in] enDemuxId demux id.CNcomment:demux id CNend
     \retval ::ERROR_NOT_SUPPORTED not supported.CNcomment:此函数已废弃使用。CNend
     \see \n
    ::DMX_ID_E
     */
    ...
    /**
     ...
     \attention \n
     The fuction deprecated now, will be abandoned.
     CNcomment: 本函数不赞成使用了，后续会被废弃。CNend
     \param[in] enDemuxId demux id.CNcomment:demux id CNend
     \retval ::ERROR_NOT_SUPPORTED not supported.CNcomment:此函数已废弃使用。CNend
     \see \n
    ::DMX_ID_E
    ...
    /**
     ...
     \attention \n
    CNcomment:如果(u16Pid==INVALID_PID)如果平台支持,请clear该通道的pid. 如果pid 参数大于 INVALID_PID, HDI 需要修正为INVALID_PID. 设置PID必须在通道disable状态时设置. CNend
     \param[in] u32ChannelId demux channel id.CNcomment:Demux通道ID CNend
    ...
    */
    ...
    /**
     ...
     \attention \n
     CNcomment:设置过滤器条件必须在过滤器disable状态时设置. CNend
     \param[in] u32ChannelId demux channel id.CNcomment:数据通道ID CNend
    ...
    /**
    \brief Query secure flag, mem info. CNcomment:用于查询通道信息，包括通道是否安全通道，内存信息等。CNend
    \attention \n
    \param[in] pu32AVFilterId av filter id .CNcomment:过滤器句柄 CNend
    \param[out] pstAvFilterInfo point to av filter info.CNcomment:获取到的通道信息 CNend
    \retval ::SUCCESS Success.CNcomment:成功。CNend
    \retval ::ERROR_NOT_INITED not inited.CNcomment:所在模块未初始化。CNend
    \retval ::ERROR_INVALID_PARAM invalid param, such as invalid hanlde.CNcomment:无效参数，如无效句柄。CNend
    \retval ::ERROR_DMX_OP_FAILED DMX operate failed.CNcomment:DMX设备操作错误。CNend
