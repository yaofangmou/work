...
{
#ifndef __DISP_PLATFORM_BOOT__
    DISP_INTF_OPERATION_S *pstDispHalFunc = HI_NULL;
    //memset(&pstDispHalFunc, 0x0, sizeof(pstDispHalFunc));

    (HI_VOID)DISP_HAL_GetOperation(&pstDispHalFunc);

    if ((HI_NULL == pstDispHalFunc->PF_DispMask)
        || (HI_NULL == pstDispHalFunc->PF_DispUnMask))
    {
        return;
    }
    ...
    if (bMask)
    {
        pstDispHalFunc->PF_DispMask(enDisp);
    }
    else
    {
        pstDispHalFunc->PF_DispUnMask(enDisp);
    }
#endif
    return;
}
                                                   ...
                                                   WIN_SHARE_UNIFIED_METADATA_OUT_S *pstMdInfoOut)
{
    ...
    switch(stMdInfoIn.enSrcFrameType)
    {
        ...
        case HI_DRV_VIDEO_FRAME_TYPE_JTP_SL_HDR:
            stMdInfoIn.stColorDescriptionInfo = pstFrame->stColourInfo;
            stMdInfoIn.u8TransferCharacteristics = pstFrame->unHDRInfo.stJTPSlHDRInfo.staticMetadata.u8TransferCharacteristics;
            stMdInfoIn.bMasteringAvailable = pstFrame->unHDRInfo.stJTPSlHDRInfo.staticMetadata.u8MasteringAvailable;
            stMdInfoIn.stMasteringInfo = pstFrame->unHDRInfo.stJTPSlHDRInfo.staticMetadata.stMasteringInfo;
            stMdInfoIn.bContentAvailable = pstFrame->unHDRInfo.stJTPSlHDRInfo.staticMetadata.u8ContentAvailable;
            stMdInfoIn.stContentInfo  = pstFrame->unHDRInfo.stJTPSlHDRInfo.staticMetadata.stContentInfo;
            return;
        default:
            break;
                                             ...
                                             HI_DRV_COLOR_SPACE_E enOutCs)
{
    ...
    pstDisp->pstIntfOpt->PF_WorkModeChangeProcess(bWorkModeChange,bCurSdrWorkMode,enOutCs);
#endif
    return;
                                       ...
                                       HI_DRV_DISP_PROCESS_MODE_E  *penDispProcessMode)
{
    ...
    *penDispProcessMode = ((HI_DRV_DISP_TYPE_HDR10 == pstDisp->stSetting.stDispHDRAttr.enDispType)
    ...
    XDR_Process_WorkModeChange(pstDisp, *penDispProcessMode, pstDisp->stDispInfo.enDispProcessMode, pstXdrPlayInfo->enColorSpace);
    if (HI_DRV_DISP_PROCESS_MODE_SDR != *penDispProcessMode)
    {
        //config gfx.
