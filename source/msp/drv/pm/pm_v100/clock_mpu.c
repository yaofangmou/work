...
static int mpu_clk_set_rate(struct clk *clk, unsigned int rate)
{
    ...
    if (i == MAX_FREQ_NUM)
    {
        PM_ERR_PrintInfo("not support:");
        PM_ERR_PrintU32(rate);
        return -1;
    }
...
static unsigned int find_clk_in_table(unsigned int rate)
{
    ...
    if (i >= MAX_FREQ_NUM)
    {
        PM_ERR_PrintInfo("cannot find this clk in freq table, return default");
        PM_ERR_PrintS32(rate);
