                                      ...
                                      HI_PQ_VDP_WBC_INFO_S *pstVdpLayerAlgInfor)
{
    ...
    pstVdpLayerAlgInfor->pstVdpWbcReg = (HI_VOID *)(pstWinInfo->stLayerAlgMmz.pu8StartVirAddr);
        ...
        VDP_LAYER_VID_INFO_S *pstVidInfo)
{
    ...
    if (pstVidInfo->bRegionEn == HI_TRUE)
    {
        ...
        pstVidInfo->stSrcInfo.bSecureEn = pstWinInfo->bSecure;
...
static HI_VOID ADAPTER_TransferWinInfoToHiHdrCfg(WIN_INFO_S *pstWinInfo, VDP_HISI_HDR_CFG_S *pstHdrCfg)
{
    ...
    pstHdrCfg->stVdpDispInfo.bInterlace  = pstWinInfo->pstDispInfo->bInterlace;
    pstHdrCfg->stVdpDispInfo.stFmtResolution.u32X  = pstWinInfo->pstDispInfo->stFmtResolution.s32X;
    pstHdrCfg->stVdpDispInfo.stFmtResolution.u32Y  = pstWinInfo->pstDispInfo->stFmtResolution.s32Y;
    pstHdrCfg->stVdpDispInfo.stFmtResolution.u32Wth  = pstWinInfo->pstDispInfo->stFmtResolution.s32Width;
    pstHdrCfg->stVdpDispInfo.stFmtResolution.u32Hgt  = pstWinInfo->pstDispInfo->stFmtResolution.s32Height;
    pstHdrCfg->stVdpDispInfo.stFmtResolution.u32SX = 0;
                                        ...
                                        HI_BOOL bDolbyPath)
{
    ...
    pstLayerInfo->stXdpLayerExtern.bHdcpDegradePQ= pstWinInfo->bHdcpDegradePQ;
    pstLayerInfo->stXdpSysCfg.bMask =  pstWinInfo->bMask;
    ...
    if (HI_TRUE == pstLayerInfo->stXdpSysCfg.bRegionEn)
    {
        ...
        pstLayerInfo->stXdpSysCfg.bSecureEn = pstWinInfo->bSecure;
        //===============================src process=============================
        //if (HI_NULL != pstWinInfo->pCurrentFrame)
        if (HI_FALSE == pstWinInfo->bRegionMute)
        {
        ...
        else
        {
            ...
            pstLayerInfo->stXdpSrcCfg.enInFmt = VDP_VID_IFMT_SP_420;
        }
    }
    return ;
}
                            ...
                            HI_BOOL bConfigWholeLayer)
{
    ...
    if (HI_NULL != pstWinInfo->pCurrentFrame)
    {
        ...
        stLayerInfoIn.pstSlHdrPsParam = &pstDispBufNode->stSlHdrPsParam;
    }
...
HI_PQ_GFXHDR_CFG g_stPqGfxHdrCfg = {0}; /* 728 Byte */
...
HI_VOID ADAPTER_SetGfxXdrMode(HI_DRV_DISP_OUT_TYPE_E enDispOutType, HI_DRV_COLOR_SPACE_E enOutColorSpace)
{
    ...
    memset(&g_stPqGfxHdrCfg, 0, sizeof(g_stPqGfxHdrCfg));
    ...
    DRV_PQ_GetGfxHdrCfg(&stGfxHdrInfo, &g_stPqGfxHdrCfg);
    enHalOutColorSpace = XDR_COLOR_SPACE_TRANSFER(enOutColorSpace);

    VDP_DRV_SetHisiGfxHdr(&g_stPqGfxHdrCfg, enHalOutColorSpace);
    return;
}
