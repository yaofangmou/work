...
static HI_S32 PQ_MNG_GZmeLoadCoefH(PQ_GZME_COEF_RATIO_E enCoefRatio, HI_U8 *pu8Addr)
{
    ...
    u32TableTmpSize = PQ_MNG_GZmeHTransCoefAlign(g_pPQGfxZmeReduceCoef[enCoefRatio][PQ_GZME_COEF_8T8P_LH], (HI_S16)(8 * 8), u16TableTmp);
...
{
    HI_S16 u16TableTmp[16 * 8];
    HI_U32 u32TableTmpSize;

    memset(u16TableTmp, 0, sizeof(u16TableTmp));

    u32TableTmpSize = PQ_MNG_GZmeVTransCoefAlign(g_pPQGfxZmeReduceCoef[enCoefRatio][PQ_GZME_COEF_4T16P_LV], (HI_S16)(16 * 8), u16TableTmp);
...
static PQ_GZME_VER_TAP_E pq_mng_GetGZmeTapV(HI_U32 u32ZmeFrmWOut, HI_U32 u32ZmeFrmWIn)
{
    ...
    if ((u32ZmeFrmWOut != u32ZmeFrmWIn) && (u32ZmeFrmWIn > 1920))
    {
...
static HI_VOID PQ_MNG_GZmeComnSet(HI_PQ_GFX_ZME_PARA_S *pstZmeDrvPara, PQ_GZME_RTL_PARA_S *pstZmeRtlPara)
{
    ...
    pstZmeRtlPara->bZmeFrmFmtIn = pstZmeDrvPara->bZmeFrmFmtIn;
    pstZmeRtlPara->bZmeFrmFmtOut = pstZmeDrvPara->bZmeFrmFmtOut;
...
static HI_S32 PQ_MNG_SetGfxZmeRegCfg(HI_PQ_GFX_LAYER_E enGfxZmeLayer, HI_BOOL bSlvGp, PQ_GZME_RTL_PARA_S *pstZmeRtlPara)
{
    ...
    if (PQ_GFX_GP0 == enGfxZmeLayer)
    {
        ...
        /*if (pstZmeRtlPara->bZmeEnH || pstZmeRtlPara->bZmeEnV)*/
        {
            ...
            PQ_HAL_SetGZmeHorOutWidth     (pstZmeRtlPara->u32ZmeWOut);
            ...
            PQ_HAL_SetGZmeVerOutWidth(pstZmeRtlPara->u32ZmeHOut);
            PQ_HAL_SetGZmeVerOutPro(HI_TRUE);//强制设置为输出逐行，后续有问题再改

            PQ_HAL_SetGZmeHfirOrder       (enGfxZmeLayer, 1);//pstZmeRtlPara->bZmeOrder);
    ...
    {
        PQ_HAL_SetWbcGpZmeEnable(enGfxZmeLayer, PQ_GZME_MODE_HOR, pstZmeRtlPara->bZmeEnH);
        PQ_HAL_SetWbcGpZmeEnable(enGfxZmeLayer, PQ_GZME_MODE_VER, pstZmeRtlPara->bZmeEnV);

        PQ_HAL_SetWbcGpZmeTabV(enGfxZmeLayer, pstZmeRtlPara->bZmeTapV);

        /*if (pstZmeRtlPara->bZmeEnH || pstZmeRtlPara->bZmeEnV)*/
        {
            /*set zme mode of horizontal luma and chroma*/
            PQ_HAL_SetWbcGpZmeFirEnable       (enGfxZmeLayer, PQ_GZME_MODE_HOR, pstZmeRtlPara->bZmeMdHLC);
            PQ_HAL_SetWbcGpZmeFirEnable       (enGfxZmeLayer, PQ_GZME_MODE_ALPHA, pstZmeRtlPara->bZmeMdHLC);
            /*set zme mode of horizontal alpha*/
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_ALPHA, pstZmeRtlPara->bZmeMedHA);
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_HORL, pstZmeRtlPara->bZmeMedHL);
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_HORC, pstZmeRtlPara->bZmeMedHC);
            PQ_HAL_SetWbcGpZmePhase           (enGfxZmeLayer, PQ_GZME_MODE_HORL, pstZmeRtlPara->s32ZmeOffsetHL);
            PQ_HAL_SetWbcGpZmePhase           (enGfxZmeLayer, PQ_GZME_MODE_HORC, pstZmeRtlPara->s32ZmeOffsetHC);
            PQ_HAL_SetWbcGpZmeHorRatio        (enGfxZmeLayer, pstZmeRtlPara->u32ZmeRatioHL);
            PQ_HAL_SetWbcGpZmeHorOutWidth     (pstZmeRtlPara->u32ZmeWOut);

            /*set zme mode of horizontal luma and chroma*/
            PQ_HAL_SetWbcGpZmeFirEnable       (enGfxZmeLayer, PQ_GZME_MODE_VER, pstZmeRtlPara->bZmeMdVLC);
            PQ_HAL_SetWbcGpZmeFirEnable       (enGfxZmeLayer, PQ_GZME_MODE_ALPHAV, pstZmeRtlPara->bZmeMdVLC);
            /*set zme mode of horizontal alpha*/
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_ALPHAV, pstZmeRtlPara->bZmeMedVA);
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_VERL, pstZmeRtlPara->bZmeMedVL);
            PQ_HAL_SetWbcGpZmeMidEnable       (enGfxZmeLayer, PQ_GZME_MODE_VERC, pstZmeRtlPara->bZmeMedVC);
            PQ_HAL_SetWbcGpZmePhase           (enGfxZmeLayer, PQ_GZME_MODE_VERL, pstZmeRtlPara->s32ZmeOffsetVBtm);
            PQ_HAL_SetWbcGpZmePhase           (enGfxZmeLayer, PQ_GZME_MODE_VERC, pstZmeRtlPara->s32ZmeOffsetVTop);
            PQ_HAL_SetWbcGpZmeVerRatio        (enGfxZmeLayer, pstZmeRtlPara->u32ZmeRatioVL);
            PQ_HAL_SetWbcGpZmeVerOutWidth(pstZmeRtlPara->u32ZmeHOut);
            PQ_HAL_SetWbcGpZmeVerOutPro(pstZmeRtlPara->bZmeFrmFmtOut);//强制设置为输出逐行，后续有问题再改

            PQ_HAL_SetWbcGpZmeHfirOrder       (enGfxZmeLayer, 0);//pstZmeRtlPara->bZmeOrder);

            PQ_HAL_SetWbcGpZmeCoefAddr        (enGfxZmeLayer, PQ_GP_PARA_ZME_HOR, pstZmeRtlPara->u32ZmeCoefAddrHL);
            PQ_HAL_SetWbcGpZmeParaUpd         (enGfxZmeLayer, PQ_GP_PARA_ZME_HOR);
        }
    }
    else
    {
...
HI_S32 PQ_MNG_SetGfxZme(HI_PQ_GFX_LAYER_E enGfxZmeLayer, HI_PQ_GFX_ZME_PARA_S *pstGfxZmePara)
{
    ...
    else if (PQ_GFX_GP1 == enGfxZmeLayer)
    {
        if (HI_FALSE == pstGfxZmePara->bZmeFrmFmtOut)
        {
            PQ_HAL_SetWbcGpZmeEnable(enGfxZmeLayer, PQ_GZME_MODE_VER, HI_TRUE);
        }
    }
...
#endif
{
    ...
    s32Ret = PQ_HAL_MMZ_AllocAndMap("GFX_ZmeCoef", HI_NULL, GZME_MMZ_SIZE, 16, &stMMZ);
    //s32Ret = PQ_HAL_MMZ_AllocAndMap(PQ_GFX_ZMECOEF_BUFF, HI_NULL, GZME_MMZ_SIZE, 16, &stMMZ);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PQ("PQ_GfxZmeCoef MMZ Malloc failed\n");
