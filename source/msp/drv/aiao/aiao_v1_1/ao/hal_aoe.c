...
HI_S32 HAL_AOE_AIP_Group_Pause(HI_U32 u32PauseMask)
{
    HI_U32 u32AipId = 0;
    HI_U32 u32CmdDone = 0;
    AOE_AIP_CHN_STATE_S* state[AOE_MAX_AIP_NUM] = {HI_NULL};

    u32CmdDone = iHAL_AOE_AIP_Group_SetCmd(AOE_AIP_CMD_PAUSE, u32PauseMask);

    for (u32AipId = 0; u32AipId < AOE_MAX_AIP_NUM; u32AipId++)
    {
        if ((1 << u32AipId) & u32PauseMask)
        {
            if ((1 << u32AipId) & u32CmdDone)
            {
                state[u32AipId] = (AOE_AIP_CHN_STATE_S*)g_AoeRm.pAip[u32AipId];
                state[u32AipId]->enCurnStatus = AOE_AIP_STATUS_PAUSE;
            }
            else
            {
                HI_ERR_AO("Err: AIP(%d) track pause failed!\n", u32AipId);
            }
        }
    }

    if (u32CmdDone == u32PauseMask)
    {
        return HI_SUCCESS;
    }
    else
