...
static HI_VOID ShowSourceInfo(HI_PROC_SHOW_BUFFER_S* pstBuf, HI_VOID* pPrivData)
{
    ...
    MS12_STREAM_PROC_S* pstStreamProc = &(pstRender->pstEngineManger->pstCodec->pstMs12Proc->stProc.stStreamProc[0]);
    ...
    if (HI_NULL != pstMSource)
    {
        HI_PROC_Printf(pstBuf, "%-20s | ", pstStreamProc[SOURCE_ID_MAIN].bMute == HI_TRUE ? "True" : "False");
    }
    if (HI_NULL != pstASource)
    {
        HI_PROC_Printf(pstBuf, "%-20s | ", pstStreamProc[SOURCE_ID_ASSOC].bMute == HI_TRUE ? "True" : "False");
    }
    ...
    if (HI_NULL != pstMSource)
    {
        snprintf(aszBuf, sizeof(aszBuf), "%s%d.%ddB", (0 == pstStreamProc[SOURCE_ID_MAIN].u32IntPrescale && pstStreamProc[SOURCE_ID_MAIN].s32DecPrescale < 0) ? "-" : "",
                 pstStreamProc[SOURCE_ID_MAIN].u32IntPrescale,
                (pstStreamProc[SOURCE_ID_MAIN].s32DecPrescale < 0) ? (-pstStreamProc[SOURCE_ID_MAIN].s32DecPrescale) : pstStreamProc[SOURCE_ID_MAIN].s32DecPrescale);
        HI_PROC_Printf(pstBuf, "%-20s | ", aszBuf);
    }
    if (HI_NULL != pstASource)
    {
        snprintf(aszBuf, sizeof(aszBuf), "%s%d.%ddB", (0 == pstStreamProc[SOURCE_ID_ASSOC].u32IntPrescale && pstStreamProc[SOURCE_ID_ASSOC].s32DecPrescale < 0) ? "-" : "",
                 pstStreamProc[SOURCE_ID_ASSOC].u32IntPrescale,
                (pstStreamProc[SOURCE_ID_ASSOC].s32DecPrescale < 0) ? (-pstStreamProc[SOURCE_ID_ASSOC].s32DecPrescale) : pstStreamProc[SOURCE_ID_ASSOC].s32DecPrescale);
        HI_PROC_Printf(pstBuf, "%-20s | ", aszBuf);
    }
#endif
...
HI_VOID Proc_Engine(ENGINE_MANAGER_S* pstEngine, STATISTICS_CMD_E enCmd)
{
    ...
    if (STATISTICS_STOP == enCmd)
    {
        ...
        {
            HI_WARN_AO("**********TimeOut ConsumeMs(%d)!!!!!!!\n", u32CostTime);
            pstEngine->u32RenderTimeOutCnt++;
        }
        HI_WARN_AO("ConsumeMs(%d)\n", u32CostTime);
    }
}
