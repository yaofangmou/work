...
static HI_S32 SYS_Ioctl(struct inode *pInode, struct file *pFile, HI_U32 cmd, HI_VOID *arg)
{
    ...
    switch (cmd)
    {
        ...
        case SYS_WRITE_REGISTER:
            ret = HI_DRV_SYS_WriteRegister((HI_SYS_IO_REG_S*)arg);
            break;

        case SYS_READ_REGISTER:
            ret = HI_DRV_SYS_ReadRegister((HI_SYS_IO_REG_S*)arg);
            break;
...
static HI_S32 SysProcShow(struct seq_file *s, HI_VOID *pArg)
{
    ...
    if(HI_CHIP_TYPE_HI3798M_H == ChipType)
    {
        PROC_PRINT(s, "CHIP_VERSION: Hi3798MV%xH\n", ChipVersion);
    }
    else
    {
