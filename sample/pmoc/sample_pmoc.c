...
HI_S32  main(HI_S32 argc,HI_CHAR *argv[])
{
    ...
    HI_UNF_PMOC_WAKEUP_MODE_S wakeUpAttr = {0};
    ...
    HI_BOOL bMemorySuspend = 0;
    HI_UNF_PDM_PMOC_PARAM_S stPmocParam = {0};

    if ((argc <= 1) || (argc > 14))
    {
        printf("Usage:  %s [TimerCount] [Display] [Running Count] [Scene] [IR Type] [Keyled Type] [Eth | USB | WiFi Type] [Reset] [WakeUp GPIO No.] [HDMI CEC] [TimeToDeepStandby] [PowerCtrl GPIO No.] [memory suspend]\n"
               ...
               "\tsuspend from memory when powerup: 0: Disable; 1: Enable. \n ",
                argv[0]);
        printf("Example: %s 0 2 1 0 0 2 0 0 0 0 0 0 0\n", argv[0]);
        return HI_FAILURE;
    }
    ...
    if (argc > 13)
    {
        bMemorySuspend = strtol(argv[13],NULL,0);
    }
    ...
    if (devType.irtype == HI_UNF_IR_CODE_RAW)
    {
        ...
        wakeup.u32IrPowerKey0[0] = 0x48140f00; //XMP-1 short-IR
        wakeup.u32IrPowerKey1[0] = 0x0;
        wakeup.u32IrPowerKey0[1] = 0x47180f00; //XMP-1 long-IR
        wakeup.u32IrPowerKey1[1] = 0x0;
        wakeup.u32IrPowerKey0[2] = 0x639cff00; //nec simple
    ...
    ret = HI_UNF_PDM_GetBaseParam(HI_UNF_PDM_BASEPARAM_PMOC, &stPmocParam);
    if (HI_SUCCESS != ret)
    {
        HI_INFO_PMOC("HI_UNF_PDM_GetBaseParam err, 0x%08x !\n", ret);
        HI_UNF_PMOC_DeInit();
        return ret;
    }
    ...
    if (0 != bMemorySuspend)
    {
        stPmocParam.u32Flag = bMemorySuspend;

        ret = HI_UNF_PDM_UpdateBaseParam(HI_UNF_PDM_BASEPARAM_PMOC, &stPmocParam);
        if (HI_SUCCESS != ret)
        {
            HI_INFO_PMOC("HI_UNF_PDM_GetBaseParam err, 0x%08x !\n", ret);
            HI_UNF_PMOC_DeInit();
    ...
    for (i = 0; i < g_RunningCount; i++)
    {
        ...
        HI_UNF_PMOC_GetWakeUpMode(&wakeUpAttr);
        if (actual == HI_UNF_PMOC_WKUP_IR)
        {
            HI_INFO_PMOC("wakeup key(Low): 0x%x, key(High):0x%x \n", (HI_U32)(wakeUpAttr.u64IRPowerKey)& 0xffffffff,
                          (HI_U32)(wakeUpAttr.u64IRPowerKey >> 32) & 0xffffffff);
        }

        if (actual == HI_UNF_PMOC_WKUP_GPIO)
        {
            HI_INFO_PMOC("wakeup GPIO NO. :0x%x \n", wakeUpAttr.u32GPIONo);
        }
