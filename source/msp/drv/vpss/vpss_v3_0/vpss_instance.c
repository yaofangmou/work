...
HI_S32 VPSS_INST_RevisePortCmpCfg(VPSS_INSTANCE_S *pstInstance)
{
    ...
    for (u32Count = 0; u32Count < DEF_HI_DRV_VPSS_PORT_MAX_NUMBER; u32Count++)
    {
        ...
        if (pstPort->s32PortId != VPSS_INVALID_HANDLE)
        {
            // only HD port support compression!
            if ((pstPort->u32cmpflag == 1) && (pstPort->stFrmInfo.stBufListCfg.eBufType == HI_DRV_VPSS_BUF_VPSS_ALLOC_MANAGE))
            {
...
HI_S32 VPSS_INST_CheckUndoImage(VPSS_INSTANCE_S *pstInstance)
{
    ...
    if (pstEntity->stStreamInfo.u32IsNewImage == HI_TRUE)
    {
        ...
        (HI_VOID)DRV_PQ_UpdateVpssPQ(pstInstance->ID, &stVpssPqInfo,(HI_VOID *)(&pstInstance->stPqRegData),(HI_VOID *)(&pstInstance->stPqWbcReg), &pstInstance->stPQModule);
                               ...
                               HI_RECT_S *pstOutCropRect)
{
             ...
             || pstInst->stPort[PortId].eAspMode == HI_DRV_ASP_RAT_MODE_PANANDSCAN)
    {
        ...
        if (pstInst->stPort[PortId].eAspMode == HI_DRV_ASP_RAT_MODE_LETTERBOX)
        {
            ...
            pstOutCropRect->s32X = (HI_U32)(pstOutCropRect->s32X) & VPSS_WIDTH_ALIGN;
            pstOutCropRect->s32Width = (HI_U32)(pstOutCropRect->s32Width) & VPSS_WIDTH_ALIGN;
            pstOutCropRect->s32Y = (HI_U32)(pstOutCropRect->s32Y) & VPSS_HEIGHT_ALIGN;
            pstOutCropRect->s32Height = (HI_U32)(pstOutCropRect->s32Height) & VPSS_HEIGHT_ALIGN;
#endif
...
HI_BOOL VPSS_INST_CheckPassThrough(VPSS_INSTANCE_S *pstInstance, HI_DRV_VIDEO_FRAME_S *pstInImage)
{
    ...
    for (u32Count = 0; u32Count < VPSS_PORT_MAX_NUMB; u32Count++)
    {
            ...
            && pstPort->s32PortId != HI_INVALID_HANDLE)
        {
            ...
            {
                if ((VPSS_BYPASS_AUTOBYVPSS == pstPort->bPassThrough)
                    || (VPSS_FORCE_BYPASS == pstPort->bPassThrough))
                {
...
HI_S32 VPSS_INST_SetBandPolicy(VPSS_INSTANCE_S *pstInstance)
{
    ...
    for (u32Count = 0; u32Count < VPSS_PORT_MAX_NUMB; u32Count++)
    {
        ...
        {
            u32PortCnt++;
            if (pstPort->bPassThrough == VPSS_FORCE_NOT_BYPASS)
            {
                u32NoByPassCnt++;
            }
        }
