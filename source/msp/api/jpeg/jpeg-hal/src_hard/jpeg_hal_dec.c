...
extern HI_VOID JPEG_HDEC_WhetherDebug(HI_U32* pLogLevel);
#endif
...
static inline HI_S32 JPEG_HDEC_Decompress(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    do
    {
        ...
        {
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "JPG_INTTYPE_CONTINUE = 1");
            HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "JPG_INTTYPE_ERROR = 4");
            HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, GetIntType.IntType);
            return HI_FAILURE;
        }
        else
        {
     ...
     if (diff_ms >= 3000)
     {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, diff_ms);
        HI_GFX_OutErrInfoMsg(HI_GFX_MSG_ERR_INFO, "hard decompress time out");
     }
#endif
 ...
 **BEG SYS FUNC
 **===========================================================================================**/
HI_VOID JPEG_HDEC_Memset(HI_VOID *pDst, HI_S32 InitValue, HI_U32 DstSize)
{
    if ((NULL != pDst) && (0 != DstSize)) { memset(pDst, InitValue, DstSize); }
    return;
}

HI_VOID JPEG_HDEC_Memcpy(HI_VOID *pDst, const HI_VOID *pSrc, HI_U32 SrcSize)
{
    if ((NULL != pDst) && (NULL != pSrc) && (0 != SrcSize)) { memcpy(pDst, pSrc, SrcSize); }
    return;
}

HI_VOID JPEG_HDEC_Snprintf(HI_CHAR *pStr, HI_U32 Size, const HI_CHAR *Format, ...)
{
    va_list args;
    HI_U32 MsgLen = 0;

    if ((NULL == pStr) || (0 == Size) || (NULL == Format))
    {
         return;
    }

    va_start(args, Format);
    MsgLen = vsnprintf(pStr, Size, Format, args);
    HI_UNUSED(MsgLen);
    va_end(args);
    pStr[Size - 1] = '\0';
