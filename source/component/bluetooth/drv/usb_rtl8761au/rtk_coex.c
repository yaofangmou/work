...
static int rtl_alloc_buff(rtk_usb_coexistence_info *coex)
{
...
static void rtl_free_buff(rtk_usb_coexistence_info *coex)
{
	struct rtl_hci_ev *ev;
	struct rtl_l2_buff *l2;
	unsigned long flags;
...
static struct rtl_hci_ev *rtl_ev_node_get(rtk_usb_coexistence_info *coex)
{
	struct rtl_hci_ev *ev;
	unsigned long flags;
...
static int rtl_ev_node_to_used(rtk_usb_coexistence_info *coex,
		struct rtl_hci_ev *ev)
{
	unsigned long flags;
...
static rtk_conn_prof *find_connection_by_handle(rtk_usb_coexistence_info * coex,
						uint16_t handle)
{
...
static void add_connection_to_hash(rtk_usb_coexistence_info * coex,
				   rtk_conn_prof * desc)
{
	struct list_head *head = &coex->conn_hash;
...
{
	RTKBT_DBG("delete profile for handle: %x, psm:%x, dcid:%x, scid:%x",
		  desc->handle, desc->psm, desc->dcid, desc->scid);
	if (desc) {
		list_del(&desc->list);
		kfree(desc);
		desc = NULL;
...
static rtk_prof_info *find_profile_by_handle_scid(rtk_usb_coexistence_info *
...
static rtk_prof_info *find_profile_by_handle_dcid(rtk_usb_coexistence_info *
						  coex, uint16_t handle,
						  uint16_t dcid)
{
	struct list_head *head = &coex->profile_list;
...
static rtk_prof_info *find_profile_by_handle_dcid_scid(rtk_usb_coexistence_info
						       * coex, uint16_t handle,
						       uint16_t dcid,
						       uint16_t scid)
{
				 ...
				 uint8_t * parameter)
{
	...
	uint8_t *p;
	struct sk_buff *skb;
	struct hci_dev *hdev = usb_coex_info.hdev;
...
static void rtk_notify_profileinfo_to_fw(void)
{
	...
	RTKBT_DBG("rtk_notify_profileinfo_to_fw, BufferSize is %x",
		  buffer_size);
	*p++ = handle_number;
	RTKBT_DBG("rtk_notify_profileinfo_to_fw, NumberOfHandles is %x",
		  handle_number);
	head = &usb_coex_info.conn_hash;
	list_for_each(iter, head) {
		...
		if (hci_conn && hci_conn->profile_bitmap) {
			...
			RTKBT_DBG("rtk_notify_profileinfo_to_fw, handle is %x",
				  hci_conn->handle);
			*p++ = hci_conn->profile_bitmap;
			RTKBT_DBG
			    ("rtk_notify_profileinfo_to_fw, profile_bitmap is %x",
	...
	*p++ = usb_coex_info.profile_status;
	RTKBT_DBG("rtk_notify_profileinfo_to_fw, profile_status is %x",
		  usb_coex_info.profile_status);
...
static void rtk_check_setup_timer(int8_t profile_index)
{
	if (profile_index == profile_a2dp) {
		...
		setup_timer(&(usb_coex_info.a2dp_count_timer),
			    count_a2dp_packet_timeout, 0);
		usb_coex_info.a2dp_count_timer.expires =
	...
	if (profile_index == profile_pan) {
		...
		setup_timer(&(usb_coex_info.pan_count_timer),
			    count_pan_packet_timeout, 0);
		usb_coex_info.pan_count_timer.expires =
		    jiffies + msecs_to_jiffies(1000);
	...
	if ((profile_index == profile_hogp) || (profile_index == profile_voice)) {
		if ((0 == usb_coex_info.profile_refcount[profile_hogp])
			...
			setup_timer(&(usb_coex_info.hogp_count_timer),
				    count_hogp_packet_timeout, 0);
			usb_coex_info.hogp_count_timer.expires =
			    jiffies + msecs_to_jiffies(1000);
...
static void update_profile_state(uint8_t profile_index, uint8_t is_busy)
{
	...
	if ((usb_coex_info.profile_bitmap & BIT(profile_index)) == 0) {
		RTKBT_ERR
		    ("update_profile_state: ERROR!!! profile(Index: %x) does not exist",
		     profile_index);
		return;
	}
	...
	if (need_update) {
		RTKBT_DBG
		    ("update_profile_state, usb_coex_info.profie_bitmap = %x",
		     usb_coex_info.profile_bitmap);
		RTKBT_DBG
		    ("update_profile_state, usb_coex_info.profile_status = %x",
		     usb_coex_info.profile_status);
				      ...
				      int8_t profile_index, uint8_t is_add)
{
	...
	RTKBT_DBG("update_profile_connection: is_add=%d, profile_index=%x",
	...
	if (is_add) {
		...
		usb_coex_info.profile_refcount[profile_index]--;
		RTKBT_DBG
		    ("for test: --, usb_coex_info.profile_refcount[%x] = %x",
		     profile_index,
		     usb_coex_info.profile_refcount[profile_index]);
		if (usb_coex_info.profile_refcount[profile_index] == 0) {
			need_update = TRUE;
	...
	RTKBT_DBG
	    ("update_profile_connection: usb_coex_info.profile_bitmap = %x",
	     usb_coex_info.profile_bitmap);
	for (kk = 0; kk < 8; kk++)
		RTKBT_DBG
		    ("update_profile_connection: usb_coex_info.profile_refcount[%d] = %d",
		     kk, usb_coex_info.profile_refcount[kk]);
...
static void update_hid_active_state(uint16_t handle, uint16_t interval)
{
	...
	rtk_conn_prof *phci_conn =
	    find_connection_by_handle(&usb_coex_info, handle);

	if (phci_conn == NULL)
		return;

	RTKBT_DBG("update_hid_active_state: handle = %x, interval = 0x%x",
		  handle, interval);
	if (((phci_conn->profile_bitmap) & (BIT(profile_hid))) == 0) {
		RTKBT_DBG
		    ("hid not connected in the handle, nothing to be down");
		return;
	}
				    ...
				    uint16_t scid, uint8_t direction)
{
	...
	if (direction)		//1: out
		prof_info =
		    find_profile_by_handle_scid(&usb_coex_info, handle, scid);
	else			// 0:in
		prof_info =
		    find_profile_by_handle_dcid(&usb_coex_info, handle, scid);

	if (prof_info) {
		RTKBT_DBG
		    ("handle_l2cap_con_req: this profile is already exist!!!");
		spin_unlock(&usb_coex_info.spin_lock_profile);
		return status;
	}
				       ...
				       uint16_t scid, uint8_t direction)
{
	...
	rtk_conn_prof *phci_conn = NULL;
	RTKBT_DBG
	    ("l2cap_discon_req, handle = %x, dcid = %x, scid = %x, direction = %x",
	     handle, dcid, scid, direction);

	spin_lock(&usb_coex_info.spin_lock_profile);
	if (!direction)		//0: in
		prof_info =
		    find_profile_by_handle_dcid_scid(&usb_coex_info, handle,
						     scid, dcid);
	else			//1: out
		prof_info =
		    find_profile_by_handle_dcid_scid(&usb_coex_info, handle,
			  ...
			  uint8_t direction, u8 *user_data)
{
	...
	if (0 == hci_conn->type) {
		if (!direction)	//0: in
			prof_info =
			    find_profile_by_handle_scid(&usb_coex_info, handle,
							scid);
		else		//1: out
			prof_info =
			    find_profile_by_handle_dcid(&usb_coex_info, handle,
							scid);
...
static void count_a2dp_packet_timeout(unsigned long data)
{
	RTKBT_DBG("count a2dp packet timeout, a2dp_packet_count = %d",
		  usb_coex_info.a2dp_packet_count);
	...
	usb_coex_info.a2dp_packet_count = 0;
	mod_timer(&(usb_coex_info.a2dp_count_timer),
		  jiffies + msecs_to_jiffies(1000));
}

static void count_pan_packet_timeout(unsigned long data)
{
	RTKBT_DBG("count pan packet timeout, pan_packet_count = %d",
		  usb_coex_info.pan_packet_count);
	...
	usb_coex_info.pan_packet_count = 0;
	mod_timer(&(usb_coex_info.pan_count_timer),
		  jiffies + msecs_to_jiffies(1000));
}

static void count_hogp_packet_timeout(unsigned long data)
{
	RTKBT_DBG("count hogp packet timeout, hogp_packet_count = %d",
		  usb_coex_info.hogp_packet_count);
	...
	RTKBT_DBG("count hogp packet timeout, voice_packet_count = %d",
		  usb_coex_info.voice_packet_count);
	...
	usb_coex_info.voice_packet_count = 0;
	mod_timer(&(usb_coex_info.hogp_count_timer),
		  jiffies + msecs_to_jiffies(1000));
}
...
static void create_udpsocket(void)
{
	...
	err =
	    usb_coex_info.udpsock->ops->bind(usb_coex_info.udpsock,
					     (struct sockaddr *)&usb_coex_info.
					     addr, sizeof(struct sockaddr));
	if (err < 0) {
		sock_release(usb_coex_info.udpsock);
		RTKBT_DBG("bind udp socket error, err = %d", err);
		return;
	}
...
static void rtk_notify_extension_version_to_wifi(void)
{
	...
	*p++ = para_length;
	...
	if (udpsocket_send(p_buf, para_length + HCI_CMD_PREAMBLE_SIZE) < 0)
		RTKBT_ERR
		    ("rtk_notify_extension_version_to_wifi: udpsocket send error");
}
...
static void rtk_notify_btpatch_version_to_wifi(void)
{
	...
	*p++ = para_length;
	...
	RTKBT_DBG
	    ("btpatch_version, length is 0x%x, hci_reversion is 0x%x, lmp_subversion is %x",
	     para_length, usb_coex_info.hci_reversion,
	     usb_coex_info.lmp_subversion);

	if (udpsocket_send(p_buf, para_length + HCI_CMD_PREAMBLE_SIZE) < 0)
		RTKBT_ERR
		    ("rtk_notify_btpatch_version_to_wifi: udpsocket send error");
}
...
static void rtk_notify_afhmap_to_wifi(void)
{
	...
	*p++ = 10;
	...
	RTKBT_DBG("afhmap, piconet_id is 0x%x, map type is 0x%x",
		  usb_coex_info.piconet_id, usb_coex_info.mode);
	for (kk = 0; kk < 10; kk++)
		RTKBT_DBG("afhmap data[%d] is 0x%x", kk,
			  usb_coex_info.afh_map[kk]);
					   ...
					   uint8_t * append_data)
{
	...
	*p++ = append_data_length;
	...
	RTKBT_DBG("btoperation, opration is 0x%x, append_data_length is 0x%x",
	...
	if (udpsocket_send(p_buf, para_length + HCI_CMD_PREAMBLE_SIZE) < 0)
		RTKBT_ERR
		    ("rtk_notify_btoperation_to_wifi: udpsocket send error");
}
				    ...
				    uint8_t *report_info)
{
	...
	*p++ = length;
	...
	RTKBT_DBG("para length %2x, polling_enable %u, poiiling_interval %u",
	     para_length, usb_coex_info.polling_enable,
	     usb_coex_info.polling_interval);
...
static void rtk_handle_cmd_complete_evt(u8 total_len, u8 * p)
{
	...
	if (opcode == HCI_OP_PERIODIC_INQ) {
		if (*p++ && usb_coex_info.isinquirying) {
			usb_coex_info.isinquirying = 0;
			RTKBT_DBG
			    ("HCI_PERIODIC_INQUIRY starte error, notify wifi inquiry stop");
			rtk_notify_btoperation_to_wifi(BT_OPCODE_INQUIRY_END, 0,
						       NULL);
		}
	}
	...
	if (opcode == HCI_OP_READ_LOCAL_VERSION) {
		if (!(*p++)) {
			...
			RTKBT_DBG("coex_info.hci_reversion = %x",
				  usb_coex_info.hci_reversion);
			RTKBT_DBG("coex_info.lmp_subversion = %x",
				  usb_coex_info.lmp_subversion);
		}
	}
...
static void rtk_handle_cmd_status_evt(u8 * p)
{
	...
	if ((opcode == HCI_OP_INQUIRY) && (status)) {
		if (usb_coex_info.isinquirying) {
			usb_coex_info.isinquirying = 0;
			RTKBT_DBG
			    ("inquiry start error, notify wifi inquiry stop");
			rtk_notify_btoperation_to_wifi(BT_OPCODE_INQUIRY_END, 0,
						       NULL);
		}
	}
...
static void rtk_handle_connection_complete_evt(u8 * p)
{
	...
	if (status == 0) {
		...
		if (hci_conn == NULL) {
			...
			if (hci_conn) {
				add_connection_to_hash(&usb_coex_info,
						       hci_conn);
			...
			RTKBT_DBG
			    ("hci connection handle(0x%x) has already exist!",
			     handle);
			hci_conn->profile_bitmap = 0;
			memset(hci_conn->profile_refcount, 0, 8);
			if ((0 == link_type) || (2 == link_type)) {	//sco or esco
...
{
	struct rtl_coex_struct *coex = &usb_coex_info;
	rtk_prof_info *prof_info = NULL;
	struct list_head *iter = NULL, *temp = NULL;
...
static void rtk_handle_disconnect_complete_evt(u8 * p)
{
	...
	if (status == 0) {
		...
		if (hci_conn) {
			...
			RTKBT_ERR("hci connection handle(0x%x) not found",
				  handle);
		}
	}
}
...
void rtl_process_l2_sig(struct rtl_l2_buff *l2)
{
	...
	case L2CAP_CONN_REQ:
		...
		RTKBT_DBG("%s l2cap conn req, hndl %x, PSM %x, scid %x",
			  l2_dir_str[l2->out], handle, psm, scid);
	...
	case L2CAP_CONN_RSP:
		...
		pp += 2;
		RTKBT_DBG("%s l2cap conn rsp, hndl %x, dcid %x, "
			  "scid %x, result %x", l2_dir_str[l2->out],
	...
	case L2CAP_DISCONN_REQ:
		...
		STREAM_TO_UINT16(dcid, pp);
		STREAM_TO_UINT16(scid, pp);
		RTKBT_DBG("%s l2cap disconn req, hndl %x, dcid %x, "
			  "scid %x", l2_dir_str[l2->out], handle, dcid, scid);
...
void rtl_usb_l2_data(u8 *pp, u16 len, int dir)
{
	u8 code;
	u8 flag;
	...
	if (channel_id == 0x0001) {
		...
		case L2CAP_DISCONN_REQ:
			RTKBT_DBG("l2cap op %u, len %u, out %d", code, len,
				  dir);
			l2 = rtl_l2_node_get(&usb_coex_info);
			if (l2) {
				u16 n;
				n = min_t(uint, len, L2_MAX_SUBSEC_LEN);
				memcpy(l2->data, hd, n);
				l2->out = dir;
				rtl_l2_node_to_used(&usb_coex_info, l2);
				queue_delayed_work(usb_coex_info.fw_wq,
						&usb_coex_info.l2_work, 0);
			} else
...
void rtk_usb_parse_event(struct urb *urb)
{
	...
	ev = rtl_ev_node_get(&usb_coex_info);
	if (!ev) {
	...
	rtl_ev_node_to_used(&usb_coex_info, ev);
...
{
	rtl_usb_l2_data((u8 *)skb->data, skb->len, 1);
...
{
	rtl_usb_l2_data(urb->transfer_buffer, urb->actual_length, 0);
	//u16 handle, total_len, pdu_len, channel_ID, command_len, psm, scid,
	//    dcid, result, status;
	//u8 flag, code, identifier;
...
static void polling_bt_info(unsigned long data)
{
	...
	mod_timer(&(usb_coex_info.polling_timer),
		  jiffies +
		  msecs_to_jiffies(1000 * usb_coex_info.polling_interval));
}
...
static void rtk_handle_bt_info_control(uint8_t *p)
{
	...
	RTKBT_DBG("coex: original polling_enable %u",
		  usb_coex_info.polling_enable);

	if (ctl->polling_enable && !usb_coex_info.polling_enable) {
		/* setup polling timer for getting bt info from firmware */
		setup_timer(&(usb_coex_info.polling_timer), polling_bt_info, 0);
		usb_coex_info.polling_timer.expires =
	...
	if (usb_coex_info.autoreport != ctl->autoreport_enable) {
	...
	usb_coex_info.polling_enable = ctl->polling_enable;
	usb_coex_info.polling_interval = ctl->polling_time;
	usb_coex_info.autoreport = ctl->autoreport_enable;
...
static void rtk_handle_bt_coex_control(uint8_t * p)
{
	...
	opcode = *p++;
	RTKBT_DBG("receive bt coex control event from wifi, opration is 0x%x",
		  opcode);
...
static void rtk_handle_event_from_wifi(uint8_t * msg)
{
	...
	if (event_code == 0xFE) {
		...
		case RTK_HS_EXTENSION_EVENT_WIFI_SCAN:
			operation = *p;
			RTKBT_DBG
			    ("receive wifi scan notify evnet from wifi, operation is 0x%x",
			     operation);
			break;
	...
	if (event_code == 0x0E) {
		...
		op_status = *p;
		RTKBT_DBG
		    ("receive command complete event from wifi, op code is 0x%x, status is 0x%x",
...
{
	if (test_and_set_bit(RTL_COEX_RUNNING, &usb_coex_info.flags)) {
	...
	INIT_DELAYED_WORK(&usb_coex_info.fw_work, (void *)rtl_ev_work);
	INIT_DELAYED_WORK(&usb_coex_info.sock_work,
			  (void *)udpsocket_recv_data);
	INIT_DELAYED_WORK(&usb_coex_info.l2_work, (void *)rtl_l2_work);
...
void rtk_usb_coex_close(void)
{
	...
	if (!test_and_clear_bit(RTL_COEX_RUNNING, &usb_coex_info.flags)) {
	...
	cancel_delayed_work(&usb_coex_info.l2_work);
...
void rtk_usb_coex_init(void)
{
	...
	rtl_alloc_buff(&usb_coex_info);
	spin_lock_init(&usb_coex_info.rxlock);
}
