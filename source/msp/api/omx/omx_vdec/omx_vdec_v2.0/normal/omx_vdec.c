    ...
    OMX_IN OMX_U8 *p_handle)
{
    ...
    *omx_bufhdr = pomx_buf;
    ...
    DEBUG_PRINT_ALWS("[UNB] Use %s buffer %d success: phyaddr = %x, size = %d\n",
...
static void omx_setprop(void)
{
    ...
    if (property_get("vendor.app.compatibility.enhance", value, NULL)
        && (!strcasecmp("true", value)))
    {
    ...
    if (property_get("ro.config.hisi.lowram", value, NULL)
            && (!strcasecmp("true", value)))
    {
        g_LowRam = OMX_TRUE;
    }

    DEBUG_PRINT_ALWS("setprop logcat:%d test:%d render:%d lowram %d\n", g_EnableLogcatV, g_AndroidTest, g_NotRender, g_LowRam);
...
static OMX_S32 get_buffer_phyaddr(OMX_U32 port, OMXVDEC_DRV_CFG *drv_cfg, OMX_U8 *buffer, HI_U32 *phyaddr, eBUFFER_TYPE *buffer_type)
{
    ...
    {
        if (drv_cfg->bIndirectInput && (port == INPUT_PORT_INDEX))
        {
            if (isOnLive)
            {
...
#ifdef HI_OMX_TEE_SUPPORT
    if (pcom_priv->drv_ctx.drv_cfg.bIndirectInput && (port == INPUT_PORT_INDEX))
    {
        buffer_type = OMX_USE_OTHER;
    ...
    OMX_IN OMX_U32 bytes)
{
    ...
    *omx_bufhdr = pomx_buf;
    ...
    DEBUG_PRINT_ALWS("[AB] Alloc %s buffer %d success: phyaddr = 0x%x, useraddr = 0x%llx, size = %d, pomx_buf=%p\n",
    ...
    OMX_IN OMX_BUFFERHEADERTYPE *omx_bufhdr)
{
    ...
    if (OMX_ALLOCATE_USR == puser_buf->buffer_type || OMX_ALLOCATE_SECURE == puser_buf->buffer_type)
    {
        ...
        {
            (HI_VOID)HI_MMZ_Unmap(puser_buf->private_phyaddr);
        }
    ...
    DEBUG_PRINT_ALWS("[FB] Free %s buffer %d success: phyaddr = 0x%x, useraddr = 0x%llx, size = %d, omx_bufhdr=%p\n",
            (port == INPUT_PORT_INDEX) ? "in" : "out", (int)i,
...
static OMX_ERRORTYPE update_port_definitions(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_BOOL update_input_size)
{
    OMX_PORT_PRIVATE *port_priv = NULL;
    ...
    DEBUG_PRINT_ALWS("%s: max width * max height: %ld*%ld, buffer size: %ld\n", __func__, \
            max_frame_width, max_frame_height, port_priv->port_pro.buffer_size);
...
static OMX_ERRORTYPE update_picture_info(
        ...
        OMX_U32 width, OMX_U32 height, OMX_U32 stride, OMX_BOOL UpdataCrop)
{
    OMX_PORT_PRIVATE *port_priv = NULL;
    ...
    pcomp_priv->pic_info.frame_width = MAX(pcomp_priv->is_adaptive ? pcomp_priv->pic_info.max_frame_width : 0,
                                           width);
    pcomp_priv->pic_info.frame_height = MAX(pcomp_priv->is_adaptive ? pcomp_priv->pic_info.max_frame_height : 0,
                                           height);

    if (pcomp_priv->is_adaptive)
    {
        pcomp_priv->pic_info.max_frame_width = MAX(pcomp_priv->pic_info.max_frame_width, width);
        pcomp_priv->pic_info.max_frame_height = MAX(pcomp_priv->pic_info.max_frame_height, height);
        pcomp_priv->drv_ctx.drv_cfg.max_width = pcomp_priv->pic_info.max_frame_width;
        pcomp_priv->drv_ctx.drv_cfg.max_height = pcomp_priv->pic_info.max_frame_height;
    }

    if (pcomp_priv->m_output_use_native_buf)
    {
        if ((pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == OMX_HEVC)
           || (pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == OMX_VP9)
           || (pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd == OMX_AVS2))
        {
    ...
    pcomp_priv->pic_info.scan_lines = pcomp_priv->pic_info.frame_height;

    drv_cfg             = &(pcomp_priv->drv_ctx.drv_cfg);
    drv_cfg->cfg_width  = width;
    drv_cfg->cfg_height = height;
    ...
    DEBUG_PRINT_ALWS("%s: adaptive:%d max w:%ld h:%ld w:%ld h:%ld\n", __func__, pcomp_priv->is_adaptive,
        pcomp_priv->pic_info.max_frame_width, pcomp_priv->pic_info.max_frame_height, width, height);
...
static void message_resp_output_done(OMX_COMPONENT_PRIVATE *pcom_priv, OMXVDEC_MSG_INFO *vdec_msg)
{
    ...
    {
        DEBUG_PRINT_ALWS("[MP] report EOS");
...
static void message_resp_img_size_change(OMX_COMPONENT_PRIVATE *pcom_priv, OMXVDEC_MSG_INFO *vdec_msg)
{
    ...
    if (pcom_priv->drv_ctx.drv_cfg.TunnelModeEnable == 1)
    {
        ...
        update_picture_info(pcom_priv, width, height, stride, OMX_TRUE);
        post_event(pcom_priv, 0, 0, OMX_GENERATE_IMAGE_SIZE_CHANGE);
    }
    else
    {
        if (width != pcom_priv->pic_info.crop_width || height != pcom_priv->pic_info.crop_height || stride != pcom_priv->pic_info.stride)
        {
            DEBUG_PRINT_ALWS("image size changed: old_w: %ld, old_h:%ld, old stride:%ld new_w:%ld, new_h:%ld new_stide:%ld\n",
                    pcom_priv->pic_info.crop_width, pcom_priv->pic_info.crop_height, pcom_priv->pic_info.stride,
                    width, height, stride);

            if (pcom_priv->is_adaptive == OMX_FALSE)
            {
                pcom_priv->need_realloc = OMX_TRUE;
            }
            else if (width > pcom_priv->pic_info.max_frame_width || height > pcom_priv->pic_info.max_frame_height)
            {
                pcom_priv->need_realloc = OMX_TRUE;
                pcom_priv->pic_info.max_frame_width  = width;
                pcom_priv->pic_info.max_frame_height = height;
                pcom_priv->drv_ctx.drv_cfg.max_width = width;
                pcom_priv->drv_ctx.drv_cfg.max_height = width;
            }
            DEBUG_PRINT_ALWS("%s adaptive:%d need_realloc:%d max_w:%ld max_h:%ld w:%ld h:%ld\n", __func__, pcom_priv->is_adaptive,
                pcom_priv->need_realloc, pcom_priv->pic_info.max_frame_width, pcom_priv->pic_info.max_frame_height, width, height);
    ...
    OMX_U32 param2)
{
    ...
    if (pcom_priv->need_realloc == OMX_TRUE)
    {
        pcom_priv->need_realloc = OMX_FALSE;
...
static void config_compress_format(OMX_COMPONENT_PRIVATE *pcomp_priv, OMX_U32 codec_index)
{
    ...
    if ((pcomp_priv->m_dec_fmt != OMX_VIDEO_CodingRV)
        && (pcomp_priv->m_dec_fmt != OMX_VIDEO_CodingVP6))
    {
        pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd = codec_trans_list[codec_index].codec_type;
         //fix vts error
        if ((codec_trans_list[codec_index].codec_type == OMX_Dolby_Vision)
         && (pcomp_priv->drv_ctx.drv_cfg.android_test == HI_TRUE))
        {
            pcomp_priv->drv_ctx.drv_cfg.chan_cfg.eVidStd = (OMX_S32)OMX_HEVC;
        }
    }
}
...
static OMX_ERRORTYPE get_parameter_IndexParamPortDefinition(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    if (portDefn->nPortIndex == INPUT_PORT_INDEX)
    {
        ...
        if (pcomp_priv->drv_ctx.drv_cfg.bIndirectInput)
        {
            port_priv->port_pro.max_count   = DEF_MAX_IN_BUF_CNT_INDIRECT;
            port_priv->port_pro.buffer_size = DEFAULT_IN_BUF_SIZE_INDIRECT;
        }
        else if (pcomp_priv->drv_ctx.drv_cfg.bIsOnLive)
        {
        ...
        portDefn->format.video.nFrameHeight = pcomp_priv->pic_info.inport_frame_height;
        portDefn->format.video.nFrameWidth = pcomp_priv->pic_info.inport_frame_width;
    }
    ...
    DEBUG_PRINT_TRACE("%s:%d port:%d w:%d h:%d\n", __func__, __LINE__, portDefn->nPortIndex, portDefn->format.video.nFrameWidth,
...
static OMX_ERRORTYPE set_parameter_IndexParamPortDefinition(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    DEBUG_PRINT_TRACE("set_parameter: port:%d set w:%d info w:%d\n", portDefn->nPortIndex,
        portDefn->format.video.nFrameWidth, pcomp_priv->pic_info.frame_width);
        ...
        || portDefn->format.video.nFrameHeight != pcomp_priv->pic_info.frame_height)
    {
        if (portDefn->nPortIndex == OUTPUT_PORT_INDEX)
        {
            ret = update_picture_info(pcomp_priv, portDefn->format.video.nFrameWidth,
                                      portDefn->format.video.nFrameHeight, pcomp_priv->pic_info.stride, OMX_TRUE);
            if (ret != OMX_ErrorNone)
            {
        ...
        {
            pcomp_priv->pic_info.inport_frame_width = portDefn->format.video.nFrameWidth;
            pcomp_priv->pic_info.inport_frame_height = portDefn->format.video.nFrameHeight;
        }

        update_port_definitions(pcomp_priv, OMX_TRUE);
    }
...
static OMX_ERRORTYPE set_parameter_HISIIndexSetIndirectInputBuffer(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    OMX_COMPONENT_PRIVATE *pcomp_priv = (OMX_COMPONENT_PRIVATE *)(pcomp->pComponentPrivate);
    OMX_BOOL bIndirect = *((OMX_BOOL*)(param_data));
    DEBUG_PRINT_ERROR("Set Indirect Flag: %d\n", bIndirect);
    pcomp_priv->drv_ctx.drv_cfg.bIndirectInput = bIndirect;
    return OMX_ErrorNone;
}
...
static OMX_ERRORTYPE set_parameter_HISIIndexParamVideoAdaptivePlaybackMode(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    pcomp_priv->drv_ctx.drv_cfg.is_adaptive = pParams->bEnable;
    ...
    else
    {
        ...
        pcomp_priv->drv_ctx.drv_cfg.max_width = pParams->nMaxFrameWidth;
        pcomp_priv->drv_ctx.drv_cfg.max_height = pParams->nMaxFrameHeight;
    }

    if (pParams->nMaxFrameWidth <= DEFAULT_FRAME_WIDTH && pParams->nMaxFrameHeight <= DEFAULT_FRAME_HEIGHT)
    {
        pcomp_priv->drv_ctx.drv_cfg.force_bypass = OMX_FALSE;
    }
    else
    {
        pcomp_priv->drv_ctx.drv_cfg.force_bypass = OMX_TRUE;
    }
    DEBUG_PRINT_ALWS("%s:%d adaptive:%d max W:%d H:%d\n", __func__, __LINE__, pcomp_priv->is_adaptive,
...
static OMX_ERRORTYPE set_parameter_HISIIndexSetCallerName(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    static const OMX_U8 *xts_process[] = { "cts.media",
                                           ...
                                           "youtube.gts",
                                           "exoplayer.gts" };
    static const OMX_U8 *apk_process[] = { "m.netflix.ninja",
                                           "roid.youtube.tv",
                                           "android.videos" };
    OMX_S32 i = 0;
    OMX_ERRORTYPE ret = OMX_ErrorNone;
    ...
    pcomp_priv->drv_ctx.drv_cfg.bProDetectOff = OMX_FALSE;

    for (i = 0; i < NELEM(apk_process); i++)
    {
        if (strstr(pCallerName->strCallerName, apk_process[i]))
        {
            DEBUG_PRINT_ERROR("Set Not Progress Dectect\n");
            pcomp_priv->drv_ctx.drv_cfg.bProDetectOff = OMX_TRUE;
            break;
        }
    }
...
static OMX_ERRORTYPE get_parameter_HisiIndexChannelAttributes(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    OMX_CHAN_CFG_S *pchan_cfg = &(pcomp_priv->drv_ctx.drv_cfg.chan_cfg);
...
static OMX_ERRORTYPE set_parameter_HisiIndexChannelAttributes(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    pchan_cfg->s32DecMode        = chan_attr->nDecodeMode;
...
static OMX_ERRORTYPE set_parameter_IndexParamVideoRv(OMX_COMPONENTTYPE *pcomp, OMX_PTR param_data)
{
    ...
    {
        pchan_cfg->eVidStd = OMX_REAL8;
    }
    else if (pVideoRv->eFormat == OMX_VIDEO_RVFormat9)
    {
        pchan_cfg->eVidStd = OMX_REAL9;
    }
    else
    {
...
static const PARAMETER_HANDLER g_ParamerHandlerTable[] =
{
    ...
    {
        OMX_HisiIndexIndirectInputBuffer,
        NULL,
        set_parameter_HISIIndexSetIndirectInputBuffer,
    },
    ...
    OMX_OUT OMX_INDEXTYPE *pindex_type)
{
    ...
    else if (!strncmp(param_name, "OMX.Hisi.Param.Index.IndirectInputBuffer",\
                MAX(param_length, strlen("OMX.Hisi.Param.Index.IndirectInputBuffer"))))
    {
        *pindex_type = (OMX_INDEXTYPE)OMX_HisiIndexIndirectInputBuffer;
    }
    ...
    OMX_IN OMX_U32 bytes)
{
    ...
    DEBUG_PRINT_TRACE("%s:%d port:%d enter\n", __func__, __LINE__, port);
...
OMX_ERRORTYPE component_init(OMX_HANDLETYPE phandle, OMX_STRING comp_name)
{
    ...
    pcom_priv->pic_info.crop_width      = DEFAULT_FRAME_WIDTH;
    pcom_priv->pic_info.crop_height     = DEFAULT_FRAME_HEIGHT;
    pcom_priv->pic_info.inport_frame_width  = DEFAULT_FRAME_WIDTH;
    pcom_priv->pic_info.inport_frame_height = DEFAULT_FRAME_HEIGHT;
