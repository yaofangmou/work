...
{
    if (CBB_VID_IFMT_SP_422 == pstWbcCfgBasePara->eSrcPixelFmt )
    {
        puWbcZmeIn->stWbcStrategy.stLayerInfo[0].u32FmtIn = 0;
    }
    else if (CBB_VID_IFMT_SP_444 == pstWbcCfgBasePara->eSrcPixelFmt )
    {
        puWbcZmeIn->stWbcStrategy.stLayerInfo[0].u32FmtIn = 2;
    }
    else
    {
...
VDP_LAYER_WBC_INFO_S g_stWbcInfo = {0};
...
HI_S32 CBB_Video_SetWbcParam(CBB_WBC_CFG_INFO_S *pLayerCfgBasePara, CBB_CFG_EXT_S *pVDPCfgExtPara)
{
    ...
    if (pLayerCfgBasePara->enWBC == DRV_VDP_WBC_WIN)
    {
        if (pLayerCfgBasePara->bWbcEnable == HI_TRUE)
        {
            ...
            g_stWbcInfo.bZmeFirHL   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeFirHL;
            g_stWbcInfo.bZmeFirHC   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeFirHC;

            g_stWbcInfo.bZmeFirVL   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeFirVL;
            g_stWbcInfo.bZmeFirVC   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeFirVC;

            g_stWbcInfo.bZmeMedH   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeMedH;
            g_stWbcInfo.bZmeMedV   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeMedV;
            g_stWbcInfo.bZmeDefault   = unionZmeOut.stStrategyWbcVp.bZmeDefault;
#else

            g_stWbcInfo.bZmeFirHL   = HI_FALSE;
            g_stWbcInfo.bZmeFirHC   = HI_FALSE;

            g_stWbcInfo.bZmeFirVL   = HI_FALSE;
            g_stWbcInfo.bZmeFirVC   = HI_FALSE;

            g_stWbcInfo.bZmeMedH   = HI_FALSE;
            g_stWbcInfo.bZmeMedV   = HI_FALSE;
            g_stWbcInfo.bZmeDefault   = HI_FALSE;
#endif


            g_stWbcInfo.enWbcVpConnPoint   = (unionZmeOut.stStrategyWbcVp.enVpPoint == HI_PQ_WBC_VP_POINT_VP) ? VDP_LAYER_CONN_VPACSC : VDP_LAYER_CONN_VIDBZME;
            g_stWbcInfo.enInFmt =  (unionZmeOut.stStrategyWbcVp.stZmeFmt.u32PixInFmt == 0) ? VDP_VID_IFMT_SP_422 : VDP_VID_IFMT_SP_420;
            g_stWbcInfo.bP2iEn   = unionZmeOut.stStrategyWbcVp.bOpenP2I;

            g_stWbcInfo.stInRect.u32Wth = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32FrmInWidth;
            g_stWbcInfo.stInRect.u32Hgt = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32FrmInHeight;

            g_stWbcInfo.stOutRect.u32Wth = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32FrmOutWidth;
            g_stWbcInfo.stOutRect.u32Hgt = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32FrmOutHeight;
            g_stWbcInfo.stOutRect.u32Hgt = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32FrmOutHeight;
            g_stWbcInfo.s32ZmeOffsetHL = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetHL;
            g_stWbcInfo.s32ZmeOffsetHC = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetHC;
            g_stWbcInfo.s32ZmeOffsetVL = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetVL;
            g_stWbcInfo.s32ZmeOffsetVC = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetVC;
            g_stWbcInfo.s32ZmeOffsetVLBtm = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetVLBtm;
            g_stWbcInfo.s32ZmeOffsetVCBtm = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeOffset.s32ZmeOffsetVCBtm;
#if 0
            ...
            g_stWbcInfo.enOutFmt  = (unionZmeOut.stStrategyWbcVp.stZmeFmt.u32PixOutFmt == 0) ? VDP_WBC_OFMT_SP422 : VDP_WBC_OFMT_SP420;
            g_stWbcInfo.enWbcVpConnPoint = (unionZmeOut.stStrategyWbcVp.enVpPoint == HI_PQ_WBC_VP_POINT_VP) ? VDP_LAYER_CONN_VPACSC : VDP_LAYER_CONN_VIDBZME;


            g_stWbcInfo.stAddr[VDP_ADDR_WBCVP0].u32LumStr = pLayerCfgBasePara->stAddr.u32Stride_Y;
            g_stWbcInfo.stAddr[VDP_ADDR_WBCVP0].u32ChmStr = pLayerCfgBasePara->stAddr.u32Stride_C;

            g_stWbcInfo.stAddr[VDP_ADDR_WBCVP0].u32LumAddr = pLayerCfgBasePara->stAddr.u32PhyAddr_Y;
            g_stWbcInfo.stAddr[VDP_ADDR_WBCVP0].u32ChmAddr = pLayerCfgBasePara->stAddr.u32PhyAddr_C;

            switch (pLayerCfgBasePara->eFieldMode)
            {
                case CBB_FIELD_TOP:
                    g_stWbcInfo.u32OutIop = VDP_RMODE_TOP;
                    break;
                case CBB_FIELD_BOTTOM:
                    g_stWbcInfo.u32OutIop = VDP_RMODE_BOTTOM;
                    break;
                case CBB_FIELD_ALL:
                    g_stWbcInfo.u32OutIop = VDP_RMODE_PROGRESSIVE;
                    break;
                default:
                    break;
            }

            g_stWbcInfo.bSmmu       = pLayerCfgBasePara->bSmmu;
            g_stWbcInfo.bSecure     = pLayerCfgBasePara->bSecure;
        }


        g_stWbcInfo.bWbcLayerEn = pLayerCfgBasePara->bWbcEnable ;
        g_stWbcInfo.enDispMode  = pLayerCfgBasePara->enDispMode;

        VDP_DRV_SetWbcLayer(VDP_LAYER_WBC_VP0, &g_stWbcInfo);
    }
    ...
    if (pLayerCfgBasePara->enWBC  == DRV_VDP_WBC_WIN)
    {
        g_stWbcInfo.stInRect.u32Wth = pLayerCfgBasePara->stHdWinInputSize.s32Width;
        g_stWbcInfo.stInRect.u32Hgt = pLayerCfgBasePara->stHdWinInputSize.s32Height;

        g_stWbcInfo.stOutRect.u32Wth = pLayerCfgBasePara->stSdWinOutputSize.s32Width;
        g_stWbcInfo.stOutRect.u32Hgt = pLayerCfgBasePara->stSdWinOutputSize.s32Height;

        g_stWbcInfo.enOutFmt  = (pLayerCfgBasePara->eOutPutPixelFmt == CBB_VID_IFMT_SP_422) ? VDP_WBC_OFMT_SP422 : VDP_WBC_OFMT_SP422;
        ...
        g_stWbcInfo.enWbcVpConnPoint = VDP_LAYER_CONN_VPACSC;
        g_stWbcInfo.bWbcLayerEn = pLayerCfgBasePara->bWbcEnable ;
        g_stWbcInfo.bSmmu       = pLayerCfgBasePara->bSmmu;
        g_stWbcInfo.bSecure     = pLayerCfgBasePara->bSecure;
        g_stWbcInfo.enDispMode  = pLayerCfgBasePara->enDispMode;
