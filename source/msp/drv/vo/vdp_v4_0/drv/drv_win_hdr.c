...
#endif
static WIN_TECHNICOLOR_LIB_INFO_S  s_TechnicolorLibInfo;
...
HI_VOID WIN_ShowTechniLibInfo(HI_VOID)
{
     WIN_ERROR("bValid    : %d\n",s_TechnicolorLibInfo.bValid);
     WIN_ERROR("Version   : %s\n",s_TechnicolorLibInfo.cVersion);
     WIN_ERROR("Buildtime : %s\n",s_TechnicolorLibInfo.cBuildtime);
     return ;
}
...
HI_S32 WIN_SetTechnicolorLibInfo(WIN_TECHNICOLOR_LIB_INFO_S  *pstTechnicolorLibInfo )
{
    memcpy(&s_TechnicolorLibInfo, pstTechnicolorLibInfo, sizeof(WIN_TECHNICOLOR_LIB_INFO_S));
    return HI_SUCCESS;
}

HI_BOOL WIN_GetTechnicolorLibStatus(HI_VOID)
{
    return s_TechnicolorLibInfo.bValid;
}
...
HI_S32 WinBuf_CopyPsParaToNode(HI_TECHNICOLOR_PS_CFG_S *pstSlHdrPsParamNode,
                                            HI_DRV_DISP_XDR_ENGINE_E enXdrEngine,
                                            HI_TECHNICOLOR_PS_CFG_S *pstSlHdrPsParamFrm)
{
    if (DRV_XDR_ENGINE_JTP == enXdrEngine)
    {
        if (0 != copy_from_user(pstSlHdrPsParamNode, (HI_VOID __user*)pstSlHdrPsParamFrm, sizeof(HI_TECHNICOLOR_PS_CFG_S)))
        {
                                  ...
                                  HI_U32            u32PlayCnt)
{
    ...
    */
    ...
    /* Tecnicolor PS config copy from user space */
    nRet = WinBuf_CopyPsParaToNode(&pstNode->stSlHdrPsParam,
                pstWinXdrFrm->enXdrEngine,
                (HI_TECHNICOLOR_PS_CFG_S *)(uintptr_t)pstWinXdrFrm->u64StSlHdrPsParamAddr);
    if (HI_SUCCESS != nRet)
    {
        return nRet;
    }
...
#endif
    {
        ...
        stPqWinHdrMetaInfo.bTechnilolorEn = pstNode->stSlHdrPsParam.stPsParam.tchdr_en;
        ...
        && (HI_DRV_FRAME_VDP_ALLOCATE_STILL != pstFrame->u32StillFrame))
    {
        ...
        if (pstWinXdrFrm->bEnhanceValid)
        {
            ...
            if (HI_SUCCESS != nRet)
            {
                WIN_ERROR("Invalid el frame refcnts!\n");
                return nRet;
            }
        }
    }
