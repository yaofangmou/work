...
HI_BOOL VPSS_IN_CheckImage_V3(HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    if (((pstImage->stDispRect.s32X + pstImage->stDispRect.s32Width) > pstImage->u32Width) ||
        ((pstImage->stDispRect.s32Y + pstImage->stDispRect.s32Height) > pstImage->u32Height))
    {
        VPSS_ERROR("In image DispRect(x %d y %d w %d h %d) is wrong, image W %u H %u \n",
                   pstImage->stDispRect.s32X, pstImage->stDispRect.s32Y,
                   pstImage->stDispRect.s32Width, pstImage->stDispRect.s32Height,
...
HI_S32 VPSS_IN_CorrectProgInfo(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pImage)
{
    ...
    if (HI_UNF_VCODEC_TYPE_AVS == pstVdecPriv->entype)
    {
...
HI_S32 VPSS_IN_ReviseImage(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pImage)
{
    ...
    if (pstEntity->stDbginfo.bImageRevise)
    {
        ...
        pImage->stDispRect.s32Width = pstEntity->stDbginfo.u32InputWidth;
        pImage->stDispRect.s32Height = pstEntity->stDbginfo.u32InputHeight;
    }
    ...
    if (pImage->u32Width <= 1920)
    {
        ...
        {
            pImage->u32Height = pImage->u32Height & VPSS_IN_HEIGHT_ALIGN;//align 4
        }
    }
    else //need enable 4p/c
    {
        if (HI_FALSE == pstEntity->bAndroidCts)
        {
            VPSS_PIXLE_DOWNALIGN_H(pImage->u32Height, HI_TRUE);  //align 4
        }
    }

    if ((pImage->stDispRect.s32Width == 0) || (pImage->stDispRect.s32Height == 0))
    {
        pImage->stDispRect.s32X = 0;
        pImage->stDispRect.s32Y = 0;
        pImage->stDispRect.s32Width = pImage->u32Width;
        pImage->stDispRect.s32Height = pImage->u32Height;
    }

    if ((pImage->stDispRect.s32Width > pImage->u32Width) ||
        (pImage->stDispRect.s32Height > pImage->u32Height))
    {
        pImage->stDispRect.s32X = 0;
        pImage->stDispRect.s32Y = 0;
    ...
    if (((pImage->stDispRect.s32X + pImage->stDispRect.s32Width) > pImage->u32Width) ||
        ((pImage->stDispRect.s32Y + pImage->stDispRect.s32Height) > pImage->u32Height))
    {
        pImage->stDispRect.s32X = 0;
        pImage->stDispRect.s32Y = 0;
        pImage->stDispRect.s32Width = pImage->u32Width;
...
HI_S32 VPSS_IN_UpdateStreamInfo(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pstSrcImage)
{
    ...
    pstStreamInfo->stDispRect = pstSrcImage->stDispRect;
...
HI_S32 VPSS_IN_NewImgInit(VPSS_IN_ENTITY_S *pstEntity, HI_DRV_VIDEO_FRAME_S *pstImage)
{
    ...
    stWbcAttr.enBitWidth = HI_DRV_PIXEL_BITWIDTH_10BIT; //for rwzb fast check mode
#else
    ...
    if (HI_TRUE == pstEntity->stStreamInfo.bH265Interlace)
    {
        pstEntity->bSplitEnable = HI_FALSE;
    }
...
HI_S32 VPSS_IN_Refresh_V3(VPSS_IN_ENTITY_S *pstEntity)
{
    ...
    HI_BOOL bLastFrmBypass = HI_FALSE;
    HI_BOOL bNeedBypass = HI_FALSE;
    ...
    VPSS_CHECK_NULL(pstEntity);
    pstInstance = (VPSS_INSTANCE_S *)pstEntity->pstInstance;
...
#ifdef VPSS_SUPPORT_PROC_V2
    pstInstance->aulTrySrcTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
...
#ifdef VPSS_SUPPORT_PROC_V2
    pstInstance->aulGetSrcTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
    ...
    */
    ...
    bLastFrmBypass = pstEntity->stStreamInfo.bStreamByPass;
    (HI_VOID)VPSS_IN_UpdateStreamInfo(pstEntity, pstImage);

    bNeedBypass = VPSS_TRANS_FB_NeedTrans(&(pstEntity->stTransFbInfo), pstImage);
    ...
    //    (HI_VOID)VPSS_IN_UpdateStreamInfo(pstEntity, pstImage);
    /*
    In 4k-play scene, when bypass mode changes to normal mode, the new image flag should be set
    */
    if ((HI_TRUE == bLastFrmBypass) && (HI_FALSE == bNeedBypass))
    {
        pstEntity->stStreamInfo.u32IsNewImage = HI_TRUE;
    }
...
#ifdef VPSS_SUPPORT_PROC_V2
    pstInstance->aulInBufDoneTime[pstInstance->u32TimeStampIndex] = jiffies;
#endif
    return s32Ret;
}
                               ...
                               HI_VOID *pstInfo)
{
    ...
    switch (pstHalInfo->enNodeType)
    {
        ...
        case VPSS_HAL_NODE_2D_DEI_FRAME_1:
        case VPSS_HAL_NODE_2D_DEI_FRAME_1_VIRTUAL:
        case VPSS_HAL_NODE_2D_DEI_FIELD_23:
        case VPSS_HAL_NODE_2D_DEI_FIELD_23_VIRTUAL:
#endif
