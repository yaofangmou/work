...
static HI_VOID convert_cctextattr_to_mgr(CAPTION_OSD_CCTEXT_ATTR_S* pstOsdAttr, CAPTION_MGR_CCTEXT_ATTR_S* pstMgrAttr)
{
    ...
    pstMgrAttr->u32BufLen = pstOsdAttr->u32BufLen;
    pstMgrAttr->u32bgColor = pstOsdAttr->u32bgColor;
    pstMgrAttr->u32fgColor = pstOsdAttr->u32fgColor;
    pstMgrAttr->u32edgeColor = pstOsdAttr->u32edgeColor;
    pstMgrAttr->u8justify = pstOsdAttr->u8justify;
    pstMgrAttr->u8WordWrap = pstOsdAttr->u8WordWrap;
    pstMgrAttr->u8fontstyle = pstOsdAttr->u8fontstyle;
    pstMgrAttr->u8fontSize = pstOsdAttr->u8fontSize;
    pstMgrAttr->u8edgeType = pstOsdAttr->u8edgeType;
    pstMgrAttr->bUnderline = pstOsdAttr->bUnderline;
    pstMgrAttr->bItalic = pstOsdAttr->bItalic;
}
...
HI_S32 caption_osd_set_surface(HI_VOID* pvCommonSurface)
{
    OSD_CHECK_INIT();
    return g_om->SetCommonSurface(pvCommonSurface);
}
...
HI_S32 caption_osd_fillrect(HI_HANDLE hSurface, CAPTION_OSD_SURFACE_ATTR_S* pstSurfaceAttr, HI_RECT_S* pstRect, HI_U32 u32Color)
{
    ...
    return g_om->FillRect(hSurface, &stSurfaceAttr, pstRect, u32Color);
}

/* Just fill to surface, not to display. */
HI_S32 caption_osd_fillrect_ex(HI_HANDLE hSurface, CAPTION_OSD_SURFACE_ATTR_S *pstSurfaceAttr,HI_RECT_S *pstRect, HI_U32 u32Color)
{
    OSD_CHECK_INIT();
    ...
    return g_om->FillRect_Ex(hSurface, &stSurfaceAttr, pstRect, u32Color);
}
...
HI_S32 caption_osd_drawtext2Surface(HI_HANDLE hSurface, CAPTION_OSD_SURFACE_ATTR_S * pstSurfaceAttr,HI_RECT_S *pstRect, HI_CHAR *pTest, CAPTION_OSD_CCTEXT_ATTR_S * pstAttr)
{
    OSD_CHECK_INIT();
    ...
    return g_om->DrawCCTextToSurface(hSurface, &stSurfaceAttr,pstRect, pTest, &stCCTextAtrr);
}
...
HI_S32 caption_osd_ttx_draw_bitmap(HI_HANDLE hSrcSurface, HI_U8* bitmap, HI_RECT_S* pstSrcRect, HI_RECT_S* pstDstRect)
{
    ...
    s32Ret = g_om->TeletextDrawBitmap(hSrcSurface, bitmap,pstSrcRect,pstDstRect);
