...
#define HI_SYS_IO_REGISTER_SIZE 4

/* for irq request, define the CPU num macro  */
#define FIRST_CPU_INDEX              0
   ...
   switch (g_pstRegSysCtrl->SC_SYSID)
    {
        ...
        case 0x37980210:
            ...
            break;
        case 0x37980300:
            if (0x00 == g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
            {
                ChipType    = HI_CHIP_TYPE_HI3798M;
                ChipVersion = HI_CHIP_VERSION_V310;
            }
            else if (0x01 == g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
            {
                ChipType    = HI_CHIP_TYPE_HI3798M_H;
                ChipVersion = HI_CHIP_VERSION_V300;
            }
            else if (0x02 == g_pstRegPeri->PERI_SOC_FUSE_0.bits.chip_id)
            {
                ChipType    = HI_CHIP_TYPE_HI3798M_H;
                ChipVersion = HI_CHIP_VERSION_V200;
            }
            else
            {
                ...
                ChipVersion = HI_CHIP_VERSION_V110;
            }
            break;
        case 0x37960200:
    ...
    if ((0x37160430 == g_pstRegSysCtrl->SC_SYSID))
    {
        ChipType    = HI_CHIP_TYPE_HI3716M;
        ChipVersion = HI_CHIP_VERSION_V430;
    }
#endif
...
static HI_SYS_ADDR_RANGE_S totalRange[] =
{
    {HI_SYS_BASE_ADDR, HI_SYS_END_ADDR},
    {HI_PERI_BASE_ADDR, HI_PERI_END_ADDR},
    {HI_IO_BASE_ADDR, HI_IO_END_ADDR},
    {HI_CRG_BASE_ADDR, HI_IO_END_ADDR},
    {HI_OTP_BASE_ADDR, HI_OTP_END_ADDR},
};

static HI_S32 hiDrvSysCheckAddr(HI_U32 u32RegAddr, HI_BOOL bRead)
{
    HI_S32 i = 0;
    HI_U32 regRangeCnt = 0;

    /* attention: write not support the otp register */
    if (HI_TRUE == bRead)
    {
        regRangeCnt = (sizeof(totalRange)/sizeof(HI_SYS_ADDR_RANGE_S));
    }
    else
    {
        regRangeCnt = (sizeof(totalRange)/sizeof(HI_SYS_ADDR_RANGE_S)) - 1;
    }

    for(i = 0; i< regRangeCnt; i++)
    {
        if((totalRange[i].startaddr <= u32RegAddr) && ((totalRange[i].endaddr & (~0x3)) >= u32RegAddr))
        {
            return HI_SUCCESS;
        }
    }
...
HI_S32 HI_DRV_SYS_WriteRegister(HI_SYS_IO_REG_S *stAddr)
{
    HI_VOID *vaddrBase = NULL;
    HI_S32 s32Ret = HI_FAILURE;

    if (HI_NULL == stAddr)
    {
        HI_ERR_SYS("Invalid sAddr[0x%x]!\n",stAddr);
        return HI_FAILURE;
    }

    s32Ret = hiDrvSysCheckAddr(stAddr->u32BaseAddr, HI_FALSE);
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_SYS("Invalid range u32RegAddr[0x%x]!\n",stAddr->u32BaseAddr);
        return s32Ret;
    }

    kSys_lock();

    vaddrBase = ioremap_nocache(stAddr->u32BaseAddr, HI_SYS_IO_REGISTER_SIZE);
    if(HI_NULL == vaddrBase)
    {
        HI_ERR_SYS("ioremap_nocache[0x%x] failed!\n",stAddr->u32BaseAddr);
        kSys_unlock();
        return HI_FAILURE;
    }
    *(volatile unsigned int *)vaddrBase = stAddr->u32RegValue;

    iounmap((HI_VOID *)vaddrBase);
...
HI_S32 HI_DRV_SYS_ReadRegister(HI_SYS_IO_REG_S * stAddr)
{
    HI_VOID *vaddrBase = NULL;
    HI_S32 s32Ret = HI_FAILURE;
    ...
    s32Ret = hiDrvSysCheckAddr(stAddr->u32BaseAddr, HI_TRUE);
    if(HI_SUCCESS != s32Ret)
    {
        HI_ERR_SYS("Invalid range u32RegAddr[0x%x]!\n",stAddr->u32BaseAddr);
        return s32Ret;
    ...
    stAddr->u32RegValue = *(volatile unsigned int *)vaddrBase;
...
HI_S32 HI_DRV_SYS_SetIrqAffinity(HI_MOD_ID_E eModuleId, HI_U32 u32irqNum, const HI_CHAR *name)
{
    ...
    {
#ifndef CHIP_TYPE_hi3716mv430 /* exclude 16mv430 chipset */
        switch (eModuleId)
        {
            case HI_ID_IR:
            case HI_ID_RTC:
