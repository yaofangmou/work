...
/* get_heap_pool_size - get pool size
 * @client: the client
 * @heap_id_mask: heap mask
 * @pool_limit: the limit of total size
 * return the total size of the heap pool size
 *
 * Attention: this function is just to fix compile err in sdk, it is not used in
 * linux-4.4 and linux-3.18 no noww
 */
static inline unsigned long get_heap_pool_size(struct ion_client *client,
