...
static void sAudioformatSet(HdmiTxObj_t* pObj)
{
    ...
    // Set CTS value
    u8_cts_value = pObj->audioFormat.sw_cts & 0xff;
    SiiDrvCraWrReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL1, u8_cts_value);

    u8_cts_value = (pObj->audioFormat.sw_cts >> 8) & 0xff;
    SiiDrvCraWrReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL2, u8_cts_value);

    u8_cts_value = (pObj->audioFormat.sw_cts >> 16) & 0x0f;
    SiiDrvCraWrReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL3, u8_cts_value);
...
static void sAudioFormatGet(HdmiTxObj_t* pObj, SiiAudioFormat_t *audioFmt)
{
    ...
    audioFmt->hw_cts  = SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXHVAL1);
    audioFmt->hw_cts |= SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXHVAL2) << 8;
    audioFmt->hw_cts |= SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXHVAL3) << 16;

    audioFmt->sw_cts  = SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL1);
    audioFmt->sw_cts |= SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL2) << 8;
    audioFmt->sw_cts |= SiiDrvCraRdReg8(craInst, baseAddr | REG_ADDR__CTS_TXSVAL3) << 16;

    audioFmt->bswcts = (SiiDrvCraRdReg8(craInst, baseAddr| REG_ADDR__ACR_CTRL) & BIT_MSK__ACR_CTRL__REG_CTS_HW_SW_SEL) ? HI_TRUE : HI_FALSE;
}
...
static void sHardwareInit(HdmiTxObj_t* pObj)
{
    ...
    //hdmi tx default audio umute
    SiiDrvCraClrBit8(craInst, baseAddr | REG_ADDR__AUDP_TXCTRL, BIT_MSK__AUDP_TXCTRL__REG_AUD_MUTE_EN);
    SiiDrvCraSetBit8(craInst, baseAddr | REG_ADDR__AUD_EN, BIT_MSK__AUD_EN__REG_AUD_IN_EN);// enable audio
    SiiDrvCraClrBit8(craInst, baseAddr | REG_ADDR__AUD_EN, BIT_MSK__AUD_EN__REG_AUD_SEL_OWRT); // HBRA authentication pass
    SiiDrvCraSetBit8(craInst, baseAddr|REG_ADDR__ACR_CTRL, BIT_MSK__ACR_CTRL__REG_CTS_REQ_EN);              // configure AIP cts generation
...
static void sAudioMuteSet(HdmiTxObj_t* pObj, uint8_t bAudioMute)
{
    ...
    if(pObj->tmdsMode == SII_TMDS_MODE__HDMI1 || pObj->tmdsMode == SII_TMDS_MODE__HDMI2)
    {
        ...
        if( bAudioMute )
        {
            ...
            SiiDrvCraSetBit8(sCraInstGet(pObj), sBaseAddrGet(pObj)| REG_ADDR__ACR_CTRL, BIT_MSK__ACR_CTRL__REG_CTS_HW_SW_SEL );
        ...
        {
            //sAudioReset(pObj, false);
            SiiDrvCraClrBit8(sCraInstGet(pObj), sBaseAddrGet(pObj)| REG_ADDR__ACR_CTRL, BIT_MSK__ACR_CTRL__REG_CTS_HW_SW_SEL );
            //SiiDrvCraClrBit8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__AUDP_TXCTRL, BIT_MSK__AUDP_TXCTRL__REG_AUD_MUTE_EN);

            /*check CTS stable*/
            do
            {
                u32Hw  = SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXHVAL1);
                u32Hw |= SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXHVAL2) << 8;
                u32Hw |= SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXHVAL3) << 16;

                u32Sw  = SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXSVAL1);
                u32Sw |= SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXSVAL2) << 8;
                u32Sw |= SiiDrvCraRdReg8(sCraInstGet(pObj), sBaseAddrGet(pObj) | REG_ADDR__CTS_TXSVAL3) << 16;
        ...
        SII_LIB_LOG_DEBUG1(pObj, "i = %d ms.\n",i);
    }
}
/*****************************************************************************************************************/
