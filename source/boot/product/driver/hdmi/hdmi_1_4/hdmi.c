...
//    crg70.bits.aiao_clk_sel     = 0;
    ...
    */
    ...
    SI_SetAudioPath(u8AudioPath);
    HDMI_REG_WRITE(0xf8ce1808,0x02);
    memset(pu8InfoFramePacket ,0x0 ,32);
    //BlockWriteEEPROM( 13, EE_TXAVIINFO_ADDR, pu8AviInfoFrame);
    if(hdmi_dvi_mode){
        SendAVIInfoFrame(pu8AviInfoFrame,13);
        EnableInfoFrame(AVI_TYPE);
        SendAudioInfoFrame( AUD_TYPE, pu8InfoFramePacket );
        EnableInfoFrame(AUD_TYPE);
    }
