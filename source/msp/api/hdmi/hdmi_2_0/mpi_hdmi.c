...
HI_S32 HI_MPI_HDMI_ComInit(void)
{
    ...
    s32Ret = pthread_mutex_init(&s_stCECAttr.stCecMutex, NULL);
    if (HI_SUCCESS != s32Ret)
    {
        HI_FATAL_HDMI("cec mutex init failed!\n");
        return HI_FAILURE;
    }
    ...
    s32Ret = pthread_mutex_init(&s_stHdmiMutex, NULL);
    if (HI_SUCCESS != s32Ret)
    {
        (HI_VOID)pthread_mutex_destroy(&s_stCECAttr.stCecMutex);
        HI_FATAL_HDMI("hdmi mutex init failed!\n");
        return HI_FAILURE;
    }
    ...
    if (s_s32HdmiDevFd <= 0)
    {
        (HI_VOID)pthread_mutex_destroy(&s_stHdmiMutex);
        (HI_VOID)pthread_mutex_destroy(&s_stCECAttr.stCecMutex);
...
HI_S32 HI_MPI_HDMI_ComDeInit(void)
{
    ...
    /*destory mutex */
    s32Ret = pthread_mutex_destroy(&s_stCECAttr.stCecMutex);
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_HDMI("CEC mutex destory failed!\n");
    }

    s32Ret = pthread_mutex_destroy(&s_stHdmiMutex);
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_HDMI("HDMI mutex destory failed!\n");
    }
...
HI_S32 HI_MPI_HDMI_ComRegCallbackFunc(HI_UNF_HDMI_ID_E enHdmi, HI_UNF_HDMI_CALLBACK_FUNC_S *pstCallbackFunc)
{
    ...
    if(NULL != pstCallbackFunc->pfnHdmiEventCallback)
    {
        list_for_each_safe(pos, q , &s_stHDMICallBackList[enHdmi].list)
        {
            ...
            {
                HI_INFO_HDMI("This Callback already register\n");
                HDMI_UNLOCK();
                return HI_SUCCESS;
            }
        }
