...
HI_S32 SRH_PATRequestExt(HI_U32 u32DmxID,PAT_TB * pat_tb,HI_BOOL bTeeEnable)
{
    ...
    /* start data filter*/
    s32Ret = DMX_SectionStartDataFilterExt(u32DmxID, &stDataFilter, bTeeEnable);
...
{
    return SRH_PATRequestExt(u32DmxID, pat_tb, HI_FALSE);
}
...
{
    return SRH_PMTRequestExt(u32DmxID, pmt_tb, u16PmtPid, u16ServiceId, HI_FALSE);
}
...
HI_S32  HIADP_Search_GetAllPmtExt(HI_U32 u32DmxId, PMT_COMPACT_TBL **ppProgTable, HI_BOOL bTeeEnable)
{
    ...
    do
    {
        s32Ret = SRH_PATRequestExt(u32DmxId, &pat_tb, bTeeEnable);
    ...
    for (i = 0; i < pat_tb.u16ProgNum; i++)
    {
        ...
        memset(&pmt_tb, 0, sizeof(PMT_TB));
        s32Ret = SRH_PMTRequestExt(u32DmxId, &pmt_tb, pat_tb.PatInfo[i].u16PmtPid, pat_tb.PatInfo[i].u16ServiceID, bTeeEnable);
        if (HI_SUCCESS != s32Ret)
        {
       ...
       for (j=0; j<pmt_tb.u16CANum; j++)
       {
        ...
        if (SUBTITLING_MAX < pProgTable->proginfo[u32Promnum].u16SubtitlingNum)
        {
            sample_common_printf("receive u16SubtitlingNum(%d) are over than the max number:%d\n",
                                pProgTable->proginfo[u32Promnum].u16SubtitlingNum, SUBTITLING_MAX);
            pProgTable->proginfo[u32Promnum].u16SubtitlingNum = SUBTITLING_MAX;
        }
...
{
    return HIADP_Search_GetAllPmtExt(u32DmxId, ppProgTable, HI_FALSE);
}
