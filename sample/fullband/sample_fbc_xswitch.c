...
static HI_UNF_DMX_PORT_E  g_DmxPort[HI_TUNER_NUMBER] = {0};
...
HI_S32 main(HI_S32 argc, HI_CHAR *argv[])
{
    ...
    {
        snprintf(SectionName,SECTION_MAX_LENGTH,"tuner%dinfo",g_TunerID[i]);
        g_DmxPort[i] = HIADP_INI_Getl(SectionName, "DemuxPort", DEFAULT_DEMUX_PORT, FRONTEND_CONFIG_FILE);
        Ret = HI_UNF_DMX_GetTSPortAttr(g_DmxPort[i],&stDmxPortAttr);

        stDmxPortAttr.enPortType           = HIADP_INI_Getl(SectionName, "DemuxPortType", DEFAULT_DEMUX_PORT_TYPE, FRONTEND_CONFIG_FILE);
        stDmxPortAttr.u32SerialBitSelector = HIADP_INI_Getl(SectionName, "DemuxBitSel",   DEFAULT_DEMUX_BITSEL,    FRONTEND_CONFIG_FILE);
        stDmxPortAttr.u32TunerInClk        = HIADP_INI_Getl(SectionName, "DemuxInClk",    DEFAULT_DEMUX_INCLK,     FRONTEND_CONFIG_FILE);
        stDmxPortAttr.enSerialPortShareClk = HIADP_INI_Getl(SectionName, "DemuxPortShareClk", DEFAULT_DEMUX_PORT_SHARECLK, FRONTEND_CONFIG_FILE);
