...
VOID VDEC_InitStaticResource(VOID)
{
    ...
    OSAL_FP_spin_lock_init(G_TASK_LOCK);
    OSAL_FP_spin_lock_init(G_MEM_LOCK);
    OSAL_FP_spin_lock_init(G_VOQUE_LOCK);
    OSAL_FP_spin_lock_init(G_FSP_LOCK);
    OSAL_FP_spin_lock_init(G_DESTROY_LOCK);

    for (ChanID = 0; ChanID < MAX_CHAN_NUM; ChanID++)
    {
        OSAL_FP_sema_init(G_RLSSTREAM_SEM, ChanID);
    }

    OSAL_FP_init_event(G_INTWAITQUEUE, 1);
...
SINT32 VDEC_InitSecureShareData(VDH_SHARE_DATA_S *ShareData)
{
    ...
    SOS_PROC_Write(1, ShareData->hal_disable);
#endif

    SOS_PROC_Write(0, ShareData->sec_print_word);

    if (ShareData->share_mem_addr == 0)
    {
        dprint(PRN_ERROR, "SOS: share_mem_addr = 0\n");

        return VDEC_ERR;
    }

    g_pVfmSharewMem = (MEMORY_NEEDED_SECVFMW_S *)OSAL_FP_kernel_ns_map(ShareData->share_mem_addr, sizeof(MEMORY_NEEDED_SECVFMW_S));
...
VOID VDEC_ExitSecureShareData(VOID)
{
    if (g_pVfmSharewMem != NULL)
    {
        OSAL_FP_kernel_ns_unmap(g_pVfmSharewMem);
...
SINT32 VDEC_Init(VDEC_OPERATION_S *pArgs)
{
    ...
    OSAL_FP_sema_down(G_VFMW_SEM, 0xFFFFFFFF);
...
OK_EXIT:
    ...
    if (g_VfmwInitCount < 0x7FFFFFFF)
    {
        ...
        OSAL_FP_sema_up(G_VFMW_SEM, 0xFFFFFFFF);
