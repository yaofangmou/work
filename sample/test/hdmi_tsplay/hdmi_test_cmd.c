...
static HI_U32 Hdmi_ParseArg(HI_U32 argc)
{
    ...
    else if (0 == strcmp("hdmi_video_timing", (char *)args[0]))
    {
        ...
        timing_index = (HI_U32)Hdmi_String_to_Integer(args[1]);
...
HI_U32 hdmi_get_pixel_clock(HI_UNF_ENC_FMT_E enFormat)
{
    HI_U32 i = 0, u32PxlClk = 0, u32FormatPixClkSize = 0;
    HDMI_TIMING_PIXEL_CLOCK_S *pstTimingPixClk = &stFormatPixClk[0];

    u32FormatPixClkSize = sizeof(stFormatPixClk)/sizeof(stFormatPixClk[0]);

    for ( i = 0; i < u32FormatPixClkSize; i++, pstTimingPixClk++)
    {
        if (enFormat == pstTimingPixClk->enFormat)
        {
            u32PxlClk = pstTimingPixClk->u32Pixclk;
            return u32PxlClk;
        }
    }
    ...
    if (0 == u32PxlClk)
    {
       printf("CAN NOT get the pixel_clock of:%d .\n", enFormat);
    }
...
HI_S32 hdmi_match_format_colorspace(HI_UNF_ENC_FMT_E enFormat, HI_UNF_HDMI_VIDEO_MODE_E *pColorSpace, HI_UNF_HDMI_DEEP_COLOR_E *pDeepColor)
{
    HI_U32                   u32RetError = HI_SUCCESS;
    HI_UNF_EDID_BASE_INFO_S  stSinkCap;

    printf("match_format_colorspace: enFormat = %d  \n", (int)enFormat);

    if(enFormat >= HI_UNF_ENC_FMT_861D_640X480_60 /* vesa format --> rgb 8 */
       && enFormat <= HI_UNF_ENC_FMT_VESA_2560X1600_60_RB)
    {
        *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_RGB444;
        *pDeepColor  = HI_UNF_HDMI_DEEP_COLOR_24BIT;
        printf("VESA format to rgb 8Bit !");
        return HI_SUCCESS;
    }

    /* The sink capability get failed situation has been define before. */
    HI_UNF_HDMI_GetSinkCapability(enTest_Hdmi_ID, &stSinkCap);

    /* 4k 50_60P */
    if ((HI_UNF_ENC_FMT_3840X2160_50 == enFormat) || (HI_UNF_ENC_FMT_3840X2160_60 == enFormat)
        || (HI_UNF_ENC_FMT_4096X2160_50 == enFormat) || (HI_UNF_ENC_FMT_4096X2160_60 == enFormat))
    {
        /* base on the sinkCap to decide the color&space type */
        if ((HI_TRUE == stSinkCap.stColorSpace.bYCbCr420)  /* if the TV support YUV420 for the format */
            && (HI_TRUE == stSinkCap.bSupportY420Format[enFormat]))
        {
            if ((HI_TRUE == stSinkCap.stY420DeepColor.bY420DeepColor30Bit) /* if the TV support YUV420 30 bit and the TMDS_Character_rate is not bigger than the MaxTMDSClock */
                && ((hdmi_get_pixel_clock(enFormat) * 125 / 100 / 2) <= stSinkCap.u32MaxTMDSClock * 1000)
                && (HI_TRUE == stSinkCap.bHdrSupport))
            {
                *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_YCBCR420; /* YCbCr420 10Bit */
                *pDeepColor  = HI_UNF_HDMI_DEEP_COLOR_30BIT;
                printf("The pixel clock of the format:%d is %d. \n", (int)enFormat, hdmi_get_pixel_clock(enFormat));
                printf("4K50_60P --> adapt to YCbCr420 10Bit \n");
            }
            else
            {
                *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_YCBCR420; /* YCbCr420 8Bit */
                *pDeepColor  = HI_UNF_HDMI_DEEP_COLOR_24BIT;
                printf("4K50_60P --> adapt to YCbCr420 8Bit \n");
            }
        }
        else if (hdmi_get_pixel_clock(enFormat) <= (stSinkCap.u32MaxTMDSClock * 1000))
        {
            if (HI_TRUE == stSinkCap.stDeepColor.bDeepColorY444)
            {
                *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_YCBCR444; /* YCbCr444 8Bit */
                *pDeepColor = HI_UNF_HDMI_DEEP_COLOR_24BIT;
                printf("4K50_60P --> adapt to YCbCr444 8Bit \n");
             }
            else if (HI_TRUE == stSinkCap.stColorSpace.bRGB444)
            {
                *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_RGB444; /* RGB444 8bit*/
                *pDeepColor  = HI_UNF_HDMI_DEEP_COLOR_24BIT;
                printf("4K50_60P --> adapt to RGB444 8Bit \n");
            }
            else if (HI_TRUE == stSinkCap.stColorSpace.bYCbCr422)
            {
                *pColorSpace = HI_UNF_HDMI_VIDEO_MODE_YCBCR422; /* YCbCr422 8Bit */
                *pDeepColor = HI_UNF_HDMI_DEEP_COLOR_24BIT;
                printf("4K50_60P --> adapt to YCbCr422 8Bit \n");
            }
            else
            {
                return HI_FAILURE;
        ...
        else /* if the TV's MaxTMDSClock is less than 594MHz*/
        {
            printf("TV CAN NOT display the format, the format's TMDS_Character_rate is bigger than the MaxTMDSClock.\n");
            return HI_FAILURE;
        }
    }
      /* other format */
    ...
    printf("match_format_colorspace return %d, result:colorspace = %d   deepcolor = %d .\n", u32RetError, *pColorSpace, *pDeepColor);
    return u32RetError;
}
#endif/* video timing adapt feature */
...
to use hdmi output strategy.*/
    ...
    HI_UNF_EDID_BASE_INFO_S           stSinkCap;
    HI_UNF_HDMI_VIDEO_MODE_E          enMatchColorSpace = HI_UNF_HDMI_VIDEO_MODE_RGB444;
    HI_UNF_HDMI_DEEP_COLOR_E          enMatchDeepColor  = HI_UNF_HDMI_DEEP_COLOR_24BIT;

    RetError = HI_UNF_HDMI_GetSinkCapability(enTest_Hdmi_ID, &stSinkCap);

    if (HI_SUCCESS == RetError)
    {
        if (HI_TRUE == stSinkCap.bSupportFormat[u32TimingIndex])
        {
    ...
    {
        printf("Get sink capability FAILED!!\n");
        return HI_FAILURE;
    }

    if(u32TimingIndex >= HI_UNF_ENC_FMT_BUTT)
    {
           printf("Set format exceed limits.\n");
           return -1;
    }

    RetError = hdmi_match_format_colorspace(u32TimingIndex, & enMatchColorSpace, &enMatchDeepColor);
    ...
    else if ((HI_UNF_ENC_FMT_VESA_800X600_60 <= u32TimingIndex) && (HI_UNF_ENC_FMT_VESA_2560X1600_60_RB >= u32TimingIndex))
    {
        ...
        if (stAttr.bEnableHdmi != HI_FALSE)
        {
...
/*
    //stAttr.enVideoFmt = u32TimingIndex;
...
*/
    ...
    Last2DFmt = u32TimingIndex;
    HDMI_SaveFmt(Last2DFmt);    /* save new format */

    RetError = HI_UNF_DISP_SetFormat(HI_UNF_DISPLAY1, u32TimingIndex); /* set new format */
...
to use hdmi output strategy.*/
    ...
    if (!(stAttr.enVidOutMode == enMatchColorSpace && stAttr.enDeepColorMode == enMatchDeepColor))
    {   /* deep color or color space has changed */
        stAttr.enVidOutMode    = enMatchColorSpace;
        stAttr.enDeepColorMode = enMatchDeepColor;

        RetError = HI_UNF_HDMI_SetAttr(enTest_Hdmi_ID, &stAttr);

        if (HI_SUCCESS != RetError)
        {
            printf("hdmi_video_timing()> HI_UNF_HDMI_SetAttr failed,colorspace change decide by hdmi drv. \n");
        }
    }
    else
    {
       printf("Hdmi_test_cmd.c>hdmi_video_timing()>setcolorspace&deepcolor>the match result same as current!\n");
    }
#endif /* video timing adapt feature */
