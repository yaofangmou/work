...
static int check_image(const char *addr, kernel_fn *kernel)
{
	...
	if ((uint32)data != hdr.load) {
		printf("Loading Kernel Image from 0x%x to 0x%x ... ", (uint32)data, hdr.load);
...
void start_tee(uint32_t arch, uint32_t entry, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
	...
	if (SCPU_NOT_SUPPORT != (readl(REG_SCPU_SUPPORT) & SCPU_SUPPORT_MASK))  {
		...
		mailbox_set_and_wfi(&command, NULL, 0);
		/* mailbox_set_and_wfi() MUST not return, otherwise, reset the cpu */
		goto error;
	} else {
...
void start_tee_atf(uint32_t arch, char *buf, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
	...
	if(arch == BL33_IMG_ARM64) {
		bl33_ep.pc = (uint64_t)(get_image_ep(hdr));
		bl33_ep.args.arg0 = (uint64_t)((uint32_t)arg0);
...
int kern_load(const char *addr)
{
	...
	char *buf = (char *)addr;
	char *fdt = NULL;
	image_header_t hdr_copy, *hdr = NULL;
	unsigned int kernel_buf = (unsigned int)buf;
	kernel_fn kernel = NULL;
#ifdef CONFIG_TEE_SUPPORT
#if defined(CONFIG_ASC_SUPPORT) || defined(CONFIG_TEE_ATF_SUPPORT)
    char	*s;
    int notee = 0;
#endif
#endif
	...
	memcpy((char *)hdr, (char *)kernel_buf, sizeof(image_header_t));
	ret = check_image((char *)kernel_buf, &kernel);
	if (ret)
		return ret;

	/* assign a valid address to fdt if you have, otherwise it will get it from the end of kernel image */
	if (fdt) {
		if (fdt_check_header(fdt)) {
			printf("Invalid FDT at %p, hdr at %p  error=%d-->run.c\n", fdt, hdr,fdt_check_header(fdt));
			return 1;
		}
	}
	...
	if(is_tee_type()) {
		...
		if (!notee) {
			if (0 == check_image_arch(hdr, IH_ARCH_ARM64)) {
				...
				bl33_ep_info->pc = (uint64_t)(get_image_ep(hdr));
			...
			else if (0 == check_image_arch(hdr, IH_ARCH_ARM)) {
		...
		if(!notee) {
			if (0 == check_image_arch(hdr, IH_ARCH_ARM)) {
				printf("Start TEE with aarch32 kernel...\n");
				start_tee_atf(BL33_IMG_ARM32, (char *)hdr, 0, 0, 0, 0);
			}
			else if (0 == check_image_arch(hdr, IH_ARCH_ARM64)) {
				printf("Start TEE with aarch64 kernel...\n");
				start_tee_atf(BL33_IMG_ARM64, (char *)hdr, (uint32_t)fdt, 0, 0, 0);
			}
			else {
				printf("ARCH type not support!!!\n");
				return -1;
...
#ifdef CONFIG_SBL_VXX_UXX
	{
		...
		if (0 == check_image_arch ((image_header_t *)(kernel_buf), IH_ARCH_ARM64)) {
			kernel = (void (*)(int, int, uint32))get_image_data(hdr);
			cmd_args.arg1 = (uint64_t)((uint32_t)fdt);
			cmd_args.arg2 = (uint64_t)(get_image_ep(hdr));
			cmd_args.arg3 = (uint64_t)image_get_data_size(hdr) + CONFIG_DTB_MAX_SIZE;
			cmd_args.arg4 = CONFIG_TAG_OFFSET + CONFIG_TAG_MAX_SIZE;
			...
			return exec_smc(ARM_BOOT_SMC_CALL, (u32)kernel, spsr, (u32)&cmd_args);
		} else
