...
HI_S32 ARIBCC_GRAPHIC_ClearScreen(HI_S32 ccType, HI_BOOL bBlit)
{
    ...
    {
        //printf("[%s %d] <%d,%d,%d,%d> blit:%d\n", __FUNCTION__, __LINE__, stDispParam.stRect.x, stDispParam.stRect.y,
         //                           stDispParam.stRect.width, stDispParam.stRect.height, stDispParam.bBlit);
...
HI_S32 ARIBCC_GRAPHIC_GetCurPts(HI_S64 *ps64CurPts)
{
    ARIB_CCDisp_Screen_S *pstScreen = &s_astAribCCDispScreen[0];
...
HI_S32 _ARIBCC_CharFlash(CCDISP_FlashText_S * text,HI_U32 flag)
{
    ARIB_CCDisp_Screen_S *pstScreen = &s_astAribCCDispScreen[0];
    HI_UNF_CC_DISPLAY_PARAM_S stDispParam;
    ...
    stDispParam.unDispParam.stText.u8AribFontSize = text->u8AribFontSize;
    stDispParam.unDispParam.stText.enScaleType = text->enScaleType;
    stDispParam.unDispParam.stText.u8CharInterval = text->u8CharInterval;
    stDispParam.unDispParam.stText.u8LineInterval = text->u8LineInterval;
    stDispParam.unDispParam.stText.enPrtDirect = text->enPrtDirect;
...
HI_S32 ARIBCC_Text_CharFlash(HI_VOID)
{
    HI_U32 winCount;
    ...
    for( winCount = 0 ; winCount < 8 ; winCount++ )
    {
        ...
        else
        {
            ...
            {
                (void)_ARIBCC_CharFlash(flashText,count%2);
...
HI_S32 ARIBCC_Text_DeleteCharFlash(HI_VOID)
{
    //delete flash word
    CCDISP_MUTEX_LOCK(&flashMutex);
    CCDISP_FlashText_S * flashText_A = s_astCCDispFlashText[ARIBCC_WINDOW_ID];
    CCDISP_FlashText_S * flashText_B = HI_NULL;
    s_astCCDispFlashText[ARIBCC_WINDOW_ID]  = HI_NULL;
