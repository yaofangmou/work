...
static char *mkpath(const char *path, const char *name)
{
    ...
    HI_CHECK_PARAM(NULL == path, HI_ERR_PMOC_INVALID_POINT, NULL);
    HI_CHECK_PARAM(NULL == name, HI_ERR_PMOC_INVALID_POINT, NULL);

    len1 = strlen(path);
    len2 = strlen(name);

    HI_CHECK_PARAM(len1 > PATH_MAX, HI_ERR_PMOC_INVALID_PARA, NULL);
    HI_CHECK_PARAM(len2 > PATH_MAX, HI_ERR_PMOC_INVALID_PARA, NULL);

    n = (char*)malloc(len1 + len2 + 2);
    if (!n)
    {
        HI_ERR_PrintFuncResult(calloc, HI_FAILURE);
        HI_ERR_PrintS32(len1 + len2 + 2);
        return NULL;
    }
                               ...
                               char c, char d, char e, char f, char g, char * busnum, char* devnum)
{
    ...
    {
        HI_WARN_PrintFuncResult(realpath, errno);
    }
    ...
    if (fd < 0)
    {
        ...
        if (fd < 0)
        {
            HI_ERR_PrintInfo("open failed: \n");
            return -1;
        }
        HI_INFO_PrintStr(file);
    }
...
static int usb_get_remote(struct libusb * desc, unsigned char * intmask)
{
    ...
    if (intmask == NULL)
    {
        HI_ERR_PrintInfo(" initmask is NULL\n");
        return -1;
    }
    else
    {
            ...
            if (rc != -1)
            {
                ...
                if (rc != -1)
                {
                    ...
                    if (value != USB_CLASS_HUB)
                    {
                        ...
                        if ((name[0]==0x32)&&(name[2]==0x31)) {
                            *intmask |= (0x01<<0);
                        } else if((name[0]==0x32)&&(name[2]==0x32)) {
                            *intmask |= (0x01<<1);
                        }
#else
                        if (name[2] == 0x31)
...
int get_remotewakeup_devnum(unsigned char * intmask)
{
    ...
    if (lib == NULL)
    {
        HI_ERR_PrintFuncResult(libusb_open, -1);
        return -1;
    }
