...
)
{
    ...
    {
        COM_PrintFuncErr(HI_SSD_Init, s32Ret);
        return s32Ret;
    }

    s32Ret = HI_SSD_WholeImageVerify(pu8FileData, u32FileSize, &pu8ImageData, &u32ImageSize);
    if (HI_SUCCESS != s32Ret)
    {
        COM_PrintFuncErr(HI_SSD_WholeImageVerify, s32Ret);

        COM_CHECK(HI_SSD_DeInit());
        return s32Ret;
    }
    ...
    for (i = 0; i < pstManuInfo->u16FlashMapNum; i++)
    {
        ...
        s32Ret = HI_SSD_PartitionImageVerify(pstFlashMap->au8PartName,
        ...
        if (HI_SUCCESS != s32Ret)
        {
            COM_PrintFuncErr(HI_SSD_PartitionImageVerify, s32Ret);
            COM_CHECK(HI_SSD_DeInit());
            return s32Ret;
        }
    }
