...
//#define TESTBOARD
//#define PCIE_PHYCLK
//#define PCIE_SPREAD_SPECTRUM_ENABLE
#if defined(CONFIG_GPIO_PCIE_POWER_EN_SUPPORT) || defined(CONFIG_PCIE_RESET)
...
#define PCIE_GPIO_MAX                    (0x0D)
#define PCIE_GPIO_NUM_MASK               (0x07)

#define PERI_CTRL                        (0x0008)
#define PERI_COMBPHY0_CFG                (0x858)

#define COMBPHY0_REF_CKEN                BIT(8)
#define COMBPHY0_SRST_REQ                BIT(12)
#define COMBPHY0_RSTN_SEL_CTRL           BIT(13)
...
static int __maybe_unused hipcie_ioremap(u32 addr, void **pcie_reg_addr)
{
	if (!(*pcie_reg_addr)) {
...
static void __maybe_unused hipcie_iounmap(void **pcie_reg_addr)
{
	if (*pcie_reg_addr) {
...
static int hiclk_enable_pcie(struct clk_hw *hw)
{
	...
	void __iomem __maybe_unused *pcie_reg_addr0 = NULL;
...
#ifdef TESTBOARD
	/*config GPIO4_0 pinmux*/
	ret = hipcie_ioremap(0xf8a21094, &pcie_reg_addr0);
	if (ret == -1)
		return -1;
	reg = readl(pcie_reg_addr0);
	reg &= ~0x1;
...
#endif
#ifdef PCIE_SPREAD_SPECTRUM_ENABLE
