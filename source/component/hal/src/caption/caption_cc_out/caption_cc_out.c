...
#define CC_FONT_FILE "/usr/data/font/cc_mono.ttf"
#else
...
static HI_S32 cc_higo_suraface_init(HI_HANDLE hOut)
{
    ...
    HAL_CHK_RETURN((HIGO_INVALID_HANDLE == s_hccLayer), FAILURE,
...
static HI_VOID cc_higo_drawText(const HI_UNF_CC_TEXT_S stText, HI_UNF_CC_RECT_S stRect)
{
    ...
    HAL_CHK_PRINTF((SUCCESS != s32Ret), "failed to UTF16LE_to_UTF8,ret = %x\n", s32Ret);
