...
{
    //1 数据格式不能固定，需要根据不同回写点来适配，此处建议给源的数据格式
    //1 此处的分辨率需是宽高比决策后的高清和标清的输入输出，实际video大小，需要修改

    if ((CBB_VID_IFMT_SP_420 == pstWbcCfgBasePara->eInPutPixelFmt) || (CBB_VID_IFMT_SP_422 == pstWbcCfgBasePara->eInPutPixelFmt)
    || (CBB_VID_IFMT_SP_TILE_64 == pstWbcCfgBasePara->eInPutPixelFmt))
    {
        puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].enZmeInFmt = (CBB_VID_IFMT_SP_422 == pstWbcCfgBasePara->eInPutPixelFmt) ?
        HI_PQ_ALG_PIX_FORMAT_SP422 : HI_PQ_ALG_PIX_FORMAT_SP420;
    }
    else
    {
        DISP_ERROR("Invalid eInPutPixelFmt! %u\n", pstWbcCfgBasePara->eInPutPixelFmt);
        puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].enZmeInFmt = HI_PQ_ALG_PIX_FORMAT_SP422;
    }

    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].u32FrmWIn = pstWbcCfgBasePara->stHdWinInputSize.s32Width;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].u32FrmHIn = pstWbcCfgBasePara->stHdWinInputSize.s32Height;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].u32FrmWOut = pstWbcCfgBasePara->stHdWinOutputSize.s32Width;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].u32FrmHOut = pstWbcCfgBasePara->stHdWinOutputSize.s32Height;

    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].enZmeInFmt = HI_PQ_ALG_PIX_FORMAT_SP422;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].u32FrmWIn = pstWbcCfgBasePara->stHdWinInputSize.s32Width;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].u32FrmHIn = pstWbcCfgBasePara->stHdWinInputSize.s32Height;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].u32FrmWOut = pstWbcCfgBasePara->stSdVideoRect.s32Width;
    puWbcZmeIn->stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].u32FrmHOut = pstWbcCfgBasePara->stSdVideoRect.s32Height;
        ...
        VDP_LAYER_VID_INFO_S *pstVidInfo)
{
    ...
    if (!pLayerCfgBasePara->bMuteEn && pLayerCfgBasePara->bRegionEn)
    {
        ...
        pstVidInfo->bHzmeEn = stZmeOut.stStrategyLayerId.stHZmeCfg.bPreZme;
        pstVidInfo->enHzmeMode = stZmeOut.stStrategyLayerId.stHZmeCfg.bPreZmeFir ? VDP_HZME_MODE_FIR:VDP_HZME_MODE_COPY;
...
HI_S32 CBB_Video_SetLayerParam(CBB_LAYER_CFG_INFO_S *pLayerCfgBasePara, CBB_CFG_EXT_S *pVDPCfgExtPara)
{
    ...
    HW_BUF_QueueFrame(pLayerCfgBasePara->u32LayerNO, &stVidInfo);
#endif
    return HI_SUCCESS;
}
...
HI_S32 CBB_Video_SetWbcParam(CBB_WBC_CFG_INFO_S *pLayerCfgBasePara, CBB_CFG_EXT_S *pVDPCfgExtPara)
{
    ...
    if (pLayerCfgBasePara->enWBC == DRV_VDP_WBC_WIN)
    {
        if (pLayerCfgBasePara->bWbcEnable == HI_TRUE)
        {
            ...
            unionZmeIn.stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].bZmeBFIn    = 1 - bBtm;
            unionZmeIn.stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V3].bZmeBFOut   = 1 - bBtm;
            /* WBC SEL POINT FIRLD OR FRAME */
            unionZmeIn.stWbcStrategy.stLayerInfo[HI_PQ_WBC_LAYER_STRATEGY_V0].bZmeFrmFmtIn = pLayerCfgBasePara->bFieldMatch ? 0:1;

            if (HI_SUCCESS != (s32Ret = DRV_PQ_GetVdpZmeStrategy (HI_PQ_ZME_LAYER_WBC_VP, &unionZmeIn, &unionZmeOut)))
            {
                return s32Ret;
            }

#ifndef VDP_ZME_COPY_MODE
            stWbcInfo.bZmeHorEnable   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeHorEnable;
            stWbcInfo.bZmeVerEnable   = unionZmeOut.stStrategyWbcVp.stZmeFmt.bZmeVerEnable;
...
#endif
            stWbcInfo.enWbcVpConnPoint   = (unionZmeOut.stStrategyWbcVp.enVpPoint == HI_PQ_WBC_VP_POINT_VP) ? VDP_WBC_ROOT_PATH_BLBX : VDP_WBC_ROOT_PATH_BZME;
            stWbcInfo.enInFmt =  (unionZmeOut.stStrategyWbcVp.stZmeFmt.enZmeInFmt == HI_PQ_ALG_PIX_FORMAT_SP422) ? VDP_VID_IFMT_SP_422 : VDP_VID_IFMT_SP_420;
            stWbcInfo.bP2iEn   = unionZmeOut.stStrategyWbcVp.bOpenP2I;

            stWbcInfo.stInRect.u32Wth = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32InWidth;
            stWbcInfo.stInRect.u32Hgt = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32InHeight;
           // printk("SetWbcParam InRect w h %d %d\n", stWbcInfo.stInRect.u32Wth, stWbcInfo.stInRect.u32Hgt);

            stWbcInfo.stVideoRect.u32Wth = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutWidth;
            stWbcInfo.stVideoRect.u32Hgt = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutHeight;
            stWbcInfo.stVideoRect.u32X = pLayerCfgBasePara->stSdVideoRect.s32X;
            stWbcInfo.stVideoRect.u32Y = pLayerCfgBasePara->stSdVideoRect.s32Y;
            stWbcInfo.stOutRect.u32X = pLayerCfgBasePara->stSdOutRect.s32X;
            stWbcInfo.stOutRect.u32Y = pLayerCfgBasePara->stSdOutRect.s32Y;
            stWbcInfo.stOutRect.u32Wth = pLayerCfgBasePara->stSdOutRect.s32Width;
            stWbcInfo.stOutRect.u32Hgt = pLayerCfgBasePara->stSdOutRect.s32Height;

        //    printk("SetWbcParam OutRect w h %d %d\n", stWbcInfo.stOutRect.u32Wth, stWbcInfo.stOutRect.u32Hgt);
...
#endif
            stWbcInfo.enOutFmt  = (unionZmeOut.stStrategyWbcVp.stZmeFmt.enZmeOutFmt == HI_PQ_ALG_PIX_FORMAT_SP422) ? VDP_WBC_OFMT_SP422 : VDP_WBC_OFMT_SP420;
            ...
            //1 增加缩放系数地址
            stWbcInfo.stZmeAddr.u32ZmeCoefAddrHL = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeAddr.u32ZmeCoefAddrHL;
            stWbcInfo.stZmeAddr.u32ZmeCoefAddrVL = unionZmeOut.stStrategyWbcVp.stZmeFmt.stZmeAddr.u32ZmeCoefAddrVL;
    ...
    if (pLayerCfgBasePara->enWBC  == DRV_VDP_WBC_WIN)
    {
        ...
        stWbcInfo.stVideoRect.u32Wth = pLayerCfgBasePara->stSdVideoRect.s32Width;
        stWbcInfo.stVideoRect.u32Hgt = pLayerCfgBasePara->stSdVideoRect.s32Height;
        ...
        stWbcInfo.enWbcVpConnPoint = VDP_WBC_ROOT_PATH_BLBX;
        stWbcInfo.bWbcLayerEn = pLayerCfgBasePara->bWbcEnable ;
        stWbcInfo.bSmmu       = pLayerCfgBasePara->bSmmu;
        stWbcInfo.bSecure     = pLayerCfgBasePara->bSecure;
                              ...
                              CBB_WBC_POLICY_S *pWbcPolicy)
{
    ...
    if (pLayerCfgBasePara->enWBC  == DRV_VDP_WBC_WIN)
    {
        ...
        enWbcVpConnPoint = (unionZmeOut.stStrategyWbcVp.enVpPoint == HI_PQ_WBC_VP_POINT_VP) ? VDP_WBC_ROOT_PATH_BZME : VDP_WBC_ROOT_PATH_BLBX;
        ...
        if (0)
        {
            ...
            pWbcPolicy->stSdFinaleDispRect.s32Width = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutWidth;
            pWbcPolicy->stSdFinaleDispRect.s32Height = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutHeight;
        ...
        else
        {
            ...
            pWbcPolicy->stSdFinaleDispRect.s32X = pLayerCfgBasePara->stSdVideoRect.s32X;
            pWbcPolicy->stSdFinaleDispRect.s32Y = pLayerCfgBasePara->stSdVideoRect.s32Y;
            pWbcPolicy->stSdFinaleDispRect.s32Width = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutWidth;
            pWbcPolicy->stSdFinaleDispRect.s32Height = unionZmeOut.stStrategyWbcVp.stZmeFmt.u32OutHeight;
        }
...
HI_S32 CBB_Video_GetLayersCapability(CBB_VIDEO_LAYER_CAPABILITY_S *pstLayerCapability)
{
    ...
    if (pstLayerCapability->eId == VDP_LAYER_SD0)
    {
        pstLayerCapability->bZme = HI_FALSE;
        pstLayerCapability->bZmePrior = HI_FALSE;
        pstLayerCapability->bTnrSnrDbDMDrDs = HI_FALSE;
