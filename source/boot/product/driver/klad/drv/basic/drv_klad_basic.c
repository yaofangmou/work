...
HI_S32 EXT_DRV_KLAD_BASIC_SetClearCWKey(EXT_CLEAR_CW_KLAD_PARA_S *stClearKey)
{
    ...
    DRV_KLAD_CHECK_NULL_PTR(stClearKey);
    DRV_KLAD_CHECK_TARGET(stClearKey->enEngine, HI_UNF_KLAD_TARGET_ENGINE_BUTT);
    target = DRV_KLAD_GetTargetModID(stClearKey->handle);
    if (DRV_KLAD_TARGET_DEMUX == target)
    {
        DRV_KLAD_CHECK_TARGET_ADDR((HI_U8)HI_HANDLE_GET_CHNID(stClearKey->handle), DRV_KLAD_MAX_DEMUX_ADDR_CNT);
    }
    else
    {
        DRV_KLAD_CHECK_TARGET_ADDR((HI_U8)HI_HANDLE_GET_CHNID(stClearKey->handle), DRV_KLAD_MAX_CIPHER_ADDR_CNT);
    }

    HAL_KLAD_SetClearCWSecEn(stClearKey->enSecEn);
    HAL_KLAD_SetClearCwDataIn(stClearKey->u8Key);
    HAL_KLAD_CLEAR_CWStart((HI_U8)HI_HANDLE_GET_CHNID(stClearKey->handle), stClearKey->bOdd,
                                    stClearKey->enEngine, target);
    ...
    HAL_KLAD_SetClearCWSecEn(KLAD_SEC_ENABLE);
...
HI_S32 DRV_KLAD_BASIC_Init(HI_VOID)
{
    ...
    s_klad_basic_drv_zone.setsecen = HAL_KLAD_Basic_SetSecen;
