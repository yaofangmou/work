...
static HI_S32 subt_draw_8bitmap(const HI_UNF_SO_GFX_S* pstGfx, unsigned char* pBuffer)
{
    ...
    for (i = 0; i < pstGfx->h; i++)
    {
        for (j = 0; j < pstGfx->w; j++)
        {
            ...
            u32TmpBaseIdx = i * (s_stBitmapBuffInfo.u32BufferW) * SUBT_PIXBYTE + SUBT_PIXBYTE * j;
#endif
...
#else
            u8Alpha = pstGfx->stPalette[u32PaletteIdx].u8Alpha;
            u8Blue = pstGfx->stPalette[u32PaletteIdx].u8Blue;
            u8Green = pstGfx->stPalette[u32PaletteIdx].u8Green;
            u8Red = pstGfx->stPalette[u32PaletteIdx].u8Red;
            ...
            pBuffer[u32TmpBaseIdx + 3] = u8Alpha;//Alpha
            pBuffer[u32TmpBaseIdx + 2] = u8Blue;//Blue
            pBuffer[u32TmpBaseIdx + 1] = u8Green;//Green
            pBuffer[u32TmpBaseIdx + 0] = u8Red;//Red
#endif
        }
    }
...
static HI_S32 subt_draw_32bitmap(const HI_UNF_SO_GFX_S* pstGfx, unsigned char* pBuffer)
{
    ...
    for (i = 0; i < pstGfx->h; i++)
    {
        for (j = 0; j < pstGfx->w; j++)
        {
            ...
            u8Alpha = pstGfx->pu8PixData[u32PixIdx++];
            u8Blue = pstGfx->pu8PixData[u32PixIdx++];
            u8Green = pstGfx->pu8PixData[u32PixIdx++];
            u8Red = pstGfx->pu8PixData[u32PixIdx++];
            ...
            pBuffer[u32TmpBaseIdx + 3] = u8Alpha;/*alpha*/
            pBuffer[u32TmpBaseIdx + 0] = u8Blue;/*u8Blue*/
            pBuffer[u32TmpBaseIdx + 1] = u8Green;/*u8Green*/
            pBuffer[u32TmpBaseIdx + 2] = u8Red;/*u8Red*/
#endif
        }
    }
