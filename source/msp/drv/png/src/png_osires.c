...
HI_S32 PngOsiResAllocHandle(HI_PNG_HANDLE *ps32Handle)
{
    ...
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(ps32Handle, HI_ERR_PNG_NULLPTR);
...
HI_S32 PngOsiResReleaseHandle(HI_PNG_HANDLE s32Handle)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, s32Handle);
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, s32Handle);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, g_PngInstance[s32Handle - 1].bInUse);
        PNG_UP(&g_HandleMutex);
        return HI_ERR_PNG_INVALID_HANDLE;
    }
...
HI_S32 PngOsiResGetInstance(HI_PNG_HANDLE s32Handle, PNG_INSTANCE_S **ppstInstance)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_GFX_OutDebugFuncMsg(HI_GFX_MSG_INNER_ENTER_FUNC);
    HI_GFX_CHECK_NULLPOINTER_RETURN_VALUE(ppstInstance, HI_ERR_PNG_NULLPTR);

    if ((s32Handle <= 0) || (s32Handle > PNG_MAX_HANDLE))
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, s32Handle);
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_UINT, PNG_MAX_HANDLE);
        return HI_ERR_PNG_INVALID_HANDLE;
    }
...
HI_S32 PngOsiResAllocMem(HI_UCHAR *pName, HI_U32 u32Size, HI_U32 u32Align, HI_U32 *pu32Phyaddr)
{
    ...
    {
        HI_GFX_OutErrValueMsg(HI_GFX_MSG_ERR_VOID, pu32Phyaddr);
