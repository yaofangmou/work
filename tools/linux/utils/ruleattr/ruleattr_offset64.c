...
static char *_dbg_strdup(const char *str)
{
	nr_malloc++;
	return strdup(str);
}
...
int ra_init_file_offset64(const char *rulefile, FILE *flwarning, FILE *flattrpath,
...
int ra_init_file_debuglevel_offset64(const char *rulefile, int debuglevel)
{
	FILE *flwarning = NULL;
	...
	return ra_init_file_offset64(rulefile, flwarning, flattrpath, flnode, flmatch);
}
/******************************************************************************/

int ra_init_data_offset64(const char *rulelist, int sz_rulelist, FILE *flwarning,
...
static struct fileattr *__find_fileattr(const char *path, int sz_path)
{
	...
	if (g.fl_dump_match) {
		for (ptr = (char *)path; ptr < path + sz_path; ptr++)
...
int ra_set_rootdir_offset64(const char *path)
{
...
int ra_host_attr_offset64(const char *path, void **attr)
{
...
int ra_sys_lstat_offset64(const char *path, struct stat *buf)
{
	int ret;
	void *attr;
	...
	ret = ra_host_attr_offset64(path, &attr);
	if (!ret)
		ra_attr_stat_offset64(attr, buf);

	return 0;
}
/******************************************************************************/

int ra_target_attr_offset64(const char *path, int sz_path, void **attr)
{
	struct fileattr *_attr;
...
int ra_attr_cap_offset64(void *attr, void *data)
{
...
int ra_attr_stat_offset64(void *attr, struct stat *buf)
{
	struct fileattr *_attr = (struct fileattr *)attr;
...
int ra_target_stat_offset64(const char *path, int sz_path, struct stat *buf)
{
	int ret;
	void *attr;

	ret = ra_target_attr_offset64(path, sz_path, &attr);
	if (ret)
		return ret;

	return ra_attr_stat_offset64(attr, buf);
}
/******************************************************************************/

int ra_attr_stat_cap_offset64(void *attr, short *gid, short *uid, unsigned short *mode,
