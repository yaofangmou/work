...
typedef struct hiDMX_DEVICE_S
{
    atomic_t atmOpenCnt;
    UMAP_DEVICE_S stDemuxDev;
}DMX_DEVICE_S;

/**************************** global variables ****************************/
static DMX_DEVICE_S g_stDemuxDev;
...
#ifdef DEMUX_DUMP_PROC
#define DEFAULT_OFFSET_SIZE 0x1000 //Every time you read the register's default size
#define TOTAL_SIZE 0x10000//register's total size
#define DEMUX_NUMBER 1024
char *ADDRESS_STR = "address";
static HI_BOOL isinput = HI_FALSE;//to mark wether the starting address has been specified
static HI_U32 startaddress = 0;//to mark the starting address of read
static HI_BOOL lock = HI_FALSE;//lock to buf(demux_dump)
static HI_U32 offset = DEFAULT_OFFSET_SIZE;
#endif
...
#endif
#ifdef DEMUX_DUMP_PROC
    HI_DRV_PROC_EchoHelper("echo %s > /proc/msp/demux_dump_reg  -- Input address\n",ADDRESS_STR);
#endif
...
#ifdef DEMUX_DUMP_PROC
static HI_S32 DMXDumpProcRead(struct seq_file* p, HI_VOID* v)
{
    HI_U32 i = 0;
    HI_U32 address = startaddress;
    HI_U32 *pdemux_dump = NULL;
    Dmx_Set_S* DmxSet = HI_NULL;
    HI_U32 offset1 = 0;
    HI_U32 *ptr = NULL;
    HI_U32 *ptr1 = NULL;

    offset1 = address - DMX_SET_1_BASE;
    DmxSet = GetDefDmxSet();
    ...
    if (HI_FALSE == isinput || (0 == address)\
        || (address > (TOTAL_SIZE + DMX_SET_1_BASE)))
    {
        HI_PRINT("please input 0xf9c0****(0xf9c00000--0xf9c10000) correct start addr of the demux register first!\n");
        return HI_FAILURE;
    }

    if (lock != HI_FALSE)
    {
        HI_ERR_DEMUX("please try again later !\n");
        return HI_FAILURE;
    }
    lock = HI_TRUE;

    PROC_PRINT(p, "====dump memory 0X%08x start====\n", address);

    pdemux_dump = HI_KMALLOC(HI_DEMUX_ID, sizeof(HI_U32) * DEMUX_NUMBER, GFP_KERNEL);
    if (NULL == pdemux_dump)
    {
        HI_ERR_DEMUX("HI_KMALLOC failed!\n");
        return HI_FAILURE;
    }
    memset(pdemux_dump,0x0,sizeof(HI_U32) * DEMUX_NUMBER);
    ptr = pdemux_dump;
    ptr1 = ptr;
    for (i = 0; i < offset /4; i = i + 4)
    {
        ptr[0] = DMX_READ_REG((DmxSet->IoBase + offset1), i);
        ptr++;
    }
    for (i = 0 ; i < DEFAULT_OFFSET_SIZE / 4; i = i + 4)
    {
        PROC_PRINT(p, "%08x:   %08x %08x %08x %08x\n", address, ptr1[0], ptr1[1], ptr1[2], ptr1[3]);
        ptr1 = ptr1 + 4;
        address = address + 16;
        if (address >= (TOTAL_SIZE + DMX_SET_1_BASE))
        {
            break;
        }
    }
    HI_KFREE(HI_ID_DEMUX, pdemux_dump);
    PROC_PRINT(p, "====dump memory  end====\n");
    lock = HI_FALSE;
    return HI_SUCCESS;
}
static HI_S32 DMXDumpProcWrite(struct file* file, const char __user* buf, size_t count, loff_t* ppos)
{
    HI_U32 maxaddr = DMX_SET_1_BASE + TOTAL_SIZE;
    HI_ULONG inputaddr = 0;
    HI_U32 err = 0;
    char kbuf[256] = {0};
    char* p1 = kbuf;
    ...
    STRSKIPBLANK(p1);
    err = kstrtoul(p1, 16, &inputaddr);
    if (err)
    {
        return HI_FAILURE;
    }

    if ((inputaddr < DMX_SET_1_BASE) || (inputaddr > maxaddr))
    {
        HI_PRINT("unknow command:%s,please input 0xf9c0****(0xf9c00000--0xf9c10000) correct start addr of the demux register\n", p1);
        return (HI_FAILURE);
    }
    offset = ((maxaddr - inputaddr) > offset) ? offset : (maxaddr - inputaddr);
...
#endif
    else if(Port <= HI_UNF_DMX_PORT_IF_15)
    {
    ...
    switch (cmd)
    {
        case CMD_DEMUX_GET_DATA_FLAG:
        {
            ...
            if (pPara->WatchChannelNum <= GetDmxCluster()->Ops->GetChnNum())
            {
                ...
                memset(ValidChannel, 0, sizeof(ValidChannel));
        ...
        case CMD_DEMUX_ACQUIRE_MSG:
        {
            ...
            {
                HI_FATAL_DEMUX("u32AcquireNum too many: 0x%x\n", u32AcquireNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
        ...
        case CMD_DEMUX_RELEASE_MSG:
        {
            ...
            {
                HI_FATAL_DEMUX("u32ReleaseNum too many: 0x%x\n", u32ReleaseNum);
                ret = HI_ERR_DMX_INVALID_PARA;
                break;
            }
    ...
    switch (cmd)
    {
        ...
        case CMD_DEMUX_ES_BUFFER_GET:
        {
            ...
            if (HI_SUCCESS == ret)
            {
                ...
                pPara->u64BufPhyAddr = stEsBuf.u64BufPhyAddr;
                pPara->u64BufKerVirAddr = stEsBuf.u64BufKerVirAddr;
...
static HI_S32 DMX_DRV_Open(struct inode * inode, struct file * file)
{
    ...
    if (1 == atomic_inc_return(&g_stDemuxDev.atmOpenCnt))
    {
        s32Ret = HI_DRV_DMX_Open((HI_VOID*)file);
        if (HI_SUCCESS != s32Ret)
        {
            atomic_dec(&g_stDemuxDev.atmOpenCnt);
            return s32Ret;
        }
    }
...
{
    if (0 == atomic_dec_return(&g_stDemuxDev.atmOpenCnt))
    {
...
HI_S32 DMX_DRV_ModInit(HI_VOID)
{
    ...
    strncpy(g_stDemuxDev.stDemuxDev.devfs_name, UMAP_DEVNAME_DEMUX, strlen(UMAP_DEVNAME_DEMUX));
    g_stDemuxDev.stDemuxDev.devfs_name[strlen(UMAP_DEVNAME_DEMUX)] = '\0';
    atomic_set(&g_stDemuxDev.atmOpenCnt, 0);
    g_stDemuxDev.stDemuxDev.fops   = &DMX_DRV_Fops;
    g_stDemuxDev.stDemuxDev.minor  = UMAP_MIN_MINOR_DEMUX;
    g_stDemuxDev.stDemuxDev.owner  = THIS_MODULE;
    g_stDemuxDev.stDemuxDev.drvops = &dmx_drvops;

    if (HI_DRV_DEV_Register(&g_stDemuxDev.stDemuxDev) < 0)
    {
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_port failed\n");
    }
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_chan failed\n");
    }
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_chanbuf failed\n");
    }
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_filter failed\n");
    }
...
/******* proc function begin ********/
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_key failed\n");
    }
#endif
/******* proc function end ********/
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_pcr failed\n");
    }
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_rec failed\n");
    }
#endif
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_rec_index failed\n");
    }
#endif
    ...
    if (!item)
    {
        HI_ERR_DEMUX("add demux_rmx failed\n");
    }
#endif

#ifdef DEMUX_DUMP_PROC
    stFnOpt.fnWrite = DMXDumpProcWrite;
    stFnOpt.fnRead = DMXDumpProcRead;
    item = HI_DRV_PROC_AddModule("demux_dump_reg", &stFnOpt, NULL);
    if (!item)
    {
        HI_ERR_DEMUX("add proc demux_dump_reg failed\n");
    }
#endif
...
#ifdef DEMUX_DUMP_PROC
    HI_DRV_PROC_RemoveModule("demux_dump_reg");
#endif
#endif

    HI_DRV_DEV_UnRegister(&g_stDemuxDev.stDemuxDev);
