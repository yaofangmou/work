...
static HI_BOOL PVRIndexIsFrameValid(HI_U32 u32Start, HI_U32 u32End, HI_U32 u32Last, HI_U32 u32FrmPos);
STATIC HI_U32 PVRIndexCalcNewPos(HI_U32 u32StartFrame, HI_U32 u32EndFrame, HI_U32 u32LastFrame, HI_U32 u32FrmPos, HI_S32 s32Offset);
...
STATIC HI_S32 PVRIndexGetHeaderInfo(HI_S32 s32Fd, PVR_IDX_HEADER_INFO_S* pstHeadInfo);
...
STATIC HI_VOID PVRIndexSetDftAttr(PVR_INDEX_HANDLE handle);
...
STATIC HI_S32 PVRIndexGetPreEntry(PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S* pEntry);
...
STATIC HI_U32 PVRIndexFindFrameByTime(PVR_INDEX_HANDLE handle, HI_U32 u32FindTime);
...
#if defined(HI_PVR_PID_CHANGE_SUPPORT) && defined(HI_PVR_SWITCH_REWIND_SUPPORT)
static HI_VOID PVRIndexProcPIDChangeInfoWhenRewindChg(PVR_INDEX_HANDLE handle);
#endif

#if defined(HI_PVR_PID_CHANGE_SUPPORT) && (defined(HI_PVR_LINEARIZATION_SUPPORT) || defined(HI_PVR_POST_PROCESSING_SUPPORT))
static HI_VOID PVRIndexProcPIDChangeInfoWhenLinearizationOrTruncate(PVR_INDEX_REC_PID_CTRL_S *pstRecPidCtrl, PVR_CYC_FRAME_NUM_INFO* pstCycFrm, HI_U32 u32StartDispTime, HI_BOOL bTimeshiftToRec, HI_U32 u32HeadOffset);
#endif

#if defined(HI_PVR_PID_CHANGE_SUPPORT) && (defined(HI_PVR_SWITCH_REWIND_SUPPORT) || defined(HI_PVR_LINEARIZATION_SUPPORT) || defined(HI_PVR_POST_PROCESSING_SUPPORT))
static HI_VOID PVRIndexUpdateFrameNumWhenRewindChg(PVR_CYC_MGR_S *pstLastCycMgr, HI_U32 *pu32FrameNum, HI_U32 u32HeadPosOffset);
#endif

#if defined(HI_PVR_HEVC_SUPPORT) && (defined(HI_PVR_LINEARIZATION_SUPPORT) || defined(HI_PVR_POST_PROCESSING_SUPPORT) || defined(HI_PVR_SWITCH_REWIND_SUPPORT))
static HI_VOID PVRIndexFileOperationProcRef(PVR_CYC_FRAME_NUM_INFO* pstCycFrmInfo, HI_U32 u32IdxVersion,
                                                    HI_S32 s32HeadPosOffset, PVR_INDEX_ENTRY_NEW_S* pstEntry);

#endif

STATIC INLINE HI_VOID PVRIndexCycMoveReadFrameRewindChg(PVR_INDEX_HANDLE handle, HI_S32 s32Offset);

static HI_U32 PVRIndexFindValidEndFrame(PVR_FILE s32IdxFd, PVR_IDX_HEADER_INFO_S* pstHeaderInfo,
                                                    PVR_INDEX_PRIV_DATA_INFO_S* pstPrivInfo, HI_U64 u64TsFileSize, PVR_INDEX_ENTRY_NEW_S* pstEndEntry);
...
#endif
/***************************************************************************/
volatile PVR_INDEX_HANDLE PVRGetIndexHandle(HI_U32 idx)
{
    return &g_stPVRIndex[idx];
}
...
static HI_U32 PVRIndexGetRecTimeStamp(PVR_INDEX_HANDLE handle, const HI_MPI_DMX_REC_INDEX_S* pstRecIdx)
{
    ...
    if (HI_UNF_PVR_REC_INDEX_TYPE_VIDEO != handle->enIndexType)
    {
        ...
        {
            if ((PVR_INDEX_INVALID_PTSMS == pstRecIdx->u32PtsMs) || (PVR_INDEX_INVALID_PTSMS == pstTimeStamp->stLastIFrmPtsInfo.u32PtsMs))
            {
        ...
        if (PVR_INDEX_INVALID_PTSMS != pstRecIdx->u32PtsMs)
        {
    ...
    if (((HI_UNF_FRAME_TYPE_IDR == pstRecIdx->enFrameType) || (HI_UNF_FRAME_TYPE_I == pstRecIdx->enFrameType))
        && (PVR_INDEX_INVALID_PTSMS == pstTimeStamp->stLastIFrmPtsInfo.u32PtsMs))
    {
        pstTimeStamp->stLastIFrmPtsInfo.u32PtsMs = pstRecIdx->u32PtsMs;
    ...
    if (((HI_UNF_FRAME_TYPE_IDR == pstRecIdx->enFrameType) || (HI_UNF_FRAME_TYPE_I == pstRecIdx->enFrameType))
        && (PVR_INDEX_MAX_TIMESTAMP_BASE <= pstTimeStamp->u32FrmCnt))
    {
    ...
    if ((PVR_INDEX_INVALID_PTSMS == pstTimeStamp->stCurIFrmPtsInfo.u32PtsMs)
    ...
    if (HI_TRUE == pstTimeStamp->bShouldUpdateTimeStamp)
    {
        ...
        else
        {
            ...
            if ((0 != u32DeltaTime) && (u32DeltaFrameNum >= 1))
            {
                                                      ...
                                                      HI_U64 u64CycSize)
{
    ...
    {
        HI_WARN_PVR("End offset cover start offset return false!\n");
        return HI_FALSE;
    }
}
...
{
    HI_U32 i = 0;
    PVR_INDEX_HANDLE handleLocal = HI_NULL;
    ...
    {
        handleLocal= PVRGetIndexHandle(i);

        if (HI_TRUE == handleLocal->bIsRec)
        {
            if  (0 == strncmp(handleLocal->szIdxFileName, pIdxFileName,strlen(pIdxFileName)))
            {
                *handle = handleLocal;
...
{
    handle->pstPidCtrl->stPidInfo[handle->u32CurRecIdx].u64GlobalOffset -= u32ReadSize;
    PVR_INDEX_LOCK(&handle->stMutex);
    handle->pstPidCtrl->stPidInfo[handle->u32CurRecIdx].u64FileOffset-= u32ReadSize;
...
static HI_BOOL PVRIndexIsFilePlaying(const HI_CHAR* pIdxFileName, PVR_INDEX_HANDLE* handle)
{
    ...
    {
        handleLocal= PVRGetIndexHandle(i);
        if (HI_TRUE == handleLocal->bIsPlay)
        {
            if  (0 == strncmp(handleLocal->szIdxFileName, pIdxFileName, strlen(pIdxFileName)))
            {
                *handle = handleLocal;
...
static HI_BOOL PVRIndexIsFrameValid(HI_U32 u32Start, HI_U32 u32End, HI_U32 u32Last, HI_U32 u32FrmPos)
{
    ...
    if (u32Start < u32End)
    {
        if ((u32FrmPos >= u32Start) && (u32FrmPos <= u32End))
        {
            HI_PVR_DEBUG_EXIT();
            return HI_TRUE;
        }
    ...
    {
        /* rewind case */
        if (((u32FrmPos >= u32Start) && (u32FrmPos <= u32Last))
            || (u32FrmPos <= u32End))
        {
            HI_PVR_DEBUG_EXIT();
            return HI_TRUE;
...
STATIC HI_U32 PVRIndexCalcNewPos(HI_U32 u32StartFrame, HI_U32 u32EndFrame, HI_U32 u32LastFrame, HI_U32 u32FrmPos, HI_S32 s32Offset)
{
    ...
    if (HI_FALSE == PVRIndexIsFrameValid(u32StartFrame, u32EndFrame, u32LastFrame, u32FrmPos))
    {
#ifdef DBG_FRAME_POS
        HI_ERR_PVR("\033[1;45;37m!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! Info start !!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
        HI_ERR_PVR("u32StartFrame = %u, u32EndFrame = %u, u32LastFrame = %u\n", u32StartFrame, u32EndFrame, u32LastFrame);
...
STATIC INLINE HI_VOID PVRIndexCycMoveReadFrame(PVR_INDEX_HANDLE handle, HI_S32 s32Offset)
{
    ...
    handle->u32ReadFrame = PVRIndexCalcNewPos(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, handle->u32ReadFrame, s32Offset);
...
STATIC INLINE HI_VOID PVRIndexCycMoveReadFrameRewindChg(PVR_INDEX_HANDLE handle, HI_S32 s32Offset)
{
    ...
    {
        if ((handle->u32ReadFrame < handle->stLastFileInfo.stCycInfo.u32LastFrame) || ((HI_S32)(handle->u32ReadFrame + s32Offset) < handle->stLastFileInfo.stCycInfo.u32LastFrame))
        {
            for (i = 0; i < abs(s32Offset); i++)
            {
                if (s32Offset > 0)
                {
                    PVRIndexGetNextWhenRewindChg(handle, handle->u32ReadFrame, &handle->u32ReadFrame, HI_TRUE);
                }
                else
                {
                    PVRIndexGetNextWhenRewindChg(handle, handle->u32ReadFrame, &handle->u32ReadFrame, HI_FALSE);
                }
            }
    ...
    (HI_VOID)PVRIndexCycMoveReadFrame(handle, s32Offset);
...
static HI_S32 PVRIndexWriteIndexCycMgr(PVR_INDEX_HANDLE hIndex, PVR_CYC_MGR_S* pstCycMgr)
{
    ...
    PVR_CYC_HEADER_INFO_S stHeaderCycInfo;
    ...
    memset(&stHeaderCycInfo, 0, sizeof(stHeaderCycInfo));
...
static HI_S32 PVRIndexReadHeaderInfo(PVR_INDEX_HANDLE handle)
{
    ...
    s32Ret =PVRIndexGetHeaderInfo(handle->s32HeaderFd, &stIdxHeaderInfo);
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_PVR("Do not found the header info in this index file!\n");
        return HI_FAILURE;
    }
    else
    {
        ...
        if (0 == handle->stCycMgr.u32LastFrame)
        {
...
static HI_S32 PVRIndexWriteIndexInfo(PVR_INDEX_HANDLE hIndex, PVR_REC_INDEX_INFO_S* pstIdxInfo)
{
    ...
    memset(&stIdxHeaderInfo, 0, sizeof(stIdxHeaderInfo));
...
/* 检查index 文件某段空间的内容是否全为s32Value */
static INLINE HI_BOOL PVRIndexCheckMemValue(HI_U32* pu8addr, HI_U32 u32MemSize, HI_S32 s32Value)
{
    while(u32MemSize--)
    {
        if (*pu8addr++ != s32Value)
...
/*Check the last index of the index file is valid*/
static HI_BOOL PVRIndexCheckValidEndFrame(PVR_FILE s32IdxFd, HI_U32 u32EntryVersion, HI_U32 u32HeaderLen, HI_U32 u32EndFrame, PVR_INDEX_ENTRY_NEW_S* pstEndEntry)
{
    HI_U32 u32EntryLen = 0;
    HI_S32 s32Length = 0;
    HI_U32 u32IdxOffset = 0;
    HI_U32 u32TmpFrameSize = 0;
    HI_U64 u64EndEntryGlobalOffset = 0;
    HI_U64 u64TmpEntryGlobalOffset = 0;
    PVR_INDEX_ENTRY_NEW_S stTmpEntry;

    HI_PVR_LOG_ENTER();

    u32EntryLen = (0 == u32EntryVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_V1_S);
    /*check the last index data is all 0 or 1*/
    if (HI_TRUE == PVRIndexCheckMemValue((HI_U32*)pstEndEntry, u32EntryLen, 0))
    {
        HI_WARN_PVR("The End Entry of index file is  all 0x0!\n");
        return HI_FALSE;
    }
    if (HI_TRUE == PVRIndexCheckMemValue((HI_U32*)pstEndEntry, u32EntryLen, 0xffffffff))
    {
        HI_WARN_PVR("The End Entry of index file is  all 0xffffffff!\n");
        return HI_FALSE;
    }
    memset(&stTmpEntry, 0, sizeof(stTmpEntry));
    u32IdxOffset = u32HeaderLen + u32EntryLen * (u32EndFrame - 1);

    /*check the last index is valid by globaloffset and framesize*/
    s32Length = PVR_READ(&stTmpEntry, u32EntryLen, s32IdxFd, u32IdxOffset);
    if (s32Length != u32EntryLen)
    {
        HI_ERR_PVR("read penultimate frame failed! want: %u, read: %d\n", u32EntryLen, s32Length);
        return HI_FALSE;
    }

    u32TmpFrameSize = PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &stTmpEntry);
    u64TmpEntryGlobalOffset = PVRINDEX_GET_GLOBALOFFSET(u32EntryVersion, &stTmpEntry);
    u64EndEntryGlobalOffset = PVRINDEX_GET_GLOBALOFFSET(u32EntryVersion, pstEndEntry);
    if(u64EndEntryGlobalOffset != u64TmpEntryGlobalOffset + u32TmpFrameSize)
    {
        HI_ERR_PVR("The End Frame's Globaloffset:%llu not equal Penultimate Frame's Globaloffset:%llu plus FrameSize:%u!\n",
                    u64EndEntryGlobalOffset, u64TmpEntryGlobalOffset, u32TmpFrameSize);
        return HI_FALSE;
    }
                                        ...
                                        PVR_INDEX_PRIV_DATA_INFO_S* pstPrivInfo, HI_U64 u64TsFileSize, PVR_INDEX_ENTRY_NEW_S* pstEndEntry)
{
    ...
    HI_U32 u32CurEndNum = PVR_INDEX_ERR_INVALID;
    HI_U32 u32IdxOffset = 0;
    HI_U32 u32IndexEntryNum = 0;
    HI_U32 u32EntryVersion = 0;
    HI_U32 u32EntryLen = 0;
    HI_S32 s32Length = 0;
    HI_U64 u64IndexFileSize = 0;
    HI_U64 u64Offset = 0;

    HI_PVR_LOG_ENTER();

    u32EntryVersion = (pstHeaderInfo->u32Reserved & 0xF0000) >> 16;
    u32EntryLen = (0 == u32EntryVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_V1_S);

    if ((HI_S32)pvr_lseek(s32IdxFd, 0, SEEK_SET) < 0)
    {
        HI_ERR_PVR("Index file can't seek to 0\n");
        return HI_FAILURE;
    }

    u64IndexFileSize = (HI_U64)pvr_lseek(s32IdxFd, 0, SEEK_END);

    if (u64IndexFileSize <= (HI_U64)pstHeaderInfo->u32HeaderLen)
    {
        HI_ERR_PVR("index file error! indexFileSize: %llu, HeaderLen: %u\n", u64IndexFileSize, pstHeaderInfo->u32HeaderLen);
        return HI_FAILURE;
    }

    u32IndexEntryNum = (HI_U32)((u64IndexFileSize - (HI_U64)pstHeaderInfo->u32HeaderLen) / (HI_U64)u32EntryLen);

    if ((pstHeaderInfo->stCycInfo.u32EndFrame > (u32IndexEntryNum - 1))
            || (pstHeaderInfo->stCycInfo.u32LastFrame > (u32IndexEntryNum - 1)))
    {
        HI_WARN_PVR("HeadInfo's CycInfo.EndFrame(%u) or LastFrame(%u) > indexEntryNum(%u).\n",
                pstHeaderInfo->stCycInfo.u32EndFrame, pstHeaderInfo->stCycInfo.u32LastFrame, (u32IndexEntryNum - 1));
        pstHeaderInfo->stCycInfo.u32EndFrame = u32IndexEntryNum - 1;
        pstHeaderInfo->stCycInfo.u32LastFrame = u32IndexEntryNum - 1;
    }

    /* Find the last valid index in the index file*/
    while(pstHeaderInfo->stCycInfo.u32EndFrame >= pstHeaderInfo->stCycInfo.u32StartFrame)
    {
        u32IdxOffset = pstHeaderInfo->u32HeaderLen + u32EntryLen * pstHeaderInfo->stCycInfo.u32EndFrame;
        s32Length = PVR_READ(pstEndEntry, u32EntryLen, s32IdxFd, u32IdxOffset);
        if (s32Length != u32EntryLen)
        {
        ...
        if (HI_TRUE != PVRIndexCheckValidEndFrame(s32IdxFd, u32EntryVersion, pstHeaderInfo->u32HeaderLen, pstHeaderInfo->stCycInfo.u32EndFrame, pstEndEntry))
        {
            pstHeaderInfo->stCycInfo.u32EndFrame--;
            pstHeaderInfo->stCycInfo.u32LastFrame--;
            continue;
        }
        else
        {
            u64Offset = PVRINDEX_GET_OFFSET(u32EntryVersion, pstEndEntry);
            u32FrameSize = PVRINDEX_GET_FRAMESIZE(u32EntryVersion, pstEndEntry);
            u32CurEndNum = pstHeaderInfo->stCycInfo.u32EndFrame;
            break;
        }
    }

    if (PVR_INDEX_ERR_INVALID == u32CurEndNum)
    {
        HI_ERR_PVR("Find the last valid index failed!\n");
        return HI_FAILURE;
    }

    while (u64Offset + u32FrameSize > u64TsFileSize)
    {
        if (pstHeaderInfo->stCycInfo.u32StartFrame == u32CurEndNum)
        {
            HI_ERR_PVR("Find end frm to start! endFrmNum: %u, startFrmNum: %u\n", u32CurEndNum, pstHeaderInfo->stCycInfo.u32StartFrame);
            return HI_FAILURE;
        }
        ...
        u32CurEndNum--;
        u32IdxOffset = pstHeaderInfo->u32HeaderLen + u32EntryLen * u32CurEndNum;
        s32Length = PVR_READ(pstEndEntry, u32EntryLen, s32IdxFd, u32IdxOffset);
        if (s32Length != u32EntryLen)
        {
            HI_ERR_PVR("read End frame failed! want: %u, read: %d\n", u32EntryLen, s32Length);
    ...
    pstHeaderInfo->stCycInfo.u32EndFrame = u32CurEndNum;
    pstHeaderInfo->stCycInfo.u32LastFrame = u32CurEndNum;
...
static HI_S32 PVRIndexCorrectFile(HI_CHAR* pszFileName, PVR_INDEX_HANDLE handle)
{
    ...
    /*读取索引头信息*/
    s32Ret = PVRIndexGetHeaderInfo((HI_S32)s32IndexFd, &stHeaderInfo);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32Ret = PVRIndexFindValidEndFrame(s32IndexFd, &stHeaderInfo, pstPrivInfo, u64TsFileSize, &stEndEntry);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("get valid endFrame failed! ret = 0x%x\n",s32Ret);
        PVR_CLOSE(s32IndexFd);
        return s32Ret;
    }
    ...
    if (u32EntryVersion >= 2)
    {
        ...
        for (; i >= 0; i--)
        {
            ...
            {
                pstPIDInfo->u32EndFrame = stHeaderInfo.stCycInfo.u32EndFrame;
                pstPIDInfo->u32EndDispTime = PVRINDEX_GET_DISPTIME(u32EntryVersion, &stEndEntry);
                break;
            }
        }
    ...
    (HI_VOID)PVR_SET_MAXFILE_SIZE(tsFd, u64TsFileSize);

    u64TsTruncateSize = ((HI_U64)(PVRINDEX_GET_OFFSET(u32EntryVersion, &stEndEntry) + PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &stEndEntry)) / u32AlignLen + 1) * u32AlignLen;
    HI_WARN_PVR("u64TsTruncateSize: %llu, u64TsFileSize: %llu \n", u64TsTruncateSize, u64TsFileSize);
    s32Ret = PVR_FTRUNCATE64(tsFd, u64TsTruncateSize, HI_FALSE);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    if (HI_NULL != handle)
    {
        handle->stCycMgr.u32EndFrame = stHeaderInfo.stCycInfo.u32EndFrame;
        handle->stCycMgr.u32LastFrame = stHeaderInfo.stCycInfo.u32EndFrame;
        handle->stCycMgr.u32CycTimes = PVRINDEX_GET_CYCTIMES(u32EntryVersion, &stEndEntry);
#ifdef HI_PVR_EXTRA_BUF_SUPPORT
        if (HI_NULL != handle->pstExtraBuffer)
        {
            handle->pstExtraBuffer->stFileCycInfo.u32EndFrame = stHeaderInfo.stCycInfo.u32EndFrame;
            handle->pstExtraBuffer->stFileCycInfo.u32LastFrame = stHeaderInfo.stCycInfo.u32EndFrame;
        }
#endif
    }
...
#endif
    {
        s32Ret = PVRIndex_CacheWriteIdx1(handle, pstEntry, s32Num, u32Offset, bWriteFlag);
                                           ...
                                           HI_U32 *pu32NextFrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(pstCycMgr->u32StartFrame, pstCycMgr->u32EndFrame, pstCycMgr->u32LastFrame, u32CurFrameNum))
    {
    ...
    while (1)
    {
        ...
        if (HI_TRUE != PVRIndexIsFrameValid(pstCycMgr->u32StartFrame, pstCycMgr->u32EndFrame, pstCycMgr->u32LastFrame, s32NextFrameNum))
        {
        ...
        if (HI_TRUE == PVRINDEX_IS_CERTAINTYPE(handle->u32EntryVersion, u16FrameTypeAndGop, u32FrameType))
        {
            break;
        }
...
static HI_VOID PVRIndexCheckReadFrmValidWhenTimeshift(PVR_INDEX_HANDLE hIndex)
{
    ...
    if (HI_FALSE == hIndex->bIsPlay)
    {
        HI_WARN_PVR("Did not play!\n");
        return ;
    }

    if (HI_TRUE == hIndex->bRecReachPlay)
    {
        HI_WARN_PVR("Record reach play!\n");
        return ;
    }

    if (PVRIndexIsFrameValid(hIndex->stCycMgr.u32StartFrame, hIndex->stCycMgr.u32EndFrame, hIndex->stCycMgr.u32LastFrame, hIndex->u32ReadFrame) == 0)
    {
                                   ...
                                   PVR_INDEX_ENTRY_NEW_S *pstIdxEntry)
{
    ...
    if (PVR_INDEX_REWIND_BY_TIME == pstCycMgr->enRewindType)
    {
        ...
        if (HI_TRUE == hIndex->bTimeRewinding)
        {
            HI_WARN_PVR("Is time rewinding!\n");
            return HI_FALSE;
        }
...
static HI_VOID PVRIndexAddToRefList(PVR_REC_REF_LIST_S* pstRefList, HI_S32 s32CurPoc, HI_U32 u32FrmNum)
{
    ...
    if (NULL == pstRefList)
    {
        HI_WARN_PVR("Input parameter \"pstRefList\" is NULL!\n");
        return ;
    }

    //CNComment:参考列表已经到达最大值了，则清掉开始的参考节点
    if (PVR_INDEX_REF_LIST_NODE_NUM == pstRefList->u32Write)
    {
                                               ...
                                               PVR_INDEX_ENTRY_NEW_S *pstIdxEntry)
{
    ...
    HI_U64 u64Offset = 0;
    HI_U16 u16FrameTypeAndGop = 0;
    HI_U32 u32Version __attribute__((unused)) = hIndex->u32EntryVersion;
#ifdef HI_PVR_HEVC_SUPPORT
    HI_U32 i = 0;
    HI_U32 u32RefNum = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    ...
    hIndex->u32DmxClkTimeMs = pstDmxIndexInfo->u32DataTimeMs + hIndex->u32OverFlowTimeMs;
    ...
    else
    {
        ...
        else
        {
            ...
            if (((hIndex->u32FRollTime * PVR_INDEX_SCD_WRAP_MS + u32TimeNow) < u32DisplayTimeMs)
                && (((hIndex->u32FRollTime + 1) * PVR_INDEX_SCD_WRAP_MS) < u32DisplayTimeMs + 1500))
            {
                hIndex->u32FRollTime++;
            ...
            {
                /*over flow*/
                u32CurFrmTimeMs = hIndex->u32OverFlowTimeMs + u32TimeNow;
            }
...
#endif
    HI_WARN_PVR("u32DataTimeMs:%ums; u32LastDispTime:%ums; u32CurFrmTimeMs:%ums, u32DeltaDispTimeMs:%ums, u32TotalRecTimeMs:%ums; u32OverFlowTimeMs:%ums\n",
        pstDmxIndexInfo->u32DataTimeMs, hIndex->u32LastDispTime, u32CurFrmTimeMs, hIndex->u32DeltaDispTimeMs, hIndex->u32TotalRecTimeMs, hIndex->u32OverFlowTimeMs);
    /* In case the recording data lose, compensate the current time */
    if (u32CurFrmTimeMs >= 1000 + hIndex->u32LastDispTime)
    {
...
static HI_U32 PVRIndexGetRecPidNumByFrmNum(PVR_INDEX_HANDLE hIndex, HI_U32 u32FrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(hIndex->stCycMgr.u32StartFrame, hIndex->stCycMgr.u32EndFrame, hIndex->stCycMgr.u32LastFrame, u32FrameNum))
    {
...
static HI_VOID PVRIndexDealTheRecPidInfoWhenRewindForPidChange(PVR_INDEX_HANDLE hIndex, PVR_REC_INDEX_INFO_S* pstIdxInfo, HI_U32 u32FrmNumBeforeIFrm)
{
    ...
    if (-1 == u32PidNum)
    {
        HI_ERR_PVR("get start Frame's Pid Info failed! \n");
        return;
    }
...
static HI_BOOL PVRIndexCurFrameIsInCurrentRecPidInfoList(PVR_INDEX_HANDLE hIndex, HI_U32 u32CurFrmNum)
{
    ...
    HI_WARN_PVR("Current frame display time is not correct!\n");
    return HI_FALSE;
}
#endif
                                      ...
                                      HI_U64 u64NextEndOffset)
{
    ...
    if (HI_SUCCESS == s32Ret)
    {
        ...
        if (HI_FALSE == PVRIndexCurFrameIsInCurrentRecPidInfoList(hIndex, pstCycMgr->u32StartFrame))
        {
...
static HI_VOID PVRIndexUpdateIdxInfo(PVR_INDEX_HANDLE hIndex, PVR_CYC_MGR_S* pstCycMgr, PVR_REC_INDEX_INFO_S* pstIdxInfo, PVR_INDEX_ENTRY_NEW_S* pstEndEntry)
{
    HI_U16 u16FrameTypeAndGop = 0;
#ifdef HI_PVR_PID_CHANGE_SUPPORT
    PVR_INDEX_PID_INFO_S* pstRecPidInfo = HI_NULL;
                                                        ...
                                                        PVR_INDEX_ENTRY_NEW_S *pstEndEntry)
{
    ...
    PVRIndexUpdateIdxInfo(hIndex, pstCycMgr, pstIdxInfo, pstEndEntry);
                                       ...
                                       PVR_INDEX_ENTRY_NEW_S *pstIdxEntry)
{
    ...
    if ((0 == pstCycMgr->u32MaxCycTimeInMs) && (0 == pstCycMgr->u64MaxCycSize))
    {
...
static HI_S32 PVRIndexCheckLinearConfigForAppendRec(PVR_IDX_HEADER_INFO_S* pstHeaderInfo, HI_U32* pU32ValidStartFrm, PVR_FILE s32Fd)
{
    HI_S32 s32ReadLen = 0;
    ...
    s32ReadLen = PVR_READ((HI_VOID *)&stPrivData, sizeof(stPrivData), s32Fd, PVR_GET_PRIVATE_DATA_OFFSET(*pstHeaderInfo));
...
STATIC INLINE HI_S32 PVRIndexGetNextIEntry(const PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S* pEntry)
{
    ...
    while (1)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_WARN_PVR("Get next entry failed!\n");
            return ret;
        }
...
STATIC INLINE HI_S32 PVRIndexGetPreIEntry(const PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S* pEntry)
{
    ...
    */
    while (1)
    {
        ...
        if (HI_SUCCESS != ret)
        {
            HI_WARN_PVR("Pvr index get PreEntry failed!\n");
            return ret;
        }
        u16FrameTypeAndGop = PVRINDEX_GET_FRAMETYPEANDGOP(handle->u32EntryVersion, pEntry);
        /* I frame, and not found the frame upflow flag*/
...
static HI_S32 PVRIndexGetNextWhenRewindChg(PVR_INDEX_HANDLE handle, HI_U32 u32CurFrm, HI_U32* pu32NextFrm, HI_BOOL bDirection)
{
    ...
    if ((HI_TRUE != handle->stLastFileInfo.bOnlineLinearization) || (HI_TRUE != handle->bNeedCheckRead))
    {
        ...
        { u32NextFrm = PVRIndexCalcNewPos(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, u32CurFrm, 1); }
        else
        { u32NextFrm = PVRIndexCalcNewPos(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, u32CurFrm, -1); }
    }
    else
    {
        ...
        PVR_Index_CheckCurrentReadFrameIsInValidWhenRewindChg(handle);
    }
...
static HI_VOID PVRIndexCheckLastFileInfoValid(PVR_CYC_MGR_S* pstLastCycMgr, PVR_INDEX_ENTRY_NEW_S* pstIdxBuf, HI_U32* pu32Delta)
{
    ...
    PVR_INDEX_ENTRY_NEW_S* pstCurEndEntry = HI_NULL;
    PVR_INDEX_ENTRY_NEW_S* pstPreEndEntry = HI_NULL;
    PVR_INDEX_ENTRY_NEW_S* pstStartEntry = HI_NULL;
    *pu32Delta = 0;
...
static HI_VOID PVRIndexProcPIDChangeInfoWhenRewindChg(PVR_INDEX_HANDLE handle)
{
    ...
    for (i = 0; i < pstRecPidCtrl->u32UsedNum; i++)
    {
        ...
        {
            PVRIndexUpdateFrameNumWhenRewindChg(pstLastCycMgr, &pstCurPidInfo->u32StartFrame, 0);
            HI_PVR_LOG_EXIT();
            return;
        }
                ...
                && (pstCurPidInfo->u32EndDispTime < PVRINDEX_GET_DISPTIME(handle->u32EntryVersion, &handle->stEndFrame)))
        {
            ...
            PVRIndexUpdateFrameNumWhenRewindChg(pstLastCycMgr, &pstCurPidInfo->u32EndFrame, 0);
        }
        else
        {
            /*当前PID数据全在有效区间内*/
...
#if (defined(HI_PVR_SWITCH_REWIND_SUPPORT) || defined(HI_PVR_LINEARIZATION_SUPPORT) || defined(HI_PVR_POST_PROCESSING_SUPPORT)) && defined(HI_PVR_PID_CHANGE_SUPPORT)
static HI_VOID PVRIndexUpdateFrameNumWhenRewindChg(PVR_CYC_MGR_S *pstLastCycMgr, HI_U32 *pu32FrameNum, HI_U32 u32HeadPosOffset)
{
    HI_U32 u32CurFrameNum = *pu32FrameNum;

    HI_PVR_LOG_ENTER();
    if (pstLastCycMgr->u32EndFrame < pstLastCycMgr->u32StartFrame && pstLastCycMgr->u32StartFrame < pstLastCycMgr->u32LastFrame)
    {
        /*|--E--S--L|*/
        /*
         *    x' = L-E+x [0,E]; x'=x-E-1 [S,L];  x' = LastFile.u32LastFrmNum (E,S)
    ...
    if (pstLastCycMgr->u32StartFrame < pstLastCycMgr->u32EndFrame && pstLastCycMgr->u32EndFrame < pstLastCycMgr->u32LastFrame)
    {
        /*|-S--E--L|*/
        /*
         *x' = x+L-E [S,E]; x'=LastFile.u32LastFrmNum (E,L]; x'= S + L - W(0,S)
    ...
    if ((HI_S32)u32HeadPosOffset > 0)
    {/*此处处理文件头部truncate时的偏移量*/
        if (*pu32FrameNum > u32HeadPosOffset)
        {
            *pu32FrameNum -= u32HeadPosOffset;
        }
        else
        {
            HI_ERR_PVR("invalid Pid EndFrame, old: %u, new: %u\n", u32CurFrameNum, *pu32FrameNum+pstLastCycMgr->u32LastFrame);
            *pu32FrameNum += pstLastCycMgr->u32LastFrame;
        }
    }
    HI_PVR_LOG_EXIT();
...
static HI_VOID PVRIndexProcPIDChangeInfoWhenLinearizationOrTruncate(PVR_INDEX_REC_PID_CTRL_S *pstRecPidCtrl, PVR_CYC_FRAME_NUM_INFO* pstCycFrm, HI_U32 u32StartDispTime, HI_BOOL bTimeshiftToRec, HI_U32 u32HeadOffset)
{
    ...
    stCycMgr.u32StartFrame = pstCycFrm->u32StartFrame;
    stCycMgr.u32EndFrame = pstCycFrm->u32EndFrame;
    stCycMgr.u32LastFrame = pstCycFrm->u32LastFrame;
    ...
    for (i = 0; i < pstRecPidCtrl->u32UsedNum; i++)
    {
        ...
        /*如果当前PID的EndFrameTime < IndexHandle中的startFrameTime，则认为该段PID已被完全覆盖，不进行处理。*/
        if (pstCurPidInfo->u32EndDispTime < u32StartDispTime)
        {
            continue;
        }

        if (HI_TRUE != bTimeshiftToRec)
        {
            if ((pstCurPidInfo->u32StartDispTime < u32StartDispTime)
                        && (pstCurPidInfo->u32EndDispTime > u32StartDispTime))
            {
                /*如果当前PID的一部分数据在有效区间内*/
                pstCurPidInfo->u32StartFrame = 0;
                pstCurPidInfo->u32StartDispTime = u32StartDispTime;
                PVRIndexUpdateFrameNumWhenRewindChg(&stCycMgr, &pstCurPidInfo->u32EndFrame, u32HeadOffset);
            }
            else
            {
                /*当前PID数据全在有效区间内*/
                PVRIndexUpdateFrameNumWhenRewindChg(&stCycMgr, &pstCurPidInfo->u32StartFrame, u32HeadOffset);
                PVRIndexUpdateFrameNumWhenRewindChg(&stCycMgr, &pstCurPidInfo->u32EndFrame, u32HeadOffset);
            }
        }
        else
        {
            ...
            pstCurPidInfo->u32StartFrame = (pstCurPidInfo->u32StartFrame > u32HeadOffset) ? (pstCurPidInfo->u32StartFrame - u32HeadOffset) : 0;
            pstCurPidInfo->u32EndFrame = (pstCurPidInfo->u32EndFrame > u32HeadOffset) ? (pstCurPidInfo->u32EndFrame - u32HeadOffset) : 0;
        }
    }
...
{
    HI_S32 s32Ret = 0;
    HI_U32 u32Read = 0, u32Write = 0, u32RewindInCache = 0, u32IdxCount = 0;
    HI_BOOL bRewindInCache = HI_FALSE;
    PVR_CYC_MGR_S        stCycMgr;
    PVR_REC_INDEX_INFO_S stIdxInfo;
    ...
    else if ((u32IdxCount > PVR_INDEX_WRITE_FILE_LIMIT) || (HI_TRUE == bDirectWrite))
    {
...
HI_S32 PVR_Index_SaveToCache(PVR_INDEX_HANDLE hIndex, HI_MPI_DMX_REC_INDEX_S* pstDmxIndexInfo)
{
    ...
    if (HI_TRUE == hIndex->bRecReachFix)
    {
        HI_PVR_DEBUG_EXIT();
        return HI_SUCCESS;
    }

    if(HI_UNF_FRAME_TYPE_BLA == pstDmxIndexInfo->enFrameType)
    {
        pstDmxIndexInfo->enFrameType = HI_UNF_FRAME_TYPE_IDR;
    }

    if(HI_UNF_FRAME_TYPE_CRA == pstDmxIndexInfo->enFrameType)
    {
        pstDmxIndexInfo->enFrameType = HI_UNF_FRAME_TYPE_I;
    }
    ...
    if (PVR_INDEX_WRITE_CACHE_NUM == pstIdxBuf->u32UsedNum)
    {
        HI_WARN_PVR("pstIdxBuf->u32UsedNum == PVR_INDEX_WRITE_CACHE_NUM\n");
        return HI_FAILURE;
    }
    ...
    if (PVR_INDEX_REWIND_BY_BOTH == hIndex->stCycMgr.enRewindType)
    {
    ...
    if (HI_TRUE == PVR_IDX_ENABLE_REWIND(hIndex))      //支持回绕
    {
        if (HI_TRUE == PVRIndexCheckRewind(hIndex, pstCycMgr, &stIndexEntry))      //如果发生回绕
        {
    ...
    if (HI_TRUE == hIndex->bRewindFlagForIndex)
    {
...
HI_S32 PVRIndexFlushToFile(PVR_INDEX_HANDLE hIndex)
{
    ...
    if (PVR_FILE_INVALID_FILE != hIndex->s32WriteFd)
    {
        PVR_FSYNC(hIndex->s32WriteFd);
    }

    if (PVR_FILE_INVALID_FILE != hIndex->s32HeaderFd)
    {
...
HI_S32 PVR_Index_SaveFramePosition(PVR_INDEX_HANDLE hIndex, HI_MPI_DMX_REC_INDEX_S* pstDmxIndexInfo)
{
    ...
    if (HI_TRUE == PVR_IDX_ENABLE_REWIND(hIndex))
    {
...
HI_S32 PVR_Index_RecAppendCheck(HI_UNF_PVR_REC_ATTR_S* pstRecAttr, PVR_RECORD_LAST_INFO_S* pstLastInfo)
{
    ...
    s32Fd = PVR_OPEN(szIndexName, PVR_FOPEN_MODE_DATA_READ);
    if (PVR_FILE_INVALID_FILE == s32Fd)
    {
    ...
    (HI_VOID)PVR_SET_MAXFILE_SIZE(tsFd, u64TsFileSize);
    s32Ret = PVR_FTRUNCATE64(tsFd, u64TsFileSize, HI_FALSE);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    s32ReadLen = PVR_READ((HI_U8*)&stHeaderInfo, sizeof(stHeaderInfo), s32Fd, 0);
    ...
    s32ReadLen = PVR_READ((HI_U8 *)&stStartEntry, sizeof(stStartEntry), s32Fd, (HI_U64)((HI_U64)stHeaderInfo.stCycInfo.u32StartFrame*sizeof(stStartEntry)+ stHeaderInfo.u32HeaderLen));
    ...
    s32ReadLen = PVR_READ((HI_U8 *)&stValidStartEntry, sizeof(stValidStartEntry), s32Fd, (HI_U64)((HI_U64)u32ValidStartFrame * sizeof(stValidStartEntry) + stHeaderInfo.u32HeaderLen));
    ...
    {
        s32ReadLen = PVR_READ((HI_U8 *)&stEndEntry, sizeof(stEndEntry), s32Fd, (HI_U64)((HI_U64)stHeaderInfo.stCycInfo.u32EndFrame * sizeof(stEndEntry) + stHeaderInfo.u32HeaderLen));
        if (s32ReadLen != sizeof(stEndEntry))
        {
            stHeaderInfo.stCycInfo.u32EndFrame--;
            continue;
        }
        else if(HI_TRUE != PVRIndexCheckValidEndFrame(s32Fd, u32Version, stHeaderInfo.u32HeaderLen, stHeaderInfo.stCycInfo.u32EndFrame, &stEndEntry))
        {
            stHeaderInfo.stCycInfo.u32EndFrame--;
            continue;
        }
    ...
    s32ReadLen = PVR_READ((HI_U8*) & (pstLastInfo->stRecIdxInfo), sizeof(PVR_REC_INDEX_INFO_S), s32Fd, (HI_U64)(PVR_GET_IDX_INFO_OFFSET(stHeaderInfo)));
...
HI_U64 PVR_Index_CACorrectFileOffset(PVR_INDEX_HANDLE handle, HI_U64 u64GlobalOffset, PVR_RECORD_LAST_INFO_S* pstLastInfo)
{
    ...
    s32Fd = PVR_OPEN(handle->szIdxFileName, PVR_FOPEN_MODE_INDEX_READ);
    if (PVR_FILE_INVALID_FILE == s32Fd)
    {
    ...
    {
        s32ReadLen = PVR_READ((HI_U8 *)&stEndEntry, sizeof(stEndEntry), s32Fd, (HI_U64)((HI_U64)pstLastInfo->u32EndFrame * sizeof(stEndEntry)+ stHeaderInfo.u32HeaderLen));
        if (s32ReadLen != sizeof(stEndEntry))
        {
...
HI_S32 PVR_Index_RecAppendConfigure(PVR_INDEX_HANDLE hIndex, PVR_RECORD_LAST_INFO_S* pstInfo)
{
    ...
    PVR_TS_CACHE_SetHeadGlobleOffset(&hIndex->pstExtraBuffer->stTsCacheBufInfo,pstInfo->u64GlobalOffset);
#endif
...
HI_S32 PVR_Index_IfOffsetInWriteCache(PVR_INDEX_HANDLE  handle, HI_U32 u32Offset, HI_U32 u32Size)
{
    ...
    if (0 == handle->stIdxWriteCache.u32BufferLen)
    {
        HI_PVR_DEBUG_EXIT();
        return 0;
    }
    ...
    if ((0 != handle->stIdxWriteCache.u32BufferLen ) && (0 != handle->stIdxWriteCache.u32UsedSize))
    {
...
HI_S32 PVR_Index_FlushIdxWriteCache(PVR_INDEX_HANDLE handle)
{
    ...
    if (0 != handle->stIdxWriteCache.u32UsedSize)
    {
        //lint -e774
...
STATIC HI_S32 PVRIndexOpenFds(const HI_CHAR* pIdxFileName, PVR_INDEX_HANDLE handle)
{
    HI_PVR_LOG_ENTER();
    ...
    if (HI_TRUE == handle->bIsRec)
    {
        handle->s32WriteFd = PVR_OPEN(pIdxFileName, PVR_FOPEN_MODE_INDEX_WRITE);
    ...
    if (PVR_CHECK_FILE_EXIST(pIdxFileName))
    {
        handle->s32ReadFd = PVR_OPEN(pIdxFileName, PVR_FOPEN_MODE_INDEX_READ);
    ...
    handle->s32SeekFd = PVR_OPEN(pIdxFileName, PVR_FOPEN_MODE_INDEX_READ);
    ...
    handle->s32HeaderFd = PVR_OPEN(pIdxFileName, PVR_FOPEN_MODE_INDEX_BOTH);
...
STATIC HI_VOID PVRIndexCloseFds(PVR_INDEX_HANDLE handle)
{

    HI_PVR_LOG_ENTER();

    /* close index file */
    if (handle->s32ReadFd != PVR_FILE_INVALID_FILE)
    {
        PVR_CLOSE(handle->s32ReadFd);
        handle->s32ReadFd = PVR_FILE_INVALID_FILE;
    }

    if (handle->s32SeekFd != PVR_FILE_INVALID_FILE)
    {
        PVR_CLOSE(handle->s32SeekFd);
        handle->s32SeekFd = PVR_FILE_INVALID_FILE;
    }

    if (handle->s32HeaderFd != PVR_FILE_INVALID_FILE)
    {
        PVR_CLOSE(handle->s32HeaderFd);
        handle->s32HeaderFd = PVR_FILE_INVALID_FILE;
    }

    if (handle->s32WriteFd != PVR_FILE_INVALID_FILE)
    {
        PVR_CLOSE(handle->s32WriteFd);
        handle->s32WriteFd = PVR_FILE_INVALID_FILE;
    }
                                     ...
                                     HI_BOOL *pIsNoIdx)
{
    ...
    if (PVR_REC_MAX_CHN_NUM < chnID)
    {
        HI_ERR_PVR("parameter chnID is too large! \n");
        return HI_NULL_PTR;
    }
    ...
    handle = PVRGetIndexHandle(chnID);
    ...
    ret = PVRIndexOpenFds(szIndexName, handle);
    if (HI_FAILURE == ret)
    {
        HI_ERR_PVR("PVR open Index File for read failed !\n");
        goto ErrorExit0;
    ...
    else
    {
        ...
        if (HI_UNF_PVR_REC_INDEX_TYPE_BUTT == handle->pstPidCtrl->u32IndexType)
        {
...
PVR_INDEX_HANDLE PVR_Index_CreatRec(HI_U32 chnID, HI_UNF_PVR_REC_ATTR_S* pstRecAttr)
{
    ...
    if (HI_TRUE == PVRIndexIsFilePlaying(szIndexName, &handle))
    {
    ...
    handle = PVRGetIndexHandle(i + PVR_PLAY_MAX_CHN_NUM);
    ...
    if ((0 == handle->stCycMgr.u64MaxCycSize) && (0 < handle->stCycMgr.u32MaxCycTimeInMs))
    {
    ...
    s32Ret = PVRIndexOpenFds(szIndexName, handle);
    ...
    u32RecBufSize = ((HI_PVR_EXTRA_TSBUF_SIZE * 1024LLU) / PVR_FIFO_WRITE_BLOCK_SIZE(handle->u32TsPacketLen)) * PVR_FIFO_WRITE_BLOCK_SIZE(handle->u32TsPacketLen);
    if ((u32RecBufSize < (940 * 1024)) || (u32RecBufSize > (94 * 1024 * 1024)))
    {
        HI_ERR_PVR("config extra record buffer size(%u) is too small. \n",u32RecBufSize);
        u32RecBufSize = 940 * 1024;
    }
#else
    u32RecBufSize = 2 * handle->u32DavBufSize;
#endif

    if (HI_SUCCESS != PVR_TS_CACHE_BUFFER_Init(u32RecBufSize, &handle->pstExtraBuffer->stTsCacheBufInfo))
    {
    ...
    if (HI_NULL == handle->stRefList.pstRefHead)
    {
...
ErrorExit:
    ...
    if (HI_NULL != handle->pstExtraBuffer)
    {
        if (HI_NULL != handle->pstExtraBuffer->stTsCacheBufInfo.pu8Addr)
        {
...
HI_S32 PVR_Index_Destroy(PVR_INDEX_HANDLE handle, HI_U32 u32PlayOrRec)
{
...
#ifndef HI_PVR_EXTRA_BUF_SUPPORT
    if ((PVR_INDEX_PLAY != u32PlayOrRec) && (-1 != handle->stCycMgr.u32LastFrame))
    {
    ...
    if ((HI_TRUE == handle->bIsPlay) || (HI_TRUE == handle->bIsRec))
    {
    ...
    if ((0xffffffff != handle->u32RecPicParser) && (PVR_REC_MAX_CHN_NUM > handle->u32RecPicParser))
    {
...
HI_S32 PVR_Index_PrepareHeaderInfo(PVR_INDEX_HANDLE handle, HI_U32 u32UsrDataLen, HI_U32 u32Vtype)
{
    ...
    s32WriteRet = PVR_WRITE((HI_VOID*)pTmpBuff, (size_t)u32HeadInfoSize, handle->s32HeaderFd, 0);
    if ((HI_S32)u32HeadInfoSize != s32WriteRet)
    {
...
HI_S32 PVR_Index_GetUsrDataInfo(HI_S32 s32Fd, HI_U8* pBuff, HI_U32 u32BuffSize, HI_U32* pUsrDataLen)
{
    ...
    s32Ret = PVRIndexGetHeaderInfo(s32Fd, &stIdxHeaderInfo);
    if (HI_SUCCESS != s32Ret)
    {
...
HI_BOOL PVR_Index_CheckSetRecReachPlay(PVR_INDEX_HANDLE handle, HI_U32 u32MinDistance)
{
    ...
    {
        PVR_INDEX_UNLOCK(&handle->stMutex);
        HI_WARN_PVR("Is not rewind file!\n");
        return HI_FALSE;
    }
...
HI_S32 PVR_Index_SeekToTime(PVR_INDEX_HANDLE handle, HI_U32 u32TimeMs)
{
    ...
    if (HI_TRUE == PVR_IDX_ENABLE_REWIND(handle)) /* rewind */
    {
        if (HI_FALSE == PVRIndexIsFrameValid(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, frameToSeek))
        {
    ...
    s32Ret = PVR_Index_SeekByFrame2I(handle, (HI_S32)frameToSeek, SEEK_SET);
...
HI_S32 PVR_Index_SeekByTime(PVR_INDEX_HANDLE handle, HI_S64 s64Offset, HI_S32 whence, HI_U32 u32CurPlayTime)
{
    ...
    HI_U32 u32CurFrmTime = 0;
    HI_U32 u32StartFrmTime = 0;
    HI_U32 u32EndFrmTime = 0;
    HI_U32 u32SeekToTime = 0;
    HI_U32 u32StartFrmPos = 0;
    HI_U32 u32EndFrmPos = 0;
...
HI_S32 PVR_Index_SeekToPauseOrStart(PVR_INDEX_HANDLE handle)
{
    HI_S32 ret = HI_SUCCESS;
    PVR_INDEX_ENTRY_NEW_S stStartEntry;
    HI_U32 u32StartFrameNum = 0;
    HI_U32 u32SeekToPos = 0;
    ...
    if (PVR_INDEX_PAUSE_INVALID_OFFSET != handle->u64PauseOffset)
    {
        ...
        u32StartFrameNum = PVRIndexCalcNewPos(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, handle->stCycMgr.u32StartFrame, 20);
...
HI_S32 PVR_Index_GetRecStatus(PVR_INDEX_HANDLE handle, HI_UNF_PVR_REC_STATUS_S* pstRecStatus)
{
    ...
    {
        if (0 != handle->stCycMgr.u32MaxCycTimeInMs)
        {
...
HI_S32 PVR_Index_PlayGetFileAttrByFileName(const HI_CHAR* pFileName, PVR_INDEX_HANDLE handle, HI_UNF_PVR_FILE_ATTR_S* pAttr)
{
    ...
    HI_CHAR           szIndexName[PVR_MAX_FILENAME_LEN] = {0};
    HI_S32            fdIdx = 0;
    HI_U32            u32EntryVersion __attribute__((unused)) = 0;
    PVR_INDEX_ENTRY_NEW_S stStartEntry;
    PVR_INDEX_ENTRY_NEW_S stEndEntry;
    PVR_IDX_HEADER_INFO_S stIdxHeaderInfo;
    ...
    if (HI_NULL != handle)
    {
        ...
        if (-1 == handle->stCycMgr.u32LastFrame)
        {
    ...
    else
    {
        ...
        s32Ret = PVRIndexGetHeaderInfo(fdIdx, &stIdxHeaderInfo);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("No Header Info in index File:%s\n", szIndexName);
            PVR_CLOSE(fdIdx);
...
HI_S32 PVR_Index_GetVtype(PVR_INDEX_HANDLE handle)
{
    ...
    if (HI_SUCCESS != s32Ret)
    {
        HI_WARN_PVR("Could not get header info!\n");
        return HI_FAILURE;
    }
    else
    {
...
HI_S32 PVR_Index_GetFBwardIPBFrameNum(PVR_INDEX_HANDLE handle, PVR_INDEX_DIRECTION_E enDirection, HI_U32 u32FrameType, HI_U32 u32CurFrameNum, HI_U32* pu32NextFrameNum)
{
    ...
    HI_PVR_DEBUG_ENTER();
    s32Ret = PVRIndexGetFBwardIPBFrameNum(handle, enDirection, u32FrameType, u32CurFrameNum, pu32NextFrameNum);
    HI_PVR_DEBUG_EXIT();
    return s32Ret;
}
HI_S32 PVR_Index_PushStartWhenTsLead(PVR_INDEX_HANDLE handle, HI_U64 u64CurFileSize)
{
    ...
    HI_U32 u32Version __attribute__((unused))= 0;
...
HI_VOID PVR_Index_ProcOverFlow(PVR_INDEX_HANDLE handle)
{
    ...
    handle->u32OverFlowTimeMs = handle->u32LastDispTime + handle->u32RecFirstFrmTimeMs;
    HI_PVR_LOG_EXIT();
...
HI_S32 PVR_Index_CycMoveFrmNum(PVR_INDEX_HANDLE handle, HI_BOOL bForward, HI_S32 s32CurFrmNum, HI_U32 Distance, HI_S32* ps32NextFrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(pstCycMgr->u32StartFrame, pstCycMgr->u32EndFrame, pstCycMgr->u32LastFrame, (HI_U32)s32CurFrmNum))
    {
...
HI_S32 PVR_Index_GetPreIFrame(PVR_INDEX_HANDLE handle, HI_U32 u32CurFrameNum, HI_U32* pu32PreIFrameNum, PVR_INDEX_ENTRY_NEW_S* pstPreIFrame)
{
    ...
    memset(&stIndexEntry, 0x00, sizeof(stIndexEntry));
    if (HI_TRUE != PVRIndexIsFrameValid(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, u32CurFrameNum))
    {
        HI_ERR_PVR("current frame number %d is invalid. start=%d end=%d\n",
    ...
    if (HI_SUCCESS != ret)
    {
        HI_ERR_PVR("index get frame by number failed!\n");
        return HI_FAILURE;
    }
    ...
    if ((PVR_INDEX_ERR_INVALID == ret) || (HI_ERR_PVR_FILE_TILL_START == ret))
    {
                                   ...
                                   HI_U32 u32FrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(piIndexHandle->stCycMgr.u32StartFrame, piIndexHandle->stCycMgr.u32EndFrame, piIndexHandle->stCycMgr.u32LastFrame, u32StartFrameNum))
    {
    ...
    for (i = u32StartFrameNum; i < u32ReadFrameEnd; i++)
    {
        ...
        if (HI_TRUE != PVRIndexIsFrameValid(piIndexHandle->stCycMgr.u32StartFrame, piIndexHandle->stCycMgr.u32EndFrame, piIndexHandle->stCycMgr.u32LastFrame, i))
        {
            HI_PVR_DEBUG_EXIT();
            return HI_SUCCESS;
        }
...
{
    HI_U32 i = 0, u32GopFlag = 0, u32FrmCount = 0, u32FrmNum = u32StartFrameNum;
    HI_S32 j = 0;
    HI_S32 ret = HI_SUCCESS;
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(pstCycMgr->u32StartFrame, pstCycMgr->u32EndFrame, pstCycMgr->u32LastFrame, u32StartFrameNum))
    {
        HI_ERR_PVR("Input frame number %d is invalid. S/E/L %d/%d/%d\n",
                    u32StartFrameNum,
                    piIndexHandle->stCycMgr.u32StartFrame,
    ...
    for (i = u32FrameCount; i > 0; i--)
    {
        ...
        if (HI_TRUE != PVRIndexIsFrameValid(pstCycMgr->u32StartFrame, pstCycMgr->u32EndFrame, pstCycMgr->u32LastFrame, u32FrmNum))
        {
            ret = HI_FAILURE;
        ...
        if (1 == u32GopFlag)
        {
            j = (HI_S32)pstFetchGopAttr->u32TotalFrameNum - 1;
            for (; j >= 0 ; j--)
            {
                               ...
                               HI_U32 u32StartIFrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(piIndexHandle->stCycMgr.u32StartFrame, piIndexHandle->stCycMgr.u32EndFrame, piIndexHandle->stCycMgr.u32LastFrame, u32StartIFrameNum))
    {
...
HI_S32 PVR_Index_SetCurReadFrameNum(PVR_INDEX_HANDLE handle, HI_U32 u32ReadFrame)
{
    ...
    if (HI_TRUE == handle->bIsRec)
    {
        if (handle->bRecReachPlay && PVRIndexIsFrameValid(handle->stCycMgr.u32StartFrame, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame, u32ReadFrame))
        {
            handle->bRecReachPlay = HI_FALSE;
        }
    }
...
HI_BOOL PVR_Index_CheckIndexFrmValid(HI_U32 u32IndexFrm, PVR_CYC_MGR_S* pstIndexCycMgr)
{
    ...
    ret = PVRIndexIsFrameValid(pstIndexCycMgr->u32StartFrame, pstIndexCycMgr->u32EndFrame, pstIndexCycMgr->u32LastFrame, u32IndexFrm);
...
HI_U32 PVR_Index_CalcFrameDistance(HI_U32 u32Dst, HI_U32 u32Src, PVR_INDEX_DIRECTION_E enDirection, PVR_CYC_MGR_S* pstCycMgr)
{
    ...
    if (PVR_INDEX_DIRECTION_FORWARD == enDirection)
    {
        HI_PVR_DEBUG_EXIT();
...
HI_VOID PVR_Index_SetIndexInfo(PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S* pstEntry, PVR_INDEX_ENTRY_ELEMENT_E enType, HI_VOID* pArgs)
{
    ...
    HI_U32 u32EntryVersion __attribute__((unused)) = handle->u32EntryVersion;
...
HI_BOOL PVR_INDEX_CheckIsFrameType(PVR_INDEX_HANDLE handle, PVR_INDEX_ENTRY_NEW_S* pstEntry, HI_U32 u32FrameType)
{
    ...
    ret = PVRINDEX_IS_CERTAINTYPE(handle->u32EntryVersion, u16FrameTypeAndGop, u32FrameType);
...
HI_VOID PVR_Index_GetStartAndEndPosByIndex(PVR_INDEX_HANDLE IndexHandle, HI_UNF_PVR_DATA_ATTR_S* stDataAttr)
{
    ...
    u32StartFrm = IndexHandle->stCycMgr.u32StartFrame;

    s32Ret = PVR_Index_GetFrameByNum(IndexHandle, &stStartFrame, u32StartFrm);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    {
        PVR_Index_GetIndexInfo(IndexHandle, &stStartFrame, PVR_INDEX_OFFSET, &stDataAttr->u64FileStartPos);
    }

    if (IndexHandle->stCycMgr.u32EndFrame > 0)
    {
        u32EndFrm = IndexHandle->stCycMgr.u32EndFrame;
    }
    else
    {
        u32EndFrm = IndexHandle->stCycMgr.u32LastFrame;
    }

    s32Ret = PVR_Index_GetFrameByNum(IndexHandle, &stEndFrame, u32EndFrm);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    {
        PVR_Index_GetIndexInfo(IndexHandle, &stEndFrame, PVR_INDEX_OFFSET, &stDataAttr->u64FileEndPos);
    }
...
HI_U32 PVR_Index_GetFrameBufUsed(HI_BOOL bDispOptimizaFlag)
{
    ...
    s32Ret = HI_SYS_GetVersion(&stSysVer);

    if ((HI_SUCCESS == s32Ret) &&(HI_CHIP_TYPE_HI3716M == stSysVer.enChipTypeHardWare) && (HI_CHIP_VERSION_V430 == stSysVer.enChipVersion))
    {
        HI_PVR_DEBUG_EXIT();
        return 0;
    }

    if (HI_TRUE == bDispOptimizaFlag)
    {
        HI_PVR_DEBUG_EXIT();
        return PVR_VO_FRMBUFF_NUM_OF_ENABLE_DEI;
    }

    HI_PVR_DEBUG_EXIT();

    return PVR_VO_FRMBUFF_NUM_OF_DISABLE_DEI;
}
...
PVR_INDEX_PID_INFO_S* PVR_Index_GetPidInfoByPidIdx(PVR_INDEX_HANDLE IndexHandle, HI_U32 u32PidIdx)
{
    ...
    if (u32PidIdx > IndexHandle->pstPidCtrl->u32UsedNum)
    {
        HI_ERR_PVR("invalid pid index num!\n");
        return HI_NULL;
    }

    HI_PVR_LOG_EXIT();
    return &(IndexHandle->pstPidCtrl->stPidInfo[u32PidIdx]);
}
...
HI_S32 PVR_Index_LinearizationCheckNeedToDo(PVR_FILE s32IdxFd, HI_BOOL* bNeedLinear)
{
    ...
    PVR_CHECK_POINTER(bNeedLinear);

    memset(&stHeaderInfo, 0x0, sizeof(stHeaderInfo));
    memset(&stPrivData, 0x0, sizeof(stPrivData));

    s32ReadLen = PVR_READ(&stHeaderInfo, sizeof(stHeaderInfo), s32IdxFd, 0);
    ...
    s32ReadLen = PVR_READ((HI_VOID*)&stPrivData, sizeof(stPrivData), s32IdxFd, PVR_GET_PRIVATE_DATA_OFFSET(stHeaderInfo));
...
HI_S32 PVR_Index_LinearizationAnalyzeIndexHeader(PVR_FILE s32IdxFd, PVR_LINEAR_REWIND_INFO_S* pstRewindInfo, HI_U32* pu32TotalProcFrmNum)
{
    ...
    PVR_CHECK_POINTER(pu32TotalProcFrmNum);
    ...
    u32EntryLen = (0 == pstRewindInfo->u32IdxVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_NEW_S);
    ...
    pstRewindInfo->stOldCycFrmInfo.u32StartFrame = stHeaderInfo.stCycInfo.u32StartFrame;
    pstRewindInfo->stOldCycFrmInfo.u32EndFrame = stHeaderInfo.stCycInfo.u32EndFrame;
    pstRewindInfo->stOldCycFrmInfo.u32LastFrame = stHeaderInfo.stCycInfo.u32LastFrame;
    if (HI_TRUE == pstPrivInfo->bTimeshiftToRec)
    {
        /*需要修复ts的配置修复参数*/
        pstPrivInfo = (PVR_INDEX_PRIV_DATA_INFO_S*)stPrivData.u8Reserved;
        ...
        *pu32TotalProcFrmNum = pstRewindInfo->stOldCycFrmInfo.u32EndFrame - pstRewindInfo->stOldCycFrmInfo.u32StartFrame + 1;
    }
    else
    {
        /*原索引文件未进行时移切换录制*/
        ...
        *pu32TotalProcFrmNum = (pstRewindInfo->stOldCycFrmInfo.u32EndFrame + 1) + (pstRewindInfo->stOldCycFrmInfo.u32LastFrame - pstRewindInfo->stOldCycFrmInfo.u32StartFrame + 1);

        s32ReadLen = PVR_READ(&stEndFrame, u32EntryLen, s32IdxFd, stHeaderInfo.u32HeaderLen + stHeaderInfo.stCycInfo.u32EndFrame * u32EntryLen);
        if (s32ReadLen != u32EntryLen)
        {
            HI_ERR_PVR("read end frame failed. [expect, actual] = [%u, %u]!\n", (HI_U32)u32EntryLen, s32ReadLen);
            return HI_ERR_PVR_FILE_CANT_READ;
        }
...
HI_S32 PVR_Index_ProcessRewindChg(PVR_INDEX_HANDLE handle)
{
    ...
    PVR_CYC_MGR_S* pstLastCycMgr = &handle->stLastFileInfo.stCycInfo;
#ifdef HI_PVR_HEVC_SUPPORT
    PVR_CYC_FRAME_NUM_INFO stCycFrameInfo;
    stCycFrameInfo.u32StartFrame = pstLastCycMgr->u32StartFrame;
    stCycFrameInfo.u32EndFrame = pstLastCycMgr->u32EndFrame;
    stCycFrameInfo.u32LastFrame = pstLastCycMgr->u32LastFrame;
#endif
...
#ifdef HI_PVR_HEVC_SUPPORT
            (HI_VOID)PVRIndexFileOperationProcRef(&stCycFrameInfo, handle->u32EntryVersion, -u32StartFrame, pstEntry);
#endif
    ...
    if (2 == u32RewindType)
    {
        ...
        for (i = pstLastCycMgr->u32StartFrame; i <= pstLastCycMgr->u32EndFrame; i++)
        {
            ...
            PVRINDEX_SET_CYCTIMES(handle->u32EntryVersion, pstEntry, 0);
#ifdef HI_PVR_HEVC_SUPPORT
            (HI_VOID)PVRIndexFileOperationProcRef(&stCycFrameInfo, handle->u32EntryVersion, pstLastCycMgr->u32StartFrame, pstEntry);
#endif
            memcpy(pu8DstIdxBuf + u32DstIdxOffset, pstEntry, sizeof(PVR_INDEX_ENTRY_NEW_S));
            u32DstIdxOffset += sizeof(PVR_INDEX_ENTRY_NEW_S);
            u32TotalFrameNum++;
...
HI_BOOL PVR_Index_CheckCurrentReadFrameIsInValidWhenRewindChg(PVR_INDEX_HANDLE handle)
{
    ...
    u32CurSendFrmNum = handle->u32ReadFrame;
    pstLastCycMgr = &handle->stLastFileInfo.stCycInfo;

    if (pstLastCycMgr->u32StartFrame > pstLastCycMgr->u32EndFrame)
    {
        /*|--E--S--L|*/
        if ((u32CurSendFrmNum == pstLastCycMgr->u32LastFrame))
        {
            bInvalid = HI_TRUE;
            handle->u32ReadFrame = 0;
        }

        if ((u32CurSendFrmNum < pstLastCycMgr->u32StartFrame) && (u32CurSendFrmNum >= pstLastCycMgr->u32EndFrame))
        {
            bInvalid = HI_TRUE;
            handle->u32ReadFrame = pstLastCycMgr->u32LastFrame + 1;
        }
    }
    else
    {
        /*|-S-----E-L|*/
        if ((u32CurSendFrmNum >= pstLastCycMgr->u32EndFrame) && (u32CurSendFrmNum <= pstLastCycMgr->u32LastFrame))
        {
            bInvalid = HI_TRUE;
            handle->u32ReadFrame = pstLastCycMgr->u32LastFrame + 1;
        }

        if (u32CurSendFrmNum < pstLastCycMgr->u32StartFrame)
        {
            bInvalid = HI_TRUE;
            handle->u32ReadFrame = pstLastCycMgr->u32StartFrame;
        }
    }

    if ((HI_TRUE == bInvalid) && (handle->u32ReadFrame > handle->stCycMgr.u32LastFrame) && (handle->stCycMgr.u32LastFrame == pstLastCycMgr->u32LastFrame))
    {
        handle->u32ReadFrame = handle->stCycMgr.u32EndFrame;
        handle->bNeedCheckRead = HI_FALSE;
    }

    HI_WARN_PVR("curReadFrame is invalid , FrmNum: %u  CycE: %u, CycLast: %u\n", u32CurSendFrmNum, handle->stCycMgr.u32EndFrame, handle->stCycMgr.u32LastFrame);
    HI_PVR_LOG_EXIT();
    return bInvalid;
}
#endif
...
*[pstCycFrmInfo] :  索引文件中的S/E/L信息
*[u32IdxVersion] : 当前处理的索引版本号
*[bTimeShiftProcessed] : 当前处理的索引文件是否进行过时移切换录制
*[s32HeadPosOffset] : 当前索引的起始索引与0的偏移量，当发生过时移切换录制或者文件头部截断时，
                                 该值可能非0，其余情况传入0。
      s32HeadPosOffset = -(pstCycFrmInfo.u32StartFrame - pstCycFrmInfo.u32EndFrame)         【发生时移切换录制时的偏移量，为负数】
      s32HeadPosOffset = pstCycFrmInfo.u32StartFrame;  ---- 【进行过时移切换录制的情况，或回绕情况为s--e--l】
      s32HeadPosOffset = (u32TruncateHeaderPos + u32OrigLastFrm + 1 - u32OrigStartFrm)%(u32OrigLastFrm + 1);      ---- 【只进行文件头部截断】
      s32HeadPosOffset = pstCycFrmInfo.u32StartFrame +  (u32TruncateHeaderPos + u32OrigLastFrm + 1 - u32OrigStartFrm)%(u32OrigLastFrm + 1);   ---   【进行过时移切录制，又进行头部截断】
*[pstEntry] :当前处理的索引
*/
static HI_VOID PVRIndexFileOperationProcRef(PVR_CYC_FRAME_NUM_INFO* pstCycFrmInfo, HI_U32 u32IdxVersion,
                                                    HI_S32 s32HeadPosOffset, PVR_INDEX_ENTRY_NEW_S* pstEntry)
{
    ...
    u8OldRefCnt = PVRINDEX_GET_REFCNT(u32IdxVersion, pstEntry);
    ...
    PVRINDEX_GET_REFINFO(u32IdxVersion, pstEntry, au32OldRefInfo);

    /*|--E--S--L|*/
    if (pstCycFrmInfo->u32EndFrame < pstCycFrmInfo->u32LastFrame)
    {
        /*
        ...
        */
        ...
        {
            if (au32OldRefInfo[i] <= pstCycFrmInfo->u32EndFrame)
            {
                au32OldRefInfo[i] = pstCycFrmInfo->u32LastFrame - pstCycFrmInfo->u32StartFrame + au32OldRefInfo[i] + 1;
                continue;
            }
            else if (au32OldRefInfo[i] >= pstCycFrmInfo->u32StartFrame && au32OldRefInfo[i] <= pstCycFrmInfo->u32LastFrame)
            {
                au32OldRefInfo[i] = au32OldRefInfo[i] - pstCycFrmInfo->u32StartFrame;
                continue;
            }
            else
            {/*参考帧无效*/
                au32OldRefInfo[i] += pstCycFrmInfo->u32LastFrame;
                continue;
            }
        }
    }

    /*针对头部处理和时移切换录制后的索引修复参考关系*/
    if (0 != s32HeadPosOffset)
    {
        for (i = 0; i < u8OldRefCnt; i++)
        {
            if (s32HeadPosOffset < 0)
            {
                au32OldRefInfo[i] += abs(s32HeadPosOffset);
            }
            else if ((s32HeadPosOffset > 0) && (au32OldRefInfo[i] >= s32HeadPosOffset))
            {
                au32OldRefInfo[i] -= s32HeadPosOffset;
            }
            else
            {/*参考帧无效*/
                au32OldRefInfo[i] += pstCycFrmInfo->u32LastFrame;
    ...
    for (i = 0; i < u8OldRefCnt; i++)
    {
        PVRINDEX_SET_REFINFO(u32IdxVersion, pstEntry, au32OldRefInfo[i], i);
    }
...
static HI_S32 PVRIndexRecFileFindFrameNumByTime(PVR_FILE s32IndexFd, HI_U32 u32EntryVersion, HI_U32 u32IdxHeaderLen, HI_U32 u32TargetEndTimeMs, HI_U32 start, HI_U32 end)
{
    ...
    HI_U32 u32IdxOffset = 0;
    HI_S32 s32ReadLen = 0;
    HI_U32 u32EntrySize = (0 == u32EntryVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_NEW_S);
    PVR_INDEX_ENTRY_NEW_S stEntry;
    ...
    s32Ret = PVRIndexRecFileFindFrameNumByTime(s32IndexFd, u32EntryVersion, u32IdxHeaderLen, u32TargetEndTimeMs, nextStart, nextEnd);

    HI_PVR_LOG_EXIT();
    return s32Ret;
}

static HI_S32 PVRIndexRecFileFindTargetIdxPos(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo, HI_U32 u32TargetTimeMs, HI_BOOL bBackwardI, HI_U32* pu32TargetIdxPos)
{
    HI_U16 u16FrameTypeAndGop = 0;
    HI_U32 u32IdxOffset = 0;
    HI_U32 u32StartFindPos = 0;
    HI_U32 u32EndFindPos = 0;
    HI_U32 u32EntrySize = (0 == pstTruncateProcInfo->u32EntryVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_NEW_S);
    HI_U32 u32EndTimeMs = 0;
    HI_U32 u32LastTimeMs = 0;
    HI_S32 s32ReadLen = 0;
    HI_S32 s32FindPos = -1;
    PVR_INDEX_ENTRY_NEW_S stEntry;
    PVR_CYC_FRAME_NUM_INFO* pstCycFrmInfo = &(pstTruncateProcInfo->stOldCycFrmInfo);
    PVR_FILE_OPT_INFO_S* pstFileOptInfo = &(pstTruncateProcInfo->stFileOptInfo);

    HI_PVR_LOG_ENTER();

    u32IdxOffset = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * pstCycFrmInfo->u32EndFrame;
    PVR_READ_ONE_ENTRY(s32ReadLen, &stEntry, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read End frame(%u) failed! want: %u, read: %d\n", pstCycFrmInfo->u32EndFrame, u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    }
    u32EndTimeMs = PVRINDEX_GET_DISPTIME(pstTruncateProcInfo->u32EntryVersion, &stEntry);

    u32IdxOffset = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * pstCycFrmInfo->u32LastFrame;
    PVR_READ_ONE_ENTRY(s32ReadLen, &stEntry, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read Last frame(%u) failed! want: %u, read: %d\n", pstCycFrmInfo->u32LastFrame, u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    }
    u32LastTimeMs = PVRINDEX_GET_DISPTIME(pstTruncateProcInfo->u32EntryVersion, &stEntry);

    /*根据时间寻找帧号*/
    if (u32LastTimeMs == u32EndTimeMs)
    {
        u32StartFindPos = pstCycFrmInfo->u32StartFrame;
        u32EndFindPos = pstCycFrmInfo->u32EndFrame;
    }
    else
    {
        if (u32TargetTimeMs > u32LastTimeMs)
        {/*L--target---E*/
            u32StartFindPos = 0;
            u32EndFindPos = pstCycFrmInfo->u32EndFrame;
        }
        ...
        {/*S--target---E*/
            u32StartFindPos = pstCycFrmInfo->u32StartFrame;
            u32EndFindPos = pstCycFrmInfo->u32LastFrame;
        }
    }

    s32FindPos = PVRIndexRecFileFindFrameNumByTime(pstFileOptInfo->s32SrcIdxFd, pstTruncateProcInfo->u32EntryVersion,
                            pstTruncateProcInfo->u32IndexHeaderLen, u32TargetTimeMs, u32StartFindPos, u32EndFindPos);
    if (-1 == s32FindPos)
    {
        HI_ERR_PVR("failed to find frame by time(%u), range: %u~%u\n", u32TargetTimeMs, u32StartFindPos, u32EndFindPos);
        return HI_ERR_PVR_INDEX_DATA_ERR;
    }

    /*根据帧号寻找附近I帧*/
    u32IdxOffset = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * s32FindPos;
    PVR_READ_ONE_ENTRY(s32ReadLen, &stEntry, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read frame(%u) failed! want: %u, read: %d\n", s32FindPos, u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    }
    ...
    while(PVRIndexIsFrameValid(pstCycFrmInfo->u32StartFrame, pstCycFrmInfo->u32EndFrame, pstCycFrmInfo->u32LastFrame, s32FindPos))
    {
        ...
        {
            s32FindPos = PVRIndexCalcNewPos(pstCycFrmInfo->u32StartFrame, pstCycFrmInfo->u32EndFrame, pstCycFrmInfo->u32LastFrame, s32FindPos, -1);
        }
        else
        {
            s32FindPos = PVRIndexCalcNewPos(pstCycFrmInfo->u32StartFrame, pstCycFrmInfo->u32EndFrame, pstCycFrmInfo->u32LastFrame, s32FindPos, 1);
...
HI_S32 PVR_Index_RecFileGetTargetIdxPos(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo)
{
    ...
    PVR_CHECK_POINTER(pstTruncateProcInfo);

    HI_PVR_LOG_ENTER();

    if (HI_TRUE == pstTruncateProcInfo->stTruncateUserCfg.bTruncateHead)
    {/*处理头部truncateHeadPos*/
        u32TargetTimeMs = pstTruncateProcInfo->stTruncateUserCfg.u32StartTimeMs - pstTruncateProcInfo->stTruncateUserCfg.u32PaddingTimeMs;
        s32Ret = PVRIndexRecFileFindTargetIdxPos(pstTruncateProcInfo, u32TargetTimeMs, HI_TRUE, &pstTruncateProcInfo->u32HeaderPos);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("failed to find head truncate position! ret = %x\n", s32Ret);
            return s32Ret;
        }
    }
    ...
    {/*处理尾部truncateTailPos*/
        ...
        s32Ret = PVRIndexRecFileFindTargetIdxPos(pstTruncateProcInfo, u32TargetTimeMs, HI_FALSE, &pstTruncateProcInfo->u32TailPos);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_PVR("failed to find tail truncate position! ret = %x\n", s32Ret);
            return s32Ret;
        }
    }
...
static HI_S32 PVRIndexRecFileTruncateProcIndexHeaderOnOrigFile(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo, HI_U32 u32TargetIndexPos, HI_BOOL bProcHead)
{
    HI_S32 s32Length = 0;
    PVR_IDX_HEADER_INFO_S stHeaderInfo;
    ...
    memset(&stHeaderInfo, 0x00, sizeof(stHeaderInfo));
    PVR_READ_INDEX_HEADER(s32Length, &stHeaderInfo, pstTruncateProcInfo->stFileOptInfo.s32SrcIdxFd);

    if (HI_TRUE == bProcHead)
    {
        stHeaderInfo.stCycInfo.u32StartFrame = u32TargetIndexPos;
    }
    else
    {
    ...
    s32Length = PVR_WRITE(&stHeaderInfo, sizeof(stHeaderInfo), pstTruncateProcInfo->stFileOptInfo.s32SrcIdxFd, 0);
    if (s32Length != sizeof(stHeaderInfo))
    {
    ...
    PVR_FSYNC(pstTruncateProcInfo->stFileOptInfo.s32SrcIdxFd);
...
HI_S32 PVR_Index_RecFileTruncateOnlyProcTailOrHeadOnOrig(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo, HI_U64* pu64TsTrucateTailOffset, HI_BOOL bProcHead)
{
    HI_S32 s32Ret = HI_SUCCESS;
    HI_S32 s32ReadLen = 0;
    HI_U32 u32EntrySize = 0;
    HI_U64 u64IndexTruncateSize = 0;
    PVR_INDEX_ENTRY_NEW_S stEntry;
    PVR_CYC_FRAME_NUM_INFO* pstCycFrmInfo = &(pstTruncateProcInfo->stOldCycFrmInfo);
    HI_U32 u32ProcIdxPos = (HI_TRUE == bProcHead) ? pstTruncateProcInfo->u32HeaderPos : pstTruncateProcInfo->u32TailPos;

    HI_PVR_LOG_ENTER();

    PVR_CHECK_POINTER(pstTruncateProcInfo);
    PVR_CHECK_POINTER(pu64TsTrucateTailOffset);

    u32EntrySize = (0 == pstTruncateProcInfo->u32EntryVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_NEW_S);
    /*判断truncate位置是否有效*/
    if ((u32ProcIdxPos < pstCycFrmInfo->u32StartFrame) || (u32ProcIdxPos > pstCycFrmInfo->u32EndFrame))
    {
        HI_ERR_PVR("the truncate point is invalid! targetpos: %u, S/E: %u/%u\n",
                   u32ProcIdxPos, pstCycFrmInfo->u32StartFrame, pstCycFrmInfo->u32EndFrame);
        return HI_ERR_PVR_INVALID_PARA;
    }
    ...
    if ((0 != pstTruncateProcInfo->u32RewindLastFrm) && (u32ProcIdxPos < pstTruncateProcInfo->u32RewindLastFrm))
    {
        HI_ERR_PVR("the truncate point lies in the rewind part! targetpos: %u, u32RewindLastFrm: %u\n", u32ProcIdxPos, pstTruncateProcInfo->u32RewindLastFrm);
        return HI_ERR_PVR_INVALID_PARA;
    }

    /*修正索引头信息*/
    s32Ret = PVRIndexRecFileTruncateProcIndexHeaderOnOrigFile(pstTruncateProcInfo, u32ProcIdxPos, bProcHead);
    if (HI_SUCCESS != s32Ret)
    {
    ...
    {/*truncate索引，只有当处理尾部时才能直接truncate索引文件，且该处多留了16个索引的余量，保证索引文件的有效性*/
        u64IndexTruncateSize = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * (u32ProcIdxPos + 16);
        if (0 != ftruncate(pstTruncateProcInfo->stFileOptInfo.s32SrcIdxFd, u64IndexTruncateSize))
        {
    ...
    PVR_READ_ONE_ENTRY(s32ReadLen, &stEntry, pstTruncateProcInfo->stFileOptInfo.s32SrcIdxFd, pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * u32ProcIdxPos, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read truncate tail pos frame(%u) failed! want: %u, read: %d\n", u32ProcIdxPos, u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    }

    *pu64TsTrucateTailOffset = PVR_IDX_ALIGN_DATA(PVRINDEX_GET_OFFSET(pstTruncateProcInfo->u32EntryVersion, &stEntry) + PVRINDEX_GET_FRAMESIZE(pstTruncateProcInfo->u32EntryVersion, &stEntry));
...
HI_S32 PVR_Index_CorretIndexFile(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo, HI_U64 u64FileSize)
{
    ...
    /*读取索引头信息*/
    PVR_READ_INDEX_HEADER(s32Length, &stHeaderInfo, pstFileOptInfo->s32SrcIdxFd);

    u32IdxVersion = (stHeaderInfo.u32Reserved & 0xF0000) >> 16;
    pstTruncateProcInfo->u32EntryVersion = u32IdxVersion;
    u32EntrySize = (0 == u32IdxVersion) ? sizeof(PVR_INDEX_ENTRY_S) : sizeof(PVR_INDEX_ENTRY_NEW_S);
    pstTruncateProcInfo->u64MaxCycSize = stHeaderInfo.u64ValidSize;
    pstTruncateProcInfo->u32IndexHeaderLen = stHeaderInfo.u32HeaderLen;
    pstTruncateProcInfo->stOldCycFrmInfo.u32StartFrame = stHeaderInfo.stCycInfo.u32StartFrame;
    pstTruncateProcInfo->stOldCycFrmInfo.u32EndFrame = stHeaderInfo.stCycInfo.u32EndFrame;
    pstTruncateProcInfo->stOldCycFrmInfo.u32LastFrame = stHeaderInfo.stCycInfo.u32LastFrame;

    u32IdxOffset = stHeaderInfo.u32HeaderLen + u32EntrySize * stHeaderInfo.stCycInfo.u32EndFrame;
    PVR_READ_ONE_ENTRY(s32Length, &stEndEntry, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, u32IdxVersion);
    if (s32Length != u32EntrySize)
    {
        HI_ERR_PVR("read End frame failed! want: %u, read: %d\n", u32EntrySize, s32Length);
        return HI_ERR_PVR_FILE_CANT_READ;
    }
    pstTruncateProcInfo->u64LastWritePos = PVRINDEX_GET_OFFSET(u32IdxVersion, &stEndEntry) + PVRINDEX_GET_FRAMESIZE(u32IdxVersion, &stEndEntry);
    /*读取私有头信息*/
    if (u32IdxVersion >= 3)
    {
        s32Length = PVR_READ((HI_VOID *)&stPrivData, sizeof(stPrivData), pstFileOptInfo->s32SrcIdxFd, PVR_GET_PRIVATE_DATA_OFFSET(stHeaderInfo));
        ...
        if ((HI_TRUE == pstPrivInfo->bTimeshiftToRec) && (HI_FALSE == pstPrivInfo->bLinearied))
        {
            ...
            pstTruncateProcInfo->bProcTimeshift = HI_TRUE;
            ...
            pstTruncateProcInfo->u64MaxCycSize = pstPrivInfo->u64MaxCycSize;
        }
    }

    /*此处在Ts句柄中储存原始最大文件大小，后续truncate时可用于删除多余ts分片文件*/
    PVR_SET_MAXFILE_SIZE(pstTruncateProcInfo->s32OrigTsWriteFd, u64FileSize);

    HI_WARN_PVR("before correct, S/E/L: %u/%u/%u\n",stHeaderInfo.stCycInfo.u32StartFrame, stHeaderInfo.stCycInfo.u32EndFrame, stHeaderInfo.stCycInfo.u32LastFrame);
    if ((HI_TRUE == stHeaderInfo.stCycInfo.u32IsRewind)
            && (stHeaderInfo.stCycInfo.u32EndFrame != stHeaderInfo.stCycInfo.u32LastFrame))
    {//暂不支持修复已发生回绕的录制文件
        HI_WARN_PVR("current file is rewind,not support to corret!\n");
        HI_PVR_LOG_EXIT();
        return HI_SUCCESS;
    }

    s32Ret = PVRIndexFindValidEndFrame(pstFileOptInfo->s32SrcIdxFd, &stHeaderInfo, pstPrivInfo, u64FileSize, &stEndEntry);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_PVR("get valid endFrame failed! ret = 0x%x\n",s32Ret);
        return s32Ret;
    ...
    /*此处在Ts句柄中储存原始最大文件大小，后续truncate时可用于删除多余ts分片文件*/
    PVR_SET_MAXFILE_SIZE(pstTruncateProcInfo->s32OrigTsWriteFd, pstTruncateProcInfo->u64LastWritePos);

    HI_WARN_PVR("after correct, S/E/L: %u/%u/%u\n",stHeaderInfo.stCycInfo.u32StartFrame, stHeaderInfo.stCycInfo.u32EndFrame, stHeaderInfo.stCycInfo.u32LastFrame);
...
HI_S32 PVR_Index_RecFileTruncateCheckNeedToDo(PVR_TRUNCATE_PROC_S* pstTruncateProcInfo)
{
    ...
    /*读取StartFrame信息*/
    u32IdxOffset = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * pstTruncateProcInfo->stOldCycFrmInfo.u32StartFrame;
    PVR_READ_ONE_ENTRY(s32ReadLen, &stStartFrame, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read Start frame failed! want: %u, read: %d\n", u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    }

    /*读取EndFrame信息*/
    u32IdxOffset = pstTruncateProcInfo->u32IndexHeaderLen + u32EntrySize * pstTruncateProcInfo->stOldCycFrmInfo.u32EndFrame;
    PVR_READ_ONE_ENTRY(s32ReadLen, &stEndFrame, pstFileOptInfo->s32SrcIdxFd, u32IdxOffset, pstTruncateProcInfo->u32EntryVersion);
    if (s32ReadLen != u32EntrySize)
    {
        HI_ERR_PVR("read End frame failed! want: %u, read: %d\n", u32EntrySize, s32ReadLen);
        return HI_ERR_PVR_FILE_CANT_READ;
    ...
    u32TruncateHeadTime = pstTruncateProcInfo->stTruncateUserCfg.u32StartTimeMs - pstTruncateProcInfo->stTruncateUserCfg.u32PaddingTimeMs;

    /*如果原文件是回绕的，且新文件名为空，返回不支持*/
    if ((pstTruncateProcInfo->stOldCycFrmInfo.u32LastFrame != pstTruncateProcInfo->stOldCycFrmInfo.u32EndFrame)
        && (0 == strlen(pstFileOptInfo->szDstFileName)))
    {
        HI_ERR_PVR("Pvr does not support truncate in rewind original files!\n");
        return HI_ERR_PVR_NOT_SUPPORT;
    }

    /*如果truncate尾部，且truncate时间不在合理范围内，返回失败*/
    if ((HI_TRUE == pstTruncateProcInfo->stTruncateUserCfg.bTruncateTail)
    ...
    /*如果truncate头部，且truncate时间不在合理范围内，返回失败*/
    if ((HI_TRUE == pstTruncateProcInfo->stTruncateUserCfg.bTruncateHead)
            && ((u32StartDisplayTime > u32TruncateHeadTime) || (u32EndDisplayTime < u32TruncateHeadTime)))
    {
        HI_ERR_PVR("truncate time is invalid, can't truncate Head! truncateTime: %u startTime/endTime: %u/%u\n",
                        u32TruncateHeadTime, u32StartDisplayTime, u32EndDisplayTime);
        return  HI_ERR_PVR_INVALID_PARA;
    }

    /*如果头尾都truncate，且TruncateHeadPos > TruncateTailPos，返回失败*/
    if ((HI_TRUE == pstTruncateProcInfo->stTruncateUserCfg.bTruncateHead) &&
            (HI_TRUE == pstTruncateProcInfo->stTruncateUserCfg.bTruncateTail) &&
                (u32TruncateHeadTime > u32TruncateTailTime))
    {
        HI_ERR_PVR("truncate time is invalid, truncateHeadTime(%u) > truncateTailTime(%u) \n",
                        u32TruncateHeadTime, u32TruncateTailTime);
        return  HI_ERR_PVR_INVALID_PARA;
    }
...
/*线性化或truncate时从源文件中读取一个索引，并保存上一个索引*/
HI_S32 PVR_Index_RecLinearizationOrTruncateGetOneIndex(PVR_FRAME_DATA_DISTRIBUTED_INFO* pstDataInfo, HI_S32 s32IdxFd, HI_U32 u32CurIdxOffset, HI_U32 u32EntryVersion)
{
    ...
    PVR_CHECK_POINTER(pstDataInfo);

    /*先保存上一帧数据,再memset*/
    memcpy(&(pstDataInfo->stLastProcEntry), &(pstDataInfo->stCurProcEntry), u32EntrySize);
    memset(&(pstDataInfo->stCurProcEntry), 0x00, u32EntrySize);

    s32Length = PVR_READ(&(pstDataInfo->stCurProcEntry), u32EntrySize, s32IdxFd, u32CurIdxOffset);
    if (s32Length != u32EntrySize)
    {
        HI_ERR_PVR("read index failed!read: %d, want: %u, offset: 0x%08x\n",s32Length, u32EntrySize, u32CurIdxOffset);
        return HI_ERR_PVR_FILE_CANT_READ;
    }

    if (0 == PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &(pstDataInfo->stCurProcEntry)))
    {
        HI_WARN_PVR("Cur Frame is invalid! size: 0\n");
        return HI_ERR_PVR_INDEX_DATA_ERR;
    }
...
/*从获取的索引信息中确定当前处理帧的TS数据在源TS文件中的位置及size*/
HI_VOID PVR_Index_RecLinearizationOrTruncateGetTsOffsetByIndex(PVR_FRAME_DATA_DISTRIBUTED_INFO* pstDataInfo, HI_U32 u32EntryVersion, HI_U64 u64MaxCycSize, HI_U64 u64LastWritePos)
{
    ...
    if (HI_NULL == pstDataInfo)
    {
        HI_ERR_PVR("point of data info is null!\n");
        return ;
    }

    u64CurFrmGlobalOffset = PVRINDEX_GET_GLOBALOFFSET(u32EntryVersion, &pstDataInfo->stCurProcEntry);
    u64CurFrmOffset = PVRINDEX_GET_OFFSET(u32EntryVersion, &pstDataInfo->stCurProcEntry);
    u64CurFrmSize = PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &pstDataInfo->stCurProcEntry);
    u64LastGlobaloffset = PVRINDEX_GET_GLOBALOFFSET(u32EntryVersion, &pstDataInfo->stLastProcEntry);
    u32LastFrameSize = PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &pstDataInfo->stLastProcEntry);
            ...
            && (u64CurFrmGlobalOffset != (u64LastGlobaloffset + u32LastFrameSize)))
    {
        ...
        HI_WARN_PVR("append point, indexPos: %u, appendAlignLen: %u\n", pstDataInfo->u32CurProcIdxPos, u32AppendAlignSize);

        pstDataInfo->u64Offset1 = u64CurFrmOffset - u32AppendAlignSize;
        pstDataInfo->u32Length1 = u64CurFrmSize + u32AppendAlignSize;
        HI_PVR_LOG_EXIT();
        return;
    }
/*
...
*/
    /*first frame*/
    if ((0 == pstDataInfo->u32CurProcIdxPos) || (PVRINDEX_GET_FRAMESIZE(u32EntryVersion, &pstDataInfo->stLastProcEntry) == 0))
    {
        /*ts起始帧的offset 进行188k 对齐处理*/
        pstDataInfo->u32DeltaLength = u64CurFrmGlobalOffset % (4 * 47 * 1024);
        pstDataInfo->u64Offset1 = u64CurFrmOffset - pstDataInfo->u32DeltaLength;
        pstDataInfo->u32Length1 = u64CurFrmSize+ pstDataInfo->u32DeltaLength;

        /*area 1 and area 2*//*area 1, area 2 and area3:不需要考虑，应该一帧不会超过这么大的*/
        if ((u64CurFrmOffset + u64CurFrmSize > u64MaxCycSize)
                && (u64CurFrmOffset < u64MaxCycSize))
        {
            pstDataInfo->u32Length2 = u64CurFrmOffset + u64CurFrmSize - u64MaxCycSize;
            pstDataInfo->u32Length1 -= pstDataInfo->u32Length2;
            pstDataInfo->u64Offset2 = 0;
        }
    }
    else
    {
        /*When the data is all in one of these three areas: area1, area2, area3*/
        pstDataInfo->u64Offset1 = u64CurFrmOffset;
        pstDataInfo->u32Length1 = u64CurFrmSize;
            ...
            && (u64CurFrmOffset < u64MaxCycSize))
        {
            pstDataInfo->u64Offset1 = u64CurFrmOffset;
            pstDataInfo->u32Length2 = u64CurFrmOffset + u64CurFrmSize - u64MaxCycSize;
            pstDataInfo->u32Length1 = u64CurFrmSize - pstDataInfo->u32Length2;
            pstDataInfo->u64Offset2 = 0;
        }

        /*area 2 and area 3*/
        if ((u64CurFrmOffset + u64CurFrmSize > u64LastWritePos)
            && (u64CurFrmOffset < u64LastWritePos))
        {
            pstDataInfo->u64Offset1 = u64CurFrmOffset;
            pstDataInfo->u32Length2 = u64CurFrmOffset + u64CurFrmSize - u64LastWritePos;
            pstDataInfo->u32Length1 = u64CurFrmSize - pstDataInfo->u32Length2;
            pstDataInfo->u64Offset2 = u64MaxCycSize;
        }

        /*area 1, area 2 and area 3: area 2 is very short*/
        if ((u64CurFrmOffset > u64LastWritePos)
            && (u64CurFrmOffset < u64MaxCycSize)
            && (u64CurFrmOffset + u64CurFrmSize > u64MaxCycSize + u64LastWritePos))
        {
            pstDataInfo->u64Offset1 = u64CurFrmOffset;
            pstDataInfo->u32Length1 = u64MaxCycSize - u64CurFrmOffset;
            pstDataInfo->u64Offset2 = 0;
            pstDataInfo->u32Length2 = u64LastWritePos;
            pstDataInfo->u64Offset3 = u64MaxCycSize;
            pstDataInfo->u32Length3 = u64CurFrmSize - pstDataInfo->u32Length1 - pstDataInfo->u32Length2;
        }
    }
...
/*修正当前索引的信息，如offset、cycTimes、参考关系等*/
                                                                            ...
                                                                            HI_U32 u32HeadPosOffset)
{
    ...
    if ((HI_NULL == pstCurProcTsInfo) || (HI_NULL == pstCycFrmInfo))
    {
        HI_ERR_PVR("Null pointer error, pstCurProcTsInfo(%p), pstCycFrmInfo(%p)\n", pstCurProcTsInfo, pstCycFrmInfo);
        return ;
    }

    u64LastGlobaloffset = PVRINDEX_GET_GLOBALOFFSET(u32IdxVersion, &(pstCurProcTsInfo->stLastProcEntry));
    u64LastOffset = PVRINDEX_GET_OFFSET(u32IdxVersion, &(pstCurProcTsInfo->stLastProcEntry));
    u32LastFrameSize = PVRINDEX_GET_FRAMESIZE(u32IdxVersion, &(pstCurProcTsInfo->stLastProcEntry));
    u64CurFrmGlobalOffset = PVRINDEX_GET_GLOBALOFFSET(u32IdxVersion, &(pstCurProcTsInfo->stCurProcEntry));

    /*revise current entry info*/
    PVRINDEX_SET_CYCTIMES(u32IdxVersion, &(pstCurProcTsInfo->stCurProcEntry), 0);
#ifdef HI_PVR_HEVC_SUPPORT
    (HI_VOID)PVRIndexFileOperationProcRef(pstCycFrmInfo, u32IdxVersion, u32HeadPosOffset, &(pstCurProcTsInfo->stCurProcEntry));
#endif
    ...
    {
        u32AppendAlignSize = u64CurFrmGlobalOffset - (u64LastGlobaloffset + u32LastFrameSize);
        PVRINDEX_SET_OFFSET(u32IdxVersion, &(pstCurProcTsInfo->stCurProcEntry), u64LastOffset + u32LastFrameSize + u32AppendAlignSize);
        HI_PVR_DEBUG_EXIT();
        return;
    }

    if ((0 == pstCurProcTsInfo->u32CurProcIdxPos) || (0 == PVRINDEX_GET_FRAMESIZE(u32IdxVersion, &(pstCurProcTsInfo->stLastProcEntry))))
    {
        PVRINDEX_SET_OFFSET(u32IdxVersion, &(pstCurProcTsInfo->stCurProcEntry), (HI_U64)pstCurProcTsInfo->u32DeltaLength);
    }
    else
    {
        PVRINDEX_SET_OFFSET(u32IdxVersion, &(pstCurProcTsInfo->stCurProcEntry), (u64LastOffset + u32LastFrameSize));
    }
...
/*保存已修正的索引到目标索引文件中*/
HI_S32 PVR_Index_RecLinearizationOrTruncateSaveOneIndex(PVR_INDEX_ENTRY_NEW_S* pstCurProcEntry, HI_S32 s32IdxFd, HI_U32 u32IdxOffset, HI_U32 u32WriteSize)
{
    HI_S32 s32Length = 0;

    HI_PVR_DEBUG_ENTER();
    PVR_CHECK_POINTER(pstCurProcEntry);

    /*save current entry to repaire file(include index file and ts file)*/
    s32Length = PVR_WRITE(pstCurProcEntry, u32WriteSize, s32IdxFd, u32IdxOffset);
    if (s32Length != u32WriteSize)
    {
        HI_ERR_PVR("write index to new linearization file failed, expect:%u; actual:%u\n", u32WriteSize, s32Length);
        return HI_ERR_PVR_FILE_CANT_WRITE;
    }
...
/*线性化或truncate 对帧数据处理完成后，相应的修正索引头信息，并写入目标索引文件中*/
                                                                            ...
                                                                            HI_U32 u32IndexHeaderLen, HI_U32 u32HeadPosOffset)
{
...
#ifdef HI_PVR_PID_CHANGE_SUPPORT
    PVR_INDEX_ENTRY_NEW_S stStartFrame;
    HI_U32 u32StartFrmDispTime = 0;
    PVR_INDEX_REC_PID_CTRL_S* pstPidCtrl = HI_NULL;
#endif

    HI_PVR_LOG_ENTER();

    pIndexHeadBuf = (HI_U8*)HI_MALLOC(HI_ID_PVR, u32IndexHeaderLen);
    ...
    memset(pIndexHeadBuf, 0x00, u32IndexHeaderLen);
    s32Length = PVR_READ(pIndexHeadBuf, u32IndexHeaderLen, pstFileOptInfo->s32SrcIdxFd, 0);

    if (s32Length != u32IndexHeaderLen)
    {
        HI_ERR_PVR("read headInfo failed! read: %d, want: %d\n", s32Length, u32IndexHeaderLen);
...
#ifdef HI_PVR_PID_CHANGE_SUPPORT
    if (u32IndexVersion >= 2)
    {
        pstPidCtrl = (PVR_INDEX_REC_PID_CTRL_S*)(pIndexHeadBuf + PVR_GET_PID_INFO_OFFSET(*pstHeaderInfo));

        s32Length = PVR_READ(&stStartFrame, sizeof(stStartFrame), pstFileOptInfo->s32DstIdxFd, u32IndexHeaderLen);
        if (s32Length != sizeof(stStartFrame))
        {
            HI_ERR_PVR("read index failed!read: %d, want: %u\n",s32Length, sizeof(stStartFrame));
            HI_FREE(HI_ID_PVR, pIndexHeadBuf);
            pIndexHeadBuf = HI_NULL;
            return HI_ERR_PVR_FILE_CANT_READ;
        }

        u32StartFrmDispTime = PVRINDEX_GET_DISPTIME(u32IndexVersion, &stStartFrame);
        (HI_VOID)PVRIndexProcPIDChangeInfoWhenLinearizationOrTruncate(pstPidCtrl, pstOldCycFrmInfo, u32StartFrmDispTime, bTimeshiftProcessed, u32HeadPosOffset);
    }
#endif

    pstHeaderInfo->stCycInfo.u32IsRewind = 0;
    pstHeaderInfo->stCycInfo.u32StartFrame = pstNewCycFrmInfo->u32StartFrame;
    pstHeaderInfo->stCycInfo.u32EndFrame = pstNewCycFrmInfo->u32EndFrame;
    pstHeaderInfo->stCycInfo.u32LastFrame = pstNewCycFrmInfo->u32LastFrame;
    pstHeaderInfo->u64ValidSize = 0;

    HI_WARN_PVR("New linear file: S/E/L: %u/%u/%u \n", pstNewCycFrmInfo->u32StartFrame, pstNewCycFrmInfo->u32EndFrame, pstNewCycFrmInfo->u32LastFrame);

    if (u32IndexVersion >= 3)
    {
    ...
    s32Length = PVR_WRITE(pIndexHeadBuf, u32IndexHeaderLen, pstFileOptInfo->s32DstIdxFd, 0);
    if (s32Length != u32IndexHeaderLen)
    {
...
HI_S32 PVR_Index_GetFrmNumByEntry(PVR_INDEX_HANDLE pstIndexHandle, PVR_INDEX_ENTRY_NEW_S stIndexEntry, HI_S32* ps32FrmNum)
{
    ...
    while ((HI_S32)u32StartFrmNum <= (HI_S32)u32EndFrmNum)
    {
        ...
        {
            if (HI_TRUE == PVRIndexIsFrameValid(pstIndexHandle->stCycMgr.u32StartFrame, pstIndexHandle->stCycMgr.u32EndFrame, pstIndexHandle->stCycMgr.u32LastFrame, u32MidFrmNum))
            {
                                  ...
                                  HI_U32 u32EndFrameNum)
{
    ...
    if (HI_TRUE != PVRIndexIsFrameValid(piIndexHandle->stCycMgr.u32StartFrame, piIndexHandle->stCycMgr.u32EndFrame, piIndexHandle->stCycMgr.u32LastFrame, u32EndFrameNum))
    {
