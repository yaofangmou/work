...
static HI_S32 CecMsgRx(CEC_MSG_S *pstMsg)
{
    ...
    pu8Data = &pstMsg->stOperand.stRawData.u8Data[0];
    for(pu8Tmp = pu8Data; pu8Tmp < pu8Data + CEC_MAX_RAW_SIZE ; pu8Tmp++)
    {
        CEC_NULL_CHK(pu8Tmp);
    }

    HDMI_REG_CecRxDataGet(&pstMsg->u8Opcode, pu8Data);
...
static HI_S32 CecMsgTx(CEC_MSG_S *pstMsg)
{
    ...
    HDMI_REG_CecTxDataSet(pstMsg->u8Opcode, pu8Data);
...
HI_S32 CecBroadAddr(HDMI_DEVICE_ID_E enHdmiId)
{
    ...
    stMsg.stOperand.stRawData.u8Data[2] = CEC_LOGICALADD_PLAYDEV_1; // [Device Type]
#else
    stMsg.stOperand.stRawData.u8Data[2] = CEC_LOGICALADD_TUNER_1;
#endif
...
static HI_S32 CecTxPoolWrite(CEC_INFO_S *pstCecInfo,CEC_MSG_S *pstMsg)
{
    ...
    if (CEC_OPCODE_STANDBY == pstMsg->u8Opcode)
    {
        ...
        HAL_HDMI_CecClearPool(pstCecInfo->enHdmiId);
        CecFlushTxFifo();
        CecMsgTx(pstMsg);
        ...
        switch (u32PingStatus)
        {
            case CEC_PING_ACK:
                ...
                *penMachStatus = CEC_MACH_PING_START;
                ...
                if (  pstCecInfo->stDrvInfo.au8Network[HDMI_CEC_LOGICALADD_TV]
                    || pstCecInfo->stDrvInfo.au8Network[HDMI_CEC_LOGICALADD_AUDIOSYSTEM])
                {
                    pstCecInfo->stDrvInfo.enCap = HDMI_CEC_CAP_SUPPORT;
                }
                break;
            case CEC_PING_NAK:
                ...
                if (  (!pstCecInfo->stDrvInfo.au8Network[HDMI_CEC_LOGICALADD_TV])
                    && (!pstCecInfo->stDrvInfo.au8Network[HDMI_CEC_LOGICALADD_AUDIOSYSTEM]) )
                {
                    pstCecInfo->stDrvInfo.enCap = HDMI_CEC_CAP_UNSUPPORT;
                }
    ...
    switch(pstCecInfo->enMachStatus)
    {
        ...
        case CEC_MACH_IDLE:
            ...
            if ( CecPingStartCheck(pstCecInfo) )
            {
                ...
                pstCecInfo->stDrvInfo.enCap = HDMI_CEC_CAP_UNKNOWN;
            }
...
HI_S32 HAL_HDMI_CecNetGet(HDMI_DEVICE_ID_E enHdmiId,HI_U8 *pu8PhyAddr)
{
    ...
    for(u32Idx=0;u32Idx<HDMI_ARRAY_SIZE(pstCecInfo->enPingAddrSequence);u32Idx++)
    {
        pstCecInfo->enPingAddrSequence[u32Idx] = CEC_LOGICALADD_BUTT;
    }

    for(u32Idx=0;u32Idx<HDMI_ARRAY_SIZE(pstCecInfo->stDrvInfo.au8Network);u32Idx++)
    {
        pstCecInfo->stDrvInfo.au8Network[u32Idx] = 0x00;
    }
...
HI_S32 HAL_HDMI_CecInfoGet(HDMI_DEVICE_ID_E enHdmiId,HDMI_CEC_INFO_S *pstDrvInfo)
{
    ...
    HAL_HDMI_CtrlHpdRsenGet(enHdmiId,&stHpdRsen);
    if( (!stHpdRsen.bHpdOn) || (!pstCecInfo->stDrvInfo.bCecEnable) )
    {
        pstDrvInfo->enCap = HDMI_CEC_CAP_UNKNOWN;
    }
    return HI_SUCCESS;
}
...
HI_VOID HAL_HDMI_CecClearPool(HDMI_DEVICE_ID_E enHdmiId)
{
    CEC_INFO_S          *pstCecInfo = CecInfoPtrGet(enHdmiId);
