...
#ifdef CHIP_TYPE_hi3798mv200
static HI_U32 isVp9Enhanced = 0;
static HI_U32 g_OrgCoreVoltVal = 0;
...
static HI_S32 VDEC_AddFrmBufRec(HI_S32 chanID, FRAME_BUF_S* frmBufRec, HI_U32* pu32BufIndex);
static HI_S32 VDEC_DelFrmBufRec(VDEC_CHANNEL_S* pstChan, HI_U32 u32BufIndex);
...
static HI_VOID VDEC_SetVp9Enhance(HI_U8 Value)
{
    ...
    if (Value == HI_TRUE)
    {
        ...
        g_OrgCoreVoltVal = *(HI_U32*)pCORE_VOLT;
    ...
    {
        pCORE_VOLT = ioremap_nocache( 0xf8a23020, 0x10000) ;
        *(HI_U32*)pCORE_VOLT = g_OrgCoreVoltVal;
        s32Ret = HI_DRV_MODULE_GetFunction(HI_ID_GPU, (HI_VOID**)&g_pGpuFunc);
        if ((s32Ret == HI_SUCCESS) && (g_pGpuFunc != HI_NULL))
        {
...
static HI_S32 VDEC_Event_ArrangeFrameBuf(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    HI_U32 u32ReportTime = 0;
#ifdef VFMW_VPSS_BYPASS_EN
    VDEC_VPSSCHANNEL_S* pstVpssChan = HI_NULL;
#endif
    HI_S32 s32ImgOutputEn;
    ...
    stReportFrmRec.needRecNoOutput = 1;
    ...
    pstChan->stRecordFrameTime.u32RecordNum = 0;
    pstChan->stRecordFrameTime.u32FreeNum = 0;
    pstChan->stRecordFrameTime.RecState = WAIT_ALLOC;
    HI_DRV_SYS_GetTimeStampMs(&u32ReportTime);
    pstChan->stRecordFrameTime.u32ReportTime = u32ReportTime;
    pstChan->stRecordFrameTime.u32RecvNum = 0;
...
static HI_S32 VDEC_Event_AllocFrameBufOnly(VDEC_CHANNEL_S* pstChan, HI_HANDLE hHandle, HI_VOID* pArgs)
{
    ...
    if (pstChan->stVFMWReportFrameInfo.u32ImageSize != pstChan->stVFMWReportFrameInfo.u32ImageSizeOnly)
    {
        pstChan->stRecordFrameTime.u32RecordNum = 0;
        pstChan->stRecordFrameTime.u32FreeNum = 0;
        pstChan->stRecordFrameTime.RecState = WAIT_ALLOC;
...
static HI_S32 VDEC_SetAttr(HI_HANDLE hHandle, VDEC_CHANNEL_S* pstChan)
{
    ...
    if (pstChan->OmxTunnelMode)
    {
        ...
        stVdecChanCfg.hVdecHandle = ((HI_ID_VDEC << 16) | pstChan->hOmxChn);
    }
    else
    {
...
static HI_S32 VDEC_Chan_DetachStrmBuf(HI_HANDLE hHandle)
{
    ...
    if (pstChan->stEsBuf.u32StartPhyAddr != 0)
    {
        pstChan->stEsBuf.u32StartPhyAddr = 0;
        pstChan->stEsBuf.u32Size = 0;
        pstChan->stEsBuf.pu8StartVirAddr = HI_NULL;
    }
...
static HI_S32 VDEC_ConvertWinInfo(HI_HANDLE hPort, HI_DRV_VPSS_PORT_CFG_S* pstVpssPortCfg, HI_DRV_WIN_PRIV_INFO_S* pstWinInfo)
{
    ...
    pstVpssPortCfg->bPassThrough = (pstWinInfo->bPassThrough == HI_TRUE) ? VPSS_BYPASS_AUTOBYVPSS : VPSS_FORCE_NOT_BYPASS;
...
static HI_S32 VDEC_Chan_SetExtBuffer(HI_HANDLE hVpss, VDEC_BUFFER_ATTR_PRI_S* pstBufferAttr)
{
    ...
    if (s32Ret == HI_SUCCESS)
    {
        stVpssPortCfg.bPassThrough = (pstBufferAttr->bVideoBypass == HI_TRUE) ? VPSS_BYPASS_AUTOBYVPSS : VPSS_FORCE_NOT_BYPASS;
...

{
    ...
    pstVideoPriv = (HI_DRV_VIDEO_PRIVATE_S*)(pstChan->stLastVpssFrm.u32Priv);
    if ((pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Width != pstChan->stRecordFrameTime.u32AvplayLastWidth
     || pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Height != pstChan->stRecordFrameTime.u32AvplayLastHeight)
   && pstVideoPriv->u32LastFlag != DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        HI_DRV_SYS_GetTimeStampMs(&u32CurrTime);
        pstChan->stRecordFrameTime.u32AvplayDelayTime = u32CurrTime - pstChan->stRecordFrameTime.u32AvplayLastRecvTime;
        pstChan->stRecordFrameTime.u32AvplayLastWidth = pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Width;
        pstChan->stRecordFrameTime.u32AvplayLastHeight = pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Height;
    }

    if (pstVideoPriv->u32LastFlag != DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        HI_DRV_SYS_GetTimeStampMs(&u32CurrTime);
        pstChan->stRecordFrameTime.u32AvplayLastRecvTime = u32CurrTime;
        pstChan->stRecordFrameTime.u32AvplayLastWidth = pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Width;
        pstChan->stRecordFrameTime.u32AvplayLastHeight = pstFrm->stFrame[pstFrm->u32FrmNum].stFrameVideo.u32Height;
    }
...
HI_S32 VDEC_AddFrmBufRec(HI_S32 chanID, FRAME_BUF_S* pstFrameBuf, HI_U32* pu32BufIndex)
{
    HI_U32 i = 0;
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
        if (HI_FALSE == pstChan->stFrameBufferRecord[i].bConfigured && HI_FALSE == pstChan->stFrameBufferRecord[i].bAvailable
         && HI_FALSE == pstChan->stFrameBufferRecord[i].bWaitRelease)
        {
...
HI_S32 VDEC_DelFrmBufRec(VDEC_CHANNEL_S* pstChan, HI_U32 u32BufIndex)
{
...
static HI_S32 VDEC_RecvFrm(HI_HANDLE hHandle, VDEC_CHANNEL_S* pstChan, HI_DRV_VIDEO_FRAME_S* pstFrame)
{
    ...
    if (pstLastPriv->u32LastFlag == DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        ...
        HI_DRV_SYS_GetTimeStampMs(&u32CurTime);
        pstChan->stRecordFrameTime.u32RecvTime[pstChan->stRecordFrameTime.u32RecvNum] = u32CurTime;
        pstChan->stRecordFrameTime.u32RecvNum++;

        pstChan->stRecordFrameTime.u32VpssFirstIRecvTime = u32CurTime;
        pstChan->stRecordFrameTime.u32VpssFirstIIndex = pstFrame->u32FrameIndex;
        pstChan->stRecordFrameTime.u32VpssDelayTime = u32CurTime - pstChan->stRecordFrameTime.u32VpssLastRecvTime;
        pstChan->stRecordFrameTime.u32VpssLastWidth = pstFrame->u32Width;
        pstChan->stRecordFrameTime.u32VpssLastHeight = pstFrame->u32Height;
        return HI_SUCCESS;
    }
    ...
    {
        pstVideoPriv->u32LastFlag = DEF_HI_DRV_VPSS_FAKE_FLAG;
        pstChan->stRecordFrameTime.u32ResizeStart = 1;
    }
    memcpy(&(pstChan->stLastFrm), pstFrame, sizeof(HI_DRV_VIDEO_FRAME_S));

    if ((pstFrame->u32Width != pstChan->stRecordFrameTime.u32VpssLastWidth
         || pstFrame->u32Height != pstChan->stRecordFrameTime.u32VpssLastHeight)
     && pstChan->stRecordFrameTime.u32VpssLastWidth != 0
     && pstChan->stRecordFrameTime.u32VpssLastHeight != 0
     && pstVideoPriv->u32LastFlag != DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        HI_DRV_SYS_GetTimeStampMs(&u32CurTime);
        pstChan->stRecordFrameTime.u32VpssFirstIRecvTime = u32CurTime;
    ...
    {
        HI_DRV_SYS_GetTimeStampMs(&u32CurTime);
        pstChan->stRecordFrameTime.u32VpssLastRecvTime = u32CurTime;
        pstChan->stRecordFrameTime.u32VpssLastWidth = pstFrame->u32Width;
        pstChan->stRecordFrameTime.u32VpssLastHeight = pstFrame->u32Height;
    }

    if (pstChan->stRecordFrameTime.u32ResizeStart == 1 && pstVideoPriv->u32LastFlag != DEF_HI_DRV_VPSS_FAKE_FLAG)
    {
        HI_DRV_SYS_GetTimeStampMs(&u32CurTime);
        pstChan->stRecordFrameTime.u32RecvTime[pstChan->stRecordFrameTime.u32RecvNum] = u32CurTime;
        pstChan->stRecordFrameTime.u32RecvNum++;
        if (pstChan->stRecordFrameTime.u32RecvNum > 10)
        {
            pstChan->stRecordFrameTime.u32ResizeStart = 0;
        }
    }
...
static HI_S32 VDEC_Chan_GetMemSize(HI_HANDLE hHandle)
{
    ...
    pstChan->stOption.eVidStd = VDEC_CodecTypeUnfToFmw(pstChan->stCurCfg.enType);
...
HI_S32 VDEC_Chan_AllocVFMWMem(HI_HANDLE hHandle)
{
...
#endif
    {
        ...
        else if (pstChan->stMemSize.ScdDetailMem == 0)
        {
            HI_U32  BufSize = HI_VDEC_EOS_MEM_SIZE;
            s32Ret = HI_DRV_VDEC_AllocAndMap("VDEC_Eos", HI_NULL, BufSize, 0, &pstChan->stEOSBuffer);
            VDEC_ASSERT_RETURN(s32Ret == HI_SUCCESS, HI_FAILURE);
        }
        ...
        else
        {
            HI_INFO_VDEC("Chan %d VDEC_CID_REPORT_OCCUQIED_FRM occupied frame = %d!\n", hHandle, stReportFrmRec.specialFrameNum);
            {
                ...
                for (i = 0; i < stReportFrmRec.specialFrameNum ; i++)
                {
                    ...
                    {
                        if (pstLastPriv->u32LastFlag == DEF_HI_DRV_VPSS_FAKE_FLAG &&
                            stReportFrmRec.specialFrmRec[i].PhyAddr == pstChan->stLastFrm.stBufAddr[0].u32PhyAddr_YHead)
                        {
                            pstLastPriv->u32LastFlag = 0;
                            stSpecialFrmInfo.bCanRls = HI_TRUE;
                        }
                        else
                        {
                            stSpecialFrmInfo.bCanRls = HI_FALSE;
...
HI_DRV_VPSS_SCENEMODE_E VDEC_Convert_SceneMode(HI_UNF_AVPLAY_SCENE_MODE_E unf_scene_mode)
{
    HI_DRV_VPSS_SCENEMODE_E vpss_scene_mode = HI_DRV_VPSS_SCENE_MODE_BUTT;
    switch (unf_scene_mode)
    {
        case HI_UNF_AVPLAY_SCENE_MODE_NORMAL:
            vpss_scene_mode = HI_DRV_VPSS_SCENE_MODE_NORMAL;
            break;
        case HI_UNF_AVPLAY_SCENE_MODE_VIDEOPHONE_PREVIEW:
            vpss_scene_mode = HI_DRV_VPSS_SCENE_MODE_VP_PREVIEW;
            break;
        case HI_UNF_AVPLAY_SCENE_MODE_VIDEOPHONE_REMOTE:
            vpss_scene_mode = HI_DRV_VPSS_SCENE_MODE_VP_REMOTE;
            break;
        case HI_UNF_AVPLAY_SCENE_MODE_VIRTUAL_REAL:
        case HI_UNF_AVPLAY_SCENE_MODE_BUTT:
            vpss_scene_mode = HI_DRV_VPSS_SCENE_MODE_BUTT;
            break;
    }

    return vpss_scene_mode;
}
...
HI_S32 HI_DRV_VDEC_SetSceneMode(HI_HANDLE hHandle, HI_UNF_AVPLAY_SCENE_MODE_E eSceneMode)
{
    ...
    {
        stVpssCfg.enSceneMode = VDEC_Convert_SceneMode(eSceneMode);
...
HI_S32 HI_DRV_VDEC_DecodeIFrame(HI_HANDLE hHandle, HI_UNF_AVPLAY_I_FRAME_S* pstStreamInfo,HI_BOOL bCapture, HI_BOOL bUserSpace)
{
    ...
    enType = pstChan->stCurCfg.enType;
    eVidStd = pstChan->stOption.eVidStd;
    pstChan->stCurCfg.enType = pstStreamInfo->enType;
    pstChan->stOption.eVidStd = VDEC_CodecTypeUnfToFmw(pstChan->stCurCfg.enType);
...
OUT0:
    ...
    pstChan->stCurCfg.enType = enType;
    pstChan->stOption.eVidStd = eVidStd;
    s32Ret |= VDEC_Chan_Destroy(hHandle);
...
static HI_S32 VDEC_Ioctl_CreatEsBuffer(struct file*  filp, unsigned int cmd, void* arg)
{
    ...
    else
    {
        ...
        pstEsBuf->pu8StartVirAddr = HI_NULL;
    }
...
//sdk start//
static HI_U32 s_VdecTaskSleepTimems = 2;
static HI_U32 u32TaskRunFlag = 0;

static HI_U32 VDEC_RecordReleaseMem(HI_HANDLE hHandle)
{
    HI_U32 i = 0;
    VDEC_CHANNEL_S* pstChan = NULL;
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
        ...
        if (!((s_stVdecDrv.stRemainFrmList.s32Num != 0) && (VDEC_IsSpecialFrm(&pstChan->stFrameBufferRecord[i].stFrameBuffer))))
        {
            ...
            {
                pstChan->stFrameBufferRecord[i].bWaitRelease = HI_TRUE;
            }
        }
        else
        {
    ...
    for (i = 0; i < VDEC_MAX_BUFFER_RECORD; i++)
    {
        if (pstChan->stFrameBufferRecord[i].bWaitRelease == HI_FALSE)
        {
            /*此处无需关心是否有special帧，因为此前都已经把special帧都记录到全局的数组里面了*/
            memset(&pstChan->stFrameBufferRecord[i], 0, sizeof(CHAN_FRAME_BUFFER_S));
            pstChan->stFrameBufferRecord[i].enFrameBufferNodeStatus = DRV_VDEC_BUFFER_STATUS_BUTT;
...
//Not vp9 release frame buffer
static HI_S32 VDEC_ReleaseMem(VDEC_CHANNEL_S *pstChan, HI_BOOL bRlsAll)
{
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    HI_U32 u32FreeNum = 0;
    HI_U32 u32FreeStartTime = 0;
    HI_U32 u32FreeEndTime = 0;
    HI_U32 index;
...
#endif
        {
            if (pstChan->stFrameBufferRecord[i].bWaitRelease == HI_TRUE)
            {
                //1:释放帧存节点
                index = pstChan->stRecordFrameTime.u32FreeNum;
                HI_DRV_SYS_GetTimeStampMs(&u32FreeStartTime);
                if (index < VDEC_MAX_BUFFER_RECORD)
                {
                    pstChan->stRecordFrameTime.u32FreeStartTime[index] = u32FreeStartTime;
                }
                ...
                //4:复位总帧存状态
                s32Ret = VDEC_DelFrmBufRec(pstChan, i);
                if (HI_SUCCESS != s32Ret)
                {
                ...
                u32FreeNum++;
                HI_DRV_SYS_GetTimeStampMs(&u32FreeEndTime);
                if (index < VDEC_MAX_BUFFER_RECORD)
                {
                    pstChan->stRecordFrameTime.u32FreeNum++;
                    pstChan->stRecordFrameTime.u32FreeEndTime[index] = u32FreeEndTime;
                }
            }
        }

        if ((bRlsAll == HI_FALSE) && (u32FreeNum > 0))
        {
            break;
        }
    }
...
static HI_VOID GetAllocMemInfo(VDEC_CHANNEL_S* pstChan, VDEC_AllocMem_Info_S *pstAllocMemInfo)
{
    ...
    if (pstChan->stCurCfg.enType == HI_UNF_VCODEC_TYPE_AVS2)
    {
        pstAllocMemInfo->u32NeededHdrMetadataNum = pstAllocMemInfo->u32NeededFrameNum;
    }
                               ...
                               FRAME_BUF_S   *pstFrameBuf)
{
    ...
    if (CheckIsAllMemReady(pstAllocMemInfo))
    {
        ...
        if (pstChan->stRecordFrameTime.RecState == WAIT_ALLOC)
        {
            HI_DRV_SYS_GetTimeStampMs(&u32ConfigTime);
            pstChan->stRecordFrameTime.RecState = WAIT_RESET;
            pstChan->stRecordFrameTime.u32ConfigTime = u32ConfigTime;
        }
...
// release frame for vp9
static HI_VOID VDEC_FreeMem(VDEC_CHANNEL_S *pstChan, HI_BOOL bRlsAll)
{
    HI_U32 u32Index = 0;
    HI_U32 u32FreeStartTime = 0;
    HI_U32 u32FreeEndTime = 0;
    ...
    for (u32Index = 0; u32Index < VDEC_MAX_BUFFER_RECORD; u32Index++)
    {
        ...
        {
            i = pstChan->stRecordFrameTime.u32FreeNum;
            HI_DRV_SYS_GetTimeStampMs(&u32FreeStartTime);
            if (i < VDEC_MAX_BUFFER_RECORD)
            {
                pstChan->stRecordFrameTime.u32FreeStartTime[i] = u32FreeStartTime;
            }
            ...
            if (pstChan->stFrameBufferRecord[u32Index].enHdrBufferNodeStatus != DRV_VDEC_BUFFER_STATUS_BUTT)
            {
                HI_DRV_VDEC_ReleaseMem(&pstChan->stFrameBufferRecord[u32Index].stHdrBuffer, HI_FALSE, HI_TRUE);

                pstChan->stFrameBufferRecord[u32Index].stHdrBuffer.u32Size = 0;
                pstChan->stFrameBufferRecord[u32Index].stHdrBuffer.u32StartPhyAddr = 0;
            ...
            if (i < VDEC_MAX_BUFFER_RECORD)
            {
                pstChan->stRecordFrameTime.u32FreeEndTime[i] = u32FreeEndTime;
                pstChan->stRecordFrameTime.u32FreeNum++;
            }
        }
...
static HI_S32 VDEC_AllocMemForDecoder(HI_HANDLE hHandle)
{
    ...
    HI_U32 u32AllocStartTime = 0;
    HI_U32 u32AllocEndTime = 0;
    ...
    if (CheckIsNeedAllocMem(hHandle, pstChan, &stAllocMemInfo))
    {
        ...
        if (0 == pstChan->stVFMWConfiguredBufferNum.u32ImageNum && pstChan->stVFMWReportFrameInfo.u32AllocFrameOnly == 0)
        {
            ...
            s32Ret = VDEC_RecordReleaseMem(hHandle);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_VDEC("VDEC_RecordReleaseMem err! return:%#x.\n", s32Ret);
            }
        }
        ...
        for (i = 0; i < stAllocMemInfo.u32NeededFrameNum; i++)
        {
            ...
            u32Index = pstChan->stRecordFrameTime.u32RecordNum;
            HI_DRV_SYS_GetTimeStampMs(&u32AllocStartTime);
            if (u32Index < VDEC_MAX_BUFFER_RECORD)
            {
                pstChan->stRecordFrameTime.u32AllocStartSysTime[u32Index] = u32AllocStartTime;
            }
            ...
            HI_DRV_SYS_GetTimeStampMs(&u32AllocEndTime);

            if (u32Index < VDEC_MAX_BUFFER_RECORD)
            {
                pstChan->stRecordFrameTime.u32AllocEndSysTime[u32Index] = u32AllocEndTime;
                pstChan->stRecordFrameTime.u32AllocTime[u32Index] = u32AllocEndTime - u32AllocStartTime;
                pstChan->stRecordFrameTime.u32RecordNum++;
            }
    ...
    {
        if (pstChan->stVFMWReportFrameInfo.u32AllocFrameOnly == 1 && stAllocMemInfo.u32NeededFrameNum == 0)
        {
            VDEC_FreeMem(pstChan, HI_TRUE);
        }
        if (pstChan->stVFMWReportFrameInfo.u32AllocFrameOnly == 0)
        {
            VDEC_ReleaseMem(pstChan, HI_TRUE);
        }
    }
...
static HI_S32 VdecTaskFunc(VOID* param)   //sdk
{
    HI_U32 i = 0;
    HI_S32 s32Ret = HI_SUCCESS;
    VDEC_CHANNEL_S* pstChan = HI_NULL;
...
{
    HI_S32 s32Ret;
    s32Ret = VDEC_Chan_SetAllocEn((*phHandle) & 0xff, set_dat);
    return s32Ret;
}
...
HI_S32 VDEC_DRV_GlobalRelFrmInter(HI_U32 u32Index)
{
...
#endif
    {
        ...
        {
            s32Ret = VDEC_Chan_ReleasePreMem(&pSpecialFrmRec->frmBufRec);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_VDEC("call VDEC_Chan_ReleasePreMem err, return :%#x \n", s32Ret);
                return HI_FAILURE;
    ...
    {
        s32Ret = VDEC_Chan_ReleasePreMem(&pSpecialFrmRec->hdrBufRec);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_VDEC("call VDEC_Chan_ReleasePreMem err, return :%#x \n", s32Ret);
            return HI_FAILURE;
...
HI_S32 HI_DRV_VDEC_GlobalRelFrm(HI_DRV_VIDEO_FRAME_S* pstFrame)
{
...
#endif
    {
        ...
        {
            s32Ret = VDEC_Chan_ReleasePreMem(&pSpecialFrmInfo->frmBufRec);
            if (HI_SUCCESS != s32Ret)
            {
                HI_ERR_VDEC("call VDEC_Chan_ReleasePreMem err, return :%#x \n", s32Ret);
                return HI_FAILURE;
    ...
    {
        s32Ret = VDEC_Chan_ReleasePreMem(&pSpecialFrmInfo->hdrBufRec);
        if (HI_SUCCESS != s32Ret)
        {
            HI_ERR_VDEC("call VDEC_Chan_ReleasePreMem err, return :%#x \n", s32Ret);
            return HI_FAILURE;
...
HI_S32 HI_DRV_VDEC_AllocBuf(BUFMNG_INST_CONFIG_S *pstConfig, VDEC_BUFFER_S* pstVDECAllocBuf)
{
    ...
    if (s32Ret != VDEC_OK)
    {
        HI_ERR_VDEC("alloc es buf error %d\n", s32Ret);
        return s32Ret;
    }
...
HI_S32 HI_DRV_VDEC_ReleaseBuf(VDEC_BUFFER_S* pstVDECBuf)
{
    ...
    if (s32Ret != VDEC_OK)
    {
        HI_ERR_VDEC("release es buf error %d\n", s32Ret);
        return s32Ret;
    }
