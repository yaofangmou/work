...
static HI_S32 winDrawCursor(HI_HANDLE hSurface, UI_ELEMENT_S* pstElement)
{
    ...
    HI_S32 s32W = 0;
    HI_S32 s32H = 0;
    HI_CHAR  acText[UI_D_WIDGHT_TEXT_MAXLEN] = {0};
    HI_CHAR* pcContent = pstElement->astContent[pstElement->u32ContentFocus].acText;

    HI_S32 s32Ret = HI_FAILURE;

    s32Ret = UI_GetTextExtent(pcContent, &s32W, &s32H);
    ...
    {
        COM_StrNCpy(acText, pcContent, pstElement->u32CursorPos);

        s32Ret = UI_GetTextExtent(acText, &s32W, &s32H);
        if (HI_SUCCESS != s32Ret)
        {
    ...
    COM_MemSet(acText, 0x00, sizeof(acText));
    acText[0] = pcContent[pstElement->u32CursorPos];
    s32Ret = UI_GetTextExtent(acText, &s32W, &s32H);
    if (HI_SUCCESS != s32Ret)
    {
        COM_PrintFuncErr(UI_GetTextExtent, s32Ret);
