...
static UINT64 PTS_Info_GetFrameTime_User(PTS_CHAN_INFO_S *pstPtsInfo, IMAGE *pstImage)
{
    ...
    /* the max pts difference(us) should be less then the 2*value of pts difference of FrameRateMin(12fps) and current rate */
    /* pts差值(us)应小于正常最小帧率下的pts的差值的2倍，且小于根据帧率计算的pts的差值的2倍 */
    ...
    if (FRD_INVALID_PTS_64 != pstPtsInfo->u64LastSrcPts)
    {
        ...
        if (u32NumPass)
        {
            if (u64PtsPass <= PTS_MAX_PASS_TIME)
            {
                ...
                /* delta should be less than pts difference of FrameRateMin(12fps) and current rate */
                /* delta 应小于正常最小帧率下的pts的差值且小于根据帧率计算的pts的差值，理论上等于帧率计算的pts差值/2 */
                if (pstImage->frame_rate >= 1000 && g_PtsChan[ChanID].s32InterPtsDelta > (1000 / 12)
                    && g_PtsChan[ChanID].s32InterPtsDelta > 1000 / (pstImage->frame_rate / 1000))
                {
                    g_PtsChan[ChanID].s32InterPtsDelta = 1000 / (pstImage->frame_rate / 1000 * 2);
                }
            }
            else
            {
...
static SINT32 PTS_Info_GuessFirstFrameRate(SINT32 ChanID, UINT32 u32SrcFrameRate, IMAGE *pstImage)
{
    ...
    else if ((12 * 1000 <= u32SrcFrameRate) && (u32SrcFrameRate <= 120 * 1000))
    {
        pstImage->frame_rate = u32SrcFrameRate;
    }
    else
    {
        if (pstImage->image_width <= 720 && pstImage->image_height <= 480)
