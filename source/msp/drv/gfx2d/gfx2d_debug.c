...
HI_VOID DRV_GFX2D_OutputMessage(const HI_CHAR* pFunc, const HI_U32 Line, const HI_GFX_MSG_TYPE_E eMsgType, const HI_U32 Format, ...)
{
    ...
    if (NULL == pFunc)
    {
        HI_GFX_Print("[module-gfx-gfx2d][err] : input func pointer %p\n",pFunc);
        return;
    }

    if ((HI_GFX_MSG_BEG == eMsgType) || (HI_GFX_MSG_END == eMsgType))
    {
        FormatMessage("gfx2d", pFunc, Line, eMsgType, TmpFormat, &LogErr, &LogFunc, &LogValue);
        return;
    }
...
{
    DRV_GFX_SetLogFunc("gfx2d", pFunc);
    return;
}
