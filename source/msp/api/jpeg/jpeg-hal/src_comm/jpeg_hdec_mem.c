...
static HI_BOOL JPEG_HDEC_AllocDecodeMemory(HI_ULONG HdecHandle)
{
    ...
    if (HI_SUCCESS != Ret)
    {
        ...
        {
            HI_GFX_Print("[module-gfx-jpeg] : %s[%d] : alloc input data buffer failure\n",   __FUNCTION__,__LINE__);
        }
        return HI_FALSE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        if (7 == pJpegHandle->LogLevel)
        {
            HI_GFX_Print("[module-gfx-jpeg] : %s[%d] : alloc yuv buffer failure\n",   __FUNCTION__,__LINE__);
        }
        return HI_FALSE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        if (7 == pJpegHandle->LogLevel)
        {
            HI_GFX_Print("[module-gfx-jpeg] : %s[%d] : alloc min buffer failure\n",   __FUNCTION__,__LINE__);
        }
        return HI_FALSE;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        if (7 == pJpegHandle->LogLevel)
        {
            HI_GFX_Print("[module-gfx-jpeg] : %s[%d] : alloc rgb buffer failure\n",   __FUNCTION__,__LINE__);
        }
        return HI_FALSE;
    }
...
static HI_S32 JPEG_HDEC_AllocYUVMem(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    if (6 == pJpegHandle->LogLevel)
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("alloc yuv buffer\n");
        HI_GFX_Print("pJpegHandle->bOutUsrBuf              = %d\n", pJpegHandle->bOutUsrBuf);
        HI_GFX_Print("pJpegHandle->bOutYCbCrSP             = %d\n",pJpegHandle->bOutYCbCrSP);
        HI_GFX_Print("pJpegHandle->bDecOutColorSpaecXRGB   = %d\n",pJpegHandle->bDecOutColorSpaecXRGB);
        HI_GFX_Print("pJpegHandle->stJpegSofInfo.u32YSize  = %d\n",pJpegHandle->stJpegSofInfo.u32YSize);
        HI_GFX_Print("pJpegHandle->stJpegSofInfo.u32CSize  = %d\n",pJpegHandle->stJpegSofInfo.u32CSize);
        HI_GFX_Print("============================================================================\n");
    }
    ...
    if (6 == pJpegHandle->LogLevel)
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("alloc yuv buffer\n");
        HI_GFX_Print("pJpegHandle->stDecSurface.BufPhy[0] = 0x%x\n", pJpegHandle->stDecSurface.BufPhy[0]);
        HI_GFX_Print("pJpegHandle->stDecSurface.BufPhy[1] = 0x%x\n", pJpegHandle->stDecSurface.BufPhy[1]);
        HI_GFX_Print("============================================================================\n");
    }
...
static HI_S32 JPEG_HDEC_AllocXRGBMem(JPEG_HDEC_HANDLE_S_PTR pJpegHandle)
{
    ...
    {
        HI_GFX_Print("============================================================================\n");
        HI_GFX_Print("alloc xrgb buffer\n");
        HI_GFX_Print("width  = %d\n", pJpegHandle->stOutSurface.stCropRect.w);
        HI_GFX_Print("height = %d\n", pJpegHandle->stOutSurface.stCropRect.h);
        HI_GFX_Print("u32OutStride = %d\n", u32OutStride);
        HI_GFX_Print("u32MemSize   = %d\n", u32MemSize);
        HI_GFX_Print("pJpegHandle->stOutSurface.BufPhy[0] = 0x%x\n", pJpegHandle->stOutSurface.BufPhy[0]);
        HI_GFX_Print("pJpegHandle->stOutSurface.BufPhy[1] = 0x%x\n", pJpegHandle->stOutSurface.BufPhy[1]);
        HI_GFX_Print("============================================================================\n");
    }
