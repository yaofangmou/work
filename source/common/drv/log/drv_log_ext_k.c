...
static HI_BOOL LOGLevelChk(HI_S32 s32Levle, HI_U32 enModId)
{
    if (enModId < LOG_CONFIG_BUF_SIZE/sizeof(LOG_CONFIG_INFO_S))
    {
        ...
        if (g_LogModInit && (s32Levle <= g_pLogConfigInfo[enModId].u8LogLevel) && (g_pLogConfigInfo[enModId].u8LogLevel != HI_LOG_LEVEL_TRACE))
        {
            return HI_TRUE;
        }
        if (g_LogModInit && (s32Levle == g_pLogConfigInfo[enModId].u8LogLevel) && (g_pLogConfigInfo[enModId].u8LogLevel == HI_LOG_LEVEL_TRACE))
...
static HI_U8 szBuf[700] = {0};
static HI_U8 szFileName[MAX_FILENAME_LENTH] = {0};
