...
static HI_S32 DRV_ADVCA_ProcGetSecretKeyChecksumStatus(struct seq_file* p)
{
    ...
    if (0 == unChecksum.bits.JTAG_Key)
    {
        PROC_PRINT(p, "--JTAG_Key verify:           failed\n");
    }
    else
    {
        PROC_PRINT(p, "--JTAG_Key verify            success\n");
    }
    ...
    if (0 == unChecksum.bits.MISC_RK_CSA3)
    {
        PROC_PRINT(p, "--MISC_RK_CSA3 verify:       failed\n");
    }
    else
    {
        PROC_PRINT(p, "--MISC_RK_CSA3 verify:       success\n");
    }
    ...
    if (0 == unChecksum.bits.MISC_RK_SM4)
    {
        PROC_PRINT(p, "--MISC_RK_SM4 verify:        failed\n");
    }
    else
    {
        PROC_PRINT(p, "--MISC_RK_SM4 verify:        success\n");
    }
    ...
    if (0 == unChecksum.bits.hisi_rootkey)
    {
        PROC_PRINT(p, "--HISI_RootKey verify:       failed\n");
    }
    else
    {
        PROC_PRINT(p, "--HISI_RootKey verify:       success\n");
    }

    if (0 == unChecksum.bits.MISC_RK_AES)
    {
        PROC_PRINT(p, "--MISC_RK_AES verify:        failed\n");
    }
    else
    {
        PROC_PRINT(p, "--MISC_RK_AES verify:        success\n");
    }
    ...
    if (0 == unChecksum.bits.deob_ext_key)
    {
        PROC_PRINT(p, "--DEOB_EXT_Key verify:       failed\n");
    }
    else
    {
        PROC_PRINT(p, "--DEOB_EXT_Key verify:       success\n");
    }

    if (0 == unChecksum.bits.smk_ext_key)
    {
        PROC_PRINT(p, "--SMK_EXT_Key verify:        failed\n");
    }
    else
    {
        PROC_PRINT(p, "--SMK_EXT_Key verify:        success\n");
    }
    return HI_SUCCESS;
}
...
#ifdef CHIP_TYPE_hi3798mv310
    PROC_PRINT(p, "RSA/SM2 CRC check infomation: addr 0x%08x: value 0x%08x\n", au32Debug[0], au32Debug[1]);
    crc_check_sta.u32 = au32Debug[1];
    if (0 == crc_check_sta.bits.rsa_crc_finish)
    {
        PROC_PRINT(p, "--RSA CRC Check unfinished\n");
    }
    else if (0 == crc_check_sta.bits.rsa_crc_ok_flag)
    {
        PROC_PRINT(p, "--RSA CRC Check:        failed\n");
    }
    else
    {
        PROC_PRINT(p, "--RSA CRC Check:        success\n");
    }

    if (0 == crc_check_sta.bits.sm2_crc_finish)
    {
        PROC_PRINT(p, "--SM2 CRC Check unfinished\n");
    }
    else if (0 == crc_check_sta.bits.sm2_crc_ok_flag)
    {
        PROC_PRINT(p, "--SM2 CRC Check:        failed\n");
    }
    else
    {
        PROC_PRINT(p, "--SM2 CRC Check:        success\n");
    }
