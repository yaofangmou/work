...
HI_VOID Crypto::findFactoryForScheme(const HI_U8 uuid[HI_DRM_UUID_LEN])
{
    ...
    if (hisi_index >= 0)
    {
        ...
        {
            HI_LOGD("Successed to load from cached library path!");
            mInitCheck = HI_TRUE;
            return;
        }
        else
...
HI_S32 Crypto::decrypt(
    ...
    const sp<IMemory> &sharedBuffer, HI_S32 offset,
    ...
    AString* errorDetailMsg)
{
    ...
    const HI_VOID* srcPtr = static_cast<uint8_t *>(sharedBuffer->pointer());/* + offset;*/
