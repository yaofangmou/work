...
static inline HI_U32 HI_GFX_AllocMem(HI_U32 u32Size, HI_U32 u32Align, HI_CHAR* pZoneName, HI_CHAR *pName, HI_BOOL *pbMmu)
{
    ...
    if ((NULL == pName) || (NULL == pbMmu))
    {
        HI_GFX_Print("=====the input para is null!\n");
        return 0;
    }
...
static inline HI_S32 gfx_mem_open()
{
    ...
    if (fd < 0)
    {
         HI_GFX_Print("open /dev/ion failed!\n");
    }
...
static inline HI_S32 gfx_ion_ioctl(int fd, int req, void *arg)
{
    ...
    if (ret < 0)
    {
       HI_GFX_Print("ioctl %x failed with code %d\n", req,ret);
       return HI_FAILURE;
    }
...
static inline HI_S32 gfx_ion_smmu_free(int fd, ion_user_handle_t handle, unsigned long Phyaddr)
{
    ...
    struct ion_map_iommu_data map_iommu =
        {
             .handle = handle,
             .format.iova_start = Phyaddr,
             .format.iova_size  = 0,
             .format.iommu_ptb_base = 0,
             .format.iommu_iova_base = 0,
             .format.phys_page_line = 0,
             .format.virt_page_line = 0,
             .format.is_tile = 0,
        };
...
static inline HI_S32 gfx_ion_map(int fd, ion_user_handle_t handle, size_t length, int prot, int flags, off_t offset, unsigned char **ptr, int *map_fd)
{
    ...
    {
       HI_GFX_Print("map length is zero\n");
       return -EINVAL;
    }
    ...
    *map_fd = data.fd;
    if (*map_fd < 0)
    {
        HI_GFX_Print("map ioctl returned negative fd\n");
        return -EINVAL;
    }
...
static inline HI_U32 HI_GFX_AllocMem(HI_S32 s32MMZDev,HI_U32 u32Size, HI_U32 u32Align, HI_CHAR* pZoneName, HI_CHAR *pName, HI_VOID **ppMemHandle, HI_BOOL *pbMmu)
{
    ...
    if ((NULL == ppMemHandle) || (NULL == pbMmu) || (s32MMZDev < 0))
    {
       return 0;
    }
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_GFX_Print("ion_alloc failed!size:0x%x\n", u32Size);
        free(pstMemHandle);
        *ppMemHandle = NULL;
        return 0;
    }
