...
static const OMXVDEC_CODEC_INFO_ST g_CodecTable[] =
{
    ...
    {OMX_AVS,      "AVS"},
    {OMX_AVS2,     "AVS2"},
    {OMX_SORENSON, "SORENSON"},
    {OMX_VC1,      "VC1"},
    {OMX_MJPEG,    "MJPEG"},
    {OMX_END_RESERVED, NULL},
};
...
static HI_S32 channel_get_codec_table_index(HI_U32 StdId)
{
    ...
    while (1)
    {
        if (g_CodecTable[Index].StdId == OMX_END_RESERVED)
        {
            break;
        }
...
static OMXVDEC_BUF_S *channel_lookup_addr_table(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    if (i < *num_of_buffers)
    {
        ...
        buf_addr_table[i].user_vaddr = (HI_VOID *)(HI_SIZE_T)puser_buf->bufferaddr;
...
static HI_U32 channel_insert_addr_table(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    pbuf->user_vaddr  = (HI_VOID *)(HI_SIZE_T)puser_buf->bufferaddr;
    pbuf->phy_addr    = puser_buf->phyaddr;
    pbuf->private_phy_addr = puser_buf->private_phyaddr;
    pbuf->kern_vaddr  = kern_vaddr;
    pbuf->client_data = (HI_VOID *)(HI_SIZE_T)puser_buf->client_data;
...
static HI_S32 channel_vdec_report_process(OMXVDEC_CHAN_CTX *pchan, HI_U32 EventID, HI_U32 *pstatus, HI_VOID *PrivData)
{
    unsigned long flags;
...
HI_S32 channel_Vdec_Report(HI_S32 chan_id, HI_U32 EventID, HI_S32 result, HI_VOID *PrivData)
{
    ...
    {
        ret = channel_vdec_report_process(pchan, EventID, &status, PrivData);
        if (ret != HI_SUCCESS)
        {
            OmxPrint(OMX_FATAL, "Chan:%d report process failed\n", chan_id);
            VDEC_UP_INTERRUPTIBLE(&pchan->chan_mutex);
            return HI_FAILURE;
        }
    }
...
HI_VOID channel_init_inst_config(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_DRV_CFG *pcfg)
{
    ...
    pchan->bProDetectOff = pcfg->bProDetectOff;
    pchan->bIsOnLive = pcfg->bIsOnLive;
    pchan->bIsGst    = pcfg->bIsGst;
    pchan->force_bypass = pcfg->force_bypass;
    pchan->is_adaptive  = pcfg->is_adaptive;

    pchan->max_width   = pcfg->max_width;
    pchan->max_height  = pcfg->max_height;
    pchan->bMetaDataAlloc = pcfg->bMetaDataAlloc;
    pchan->bIndirectInput = pcfg->bIndirectInput;
    OmxPrint(OMX_INFO, "%s chan bMeta %d\n", __func__, pchan->bMetaDataAlloc);

    if (pcfg->chan_cfg.eVidStd == OMX_VP6 || pcfg->chan_cfg.eVidStd == OMX_VP6F || pcfg->chan_cfg.eVidStd == OMX_VP6A)
    {
...
HI_S32 channel_global_release_frame(OMXVDEC_CHAN_CTX *pchan, OMXVDEC_BUF_DESC *puser_buf)
{
    ...
    ret = decoder_global_release_frame(&stFrame, HI_TRUE);
    if (ret != HI_SUCCESS)
    {
...
HI_S32 channel_set_port(OMXVDEC_CHAN_CTX *pchan, HI_BOOL enable)
{
    HI_S32 ret    = HI_SUCCESS;
...
HI_VOID channel_proc_entry(struct seq_file *p, OMXVDEC_CHAN_CTX *pchan)
{
    ...
    if (OMX_VP6 == pchan->protocol || OMX_VP6A == pchan->protocol || OMX_VP6F == pchan->protocol)
    {
    ...
    if (pchan->dfs.alloc_pmv_num != 0)
    {
        ...
        PROC_PRINT(p, "%-25s :%d\n",          "FailedCount", pchan->dfs.alloc_from_cached_failed);
    }
    ...
    PROC_PRINT(p, "%-10s :%d\n", "Frame Buffer",   pchan->dfs.alloc_frm_num);
    PROC_PRINT(p, " %-10s%-10s%-10s%-10s\n", "phyaddr", "size", "WaitTime(ms)", "AllocTime(ms)");

    for (i = 0; i < MAX_DFS_BUF_NUM; i++)
    {
        pBufSlot = &(pchan->dfs.single_buf[i]);
        if (pBufSlot->frm_buf.u32StartPhyAddr != 0)
        {
           PROC_PRINT(p, " %-10x%-10d%-10d%-10d\n",
                         pBufSlot->frm_buf.u32StartPhyAddr,
                         pBufSlot->frm_buf.u32Size,
                         pBufSlot->frame_wait_time,
                         pBufSlot->alloc_frame_time);
        }
    }
    PROC_PRINT(p, "\n");

    PROC_PRINT(p, "%-10s :%d\n", "Pmv Buffer",   pchan->dfs.alloc_pmv_num);
    PROC_PRINT(p, " %-10s%-10s%-10s%-10s\n", "phyaddr", "size", "WaitTime(ms)", "AllocTime(ms)");

    for (i = 0; i < MAX_DFS_BUF_NUM; i++)
    {
        pBufSlot = &(pchan->dfs.single_buf[i]);
        if (pBufSlot->pmv_buf.u32StartPhyAddr != 0)
        {
           PROC_PRINT(p, " %-10x%-10d%-10d%-10d\n",
                         pBufSlot->pmv_buf.u32StartPhyAddr,
                         pBufSlot->pmv_buf.u32Size,
                         pBufSlot->pmv_wait_time,
                         pBufSlot->alloc_pmv_time);
        }
    }
    PROC_PRINT(p, "\n");
    if ((pchan->bLowdelay || g_FastOutputMode) && (NULL != pchan->omx_vdec_lowdelay_proc_rec))
