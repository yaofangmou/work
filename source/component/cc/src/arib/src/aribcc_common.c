...
HI_S32 ARIBCC_Com_FreePESPtr (ARIBCC_CaptionType_E enCaptionType, HI_U8 *pu8CcPesPtr)
{
    ...
    if (u16NextRp == pstPes->u16Rp)
    {
        HI_INFO_CC("Rp is equal to NextRp, clear Rp!\n");
        pstPesData[pstPes->u16Rp].pu8Ptr = HI_NULL;
    }
    else
    {
...
HI_S32 ARIBCC_Com_ClearTimer (ARIBCC_CaptionType_E enCaptionType)
{
    ...
    if ((u16Wp != u16Rp) && ((pstTimerData[u16Rp].stTimeValue.tv_sec == 0) && (pstTimerData[u16Rp].stTimeValue.tv_usec== 0)))
    {
...
HI_S32 ARIBCC_Com_MessageInit(HI_VOID)
{
    ...
    if (NULL == s_aribcc_msgqueue_baseaddr)
    {
        s_aribcc_msgqueue_baseaddr = (HI_U8 *)malloc(ARIBCC_MSGQUEUE_SIZE_INIT);
        if (NULL == s_aribcc_msgqueue_baseaddr)
        {
            HI_ERR_CC("aribcc msgqueue malloc fail !\n");
            return HI_FAILURE;
        }
    }

    memset(&s_aribcc_msgqueue_handle, 0, sizeof(ARIBCC_SEGMENT_QUEUE_S));
    s32Ret = ARIBCC_PesQueue_Init(&s_aribcc_msgqueue_handle, s_aribcc_msgqueue_baseaddr, ARIBCC_MSGQUEUE_SIZE_INIT);
    if (s32Ret != HI_SUCCESS)
    {
        HI_ERR_CC("aribcc pesqueue init fail !\n");
...
{
    (HI_VOID)ARIBCC_PesQueue_Deinit(&s_aribcc_msgqueue_handle);
...
HI_VOID ARIBCC_Com_MessageReset(HI_VOID)
{
    if (s_aribcc_msgqueue_baseaddr != NULL)
    {
        (HI_VOID)ARIBCC_PesQueue_Reset(&s_aribcc_msgqueue_handle, s_aribcc_msgqueue_baseaddr, ARIBCC_MSGQUEUE_SIZE_INIT);
    }
