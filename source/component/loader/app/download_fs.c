...
static HI_S32 fsRestructureFileName(HI_CHAR* pcFileName, HI_CHAR* pcDesFileName, HI_U32 u32DesLenth)
{
    HI_U32 u32Len = 0;

    COM_DBG_FuncEnter();

    u32Len = COM_StrLen(pcFileName);
    if ((0 == u32Len)
        || (u32DesLenth <= u32Len))
    {
    ...
    if ('/' == pcFileName[0])
    {
        snprintf(pcDesFileName, u32DesLenth, "%s%s", COM_CFG_D_UPGRADE_FILEPATH, pcFileName);
    }
    else
    {
        snprintf(pcDesFileName, u32DesLenth, "%s/%s", COM_CFG_D_UPGRADE_FILEPATH, pcFileName);
...
HI_S32 DOWNLOAD_FS_Init(HI_LOADER_PARAM_S* pstLoaderParam)
{
    ...
    {
        s32Ret = fsRestructureFileName((HI_CHAR*)pstLoaderParam->unParam.stFSParam.as8FileName, acFileName, HI_LOADER_FILENAME_LEN);
        if (HI_SUCCESS != s32Ret)
        {
        ...
        s32Ret = fsFileCheckInPartiton((HI_CHAR*)pstLoaderParam->unParam.stFSParam.as8PartitonName, acFileName);
        if (HI_SUCCESS != s32Ret)
        {
    ...
    {
        s32Ret = fsRestructureFileName((HI_CHAR*)pstLoaderParam->unParam.stUSBParam.as8FileName, acFileName, HI_LOADER_FILENAME_LEN);
        if (HI_SUCCESS != s32Ret)
        {
            COM_PrintFuncErr(fsRestructureFileName, s32Ret);
            return s32Ret;
