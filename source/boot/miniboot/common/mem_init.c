...
static struct ddrlayout_t ddrlayout[] = {
	{
		...
		.start = DDRLAYOUT_AUTO,
		.end = DDRLAYOUT_AUTO,
		.size = DDR_LAYOUT_SIZE_EARLY_RESERVE_MEM,
		.attribue = 0,
		.name = "early reserve",
		.init = NULL,
	}, {
...
int get_ddr_layout_by_name(char *name, struct ddrlayout_t *tmp)
{
        struct ddrlayout_t *layout = ddrlayout;

        if(!tmp)
                return -1;

        for (; layout->name; layout++) {
                if(!strncmp(name, layout->name, strlen(layout->name))) {
                        memcpy(tmp, layout, sizeof(struct ddrlayout_t));
                        return 0;
                }
        }
        return -1;
