...
void sys_srst(void)
{
	...
	if ((_sys_srst_end - _sys_srst_startup) > SYS_SRST_MAX_SIZE) {
		uart_early_puts("ERROR: System soft reset code size(0x");
		uart_early_put_hex(_sys_srst_end - _sys_srst_startup);
		uart_early_puts(" Bytes) over 0x");
		uart_early_put_hex(SYS_SRST_MAX_SIZE);
		uart_early_puts(" Bytes, reset fail\n");
		while(1);
	}
