	...
	list_for_each_entry_safe(sync_pt, next, &sync_timeline->sync_pt_list_head,
		...
		if (fence_is_signaled_locked(&sync_pt->base)) {
			...
			if(atomic_read(&sync_pt->base.refcount.refcount) > 0) {
				/* "fence_put -> fence_release -> mali_internal_fence_release",
				   mali_internal_fence_release will lock the spin_lock "fence->lock",
				   "fence->lock" == "sync_timeline->sync_pt_list_lock",
				   to prevent the dead lock happend, we need unlock "sync_timeline->sync_pt_list_lock" */
