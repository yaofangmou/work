...
HI_S32 HI_JPEG_SetOutInfo(HI_ULONG DecHandle, HI_JPEG_OUT_INFO_S *pstOutInfo)
{
    ...
    stImgInfo.output_color_space = HI_JPEG_FMT_BUTT;
...
HI_S32 HI_JPEG_SetOutDesc(HI_ULONG DecHandle, const HI_JPEG_SURFACE_S *pstSurfaceDesc)
{
    ...
    if (NULL == pJpegHandle)
    {
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pJpegHandle = %p\n", __FUNCTION__,__LINE__,pJpegHandle);
        return HI_FAILURE;
    }

    if (NULL == pstSurfaceDesc)
    {
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc = %p\n", __FUNCTION__,__LINE__,pstSurfaceDesc);
        return HI_FAILURE;
    }

    if (   (0 == pstSurfaceDesc->u32OutPhy[0])    || (NULL == pstSurfaceDesc->pOutVir[0])
        || (0 == pstSurfaceDesc->u32OutStride[0]) || (0 == pstSurfaceDesc->u32OutSize[0]))
    {
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutPhy[0] = 0x%x\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutPhy[0]);
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->pOutVir[0] = %p\n", __FUNCTION__,__LINE__,pstSurfaceDesc->pOutVir[0]);
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutStride[0] = %d\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutStride[0]);
        HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutSize[0] = %d\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutSize[0]);
        return HI_FAILURE;
    }

    if ((HI_JPEG_FMT_YUV400 != pJpegHandle->stImgInfo.output_color_space) && (HI_TRUE == pJpegHandle->bOutYCbCrSP))
    {
        if (   (0 == pstSurfaceDesc->u32OutPhy[1])    || (NULL == pstSurfaceDesc->pOutVir[1])
            || (0 == pstSurfaceDesc->u32OutStride[1]) || (0 == pstSurfaceDesc->u32OutSize[1]))
        {
            HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutPhy[1] = 0x%x\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutPhy[1]);
            HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->pOutVir[1] = %p\n", __FUNCTION__,__LINE__,pstSurfaceDesc->pOutVir[1]);
            HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutStride[1] = %d\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutStride[1]);
            HI_GFX_Print("[module-gfx-jpeg][err] : %s[%d] : pstSurfaceDesc->u32OutSize[1] = %d\n", __FUNCTION__,__LINE__,pstSurfaceDesc->u32OutSize[1]);
            return HI_FAILURE;
        }
    }
    ...
    for (Cnt = 0; Cnt < HI_JPEG_MAX_COMPONENT_NUM; Cnt++)
    {
        ...
        pJpegHandle->stOutSurface.Size[Cnt] = pstSurfaceDesc->u32OutSize[Cnt];
        if (HI_TRUE == pJpegHandle->bOutYCbCrSP)
        {
           ...
           pJpegHandle->stDecSurface.Size[Cnt] = pstSurfaceDesc->u32OutSize[Cnt];
        }
    }
