...
static SINT32 TVP_VDEC_ReadProc(struct seq_file *p, VOID *v)
{
    ...
    VFMW_PROC_PRINT(p, "\n");
    VFMW_PROC_PRINT(p, "------------------------- SECURE INFO -------------------------\n");
    VFMW_PROC_PRINT(p, "SecEnvSetUp        : %-7d  | SecPrintEnable      : 0x%-5x\n", g_bSecEnvSetUp,  g_SecPrintEnable);
#ifdef HI_SMMU_SUPPORT
    VFMW_PROC_PRINT(p, "ThreadState        : %-7d  | VfmwMemPhyAddr      : 0x%x\n",   g_THREAD_STATE,  g_SecVfmwMem.MmuAddr);
#else
    VFMW_PROC_PRINT(p, "ThreadState        : %-7d  | VfmwMemPhyAddr      : 0x%x\n",   g_THREAD_STATE,  g_SecVfmwMem.PhyAddr);
#endif
    VFMW_PROC_PRINT(p, "SecureInstNum      : %-7d  | VfmwMemLength       : %d\n",     g_SecureInstNum, g_SecVfmwMem.Length);
#if (1 == DEBUG_SAVE_SUPPORT)
    VFMW_PROC_PRINT(p, "RawSaveEnable      : %-7d  | SavePath            : %s\n",     g_RawSaveEnable, g_SavePath);
#endif
    VFMW_PROC_PRINT(p, "PrintTime          : %-7d\n", g_bPrintTime);
    VFMW_PROC_PRINT(p, "\n");
    ...
    for (i = 0; i < MAX_CHAN_NUM; i++)
    {
        ...
        {
            VFMW_PROC_PRINT(p, "Chan %d\n", i);
            VFMW_PROC_PRINT(p, "State              : %-7d  | SecMode             : %d\n", g_ChanContext[i].ChanState, g_ChanContext[i].bIsSecMode);

            if (HI_TRUE == g_ChanContext[i].bIsSecMode)
            {
                VFMW_PROC_PRINT(p, "StreamList         : %d/%d (%d,%d,%d,%d)\n",
    ...
    if (g_bSecEnvSetUp && g_SecureInstNum > 0)
    {
        ...
        VFMW_PROC_PRINT(p, proc_buf);
    }
    else
    {
        VFMW_PROC_PRINT(p, " No secure channel exit.\n");
    }
