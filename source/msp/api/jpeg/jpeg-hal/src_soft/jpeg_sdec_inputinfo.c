...
static inline HI_VOID JPEG_SDEC_SetInputImgeInfo(j_decompress_ptr cinfo, const HI_CHAR* pFunc)
{
    HI_S32 Cnt = 0;
    HI_S32 Cnt1 = 0;
    HI_S32 Cnt2 = 0;
...
static inline HI_VOID JPEG_SDEC_SaveInputImge(j_decompress_ptr cinfo)
{
    HI_BOOL bFillRet = HI_FALSE;
    HI_BOOL bSupportSave = HI_FALSE;
    HI_CHAR* next_input_byte;
    HI_U32 bytes_in_buffer;
    HI_U32 curpos_in_buffer;
    HI_U32 ReadLoopTimes = 0;

    JPEG_DEC_SupportSaveInputImg(&bSupportSave);
    if (HI_FALSE == bSupportSave)
    {
        return;
    }

    bytes_in_buffer  = cinfo->src->bytes_in_buffer;
    next_input_byte  = (HI_CHAR*)cinfo->src->next_input_byte;
    curpos_in_buffer = cinfo->src->current_offset - cinfo->src->bytes_in_buffer;

    do
    {
        if (0 != cinfo->src->bytes_in_buffer)
        {
            JPEG_DEC_SaveInputImg((HI_CHAR*)cinfo->src->next_input_byte, cinfo->src->bytes_in_buffer, HI_FALSE);
        ...
        {
            JPEG_DEC_SaveInputImg((HI_CHAR*)cinfo->src->next_input_byte, cinfo->src->bytes_in_buffer, HI_TRUE);
            break;
        }
        ...
        /** read 100 times can finish **/
        ReadLoopTimes++;
    }while (ReadLoopTimes < 50000);

    if (NULL == cinfo->src->seek_input_data)
    {
        cinfo->src->next_input_byte = (JOCTET*)next_input_byte;
        cinfo->src->bytes_in_buffer = bytes_in_buffer;
        return;
    }

    JPEG_DEC_PrintMsg((HI_ULONG)cinfo->client_data, (const HI_CHAR*)__func__,__LINE__, HI_TRUE);
    cinfo->src->seek_input_data(cinfo, curpos_in_buffer);
    JPEG_DEC_PrintMsg((HI_ULONG)cinfo->client_data, (const HI_CHAR*)__func__,__LINE__, HI_FALSE);
