...
static HI_VOID CI_DRV_ProcInit(HI_VOID)
{
    DRV_PROC_EX_S stProc = {0};

    stProc.fnRead   = CI_ProcRead;
    stProc.fnWrite  = CI_ProcWrite;
     
    HI_DRV_PROC_AddModule(HI_MOD_CI, &stProc, NULL);
}

static HI_VOID CI_DRV_ProcDeInit(HI_VOID)
{
...
HI_S32 CI_Ioctl(struct inode *inode, struct file *file, unsigned int cmd, HI_VOID *arg)
{
    ...
    /* Added begin 2012-04-24 sdk: support various CI device */
    case CMD_CI_SETATTR:
    {
...
#ifdef CONFIG_COMPAT 
    case CMD_CI_COMPAT_PCCD_GET_STATUS:
    {
        CI_COMPAT_PCCD_STATUS_S *pCiStatus = (CI_COMPAT_PCCD_STATUS_S *)arg;
        
        s32Ret = CI_PCCD_GetStatus(pCiStatus->enCIPort, pCiStatus->enCardId, U32_TO_POINT(pCiStatus->ValueAddr));
        break;
    }

    case CMD_CI_COMPAT_PCCD_READ:
    {
        CI_COMPAT_PCCD_READ_S *pCiRead = (CI_COMPAT_PCCD_READ_S *)arg;

        s32Ret = CI_PCCD_IORead(pCiRead->enCIPort, pCiRead->enCardId, U32_TO_POINT(pCiRead->BufferAddr), U32_TO_POINT(pCiRead->ReadLenAddr));
        break;
    }

    case CMD_CI_COMPAT_PCCD_WRITE:
    {
        CI_COMPAT_PCCD_WRITE_S *pCiWrite = (CI_COMPAT_PCCD_WRITE_S *)arg;

        s32Ret = CI_PCCD_IOWrite(pCiWrite->enCIPort, pCiWrite->enCardId,
                                 U32_TO_POINT(pCiWrite->BufferAddr), pCiWrite->u32WriteLen, U32_TO_POINT(pCiWrite->WriteOKLenAddr));
        break;
    }
    case CMD_CI_COMPAT_PCCD_NEG_BUFFERSIZE:
    {
        CI_COMPAT_PCCD_BUFFERSIZE_S *pCiBufferSize = (CI_COMPAT_PCCD_BUFFERSIZE_S *)arg;

        s32Ret = CI_PCCD_NegBufferSize(pCiBufferSize->enCIPort, pCiBufferSize->enCardId, U32_TO_POINT(pCiBufferSize->BufferSizeAddr));
        break;
    }

    case CMD_CI_COMPAT_PCCD_TSCTRL:
    {
        CI_COMPAT_PCCD_TSCTRL_S *pCiTSCtrl = (CI_COMPAT_PCCD_TSCTRL_S *)arg;

        s32Ret = CI_PCCD_TSCtrl(pCiTSCtrl->enCIPort, pCiTSCtrl->enCardId, pCiTSCtrl->enCMD, U32_TO_POINT(pCiTSCtrl->ParamAddr));
        break;
    }    
    case CMD_CI_COMPAT_PCCD_GETCIS:
    {
        CI_COMPAT_PCCD_CIS_S *pCIS = (CI_COMPAT_PCCD_CIS_S *)arg;

        s32Ret = CI_PCCD_GetCIS(pCIS->enCIPort, pCIS->enCardId, U32_TO_POINT(pCIS->BufferAddr), U32_TO_POINT(pCIS->CisLenAddr));
        break;
    }
#endif
    /* Added end 2012-04-24 sdk: support various CI device */
