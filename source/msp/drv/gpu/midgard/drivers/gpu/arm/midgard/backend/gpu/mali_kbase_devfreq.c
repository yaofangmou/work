...
static struct semaphore l2_sem;

static int kbase_devfreq_init_voltage(struct kbase_device *kbdev, unsigned long initial_freq)
{
	int err;
...
static int kbase_devfreq_target2(struct device *dev, unsigned long *target_freq, u32 flags)
{
...
void kbase_devfreq_down(struct kbase_device *kbdev)
{
	if(NULL == kbdev->devfreq) {
		return;
	}

#if defined(CONFIG_PM_DEVFREQ) && (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
	/* stop the DVFS workqueue */
	if (!in_interrupt()) {
		down(&l2_sem);
	}
#endif
...
void kbase_devfreq_up(struct kbase_device *kbdev)
{
	if(NULL == kbdev->devfreq) {
		return;
	}
...
int kbase_devfreq_restore(struct kbase_device *kbdev)
{
	...
	err = kbase_devfreq_target2(kbdev->dev, &kbdev->saved_freq, 0);
