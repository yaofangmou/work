...
HI_S32 VPSS_WBC_CalBufSize(HI_U32 *pSize, HI_U32 *pStride, VPSS_WBC_ATTR_S *pstAttr)
{
    ...
    HI_U32 u32OverlapCalc = VPSS_OVERLAP_HEIGHT;
#endif
    ...
    if ((HI_FALSE == pstAttr->bProgressive)
        && (VPSS_WBC_MODE_4FIELD == pstAttr->enMode))
    {
        u32OverlapCalc = VPSS_OVERLAP_HEIGHT * 2;
    }
#endif
#endif
    ...
    switch (ePixFormat)
    {
        ...
        case HI_DRV_PIX_FMT_NV21_TILE_CMP:
        {
            if (HI_DRV_PIXEL_BITWIDTH_8BIT == enBitWidth)
            {
                ...
                if (pstAttr->bSplitEnable)
                {
                    u32RetSize = (u32Height + u32OverlapCalc) * u32RetStride * 3 / 2;
                }
#endif
            }
        ...
        case HI_DRV_PIX_FMT_UYVY:
        {
            if (HI_DRV_PIXEL_BITWIDTH_8BIT == enBitWidth)
            {
                ...
                if (pstAttr->bSplitEnable)
                {
                    u32RetSize = (u32Height + u32OverlapCalc) * u32RetStride * 2;
                }
#endif
        ...
        case HI_DRV_PIX_FMT_NV21_CMP:
        {
            if (HI_DRV_PIXEL_BITWIDTH_8BIT == enBitWidth)
            {
                ...
                if (pstAttr->bSplitEnable)
                {
                    u32RetSize = (u32Height + u32OverlapCalc) * u32RetStride * 3 / 2 + 16 * ( u32Height + VPSS_OVERLAP_HEIGHT ) * 3 / 2;
                }
#endif
        ...
        case HI_DRV_PIX_FMT_NV61_CMP:
        {
            if (HI_DRV_PIXEL_BITWIDTH_8BIT == enBitWidth)
            {
                ...
                if (pstAttr->bSplitEnable)
                {
                    u32RetSize = (u32Height + u32OverlapCalc) * u32RetStride * 3 / 2 + 16 * ( u32Height + VPSS_OVERLAP_HEIGHT ) * 2;
                }
#endif
...
HI_S32 VPSS_WBC_InitFrame(HI_DRV_VIDEO_FRAME_S *pstWbcFrame, VPSS_WBC_ATTR_S *pstAttr, HI_U32 u32PhyAddr, HI_U8 *pu8VirAddr)
{
...
#endif
    u32StrideCalc = u32Stride;
#else
    u32StrideCalc = u32Stride / 2;
#endif
    ...
    switch (pstAttr->ePixFormat)
    {
        ...
        case HI_DRV_PIX_FMT_UYVY:
        {
            ...
            {
                pstWbcFrame->stBufAddr[0].u32PhyAddr_C = u32PhyAddr + u32StrideCalc * u32ImgHeight;
            }
            ...
            if (pstAttr->bSplitEnable)
            {
                if ((HI_FALSE == pstAttr->bProgressive)
                    && (VPSS_WBC_MODE_4FIELD == pstAttr->enMode))
                {
                    pstWbcFrame->stBufAddr[0].u32PhyAddr_C = u32PhyAddr + u32StrideCalc *
                            (u32ImgHeight + VPSS_OVERLAP_HEIGHT);
                }
            }
#endif
#endif
            VPSS_DBG_INFO("WBC yaddr %#08x caddr %#08x stride h %d %d\n",
                          pstWbcFrame->stBufAddr[0].u32PhyAddr_Y,
...
HI_S32 VPSS_WBC_Init(VPSS_WBC_S *pstWbc, VPSS_WBC_ATTR_S *pstAttr)
{
    ...
    switch (pstAttr->enMode)
    {
        ...
        case VPSS_WBC_MODE_4FIELD:
        {
            ...
            //top first
            VPSS_WBC_InitFrame(&(pstWbc->stDataList[1].stWbcFrame), pstAttr, u32PhyAddr, pu8VirAddr);
            pstWbc->stDataList[1].stWbcFrame.enFieldMode = HI_DRV_FIELD_TOP;
            VPSS_WBC_InitFrame(&(pstWbc->stDataList[2].stWbcFrame), pstAttr, u32PhyAddr +
                               u32Stride / 2, pu8VirAddr + u32Stride/2);
            pstWbc->stDataList[2].stWbcFrame.enFieldMode = HI_DRV_FIELD_BOTTOM;
            u32PhyAddr = u32PhyAddr + u32NodeBuffSize;
            pu8VirAddr = pu8VirAddr + u32NodeBuffSize;
            VPSS_WBC_InitFrame(&(pstWbc->stDataList[3].stWbcFrame), pstAttr, u32PhyAddr, pu8VirAddr);
            pstWbc->stDataList[3].stWbcFrame.enFieldMode = HI_DRV_FIELD_TOP;
            VPSS_WBC_InitFrame(&(pstWbc->stDataList[0].stWbcFrame), pstAttr, u32PhyAddr +
                               u32Stride / 2, pu8VirAddr + u32Stride/2);
            pstWbc->stDataList[0].stWbcFrame.enFieldMode = HI_DRV_FIELD_BOTTOM;
#endif
            break;
        }
        case VPSS_WBC_MODE_5FIELD:
