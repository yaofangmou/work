        ...
        if (HI_SUCCESS == Ret)
        {
            ...
            switch(stStatus.enState)
            {
                ...
                case HI_UNF_PVR_PLAY_STATE_PLAY:
                    ...
                    if (HI_UNF_PVR_TRICK_WITH_AUDIO_ENABLE == (stStatus.enSpeed & SAMPLE_PVR_TRICK_WITH_AUDIO_ENABLE_MASK))
                    {
                        HI_U32 u32Ratio = 1000;
                        switch(stStatus.enSpeed)
                        {
                        ...
                        snprintf(StatString, sizeof(StatString),"Play %01d.%01d", u32Ratio/1000, (u32Ratio%1000)/100);
                    }
                    break;
...
HI_S32 main(int argc, char *argv[])
{
    ...
    while (HI_FALSE == g_bIsPlayStop)
    {
        ...
        if ('a' == InputCmd[0])
        {
            HI_UNF_PVR_PLAY_MODE_S stAudioTrickMode;
            HI_UNF_PVR_PLAY_POSITION_S stPos;
            HI_UNF_PVR_PLAY_SPEED_E enAudioSpeed[6] =
                {
                    HI_UNF_PVR_PLAY_SPEED_0_5X_WITH_AUDIO,
                    HI_UNF_PVR_PLAY_SPEED_0_8X_WITH_AUDIO,
                    HI_UNF_PVR_PLAY_SPEED_0_9X_WITH_AUDIO,
                    HI_UNF_PVR_PLAY_SPEED_1_1X_WITH_AUDIO,
                    HI_UNF_PVR_PLAY_SPEED_1_2X_WITH_AUDIO,
                    HI_UNF_PVR_PLAY_SPEED_1_5X_WITH_AUDIO
                };
            ...
            if (Ret != HI_SUCCESS)
            {
                PVR_SAMPLE_Printf("call HI_UNF_PVR_PlaySeek failed. ret = 0x%x\n", Ret);
                continue;
            }

            stAudioTrickMode.enSpeed = enAudioSpeed[u32AudioTimes%6];
            Ret = HI_UNF_PVR_PlayTPlay(PlayChn, &stAudioTrickMode);
            if (Ret != HI_SUCCESS)
            {
                PVR_SAMPLE_Printf("call HI_UNF_PVR_PlayTPlay failed. ret = 0x%x\n", Ret);
                continue;
            }
            u32AudioTimes++;
        }
