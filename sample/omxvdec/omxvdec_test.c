...
#ifdef CalcFrameRate
static HI_BOOL g_PrintAlready = HI_FALSE;
#endif
...
static void loop_function(HI_BOOL);
...
static int open_video_file (OmxTestInfo_S * pOmxTestInfo)
{
    ...
    if (HI_TRUE == pOmxTestInfo->frame_in_packet)
    {
        if (HI_FALSE == pOmxTestInfo->readsize_add_in_stream) // FIX
        {
            memset(&pOmxTestInfo->stContext, 0, sizeof(STR_CONTEXT_S));
            snprintf(tidxfile, sizeof(tidxfile), "%s.tidx", pOmxTestInfo->in_filename);
            tidxfile[sizeof(tidxfile) - 1] = '\0';
...
static int Read_Buffer_from_EsRawStream(OmxTestInfo_S * pOmxTestInfo, OMX_BUFFERHEADERTYPE  *pBufHdr)
{
    ...
    if (HI_TRUE == pOmxTestInfo->readsize_add_in_stream)
    {
       if (0 == pOmxTestInfo->frame_flag)
       {
    ...
    case OMX_EventError:
        ...
        OMX_ErrorHardware == (OMX_ERRORTYPE)nData1)
        {
            ...
            if(pOmxTestInfo->event_is_done == 0)
            {
                ...
                pOmxTestInfo->bOutputEosReached = HI_TRUE;
            }
        }
...
static int Play_Decoder(OmxTestInfo_S * pOmxTestInfo)
{
    ...
    /*if (HI_TRUE == readsize_add_in_stream)
    {
    ...
    if (error != OMX_ErrorNone)
    {
        ...
        do_freeHandle_and_clean_up(pOmxTestInfo, HI_TRUE);
        return -1;
    }
   ...
   else
   {
    ...
    if (pOmxTestInfo->freeHandle_option == FREE_HANDLE_AT_IDLE)
    {
        ...
        if (state == OMX_StateIdle)
        {
            ...
            do_freeHandle_and_clean_up(pOmxTestInfo, HI_FALSE);
        }
        else
        {
    ...
    for (bufCnt = 0; bufCnt < pOmxTestInfo->used_ip_buf_cnt; bufCnt++)
    {
        ...
        {
            pOmxTestInfo->bInputEosReached = HI_TRUE;
            break;
        }
    }
...
static int process_current_command(OmxTestInfo_S * pOmxTestInfo, const char *seq_command)
{
    ...
    /*else if(strcmp(seq_command, "disable-op") == 0)
    {
        ...
        if (disable_output_port() != 0)
        {
            ...
            do_freeHandle_and_clean_up(HI_TRUE);
            return -1;
        }
...
void SignalHandle(int sig)
{
    ...
    for (i=0; i<MAX_INST_NUM; i++)
    {
        if (1 == OmxTestInfo[i].is_open)
        {
            ...
            do_freeHandle_and_clean_up(&OmxTestInfo[i], HI_TRUE); //需要增加实例参数，比较麻烦，暂时先屏蔽
        }
    }
...
static void Init_OmxInst()
{
    ...
    for (i = 0; i < MAX_INST_NUM; i++)
    {
        ...
        OmxTestInfo[i].readsize_add_in_stream = HI_FALSE;
    }
...
HI_BOOL IsCrcModeEnable(char *pStr)
{
    if (pStr == HI_NULL)
    {
        return HI_FALSE;
...
int omxvdec_test(int argc, char **argv, OMXVDEC_TEST_ATTR *ptest_attr)
{
    ...
    int chanid = 0;
    int CurPos = 0;
    HI_BOOL auto_exit = ptest_attr->auto_exit;
    ...
    for (i = 0; i < g_TestInstNum; i++)
    {
        ...
        if (is_framepacket_standard(OmxTestInfo[i].codec_format_option))
        {
            ...
            {
                OmxTestInfo[i].readsize_add_in_stream = HI_TRUE;
            }
...
static void loop_function(HI_BOOL auto_exit)
{
    int i = 0;
    int cmd_error = 0;
    ...
    while (cmd_error == 0)
    {
        for (i = 0; i < g_TestInstNum; i++)
        {
            if (OmxTestInfo[i].is_open && OmxTestInfo[i].bOutputEosReached == HI_FALSE)
            {
                break;
            }
        }
...
static int seek_progress(OmxTestInfo_S * pOmxTestInfo)
{
    ...
    pOmxTestInfo->bInputEosReached = HI_FALSE;
