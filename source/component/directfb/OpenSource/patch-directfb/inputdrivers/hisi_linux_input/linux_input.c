...
DFB_INPUT_DRIVER( hisi_linux_input )
...
static unsigned int      g_DeviceNum = 0;
...
//[TAG]: add new key value
	DIKI_UNKNOWN,
     ...
     DIKI_UNKNOWN,//247
     /* picture mode*/DIKS_CUSTOM6,//248
    DIKI_UNKNOWN,//249
    /*source */ DIKS_CHANNEL,//250
    /*3D*/DIKS_CUSTOM7,//251
    /*sound mode*/DIKS_CUSTOM5,//252
    /*freeze*/DIKS_FREEZE,//253
    /*Nicam MTS*/DIKS_CUSTOM9,//254
    /*cc*/DIKS_CUSTOM8,//255
    /*internet*/DIKS_CUSTOM8,//256
    /*swap*/DIKS_SWAP,//257
    /*pip*/DIKS_PIP//258
...
static unsigned int device_check()
{
	DIR *pstDir;
	struct dirent *pstDirent;
	unsigned int nDevNum = 0;
	pstDir = opendir("/dev/input/");
	if(NULL == pstDir)
	{
		return 0;
	}
	...
	{
		int nIndex = 0;
		if((0==strncmp(pstDirent->d_name,"event",5)))
		{
			nIndex = atoi(&(pstDirent->d_name[5]));
			nDevNum = (nDevNum|(1<<nIndex));
		}
	}
	closedir(pstDir);
	return nDevNum;
     ...
     for (i=0; i<MAX_LINUX_INPUT_DEVICES; i++) {
          if (device_nums[i] == MAX_LINUX_INPUT_DEVICES) {
               ...
               *index = i;
               ...
               snprintf( buf, 32, "/dev/event%d", event_num);
...
udev_hotplug_EventThread(DirectThread *thread, void * hotplug_data)
{
	...
	{
		DFBResult ret;
		int   number_file, index;
		int i;
		unsigned int nCheckIndex = 0;
        
		sleep(2);
		number_file = device_check();
		if(number_file > g_DeviceNum)
		{
			nCheckIndex = (number_file&(~g_DeviceNum));
			for( i = 0;i < 16;i++)
			{
				if((nCheckIndex >> i)&0x1)
				{
					//printf("add:%d\n", i);
					ret = register_device_node( i, &index);
					if ( DFB_OK == ret) 
					{
			...
			g_DeviceNum = number_file;
		}
		else if(number_file < g_DeviceNum)
		{
			nCheckIndex = g_DeviceNum&(~number_file);
			for(i = 0;i < 16;i++)
			{
				if((nCheckIndex >> i)&0x1)
				{
					//printf("remove:%d\n", i);
					ret = unregister_device_node( i, &index);
					if ( DFB_OK == ret) 
					{
