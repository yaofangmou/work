...
#ifdef HDMI_HDCP22_SUPPORT
    PROC_PRINT(pFile, "%-15s: %-20s  ", "HDCP2.2CodeLoad", pstHdcpInfo->bHdcp22LoadCode ? STR_YES: STR_NO);
#endif
...
static HI_VOID Proc_SinkVideoCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    PROC_PRINT(pFile, "\n");
    /* line 6 Quantization Range Selectable  */
...
static HI_VOID Proc_SinkFormatCap(struct seq_file *pFile, HDMI_SINK_CAPABILITY_S *pstCap)
{
    ...
    /* Y420Format[Also] */
    PROC_PRINT(pFile, "%s:\n", "YCbCr420[Also]");
    for (i = 0, j = 0; (i < pstCap->u32SupportY420VICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
    {
        pTmpFmt = Proc_Vic2Format(pstCap->au32SupportY420Format[i]);
        if (pTmpFmt != HI_NULL)
        {
            PROC_PRINT(pFile,"%-20s ", pTmpFmt);
        }
        ...
        {
            PROC_PRINT(pFile,"(VIC)%-15u ", pstCap->au32SupportY420Format[i]);
        }
    ...
    /* Y420Format[Olny] */
    PROC_PRINT(pFile, "%s:\n", "YCbCr420[Olny]");
    for (i = 0, j = 0; (i < pstCap->u32OnlySupportY420VICNum) && (i < HDMI_EDID_MAX_VIC_COUNT); i++)
    {
        pTmpFmt = Proc_Vic2Format(pstCap->au32OnlySupportY420Format[i]);
        if (pTmpFmt != HI_NULL)
        {
            PROC_PRINT(pFile,"%-20s ", pTmpFmt);
        }
        ...
        {
            PROC_PRINT(pFile,"(VIC)%-15u ", pstCap->au32OnlySupportY420Format[i]);
        }
