...
static HI_S32 _ARIBCC_8BitCode_Sdf(ARIBCC_CaptionType_E enInCCType,ARIBCC_Control_S *pstControl)
{
    ...
    if(bApiCallFlg != HI_FALSE)
    {
        ...
        if (u16DisplayWidth > ((u16DisplayWidthMax * 7) / 10))
        {
            u16DisplayWidth = ((u16DisplayWidthMax * 8) / 10);
        }
        if (u16DisplayHeight > ((u16DisplayHeightMax * 7) / 10))
        {
            u16DisplayHeight = ((u16DisplayHeightMax * 8) / 10);
        }
...
static HI_S32 _ARIBCC_8BitCode_Sdp(ARIBCC_CaptionType_E enInCCType,ARIBCC_Control_S *pstControl)
{
    ...
    else if (u16X > (u16XMax - (u16XMax / 10)))
    {
        u16X = (u16XMax - (u16XMax / 10));
    }

    if (u16Y < ((u16YMax / 10) * 2))
    {
        u16Y = (u16YMax / 10);
    }
    else if (u16Y > (u16YMax - (u16YMax / 10)))
    {
        u16Y = (u16YMax - (u16YMax / 10));
    }
...
static HI_S32 _ARIBCC_8BitCode_Orn(ARIBCC_CaptionType_E enInCCType,ARIBCC_Control_S *pstControl)
{
    ...
    {
        case ORN_NOTHING:
            u8EdgeType = ARIBCC_EDGETYPE_ORN_NOTHING;
            break;
        case ORN_HEMMING:
            u8EdgeType = ARIBCC_EDGETYPE_ORN_HEMMING;
    ...
    while(u32Length < u32UnitLength)
    {
        ...
        if(pstStatementBody->u32ControlCount > 0)
        {
            ...
            for(i=1 ;i <= (pstStatementBody->u32ControlCount);i++)
            {
                ...
                u32TmpLength += ( sizeof(pstControl->enControlId) + (u32ParamNum*sizeof(ARIBCC_CtrlSet_E)) );
                if (u32TmpLength > pstStatementBody->u32ControlLength)
                {
                    HI_ERR_CC ("Data length %d out of bounds %d.\n", u32TmpLength, pstStatementBody->u32ControlLength);
                    break;
                }
