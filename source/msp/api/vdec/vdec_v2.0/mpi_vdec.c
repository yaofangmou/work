...
{
    pthread_mutex_t      stMutex;               /* Mutex */
    pthread_mutex_t      global_release_mutex;
...
static VDEC_GLOBAL_S s_stVdecParam =
{
    ...
    .global_release_mutex = PTHREAD_MUTEX_INITIALIZER,
    .stDefAttr         =
    {
...
static HI_S32 VDEC_DestroyCodec(VDEC_INST_S *pstVdec)
{
    ...
    if ((HI_INVALID_HANDLE != pstVdec->hCodecInst) && (pstVdec->bIsVFMW == HI_FALSE))
    {
        (HI_VOID)pthread_mutex_destroy(&pstVdec->stMutex);
    }
#endif
...
static HI_BOOL VDEC_IsOldFBSolution()
{
    HI_S32 ret = HI_FAILURE;
    HI_SYS_VERSION_S sys_version;

    ret = HI_SYS_GetVersion(&sys_version);
    ...
    if ((sys_version.enChipTypeHardWare == HI_CHIP_TYPE_HI3798C && sys_version.enChipVersion == HI_CHIP_VERSION_V200)
        || (sys_version.enChipTypeHardWare == HI_CHIP_TYPE_HI3798M && (sys_version.enChipVersion == HI_CHIP_VERSION_V200 || sys_version.enChipVersion == HI_CHIP_VERSION_V300))
        || (sys_version.enChipTypeHardWare == HI_CHIP_TYPE_HI3796M && sys_version.enChipVersion == HI_CHIP_VERSION_V200)
        || (sys_version.enChipTypeHardWare == HI_CHIP_TYPE_HI3716M && sys_version.enChipVersion == HI_CHIP_VERSION_V450))
    {
        return HI_TRUE;
    }
...
static HI_VOID *VDEC_SoftCodec(HI_VOID *phVdec)
{
    ...
    while (!pstVdec->bThreadStop)
    {
        ...
        else
        {
            ...
            if (s32Ret == HI_ERR_CODEC_UNSUPPORT)
            {
                pstVdec->bUnsupportStream = HI_TRUE;
                HI_ERR_VDEC("soft decoder not supported \n");
            }
        }
...
#ifdef HI_VDEC_REG_CODEC_SUPPORT
...
HI_S32 HI_MPI_VDEC_GetChanFrmStatusInfo(HI_HANDLE hVdec, HI_HANDLE  hPort, VDEC_FRMSTATUSINFO_S *pstVdecFrmStatus)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        #else
            s32Ret = VPSS_GetStatusInfo(hVdec, &stVdecFrmStatusInfo);
        #endif
            if (HI_SUCCESS == s32Ret)
            {
...
HI_S32 HI_MPI_VDEC_CheckNewEvent(HI_HANDLE hVdec, VDEC_EVENT_S *pstNewEvent)
{
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        ...
        else
        {
            ...
            if (pstVdec->bUnsupportStream)
            {
                pstNewEvent->bUnSupportStream = HI_TRUE;
                pstVdec->bUnsupportStream = HI_FALSE;
            }
        }
    #endif
    }
...
#else
HI_S32 HI_MPI_VDEC_ChanIFrameDecode(HI_HANDLE hVdec, HI_UNF_AVPLAY_I_FRAME_S *pstIFrameStream, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_BOOL bCapture)
#endif
{
    HI_S32 s32Ret;
    VDEC_INST_S *pstVdec = HI_NULL;
    ...
    stIFrameParam.pstFrame = pstFrame;
#endif
...
#else
HI_S32 HI_MPI_VDEC_ReceiveFrame(HI_HANDLE hVdec, HI_DRV_VIDEO_FRAME_S *pstFrame, HI_U32 TimeOut)
{
    HI_S32 s32Ret = HI_FAILURE;
    VDEC_INST_S *pstVdec = HI_NULL;
    VDEC_RECVFRM_WITH_TIMEOUT_S VidFrame;

    VDEC_CHECK_INIT;

    VDEC_FIND_INST(hVdec, pstVdec);

    if (pstVdec == HI_NULL || pstFrame == HI_NULL)
    {
        return HI_ERR_VDEC_INVALID_PARA;
    }
    ...
    if (pstVdec->pstCodec)
    {
        if (pstVdec->bIsVFMW)
        {
            VidFrame.TimeOut = TimeOut;
            VidFrame.pVidFrm = pstFrame;
            s32Ret = VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_RECEIVE_FRAME_WITH_TIMEOUT, (HI_VOID *)(&VidFrame));
        }
        else
        {
            s32Ret = VFMW_ReceiveFrame(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), (HI_VOID *)pstFrame);
        }
    }
...
HI_S32 HI_MPI_VDEC_ReleaseFrame(HI_HANDLE hVdec, HI_DRV_VIDEO_FRAME_S *pstFrame)
{
    HI_S32 s32Ret = HI_FAILURE;
    VDEC_INST_S *pstVdec = HI_NULL;
    ...
    if (HI_NULL != pstVdec->pstCodec)
    {
        if (pstVdec->bIsVFMW)
        {
            s32Ret = VDEC_VFMWSpecCMD(hVdec, VFMW_CMD_RELEASEFRAME, (HI_VOID *)pstFrame);
        }
        else
        {
            s32Ret = VFMW_ReleaseFrame(HI_CODEC_INST_HANDLE(pstVdec->hCodecInst), (HI_VOID *)pstFrame);
        }
    }
...
HI_S32 HI_MPI_VDEC_Invoke(HI_HANDLE hVdec, HI_CODEC_VIDEO_CMD_S *pstParam)
{
    ...
    if ((pstVdec->pstCodec)&&(pstVdec->pstCodec->Control))
    {
        if (pstVdec->bIsVFMW)
        {
            s32Ret = pstVdec->pstCodec->Control(
                         HI_CODEC_INST_HANDLE(pstVdec->hCodecInst),
                         pstParam->u32CmdID,
                         (HI_VOID*)pstParam->pPara);
        }
        else
        {
            if(pstParam->u32CmdID == VFMW_CMD_GETINFO) //memset when u32CmdID is VFMW_CMD_GETINFO
            {
...
#endif
    else if ( pstParam->u32CmdID == VFMW_CMD_GETINFO)  //mjpeg get vfmw info
    {
        memset((HI_UNF_AVPLAY_VDEC_INFO_S *)pstParam->pPara, 0, sizeof(HI_UNF_AVPLAY_VDEC_INFO_S)); //memset when u32CmdID is VFMW_CMD_GETINFO
...
HI_S32 HI_MPI_VDEC_ChanBufferInit(HI_HANDLE hVdec, HI_U32 u32BufSize, HI_HANDLE hDmxVidChn, VDEC_BUFFER_ATTR_S *pstBufAttr)
{
    VDEC_INST_S *pstVdec = HI_NULL;
    VFMW_STREAMBUF_S stStrmBuf;
    HI_S32 s32Ret;

    HI_UNUSED(u32BufSize);
    HI_UNUSED(pstBufAttr);

#ifdef HI_TEE_SUPPORT
    if (HI_NULL == pstBufAttr)
    {
        HI_ERR_VDEC("pstBufAttr is null\n");
        return HI_ERR_VDEC_NULL_PTR;
    }
#endif
...
HI_S32 HI_MPI_VDEC_GlobalRelFrm(HI_DRV_VIDEO_FRAME_S *pVideoFrame)
{
    ...
    VDEC_LOCK(s_stVdecParam.global_release_mutex);

    s32Ret = VDEC_GlobalReleaseFrm(pVideoFrame);
    if (HI_SUCCESS != s32Ret)
    {
        VDEC_UNLOCK(s_stVdecParam.global_release_mutex);
...
HI_S32 HI_MPI_VDEC_GlobalRelAllFrm(HI_DRV_VIDEO_FRAME_S *pVideoFrame)
{
    HI_S32 s32Ret = HI_FAILURE;

    if (pVideoFrame == HI_NULL)
    {
        HI_ERR_VDEC("Parameter is invalid\n");
    ...
    s32Ret = VDEC_GlobalReleaseAllFrm(pVideoFrame);
    if (HI_SUCCESS != s32Ret)
    {
        HI_ERR_VDEC("\n");
        VDEC_UNLOCK(s_stVdecParam.global_release_mutex);
