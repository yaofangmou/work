...
#define ARIBCC_QUEUE_MUTEX_INIT(x)  \
    do{ \
        int ret = pthread_mutex_init(x,NULL); \
        if(ret != 0){ \
            HI_ERR_CC("ARIBCC call pthread_mutex_init(QUEUE) failure, ret = 0x%x\n",ret); \
        } \
    }while(0)

#define ARIBCC_QUEUE_MUTEX_DESTROY(x)  \
    do{ \
        int ret = pthread_mutex_destroy(x); \
        if(ret != 0){ \
            HI_ERR_CC("ARIBCC call pthread_mutex_destroy(QUEUE) failure, ret = 0x%x\n",ret); \
        } \
    }while(0)
...
HI_S32 ARIBCC_PesQueue_Init(ARIBCC_SEGMENT_QUEUE_S* pstQueue, const HI_U8* pszBufAddr, HI_U32 u32BufLen)
{
    ...
    ARIBCC_QUEUE_MUTEX_INIT(&pstQueue->Queue_lock);
    ARIBCC_QUEUE_LOCK();
...
HI_S32 ARIBCC_PesQueue_Deinit(ARIBCC_SEGMENT_QUEUE_S* pstQueue)
{
    if (pstQueue == NULL)
    {
        return HI_FAILURE;
    ...
    pstQueue->pu8BaseAddr = NULL;
    pstQueue->pstSegmentFront = NULL;
    pstQueue->pstSegmentRear = NULL;

    ARIBCC_QUEUE_UNLOCK();
    ARIBCC_QUEUE_MUTEX_DESTROY(&pstQueue->Queue_lock);
...
HI_S32 ARIBCC_PesQueue_En(ARIBCC_SEGMENT_QUEUE_S* pstQueue, ARIBCC_SEGMENT_S* pSegment)
{
    ...
    if ((NULL == pstQueue->pu8BaseAddr) || (NULL == pstQueue->pstSegmentRear))
    {
        ARIBCC_QUEUE_UNLOCK();
        return HI_FAILURE;
    }
...
HI_S32 ARIBCC_PesQueue_De(ARIBCC_SEGMENT_QUEUE_S* pstQueue, ARIBCC_SEGMENT_S* pSegment)
{
    ...
    if (NULL == pstQueue->pstSegmentFront)
    {
        ARIBCC_QUEUE_UNLOCK();
        return HI_FAILURE;
    }
