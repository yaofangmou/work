...
HI_S32 HI_UNF_DMX_AcquireRecIndex(HI_HANDLE hRecChn, HI_UNF_DMX_REC_INDEX_S *pstRecIndex, HI_U32 u32TimeoutMs)
{
    ...
    if (HI_SUCCESS == ret)
    {
        ...
        if ((HI_UNF_FRAME_TYPE_BLA == pstRecIndex->enFrameType)
         || (HI_UNF_FRAME_TYPE_CRA == pstRecIndex->enFrameType))
        {
            pstRecIndex->enFrameType = HI_UNF_FRAME_TYPE_IDR;
        }
...
{
    HI_S32 ret = HI_FAILURE;
    HI_MPI_DMX_REC_DATA_INDEX_S *pstRawDataIndex = HI_NULL;
    ...
    pstRawDataIndex = HI_MALLOC(HI_ID_DEMUX, sizeof(HI_MPI_DMX_REC_DATA_INDEX_S));
    if (!pstRawDataIndex)
    {
        HI_ERR_DEMUX("malloc RecDataIndex struct failed.\n");
        return HI_ERR_DMX_ALLOC_MEM_FAILED;
    }
    pstRawDataIndex->u32IdxNum = 0;
    pstRawDataIndex->u32RecDataCnt = 0;

    ret = HI_MPI_DMX_AcquireRecDataAndIndex(hRecChn, pstRawDataIndex);
    if (HI_SUCCESS == ret)
    {
        HI_U32 idx;

        /* covert MPI to UNF structure. */
        pstRecDataIdx->u32RecDataCnt = pstRawDataIndex->u32RecDataCnt;
        for (idx = 0; idx < pstRecDataIdx->u32RecDataCnt; idx ++)
        {
            pstRecDataIdx->stRecData[idx].pDataAddr      = pstRawDataIndex->stRecData[idx].pDataAddr;
            pstRecDataIdx->stRecData[idx].u32DataPhyAddr = pstRawDataIndex->stRecData[idx].u32DataPhyAddr;
            pstRecDataIdx->stRecData[idx].u32Len         = pstRawDataIndex->stRecData[idx].u32Len;
        }

        pstRecDataIdx->u32IdxNum = pstRawDataIndex->u32IdxNum;
        for (idx = 0; idx < pstRecDataIdx->u32IdxNum; idx ++)
        {
            pstRecDataIdx->stIndex[idx].enFrameType      = pstRawDataIndex->stIndex[idx].enFrameType;
            pstRecDataIdx->stIndex[idx].u32PtsMs         = pstRawDataIndex->stIndex[idx].u32PtsMs;
            pstRecDataIdx->stIndex[idx].u64GlobalOffset  = pstRawDataIndex->stIndex[idx].u64GlobalOffset;
            pstRecDataIdx->stIndex[idx].u32FrameSize     = pstRawDataIndex->stIndex[idx].u32FrameSize;
            pstRecDataIdx->stIndex[idx].u32DataTimeMs    = pstRawDataIndex->stIndex[idx].u32DataTimeMs;
        }
    }

    HI_FREE(HI_ID_DEMUX, pstRawDataIndex);
...
HI_S32 HI_UNF_DMX_ReleaseRecDataAndIndex(HI_HANDLE hRecChn, HI_UNF_DMX_REC_DATA_INDEX_S* pstRecDataIdx)
{
    ...
    pstRawDataIndex->u32RecDataCnt = pstRecDataIdx->u32RecDataCnt;
    for (idx = 0; idx < pstRawDataIndex->u32RecDataCnt; idx ++)
    {
        pstRawDataIndex->stRecData[idx].pDataAddr      = pstRecDataIdx->stRecData[idx].pDataAddr;
        pstRawDataIndex->stRecData[idx].u32DataPhyAddr = pstRecDataIdx->stRecData[idx].u32DataPhyAddr;
        pstRawDataIndex->stRecData[idx].u32Len         = pstRecDataIdx->stRecData[idx].u32Len;
    }

    /* idx info unused when release. */
    pstRawDataIndex->u32IdxNum = 0;

    ret = HI_MPI_DMX_ReleaseRecDataAndIndex(hRecChn, pstRawDataIndex);
    if (HI_SUCCESS != ret)
    {
        HI_ERR_DEMUX("call HI_MPI_DMX_ReleaseRecDataAndIndex failed!\n");
    }
