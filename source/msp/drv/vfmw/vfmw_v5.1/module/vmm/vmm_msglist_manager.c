...
VMM_PRIVATE SINT32 Msglist_MoveFromFreeToHead(Msg_Data_S *pData, VMM_CMD_PRIO_E Proir)
{
    ...
    if ((pMsgNode != NULL) && ((VOID*)pMsgNode != (VOID*)&g_pMsgListCtx->free))
    {
        ...
        else if (Proir == VMM_CMD_PRIO_SECONDARY)
        {
            Msg_Node_S *pMsgNodeFirst = NULL;

            pMsgNodeFirst = list_first_entry(&g_pMsgListCtx->head, Msg_Node_S, list);
            if ((pMsgNodeFirst != NULL) && ((VOID*)pMsgNodeFirst != (VOID*)&g_pMsgListCtx->head))
            {
                OSAL_FP_list_del(&pMsgNodeFirst->list);
                OSAL_FP_list_add(&pMsgNode->list, &g_pMsgListCtx->head);
                OSAL_FP_list_add(&pMsgNodeFirst->list, &g_pMsgListCtx->head);
            }
            else
            {
                OSAL_FP_list_add(&pMsgNode->list, &g_pMsgListCtx->head);
...
VMM_PRIVATE SINT32 Msglist_MoveFromHeadToFree(Msg_Data_S *pData)
{
    ...
    if ((pMsgNode != NULL) && ((VOID*)pMsgNode != (VOID*)&g_pMsgListCtx->head))
    {
        OSAL_FP_list_del(&pMsgNode->list);
...
VMM_PRIVATE VOID PrintErrorDetail_MsgData(Msg_Data_S *pData)
{
    ...
    switch (pData->CmdID)
    {
        case VMM_CMD_ReleaseBurst:
        {
            ...
            PRINT(VMM_ERROR, "%s, %d, ERROR: CurCmd = ReleaseBurst, PrivMask = %d is invalid!\n", __func__, __LINE__, pRlsBurst->PrivMask);
        }
        break;
        ...
        case VMM_CMD_Alloc:
        {
            ...
            PRINT(VMM_ERROR, "%s, %d, ERROR: MemType(%d) should < %d,Cache(%d) should < 2, Map(%d) should < 2, SecFlag(%d) should < 2, Size(%d) should != 0\n", __func__, __LINE__,
        ...
        case VMM_CMD_Release:
        {
            ...
            PRINT(VMM_ERROR, "%s, %d, ERROR: MemType(%d) should < %d,Cache(%d) should < 2, Map(%d) should < 2, SecFlag(%d) should < 2, Size(%d) should != 0\n", __func__, __LINE__,
                  pBufRec->MemType, VMM_MEM_TYPE_BUTT, pBufRec->Cache, pBufRec->Map, pBufRec->SecFlag, pBufRec->Size);
            PRINT(VMM_ERROR, "Current StartPhyAddr(0x%x) pStartVirAddr(0x%x)\n", pBufRec->StartPhyAddr, pBufRec->pStartVirAddr);

        }
        break;

        default:
            PRINT(VMM_ERROR, "Unknow Command ID %d\n", pData->CmdID);
            break;
    }
}
...
VMM_PRIVATE SINT32 MsgDateCheck(Msg_Data_S *pData)
{
    ...
    switch (pData->CmdID)
    {
        ...
        case VMM_CMD_Release:
        {
            ...
            if (pBufRec->MemType != VMM_MEM_TYPE_PRE_ALLOC)
            {
                if (pBufRec->Map != 0)
                {
    ...
    if (PrivID != pData->PrivID)
    {
        ...
        PRINT(VMM_ERROR, "%s,%d, MsgData PrivID conflict!(%d,%d)\n", PrivID, pData->PrivID);
    }
...
SINT32 VMM_Msglist_ReadProc(VOID *p)
{
    ...
    //statistics info
    VFMW_PROC_PRINT(p, "================================================= MsgList Status ========================================================= \n");
    ...
    VFMW_PROC_PRINT(p, "------------------------------------------------- MsgNode Info ----------------------------------------------------------- \n");
    VFMW_PROC_PRINT(p, "\tPrivID\t    CmdID\t   retryCnt\n");
    VFMW_PROC_PRINT(p, "-------------------------------------------------------------------------------------------------------------------------- \n");
    ...
    list_for_each_safe(pos, n, &g_pMsgListCtx->head)
    {
        ...
        VFMW_PROC_PRINT(p, "0x%-16llx %-16s  %-8d\n", pData->PrivID, MsgCmdTab[pData->CmdID], pData->RetryCnt);
    }
...
SINT32 VMM_Msglist_AddNode(Msg_Data_S *pData, VMM_CMD_PRIO_E Proir)
{
    ...
    {
        PRINT(VMM_ERROR, "%s,%d, Proir not support!\n", __func__, __LINE__);
        return ERR_LIST_INVALID_PARA;
    }

    if (MsgDateCheck(pData) != LIST_SUCCESS)
    {
        PRINT(VMM_ERROR, "%s CmdID %d MsgDateCheck failed.\n", __func__, pData->CmdID);
        return ERR_LIST_INVALID_PARA;
    }
