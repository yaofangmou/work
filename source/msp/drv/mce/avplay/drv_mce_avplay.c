...
#define FASTPLAY_STEP_PLAY_COUNT         1
...
HI_S32 DRV_AVPLAY_ReceiveVidFrame(MCE_AVPLAY_S *pAvplay)
{
    HI_S32 Ret = HI_SUCCESS;
    HI_U32 sysTime = 0;
    ...
    if (HI_SUCCESS != Ret)
    {
        HI_INFO_MCE("call HI_DRV_VDEC_Chan_RecvVpssFrmBuf fail:%d\n",Ret);
        return Ret;
    }
...
HI_VOID DRV_AVPLAY_QueueVidFrame(MCE_AVPLAY_S *pAvplay)
{
...
HI_VOID DRV_AVPLAY_ProcVidFrc(MCE_AVPLAY_S *pAvplay)
{
    ...
    pAvplay->FrcParamCfg.u32InRate = pAvplay->CurFrame.u32FrameRate/100;
    pAvplay->FrcParamCfg.u32OutRate = WinInfo.u32DispRate/10;
...
HI_VOID DRV_AVPLAY_ProcVidSync(MCE_AVPLAY_S *pAvplay)
{
    ...
    pstFrmPriv = (HI_DRV_VIDEO_PRIVATE_S *)(pAvplay->CurFrame.u32Priv);
    pstFrmPriv->u32PlayTime = 1;
...
HI_VOID DRV_AVPLAY_ProcVidPlay(MCE_AVPLAY_S *pAvplay)
{
    ...
    DRV_AVPLAY_ProcVidFrameRate(pAvplay, &pAvplay->CurFrame);

    Ret = HI_DRV_WIN_QFrame(pAvplay->MasterFrmChn.hWindow, &pAvplay->CurFrame);
    if (HI_SUCCESS == Ret)
    {
        memcpy(&pAvplay->LastFrame, &pAvplay->CurFrame, sizeof(HI_DRV_VIDEO_FRAME_S));
        pAvplay->AvplayProcContinue = HI_TRUE;
        pAvplay->FrcCurPlayCnt++;
        pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO] = HI_FALSE;
    }
    else if (HI_ERR_VO_BUFQUE_FULL != Ret)
    {
        ...
        (HI_VOID)HI_DRV_VDEC_RlsFrmBuf(pAvplay->hVdec, &pAvplay->CurFrame);
...
HI_S32 DRV_AVPLAY_ReceiveVidFrame(MCE_AVPLAY_S *pAvplay)
{
    ...
    Ret = HI_DRV_VDEC_RecvFrmBuf(pAvplay->hVdec, &pAvplay->CurFrame);
    if (HI_SUCCESS != Ret)
    {
        HI_WARN_MCE("call HI_DRV_VDEC_RecvFrmBuf fail:%d\n",Ret);
        return Ret;
    }

    sysTime = MCE_GetSysTime();
    HI_INFO_MCE("=====Receive a new frame, sys=%u, id=%u, pts=%u=====\n",
        sysTime, pAvplay->CurFrame.u32FrameIndex, pAvplay->CurFrame.u32Pts);
...
HI_VOID DRV_AVPLAY_QueueVidFrame(MCE_AVPLAY_S *pAvplay)
{
    ...
    switch (pAvplay->VidOpt.SyncProc)
    {
        ...
        default:
            HI_ERR_MCE("SYNC_PROC is Invalide:0x%x\n",pAvplay->VidOpt.SyncProc);
            pstPlayInfor->u32PlayCnt = pAvplay->FrcNeedPlayCnt;
            break;
    }

    (HI_VOID)DRV_AVPLAY_ProcVidPlay(pAvplay);

    HI_INFO_MCE("sys:%u, frm:%d, need:%u, cur:%u, sync:%u, delay:%u\n",
        MCE_GetSysTime(),pAvplay->CurFrame.u32FrameIndex, pAvplay->FrcNeedPlayCnt,
        pAvplay->FrcCurPlayCnt, pAvplay->VidOpt.SyncProc, pAvplay->VidInfo.DelayTime);
...
HI_S32 DRV_AVPLAY_RelAllChnFrame(MCE_AVPLAY_S *pAvplay)
{
    ...
    {
        Ret = HI_DRV_VDEC_RlsFrmBuf(pAvplay->hVdec, &pAvplay->CurFrame);
        if (HI_SUCCESS != Ret)
        {
            HI_ERR_MCE("call HI_DRV_VDEC_RlsFrmBuf err:%d\n",Ret);
        }
...
HI_VOID DRV_AVPLAY_ProcVdecToVo(MCE_AVPLAY_S *pAvplay)
{
    ...
    if (!pAvplay->AvplayProcDataFlag[AVPLAY_PROC_VDEC_VO])
    {
        Ret = DRV_AVPLAY_ReceiveVidFrame(pAvplay);
        if (HI_SUCCESS != Ret)
        {
             return;
        }
    ...
    if (pAvplay->bStepMode)
    {
        if (pAvplay->bStepPlay)
        {
            ...
            pAvplay->CurFrame.stPlayInfor.u32PlayCnt = FASTPLAY_STEP_PLAY_COUNT;
        #endif
            DRV_AVPLAY_ProcVidPlay(pAvplay);
    ...
    DRV_AVPLAY_QueueVidFrame(pAvplay);
    return;
}
...
HI_S32 HI_DRV_AVPLAY_AttachWindow(HI_HANDLE hAvplay, HI_HANDLE hWindow)
{
    ...
    memset(&stSrcInfo, 0x0, sizeof(HI_DRV_WIN_SRC_HANDLE_S));

    stSrcInfo.enSrcMode  = HI_ID_VDEC;
    stSrcInfo.hSrc       = pAvplay->hVdec;
    stSrcInfo.hSecondSrc = HI_INVALID_HANDLE;
    Ret = HI_DRV_WIN_SetSource(hWindow, &stSrcInfo);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(HI_DRV_WIN_SetSource, Ret);
        DRV_AVPLAY_UNLOCK(&(pAvplay->AvplayMutex));
        return HI_ERR_AVPLAY_INVALID_OPT;
    }
...
HI_S32 HI_DRV_AVPLAY_DetachWindow(HI_HANDLE hAvplay, HI_HANDLE hWindow)
{
    ...
    Ret = HI_DRV_WIN_SetSource(hWindow, &WinSrcInfo);
    if (HI_SUCCESS != Ret)
    {
        HI_ERR_PrintFuncResult(HI_DRV_WIN_SetSource, Ret);
        DRV_AVPLAY_UNLOCK(&(pAvplay->AvplayMutex));
